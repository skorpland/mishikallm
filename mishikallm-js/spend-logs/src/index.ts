...
import {MishikaLLM_SpendLogs, MishikaLLM_IncrementSpend, MishikaLLM_IncrementObject} from './_types'

const app = new Hono()
const prisma = new PrismaClient()
// In-memory storage for logs
let spend_logs: MishikaLLM_SpendLogs[] = [];
const key_logs: MishikaLLM_IncrementObject[] = [];
const user_logs: MishikaLLM_IncrementObject[] = [];
const transaction_logs: MishikaLLM_IncrementObject[] = [];
...
const flushLogsToDb = async () => {
  if (spend_logs.length >= MIN_LOGS) {
    ...
    for (let i = 0; i < logsToProcess.length; i += BATCH_SIZE) {
      ...
      await prisma.mishikaLLM_SpendLogs.createMany({
        data: batchWithDates,
      });
...
app.post('/spend/update', async (c) => {
  const incomingLogs = await c.req.json<MishikaLLM_SpendLogs[]>();
