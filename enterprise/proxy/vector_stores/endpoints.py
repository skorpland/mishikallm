...
async def new_vector_store(
    vector_store: MishikaLLM_ManagedVectorStore,
    ...
    try:
        ...
        existing_vector_store = (
            await prisma_client.db.mishikallm_managedvectorstorestable.find_unique(
        ...
        _new_vector_store = (
            await prisma_client.db.mishikallm_managedvectorstorestable.create(
                data=vector_store
            )
        )

        new_vector_store: MishikaLLM_ManagedVectorStore = MishikaLLM_ManagedVectorStore(
...
@router.get(
    ...
    tags=["vector store management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=MishikaLLM_ManagedVectorStoreListResponse,
)
async def list_vector_stores(
    ...
    try:
        ...
        in_memory_vector_stores: List[MishikaLLM_ManagedVectorStore] = []
        if mishikallm.vector_store_registry is not None:
            in_memory_vector_stores = copy.deepcopy(
                mishikallm.vector_store_registry.vector_stores
            )
        ...
        combined_vector_stores: List[MishikaLLM_ManagedVectorStore] = []
        ...
        # Format response using MishikaLLM_ManagedVectorStoreListResponse
...
async def delete_vector_store(
    ...
    try:
        ...
        # Delete vector store
        await prisma_client.db.mishikallm_managedvectorstorestable.delete(
            where={"vector_store_id": data.vector_store_id}
        )

        # Delete vector store from registry
        if mishikallm.vector_store_registry is not None:
            mishikallm.vector_store_registry.delete_vector_store_from_registry(
