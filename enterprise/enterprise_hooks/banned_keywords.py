...
class _ENTERPRISE_BannedKeywords(CustomLogger):
    ...
    def __init__(self):
        banned_keywords_list = mishikallm.banned_keywords_list
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "mishikallm.enterprise.enterprise_hooks.banned_keywords::async_pre_call_hook - Exception occurred - {}".format(
                    str(e)
                )
            )
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, mishikallm.ModelResponse) and isinstance(
            response.choices[0], mishikallm.utils.Choices
        ):
            for word in self.banned_keywords_list:
