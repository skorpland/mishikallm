...
import mishikallm
from mishikallm import create_batch, create_file
...
async def test_create_batch(provider):
    ...
    create_batch_response = await mishikallm.acreate_batch(
    ...
    print("response from mishikallm.create_batch=", create_batch_response)
    ...
    retrieved_batch = await mishikallm.aretrieve_batch(
    ...
    # list all batches
    list_batches = await mishikallm.alist_batches(custom_llm_provider=provider, limit=2)
    print("list_batches=", list_batches)

    file_content = await mishikallm.afile_content(
    ...
    # Cancel Batch
    cancel_batch_response = await mishikallm.acancel_batch(
...
def cleanup_azure_files():
    ...
    """
    azure_files = mishikallm.file_list(
    ...
    for _file in azure_files:
        ...
        delete_file_response = mishikallm.file_delete(
            file_id=_file.id,
            custom_llm_provider="azure",
            api_key=os.getenv("AZURE_FT_API_KEY"),
            api_base=os.getenv("AZURE_FT_API_BASE"),
...
async def test_async_create_batch(provider):
    ...
    mishikallm.callbacks = [custom_logger, "datadog"]
    ...
    create_batch_response = await mishikallm.acreate_batch(
        ...
        custom_llm_provider=provider,
        metadata={"key1": "value1", "key2": "value2"},
        # mishikallm specific param - used for logging metadata on logging callback
        mishikallm_metadata=extra_metadata_field,
    )
    ...
    # file obj
    file_obj = await mishikallm.afile_retrieve(
        file_id=batch_input_file_id, custom_llm_provider=provider
    )
    ...
    # delete file
    delete_file_response = await mishikallm.afile_delete(
        file_id=batch_input_file_id, custom_llm_provider=provider
    )
    ...
    all_files_list = await mishikallm.afile_list(
        custom_llm_provider=provider,
    )
...
mock_file_response = {
    "kind": "storage#object",
    "id": "mishikallm-local/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb/1739598666670574",
    "selfLink": "https://www.googleapis.com/storage/v1/b/mishikallm-local/o/mishikallm-vertex-files%2Fpublishers%2Fgoogle%2Fmodels%2Fgemini-1.5-flash-001%2F5f7b99ad-9203-4430-98bf-3b45451af4cb",
    "mediaLink": "https://storage.googleapis.com/download/storage/v1/b/mishikallm-local/o/mishikallm-vertex-files%2Fpublishers%2Fgoogle%2Fmodels%2Fgemini-1.5-flash-001%2F5f7b99ad-9203-4430-98bf-3b45451af4cb?generation=1739598666670574&alt=media",
    "name": "mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb",
    "bucket": "mishikallm-local",
...
mock_vertex_batch_response = {
    ...
    "displayName": "mishikallm_batch_job",
    "model": "projects/123456789/locations/us-central1/models/gemini-1.5-flash-001",
    "modelVersionId": "v1",
    "inputConfig": {
        "gcsSource": {
            "uris": [
                "gs://mishikallm-local/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb"
            ]
        }
    },
    "outputConfig": {
        "gcsDestination": {"outputUriPrefix": "gs://mishikallm-local/batch-outputs/"}
    },
    "dedicatedResources": {
        "machineSpec": {
...
async def test_avertex_batch_prediction(monkeypatch):
    monkeypatch.setenv("GCS_BUCKET_NAME", "mishikallm-local")
    ...
    with patch.object(
        ...
        with patch(
            "mishikallm.llms.custom_httpx.http_handler.AsyncHTTPHandler.get"
        ) as mock_get:
