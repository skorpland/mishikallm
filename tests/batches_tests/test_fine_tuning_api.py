...
from mishikallm import create_fine_tuning_job
from mishikallm._logging import verbose_logger
from mishikallm.llms.vertex_ai.fine_tuning.handler import (
    FineTuningJobCreate,
    VertexFineTuningAPI,
)
from mishikallm.types.llms.openai import Hyperparameters
...
async def test_create_fine_tune_jobs_async():
    try:
        ...
        mishikallm.callbacks = ["datadog", custom_logger]
        ...
        create_fine_tuning_response = await mishikallm.acreate_fine_tuning_job(
            model="gpt-3.5-turbo-0125",
            training_file=file_obj.id,
        )

        print(
            "response from mishikallm.create_fine_tuning_job=", create_fine_tuning_response
        )
        ...
        ft_jobs = await mishikallm.alist_fine_tuning_jobs(limit=2)
        print("response from mishikallm.list_fine_tuning_jobs=", ft_jobs)
        ...
        print("response from mishikallm.retrieve_fine_tuning_job=", response)

        # delete file

        await mishikallm.afile_delete(
            file_id=file_obj.id,
        )
        ...
        print("response from mishikallm.cancel_fine_tuning_job=", response)
...
async def test_azure_create_fine_tune_jobs_async():
    try:
        ...
        # list fine tuning jobs
        print("listing ft jobs")
        ft_jobs = await mishikallm.alist_fine_tuning_jobs(
            limit=2,
            custom_llm_provider="azure",
            api_base="https://exampleopenaiendpoint-production.up.railway.app",
        )
...
async def test_mock_openai_create_fine_tune_job():
    ...
    with patch.object(client.fine_tuning.jobs, "create") as mock_create:
        ...
        response = await mishikallm.acreate_fine_tuning_job(
            model="gpt-3.5-turbo-0125",
            training_file="file-123",
...
async def test_mock_openai_list_fine_tune_jobs():
    ...
    with patch.object(
        ...
        await mishikallm.alist_fine_tuning_jobs(limit=2, after="ft-000", client=client)
...
async def test_mock_openai_cancel_fine_tune_job():
    ...
    with patch.object(client.fine_tuning.jobs, "cancel") as mock_cancel:
        await mishikallm.acancel_fine_tuning_job(
