...
import pytest
import mishikallm
from mishikallm.integrations.opentelemetry import OpenTelemetry, OpenTelemetryConfig, Span
import asyncio
import logging
...
EXPECTED_SPAN_NAMES = ["mishikallm_request", "raw_gen_ai_request"]
...
async def test_async_otel_callback(streaming):
    ...
    mishikallm.callbacks = [OpenTelemetry(config=OpenTelemetryConfig(exporter=exporter))]
    ...
    for span in spans:
        ...
        if span.name == "mishikallm_request":
            validate_mishikallm_request(span)
...
def validate_mishikallm_request(span):
    expected_attributes = [
...
def test_completion_claude_3_function_call_with_otel(model):
    ...
    try:
        ...
        print("response from MishikaLLM", response)
...
async def test_awesome_otel_with_message_logging_off(streaming, global_redact):
    ...
    tests when mishikallm.turn_off_message_logging is set to True
    ...
    mishikallm.callbacks = [otel_logger]
