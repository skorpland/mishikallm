...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Test with mishikallm.Usage object
        (
...
def test_get_model_cost_information():
    ...
    mishikallm_info_gpt_3_5_turbo_model_map_value = mishikallm.get_model_info(
    ...
    assert result["model_map_value"] == mishikallm_info_gpt_3_5_turbo_model_map_value
    # assert all fields in StandardLoggingModelInformation are present
    assert all(
        field in result for field in StandardLoggingModelInformation.__annotations__
    )
...
def test_get_final_response_obj():
    ...
    # Test redaction when mishikallm.turn_off_message_logging is True
    mishikallm.turn_off_message_logging = True
    try:
        model_response = mishikallm.ModelResponse(
            choices=[
                mishikallm.Choices(message=mishikallm.Message(content="sensitive content"))
            ]
        )
        ...
        assert result["choices"][0]["message"]["content"] == "redacted-by-mishikallm"
        # Verify that redaction occurred in kwargs
        assert kwargs["messages"][0]["content"] == "redacted-by-mishikallm"
    finally:
        # Reset mishikallm.turn_off_message_logging to its original value
...
def test_get_standard_logging_payload_trace_id():
    ...
    # Test case 1: When mishikallm_trace_id is provided in mishikallm_params
    from unittest.mock import MagicMock
    
    # Create a mock Logging object
    mock_logging_obj = MagicMock()
    mock_logging_obj.mishikallm_trace_id = "default-trace-id"
    
    # Test when mishikallm_trace_id is in mishikallm_params
    mishikallm_params = {"mishikallm_trace_id": "dynamic-trace-id"}
    ...
    # Test case 2: When mishikallm_trace_id is not provided in mishikallm_params
    mishikallm_params = {}
    result = StandardLoggingPayloadSetup._get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj,
        mishikallm_params=mishikallm_params
    ...
    # Test case 3: When mishikallm_params is None
    result = StandardLoggingPayloadSetup._get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj,
        mishikallm_params={}
    )
    ...
    # Test case 4: When mishikallm_trace_id in params is not a string
    mishikallm_params = {"mishikallm_trace_id": 12345}
    result = StandardLoggingPayloadSetup._get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj,
        mishikallm_params=mishikallm_params
    )
...
def test_get_error_information():
    ...
    # Test with mishikallm exception from provider
    mishikallm_exception = mishikallm.exceptions.RateLimitError(
        message="Test error",
        llm_provider="openai",
        model="gpt-3.5-turbo",
        response=None,
        mishikallm_debug_info=None,
        max_retries=None,
        num_retries=None,
    )
    result = StandardLoggingPayloadSetup.get_error_information(mishikallm_exception)
    print("error_information", json.dumps(result, indent=2))
    assert result["error_code"] == "429"
    assert result["error_class"] == "RateLimitError"
    assert result["llm_provider"] == "openai"
    assert result["error_message"] == "mishikallm.RateLimitError: Test error"
