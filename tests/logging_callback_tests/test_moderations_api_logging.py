...
async def test_moderations_api_logging(model):
    ...
    response = await mishikallm.amoderation(
        input=input_content,
        model=model,
    )
    ...
    assert standard_logging_payload["call_type"] == mishikallm.utils.CallTypes.amoderation.value
    assert standard_logging_payload["status"] == "success"
    assert standard_logging_payload["custom_llm_provider"] == mishikallm.LlmProviders.OPENAI.value
