...
from mishikallm.integrations.SlackAlerting.slack_alerting import (
    DeploymentMetrics,
    SlackAlerting,
)
from mishikallm.proxy._types import CallInfo, Mishikallm_EntityType, WebhookEvent
from mishikallm.proxy.utils import ProxyLogging
from mishikallm.router import AlertingConfig, Router
from mishikallm.utils import get_api_base
...
async def test_get_api_base():
    ...
    mishikallm_params = {
        ...
        "mishikallm_call_id": "68f46d2d-714d-4ad8-8137-69600ec8755c",
        "model_alias_map": {},
...
async def test_response_taking_too_long_callback(slack_alerting):
    ...
    kwargs = {"model": "test_model", "messages": "test_messages", "mishikallm_params": {}}
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
...
async def test_alerting_metadata(slack_alerting):
    ...
    kwargs = {
        "model": "test_model",
        "messages": "test_messages",
        "mishikallm_params": {"metadata": {"alerting_metadata": {"hello": "world"}}},
    }
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
...
async def test_daily_reports_completion(slack_alerting):
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
        mishikallm.callbacks = [slack_alerting]
        ...
        router = mishikallm.Router(
            model_list=[
                {
                    "model_name": "gpt-5",
                    "mishikallm_params": {"model": "gpt-3.5-turbo", "api_key": "bad_key"},
                }
            ]
        )
...
async def test_send_daily_reports_ignores_zero_values():
    ...
    router.get_model_info.side_effect = lambda x: {"mishikallm_params": {"model": x}}
...
async def test_outage_alerting_called(
    ...
    if error_code == 400:
        ...
        error_to_raise = mishikallm.BadRequestError(
...
async def test_langfuse_trace_id():
    ...
    trace_url = await _add_langfuse_trace_id_to_alert(
        request_data={"mishikallm_logging_obj": mishikallm_logging_obj}
    )
...
async def test_print_alerting_payload_warning():
    ...
    from mishikallm.integrations.SlackAlerting.batching_handler import send_to_webhook
    import logging
...
async def test_spend_report_cache(report_type):
    ...
    with patch("mishikallm.proxy.proxy_server.prisma_client") as mock_prisma:
