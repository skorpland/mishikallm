...
from mishikallm.integrations.lago import LagoLogger
from mishikallm.integrations.openmeter import OpenMeterLogger
from mishikallm.integrations.braintrust_logging import BraintrustLogger
from mishikallm.integrations.pagerduty.pagerduty import PagerDutyAlerting
from mishikallm.integrations.galileo import GalileoObserve
from mishikallm.integrations.langsmith import LangsmithLogger
...
from mishikallm.integrations.datadog.datadog_llm_obs import DataDogLLMObsLogger
from mishikallm.integrations.gcs_bucket.gcs_bucket import GCSBucketLogger
from mishikallm.integrations.gcs_pubsub.pub_sub import GcsPubSubLogger
from mishikallm.integrations.opik.opik import OpikLogger
from mishikallm.integrations.opentelemetry import OpenTelemetry
from mishikallm.integrations.mlflow import MlflowLogger
from mishikallm.integrations.argilla import ArgillaLogger
...
from mishikallm.integrations.humanloop import HumanloopLogger
from mishikallm.proxy.hooks.dynamic_rate_limiter import _PROXY_DynamicRateLimitHandler
...
async def use_callback_in_llm_call(
    ...
    if used_in == "callbacks":
        ...
        mishikallm.success_callback = [callback]
    ...
    for _ in range(5):
        ...
        if used_in == "callbacks":
            assert isinstance(mishikallm._async_success_callback[0], expected_class)
            assert isinstance(mishikallm._async_failure_callback[0], expected_class)
            assert isinstance(mishikallm.success_callback[0], expected_class)
            assert isinstance(mishikallm.failure_callback[0], expected_class)

            assert (
                len(mishikallm._async_success_callback) == 1
            ), f"Got={mishikallm._async_success_callback}"
            assert len(mishikallm._async_failure_callback) == 1
            assert len(mishikallm.success_callback) == 1
            assert len(mishikallm.failure_callback) == 1
            assert len(mishikallm.callbacks) == 1
        elif used_in == "success_callback":
            print(f"mishikallm.success_callback: {mishikallm.success_callback}")
            print(f"mishikallm._async_success_callback: {mishikallm._async_success_callback}")
            assert isinstance(mishikallm.success_callback[0], expected_class)
            assert len(mishikallm.success_callback) == 1  # ["lago", LagoLogger]
            assert isinstance(mishikallm._async_success_callback[0], expected_class)
            assert len(mishikallm._async_success_callback) == 1
...
async def test_init_custom_logger_compatible_class_as_callback():
    ...
    # used like mishikallm.callbacks = ["prometheus"]
    for callback in mishikallm._known_custom_logger_compatible_callbacks:
        print(f"Testing callback: {callback}")
        reset_all_callbacks()

        await use_callback_in_llm_call(callback, used_in="callbacks")

    # used like this mishikallm.success_callback = ["prometheus"]
    for callback in mishikallm._known_custom_logger_compatible_callbacks:
        print(f"Testing callback: {callback}")
        reset_all_callbacks()
...
def test_dynamic_logging_global_callback():
    ...
    from mishikallm.types.utils import ModelResponse, Choices, Message, Usage

    cl = CustomLogger()

    mishikallm_logging = MishikaLLMLoggingObj(
        model="claude-3-opus-20240229",
        messages=[{"role": "user", "content": "hi"}],
        stream=False,
        call_type="completion",
        start_time=datetime.now(),
        mishikallm_call_id="123",
        function_id="456",
        kwargs={
    ...
    with patch.object(cl, "log_success_event") as mock_log_success_event:
        ...
        mishikallm.success_callback = [cl]

        try:
            mishikallm_logging.success_handler(
