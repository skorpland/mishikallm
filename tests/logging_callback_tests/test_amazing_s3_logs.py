...
def test_s3_logging():
    ...
    # on circle ci - we only test mishikallm.acompletion()
    try:
        # redirect stdout to log_file
        mishikallm.cache = mishikallm.Cache(
            type="s3",
        ...
        mishikallm.success_callback = ["s3"]
        mishikallm.s3_callback_params = {
            "s3_bucket_name": "mishikallm-logs-2",
            "s3_aws_secret_access_key": "os.environ/AWS_SECRET_ACCESS_KEY",
            "s3_aws_access_key_id": "os.environ/AWS_ACCESS_KEY_ID",
        }
        mishikallm.set_verbose = True
        ...
        async def _test():
            return await mishikallm.acompletion(
                model="gpt-3.5-turbo",
...
def test_s3_logging_r2():
    ...
    try:
        # redirect stdout to log_file
        # mishikallm.cache = mishikallm.Cache(
        #     type="s3", s3_bucket_name="mishikallm-r2-bucket", s3_region_name="us-west-2"
        # )
        mishikallm.set_verbose = True
        from mishikallm._logging import verbose_logger
        import logging
        ...
        mishikallm.success_callback = ["s3"]
        mishikallm.s3_callback_params = {
            "s3_bucket_name": "mishikallm-r2-bucket",
        ...
        bucket_name = "mishikallm-r2-bucket"
        # List objects in the bucket
        response = s3.list_objects(Bucket=bucket_name)
