...
from mishikallm.integrations.langsmith import (
    LangsmithLogger,
...
async def test_is_serializable():
    from mishikallm.integrations.langsmith import is_serializable
...
async def test_langsmith_key_based_logging(mocker):
    """
    In key based logging langsmith_api_key and langsmith_project are passed directly to mishikallm.acompletion
    """
    try:
        ...
        mishikallm.DEFAULT_FLUSH_INTERVAL_SECONDS = 1

        mishikallm.callbacks = [LangsmithLogger()]
...
async def test_langsmith_queue_logging():
    try:
        ...
        mishikallm.callbacks = [test_langsmith_logger]
