...
async def test_create_datadog_logging_payload():
    ...
    assert dd_payload["ddsource"] == os.getenv("DD_SOURCE", "mishikallm")
    assert dd_payload["service"] == "mishikallm-server"
...
async def test_datadog_logging_http_request():
    ...
    try:
        ...
        mishikallm.callbacks = [dd_logger]
...
async def test_datadog_log_redis_failures():
    ...
    try:
        ...
        mishikallm.callbacks = [dd_logger]
        mishikallm.service_callback = ["datadog"]
...
async def test_datadog_logging():
    try:
        mishikallm.success_callback = ["datadog"]
        mishikallm.set_verbose = True
        response = await mishikallm.acompletion(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "what llm are u"}],
...
def test_datadog_static_methods():
    ...
    assert DataDogLogger._get_datadog_source() == "mishikallm"
    assert DataDogLogger._get_datadog_service() == "mishikallm-server"
    ...
    assert (
        "env:unknown,service:mishikallm,version:unknown,HOSTNAME:"
