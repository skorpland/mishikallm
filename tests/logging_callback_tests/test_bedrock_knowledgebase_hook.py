...
from mishikallm.types.utils import StandardLoggingPayload, StandardLoggingVectorStoreRequest
from mishikallm.types.vector_stores import VectorStoreSearchResponse
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_completion(setup_vector_store_registry):
    ...
    print("value of mishikallm.vector_store_registry:", mishikallm.vector_store_registry)
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_llm_api_call(setup_vector_store_registry):
    ...
    mishikallm.callbacks = [BedrockVectorStore()]
    response = await mishikallm.acompletion(
...
async def test_logging_with_knowledge_base_hook(setup_vector_store_registry):
    ...
    mishikallm.callbacks = [BedrockVectorStore(), test_custom_logger]
    mishikallm.set_verbose = True
    await mishikallm.acompletion(
        model="gpt-4",
        messages=[{"role": "user", "content": "what is mishikallm?"}],
    ...
    assert vector_store_request_metadata.get("query") == "what is mishikallm?"
    ...
    assert vector_store_search_response.get("search_query") == "what is mishikallm?"
...
async def test_logging_with_knowledge_base_hook_no_vector_store_registry(setup_vector_store_registry):
    ...
    test_custom_logger = TestCustomLogger()
    mishikallm.callbacks = [BedrockVectorStore(), test_custom_logger]
    mishikallm.vector_store_registry = None
    await mishikallm.acompletion(
        model="gpt-4",
        messages=[{"role": "user", "content": "what is mishikallm?"}],
    )
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_vector_store_not_in_registry(setup_vector_store_registry):
    ...
    print("Registry iniitalized:", mishikallm.vector_store_registry.vector_stores)
