...
from mishikallm.proxy.auth.auth_checks import get_user_object
...
async def test_create_new_user_in_organization(prisma_client, user_role):
    ...
    setattr(mishikallm.proxy.proxy_server, "llm_router", MagicMock())
    ...
    if user_role != None:
        ...
        assert _membership.user_role == MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY
...
async def test_org_admin_create_team_permissions(prisma_client):
    ...
    response = await organization_member_add(
        data=OrganizationMemberAddRequest(
            organization_id=org_id,
            member=OrgMember(role=MishikallmUserRoles.ORG_ADMIN, user_id=created_user_id),
        ),
        http_request=None,
    )
...
async def test_org_admin_create_user_permissions(prisma_client):
    ...
    response = await organization_member_add(
        data=OrganizationMemberAddRequest(
            organization_id=org_id,
            member=OrgMember(
                role=MishikallmUserRoles.INTERNAL_USER, user_id=new_internal_user_for_org
            ),
        ),
        http_request=request,
    )
...
async def test_org_admin_create_user_team_wrong_org_permissions(prisma_client):
    ...
    _new_key = await generate_key_fn(
        ...
        user_api_key_dict=UserAPIKeyAuth(
            user_role=MishikallmUserRoles.ORG_ADMIN,
            user_id=created_user_id,
        ),
    )
...
@pytest.mark.parametrize(
    ...
    [
        # Proxy Admin checks
        ("/global/spend/logs", MishikallmUserRoles.PROXY_ADMIN, True),
        ("/key/delete", MishikallmUserRoles.PROXY_ADMIN, True),
        ("/key/generate", MishikallmUserRoles.PROXY_ADMIN, True),
        ("/key/regenerate", MishikallmUserRoles.PROXY_ADMIN, True),
        # # Internal User checks - allowed routes
        ("/global/spend/logs", MishikallmUserRoles.INTERNAL_USER, True),
        ("/key/delete", MishikallmUserRoles.INTERNAL_USER, True),
        ("/key/generate", MishikallmUserRoles.INTERNAL_USER, True),
        ("/key/82akk800000000jjsk/regenerate", MishikallmUserRoles.INTERNAL_USER, True),
        # Internal User Viewer
        ("/key/generate", MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY, False),
        (
            "/key/82akk800000000jjsk/regenerate",
            MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY,
            False,
        ),
        ("/key/delete", MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY, False),
        ("/team/new", MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY, False),
        ("/team/delete", MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY, False),
        ("/team/update", MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY, False),
        # Proxy Admin Viewer
        ("/global/spend/logs", MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY, True),
        ("/key/delete", MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY, False),
        ("/key/generate", MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY, False),
        (
            "/key/82akk800000000jjsk/regenerate",
            MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY,
            False,
        ),
        ("/team/new", MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY, False),
        ("/team/delete", MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY, False),
        ("/team/update", MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY, False),
        # Internal User checks - disallowed routes
        ("/organization/member_add", MishikallmUserRoles.INTERNAL_USER, False),
    ],
)
async def test_user_role_permissions(prisma_client, route, user_role, expected_result):
    ...
    try:
        ...
        # Generate key for new user with team_id="mishikallm-dashboard"
        key_response = await generate_key_fn(
            data=GenerateKeyRequest(user_id=user_id, team_id="mishikallm-dashboard"),
