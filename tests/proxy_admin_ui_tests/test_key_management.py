...
async def test_get_users(prisma_client):
    ...
    test_users = [
        NewUserRequest(
            ...
            user_role=(
                MishikallmUserRoles.INTERNAL_USER.value
                if i % 2 == 0
                else MishikallmUserRoles.PROXY_ADMIN.value
            ),
        )
        for i in range(5)
    ]
    ...
    for user in result["users"]:
        assert isinstance(user, MishikaLLM_UserTable)

    # Clean up test users
    for user in test_users:
        await prisma_client.db.mishikallm_usertable.delete(where={"user_id": user.user_id})


@pytest.mark.asyncio
async def test_get_users_filters_dashboard_keys(prisma_client):
    """
    Tests that /users/list endpoint doesn't return keys with team_id='mishikallm-dashboard'
    ...
    test_user = NewUserRequest(
        user_id=new_user_id,
        user_role=MishikallmUserRoles.INTERNAL_USER.value,
    ...
    # Create two keys for the user - one with team_id="mishikallm-dashboard" and one without
    regular_key = await generate_key_helper_fn(
        user_id=test_user.user_id,
        request_type="key",
        team_id="mishikallm-dashboard",  # This key should be included in the response
        models=[],
        aliases={},
        config={},
        spend=0,
    ...
    await prisma_client.db.mishikallm_usertable.delete(
...
async def cleanup_existing_teams(prisma_client):
    all_teams = await prisma_client.db.mishikallm_teamtable.find_many()
...
async def test_list_teams(prisma_client):
    ...
    test_team = await new_team(
        ...
        user_api_key_dict=UserAPIKeyAuth(
            user_role=MishikallmUserRoles.PROXY_ADMIN, api_key="sk-1234", user_id="admin"
        ),
    )
...
def test_is_team_key():
    from mishikallm.proxy.management_endpoints.key_management_endpoints import _is_team_key
...
def test_team_key_generation_team_member_check():
    ...
    mishikallm.key_generation_settings = {
...
def test_key_generation_required_params_check(
    ...
    if key_type == "team_key":
        mishikallm.key_generation_settings = StandardKeyGenerationConfig(
...
async def test_list_key_helper_team_filtering(prisma_client):
    ...
    1. Create keys with different team_ids (None, mishikallm-dashboard, other)
    2. Verify filtering excludes mishikallm-dashboard keys
    ...
    # Create 2 keys with team_id=mishikallm-dashboard
    for i in range(2):
        key = await generate_key_fn(
            data=GenerateKeyRequest(
    ...
    try:
        # Test 1: Get all keys with pagination (exclude mishikallm-dashboard)
        all_keys = []
        page = 1
        ...
        # Verify no mishikallm-dashboard keys are present
        dashboard_keys = [k for k in all_keys if k.team_id == "mishikallm-dashboard"]
        assert len(dashboard_keys) == 0, "Should not include mishikallm-dashboard keys"
...
@pytest.mark.asyncio
@patch("mishikallm.proxy.management_endpoints.key_management_endpoints.get_team_object")
async def test_key_generate_always_db_team(mock_get_team_object):
    from mishikallm.proxy.management_endpoints.key_management_endpoints import (
        generate_key_fn,
    )

    setattr(mishikallm.proxy.proxy_server, "prisma_client", MagicMock())
