...
from mishikallm.proxy.management_endpoints.ui_sso import auth_callback
...
def mock_env_vars(monkeypatch):
    ...
    monkeypatch.setenv("MISHIKALLM_MASTER_KEY", "mock_master_key")
...
async def test_auth_callback_new_user(mock_google_sso, mock_env_vars, prisma_client):
    ...
    try:
        ...
        mishikallm.proxy.proxy_server.master_key = "mock_master_key"
        ...
        user = await prisma_client.db.mishikallm_usertable.find_first(
            where={"user_id": unique_user_id}
        )
        print("inserted user from SSO", user)
        assert user is not None
        assert user.user_email == unique_user_email
        assert user.user_role == MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY
...
async def test_auth_callback_new_user_with_sso_default(
    ...
    """
    When mishikallm_settings.default_internal_user_params.user_role = 'INTERNAL_USER'
    ...
    try:
        ...
        assert user is not None
        assert user.user_email == unique_user_email
        assert user.user_role == MishikallmUserRoles.INTERNAL_USER
        ...
        mishikallm.default_internal_user_params = None
