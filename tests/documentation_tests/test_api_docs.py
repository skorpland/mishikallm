...
def analyze_function(func_info: FunctionInfo) -> Dict:
    ...
    for name, type_name in func_info.parameters:
        if type_name.endswith("Request") or type_name.endswith("Response"):
            pydantic_model = getattr(mishikallm.proxy._types, type_name, None)
...
def main():
    ...
    # directory = "../../mishikallm/proxy/management_endpoints"  # LOCAL
    directory = "./mishikallm/proxy/management_endpoints"
