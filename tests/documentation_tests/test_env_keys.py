...
# Define the base directory for the mishikallm repository and documentation path
repo_base = "./mishikallm"  # Change this to your actual path

# Regular expressions to capture the keys used in os.getenv() and mishikallm.get_secret()
getenv_pattern = re.compile(r'os\.getenv\(\s*[\'"]([^\'"]+)[\'"]\s*(?:,\s*[^)]*)?\)')
get_secret_pattern = re.compile(
    r'mishikallm\.get_secret\(\s*[\'"]([^\'"]+)[\'"]\s*(?:,\s*[^)]*|,\s*default_value=[^)]*)?\)'
)
get_secret_str_pattern = re.compile(
    r'mishikallm\.get_secret_str\(\s*[\'"]([^\'"]+)[\'"]\s*(?:,\s*[^)]*|,\s*default_value=[^)]*)?\)'
)

# Set to store unique keys from the code
env_keys = set()

# Walk through all files in the mishikallm repo to find references of os.getenv() and mishikallm.get_secret()
