...
def test_aaaasschema_migration_check(schema_setup, monkeypatch):
    ...
    deploy_dir = Path("./mishikallm-proxy-extras/mishikallm_proxy_extras")
    ...
    try:
        ...
        if not temp_migrations_dir.exists() or not any(temp_migrations_dir.iterdir()):
            ...
            pytest.fail(
                "No existing migrations found - first migration needed. Run `mishikallm/ci_cd/baseline_db.py` to create new migration -E.g. `python mishikallm/ci_cd/baseline_db_migration.py`."
            )
        ...
        if diff_result.returncode == 2:
            ...
            pytest.fail(
                "Schema changes detected - new migration required. Run `mishikallm/ci_cd/run_migration.py` to create new migration -E.g. `python mishikallm/ci_cd/run_migration.py <migration_name>`."
            )
        else:
