...
from mishikallm.proxy.utils import CallInfo
...
async def test_get_end_user_object(customer_spend, customer_budget):
    ...
    _budget = MishikaLLM_BudgetTable(max_budget=customer_budget)
    end_user_obj = MishikaLLM_EndUserTable(
        user_id=end_user_id,
        spend=customer_spend,
        mishikallm_budget_table=_budget,
        blocked=False,
    )
    _cache = DualCache()
    ...
    try:
        ...
        if (
            isinstance(e, mishikallm.BudgetExceededError)
...
async def test_can_key_call_model(model, expect_to_work):
    ...
    """
    from mishikallm.proxy.auth.auth_checks import can_key_call_model
    ...
    router = mishikallm.Router(model_list=llm_model_list)
    args = {
        "model": model,
...
async def test_can_team_call_model(model, expect_to_work):
    from mishikallm.proxy.auth.auth_checks import model_in_access_group
    from fastapi import HTTPException
...
async def test_is_valid_fallback_model():
    from mishikallm.proxy.auth.auth_checks import is_valid_fallback_model
...
async def test_virtual_key_max_budget_check(
    ...
    """
    from mishikallm.proxy.auth.auth_checks import _virtual_key_max_budget_check
    from mishikallm.proxy.utils import ProxyLogging
    ...
    except mishikallm.BudgetExceededError as e:
...
async def test_can_user_call_model():
        ...
        model_list=[
            {
                ...
                "mishikallm_params": {"model": "anthropic/anthropic-claude"},
            },
            {
                "model_name": "gpt-3.5-turbo",
                "mishikallm_params": {"model": "gpt-3.5-turbo", "api_key": "test-api-key"},
            },
        ]
    )
    ...
    args = {
        ...
        "llm_router": router,
        "user_object": MishikaLLM_UserTable(
...
async def test_can_user_call_model_with_no_default_models():
    from mishikallm.proxy.auth.auth_checks import can_user_call_model
    from mishikallm.proxy._types import ProxyException, SpecialModelNames
...
async def test_get_fuzzy_user_object():
    from mishikallm.proxy.auth.auth_checks import _get_fuzzy_user_object
    from mishikallm.proxy.utils import PrismaClient
    ...
    mock_prisma.db.mishikallm_usertable = MagicMock()

    # Mock user data
    test_user = MishikaLLM_UserTable(
        user_id="test_123",
        sso_user_id="sso_123",
    ...
    mock_prisma.db.mishikallm_usertable.find_unique = AsyncMock(return_value=test_user)
    result = await _get_fuzzy_user_object(
        prisma_client=mock_prisma, sso_user_id="sso_123", user_email="test@example.com"
    )
    assert result == test_user
    mock_prisma.db.mishikallm_usertable.find_unique.assert_called_with(
        where={"sso_user_id": "sso_123"}, include={"organization_memberships": True}
    )

    # Test 2: SSO ID not found, find by email
    mock_prisma.db.mishikallm_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma.db.mishikallm_usertable.find_first = AsyncMock(return_value=test_user)
    mock_prisma.db.mishikallm_usertable.update = AsyncMock()
    ...
    assert result == test_user
    mock_prisma.db.mishikallm_usertable.find_first.assert_called_with(
    ...
    mock_prisma.db.mishikallm_usertable.update.assert_called_with(
        where={"user_id": "test_123"}, data={"sso_user_id": "new_sso_456"}
    )

    # Test 4: User not found by either method
    mock_prisma.db.mishikallm_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma.db.mishikallm_usertable.find_first = AsyncMock(return_value=None)
