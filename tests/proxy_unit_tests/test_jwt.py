...
def test_load_config_with_custom_role_names():
    config = {
        "general_settings": {
            "mishikallm_proxy_roles": {"admin_jwt_scope": "mishikallm-proxy-admin"}
        }
    }
    proxy_roles = MishikaLLM_JWTAuth(
        **config.get("general_settings", {}).get("mishikallm_proxy_roles", {})
    )

    print(f"proxy_roles: {proxy_roles}")

    assert proxy_roles.admin_jwt_scope == "mishikallm-proxy-admin"
...
async def test_token_single_public_key(monkeypatch):
    ...
    await cache.async_set_cache(
        key="mishikallm_jwt_auth_keys_https://example.com/public-key",
...
@pytest.mark.parametrize("audience", [None, "mishikallm-proxy"])
@pytest.mark.asyncio
async def test_valid_invalid_token(audience, monkeypatch):
    ...
    await cache.async_set_cache(
        key="mishikallm_jwt_auth_keys_https://example.com/public-key", value=[public_jwk]
    )
...
def team_token_tuple():
    ...
    import mishikallm
    from mishikallm.proxy._types import NewTeamRequest, UserAPIKeyAuth
...
async def test_team_token_output(prisma_client, audience, monkeypatch):
    ...
    jwt_handler.mishikallm_jwtauth = MishikaLLM_JWTAuth(team_id_jwt_field="client_id")
    ...
    setattr(mishikallm.proxy.proxy_server, "jwt_handler", jwt_handler)
    try:
...
async def aaaatest_user_token_output(
    ...
    import mishikallm
    from mishikallm.proxy._types import NewTeamRequest, NewUserRequest, UserAPIKeyAuth
    ...
    jwt_handler.mishikallm_jwtauth = MishikaLLM_JWTAuth()

    jwt_handler.mishikallm_jwtauth.user_id_jwt_field = "sub"
    jwt_handler.mishikallm_jwtauth.team_id_default = default_team_id
    jwt_handler.mishikallm_jwtauth.user_id_upsert = user_id_upsert

    if team_id_set:
        jwt_handler.mishikallm_jwtauth.team_id_jwt_field = "client_id"
    ...
    ## 1. INITIAL TEAM CALL - should fail
    # use generated key to auth in
    setattr(mishikallm.proxy.proxy_server, "general_settings", {"enable_jwt_auth": True})
    setattr(mishikallm.proxy.proxy_server, "jwt_handler", jwt_handler)
    try:
        result = await user_api_key_auth(request=request, api_key=bearer_token)
        pytest.fail("Team doesn't exist. This should fail")
...
async def test_allowed_routes_admin(
    ...
    pseudo_routes = jwt_handler.mishikallm_jwtauth.admin_allowed_routes

    actual_routes = []
    for route in pseudo_routes:
        if route in MishikaLLMRoutes.__members__:
            actual_routes.extend(MishikaLLMRoutes[route].value)
...
async def test_team_cache_update_called():
    ...
    with patch.object(cache, "async_get_cache", new=AsyncMock()) as mock_call_cache:
        ...
        await mishikallm.proxy.proxy_server.update_cache(
            token=None,
            user_id=None,
            end_user_id=None,
            team_id="1234",
...
async def test_allow_access_by_email(
    ...
    setattr(mishikallm.proxy.proxy_server, "jwt_handler", jwt_handler)
    setattr(mishikallm.proxy.proxy_server, "prisma_client", {})
    ...
    # ),
    with patch.object(
        mishikallm.proxy.auth.handle_jwt,
        "get_user_object",
...
async def test_end_user_jwt_auth(monkeypatch):
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", {})
    setattr(mishikallm.proxy.proxy_server, "jwt_handler", jwt_handler)
    from mishikallm.proxy.proxy_server import cost_tracking
    ...
    with patch.object(
        mishikallm.proxy.hooks.proxy_track_cost_callback, "_should_track_cost_callback"
    ) as mock_client:
...
def test_can_rbac_role_call_route():
    ...
    with pytest.raises(HTTPException):
        JWTAuthManager.can_rbac_role_call_route(
            rbac_role=MishikallmUserRoles.TEAM,
            general_settings={
                "role_permissions": [
                    RoleBasedPermissions(
                        role=MishikallmUserRoles.TEAM, routes=["/v1/chat/completions"]
                    )
                ]
            },
            route="/v1/embeddings",
...
def test_check_scope_based_access(requested_model, should_work):
    from mishikallm.proxy.auth.handle_jwt import JWTAuthManager
    from mishikallm.proxy._types import ScopeMapping
    ...
    args = {
        "scope_mappings": [
            ScopeMapping(
                ...
                scope="mishikallm.api.consumer",
            ),
            ScopeMapping(
                models=["gpt-3.5-turbo-testing"],
                routes=None,
                scope="mishikallm.api.gpt_3_5_turbo",
            ),
        ],
        "scopes": [
            "profile",
            "groups-scope",
            "email",
            "mishikallm.api.gpt_3_5_turbo",
            "mishikallm.api.consumer",
        ],
        "request_data": {
...
async def test_custom_validate_called():
    ...
    jwt_handler.mishikallm_jwtauth = MagicMock(
