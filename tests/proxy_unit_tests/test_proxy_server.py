...
import mishikallm.proxy
import mishikallm.proxy.proxy_server
...
import mishikallm
from mishikallm import RateLimitError, Timeout, completion, completion_cost, embedding
...
from mishikallm.proxy.proxy_server import (  # Replace with the actual module where your FastAPI router is defined
    app,
    initialize,
    save_worker_config,
)
from mishikallm.proxy.utils import ProxyLogging
...
def mock_patch_acompletion():
    return mock.patch(
        "mishikallm.proxy.proxy_server.llm_router.acompletion",
...
def mock_patch_aembedding():
    return mock.patch(
        "mishikallm.proxy.proxy_server.llm_router.aembedding",
...
def mock_patch_aimage_generation():
    return mock.patch(
        "mishikallm.proxy.proxy_server.llm_router.aimage_generation",
...
def client_no_auth(fake_env_vars):
    # Assuming mishikallm.proxy.proxy_server is an object
    from mishikallm.proxy.proxy_server import cleanup_router_config_variables
...
def test_chat_completion(mock_acompletion, client_no_auth):
    ...
    try:
        ...
        mock_acompletion.assert_called_once_with(
            ...
            max_tokens=10,
            mishikallm_call_id=mock.ANY,
            mishikallm_logging_obj=mock.ANY,
...
def test_get_settings_request_timeout(client_no_auth):
    """
    When no timeout is set, it should use the mishikallm.request_timeout value
    """
    # Set a known value for mishikallm.request_timeout
    import mishikallm
    ...
    assert settings["mishikallm.request_timeout"] == mishikallm.request_timeout


@pytest.mark.parametrize(
    "mishikallm_key_header_name",
    ["x-mishikallm-key", None],
)
def test_add_headers_to_request(mishikallm_key_header_name):
    from fastapi import Request
    from starlette.datastructures import URL
    import json
    from mishikallm.proxy.mishikallm_pre_call_utils import (
        clean_headers,
        MishikaLLMProxyRequestSetup,
    )
    ...
    request_headers = clean_headers(headers, mishikallm_key_header_name)
    forwarded_headers = MishikaLLMProxyRequestSetup._get_forwardable_headers(
        request_headers
    )
...
def test_chat_completion_forward_headers(
    mock_acompletion, client_no_auth, mishikallm_key_header_name, forward_headers
):
    global headers
    try:
        if forward_headers:
            gs = getattr(mishikallm.proxy.proxy_server, "general_settings")
            gs["forward_client_headers_to_llm_api"] = True
            setattr(mishikallm.proxy.proxy_server, "general_settings", gs)
        if mishikallm_key_header_name is not None:
            gs = getattr(mishikallm.proxy.proxy_server, "general_settings")
            gs["mishikallm_key_header_name"] = mishikallm_key_header_name
            setattr(mishikallm.proxy.proxy_server, "general_settings", gs)
        # Your test data
        test_data = {
            "model": "gpt-3.5-turbo",
        ...
        if mishikallm_key_header_name is not None:
            headers_to_not_forward = {mishikallm_key_header_name: "Bearer 1234"}
        else:
...
async def test_team_disable_guardrails(mock_acompletion, client_no_auth):
    ...
    from mishikallm.proxy.proxy_server import hash_token, user_api_key_cache
    ...
    team_obj = MishikaLLM_TeamTableCachedObj(
        team_id=_team_id,
        blocked=False,
        last_refreshed_at=time.time(),
    ...
    setattr(mishikallm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "prisma_client", "hello-world")
...
def test_custom_logger_failure_handler(mock_acompletion, client_no_auth):
    ...
    proxy_logging_obj: ProxyLogging = getattr(
        mishikallm.proxy.proxy_server, "proxy_logging_obj"
    )

    mishikallm.callbacks = [mock_logger, mock_logger_unit_tests]
    proxy_logging_obj._init_mishikallm_callbacks(llm_router=None)

    setattr(mishikallm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "prisma_client", "FAKE-VAR")
    setattr(mishikallm.proxy.proxy_server, "proxy_logging_obj", proxy_logging_obj)
...
def test_embedding(mock_aembedding, client_no_auth):
    global headers
    from mishikallm.proxy.proxy_server import user_custom_auth
...
def test_health(client_no_auth):
    ...
    from mishikallm._logging import verbose_logger, verbose_proxy_logger
...
def test_chat_completion_optional_params(mock_acompletion, client_no_auth):
    ...
    # This tests if all the /chat/completion params are passed to mishikallm
    try:
        # Your test data
        ...
        pytest.fail("MishikaLLM Proxy test failed. Exception", e)
...
@mock.patch("mishikallm.proxy.proxy_server.mishikallm.Cache")
def test_load_router_config(mock_cache, fake_env_vars):
    ...
    try:
        ...
        # tests for mishikallm.cache set from config
        ...
        assert mishikallm.cache is not None
        print(mishikallm.cache)
        print(mishikallm.cache.supported_call_types)
        print(vars(mishikallm.cache.cache))
        assert "redis_client" in vars(
            mishikallm.cache.cache
        )  # it should default to redis on proxy
        assert mishikallm.cache.supported_call_types == [
...
async def test_team_update_redis():
    ...
    from mishikallm.proxy._types import MishikaLLM_TeamTableCachedObj
    ...
    with patch.object(
        ...
        await _cache_team_object(
            team_id="1234",
            team_table=MishikaLLM_TeamTableCachedObj(team_id="1234"),
...
from mishikallm.proxy.management_endpoints.team_endpoints import team_member_add
from test_key_generate_prisma import prisma_client


@pytest.mark.parametrize(
    "user_role",
    [MishikallmUserRoles.INTERNAL_USER.value, MishikallmUserRoles.PROXY_ADMIN.value],
)
@pytest.mark.asyncio
async def test_create_user_default_budget(prisma_client, user_role):
    ...
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm, "max_internal_user_budget", 10)
    setattr(mishikallm, "internal_user_budget_duration", "5m")
    ...
    with patch.object(
        mishikallm.proxy.proxy_server.prisma_client, "insert_data", new=AsyncMock()
    ) as mock_client:
        await new_user(
            request,
        )
        ...
        if user_role == MishikallmUserRoles.INTERNAL_USER.value:
            assert (
...
async def test_create_team_member_add(prisma_client, new_member_method):
    ...
    from mishikallm.proxy._types import MishikaLLM_TeamTableCachedObj, MishikaLLM_UserTable
    from mishikallm.proxy.proxy_server import hash_token, user_api_key_cache
    ...
    await mishikallm.proxy.proxy_server.prisma_client.connect()
    user = f"ishaan {uuid.uuid4().hex}"
    _team_id = "mishikallm-test-client-id-new"
    team_obj = MishikaLLM_TeamTableCachedObj(
        team_id=_team_id,
        blocked=False,
        last_refreshed_at=time.time(),
    ...
    with patch(
        "mishikallm.proxy.proxy_server.prisma_client.db.mishikallm_usertable",
        new_callable=AsyncMock,
    ) as mock_mishikallm_usertable, patch(
        "mishikallm.proxy.auth.auth_checks._get_team_object_from_user_api_key_cache",
        new=AsyncMock(return_value=team_obj),
    ) as mock_team_obj:

        mock_client = AsyncMock(
            return_value=MishikaLLM_UserTable(
                user_id="1234", max_budget=100, user_email="1234"
            )
        )
        mock_mishikallm_usertable.upsert = mock_client
        mock_mishikallm_usertable.find_many = AsyncMock(return_value=None)
        team_mock_client = AsyncMock()
        original_val = getattr(
            mishikallm.proxy.proxy_server.prisma_client.db, "mishikallm_teamtable"
        )
        mishikallm.proxy.proxy_server.prisma_client.db.mishikallm_teamtable = team_mock_client

        team_mock_client.update = AsyncMock(
            return_value=MishikaLLM_TeamTableCachedObj(team_id="1234")
        )
        ...
        mishikallm.proxy.proxy_server.prisma_client.db.mishikallm_teamtable = original_val
...
async def test_create_team_member_add_team_admin_user_api_key_auth(
    ...
    from mishikallm.proxy._types import MishikaLLM_TeamTableCachedObj, Member
...
async def test_user_info_team_list(prisma_client):
    ...
    from mishikallm.proxy._types import MishikaLLM_UserTable

    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    await mishikallm.proxy.proxy_server.prisma_client.connect()

    from mishikallm.proxy.management_endpoints.internal_user_endpoints import user_info

    with patch(
        "mishikallm.proxy.management_endpoints.team_endpoints.list_team",
        new_callable=AsyncMock,
    ) as mock_client:
...
async def test_add_callback_via_key(prisma_client):
    ...
    from mishikallm.proxy.proxy_server import chat_completion
    ...
    try:
        ...
        with patch.object(
            mishikallm.mishikallm_core_utils.mishikallm_logging,
            "LangFuseLogger",
            new=MagicMock(),
        ) as mock_client:
            ...
            assert "user_api_key_metadata" in kwargs["mishikallm_params"]["metadata"]
            assert (
                "logging"
                in kwargs["mishikallm_params"]["metadata"]["user_api_key_metadata"]
            )
            checked_keys = False
            for item in kwargs["mishikallm_params"]["metadata"]["user_api_key_metadata"][
                "logging"
            ]:
...
async def test_add_callback_via_key_mishikallm_pre_call_utils(
    ...
    proxy_config = getattr(mishikallm.proxy.proxy_server, "proxy_config")
    ...
    new_data = await add_mishikallm_data_to_request(**data)
...
async def test_disable_fallbacks_by_key(disable_fallbacks_set):
    ...
    data = MishikaLLMProxyRequestSetup.add_key_level_controls(
...
async def test_add_callback_via_key_mishikallm_pre_call_utils_gcs_bucket(
    prisma_client, callback_type, expected_success_callbacks, expected_failure_callbacks
):
    import json
...
async def test_add_callback_via_key_mishikallm_pre_call_utils_langsmith(
    prisma_client, callback_type, expected_success_callbacks, expected_failure_callbacks
):
    import json
...
async def test_proxy_model_group_alias_checks(prisma_client, hidden):
    ...
    from mishikallm.proxy.proxy_server import model_group_info, model_info_v1, model_list
    ...
    _model_list = [
        {
            ...
            "mishikallm_params": {"model": "gpt-3.5-turbo"},
        }
    ]
    model_alias = "gpt-4"
    ...
    setattr(mishikallm.proxy.proxy_server, "llm_router", router)
    setattr(mishikallm.proxy.proxy_server, "llm_model_list", _model_list)
    ...
    _model_list = [
        {
            ...
            "mishikallm_params": {"model": "cohere/rerank-english-v3.0"},
            "model_info": {
                "mode": "rerank",
            },
        }
    ]
    router = mishikallm.Router(model_list=_model_list)
    setattr(mishikallm.proxy.proxy_server, "llm_router", router)
    setattr(mishikallm.proxy.proxy_server, "llm_model_list", _model_list)
...
#         new_team,
#     )
#     from mishikallm.proxy._types import TeamMemberAddRequest, Member, NewTeamRequest
...
#                     member=Member(
#                         user_id="1234",
#                         user_role=MishikallmUserRoles.INTERNAL_USER,
#                     ),
#                 )
...
#             assert new_user_info["user_role"] == MishikallmUserRoles.INTERNAL_USER
...
async def test_proxy_server_prisma_setup():
    from mishikallm.proxy.proxy_server import ProxyStartupEvent, proxy_state
    from mishikallm.proxy.utils import ProxyLogging
    ...
    with patch.object(
        mishikallm.proxy.proxy_server, "PrismaClient", new=MagicMock()
...
async def test_proxy_server_prisma_setup_invalid_db():
    ...
    """
    from mishikallm.proxy.proxy_server import ProxyStartupEvent
    from mishikallm.proxy.utils import ProxyLogging
    from mishikallm.caching import DualCache
...
async def test_get_ui_settings_spend_logs_threshold():
    ...
    """
    from mishikallm.proxy.management_endpoints.ui_sso import get_ui_settings
...
def test_get_timeout_from_request():
    ...
    headers = {
        "x-mishikallm-timeout": "90",
    }
    timeout = MishikaLLMProxyRequestSetup._get_timeout_from_request(headers)
    assert timeout == 90

    headers = {
        "x-mishikallm-timeout": "90.5",
    }
    timeout = MishikaLLMProxyRequestSetup._get_timeout_from_request(headers)
    assert timeout == 90.5
