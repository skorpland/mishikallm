...
from mishikallm.proxy._types import MishikaLLM_UserTable, MishikallmUserRoles
...
def test_check_valid_ip(
    ...
    from mishikallm.proxy.auth.auth_utils import _check_valid_ip
...
async def test_returned_user_api_key_auth(user_role, expected_role):
    from mishikallm.proxy._types import MishikaLLM_UserTable, MishikallmUserRoles
    from mishikallm.proxy.auth.user_api_key_auth import _return_user_api_key_auth_obj
    from datetime import datetime

    new_obj = await _return_user_api_key_auth_obj(
        user_obj=MishikaLLM_UserTable(
...
async def test_aaauser_personal_budgets(key_ownership):
    ...
    from mishikallm.proxy._types import MishikaLLM_UserTable, UserAPIKeyAuth
    from mishikallm.proxy.auth.user_api_key_auth import user_api_key_auth
    from mishikallm.proxy.proxy_server import hash_token, user_api_key_cache
    ...
    test_user_cache = getattr(mishikallm.proxy.proxy_server, "user_api_key_cache")
...
def test_is_ui_route_allowed(route, user_role, expected_result):
    from mishikallm.proxy.auth.auth_checks import _is_ui_route
...
def test_is_api_route_allowed(route, user_role, expected_result):
    from mishikallm.proxy.auth.auth_checks import _is_api_route_allowed
    from mishikallm.proxy._types import MishikaLLM_UserTable
...
async def test_auth_not_connected_to_db():
    ...
    setattr(mishikallm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "prisma_client", None)
    setattr(
...
def test_get_api_key_from_custom_header(headers, custom_header_name, expected_api_key):
    ...
    api_key = get_api_key_from_custom_header(
        request=request, custom_mishikallm_key_header_name=custom_header_name
    )
...
@pytest.mark.parametrize(
    ...
    [
        (MishikallmUserRoles.PROXY_ADMIN, "1234", None, True),
        (MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY, None, "1234", True),
        (MishikallmUserRoles.TEAM, "1234", None, False),
        (MishikallmUserRoles.TEAM, None, None, False),
        (MishikallmUserRoles.TEAM, "1234", "1234", True),
    ],
)
def test_allowed_route_inside_route(
    user_role, auth_user_id, requested_user_id, expected_result
):
    from mishikallm.proxy.auth.auth_checks import allowed_route_check_inside_route
    from mishikallm.proxy._types import UserAPIKeyAuth, MishikallmUserRoles
...
async def test_soft_budget_alert():
    ...
    setattr(mishikallm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "prisma_client", AsyncMock())
    ...
    alert_called = False
    original_budget_alerts = mishikallm.proxy.proxy_server.proxy_logging_obj.budget_alerts
...
def test_is_allowed_route():
    from mishikallm.proxy.auth.auth_checks import _is_allowed_route
    from mishikallm.proxy._types import UserAPIKeyAuth
    import datetime
...
def test_is_user_proxy_admin(user_obj, expected_result):
    from mishikallm.proxy.auth.auth_checks import _is_user_proxy_admin
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ),  # Case 2: user_role is PROXY_ADMIN (should return MishikallmUserRoles.PROXY_ADMIN)
        (
            MishikaLLM_UserTable(
                user_role="OTHER_ROLE",
                user_id="1234",
        ...
        ),  # Case 3: invalid user_role (should return MishikallmUserRoles.INTERNAL_USER)
    ],
)
def test_get_user_role(user_obj, expected_role):
    from mishikallm.proxy.auth.user_api_key_auth import _get_user_role
...
async def test_user_api_key_auth_websocket():
    from mishikallm.proxy.auth.user_api_key_auth import user_api_key_auth_websocket
    ...
    with patch(
        "mishikallm.proxy.auth.user_api_key_auth.user_api_key_auth", autospec=True
...
async def test_jwt_user_api_key_auth_builder_enforce_rbac(enforce_rbac, monkeypatch):
    from mishikallm.proxy.auth.handle_jwt import JWTHandler, JWTAuthManager
    ...
    monkeypatch.setenv("JWT_AUDIENCE", "api://MishikaLLM_Proxy-dev")
    ...
    local_cache.set_cache(
        key="mishikallm_jwt_auth_keys_my-fake-url",
        value=keys,
    )

    mishikallm_jwtauth = MishikaLLM_JWTAuth(
        **{
            "admin_jwt_scope": "mishikallm_proxy_endpoints_access",
...
def test_can_rbac_role_call_model():
    from mishikallm.proxy.auth.handle_jwt import JWTAuthManager
    from mishikallm.proxy._types import RoleBasedPermissions

    roles_based_permissions = [
        RoleBasedPermissions(
            role=MishikallmUserRoles.INTERNAL_USER,
            models=["gpt-4"],
        ),
        RoleBasedPermissions(
            role=MishikallmUserRoles.PROXY_ADMIN,
            models=["anthropic-claude"],
        ),
    ]

    assert JWTAuthManager.can_rbac_role_call_model(
        rbac_role=MishikallmUserRoles.INTERNAL_USER,
    ...
    with pytest.raises(HTTPException):
        JWTAuthManager.can_rbac_role_call_model(
            rbac_role=MishikallmUserRoles.PROXY_ADMIN,
            general_settings={"role_permissions": roles_based_permissions},
            model="gpt-4o",
        )
...
def test_get_model_from_request(route, request_data, expected_model):
    from mishikallm.proxy.auth.user_api_key_auth import get_model_from_request
...
async def test_jwt_non_admin_team_route_access(monkeypatch):
    ...
    from mishikallm.proxy.auth.user_api_key_auth import user_api_key_auth
    import json
    from mishikallm.proxy._types import ProxyException
    ...
    monkeypatch.setattr(
        mishikallm.proxy.proxy_server, "general_settings", {"enable_jwt_auth": True}
    )

    # Mock JWTAuthManager.auth_builder
    with patch(
        "mishikallm.proxy.auth.handle_jwt.JWTAuthManager.auth_builder",
...
@pytest.mark.asyncio
async def test_x_mishikallm_api_key():
    """
    Check if auth can pick up x-mishikallm-api-key header, even if Bearer token is provided
    """
    from fastapi import Request
    from starlette.datastructures import URL
    ...
    valid_token = await user_api_key_auth(request=request, api_key="Bearer " + ignored_key, custom_mishikallm_key_header=master_key)
