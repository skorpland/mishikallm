...
from mishikallm.proxy.utils import _get_redoc_url, _get_docs_url
import json
import pytest
...
from mishikallm.proxy._types import MishikallmUserRoles, UserAPIKeyAuth
from mishikallm.proxy.auth.auth_utils import is_request_body_safe
from mishikallm.proxy.mishikallm_pre_call_utils import (
    _get_dynamic_logging_metadata,
    add_mishikallm_data_to_request,
)
from mishikallm.types.utils import SupportedCacheControls
...
def mock_request(monkeypatch):
    ...
    monkeypatch.setattr(
        "mishikallm.proxy.mishikallm_pre_call_utils.add_mishikallm_data_to_request", mock_request
    )
    return mock_request
...
@pytest.mark.asyncio
async def test_add_mishikallm_data_to_request_thread_endpoint(endpoint, mock_request):
    ...
    data = {}
    await add_mishikallm_data_to_request(
        data, mock_request, user_api_key_dict, proxy_config
    )

    print("DATA: ", data)

    assert "mishikallm_metadata" in data
...
@pytest.mark.asyncio
async def test_add_mishikallm_data_to_request_non_thread_endpoint(endpoint, mock_request):
    mock_request.url.path = endpoint
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test_api_key", user_id="test_user_id", org_id="test_org_id"
    )
    ...
    assert "mishikallm_metadata" not in data
...
async def test_traceparent_not_added_by_default(endpoint, mock_request):
    ...
    setattr(mishikallm.proxy.proxy_server, "open_telemetry_logger", otel_logger)
    ...
    setattr(mishikallm.proxy.proxy_server, "open_telemetry_logger", None)
...
async def test_add_key_or_team_level_spend_logs_metadata_to_request(
    ...
    print(data)
    new_data = await add_mishikallm_data_to_request(
        data, mock_request, user_api_key_dict, proxy_config
    )
...
def test_reading_openai_org_id_from_headers():
    ...
    org_id = MishikaLLMProxyRequestSetup.get_openai_org_id_from_headers(headers)
...
def test_add_mishikallm_data_for_backend_llm_call(headers, expected_data):
    import json
    from mishikallm.proxy.mishikallm_pre_call_utils import MishikaLLMProxyRequestSetup
    ...
    data = MishikaLLMProxyRequestSetup.add_mishikallm_data_for_backend_llm_call(
        headers=headers,
...
def test_foward_mishikallm_user_info_to_backend_llm_call():
    import json

    mishikallm.add_user_information_to_llm_headers = True
    ...
    data = MishikaLLMProxyRequestSetup.add_headers_to_llm_call(
        headers={},
        user_api_key_dict=user_api_key_dict,
    )

    expected_data = {
        "x-mishikallm-user_api_key_user_id": "test_user_id",
        "x-mishikallm-user_api_key_org_id": "test_org_id",
        "x-mishikallm-user_api_key_hash": "test_api_key",
    }
...
def test_update_internal_user_params():
    ...
    from mishikallm.proxy._types import NewUserRequest

    mishikallm.default_internal_user_params = {
        "max_budget": 100,
        "budget_duration": "30d",
    ...
    assert updated_data_json["models"] == mishikallm.default_internal_user_params["models"]
    assert (
        updated_data_json["max_budget"]
        == mishikallm.default_internal_user_params["max_budget"]
    )
    assert (
        updated_data_json["budget_duration"]
        == mishikallm.default_internal_user_params["budget_duration"]
    )
    ...
    with patch.object(
        ...
        new=AsyncMock(
            return_value=ReturnValue(
                param_name="mishikallm_settings",
                param_value={
...
def test_prepare_key_update_data():
    ...
    from mishikallm.proxy._types import UpdateKeyRequest
...
def test_merge_tags(request_tags, tags_to_add, expected_tags):
    ...
    result = MishikaLLMProxyRequestSetup._merge_tags(
...
async def test_add_mishikallm_data_to_request_duplicate_tags(
    key_tags, request_tags, expected_tags
):
    """
    ...
    MishikaLLM_SpendLogs table request_tags column. This can mess up aggregation logic"
    """
    mock_request = Mock(spec=Request)
    mock_request.url.path = "/chat/completions"
    ...
    # Process request
    proxy_config = Mock()
    result = await add_mishikallm_data_to_request(
        data=data,
...
def test_enforced_params_check(
    ...
    from mishikallm.proxy.mishikallm_pre_call_utils import _enforced_params_check
...
def test_get_key_models():
    from mishikallm.proxy.auth.model_checks import get_key_models
...
def test_get_team_models():
    from mishikallm.proxy.auth.model_checks import get_team_models
    from collections import defaultdict
...
def test_update_config_fields():
    ...
    all_team_config = updated_config["mishikallm_settings"]["default_team_settings"]
...
def test_get_complete_model_list(proxy_model_list, provider):
    ...
    """
    from mishikallm.proxy.auth.model_checks import get_complete_model_list
...
def test_team_callback_metadata_all_none_values():
    from mishikallm.proxy._types import TeamCallbackMetadata
...
def test_proxy_config_state_post_init_callback_call():
    ...
    MishikaLLMProxyRequestSetup.add_team_based_callbacks_from_config(
        team_id="test",
        proxy_config=pc,
    )

    config = pc.get_config_state()
    assert config["mishikallm_settings"]["default_team_settings"][0]["team_id"] == "test"
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "mishikallm_budget_table_max_budget": None,
                "mishikallm_budget_table_tpm_limit": None,
                "mishikallm_budget_table_rpm_limit": 1,
                "mishikallm_budget_table_model_max_budget": None,
            },
            "rpm_limit",
            1,
        ),
        ...
        (
            {
                "mishikallm_budget_table_max_budget": None,
                "mishikallm_budget_table_tpm_limit": None,
                "mishikallm_budget_table_rpm_limit": None,
                "mishikallm_budget_table_model_max_budget": {"gpt-4o": 100},
            },
            "model_max_budget",
            {"gpt-4o": 100},
        ),
...
def test_mishikallm_verification_token_view_response_with_budget_table(
    associated_budget_table,
    expected_user_api_key_auth_key,
    expected_user_api_key_auth_value,
):
    from mishikallm.proxy._types import MishikaLLM_VerificationTokenView
    ...
    resp = MishikaLLM_VerificationTokenView(**args)
...
def test_is_allowed_to_make_key_request():
    ...
    assert (
        _is_allowed_to_make_key_request(
            user_api_key_dict=UserAPIKeyAuth(
                user_id="test_user_id", user_role=MishikallmUserRoles.PROXY_ADMIN
            ),
            user_id="test_user_id",
...
def test_get_model_group_info():
    from mishikallm.proxy.proxy_server import _get_model_group_info
    from mishikallm import Router
...
class MockDb:
    ...
    async def query_raw(self, query: str, *args):
        ...
        filtered_keys = [
            ...
            if k["team_id"] != "mishikallm-dashboard" or k["team_id"] is None
        ]
...
async def test_get_user_info_for_proxy_admin(mock_team_data, mock_key_data):
    ...
    from mishikallm.proxy._types import UserInfoResponse

    with patch(
        "mishikallm.proxy.proxy_server.prisma_client",
...
def test_custom_openid_response():
    from mishikallm.proxy.management_endpoints.ui_sso import generic_response_convertor
    from mishikallm.proxy.management_endpoints.ui_sso import JWTHandler
    from mishikallm.proxy._types import MishikaLLM_JWTAuth
...
def test_get_temp_budget_increase():
    from mishikallm.proxy.auth.user_api_key_auth import _get_temp_budget_increase
    from mishikallm.proxy._types import UserAPIKeyAuth
...
async def test_health_check_not_called_when_disabled(monkeypatch):
    ...
    monkeypatch.setattr(
        "mishikallm.proxy.proxy_server.PrismaClient", lambda **kwargs: mock_prisma
    )
...
@patch(
    "mishikallm.proxy.proxy_server.get_openapi_schema",
    return_value={
        "paths": {
...
def test_custom_openapi(mock_get_openapi_schema):
    from mishikallm.proxy.proxy_server import custom_openapi
...
from mishikallm.proxy.utils import ProxyUpdateSpend
...
async def test_spend_logs_cleanup_after_error():
    ...
    mock_client.db.mishikallm_spendlogs.create_many = AsyncMock(
...
def test_get_known_models_from_wildcard(wildcard_model, expected_models):
    from mishikallm.proxy.auth.model_checks import get_known_models_from_wildcard
...
def test_update_model_if_team_alias_exists(data, user_api_key_dict, expected_model):
    from mishikallm.proxy.mishikallm_pre_call_utils import _update_model_if_team_alias_exists
...
def mock_prisma_client():
    client = MagicMock()
    client.db = MagicMock()
    client.db.mishikallm_teamtable = AsyncMock()
    return client
...
async def test_get_admin_team_ids(
    test_id: str,
    user_info: Optional[MishikaLLM_UserTable],
    user_role: str,
    ...
    # Setup
    mock_prisma_client.db.mishikallm_teamtable.find_many.return_value = mock_teams
    user_api_key_dict = UserAPIKeyAuth(
    ...
    if should_query_db:
        mock_prisma_client.db.mishikallm_teamtable.find_many.assert_called_once_with(
            where={"team_id": {"in": user_info.teams}}
        )
    else:
        mock_prisma_client.db.mishikallm_teamtable.find_many.assert_not_called()
