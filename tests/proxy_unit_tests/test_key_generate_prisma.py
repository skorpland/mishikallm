...
from mishikallm.proxy._types import (
    ...
    GenerateKeyRequest,
    KeyRequest,
    MishikaLLM_UpperboundKeyGenerateParams,
    NewCustomerRequest,
    NewTeamRequest,
    NewUserRequest,
    ProxyErrorTypes,
...
def test_generate_and_call_with_valid_key(prisma_client, api_route):
    ...
    try:
        ...
        async def test():
            ...
            request = NewUserRequest(user_role=MishikallmUserRoles.INTERNAL_USER)
...
async def test_call_with_valid_model_using_all_models(prisma_client):
    ...
    """
    # Make a call to a key with model = `all-proxy-models` this is an Alias from MishikaLLM Admin UI
    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    try:
...
def test_call_with_user_over_budget(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            from mishikallm import Choices, Message, ModelResponse, Usage
            from mishikallm.proxy.proxy_server import _ProxyDBLogger
...
def test_call_with_end_user_over_budget(prisma_client):
    ...
    # we only check this when mishikallm.max_end_user_budget is set
    import random

    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm, "max_end_user_budget", 0.00001)
    try:
...
def test_call_with_proxy_over_budget(prisma_client):
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    mishikallm_proxy_budget_name = f"mishikallm-proxy-budget-{time.time()}"
    setattr(
        mishikallm.proxy.proxy_server,
        "mishikallm_proxy_admin_name",
        mishikallm_proxy_budget_name,
    )
    setattr(mishikallm, "max_budget", 0.00001)
    from mishikallm.proxy.proxy_server import user_api_key_cache

    user_api_key_cache.set_cache(
        key="{}:spend".format(mishikallm_proxy_budget_name), value=0
    )
...
def test_call_with_proxy_over_budget_stream(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            # request = NewUserRequest(
            #     max_budget=0.00001, user_id=mishikallm_proxy_budget_name
            # )
            request = NewUserRequest()
            key = await new_user(
                data=request,
...
def test_delete_key(prisma_client):
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "user_custom_auth", None)
    try:
        ...
        async def test():
            ...
            result.user_role = MishikallmUserRoles.PROXY_ADMIN
            # delete the key
...
def test_generate_and_call_key_info(prisma_client):
    ...
    try:
        ...
        async def test():
            await mishikallm.proxy.proxy_server.prisma_client.connect()
            request = NewUserRequest(
                metadata={"team": "mishikallm-team3", "project": "mishikallm-project3"}
            )
            key = await new_user(
                data=request,
                user_api_key_dict=UserAPIKeyAuth(
            ...
            assert result["info"]["metadata"] == {
                "team": "mishikallm-team3",
                "project": "mishikallm-project3",
            }
...
def test_generate_and_update_key(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            # create team "mishikallm-core-infra@gmail.com""
            print("creating team mishikallm-core-infra@gmail.com")
            _team_1 = "mishikallm-core-infra@gmail.com_{}".format(uuid.uuid4())
            await new_team(
                NewTeamRequest(
                    team_id=_team_1,
                ),
            ...
            request = NewUserRequest(
                metadata={"project": "mishikallm-project3"},
                team_id=_team_1,
            )
...
def test_key_generate_with_custom_auth(prisma_client):
    ...
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(
        mishikallm.proxy.proxy_server, "user_custom_key_generate", custom_generate_key_fn
    )
    try:
        ...
        async def test():
            try:
                ...
                assert (
                    "This violates MishikaLLM Proxy Rules. No team id provided."
                    in e.message
                )

            request_2 = GenerateKeyRequest(
                team_id="mishikallm-core-infra@gmail.com",
            )
...
def test_call_with_key_over_budget_no_cache(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            setattr(mishikallm.proxy.proxy_server, "proxy_batch_write_at", 1)
...
async def test_call_with_key_over_model_budget(
    ...
    mishikallm.callbacks.append(model_budget_limiter)
    ...
    try:
        ...
        mishikallm.callbacks.remove(model_budget_limiter)
...
async def test_key_name_set(prisma_client):
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "general_settings", {"allow_user_auth": True})
    await mishikallm.proxy.proxy_server.prisma_client.connect()
    try:
        request = GenerateKeyRequest()
        key = await generate_key_fn(
...
async def test_default_key_params(prisma_client):
    ...
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "general_settings", {"allow_user_auth": True})
    mishikallm.default_key_generate_params = {"max_budget": 0.000122}
    await mishikallm.proxy.proxy_server.prisma_client.connect()
    try:
        request = GenerateKeyRequest()
        key = await generate_key_fn(
...
async def test_upperbound_key_param_larger_budget(prisma_client):
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    mishikallm.upperbound_key_generate_params = MishikaLLM_UpperboundKeyGenerateParams(
...
def test_get_bearer_token():
    from mishikallm.proxy.auth.user_api_key_auth import _get_bearer_token
...
async def test_user_api_key_auth(prisma_client):
    from mishikallm.proxy.proxy_server import ProxyException
...
async def test_user_api_key_auth_without_master_key(prisma_client):
    ...
    try:
        ...
        setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
        setattr(mishikallm.proxy.proxy_server, "master_key", None)
        setattr(
            mishikallm.proxy.proxy_server, "general_settings", {"allow_user_auth": True}
        )
        await mishikallm.proxy.proxy_server.prisma_client.connect()
...
async def test_key_with_no_permissions(prisma_client):
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "general_settings", {"allow_user_auth": False})
    await mishikallm.proxy.proxy_server.prisma_client.connect()
    try:
        response = await generate_key_helper_fn(
            request_type="key",
            **{"duration": "1hr", "key_max_budget": 0, "models": [], "aliases": {}, "config": {}, "spend": 0, "user_id": "ishaan", "team_id": "mishikallm-dashboard"},  # type: ignore
        )
...
async def test_reset_spend_authentication(prisma_client):
    ...
    3. Test that non-master key with role == MishikallmUserRoles.PROXY_ADMIN or admin gets rejected
    """
    ...
    # Test 3 - Non-Master Key with role == MishikallmUserRoles.PROXY_ADMIN or admin
    _response = await new_user(
        data=NewUserRequest(
            user_role=MishikallmUserRoles.PROXY_ADMIN,
            tpm_limit=20,
...
async def test_update_user_role(prisma_client):
    ...
    await user_update(
        data=UpdateUserRequest(
            user_id=key.user_id, user_role=MishikallmUserRoles.PROXY_ADMIN
        )
    )
...
async def test_custom_api_key_header_name(prisma_client):
    ...
    setattr(
        mishikallm.proxy.proxy_server,
        "general_settings",
        {"mishikallm_key_header_name": "x-mishikallm-key"},
    )
    await mishikallm.proxy.proxy_server.prisma_client.connect()
    ...
    request = Request(
        {
            ...
            "path": api_route.path,
            "headers": [
                (b"x-mishikallm-key", b"Bearer sk-1234"),
            ],
        }
    )

    # this should pass because we pass the master key as X-Mishikallm-Key and mishikallm_key_header_name="X-Mishikallm-Key" in general settings
    result = await user_api_key_auth(request=request, api_key="Bearer invalid-key")

    # this should fail because X-Mishikallm-Key is invalid
    request = Request(
        {
            "type": "http",
            "route": api_route,
    ...
    # this should pass because X-Mishikallm-Key is valid
...
async def test_team_access_groups(prisma_client):
    ...
    mishikallm_router = mishikallm.Router(
        model_list=[
            {
    ...
    setattr(mishikallm.proxy.proxy_server, "llm_router", mishikallm_router)
...
async def test_aadmin_only_routes(prisma_client):
    ...
    from mishikallm.proxy import proxy_server
...
async def test_list_keys(prisma_client):
    ...
    from mishikallm.proxy.proxy_server import hash_token
    ...
    response = await list_keys(
        request,
        UserAPIKeyAuth(user_role=MishikallmUserRoles.PROXY_ADMIN.value),
        page=1,
        size=2,
    )
...
async def test_user_api_key_auth_db_unavailable():
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", MockPrismaClient())
    setattr(mishikallm.proxy.proxy_server, "user_api_key_cache", MockDualCache())
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(
        mishikallm.proxy.proxy_server,
        "general_settings",
    ...
    assert result.user_id == mishikallm.proxy.proxy_server.mishikallm_proxy_admin_name
...
async def test_user_api_key_auth_db_unavailable_not_allowed():
    ...
    setattr(mishikallm.proxy.proxy_server, "prisma_client", MockPrismaClient())
    setattr(mishikallm.proxy.proxy_server, "user_api_key_cache", MockDualCache())
    setattr(mishikallm.proxy.proxy_server, "general_settings", {})
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    ...
    # Run test with a sample API key
    with pytest.raises(mishikallm.proxy._types.ProxyException):
        await user_api_key_auth(
            request=request,
            api_key="Bearer sk-123456789",
        )
...
async def test_key_generate_with_secret_manager_call(prisma_client):
    ...
    """
    from mishikallm.secret_managers.aws_secret_manager_v2 import AWSSecretsManagerV2
    from mishikallm.proxy._types import KeyManagementSystem, KeyManagementSettings

    from mishikallm.proxy.hooks.key_management_event_hooks import (
        MISHIKALLM_PREFIX_STORED_VIRTUAL_KEYS,
    )
    ...
    mishikallm.secret_manager_client = aws_secret_manager_client
    mishikallm._key_management_system = KeyManagementSystem.AWS_SECRET_MANAGER
    ...
    setattr(mishikallm.proxy.proxy_server, "general_settings", general_settings)
    setattr(mishikallm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(mishikallm.proxy.proxy_server, "prisma_client", prisma_client)
    await mishikallm.proxy.proxy_server.prisma_client.connect()
    ############################################################################
    ...
    result = await aws_secret_manager_client.async_read_secret(
        secret_name=f"{mishikallm._key_management_settings.prefix_for_stored_virtual_keys}{key_alias}"
    )
    ...
    await delete_key_fn(
        ...
        user_api_key_dict=UserAPIKeyAuth(
            user_role=MishikallmUserRoles.PROXY_ADMIN, api_key="sk-1234", user_id="1234"
        ),
    )
...
def test_should_track_cost_callback():
    ...
    """
    from mishikallm.proxy.hooks.proxy_track_cost_callback import (
...
async def test_get_paginated_teams(prisma_client):
    ...
    """
    from mishikallm.proxy.management_endpoints.team_endpoints import get_paginated_teams
...
async def test_reset_budget_job(prisma_client, entity_type):
    ...
    if entity_type == "key":
        ...
        updated = await prisma_client.db.mishikallm_verificationtoken.update_many(
    ...
    if entity_type == "key":
        entity_before = await prisma_client.db.mishikallm_verificationtoken.find_unique(
            where={"token": entity_id}
        )
    elif entity_type == "user":
        entity_before = await prisma_client.db.mishikallm_usertable.find_unique(
            where={"user_id": entity_id}
        )
    elif entity_type == "team":
        entity_before = await prisma_client.db.mishikallm_teamtable.find_unique(
    ...
    if entity_type == "key":
        entity_after = await prisma_client.db.mishikallm_verificationtoken.find_unique(
            where={"token": entity_id}
        )
    elif entity_type == "user":
        entity_after = await prisma_client.db.mishikallm_usertable.find_unique(
            where={"user_id": entity_id}
        )
    elif entity_type == "team":
        entity_after = await prisma_client.db.mishikallm_teamtable.find_unique(
            where={"team_id": entity_id}
        )
