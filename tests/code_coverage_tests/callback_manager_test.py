...
ALLOWED_FILES = [
    # local files
    "../../mishikallm/mishikallm_core_utils/mishikallm.logging_callback_manager.py",
    "../../mishikallm/proxy/common_utils/callback_utils.py",
...
def check_for_callback_modifications(file_path):
    """
    Checks if any direct modifications to specific mishikallm callback lists are made in the given file.
    ...
    for node in ast.walk(tree):
        # Check for attribute calls like mishikallm.callbacks.append()
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            ...
            # Check if the attribute chain starts with 'mishikallm' and modifies a protected list
            if (
                len(attr_chain) >= 3
                and attr_chain[0] == "mishikallm"
                ...
                if (
                    ...
                    violations.append(
                        ...
                        f"Direct modification of 'mishikallm.{protected_list}' using '{operation}' is not allowed. "
...
def test_no_unauthorized_callback_modifications():
    ...
    """
    base_dir = "./mishikallm"  # Adjust this path as needed
    # base_dir = "../../mishikallm"  # LOCAL TESTING
