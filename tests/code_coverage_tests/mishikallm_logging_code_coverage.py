...
def get_function_names_from_file(file_path: str) -> List[str]:
    """
    Extracts all static method names from mishikallm_logging.py
    """
...
def main():
    logging_file = "./mishikallm/mishikallm_core_utils/mishikallm_logging.py"
    tests_dir = "./tests/"
    ...
    if untested_functions:
        ...
        raise Exception(
            f"{untested_perc * 100:.2f}% of functions in mishikallm_logging.py are not tested: {untested_functions}"
        )
    else:
        print("All functions in mishikallm_logging.py are covered by tests.")
