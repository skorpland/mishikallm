...
from mishikallm.types.passthrough_endpoints.pass_through_endpoints import EndpointType
from mishikallm.proxy.pass_through_endpoints.success_handler import (
    PassThroughEndpointLogging,
)
from mishikallm.proxy.pass_through_endpoints.streaming_handler import (
...
from mishikallm.types.passthrough_endpoints.pass_through_endpoints import PassthroughStandardLoggingPayload
...
def test_init_kwargs_for_pass_through_endpoint_basic(
    ...
    result = _init_kwargs_for_pass_through_endpoint(
        ...
        mishikallm_call_id="test-call-id",
    ...
    assert result["mishikallm_call_id"] == "test-call-id"
    ...
    assert result["mishikallm_params"]["metadata"] == expected_metadata


def test_init_kwargs_with_mishikallm_metadata(mock_request, mock_user_api_key_dict):
    """
    Expected behavior: mishikallm_metadata should be merged with default metadata

    see usage example here: https://docs.21t.cc/docs/pass_through/anthropic_completion#send-mishikallm_metadata-tags
    """
    request = mock_request()
    parsed_body = {
        "mishikallm_metadata": {"custom_field": "custom_value", "tags": ["tag1", "tag2"]}
    }
    passthrough_payload = PassthroughStandardLoggingPayload(
        url="https://test.com",
        request_body={},
    ...
    # Check that mishikallm_metadata was merged with default metadata
    metadata = result["mishikallm_params"]["metadata"]
...
athropic_request_body = {
    ...
    "mishikallm_metadata": {"tags": ["hi", "hello"]},
}
...
async def test_pass_through_request_logging_failure(
    ...
    with patch(
        "mishikallm.proxy.pass_through_endpoints.pass_through_endpoints.PassThroughEndpointLogging.pass_through_async_success_handler",
...
async def test_pass_through_request_logging_failure_with_stream(
    ...
    # Patch both the logging handler and the httpx client
    with patch(
        "mishikallm.proxy.pass_through_endpoints.streaming_handler.PassThroughStreamingHandler._route_streaming_logging_to_handler",
        new=mock_logging_failure,
    ), patch(
        "httpx.AsyncClient.send",
        return_value=mock_response,
