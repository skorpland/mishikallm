...
from mishikallm.proxy.proxy_server import app, ProxyLogging
...
def override_env_settings(monkeypatch):
    ...
    monkeypatch.setenv("MISHIKALLM_MASTER_KEY", "sk-1234")
    monkeypatch.setenv("MISHIKALLM_LOG", "DEBUG")
...
async def test_master_key_not_inserted(test_client):
    ...
    result = await prisma_client.db.mishikallm_verificationtoken.find_many()
    print(result)

    # The expectation is that no token (or unintended record) is added on startup.
    assert len(result) == 0, (
        "SECURITY ALERT SECURITY ALERT SECURITY ALERT: Expected no record in the mishikallm_verificationtoken table. On startup - the master key should NOT be Inserted into the DB."
