...
class TestOpenAIResponsesAPIConfig:
    ...
    def test_validate_environment(self):
        ...
        with patch("mishikallm.api_key", "mishikallm_api_key"):
            result = self.config.validate_environment(headers=headers, model=self.model)

            assert "Authorization" in result
            assert result["Authorization"] == "Bearer mishikallm_api_key"

        # Test with existing headers
        headers = {"Content-Type": "application/json"}

        with patch("mishikallm.openai_key", "openai_key"):
            with patch("mishikallm.api_key", None):
                result = self.config.validate_environment(
        ...
        with patch("mishikallm.api_key", None):
            with patch("mishikallm.openai_key", None):
                with patch(
                    "mishikallm.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_complete_url(self):
        ...
        # Test with mishikallm.api_base
        with patch("mishikallm.api_base", "https://mishikallm-api-base.example.com/v1"):
            result = self.config.get_complete_url(
                api_base=None,
                mishikallm_params={},
            )

            assert result == "https://mishikallm-api-base.example.com/v1/responses"

        # Test with environment variable
        with patch("mishikallm.api_base", None):
            with patch(
                "mishikallm.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_event_model_class_generic_event(self):
        ...
        from mishikallm.types.llms.openai import GenericEvent
