...
import mishikallm
from mishikallm.llms.llamafile.chat.transformation import LlamafileChatConfig
...
def test_resolve_api_key(input_api_key, api_key_from_secret_manager, expected_api_key, secret_manager_called):
    with patch("mishikallm.llms.llamafile.chat.transformation.get_secret_str") as mock_get_secret:
...
def test_get_openai_compatible_provider_info(api_base, api_key, secret_base, secret_key, expected_base, expected_key):
    ...
    patch_secret = patch("mishikallm.llms.llamafile.chat.transformation.get_secret_str", side_effect=fake_get_secret)
...
def test_completion_with_custom_llamafile_model():
    with patch("mishikallm.main.openai_chat_completions.completion") as mock_llamafile_completion_func:
