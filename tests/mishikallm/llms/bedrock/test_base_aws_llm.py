...
import mishikallm
from mishikallm.llms.bedrock.base_aws_llm import (
    AwsAuthError,
    BaseAWSLLM,
...
BASE_AWS_LLM_PATH = os.path.join(
    os.path.dirname(__file__), "../../../../mishikallm/llms/bedrock/base_aws_llm.py"
)
...
def test_boto3_init_tracer_wrapping():
    ...
    Ensures observability of boto3 calls in mishikallm.
    """
...
def test_auth_functions_tracer_wrapping():
    ...
    Ensures observability of AWS authentication calls in mishikallm.
    """
    # Get the source code of base_aws_llm.py
    with open(BASE_AWS_LLM_PATH, "r") as f:
        content = f.read()
