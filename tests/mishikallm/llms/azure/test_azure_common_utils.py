...
import mishikallm
from mishikallm.llms.azure.common_utils import BaseAzureLLM
...
def setup_mocks():
    with patch(
        "mishikallm.llms.azure.common_utils.get_azure_ad_token_from_entra_id"
    ) as mock_entra_token, patch(
        "mishikallm.llms.azure.common_utils.get_azure_ad_token_from_username_password"
    ) as mock_username_password_token, patch(
        "mishikallm.llms.azure.common_utils.get_azure_ad_token_from_oidc"
    ) as mock_oidc_token, patch(
        "mishikallm.llms.azure.common_utils.get_azure_ad_token_provider"
    ) as mock_token_provider, patch(
        "mishikallm.llms.azure.common_utils.mishikallm"
    ) as mock_mishikallm, patch(
        "mishikallm.llms.azure.common_utils.verbose_logger"
    ) as mock_logger, patch(
        "mishikallm.llms.azure.common_utils.select_azure_base_url_or_endpoint"
    ) as mock_select_url:
        # Configure mocks
        mock_mishikallm.AZURE_DEFAULT_API_VERSION = "2023-05-15"
        mock_mishikallm.enable_azure_ad_token_refresh = False
...
def test_initialize_with_oidc_token(setup_mocks, monkeypatch):
    ...
    result = BaseAzureLLM().initialize_azure_sdk_client(
        mishikallm_params={"azure_ad_token": "oidc/test-token"},
        api_key=None,
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version=None,
...
def test_initialize_with_enable_token_refresh(setup_mocks, monkeypatch):
    ...
    monkeypatch.delenv("AZURE_TENANT_ID", raising=False)
    setup_mocks["mishikallm"].enable_azure_ad_token_refresh = True
...
async def test_ensure_initialize_azure_sdk_client_always_used(call_type):
    ...
    patch_target = (
        "mishikallm.llms.azure.common_utils.BaseAzureLLM.initialize_azure_sdk_client"
    )
    if call_type == CallTypes.arerank:
        patch_target = (
            "mishikallm.rerank_api.main.azure_rerank.initialize_azure_sdk_client"
        )
    elif call_type == CallTypes.acreate_batch or call_type == CallTypes.aretrieve_batch:
        patch_target = (
            "mishikallm.batches.main.azure_batches_instance.initialize_azure_sdk_client"
        )
    elif (
        ...
        patch_target = (
            "mishikallm.assistants.main.azure_assistants_api.initialize_azure_sdk_client"
        )
    elif call_type == CallTypes.acreate_file or call_type == CallTypes.afile_content:
        patch_target = (
            "mishikallm.files.main.azure_files_instance.initialize_azure_sdk_client"
        )
    ...
    with patch(patch_target) as mock_init_azure:
        ...
        mishikallm_params = azure_calls[0].kwargs["mishikallm_params"]
        print("mishikallm_params", mishikallm_params)

        assert (
            "azure_ad_token" in mishikallm_params
...
async def test_ensure_initialize_azure_sdk_client_always_used_azure_text(call_type):
    ...
    patch_target = "mishikallm.main.azure_text_completions.initialize_azure_sdk_client"
...
async def test_azure_client_reuse(function_name, is_async, args):
    ...
    client_path = (
        "mishikallm.llms.azure.common_utils.AsyncAzureOpenAI"
        if is_async
        else "mishikallm.llms.azure.common_utils.AzureOpenAI"
    )
        ...
        for _ in range(10):
            try:
                ...
                if is_async:
                    ...
                    func = getattr(mishikallm, f"a{function_name}")
                    await func(**args)
                else:
                    func = getattr(mishikallm, function_name)
                    func(**args)
            except Exception:
