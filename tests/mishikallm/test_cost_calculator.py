...
from mishikallm.types.llms.openai import OpenAIRealtimeStreamList
from mishikallm.types.utils import ModelResponse, PromptTokensDetailsWrapper, Usage


def test_cost_calculator_with_response_cost_in_additional_headers():
    class MockResponse(BaseModel):
        _hidden_params = {
            "additional_headers": {"llm_provider-x-mishikallm-response-cost": 1000}
        }
...
def test_cost_calculator_with_usage():
    ...
    model_info = mishikallm.model_cost["gemini-2.0-flash-001"]
...
def test_handle_realtime_stream_cost_calculation():
    from mishikallm.cost_calculator import RealtimeAPITokenUsageProcessor
...
def test_azure_realtime_cost_calculator():
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        mishikallm_model_name="my-custom-azure-deployment",
    )
...
def test_default_image_cost_calculator(monkeypatch):
    from mishikallm.cost_calculator import default_image_cost_calculator

    temp_object = {
        "mishikallm_provider": "azure",
