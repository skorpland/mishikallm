...
async def test_construct_request_headers_project_id_from_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "mishikallm.proxy.proxy_server.premium_user",
        True,
    )
    ...
    try:
        ...
        with patch(
            "mishikallm.vertex_chat_completion._ensure_access_token_async"
        ) as mock_ensure_token:
            mock_ensure_token.return_value = (mock_auth_header, test_project_id)

            with patch(
                "mishikallm.vertex_chat_completion._get_token_and_url"
            ) as mock_get_token:
