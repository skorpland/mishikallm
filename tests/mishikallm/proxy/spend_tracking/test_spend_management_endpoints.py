...
import mishikallm
from mishikallm.proxy._types import SpendLogsPayload
from mishikallm.proxy.hooks.proxy_track_cost_callback import _ProxyDBLogger
from mishikallm.proxy.proxy_server import app, prisma_client
...
async def test_ui_view_spend_logs_with_user_id(client, monkeypatch):
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MockDB()
            self.db.mishikallm_spendlogs = self.db
...
class TestSpendLogsPayload:
    ...
    async def test_spend_logs_payload_e2e(self):
        mishikallm.callbacks = [_ProxyDBLogger(message_logging=False)]
        # mishikallm._turn_on_debug()

        with patch.object(
            mishikallm.proxy.db.db_spend_update_writer.DBSpendUpdateWriter,
            "_insert_spend_log_to_db",
        ) as mock_client, patch.object(mishikallm.proxy.proxy_server, "prisma_client"):
