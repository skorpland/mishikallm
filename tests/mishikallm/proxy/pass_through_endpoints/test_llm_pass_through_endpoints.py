...
from mishikallm.types.passthrough_endpoints.vertex_ai import VertexPassThroughCredentials
...
class TestBaseOpenAIPassThroughHandler:
    def test_join_url_paths(self):
        ...
        result = BaseOpenAIPassThroughHandler._join_url_paths(
            base_url, path, mishikallm.LlmProviders.OPENAI.value
        )
    ...
    @patch(
        "mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
    )
...
class TestVertexAIPassThroughHandler:
    ...
    async def test_vertex_passthrough_with_credentials(self, monkeypatch):
        ...
        """
        from mishikallm.proxy.pass_through_endpoints.passthrough_endpoint_router import (
        ...
        monkeypatch.setattr(
            "mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router",
            pass_through_router,
        )
        ...
        with mock.patch(
            "mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._ensure_access_token_async"
        ) as mock_ensure_token, mock.patch(
            "mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._get_token_and_url"
    ...
    async def test_async_vertex_proxy_route_api_key_auth(self):
        ...
        This is how Vertex AI JS SDK will Auth to Mishikallm Proxy
        """
        # Mock dependencies
        mock_request = Mock()
        ...
        with patch(
            "mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints.user_api_key_auth"
        ) as mock_auth:
