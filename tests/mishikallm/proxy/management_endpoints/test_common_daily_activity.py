...
async def test_get_daily_activity_empty_entity_id_list():
    ...
    mock_prisma.db.mishikallm_verificationtoken = MagicMock()
    mock_prisma.db.mishikallm_verificationtoken.find_many = AsyncMock(return_value=[])

    # Set the table name dynamically
    mock_prisma.db.mishikallm_dailyspend = mock_table

    # Call the function with empty entity_id list
    result = await get_daily_activity(
        prisma_client=mock_prisma,
        table_name="mishikallm_dailyspend",
