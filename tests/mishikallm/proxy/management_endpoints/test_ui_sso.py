...
import mishikallm
from mishikallm.proxy._types import NewTeamRequest
...
async def test_get_user_groups_from_graph_api():
    ...
    with patch(
        "mishikallm.proxy.management_endpoints.ui_sso.get_async_httpx_client"
    ) as mock_client:
...
async def test_default_team_params(team_params):
    """
    When mishikallm.default_team_params is set, it should be used to create a new team
    """
    # Arrange
    mishikallm.default_team_params = team_params
    ...
    mock_prisma.db.mishikallm_teamtable.find_first = AsyncMock(return_value=None)
    mock_prisma.db.mishikallm_teamtable.create = AsyncMock()
    mock_prisma.get_data = AsyncMock(return_value=None)
    mock_prisma.jsonify_team_object = MagicMock(side_effect=mock_jsonify_team_object)

    with patch("mishikallm.proxy.proxy_server.prisma_client", mock_prisma):
        # Act
        ...
        mock_prisma.db.mishikallm_teamtable.create.assert_called_once()
        print(
            "mock_prisma.db.mishikallm_teamtable.create.call_args",
            mock_prisma.db.mishikallm_teamtable.create.call_args,
        )
        create_call_args = mock_prisma.db.mishikallm_teamtable.create.call_args.kwargs[
            "data"
        ]
...
async def test_create_team_without_default_params():
    """
    Test team creation when mishikallm.default_team_params is None
    Should create team with just the basic required fields
    """
    # Arrange
    mishikallm.default_team_params = None
...
async def test_get_user_info_from_db():
    """
    received args in get_user_info_from_db: {'result': CustomOpenID(id='krrishd', email='krrishdholakia@gmail.com', first_name=None, last_name=None, display_name='a3f1c107-04dc-4c93-ae60-7f32eb4b05ce', picture=None, provider=None, team_ids=[]), 'prisma_client': <mishikallm.proxy.utils.PrismaClient object at 0x14a74e3c0>, 'user_api_key_cache': <mishikallm.caching.dual_cache.DualCache object at 0x148d37110>, 'proxy_logging_obj': <mishikallm.proxy.utils.ProxyLogging object at 0x148dd9090>, 'user_email': 'krrishdholakia@gmail.com', 'user_defined_values': {'models': [], 'user_id': 'krrishd', 'user_email': 'krrishdholakia@gmail.com', 'max_budget': None, 'user_role': None, 'budget_duration': None}}
    """
    from mishikallm.proxy.management_endpoints.ui_sso import get_user_info_from_db
    ...
    with patch.object(
        mishikallm.proxy.management_endpoints.ui_sso, "get_user_object"
