...
async def test_ui_view_users_with_null_email(mocker, caplog):
    ...
    mock_prisma_client.db.mishikallm_usertable.find_many = mock_find_many

    # Patch the prisma client import in the endpoint
    mocker.patch("mishikallm.proxy.proxy_server.prisma_client", mock_prisma_client)
    ...
    assert response == [
        MishikaLLM_UserTableFiltered(user_id="test-user-null-email", user_email=None)
    ]
...
async def test_get_users_includes_timestamps(mocker):
    ...
    mock_prisma_client.db.mishikallm_usertable.find_many = mock_find_many
    mock_prisma_client.db.mishikallm_usertable.count = mock_count
    ...
    mocker.patch(
        "mishikallm.proxy.management_endpoints.internal_user_endpoints.get_user_key_counts",
