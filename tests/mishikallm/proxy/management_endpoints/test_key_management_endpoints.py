...
from mishikallm.proxy.management_endpoints.key_management_endpoints import _list_key_helper
...
async def test_list_keys():
    ...
    mock_prisma_client.db.mishikallm_verificationtoken.find_many = mock_find_many
    args = {
    ...
    assert json.dumps({"team_id": {"not": "mishikallm-dashboard"}}) in json.dumps(
        where_condition
    )
...
async def test_key_token_handling(monkeypatch):
    ...
    mock_insert_data = AsyncMock(
        return_value=MagicMock(token="hashed_token_123", mishikallm_budget_table=None)
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.mishikallm_verificationtoken = MagicMock()
    mock_prisma_client.db.mishikallm_verificationtoken.find_unique = AsyncMock(
        return_value=None
    )
    mock_prisma_client.db.mishikallm_verificationtoken.find_many = AsyncMock(
        return_value=[]
    )
    mock_prisma_client.db.mishikallm_verificationtoken.count = AsyncMock(return_value=0)
    mock_prisma_client.db.mishikallm_verificationtoken.update = AsyncMock(
        return_value=MagicMock(token="hashed_token_123", mishikallm_budget_table=None)
    )

    from mishikallm.proxy._types import GenerateKeyRequest, MishikallmUserRoles
    ...
    monkeypatch.setattr("mishikallm.proxy.proxy_server.prisma_client", mock_prisma_client)
