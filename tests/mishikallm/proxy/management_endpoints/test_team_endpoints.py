...
from mishikallm.proxy._types import UserAPIKeyAuth  # Import UserAPIKeyAuth
from mishikallm.proxy._types import MishikaLLM_TeamTable, MishikallmUserRoles
...
def mock_db_client():
    with patch(
        "mishikallm.proxy.proxy_server.prisma_client", mock_prisma_client
...
@pytest.fixture
def mock_admin_auth():
    mock_auth = UserAPIKeyAuth(user_role=MishikallmUserRoles.PROXY_ADMIN)
    return mock_auth
...
async def test_get_team_permissions_list_success(mock_db_client, mock_admin_auth):
    ...
    with patch(
        "mishikallm.proxy.management_endpoints.team_endpoints.get_team_object",
...
async def test_update_team_permissions_success(mock_db_client, mock_admin_auth):
    ...
    mock_existing_team_row = MagicMock(spec=MishikaLLM_TeamTable)
    ...
    mock_updated_team_row = MagicMock(spec=MishikaLLM_TeamTable)
    ...
    with patch(
        ...
        mock_db_client.db.mishikallm_teamtable.update = AsyncMock(
        ...
        mock_db_client.db.mishikallm_teamtable.update.assert_awaited_once_with(
