...
async def test_daily_spend_tracking_with_disabled_spend_logs():
    ...
    with patch("mishikallm.proxy.proxy_server.disable_spend_logs", True), patch(
        "mishikallm.proxy.proxy_server.prisma_client", MagicMock()
    ), patch("mishikallm.proxy.proxy_server.user_api_key_cache", MagicMock()), patch(
        "mishikallm.proxy.proxy_server.mishikallm_proxy_budget_name", "test-budget"
    ):
        # Test data
        test_data = {
            "token": "test-token",
