...
async def test_initialize_scheduled_jobs_credentials(monkeypatch):
    ...
    with patch("mishikallm.proxy.proxy_server.proxy_config", mock_proxy_config), patch(
        "mishikallm.proxy.proxy_server.store_model_in_db", False
    ...
    with patch("mishikallm.proxy.proxy_server.proxy_config", mock_proxy_config), patch(
        "mishikallm.proxy.proxy_server.store_model_in_db", True
    ), patch("mishikallm.proxy.proxy_server.get_secret_bool", return_value=True):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
            prisma_client=mock_prisma_client,
            proxy_budget_rescheduler_min_time=1,
...
@patch(
    "mishikallm.proxy.proxy_server.ProxyStartupEvent._setup_prisma_client",
...
async def test_aaaproxy_startup_master_key(mock_prisma, monkeypatch, tmp_path):
    ...
    from mishikallm.proxy.proxy_server import proxy_startup_event

    # Mock the Prisma import
    monkeypatch.setattr("mishikallm.proxy.proxy_server.PrismaClient", MockPrisma)
    ...
    async with proxy_startup_event(app):
        from mishikallm.proxy.proxy_server import master_key
    ...
    monkeypatch.setenv("MISHIKALLM_MASTER_KEY", test_env_master_key)
