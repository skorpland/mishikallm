...
from mishikallm.proxy._types import MishikaLLM_UserTableFiltered, UserAPIKeyAuth
from mishikallm.proxy.hooks import get_proxy_hook
from mishikallm.proxy.management_endpoints.internal_user_endpoints import ui_view_users
...
def setup_proxy_logging_object(monkeypatch, llm_router: Router) -> ProxyLogging:
    ...
    monkeypatch.setattr(
        "mishikallm.proxy.proxy_server.proxy_logging_obj", proxy_logging_object
    )
...
def test_mock_create_audio_file(mocker: MockerFixture, monkeypatch, llm_router: Router):
    ...
    mock_create_file = mocker.patch("mishikallm.files.main.create_file")
    ...
    monkeypatch.setattr("mishikallm.proxy.proxy_server.llm_router", llm_router)
    monkeypatch.setattr(
        "mishikallm.proxy.proxy_server.proxy_logging_obj", proxy_logging_obj
    )
...
def test_create_file_and_call_chat_completion_e2e(
    ...
    mock.start()
    try:
        from mishikallm.types.llms.openai import OpenAIFileObject
...
@pytest.mark.skip(reason="function migrated to mishikallm/proxy/hooks/managed_files.py")
def test_create_file_for_each_model(
    ...
    from mishikallm import CreateFileRequest
    ...
    from mishikallm.proxy.utils import ProxyLogging
    from mishikallm.types.llms.openai import OpenAIFileObject, OpenAIFilesPurpose
