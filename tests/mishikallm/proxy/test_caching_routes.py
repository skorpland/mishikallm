...
def mock_redis_success(mocker):
    ...
    mocker.patch.object(mishikallm, "cache", mock_cache)
    return mock_cache
...
def test_cache_ping_with_complex_objects(mock_redis_success, mocker):
    ...
    assert data["status"] == "healthy"
    assert "mishikallm_cache_params" in data

    # Verify complex objects were converted to strings
    cache_params = json.loads(data["mishikallm_cache_params"])
...
def test_cache_ping_failure(mock_redis_failure):
    ...
    assert "mishikallm_cache_params" in error_details
...
def test_cache_ping_no_cache_initialized():
    ...
    # Set cache to None
    original_cache = mishikallm.cache
    ...
    assert "message" in error
    error_details = json.loads(error["message"])
    assert "Cache not initialized. mishikallm.cache is None" in error_details["message"]

    # Restore original cache
    mishikallm.cache = original_cache


def test_cache_ping_health_check_includes_only_cache_attributes(mock_redis_success):
    """
    Ensure that the /cache/ping endpoint only pulls HealthCheckCacheParams from mishikallm.cache.cache,
    and not from other attributes on mishikallm.cache.
    """
    ...
    # The unrelated field we attached at the top-level of mishikallm.cache should *not* be present
    assert (
    ...
    ), "Expected field on `mishikallm.cache.cache` was not found in health_check_cache_params"
