...
from mishikallm.proxy.auth.auth_checks import ExperimentalUIJWTToken
from mishikallm.proxy.common_utils.encrypt_decrypt_utils import decrypt_value_helper


@pytest.fixture(autouse=True)
def set_salt_key(monkeypatch):
    """Automatically set MISHIKALLM_SALT_KEY for all tests"""
    monkeypatch.setenv("MISHIKALLM_SALT_KEY", "sk-1234")
...
def test_get_experimental_ui_login_jwt_auth_token_valid(valid_sso_user_defined_values):
    ...
    assert token_data["user_role"] == MishikallmUserRoles.PROXY_ADMIN.value
    assert token_data["models"] == ["gpt-3.5-turbo"]
    assert token_data["max_budget"] == mishikallm.max_ui_session_budget
...
def test_get_key_object_from_ui_hash_key_valid(
    ...
    assert key_object.user_role == MishikallmUserRoles.PROXY_ADMIN
    assert key_object.models == ["gpt-3.5-turbo"]
    assert key_object.max_budget == mishikallm.max_ui_session_budget
