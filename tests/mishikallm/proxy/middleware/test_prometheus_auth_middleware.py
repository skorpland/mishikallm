...
import mishikallm
from mishikallm.proxy._types import SpecialHeaders
...
def test_valid_auth_metrics(app_with_middleware, monkeypatch):
    ...
    mishikallm.require_auth_for_metrics_endpoint = True
    # Patch the auth function to simulate a valid authentication.
    monkeypatch.setattr(
        "mishikallm.proxy.middleware.prometheus_auth_middleware.user_api_key_auth",
        fake_valid_auth,
    )
...
def test_no_auth_metrics_when_disabled(app_with_middleware, monkeypatch):
    ...
    """
    mishikallm.require_auth_for_metrics_endpoint = False
