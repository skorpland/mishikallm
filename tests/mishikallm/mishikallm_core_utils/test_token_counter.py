#### What this tests ####
#    This tests mishikallm.token_counter.token_counter() function
import os
import sys
import time
import traceback
...
import mishikallm
from mishikallm import create_pretrained_tokenizer, decode, encode, get_modified_max_tokens
from mishikallm import token_counter as token_counter_old
...
def test_get_modified_max_tokens(
    ...
    mishikallm.token_counter.side_effect = _mock_token_counter
...
def test_gpt_4o_token_counter():
    with patch.object(
        mishikallm.utils, "openai_token_counter", new=MagicMock()
    ) as mock_client:
        token_counter(
...
def test_img_url_token_counter(img_url):
    from mishikallm.mishikallm_core_utils.token_counter import get_image_dimensions
...
from mishikallm.utils import _select_tokenizer_helper, claude_json_str, encoding


class TestTokenizerSelection(unittest.TestCase):
    @patch("mishikallm.utils.Tokenizer.from_pretrained")
    ...
    def test_cohere_tokenizer_api_failure(self, mock_from_pretrained):
        ...
        mishikallm.cohere_models = ["command-r-v1"]
    ...
    @patch("mishikallm.utils.Tokenizer.from_str")
    def test_claude_tokenizer_api_failure(self, mock_from_str):
        ...
        mishikallm.anthropic_models = ["claude-2"]
    ...
    @patch("mishikallm.utils._return_huggingface_tokenizer")
    def test_disable_hf_tokenizer_download(self, mock_return_huggingface_tokenizer):
        ...
        monkeypatch.setattr(mishikallm, "disable_hf_tokenizer_download", True)
