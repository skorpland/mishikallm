...
def test_routing_strategy_init(model_list):
    ...
    from mishikallm.types.router import RoutingStrategy
...
def test_print_deployment(model_list):
    ...
    assert 10 * "*" in printed_deployment["mishikallm_params"]["api_key"]
...
async def test_image_generation(model_list, sync_mode):
    ...
    from mishikallm.types.utils import ImageResponse
...
async def test_router_schedule_atext_completion(model_list):
    ...
    from mishikallm.types.utils import TextCompletionResponse
...
async def test_router_arealtime(model_list):
    ...
    router = Router(model_list=model_list)
    with patch.object(mishikallm, "_arealtime", AsyncMock()) as mock_arealtime:
...
def test_update_kwargs_with_default_mishikallm_params(model_list):
    """Test if the '_update_kwargs_with_default_mishikallm_params' function is working correctly"""
    router = Router(
        model_list=model_list,
        default_mishikallm_params={"api_key": "test", "metadata": {"key": "value"}},
    )
    kwargs: dict = {"metadata": {"key2": "value2"}}
    router._update_kwargs_with_default_mishikallm_params(kwargs=kwargs)
...
@pytest.mark.parametrize(
    ...
    [
        ("mock_testing_fallbacks", mishikallm.InternalServerError),
        ("mock_testing_context_fallbacks", mishikallm.ContextWindowExceededError),
        ("mock_testing_content_policy_fallbacks", mishikallm.ContentPolicyViolationError),
    ],
)
...
def test_handle_mock_testing_rate_limit_error(model_list):
    ...
    router = Router(model_list=model_list)
    with pytest.raises(mishikallm.RateLimitError):
        data = {
...
async def test_routing_strategy_pre_call_checks(model_list, sync_mode):
    ...
    if sync_mode:
        ...
        await router.async_routing_strategy_pre_call_checks(
            deployment, mishikallm_logging_obj
        )
        ...
        with patch.object(
            ...
            AsyncMock(
                side_effect=mishikallm.RateLimitError(
...
def test_create_deployment(
    ...
    if set_supported_environments:
        os.environ["MISHIKALLM_ENVIRONMENT"] = "staging"
...
def test_upsert_deployment(model_list):
    ...
    deployment.mishikallm_params.model = "gpt-4o"
...
@pytest.mark.parametrize(
    ...
    [
        (mishikallm.exceptions.BadRequestError, "BadRequestError", 3),
        (mishikallm.exceptions.AuthenticationError, "AuthenticationError", 4),
        (mishikallm.exceptions.RateLimitError, "RateLimitError", 6),
        (
            mishikallm.exceptions.ContentPolicyViolationError,
...
def test_get_num_retries_from_retry_policy(
    ...
    from mishikallm.router import RetryPolicy
...
def test_get_allowed_fails_from_policy(
    ...
    from mishikallm.types.router import AllowedFailsPolicy
...
def test_initialize_alerting(model_list):
    ...
    from mishikallm.types.router import AlertingConfig
...
def test_discard(model_list):
    ...
    mishikallm.service_callback = []

    router = Router(model_list=model_list)
    router.discard()

    # Verify all callback lists are empty
    assert len(mishikallm.callbacks) == 0
    assert len(mishikallm.success_callback) == 0
    assert len(mishikallm.failure_callback) == 0
    assert len(mishikallm._async_success_callback) == 0
    assert len(mishikallm._async_failure_callback) == 0
    assert len(mishikallm.input_callback) == 0
    assert len(mishikallm.service_callback) == 0
...
def test_pass_through_assistants_endpoint_factory(model_list):
    ...
    router._pass_through_assistants_endpoint_factory(
        original_function=mishikallm.acreate_assistants,
        custom_llm_provider="openai",
        client=None,
        **{},
    )
...
def test_factory_function(model_list):
    ...
    router = Router(model_list=model_list)
    router.factory_function(mishikallm.acreate_assistants)
...
def test_get_deployment_by_mishikallm_model(model_list):
    """Test if the 'get_deployment_by_mishikallm_model' function is working correctly"""
    router = Router(model_list=model_list)
    deployment = router._get_deployment_by_mishikallm_model(model="gpt-3.5-turbo")
    assert deployment is not None
...
#     from mishikallm.router_utils.pattern_match_deployments import PatternMatchRouter
#     import re
...
#         matched_pattern=match, mishikallm_deployment_mishikallm_model="openai/*"
#     )
#     assert updated_model == "openai/fo::hi:static::hello"


@pytest.mark.parametrize(
    "user_request_model, model_name, mishikallm_model, expected_model",
    [
...
def test_pattern_match_deployment_set_model_name(
    ...
    updated_model = pattern_router.set_deployment_model_name(match, mishikallm_model)
        ...
        deployments=[
            {
                ...
                "mishikallm_params": {"model": mishikallm_model},
            }
        ],
    )

    for model in updated_models:
        assert model["mishikallm_params"]["model"] == expected_model
...
async def test_pass_through_moderation_endpoint_factory(model_list):
    ...
    response = await router._pass_through_moderation_endpoint_factory(
        original_function=mishikallm.amoderation,
