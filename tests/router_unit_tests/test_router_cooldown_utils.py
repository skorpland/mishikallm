...
from mishikallm.router_utils.cooldown_callbacks import router_cooldown_event_callback
...
from mishikallm import Router
from mishikallm.router_utils.cooldown_handlers import _should_cooldown_deployment
...
async def test_router_cooldown_event_callback():
    ...
    mishikallm.callbacks = [prometheus_logger]

    await router_cooldown_event_callback(
        mishikallm_router_instance=mock_router,
    ...
    return Router(
        model_list=[
            ...
            },
            {
                ...
                "mishikallm_params": {"model": "openai/test_deployment"},
                "model_id": "test_deployment_2",
            },
            {
                "model_name": "test_deployment",
                "mishikallm_params": {"model": "openai/test_deployment-2"},
...
def test_should_run_cooldown_logic(testing_mishikallm_router):
    testing_mishikallm_router.disable_cooldowns = True
    # don't run cooldown logic if disable_cooldowns is True
    assert (
        _should_run_cooldown_logic(
            testing_mishikallm_router, "test_deployment", 500, Exception("Test")
        )
        is False
    )

    # don't cooldown if deployment is None
    testing_mishikallm_router.disable_cooldowns = False
    assert (
        _should_run_cooldown_logic(testing_mishikallm_router, None, 500, Exception("Test"))
        is False
    )

    # don't cooldown if it's a provider default deployment
    testing_mishikallm_router.provider_default_deployment_ids = ["test_deployment"]
    assert (
        _should_run_cooldown_logic(
            testing_mishikallm_router, "test_deployment", 500, Exception("Test")
        )
...
def test_should_cooldown_deployment_rate_limit_error(testing_mishikallm_router):
    """
    ...
    _exception = mishikallm.exceptions.RateLimitError(
        "Rate limit", "openai", "gpt-3.5-turbo"
    )
    assert (
        _should_cooldown_deployment(
            testing_mishikallm_router, "test_deployment", 429, _exception
        )
        is True
    )


def test_should_cooldown_deployment_auth_limit_error(testing_mishikallm_router):
    """
    ...
    _exception = mishikallm.exceptions.AuthenticationError(
        "Unauthorized", "openai", "gpt-3.5-turbo"
    )
    assert (
        _should_cooldown_deployment(
            testing_mishikallm_router, "test_deployment", 401, _exception
        )
        is True
    )
...
@pytest.mark.asyncio
async def test_should_cooldown_deployment(testing_mishikallm_router):
    """
    ...
    available_deployment = testing_mishikallm_router.get_available_deployment(
    ...
    for _ in range(40):
        increment_deployment_successes_for_current_minute(
            mishikallm_router_instance=testing_mishikallm_router, deployment_id=deployment_id
        )
    ...
    for _ in range(41):
        tasks.append(
            testing_mishikallm_router.acompletion(
                model=deployment_id,
                messages=[{"role": "user", "content": "Hello, world!"}],
                max_tokens=100,
                mock_response="mishikallm.InternalServerError",
            )
        )
    try:
    ...
    assert (
        _should_cooldown_deployment(
            testing_mishikallm_router, deployment_id, 500, Exception("Test")
        )
        is True
    )
...
def test_increment_deployment_successes_for_current_minute_does_not_write_to_redis(
    ...
    """
    from mishikallm.caching.dual_cache import DualCache
    from mishikallm.caching.redis_cache import RedisCache
    ...
    testing_mishikallm_router.cache = DualCache(
        redis_cache=mock_redis_cache, in_memory_cache=InMemoryCache()
    )

    # Call the function we're testing
    increment_deployment_successes_for_current_minute(
        mishikallm_router_instance=testing_mishikallm_router, deployment_id="test_deployment"
    )
    ...
    print(
        ...
        testing_mishikallm_router.cache.in_memory_cache.cache_dict,
    )
    assert (
        testing_mishikallm_router.cache.in_memory_cache.get_cache(
...
@patch(
    "mishikallm.router_utils.cooldown_handlers.get_deployment_successes_for_current_minute"
)
@patch(
    "mishikallm.router_utils.cooldown_handlers.get_deployment_failures_for_current_minute"
)
def test_should_cooldown_high_traffic_all_fails(mock_failures, mock_successes, router):
    # Simulate 10 failures, 0 successes
    from mishikallm.constants import SINGLE_DEPLOYMENT_TRAFFIC_FAILURE_THRESHOLD
