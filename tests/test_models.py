...
async def get_model_info(session, key, mishikallm_model_id=None):
    """
    Make sure only models user has access to are returned
    """
    if mishikallm_model_id:
        url = f"http://0.0.0.0:4000/model/info?mishikallm_model_id={mishikallm_model_id}"
    else:
...
async def test_get_specific_model():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for idx, m in enumerate(models):
            assert m == "gpt-4"
            mishikallm_model_id = response["data"][idx]["model_info"]["id"]
            model_specific_info = response["data"][idx]
        assert mishikallm_model_id is not None
        response = await get_model_info(
            session=session, key=key, mishikallm_model_id=mishikallm_model_id
        )
        assert response["data"][0]["model_info"]["id"] == mishikallm_model_id
        assert (
