...
def validate_standard_logging_payload(
    ...
    Args:
        ...
        response (dict): The mishikallm response to compare against
...
async def test_basic_openai_responses_api_streaming_with_logging():
    ...
    response = await mishikallm.aresponses(
        model=request_model,
        input="hi",
        stream=True,
    )
    ...
    async for event in response:
        ...
        print("mishikallm response=", json.dumps(event, indent=4, default=str))
...
def validate_responses_match(slp_response, mishikallm_response):
    """Validate that the standard logging payload OpenAI response matches the mishikallm response"""
    # Validate core fields
    assert slp_response["id"] == mishikallm_response["id"], "ID mismatch"
    assert slp_response["model"] == mishikallm_response["model"], "Model mismatch"
    assert (
        slp_response["created_at"] == mishikallm_response["created_at"]
    ), "Created at mismatch"

    # Validate usage
    assert (
        slp_response["usage"]["input_tokens"]
        == mishikallm_response["usage"]["input_tokens"]
    ), "Input tokens mismatch"
    assert (
        slp_response["usage"]["output_tokens"]
        == mishikallm_response["usage"]["output_tokens"]
    ), "Output tokens mismatch"
    assert (
        slp_response["usage"]["total_tokens"]
        == mishikallm_response["usage"]["total_tokens"]
    ), "Total tokens mismatch"

    # Validate output/messages
    assert len(slp_response["output"]) == len(
        mishikallm_response["output"]
    ), "Output length mismatch"
    for slp_msg, mishikallm_msg in zip(slp_response["output"], mishikallm_response["output"]):
        assert slp_msg["role"] == mishikallm_msg.role, "Message role mismatch"
        # Access the content's text field for the mishikallm response
        mishikallm_content = mishikallm_msg.content[0].text if mishikallm_msg.content else ""
        assert (
            slp_msg["content"][0]["text"] == mishikallm_content
        ), f"Message content mismatch. Expected {mishikallm_content}, Got {slp_msg['content']}"
        assert slp_msg["status"] == mishikallm_msg.status, "Message status mismatch"
...
async def test_basic_openai_responses_api_non_streaming_with_logging():
    ...
    print("mishikallm response=", json.dumps(response, indent=4, default=str))
...
def validate_stream_event(event):
    """
    Validate that a streaming event from mishikallm.responses() or mishikallm.aresponses()
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_openai_responses_mishikallm_router(sync_mode):
    """
    Test the OpenAI responses API with MishikaLLM Router in both sync and async modes
    """
    mishikallm._turn_on_debug()
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_openai_responses_mishikallm_router_streaming(sync_mode):
    """
    Test the OpenAI responses API with streaming through MishikaLLM Router
    """
    mishikallm._turn_on_debug()
    router = mishikallm.Router(
        model_list=[
...
@pytest.mark.asyncio
async def test_openai_responses_mishikallm_router_no_metadata():
    """
...
@pytest.mark.asyncio
async def test_openai_responses_mishikallm_router_with_metadata():
    """
...
def test_bad_request_bad_param_error():
    ...
    try:
        mishikallm.responses(model="gpt-4o", input="This should fail", temperature=2000)
...
async def test_openai_o1_pro_response_api(sync_mode):
    """
    Test that MishikaLLM correctly handles an incomplete response from OpenAI's o1-pro model
...
async def test_openai_o1_pro_response_api_streaming(sync_mode):
    ...
    with patch(
        ...
        if sync_mode:
            ...
            with patch(
                "mishikallm.llms.custom_httpx.http_handler.HTTPHandler.post",
                ...
                for event in response:
                    print(
                        f"Sync mishikallm response #{event_count}:",
            ...
            async for event in response:
                print(
                    f"Async mishikallm response #{event_count}:",
                    json.dumps(event, indent=4, default=str),
                )
                event_count += 1
...
def test_basic_computer_use_preview_tool_call():
    """
    Test that MishikaLLM correctly handles a computer_use_preview tool call where the environment is set to "linux"

    linux is an unsupported environment for the computer_use_preview tool, but mishikallm users should still be able to pass it to openai
    """
