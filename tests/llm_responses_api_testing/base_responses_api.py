...
def validate_responses_api_response(response, final_chunk: bool = False):
    """
    Validate that a response from mishikallm.responses() or mishikallm.aresponses()
...
class BaseResponsesAPITest(ABC):
    ...
    async def test_basic_openai_responses_api(self, sync_mode):
        ...
        try: 
            ...
            pytest.skip("Skipping test due to mishikallm.InternalServerError")
    ...
    async def test_basic_openai_responses_delete_endpoint(self, sync_mode):
        ...
        if sync_mode:
            ...
            if isinstance(response, ResponsesAPIResponse):
                await mishikallm.adelete_responses(
                    response_id=response.id,
                    **base_completion_call_args
                )
            else:
    ...
    async def test_basic_openai_responses_get_endpoint(self, sync_mode):
        ...
        if sync_mode:
            ...
            # get the response
            if isinstance(response, ResponsesAPIResponse):
                result = mishikallm.get_responses(
                    response_id=response.id,
                    **base_completion_call_args
                )
            ...
            if isinstance(response, ResponsesAPIResponse):
                result = await mishikallm.aget_responses(
                    response_id=response.id,
                    **base_completion_call_args
                )
                assert result is not None
    ...
    async def test_multiturn_responses_api(self):
        ...
        mishikallm.set_verbose = True
        base_completion_call_args = self.get_base_completion_call_args()
        response_1 = await mishikallm.aresponses(
        ...
        response_2 = await mishikallm.aresponses(
            input="Basic ping", 
