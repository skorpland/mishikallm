...
from mishikallm.router_utils.cooldown_handlers import _async_get_cooldown_deployments
...
async def test_dynamic_cooldowns():
    """
    Assert kwargs for completion/embedding have 'cooldown_time' as a mishikallm_param
    """
    # mishikallm.set_verbose = True
    tmp_mock = MagicMock()

    mishikallm.failure_callback = [tmp_mock]
    ...
    assert "cooldown_time" in tmp_mock.call_args[0][0]["mishikallm_params"]
    assert tmp_mock.call_args[0][0]["mishikallm_params"]["cooldown_time"] == 0
