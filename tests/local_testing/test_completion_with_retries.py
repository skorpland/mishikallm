...
import openai
import mishikallm
from mishikallm import completion_with_retries, completion, acompletion_with_retries
...
async def test_completion_with_retry_policy(sync_mode):
    ...
    from mishikallm.types.router import RetryPolicy
    ...
    with patch.object(mishikallm, target_function) as mock_completion_with_retries:
        data = {
...
async def test_completion_with_retries(sync_mode):
    """
    If completion_with_retries is called with num_retries=3, and max_retries=0, then mishikallm.completion should receive num_retries , max_retries=0
    """
    from unittest.mock import patch, MagicMock, AsyncMock
    ...
    with patch.object(mishikallm, target_function) as mock_completion:
        if sync_mode:
