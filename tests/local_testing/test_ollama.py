...
from mishikallm.utils import EmbeddingResponse, get_llm_provider, get_optional_params
...
@mock.patch(
    "mishikallm.llms.ollama.completion.handler.ollama_embeddings",
...
def test_ollama_embeddings(mock_embeddings):
    ...
    try:
        embeddings = mishikallm.embedding(
...
@mock.patch(
    "mishikallm.llms.ollama.completion.handler.ollama_aembeddings",
    return_value=mock_ollama_embedding_response,
)
def test_ollama_aembeddings(mock_aembeddings):
    ...
    try:
        embeddings = asyncio.run(
            mishikallm.aembedding(model="ollama/nomic-embed-text", input=["hello world"])
        )
        print(embeddings)
...
def test_ollama_ssl_verify():
    ...
    client: HTTPHandler = mishikallm.in_memory_llm_clients_cache.get_cache(
...
async def test_async_ollama_ssl_verify(stream):
    ...
    client: AsyncHTTPHandler = mishikallm.in_memory_llm_clients_cache.get_cache(
