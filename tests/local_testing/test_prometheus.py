...
from mishikallm.types.utils import BudgetConfig, GenericBudgetConfigType
...
async def prometheus_logger():
    ...
    with patch("mishikallm.proxy.proxy_server.premium_user", True):
...
async def test_async_prometheus_success_logging_with_callbacks(prometheus_logger):
    ...
    diff = await op()
    assert diff["mishikallm_requests_metric_total"] == 1.0
...
async def test_async_prometheus_budget_logging_with_callbacks(prometheus_logger):
    ...
    # TODO: Should implement `mishikallm_provider_remaining_budget_metric` in prometheus.py
    assert diff.get("mishikallm_provider_remaining_budget_metric", 50.0) == 50.0
