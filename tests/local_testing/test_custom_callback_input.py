...
## 1: Pre-API-Call
## 2: Post-API-Call
## 3: On MishikaLLM Call success
## 4: On MishikaLLM Call failure
## 5. Caching
...
# Test interfaces
## 1. mishikallm.completion() + mishikallm.embeddings()
...
class CompletionCustomHandler(
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        try:
            ...
            assert isinstance(kwargs["mishikallm_params"], dict)
            assert isinstance(kwargs["start_time"], (datetime, type(None)))
            assert isinstance(kwargs["stream"], bool)
            assert isinstance(kwargs["user"], (str, type(None)))
            ### METADATA
            metadata_value = kwargs["mishikallm_params"].get("metadata")
            assert metadata_value is None or isinstance(metadata_value, dict)
            if metadata_value is not None:
                if mishikallm.turn_off_message_logging is True:
                    assert (
                        metadata_value["raw_request"]
                        is MishikaLLMCommonStrings.redacted_by_mishikallm.value
                    )
                else:
        ...
        try:
            ...
            assert (
                isinstance(
                    ...
                    (str, mishikallm.CustomStreamWrapper, BaseModel),
                )
    ...
    async def async_log_stream_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            ## RESPONSE OBJECT
            assert isinstance(response_obj, mishikallm.ModelResponseStream)
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            ...
            assert (
                isinstance(
                    kwargs["original_response"], (str, mishikallm.CustomStreamWrapper)
                )
                or inspect.isasyncgen(kwargs["original_response"])
                or inspect.iscoroutine(kwargs["original_response"])
            )
        ...
        try:
            ...
            assert isinstance(
                ...
                (
                    ...
                    mishikallm.EmbeddingResponse,
                    mishikallm.ImageResponse,
                ),
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            ...
            assert isinstance(kwargs["optional_params"], dict)
            assert isinstance(kwargs["mishikallm_params"], dict)
            assert isinstance(kwargs["mishikallm_params"]["api_base"], str)
            ...
            assert isinstance(
                kwargs["original_response"],
                (str, mishikallm.CustomStreamWrapper, BaseModel),
            ), "Original Response={}. Allowed types=[str, mishikallm.CustomStreamWrapper, BaseModel]".format(
                kwargs["original_response"]
            )
            assert isinstance(kwargs["additional_args"], (dict, type(None)))
            assert isinstance(kwargs["log_event_type"], str)
    ...
    def log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            assert isinstance(kwargs["optional_params"], dict)
            assert isinstance(kwargs["mishikallm_params"], dict)
            assert isinstance(kwargs["mishikallm_params"]["metadata"], Optional[dict])
            assert isinstance(kwargs["start_time"], (datetime, type(None)))
            assert isinstance(kwargs["stream"], bool)
            assert isinstance(kwargs["user"], (str, type(None)))
            assert (
        ...
        try:
            ...
            assert isinstance(
                ...
                (
                    mishikallm.ModelResponse,
                    mishikallm.EmbeddingResponse,
                    mishikallm.TextCompletionResponse,
                ),
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)
...
async def test_async_embedding_openai():
    try:
        ...
        mishikallm.callbacks = [customHandler_success]
        response = await mishikallm.aembedding(
            model="azure/azure-embedding-model", input=["good morning from mishikallm"]
        )
        await asyncio.sleep(1)
        ...
        # test failure callback
        mishikallm.logging_callback_manager._reset_all_callbacks()
        mishikallm.callbacks = [customHandler_failure]
        try:
            response = await mishikallm.aembedding(
...
async def test_async_embedding_azure_caching():
    ...
    mishikallm.callbacks = [customHandler_caching]
    unique_time = time.time()
    response1 = await mishikallm.aembedding(
        model="azure/azure-embedding-model",
        input=[f"good morning from mishikallm1 {unique_time}"],
        caching=True,
    )
    await asyncio.sleep(1)  # set cache is async for aembedding()
    response2 = await mishikallm.aembedding(
        model="azure/azure-embedding-model",
        input=[f"good morning from mishikallm1 {unique_time}"],
        caching=True,
    )
...
def test_image_generation_openai():
    try:
        ...
        pass
    except mishikallm.ContentPolicyViolationError:
...
def test_standard_logging_payload(model, turn_off_message_logging):
    ...
    mishikallm.turn_off_message_logging = turn_off_message_logging
    ...
    with patch.object(
        ...
        if turn_off_message_logging:
            print("checks redacted-by-mishikallm")
            assert "redacted-by-mishikallm" == slobject["messages"][0]["content"]
            assert {"text": "redacted-by-mishikallm"} == slobject["response"]
...
def test_logging_async_cache_hit_sync_call(turn_off_message_logging):
    ...
    with patch.object(
        ...
        if turn_off_message_logging:
            ...
            assert {"text": "redacted-by-mishikallm"} == standard_logging_object[
                "response"
            ]
...
def test_logging_key_masking_gemini():
    ...
    with patch.object(
        ...
        assert (
            ...
            not in mock_client.call_args.kwargs["kwargs"]["mishikallm_params"]["api_base"]
        )
        key = mock_client.call_args.kwargs["kwargs"]["mishikallm_params"]["api_base"]
...
async def test_standard_logging_payload_stream_usage(sync_mode):
    ...
    """
    from mishikallm.types.utils import StandardLoggingPayload
    from mishikallm.main import stream_chunk_builder
...
def test_mishikallm_logging_no_log_param(monkeypatch, disable_no_log_param):
    monkeypatch.setattr(mishikallm, "global_disable_no_log_param", disable_no_log_param)
    ...
    should_run = mishikallm_logging_obj.should_run_callback(
        callback="langfuse",
        mishikallm_params={"no-log": True},
