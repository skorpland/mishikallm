...
def test_get_model_info_simple_model_name():
    ...
    mishikallm.get_model_info(model)
...
def test_get_model_info_custom_llm_with_same_name_vllm(monkeypatch):
    ...
    model_info = mishikallm.get_model_info(model, custom_llm_provider=provider)
    print("model_info", model_info)
    assert model_info["input_cost_per_token"] == 0.0


def test_get_model_info_shows_correct_supports_vision():
    info = mishikallm.get_model_info("gemini/gemini-1.5-flash")
    print("info", info)
...
def test_get_model_info_shows_assistant_prefill():
    ...
    info = mishikallm.get_model_info("deepseek/deepseek-chat")
    print("info", info)
...
def test_get_model_info_finetuned_models():
    info = mishikallm.get_model_info("ft:gpt-3.5-turbo:my-org:custom_suffix:id")
    print("info", info)
    assert info["input_cost_per_token"] == 0.000003


def test_get_model_info_gemini_pro():
    info = mishikallm.get_model_info("gemini-1.5-pro-002")
    print("info", info)
    assert info["key"] == "gemini-1.5-pro-002"


def test_get_model_info_ollama_chat():
    from mishikallm.llms.ollama.completion.transformation import OllamaConfig

    with patch.object(
        mishikallm.module_level_client,
        "post",
...
def test_get_model_info_gemini():
    ...
    model_map = mishikallm.model_cost
...
def test_get_model_info_bedrock_region():
    ...
    mishikallm.model_cost.pop("us.anthropic.claude-3-5-sonnet-20241022-v2:0", None)
    info = mishikallm.get_model_info(**args)
    print("info", info)
    assert info["key"] == "anthropic.claude-3-5-sonnet-20241022-v2:0"
    assert info["mishikallm_provider"] == "bedrock"
...
def test_get_model_info_completion_cost_unit_tests(model):
    info = mishikallm.get_model_info(model)
    print("info", info)
...
def test_get_whitelisted_models():
    ...
    Create whitelist to prevent naming regressions for older mishikallm versions.
    """
    whitelisted_models = []
    for model, info in mishikallm.model_cost.items():
        if info["mishikallm_provider"] == "bedrock" and info["mode"] == "chat":
...
def _enforce_bedrock_converse_models(
    ...
    for model, info in mishikallm.model_cost.items():
        if (
            info["mishikallm_provider"] == "bedrock"
            and info["mode"] == "chat"
            and model not in whitelist_models
        ):
            raise AssertionError(
                f"New bedrock chat model detected: {model}. Please set `mishikallm_provider='bedrock_converse'` for this model."
            )
...
def test_model_info_bedrock_converse(monkeypatch):
    ...
    _enforce_bedrock_converse_models(
        model_cost=mishikallm.model_cost, whitelist_models=whitelist_models
    )
...
def test_model_info_bedrock_converse_enforcement(monkeypatch):
    ...
    mishikallm.model_cost["fake.bedrock-chat-model"] = {
        "mishikallm_provider": "bedrock",
        "mode": "chat",
    }
...
def test_get_model_info_custom_provider():
    # Custom provider example copied from https://docs.21t.cc/docs/providers/custom_llm_server:
    import mishikallm
    from mishikallm import CustomLLM, completion, get_llm_provider

    class MyCustomLLM(CustomLLM):
        def completion(self, *args, **kwargs) -> mishikallm.ModelResponse:
    ...
    mishikallm.custom_provider_map = [  # 👈 KEY STEP - REGISTER HANDLER
    ...
    mishikallm.register_model(model_info)
...
def test_get_model_info_bedrock_models():
    ...
    for k, v in mishikallm.model_cost.items():
        if v["mishikallm_provider"] == "bedrock":
            k = k.replace("*/", "")
            ...
            base_model_info = mishikallm.model_cost[base_model]
...
def test_get_model_info_huggingface_models(monkeypatch):
    from mishikallm import Router
    from mishikallm.types.router import ModelGroupInfo
    ...
    info = mishikallm.get_model_info("huggingface/meta-llama/Meta-Llama-3-8B-Instruct")
    print("info", info)
    assert info is not None
    ...
    INTENDED_SCHEMA = {
        ...
        "additionalProperties": {
            ...
            "properties": {
                ...
                "mishikallm_provider": {"type": "string"},
