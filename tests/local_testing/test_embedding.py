...
import mishikallm
from mishikallm import completion, completion_cost, embedding
...
def test_openai_embedding():
    try:
        ...
        mishikallm_response = dict(response)
        mishikallm_response_keys = set(mishikallm_response.keys())
        mishikallm_response_keys.discard("_response_ms")

        print(mishikallm_response_keys)
        print("MishikaLLM Response\n")
        # print(mishikallm_response)
        ...
        assert (
            mishikallm_response_keys == openai_response_keys
        )  # ENSURE the Keys in mishikallm response is exactly what the openai package returns
        assert (
            len(mishikallm_response["data"]) == 2
        )  # expect two embedding responses from mishikallm_response since input had two
        print(openai_response_keys)
...
async def test_openai_azure_embedding_simple(model, api_base, api_key, sync_mode):
    try:
        ...
        assert set(["usage", "model", "object", "data"]) == set(
            response_keys
        )  # assert mishikallm response has expected keys from OpenAI embedding response

        request_cost = mishikallm.completion_cost(
            completion_response=response, call_type="embedding"
        )

        print("Calculated request cost=", request_cost)

        assert isinstance(response.usage, mishikallm.Usage)
...
async def test_azure_ai_embedding_image(model, api_base, api_key, sync_mode):
    try:
        ...
        request_cost = mishikallm.completion_cost(completion_response=response)
...
def test_openai_azure_embedding_with_oidc_and_cf():
    ...
    try:
        response = embedding(
            ...
            api_base="https://eastus2-mishikallm.openai.azure.com/",
...
async def test_cohere_embedding(sync_mode, model, api_base):
    try:
        ...
        if sync_mode:
            ...
            response = await mishikallm.aembedding(**data)
...
async def test_bedrock_embedding_titan(model, sync_mode):
    try:
        ...
        if sync_mode:
            response = embedding(
                model=model,
                input=f"good morning from mishikallm, attempting to embed data {current_time}",  # input should always be a string in this test
...
def test_bedrock_embedding_cohere():
    try:
        ...
        response = embedding(
            ...
            input=[
                "good morning from mishikallm, attempting to embed data",
...
def test_demo_tokens_as_input_to_embeddings_fails_for_titan():
    ...
    with pytest.raises(
        mishikallm.BadRequestError,
        match='mishikallm.BadRequestError: BedrockException - {"message":"Malformed input request: expected type: String, found: JSONArray, please reformat your input and try again."}',
    ):
        mishikallm.embedding(model="amazon.titan-embed-text-v1", input=[[1]])

    with pytest.raises(
        mishikallm.BadRequestError,
        match='mishikallm.BadRequestError: BedrockException - {"message":"Malformed input request: expected type: String, found: Integer, please reformat your input and try again."}',
    ):
...
def tgi_mock_post(*args, **kwargs):
    ...
    expected_data = {
        "inputs": {
            "source_sentence": "good morning from mishikallm",
...
@pytest.mark.asyncio
@patch("mishikallm.llms.huggingface.embedding.handler.async_get_hf_task_embedding_for_model")
@patch("mishikallm.llms.huggingface.embedding.handler.get_hf_task_embedding_for_model")
@pytest.mark.parametrize("sync_mode", [True, False])
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.parametrize(
    "input", ["good morning from mishikallm", ["good morning from mishikallm"]]  #
)
@pytest.mark.asyncio
...
def test_embedding_response_ratelimit_headers(model):
    ...
    # if upstream api returns this header, we need to verify the header is transformed by mishikallm
    ...
    with patch.object(client, "post") as mock_post:
        try:
            if sync_mode:
                ...
                await mishikallm.aembedding(**data)
        except Exception as e:
            print(e)
