...
import pytest
from mishikallm import acompletion, Cache
from mishikallm._service_logger import ServiceLogging
from mishikallm.integrations.prometheus_services import PrometheusServicesLogger
from mishikallm.proxy.utils import ServiceTypes
...
async def test_completion_with_caching():
    ...
    mishikallm.service_callback = ["prometheus_system"]

    sl = ServiceLogging(mock_testing=True)
    sl.prometheusServicesLogger.mock_testing = True
    mishikallm.cache.cache.service_logger_obj = sl
        ...
        model_list = [
            {
                ...
                "mishikallm_params": get_azure_params("gpt-4o-new-test"),
                "tpm": 100,
            },
            {
                "model_name": "azure/gpt-4",
...
def test_get_metric_existing():
    ...
    retrieved_metric = pl._get_metric("mishikallm_test_service_test_type_of_request")
...
def test_create_histogram_new():
    ...
    assert pl._get_metric("mishikallm_test_service_test_type_of_request") is hist
...
def test_create_histogram_existing():
    ...
    assert pl._get_metric("mishikallm_test_service_test_type_of_request") is hist1
...
def test_create_counter_new():
    ...
    assert pl._get_metric("mishikallm_test_service_test_type_of_request") is counter
...
def test_create_counter_existing():
    ...
    assert pl._get_metric("mishikallm_test_service_test_type_of_request") is counter1
