...
from mishikallm.integrations.gcs_bucket.gcs_bucket import (
    GCSBucketLogger,
...
async def test_aaabasic_gcs_logger():
    ...
    mishikallm.callbacks = [gcs_logger]
...
async def test_basic_gcs_logger_failure():
    ...
    try:
        response = await mishikallm.acompletion(
            ...
            max_tokens=10,
            user="ishaan-2",
            mock_response=mishikallm.BadRequestError(
                model="gpt-3.5-turbo",
...
async def test_basic_gcs_logging_per_request_with_callback_set():
    ...
    Request 2 - don't pass gcs_bucket_name in kwargs - ensure 'mishikallm-testing-bucket'
    """
    import logging
    from mishikallm._logging import verbose_logger
    ...
    GCS_BUCKET_NAME = "example-bucket-1-mishikallm"
    ...
    current_date = datetime.now().strftime("%Y-%m-%d")
    standard_callback_dynamic_params = StandardCallbackDynamicParams(
        gcs_bucket_name="mishikallm-testing-bucket"
    )
...
@pytest.mark.skip(reason="This test is flaky")
@pytest.mark.asyncio
async def test_basic_gcs_logging_per_request_with_no_mishikallm_callback_set():
    """
    Test GCS Bucket logging per request

    key difference: no mishikallm.callbacks set
...
async def test_basic_gcs_logger_with_folder_in_bucket_name():
    ...
    bucket_name = "mishikallm-testing-bucket/test-folder-logs"
...
def test_create_file_e2e():
    ...
    from mishikallm import create_file
