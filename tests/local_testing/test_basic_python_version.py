...
def test_using_mishikallm():
    try:
        import mishikallm

        print("mishikallm imported successfully")
    except Exception as e:
        pytest.fail(
            f"Error occurred: {e}. Installing mishikallm on python3.8 failed please retry"
        )


def test_mishikallm_proxy_server():
    # Install the mishikallm[proxy] package
    subprocess.run(["pip", "install", "mishikallm[proxy]"])

    # Import the proxy_server module
    try:
        import mishikallm.proxy.proxy_server
    except ImportError:
        pytest.fail("Failed to import mishikallm.proxy_server")
...
def test_package_dependencies():
    try:
        ...
        # Get the mishikallm package root path
        mishikallm_path = pathlib.Path(mishikallm.__file__).parent.parent
        pyproject_path = mishikallm_path / "pyproject.toml"
...
def test_mishikallm_proxy_server_config_no_general_settings():
    # Install the mishikallm[proxy] package
    # Start the server
    try:
        subprocess.run(["pip", "install", "mishikallm[proxy]"])
        subprocess.run(["pip", "install", "mishikallm[extra_proxy]"])
