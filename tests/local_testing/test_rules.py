...
def test_pre_call_rule():
    try:
        mishikallm.pre_call_rules = [my_pre_call_rule]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
...
def test_post_call_rule():
    try:
        mishikallm.pre_call_rules = []
        mishikallm.post_call_rules = [my_post_call_rule]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
        ...
        print(type(e))
        print(vars(e))
        assert e.message == "This violates MishikaLLM Proxy Rules. Response too short"
        pass
...
def test_post_call_rule_streaming():
    try:
        mishikallm.pre_call_rules = []
        mishikallm.post_call_rules = [my_post_call_rule_2]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "say sorry"}],
        ...
        print(type(e))
        print(vars(e))
        assert "This violates MishikaLLM Proxy Rules. Response too short" in e.message
