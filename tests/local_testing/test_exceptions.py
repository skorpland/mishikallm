...
import mishikallm
from mishikallm import (  # AuthenticationError,; RateLimitError,; ServiceUnavailableError,; OpenAIError,
    ContextWindowExceededError,
    completion,
    embedding,
)

mishikallm.vertex_project = "pathrise-convert-1606954137718"
...
async def test_content_policy_exception_azure():
    ...
    except mishikallm.ContentPolicyViolationError as e:
        print("caught a content policy violation error! Passed")
        print("exception", e)
        assert e.response is not None
        assert e.mishikallm_debug_info is not None
        assert isinstance(e.mishikallm_debug_info, str)
        assert len(e.mishikallm_debug_info) > 0
        pass
...
# for model in mishikallm.models_by_provider["bedrock"]:
...
def test_azure_embedding_exceptions():
    try:
        ...
        assert (
            e.message
            == "mishikallm.APIError: AzureException APIError - Embeddings.create() got an unexpected keyword argument 'messages'"
        )
...
def test_anthropic_openai_exception():
    # test if anthropic raises mishikallm.AuthenticationError
    try:
        mishikallm.set_verbose = True
        ## Test azure call
...
def test_completion_bedrock_invalid_role_exception():
    """
    Test if mishikallm raises a BadRequestError for an invalid role on Bedrock
    """
    try:
        mishikallm.set_verbose = True
        response = completion(
        ...
        #  mishikallm.BadRequestError: mishikallm.BadRequestError: Invalid Message passed in {'role': 'very-bad-role', 'content': 'hello'}

        # IMPORTANT ASSERTION
        assert (
            (str(e))
            == "mishikallm.BadRequestError: Invalid Message passed in {'role': 'very-bad-role', 'content': 'hello'}"
        )
...
def test_mishikallm_completion_vertex_exception():
    try:
        import mishikallm
...
def test_mishikallm_predibase_exception():
    """
...
def test_exception_mapping(provider):
    ...
    mishikallm.set_verbose = True
    error_map = {
        400: mishikallm.BadRequestError,
        401: mishikallm.AuthenticationError,
        404: mishikallm.NotFoundError,
        408: mishikallm.Timeout,
        429: mishikallm.RateLimitError,
        500: mishikallm.InternalServerError,
        503: mishikallm.ServiceUnavailableError,
    }
...
def _pre_call_utils(
    ...
    if call_type == "embedding":
        ...
        if sync_mode:
            original_function = mishikallm.embedding
        else:
            original_function = mishikallm.aembedding
        ...
        if sync_mode:
            original_function = mishikallm.completion
        else:
            original_function = mishikallm.acompletion
        ...
        if sync_mode:
            original_function = mishikallm.text_completion
        else:
            original_function = mishikallm.atext_completion
...
async def test_exception_with_headers(sync_mode, provider, model, call_type, streaming):
    """
    User feedback: mishikallm says "No deployments available for selected model, Try again in 60 seconds"
    but Azure says to retry in at most 9s

    ```
    {"message": "mishikallm.proxy.proxy_server.embeddings(): Exception occured - No deployments available for selected model, Try again in 60 seconds. Passed model=text-embedding-ada-002. pre-call-checks=False, allowed_model_region=n/a, cooldown_list=[('b49cbc9314273db7181fe69b1b19993f04efb88f2c1819947c538bac08097e4c', {'Exception Received': 'mishikallm.RateLimitError: AzureException RateLimitError - Requests to the Embeddings_Create Operation under Azure OpenAI API version 2023-09-01-preview have exceeded call rate limit of your current OpenAI S0 pricing tier. Please retry after 9 seconds. Please go here: https://aka.ms/oai/quotaincrease if you would like to further increase the default rate limit.', 'Status Code': '429'})]", "level": "ERROR", "timestamp": "2024-08-22T03:25:36.900476"}
    ```
    """
    ...
    if sync_mode:
        if provider == "openai":
            ...
            openai_client = openai.AzureOpenAI(
                api_key="", base_url="", api_version=mishikallm.AZURE_DEFAULT_API_VERSION
            )
    else:
        if provider == "openai":
    ...
    with patch.object(
        ...
        try:
            ...
            assert e.mishikallm_response_headers is not None
            assert int(e.mishikallm_response_headers["retry-after"]) == cooldown_time
...
def test_openai_gateway_timeout_error():
    ...
    try: 
        with patch.object(
            ...
            mishikallm.completion(model="openai/gpt-3.5-turbo", messages=[{"role": "user", "content": "Hello world"}], client=openai_client)
...
async def test_exception_with_headers_httpx(
    ...
    with patch.object(
        ...
        try:
            ...
            exception_raised = True
            assert (
                e.mishikallm_response_headers is not None
            ), "mishikallm_response_headers is None"
            print("e.mishikallm_response_headers", e.mishikallm_response_headers)
            assert int(e.mishikallm_response_headers["retry-after"]) == cooldown_time
...
async def test_bad_request_error_contains_httpx_response(model):
    ...
    except mishikallm.BadRequestError as e:
...
def test_exceptions_base_class():
    try:
        ...
        assert isinstance(e, mishikallm.RateLimitError)
        assert e.code == "429"
        assert e.type == "throttling_error"


def test_context_window_exceeded_error_from_mishikallm_proxy():
    from httpx import Response
    from mishikallm.mishikallm_core_utils.exception_mapping_utils import (
        extract_and_raise_mishikallm_exception,
    )

    args = {
        "response": Response(status_code=400, text="Bad Request"),
        "error_str": "Error code: 400 - {'error': {'message': \"mishikallm.ContextWindowExceededError: mishikallm.BadRequestError: this is a mock context window exceeded error\\nmodel=gpt-3.5-turbo. context_window_fallbacks=None. fallbacks=None.\\n\\nSet 'context_window_fallback' - https://docs.21t.cc/docs/routing#fallbacks\\nReceived Model Group=gpt-3.5-turbo\\nAvailable Model Group Fallbacks=None\", 'type': None, 'param': None, 'code': '400'}}",
        "model": "gpt-3.5-turbo",
        "custom_llm_provider": "mishikallm_proxy",
    }
    with pytest.raises(mishikallm.ContextWindowExceededError):
        extract_and_raise_mishikallm_exception(**args)
