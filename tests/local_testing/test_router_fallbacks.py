...
class MyCustomHandler(CustomLogger):
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        ...
        print(
            f"previous_models: {kwargs['mishikallm_params']['metadata'].get('previous_models', None)}"
        )
        self.previous_models = len(
            kwargs["mishikallm_params"]["metadata"].get("previous_models", [])
...
async def test_async_fallbacks_max_retries_per_request():
    mishikallm.set_verbose = False
    mishikallm.num_retries_per_request = 0
    model_list = [
        {  # list of model deployments
            "model_name": "azure/gpt-3.5-turbo",  # openai model name
            "mishikallm_params": {  # params for mishikallm completion/embedding call
        ...
        model_list = [
            {
                ...
                "mishikallm_params": get_azure_params("chatgpt-v-3"),
                "model_info": {"id": 1},
                "rpm": AZURE_FAST_RPM,
            },
            {
            ...
            },
            {
                ...
                "mishikallm_params": get_openai_params("gpt-3.5-turbo"),
                "model_info": {"id": 3},
                "rpm": OPENAI_RPM,
            },
            {
                "model_name": "anthropic-claude-3-5-haiku-20241022",
                "mishikallm_params": get_anthropic_params("claude-3-5-haiku-20241022"),
                "model_info": {"id": 4},
                "rpm": ANTHROPIC_RPM,
            },
        ]
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.parametrize("mishikallm_module_fallbacks", [True, False])
@pytest.mark.asyncio
async def test_default_model_fallbacks(sync_mode, mishikallm_module_fallbacks):
    ...
    """
    if mishikallm_module_fallbacks:
        mishikallm.default_fallbacks = ["my-good-model"]
    router = Router(
        model_list=[
            {
        ...
        default_fallbacks=(
            ["my-good-model"] if mishikallm_module_fallbacks is False else None
        ),
    )
...
async def test_router_content_policy_fallbacks(
    ...
    if content_filter_response_exception:
        ...
        mock_response = mishikallm.ModelResponse(
            choices=[mishikallm.Choices(finish_reason="content_filter")],
            model="gpt-3.5-turbo",
            usage=mishikallm.Usage(prompt_tokens=10, completion_tokens=0, total_tokens=10),
        )
    router = Router(
        model_list=[
            {
...
async def test_using_default_fallback(sync_mode):
    ...
    mishikallm.default_fallbacks = ["very-bad-model"]
    router = Router(
        model_list=[
            {
                "model_name": "openai/*",
...
async def test_using_default_working_fallback(sync_mode):
    ...
    verbose_logger.setLevel(logging.DEBUG)
    verbose_router_logger.setLevel(logging.DEBUG)
    mishikallm.default_fallbacks = ["openai/gpt-3.5-turbo"]
    router = Router(
        model_list=[
            {
                "model_name": "openai/*",
...
def test_router_fallbacks_with_custom_model_costs():
    ...
    model_info = mishikallm.get_model_info(model="claude-3-5-sonnet-20240620")

    print(f"key: {model_info['key']}")

    assert model_info["mishikallm_provider"] == "anthropic"
...
def test_router_fallbacks_with_model_id():
    router = Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
                "mishikallm_params": {"model": "gpt-3.5-turbo", "rpm": 1},
                "model_info": {
                    "id": "123",
                },
            }
...
def test_get_fallback_model_group():
    from mishikallm.router_utils.fallback_event_handlers import get_fallback_model_group
...
async def test_router_attempted_fallbacks_in_response(expected_attempted_fallbacks):
    ...
    - Test cases: works on first try, `x-mishikallm-attempted-fallbacks` is 0
    - Works on 1st fallback, `x-mishikallm-attempted-fallbacks` is 1
    - Works on 3rd fallback, `x-mishikallm-attempted-fallbacks` is 3
    """
    router = Router(
        model_list=[
            {
    ...
    if expected_attempted_fallbacks == 0:
        ...
        assert (
            resp._hidden_params["additional_headers"]["x-mishikallm-attempted-fallbacks"]
