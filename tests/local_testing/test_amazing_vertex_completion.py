...
from mishikallm.llms.vertex_ai.gemini.transformation import (
...
def test_avertex_ai():
    ...
    load_vertex_ai_credentials()
    test_models = (
        mishikallm.vertex_chat_models
        + mishikallm.vertex_code_chat_models
        + mishikallm.vertex_text_models
        + mishikallm.vertex_code_text_models
    )
    mishikallm.set_verbose = False
    vertex_ai_project = "pathrise-convert-1606954137718"

    test_models = random.sample(test_models, 1)
    test_models += mishikallm.vertex_language_models  # always test gemini-pro
    for model in test_models:
        try:
            ...
            assert response.choices[0].finish_reason in mishikallm._openai_finish_reasons
        except mishikallm.RateLimitError as e:
            pass
...
async def test_async_vertexai_response():
    ...
    for model in test_models:
        print(
            f"model being tested in async call: {model}, mishikallm.vertex_language_models: {mishikallm.vertex_language_models}"
        )
        if model in VERTEX_MODELS_TO_NOT_TEST or (
            "gecko" in model
            or "32k" in model
        ...
        except mishikallm.NotFoundError as e:
            pass
        except mishikallm.RateLimitError as e:
            pass
...
async def test_gemini_pro_json_schema_args_sent_httpx(
    ...
    with patch.object(client, "post", new=httpx_response) as mock_call:
        ...
        except mishikallm.JSONSchemaValidationError as e:
...
async def test_gemini_pro_function_calling_streaming(sync_mode):
    ...
    try:
        if sync_mode == True:
            ...
            for chunk in response:
                chunks.append(chunk)
                assert isinstance(chunk, mishikallm.ModelResponseStream)
        else:
            response = await mishikallm.acompletion(**data)
            print(f"completion: {response}")

            assert isinstance(response, mishikallm.CustomStreamWrapper)
        ...
        complete_response = mishikallm.stream_chunk_builder(chunks=chunks)
        assert (
...
async def test_vertexai_embedding(sync_mode):
    try:
        ...
        input_text = ["good morning from mishikallm", "this is another item"]
...
async def test_vertexai_multimodal_embedding():
    ...
    with patch(
        "mishikallm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post",
        return_value=mock_response,
    ) as mock_post:
        # Act: Call the mishikallm.aembedding function
        response = await mishikallm.aembedding(
...
async def test_completion_fine_tuned_model():
    ...
    with patch(
        ...
        # Act: Call the mishikallm.completion function
...
def mock_gemini_list_request(*args, **kwargs):
    from mishikallm.types.llms.vertex_ai import (
        CachedContent,
...
async def test_vertexai_embedding_finetuned(respx_mock: MockRouter):
    ...
    - Response is properly parsed into mishikallm's embedding response format
    """
    load_vertex_ai_credentials()
    mishikallm.set_verbose = True
    ...
    expected_request = {
        "instances": [
            {"inputs": "good morning from mishikallm"},
...
async def test_vertexai_model_garden_model_completion(
    ...
    assert response.choices[0].message.content.startswith(
        "Hello, my name is Mishikallm Bot"
    )
    assert response.choices[0].finish_reason == "stop"
...
def test_mishikallm_api_base(monkeypatch, provider, route):
    from mishikallm.llms.custom_httpx.http_handler import HTTPHandler
    ...
    monkeypatch.setattr(mishikallm, "api_base", "https://mishikallm.com")
    ...
    with patch.object(client, "post", new=MagicMock()) as mock_client:
        ...
        mock_client.assert_called()
        assert mock_client.call_args.kwargs["url"].startswith("https://mishikallm.com")
