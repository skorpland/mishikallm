...
from mishikallm.proxy.utils import InternalUsageCache, ProxyLogging, hash_token
...
async def test_pre_call_hook_tpm_limits():
    ...
    kwargs = {"mishikallm_params": {"metadata": {"user_api_key": _api_key}}}

    await parallel_request_handler.async_log_success_event(
        kwargs=kwargs,
        response_obj=mishikallm.ModelResponse(usage=mishikallm.Usage(total_tokens=10)),
        start_time="",
        end_time="",
    )
...
async def test_failure_call_hook():
    ...
    kwargs = {
        "mishikallm_params": {"metadata": {"user_api_key": _api_key}},
        "exception": Exception(),
    }
...
async def test_normal_router_call():
    ...
    pl._init_mishikallm_callbacks()
    print(f"mishikallm callbacks: {mishikallm.callbacks}")
...
async def test_pre_call_hook_tpm_limits_per_model():
    ...
    await parallel_request_handler.async_log_success_event(
        kwargs=kwargs,
        response_obj=mishikallm.ModelResponse(usage=mishikallm.Usage(total_tokens=11)),
        start_time="",
        end_time="",
    )
