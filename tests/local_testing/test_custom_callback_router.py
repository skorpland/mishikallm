...
import mishikallm
from mishikallm import Cache, Router
...
class CompletionCustomHandler(
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        try:
            ...
            assert isinstance(kwargs["mishikallm_params"]["metadata"], dict)
            assert isinstance(kwargs["mishikallm_params"]["metadata"]["model_group"], str)
            assert isinstance(kwargs["mishikallm_params"]["metadata"]["deployment"], str)
            assert isinstance(kwargs["mishikallm_params"]["model_info"], dict)
            assert isinstance(kwargs["mishikallm_params"]["model_info"]["id"], str)
            assert isinstance(
                kwargs["mishikallm_params"]["proxy_server_request"], (str, type(None))
            )
            assert isinstance(
                kwargs["mishikallm_params"]["preset_cache_key"], (str, type(None))
            )
            assert isinstance(kwargs["mishikallm_params"]["stream_response"], dict)
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            assert isinstance(end_time, datetime)
            ## RESPONSE OBJECT
            assert isinstance(response_obj, mishikallm.ModelResponse)
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            assert isinstance(kwargs["messages"], list) and isinstance(
                kwargs["messages"][0], dict
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            ## RESPONSE OBJECT
            assert isinstance(
                response_obj, (mishikallm.ModelResponse, mishikallm.EmbeddingResponse)
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)

            # checking we use base_model for azure cost calculation
            base_model = mishikallm.utils._get_base_model_from_metadata(
            ...
            if (
                ...
                # this checks response_cost == mishikallm.cost_per_token(model=base_model)
                ...
                prompt_price, completion_price = mishikallm.cost_per_token(
                    model=base_model,
...
async def test_async_chat_azure():
    try:
        ...
        mishikallm.callbacks = [customHandler_completion_azure_router]
        ...
        mishikallm.callbacks = [customHandler_streaming_azure_router]
...
async def test_async_embedding_azure():
    try:
        ...
        response = await router.aembedding(
            model="azure-embedding-model", input=["hello from mishikallm!"]
        )
        await asyncio.sleep(2)
...
async def test_async_chat_azure_with_fallbacks():
    try:
        ...
        mishikallm.callbacks = [customHandler_fallbacks]
        mishikallm.set_verbose = True
        # with fallbacks
        model_list = [
            {
        ...
        router = Router(
            ...
            retry_policy=mishikallm.router.RetryPolicy(
...
async def test_rate_limit_error_callback():
    ...
    Relevant issue: https://github.com/skorpland/mishikallm/issues/4096
    """
    ...
    mishikallm_logging_obj = MishikaLLMLogging(
        model="my-test-gpt",
    ...
    with patch.object(
        ...
        except (mishikallm.RateLimitError, ValueError):
            pass
