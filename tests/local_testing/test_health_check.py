...
async def test_azure_health_check():
    response = await mishikallm.ahealth_check(
        model_params={
...
def test_update_mishikallm_params_for_health_check():
    """
    Test if _update_mishikallm_params_for_health_check correctly:
    1. Updates messages with a random message
    2. Updates model name when health_check_model is provided
    """
    from mishikallm.proxy.health_check import _update_mishikallm_params_for_health_check
    ...
    updated_params = _update_mishikallm_params_for_health_check(model_info, mishikallm_params)
...
async def test_perform_health_check_with_health_check_model():
    ...
    """
    from mishikallm.proxy.health_check import _perform_health_check

    # Mock model list with health_check_model specified
    model_list = [
        {
            "mishikallm_params": {"model": "openai/*", "api_key": "fake-key"},
            "model_info": {
                "mode": "chat",
    ...
    async def mock_health_check(mishikallm_params, **kwargs):
        health_check_calls.append(mishikallm_params["model"])
        return {"status": "healthy"}

    with patch("mishikallm.ahealth_check", side_effect=mock_health_check):
...
async def test_health_check_bad_model():
    ...
    with patch(
        "mishikallm.ahealth_check", side_effect=mock_health_check
