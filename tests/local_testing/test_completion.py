...
from mishikallm.mishikallm_core_utils.prompt_templates.factory import anthropic_messages_pt
...
def test_response_model_none():
    ...
    print(f"x: {x}")
    assert isinstance(x, mishikallm.ModelResponse)
...
def _openai_mock_response(*args, **kwargs) -> mishikallm.ModelResponse:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "https://mishikallm8397336933.openai.azure.com/openai/deployments/gpt-4o/chat/completions?api-version=2023-03-15-preview",
    ],
)
...
async def test_completion_predibase():
    try:
        ...
        pass
    except mishikallm.ServiceUnavailableError as e:
        pass
    except mishikallm.InternalServerError:
        pass
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_completion_github_api():
    ...
    try:
        ...
        # Add any assertions, here to check response args
        print(response)
    except mishikallm.AuthenticationError:
        pass
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_completion_claude_3_empty_response():
    ...
    try:
        response = mishikallm.completion(model="claude-3-opus-20240229", messages=messages)
        print(response)
    except mishikallm.InternalServerError as e:
...
async def test_model_function_invoke(model, sync_mode, api_key, api_base):
    try:
        ...
        if sync_mode:
            response = mishikallm.completion(**data)
        else:
            response = await mishikallm.acompletion(**data)
...
def test_parse_xml_params():
    from mishikallm.mishikallm_core_utils.prompt_templates.factory import parse_xml_params
...
def test_completion_function_plus_image(model):
    ...
    image_content = [
        {"type": "text", "text": "What’s in this image?"},
        {
            "type": "image_url",
            "image_url": {
                "url": "https://mishikallm-listing.s3.amazonaws.com/mishikallm_logo.png"
            },
        },
    ]
...
def test_completion_openai_response_headers():
    """
    Tests if MishikaLLM reurns response hea
    """
    ...
    streaming_response = mishikallm.completion(
        model="gpt-4o-mini",
        messages=messages,
        stream=True,
    )
    ...
    # embedding
    embedding_response = mishikallm.embedding(
    ...
    mishikallm.return_response_headers = False
...
async def test_async_completion_openai_response_headers():
    ...
    streaming_response = await mishikallm.acompletion(
        model="gpt-4o-mini",
        messages=messages,
        stream=True,
    )
    ...
    # embedding
    embedding_response = await mishikallm.aembedding(
        model="text-embedding-ada-002",
        input="hello",
    )
...
def test_lm_studio_completion(monkeypatch):
    ...
    except mishikallm.AuthenticationError as e:
        pytest.fail(f"Error occurred: {e}")
    except mishikallm.APIError as e:
        print(e)
...
# this should throw an exception, to trigger https://logs.21t.cc/
...
def test_completion_openai():
    try:
        ...
        mishikallm.api_key = os.environ["OPENAI_API_KEY"]
        response = completion(
            model="gpt-3.5-turbo",
...
def test_completion_openai_pydantic(model, api_version):
    try:
        ...
        mishikallm.enable_json_schema_validation = True
        for _ in range(3):
            try:
                response = completion(
                    model=model,
                ...
                break
            except mishikallm.JSONSchemaValidationError:
...
def test_completion_openai_with_optional_params():
    ...
    try:
        ...
        print(response)
        mishikallm.success_callback = []  # unset callbacks
...
def test_completion_logprobs():
    """
    This function is used to test the mishikallm.completion logprobs functionality.
...
def test_completion_openai_mishikallm_key():
    try:
        mishikallm.set_verbose = True
        mishikallm.num_retries = 0
        mishikallm.api_key = os.environ["OPENAI_API_KEY"]
        ...
        ###### reset environ key
        os.environ["OPENAI_API_KEY"] = mishikallm.api_key

        ##### unset mishikallm var
        mishikallm.api_key = None
    except Timeout as e:
        pass
    except Exception as e:
...
def test_completion_ollama_hosted():
    try:
        ...
        mishikallm.request_timeout = None
        pass
    except Exception as e:
...
def test_completion_ollama_function_call(model):
    ...
    try:
        mishikallm.set_verbose = True
        response = mishikallm.completion(model=model, messages=messages, tools=tools)
        print(response)
...
async def test_completion_functions_param():
    ...
    try:
        ...
        with patch.object(client, "post", side_effect=gemini_mock_post) as mock_client:
            response: mishikallm.ModelResponse = await mishikallm.acompletion(
            ...
            assert (
                "mishikallm_param_is_function_call"
...
def test_completion_azure_extra_headers():
    ...
    with patch.object(http_client, "send", new=MagicMock()) as mock_client:
        mishikallm.client_session = http_client
        try:
            response = completion(
...
def test_azure_openai_ad_token():
    ...
    # we use mishikallm.input_callbacks for this test
    def tester(
        kwargs,  # kwargs to completion
    ):
        print("inside kwargs")
    ...
    mishikallm.input_callback = [tester]
    try:
        response = mishikallm.completion(
...
def test_completion_azure3():
    ...
    try:
        print("azure gpt-3.5 test\n\n")
        mishikallm.set_verbose = True
        mishikallm.api_base = os.environ["AZURE_API_BASE"]
        mishikallm.api_key = os.environ["AZURE_API_KEY"]
        mishikallm.api_version = os.environ["AZURE_API_VERSION"]
        ...
        os.environ["AZURE_API_BASE"] = mishikallm.api_base
        os.environ["AZURE_API_VERSION"] = mishikallm.api_version
        os.environ["AZURE_API_KEY"] = mishikallm.api_key
...
# new azure test for using mishikallm. vars,
# use the following vars in this test and make an azure_api_call
#  mishikallm.api_type = self.azure_api_type
#  mishikallm.api_base = self.azure_api_base
#  mishikallm.api_version = self.azure_api_version
#  mishikallm.api_key = self.api_key
def test_completion_azure_with_mishikallm_key():
    try:
        print("azure gpt-3.5 test\n\n")
        import openai

        #### set mishikallm vars
        mishikallm.api_type = "azure"
        mishikallm.api_base = os.environ["AZURE_API_BASE"]
        mishikallm.api_version = os.environ["AZURE_API_VERSION"]
        mishikallm.api_key = os.environ["AZURE_API_KEY"]
        ...
        ######### UNSET mishikallm vars
        mishikallm.api_type = None
        mishikallm.api_base = None
        mishikallm.api_version = None
        mishikallm.api_key = None
...
async def test_completion_replicate_llama3(sync_mode):
    ...
    try:
        ...
        # Add any assertions here to check the response
        assert isinstance(response, mishikallm.ModelResponse)
...
def test_replicate_custom_prompt_dict():
    ...
    try:
        ...
        pass
    except mishikallm.APIConnectionError as e:
        pass
    except Exception as e:
        pytest.fail(f"An exception occurred - {str(e)}")
    print(f"response: {response}")
    mishikallm.custom_prompt_dict = {}  # reset
...
def test_bedrock_deepseek_custom_prompt_dict():
    ...
    assert model in mishikallm.known_tokenizer_config
...
def test_customprompt_together_ai():
    try:
        ...
        print(mishikallm.success_callback)
        print(mishikallm._async_success_callback)
        response = completion(
            model="together_ai/mistralai/Mixtral-8x7B-Instruct-v0.1",
            messages=messages,
            roles={
        ...
        print(response)
    except mishikallm.exceptions.Timeout as e:
...
def response_format_tests(response: mishikallm.ModelResponse):
    ...
    assert isinstance(choice, mishikallm.Choices)
    assert isinstance(choice.get("index"), int)

    message = choice.get("message")
    assert isinstance(message, mishikallm.Message)
    ...
    assert isinstance(response.usage, mishikallm.Usage)  # type: ignore
...
#     model_name = "qvv0xeq"
#     mishikallm.api_base = "https://app.baseten.co"
#     try:
...
def test_completion_gemini(model):
    ...
    except mishikallm.RateLimitError:
        pass
    except mishikallm.APIError:
        pass
    except Exception as e:
...
async def test_acompletion_gemini():
    ...
    messages = [{"role": "user", "content": "Hey, how's it going?"}]
    try:
        response = await mishikallm.acompletion(model=model_name, messages=messages)
        # Add any assertions here to check the response
        print(f"response: {response}")
    except mishikallm.Timeout as e:
        pass
...
def test_unified_auth_params(provider, model, project, region_name, token):
    ...
    translated_optional_params = mishikallm.utils.get_optional_params(**data)

    if provider == "azure":
        special_auth_params = (
            mishikallm.AzureOpenAIConfig().get_mapped_special_auth_params()
        )
    elif provider == "bedrock":
        special_auth_params = (
            mishikallm.AmazonBedrockGlobalConfig().get_mapped_special_auth_params()
        )
    elif provider == "vertex_ai":
        special_auth_params = mishikallm.VertexAIConfig().get_mapped_special_auth_params()
    elif provider == "watsonx":
        special_auth_params = (
            mishikallm.IBMWatsonXAIConfig().get_mapped_special_auth_params()
        )
...
#     try:
#         mishikallm.config_path = "../config.json"
#         mishikallm.set_verbose = True
#         response = mishikallm.config_completion(messages=messages)
#         # Add any assertions here to check the response
#         print(response)
#         mishikallm.config_path = None
#     except Exception as e:
#         pytest.fail(f"Error occurred: {e}")
...
def test_completion_together_ai_stream():
    mishikallm.set_verbose = True
    user_message = "Write 1pg about YC & mishikallm"
    messages = [{"content": user_message, "role": "user"}]
    try:
        response = completion(
            model="together_ai/mistralai/Mixtral-8x7B-Instruct-v0.1",
...
def test_moderation():
    response = mishikallm.moderation(input="i'm ishaan cto of mishikallm")
    print(response)
    ...
    with patch.object(
        ...
        try:
            ...
            if sync_mode:
                ...
                _ = await mishikallm.acompletion(
                    model="azure/chatgpt-v2",
                    messages=[{"role": "user", "content": "Hello world"}],
                    client=client,
                    stream=stream,
...
def test_openai_hallucinated_tool_call_util(function_name, expect_modification):
    ...
    2. Iterate over tool_uses array to call functions:
        ...
        - parameters will be JSON object for function arguments
    """
    from mishikallm.utils import _handle_invalid_parallel_tool_calls
    from mishikallm.types.utils import ChatCompletionMessageToolCall
...
def test_provider_specific_header(custom_llm_provider, expected_result):
    from mishikallm.types.utils import ProviderSpecificHeader
    from mishikallm.llms.custom_httpx.http_handler import HTTPHandler
...
def test_completion_openai_metadata(monkeypatch, enable_preview_features):
    ...
    monkeypatch.setattr(mishikallm, "enable_preview_features", enable_preview_features)
    with patch.object(
