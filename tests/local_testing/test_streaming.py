...
from mishikallm.utils import ModelResponseListIterator
from mishikallm.types.utils import ModelResponseStream
...
def test_completion_azure_stream_content_filter_no_delta():
    ...
    try:
        ...
        chunk_list = []
        for chunk in chunks:
            new_chunk = mishikallm.ModelResponse(stream=True, id=chunk["id"])
            if "choices" in chunk and isinstance(chunk["choices"], list):
                new_choices = []
                for choice in chunk["choices"]:
                    if isinstance(choice, mishikallm.utils.StreamingChoices):
                        _new_choice = choice
                    elif isinstance(choice, dict):
                        _new_choice = mishikallm.utils.StreamingChoices(**choice)
...
def test_completion_azure_stream():
    try:
        ...
        for idx, init_chunk in enumerate(response):
            ...
            if finished:
                assert isinstance(init_chunk.choices[0], mishikallm.utils.StreamingChoices)
                break
        if complete_response.strip() == "":
            raise Exception("Empty response received")
    except Exception as e:
    ...
    try:
        ...
        if sync_mode:
            ...
            for idx, init_chunk in enumerate(response):
                ...
                if finished:
                    assert isinstance(
                        init_chunk.choices[0], mishikallm.utils.StreamingChoices
                    )
                    break
            if complete_response.strip() == "":
                raise Exception("Empty response received")
        ...
        pass
    except mishikallm.ServiceUnavailableError:
        pass
...
async def test_acompletion_claude_2_stream():
    try:
        ...
        response = await mishikallm.acompletion(
            model="claude-2.1",
            messages=[{"role": "user", "content": "hello from mishikallm"}],
            stream=True,
        )
        complete_response = ""
        # Add any assertions here to check the response
...
async def test_completion_gemini_stream(sync_mode):
    try:
        ...
        complete_response = mishikallm.stream_chunk_builder(
...
async def test_completion_replicate_llama3_streaming(sync_mode):
    ...
    try:
        if sync_mode:
            final_chunk: Optional[mishikallm.ModelResponse] = None
            response: mishikallm.CustomStreamWrapper = completion(  # type: ignore
                model=model_name,
                messages=messages,
            ...
            response: mishikallm.CustomStreamWrapper = await mishikallm.acompletion(  # type: ignore
                model=model_name,
                messages=messages,
            ...
            if complete_response.strip() == "":
                raise Exception("Empty response received")
    except mishikallm.UnprocessableEntityError as e:
        pass
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
async def test_parallel_streaming_requests(sync_mode, model):
    ...
    try:
        ...
        def sync_test_streaming():
            response: mishikallm.CustomStreamWrapper = mishikallm.completion(  # type: ignore
                model=model,
                messages=messages,
                stream=True,
                max_tokens=10,
...
def test_sagemaker_weird_response():
    ...
    try:
        ...
        from mishikallm.llms.sagemaker.completion.handler import TokenIterator
...
async def test_openai_stream_options_call(model, sync):
    mishikallm.enable_preview_features = True
    mishikallm.set_verbose = True
    usage = None
    chunks = []
    if sync:
    ...
    assert isinstance(last_chunk.usage, mishikallm.Usage)
...
def test_openai_stream_options_call_text_completion():
    ...
    for idx in range(3):
        try:
            response = mishikallm.text_completion(
...
def validate_first_function_call_chunk_structure(item):
    if not (isinstance(item, dict) or isinstance(item, mishikallm.ModelResponse)):
    ...
    for choice in item["choices"]:
        if not (
            ...
            or isinstance(choice, mishikallm.utils.StreamingChoices)
        ):
        ...
        if not (
            ...
            or isinstance(choice["delta"], mishikallm.utils.Delta)
        ):
            raise Exception(
...
def validate_final_function_call_chunk_structure(data):
    if not (isinstance(data, dict) or isinstance(data, mishikallm.ModelResponse)):
        raise Exception("Incorrect format")
    ...
    for choice in data["choices"]:
        if not (
            isinstance(choice, dict) or isinstance(choice["delta"], mishikallm.utils.Delta)
        ):
            raise Exception("Incorrect format")
        for key in required_keys_in_choices_array:
            if key not in choice:
...
def test_streaming_and_function_calling(model):
    ...
    try:
        # mishikallm.set_verbose = True
        response: mishikallm.CustomStreamWrapper = completion(
            model=model,
            tools=tools,
            messages=messages,
            stream=True,
...
def test_success_callback_streaming():
    ...
    mishikallm.success_callback = [success_callback]

    messages = [{"role": "user", "content": "hello"}]
    print("TESTING MISHIKALLM COMPLETION CALL")
    response = mishikallm.completion(
        model="gpt-3.5-turbo",
        messages=messages,
        stream=True,
...
def test_unit_test_custom_stream_wrapper():
    ...
    chunk = mishikallm.ModelResponse(**chunk, stream=True)
...
@pytest.mark.parametrize(
    "loop_amount",
    [
        mishikallm.REPEATED_STREAMING_CHUNK_LIMIT + 1,
        mishikallm.REPEATED_STREAMING_CHUNK_LIMIT - 1,
    ],
)
@pytest.mark.parametrize(
...
def test_unit_test_custom_stream_wrapper_repeating_chunk(
    ...
    if (loop_amount > mishikallm.REPEATED_STREAMING_CHUNK_LIMIT) and expected_chunk_fail:
        with pytest.raises(mishikallm.InternalServerError):
            for chunk in response:
                continue
    else:
        for chunk in response:
...
def test_unit_test_custom_stream_wrapper_openai():
    ...
    chunk = {
        ...
        "choices": [
            mishikallm.utils.StreamingChoices(
...
def test_aamazing_unit_test_custom_stream_wrapper_n():
    ...
    Relevant issue: https://github.com/skorpland/mishikallm/issues/3276
    """
    chunks = [
        {
...
def test_unit_test_custom_stream_wrapper_function_call():
    ...
    """
    from mishikallm.types.llms.openai import ChatCompletionDeltaChunk
    ...
    response = mishikallm.CustomStreamWrapper(
        ...
        model="gpt-3.5-turbo",
        custom_llm_provider="cached_response",
        logging_obj=mishikallm.mishikallm_core_utils.mishikallm_logging.Logging(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "Hey"}],
            stream=True,
            call_type="completion",
    ...
    initial_model_response = mishikallm.ModelResponse(
...
def test_mock_response_iterator_tool_use():
    """
    Relevant Issue: https://github.com/skorpland/mishikallm/issues/7364
    """
    from mishikallm.llms.bedrock.chat.invoke_handler import MockResponseIterator
    from mishikallm.types.utils import (
