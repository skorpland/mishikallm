...
from mishikallm.proxy.proxy_server import initialize_pass_through_endpoints
...
def client():
    ...
    remove_rerank_route(
        app=app
    )  # remove the native rerank route on the mishikallm proxy - since we're testing the pass through endpoints
...
async def test_pass_through_endpoint_no_headers(client, monkeypatch):
    ...
    general_settings: dict = (
        getattr(mishikallm.proxy.proxy_server, "general_settings", {}) or {}
    )
...
async def test_pass_through_endpoint_rpm_limit(
    ...
    from mishikallm.proxy.proxy_server import ProxyLogging, hash_token, user_api_key_cache
    ...
    proxy_logging_obj._init_mishikallm_callbacks()
...
async def test_aaapass_through_endpoint_pass_through_keys_langfuse(
    ...
    original_user_api_key_cache = getattr(
        mishikallm.proxy.proxy_server, "user_api_key_cache", None
    )
    original_master_key = getattr(mishikallm.proxy.proxy_server, "master_key", None)
    original_prisma_client = getattr(mishikallm.proxy.proxy_server, "prisma_client", None)
    original_proxy_logging_obj = getattr(
        mishikallm.proxy.proxy_server, "proxy_logging_obj", None
    )
    ...
    try:
            ...
            "batch": [
                {
                    ...
                    "body": {
                        ...
                        "name": "test-trace-mishikallm-proxy-passthrough",
                    },
        ...
        setattr(mishikallm.proxy.proxy_server, "general_settings", old_general_settings)
    finally:
        ...
        setattr(mishikallm.proxy.proxy_server, "master_key", original_master_key)
        setattr(mishikallm.proxy.proxy_server, "prisma_client", original_prisma_client)
        setattr(
            mishikallm.proxy.proxy_server, "proxy_logging_obj", original_proxy_logging_obj
        )
