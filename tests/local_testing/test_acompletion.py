...
def test_acompletion_params():
    ...
    if keys_acompletion != keys_completion:
        pytest.fail(
            "The parameters of the mishikallm.acompletion function and mishikallm.completion are not the same. "
...
async def test_langfuse_double_logging():
    ...
    resp = await mishikallm.acompletion(
        ...
        temperature=0.0,
        fallbacks=models[1:],
        # metadata={"generation_name": "test-gen", "project": "mishikallm-test"},
    )
    return resp
