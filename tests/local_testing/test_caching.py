...
from mishikallm.caching.redis_cluster_cache import RedisClusterCache
...
def test_caching_dynamic_args():  # test in memory cache
    try:
        ...
        mishikallm.cache = None  # disable cache
...
async def test_caching_with_cache_controls(sync_flag):
    try:
        ...
        if sync_flag:
            ...
            ## TTL = 0
            response1 = await mishikallm.acompletion(
                model="gpt-3.5-turbo",
                messages=messages,
                cache={"ttl": 0},
            ...
            response2 = await mishikallm.acompletion(
                model="gpt-3.5-turbo",
                messages=messages,
                cache={"s-maxage": 10},
                mock_response="Hello world",
...
def test_caching_with_models_v2():
    messages = [
        {"role": "user", "content": "who is ishaan CTO of mishikallm from mishikallm 2023"}
    ]
    mishikallm.cache = Cache()
...
def c():
    mishikallm.enable_caching_on_provider_specific_optional_params = True
    messages = [
        {"role": "user", "content": "who is ishaan CTO of mishikallm from mishikallm 2023"}
    ]
    mishikallm.cache = Cache()
    ...
    if (
        ...
        print(f"response2: {response2}")
        pytest.fail(f"Error occurred:")
    mishikallm.enable_caching_on_provider_specific_optional_params = False
...
async def test_embedding_caching_redis_ttl():
    ...
    with patch("redis.asyncio.Redis.pipeline", return_value=mock_pipeline):
        ...
        embedding_val_1 = await mishikallm.aembedding(
            model="azure/azure-embedding-model",
            input=inputs,
            encoding_format="base64",
        )
...
async def test_redis_cache_basic():
    ...
    mishikallm.cache.add_cache(result=response1, cache_key=cache_key)
    print(f"cache key pre async get: {cache_key}")
    stored_val = await mishikallm.cache.async_get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_batch_cache_write():
    ...
    response4 = await mishikallm.acompletion(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_cache_cluster_init_unit_test():
    try:
        ...
        resp = mishikallm.Cache(type="redis", redis_startup_nodes=startup_nodes)
...
async def test_redis_cache_cluster_init_with_env_vars_unit_test():
    try:
        ...
        resp = mishikallm.Cache(type="redis")
...
async def test_redis_cache_atext_completion():
    try:
        ...
        response1 = await mishikallm.atext_completion(
        ...
        response2 = await mishikallm.atext_completion(
            model="gpt-3.5-turbo-instruct", prompt=prompt, max_tokens=40, temperature=1
        )
...
async def test_redis_cache_acompletion_stream_bedrock():
    ...
    try:
        ...
        mishikallm.cache = Cache(type="redis")
        print("test for caching, streaming + completion")
        response_1_content = ""
        response_2_content = ""
...
async def test_s3_cache_stream_azure(sync_mode):
    try:
        ...
        mishikallm.cache = Cache(
            type="s3",
            s3_bucket_name="mishikallm-proxy",
...
async def test_s3_cache_acompletion_azure():
    ...
    try:
        ...
        mishikallm.cache = Cache(
            type="s3",
            s3_bucket_name="mishikallm-my-test-bucket-2",
...
def test_custom_redis_cache_with_key():
    ...
    mishikallm.cache.get_cache_key = custom_get_cache_key
    ...
    mishikallm.cache.cache.set_cache = set_cache
    mishikallm.cache.cache.get_cache = get_cache
...
def test_cache_override():
    # test if we can override the cache, when `caching=False` but mishikallm.cache = Cache() is set
...
def test_custom_redis_cache_params():
    ...
    try:
        ...
        print(mishikallm.cache.cache.redis_client)
        mishikallm.cache = None
        mishikallm.success_callback = []
        mishikallm._async_success_callback = []
    except Exception as e:
    ...
    try:
        ...
        cache_key = cache_instance.get_cache_key(
            **{
                ...
                "temperature": 0.2,
                "stream": True,
                "mishikallm_call_id": "ffe75e7e-8a07-431f-9a74-71a5b9f35f0b",
        ...
        embedding_cache_key_2 = cache_instance.get_cache_key(
            **{
                ...
                "mishikallm_call_id": "2642e009-b3cd-443d-b5dd-bb7d56123b0e",
                "mishikallm_logging_obj": "<mishikallm.utils.Logging object at 0x12f1bddb0>",
            }
        )
...
def test_cache_context_managers():
    ...
    assert mishikallm.cache == None
    assert "cache" not in mishikallm.success_callback
    assert "cache" not in mishikallm._async_success_callback
    ...
    assert mishikallm.cache != None
    assert mishikallm.cache.type == "redis"

    print("VARS of mishikallm.cache", vars(mishikallm.cache))
...
async def test_cache_default_off_acompletion():
    ...
    from mishikallm.caching.caching import CacheMode
    ...
    response3 = await mishikallm.acompletion(
        model="gpt-3.5-turbo",
        messages=[
            {
                "role": "user",
...
async def test_redis_sentinel_caching():
    ...
    mishikallm.cache.add_cache(result=response1, cache_key=cache_key)
    print(f"cache key pre async get: {cache_key}")
    stored_val = mishikallm.cache.get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
    ...
    stored_val_2 = await mishikallm.cache.async_get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_proxy_batch_redis_get_cache():
    ...
    from mishikallm.caching.caching import Cache, DualCache
    from mishikallm.proxy._types import UserAPIKeyAuth
    from mishikallm.proxy.hooks.batch_redis_get import _PROXY_BatchRedisRequests
...
async def test_logging_turn_off_message_logging_streaming(sync_mode):
    ...
    with patch.object(mock_obj, "add_cache") as mock_client, patch.object(
        ...
        if sync_mode is True:
            ...
            resp = await mishikallm.acompletion(
                model="gpt-3.5-turbo",
                messages=[{"role": "user", "content": "hi"}],
                mock_response="hello",
                stream=True,
...
def test_basic_caching_import():
    from mishikallm.caching import Cache
...
async def test_caching_kwargs_input(sync_mode):
    ...
    input = {
        ...
        "kwargs": {
            ...
            "caching": True,
            "mishikallm_call_id": "fae2aa4f-9f75-4f11-8c9c-63ab8d9fae26",
...
async def test_audio_caching(stream):
    mishikallm.cache = Cache(type="local")

    ## CALL 1 - no cache hit
    completion = await mishikallm.acompletion(
...
def test_redis_caching_default_ttl():
    ...
    mishikallm.default_redis_ttl = 120
...
async def test_redis_get_ttl():
    ...
    test that mishikallm redis caching wrapper handles -1 and -2 values and returns them as None
    """
    try:
        from mishikallm.caching.redis_cache import RedisCache
...
def test_redis_caching_multiple_namespaces():
    ...
    messages = [{"role": "user", "content": f"what is mishikallm? {uuid.uuid4()}"}]
    mishikallm.cache = Cache(type="redis")
    namespace_1 = "org-id1"
    namespace_2 = "org-id2"
