...
import mishikallm.cost_calculator
...
from mishikallm.types.utils import PromptTokensDetails
from mishikallm.mishikallm_core_utils.mishikallm_logging import CustomLogger
...
async def test_custom_pricing(sync_mode):
    ...
    mishikallm.callbacks = [new_handler]
    if sync_mode:
...
def test_custom_pricing_as_completion_cost_param():
    from mishikallm import Choices, Message, ModelResponse
    from mishikallm.utils import Usage
    ...
    cost = mishikallm.completion_cost(
...
def test_cost_ft_gpt_35():
    try:
        # this tests if mishikallm.completion_cost can calculate cost for ft:gpt-3.5-turbo:my-org:custom_suffix:id
        # it needs to lookup  ft:gpt-3.5-turbo in the mishikallm model_cost map to get the correct cost
        from mishikallm import Choices, Message, ModelResponse
        from mishikallm.utils import Usage
...
def test_cost_azure_gpt_35():
    try:
        # this tests if mishikallm.completion_cost can calculate cost for azure/chatgpt-deployment-2 which maps to azure/gpt-3.5-turbo
...
def test_cost_azure_embedding():
    try:
        ...
        cost = mishikallm.completion_cost(completion_response=response)
...
def test_cost_bedrock_pricing():
    ...
    mishikallm.set_verbose = True
    input_tokens = mishikallm.token_counter(
        model="bedrock/anthropic.claude-instant-v1",
        messages=[{"role": "user", "content": "Hey, how's it going?"}],
    )
    print(f"input_tokens: {input_tokens}")
    output_tokens = mishikallm.token_counter(
        model="bedrock/anthropic.claude-instant-v1",
...
def test_whisper_openai():
    ...
    cost = mishikallm.completion_cost(model="whisper-1", completion_response=transcription)

    print(f"cost: {cost}")
    print(f"whisper dict: {mishikallm.model_cost['whisper-1']}")
    expected_cost = round(
        mishikallm.model_cost["whisper-1"]["output_cost_per_second"]
...
def test_dalle_3_azure_cost_tracking():
    ...
    response = mishikallm.ImageResponse(
        created=1710265780,
        data=[
            {
                "b64_json": None,
...
def test_replicate_llama3_cost_tracking():
    ...
    mishikallm.register_model(
        {
            "replicate/meta/meta-llama-3-8b-instruct": {
                ...
                "mishikallm_provider": "replicate",
            }
        }
    )
    response = mishikallm.ModelResponse(
        id="chatcmpl-cad7282f-7f68-41e7-a5ab-9eb33ae301dc",
        choices=[
            mishikallm.utils.Choices(
                finish_reason="stop",
                index=0,
    ...
    cost = round(cost, 5)
    expected_cost = round(
        mishikallm.model_cost["replicate/meta/meta-llama-3-8b-instruct"][
...
def test_together_ai_qwen_completion_cost():
    input_kwargs = {
        "completion_response": mishikallm.ModelResponse(
            **{
    ...
    response = mishikallm.cost_calculator.get_model_params_and_category(
...
def test_gemini_completion_cost(above_128k, provider):
    ...
    if above_128k:
        ...
        prompt_tokens = 128.0
        output_tokens = 228.0
    ## GET MODEL FROM MISHIKALLM.MODEL_INFO
    model_info = mishikallm.get_model_info(model=model_name, custom_llm_provider=provider)
...
def test_vertex_ai_completion_cost():
    ...
    model_info = mishikallm.get_model_info(model="gemini-1.5-flash")
...
def test_vertex_ai_embedding_completion_cost(caplog):
    """
    Relevant issue - https://github.com/skorpland/mishikallm/issues/4630
    """
    os.environ["MISHIKALLM_LOCAL_MODEL_COST_MAP"] = "True"
    mishikallm.model_cost = mishikallm.get_model_cost_map(url="")
    ...
    model_info = mishikallm.get_model_info(model="vertex_ai/textembedding-gecko")
    ...
    for item in captured_logs:
        print("\nitem:{}\n".format(item))
        if (
            "mishikallm.mishikallm_core_utils.llm_cost_calc.google.cost_per_character(): Exception occured "
            in item
        ):
...
#     ## CALCULATED COST
#     resp = mishikallm.embedding(model="textembedding-gecko", input=[text])
...
@pytest.mark.parametrize("usage", ["mishikallm_usage", "openai_usage"])
def test_vertex_ai_mistral_predict_cost(usage):
    from mishikallm.types.utils import Choices, Message, ModelResponse, Usage

    if usage == "mishikallm_usage":
    ...
    response_object = ModelResponse(
        ...
        choices=[
            Choices(
                ...
                index=0,
                message=Message(
                    content="Hello! I'm Mishikallm Bot, your helpful assistant. While I can't provide real-time weather updates, I can help you find a reliable weather service or guide you on how to check the weather on your device. Would you like assistance with that?",
                    role="assistant",
                    tool_calls=None,
                    function_call=None,
                ),
...
def test_completion_cost_deepseek():
    ...
    except mishikallm.APIError as e:
        pass
    except Exception as e:
...
def test_completion_cost_azure_common_deployment_name():
    ...
    with patch.object(
        mishikallm.cost_calculator, "completion_cost", new=MagicMock()
    ) as mock_client:
        _ = mishikallm.response_cost_calculator(
            response_object=response,
            model="gpt-4-0314",
            custom_llm_provider="azure",
            call_type=CallTypes.acompletion.value,
...
def test_completion_cost_prompt_caching(model, custom_llm_provider):
    ...
    _model_info = mishikallm.get_model_info(
...
def test_completion_cost_databricks(model):
    ...
    resp = mishikallm.completion(model=model, messages=messages)  # works fine
...
def test_completion_cost_databricks_embedding(model):
    os.environ["MISHIKALLM_LOCAL_MODEL_COST_MAP"] = "True"
    mishikallm.model_cost = mishikallm.get_model_cost_map(url="")
    resp = mishikallm.embedding(model=model, input=["hey, how's it going?"])  # works fine

    print(resp)
    cost = completion_cost(completion_response=resp)


from mishikallm.llms.fireworks_ai.cost_calculator import get_base_model_for_pricing
...
def test_cost_azure_openai_prompt_caching():
    ...
    from mishikallm import get_model_info
...
def test_completion_cost_azure_ai_rerank(model):
    from mishikallm import RerankResponse, rerank
...
def test_together_ai_embedding_completion_cost():
    from mishikallm.utils import Choices, EmbeddingResponse, Message, ModelResponse, Usage
...
def test_completion_cost_params():
    """
    Relevant Issue: https://github.com/skorpland/mishikallm/issues/6133
    """
    mishikallm.set_verbose = True
...
def test_completion_cost_params_2():
    ...
    model_info = mishikallm.get_model_info("gemini-1.5-pro-002")
...
def test_completion_cost_params_gemini_3():
    ...
    from mishikallm.llms.vertex_ai.cost_calculator import cost_per_character
    ...
    model_info = mishikallm.get_model_info("gemini-1.5-flash")
...
async def test_test_completion_cost_gpt4o_audio_output_from_model(stream):
    ...
    model_info = mishikallm.get_model_info("gpt-4o-audio-preview-2024-10-01")
...
def test_completion_cost_model_response_cost(response_model, custom_llm_provider):
    """
    Relevant issue: https://github.com/skorpland/mishikallm/issues/6310
    """
    from mishikallm import ModelResponse
...
def test_completion_cost_azure_tts():
    ...
    mishikallm.response_cost_calculator(**args)


def test_select_model_name_for_cost_calc():
    from mishikallm.cost_calculator import _select_model_name_for_cost_calc
    from mishikallm.types.utils import ModelResponse, Choices, Usage, Message
...
def test_moderations():
    ...
    os.environ["MISHIKALLM_LOCAL_MODEL_COST_MAP"] = "True"
    mishikallm.model_cost = mishikallm.get_model_cost_map(url="")
    mishikallm.add_known_models()

    assert "omni-moderation-latest" in mishikallm.model_cost
    print(
        f"mishikallm.model_cost['omni-moderation-latest']: {mishikallm.model_cost['omni-moderation-latest']}"
    )
    assert "omni-moderation-latest" in mishikallm.open_ai_chat_completion_models
...
def test_cost_calculator_azure_embedding():
    from mishikallm.cost_calculator import response_cost_calculator
    from mishikallm.types.utils import EmbeddingResponse, Usage
...
def test_add_known_models():
    mishikallm.add_known_models()
    assert (
        "bedrock/us-west-1/meta.llama3-70b-instruct-v1:0" not in mishikallm.bedrock_models
    )
...
def test_bedrock_cost_calc_with_region():
    ...
    hidden_params = {
        ...
        "optional_params": {},
        "mishikallm_call_id": "cf371a5d-679b-410f-b862-8084676d6d59",
        "model_id": None,
        "api_base": None,
    ...
    bedrock_models = mishikallm.bedrock_models + mishikallm.bedrock_converse_models

    for model in bedrock_models:
        if mishikallm.model_cost[model]["mode"] == "chat":
            response = {
                "id": "cmpl-55db75e0b05344058b0bd8ee4e00bf84",
                "choices": [
                    {
...
@pytest.mark.parametrize("base_model_arg", ["mishikallm_param", "model_info"])
...
def test_cost_calculator_with_base_model_with_router(base_model_arg):
    ...
    if base_model_arg == "mishikallm_param":
        model_item["mishikallm_params"][
            "base_model"
...
def test_cost_calculator_with_base_model_with_router_embedding(base_model_arg):
    ...
    if base_model_arg == "mishikallm_param":
        model_item["mishikallm_params"]["base_model"] = "cohere.embed-english-v3"
    elif base_model_arg == "model_info":
        model_item["model_info"] = {
...
async def test_cost_calculator_with_custom_pricing_router(model_item, custom_pricing):
    from mishikallm import Router
    if custom_pricing == "mishikallm_params":
        model_item["mishikallm_params"]["input_cost_per_token"] = 0.0000008
        model_item["mishikallm_params"]["output_cost_per_token"] = 0.0000032
...
def test_json_valid_model_cost_map():
    ...
    model_cost = mishikallm.get_model_cost_map(url="")
