...
async def test_router_retry_policy(error_type):
    from mishikallm.router import AllowedFailsPolicy, RetryPolicy
...
def test_no_retry_for_not_found_error_404():
    ...
    # Act & Assert
    error = mishikallm.NotFoundError(
...
internal_server_error = mishikallm.InternalServerError(
...
service_unavailable_error = mishikallm.ServiceUnavailableError(
    message="service unavailable error",
    model="gpt-12",
    llm_provider="azure",
)

timeout_error = mishikallm.Timeout(
