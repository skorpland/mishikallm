...
from mishikallm.proxy.proxy_server import embeddings
from mishikallm.proxy.utils import ProxyLogging, hash_token
...
async def test_moderations_on_embeddings():
    try:
        ...
        setattr(mishikallm.proxy.proxy_server, "llm_router", temp_router)

        api_route = APIRoute(path="/embeddings", endpoint=embeddings)
        mishikallm.callbacks = [lakeraAI_Moderation()]
        request = Request(
            {
                "type": "http",
                "route": api_route,
...
@pytest.mark.asyncio
@patch("mishikallm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post")
@patch(
    "mishikallm.guardrail_name_config_map",
    new=make_config_map(
        {
...
@patch("mishikallm.add_function_to_prompt", False)
@pytest.mark.skip(reason="lakera deprecated their v1 endpoint.")
...
async def test_callback_specific_param_run_pre_call_check_lakera():
    ...
    import mishikallm
    from mishikallm.proxy.guardrails.guardrail_hooks.lakera_ai import lakeraAI_Moderation
    from mishikallm.proxy.guardrails.init_guardrails import initialize_guardrails
    from mishikallm.types.guardrails import GuardrailItem, GuardrailItemSpec
    ...
    mishikallm_settings = {"guardrails": guardrails_config}

    assert len(mishikallm.guardrail_name_config_map) == 0
    ...
    assert len(mishikallm.guardrail_name_config_map) == 1

    prompt_injection_obj: Optional[lakeraAI_Moderation] = None
    print("mishikallm callbacks={}".format(mishikallm.callbacks))
