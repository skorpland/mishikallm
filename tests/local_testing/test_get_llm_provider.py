...
def test_get_llm_provider():
    _, response, _, _ = mishikallm.get_llm_provider(model="anthropic.claude-v2:1")
...
def test_get_llm_provider_catch_all():
    _, response, _, _ = mishikallm.get_llm_provider(model="*")
    assert response == "openai"


def test_get_llm_provider_gpt_instruct():
    _, response, _, _ = mishikallm.get_llm_provider(model="gpt-3.5-turbo-instruct-0914")

    assert response == "text-completion-openai"


def test_get_llm_provider_mistral_custom_api_base():
    model, custom_llm_provider, dynamic_api_key, api_base = mishikallm.get_llm_provider(
...
def test_default_api_base():
    ...
    with patch.dict(os.environ, {}, clear=True):
        for provider in mishikallm.openai_compatible_providers:
            ...
            for other_provider in mishikallm.provider_list:
