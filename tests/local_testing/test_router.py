...
import mishikallm.types.router
...
from mishikallm.router import Deployment, MishikaLLM_Params
from mishikallm.types.router import ModelInfo
...
def test_router_deployment_typing():
    deployment_typed_dict = DeploymentTypedDict(
        model_name="hi", mishikallm_params={"model": "hello-world"}
    )
...
def test_router_azure_ad_token_provider():
    ...
    for azure_cred in ["DefaultAzureCredential", "AzureCliCredential"]:
        ...
        mishikallm.enable_azure_ad_token_refresh = True
        router = Router(model_list=[_deployment])
...
async def test_router_retries(sync_mode):
    ...
    model_list = [
        {
            "model_name": "gpt-3.5-turbo",
            "mishikallm_params": {"model": "gpt-3.5-turbo", "api_key": "bad-key"},
        },
        {
            "model_name": "gpt-3.5-turbo",
            "mishikallm_params": {
    ...
    try:
        ...
        async def embedding_call():
            ...
            response = await router._aembedding(
                model="text-embedding-ada-002",
                input=["good morning from mishikallm 2"],
            )
            print(response)
            router.reset()
...
def test_azure_embedding_on_router():
    ...
    try:
        ...
        async def embedding_call():
            response = await router.aembedding(
                model="text-embedding-ada-002", input=["good morning from mishikallm"]
            )
            print(response)
        ...
        response = router.embedding(
            model="text-embedding-ada-002",
            input=["test 2 from mishikallm. async embedding"],
        )
        print(response)
        router.reset()
    except Exception as e:
        ...
        async def test():
            ...
            response = await router.acompletion(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "hello from mishikallm test",
                    }
                ],
            )
            print(response)
    ...
    try:
        ...
        async def test():
            ...
            response = await router.acompletion(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": f"hello from mishikallm test {time.time()}",
                    }
                ],
                stream=True,
            )
        ...
        response = router.completion(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "hello from mishikallm test2",
                }
            ],
        )
        print(response)
        ...
        response = router.completion(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "hello from mishikallm test3",
                }
            ],
            stream=True,
        )
        ...
        model_list=[
            {
                ...
                "mishikallm_params": {"model": "cohere.command-r-plus-v1:0"},
            }
        ]
    )
...
def test_consistent_model_id():
    """
    - For a given model group + mishikallm params, assert the model id is always the same
    ...
    id1 = Router()._generate_model_id(
        model_group=model_group, mishikallm_params=mishikallm_params
    )
...
def test_reading_keys_os_environ():
    ...
    try:
        ...
        for model in router.model_list:
            assert (
                model["mishikallm_params"]["api_key"] == os.environ["AZURE_API_KEY"]
            ), f"{model['mishikallm_params']['api_key']} vs {os.environ['AZURE_API_KEY']}"
            assert (
                model["mishikallm_params"]["api_base"] == os.environ["AZURE_API_BASE"]
            ), f"{model['mishikallm_params']['api_base']} vs {os.environ['AZURE_API_BASE']}"
            assert (
                model["mishikallm_params"]["api_version"]
                == os.environ["AZURE_API_VERSION"]
            ), f"{model['mishikallm_params']['api_version']} vs {os.environ['AZURE_API_VERSION']}"
            assert float(model["mishikallm_params"]["timeout"]) == float(
                os.environ["AZURE_TIMEOUT"]
            ), f"{model['mishikallm_params']['timeout']} vs {os.environ['AZURE_TIMEOUT']}"
            assert float(model["mishikallm_params"]["stream_timeout"]) == float(
                os.environ["AZURE_STREAM_TIMEOUT"]
            ), f"{model['mishikallm_params']['stream_timeout']} vs {os.environ['AZURE_STREAM_TIMEOUT']}"
            assert int(model["mishikallm_params"]["max_retries"]) == int(
                os.environ["AZURE_MAX_RETRIES"]
            ), f"{model['mishikallm_params']['max_retries']} vs {os.environ['AZURE_MAX_RETRIES']}"
...
def test_reading_openai_keys_os_environ():
    ...
    try:
        ...
        for model in router.model_list:
            assert (
                model["mishikallm_params"]["api_key"] == os.environ["OPENAI_API_KEY"]
            ), f"{model['mishikallm_params']['api_key']} vs {os.environ['AZURE_API_KEY']}"
            assert float(model["mishikallm_params"]["timeout"]) == float(
                os.environ["AZURE_TIMEOUT"]
            ), f"{model['mishikallm_params']['timeout']} vs {os.environ['AZURE_TIMEOUT']}"
...
def test_router_add_deployment():
    ...
    router.add_deployment(
        deployment=Deployment(
            ...
            mishikallm_params=MishikaLLM_Params(model="gpt-3.5-turbo-instruct"),
        ...
        for _ in range(300):
            tasks.append(
                router.atext_completion(
                    model="fake-openai-endpoint",
                    prompt="hello from mishikallm test",
                )
            )
...
@pytest.fixture
def mock_response() -> mishikallm.ModelResponse:
    return mishikallm.ModelResponse(
        **{
            "id": "chatcmpl-abc123",
...
async def test_router_model_usage(mock_response):
    ...
    setattr(
        mock_response,
        "usage",
        mishikallm.Usage(prompt_tokens=5, completion_tokens=5, total_tokens=10),
    )
...
def test_router_context_window_pre_call_check(model, base_model, llm_provider):
    ...
    try:
        ...
        mishikallm.token_counter = MagicMock()

        def token_counter_side_effect(*args, **kwargs):
            # Process args and kwargs if needed
            return 1000000

        mishikallm.token_counter.side_effect = token_counter_side_effect
        try:
...
def test_router_cooldown_api_connection_error():
    from mishikallm.router_utils.cooldown_handlers import _is_cooldown_required
    ...
    try:
        ...
        assert (
            _is_cooldown_required(
                mishikallm_router_instance=Router(),
                model_id="",
        ...
        model_list=[
            {
                ...
                "mishikallm_params": {"model": "vertex_ai/gemini-1.5-pro"},
            }
        ]
    )
    ...
    except mishikallm.APIConnectionError:
        pass
        ...
        model_list=[
            {
                ...
                "mishikallm_params": {
                    "model": "vertex_ai/gemini-1.5-pro",
                    "mock_response": "mishikallm.RateLimitError",
                },
            }
        ]
    )
    ...
    with patch.object(
        ...
        if sync_mode:
            cooldown_deployments = _get_cooldown_deployments(
                mishikallm_router_instance=router, parent_otel_span=None
            )
        else:
        ...
        except mishikallm.types.router.RouterRateLimitError as e:
            print(e)
...
def test_model_group_alias(hidden):
    _model_list = [
        ...
        {"model_name": "gpt-4", "mishikallm_params": {"model": "gpt-4"}},
    ]
    router = Router(
...
def test_get_team_specific_model():
    ...
    deployment = DeploymentTypedDict(
        model_name="model-z", mishikallm_params={}, model_info=ModelInfo()
    )
    assert router._get_team_specific_model(deployment, "team1") is None
...
def test_router_dynamic_credentials():
    ...
    assert deployment is not None
    assert deployment.mishikallm_params.api_key == original_api_key
