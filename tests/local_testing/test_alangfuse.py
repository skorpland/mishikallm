...
def langfuse_client():
    ...
    _cached_client = mishikallm.in_memory_llm_clients_cache.get_cache(_langfuse_cache_key)
    if _cached_client:
        ...
        mishikallm.in_memory_llm_clients_cache.set_cache(
...
def create_async_task(**completion_kwargs):
    """
    Creates an async task for the mishikallm.acompletion function.
    ...
    Any kwargs passed to this function will be passed to the mishikallm.acompletion function.
    By default a standard set of arguments are used for the mishikallm.acompletion function.
    """
    completion_args = {
    ...
    return asyncio.create_task(mishikallm.acompletion(**completion_args))
...
async def test_langfuse_logging_without_request_response(stream, langfuse_client):
    try:
        ...
        _unique_trace_name = f"mishikallm-test-{str(uuid.uuid4())}"
        mishikallm.set_verbose = True
        ...
        assert _trace_data[0].input == {
            "messages": [{"content": "redacted-by-mishikallm", "role": "user"}]
        }
        assert _trace_data[0].output == {
            "role": "assistant",
            "content": "redacted-by-mishikallm",
            "function_call": None,
            "tool_calls": None,
        }
...
async def test_langfuse_logging_audio_transcriptions(langfuse_client):
    ...
    mishikallm.set_verbose = True
    mishikallm.success_callback = ["langfuse"]
    await mishikallm.atranscription(
        model="whisper-1",
        file=audio_file,
        metadata={
    ...
    assert generations[0].name == "mishikallm-atranscription"
...
async def test_langfuse_masked_input_output(langfuse_client):
    ...
    for mask_value in [True, False]:
        ...
        print(response)
        expected_input = "redacted-by-mishikallm" if mask_value else "This is a test"
        expected_output = (
            "redacted-by-mishikallm" if mask_value else "This is a test response"
        )
        langfuse_client.flush()
...
async def test_aaalangfuse_logging_metadata(langfuse_client):
    ...
    session_id = f"mishikallm-test-session-{run_id}"
    trace_common_metadata = {
        "session_id": session_id,
        "tags": ["mishikallm-test-tag1", "mishikallm-test-tag2"],
        "update_trace_keys": [
            "output",
            "trace_metadata",
        ...
        "trace_release": "mishikallm-test-release",
        "version": "mishikallm-test-version",
    }
    for trace_num in range(1, 3):  # Two traces
        metadata = copy.deepcopy(trace_common_metadata)
        trace_id = f"mishikallm-test-trace{trace_num}-{run_id}"
        ...
        for generation_num in range(
            ...
            metadata["trace_user_id"] = f"mishikallm-test-user{generation_num}-{run_id}"
            generation_id = (
                f"mishikallm-test-trace{trace_num}-generation-{generation_num}-{run_id}"
            )
...
def test_aaalangfuse_existing_trace_id():
    ...
    response_obj = mishikallm.ModelResponse(
        ...
        usage=mishikallm.Usage(completion_tokens=37, prompt_tokens=14, total_tokens=51),
    )
    ...
    langfuse_args = {
        ...
        "user_id": None,
        "print_verbose": mishikallm.print_verbose,
        "level": "DEFAULT",
        "status_message": None,
    }
    ...
    new_response_obj = mishikallm.ModelResponse(
        id="chatcmpl-9K5HUAbVRqFrMZKXL0WoC295xhguY",
        choices=[
            mishikallm.Choices(
                finish_reason="stop",
    ...
    langfuse_args = {
        ...
        "kwargs": {
            ...
            "mishikallm_params": {
                ...
                "custom_llm_provider": "openai",
                "api_base": "https://api.openai.com/v1/",
                "mishikallm_call_id": "508113a1-c6f1-48ce-a3e1-01c6cce9330e",
                "model_alias_map": {},
                "completion_call_id": None,
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="local only test, use this to verify if we can send request to mishikallm proxy server"
)
...
generation_params = {
    ...
    "metadata": {
        ...
        "hidden_params": {
            ...
            "mishikallm_call_id": "1fa31658-20af-40b5-9ac9-60fd7b5ad98c",
        ...
        "mishikallm_response_cost": 0.00038,
        "api_base": "https://api.openai.com/v1/",
        "cache_hit": False,
    },
    "level": "DEFAULT",
...
def test_langfuse_prompt_type(prompt):
    ...
    from mishikallm.integrations.langfuse.langfuse import _add_prompt_to_generation_params
...
def test_langfuse_logging_metadata():
    from mishikallm.integrations.langfuse.langfuse import log_requester_metadata
