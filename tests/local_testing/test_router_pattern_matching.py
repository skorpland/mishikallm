...
def test_add_pattern():
    ...
    deployment = Deployment(
        model_name="openai-1",
        mishikallm_params=MishikaLLM_Params(model="gpt-3.5-turbo"),
...
def test_add_pattern_vertex_ai():
    ...
    deployment = Deployment(
        ...
        mishikallm_params=MishikaLLM_Params(model="vertex_ai/gemini-1.5-flash-latest"),
        model_info=ModelInfo(),
    )
...
def test_add_multiple_deployments():
    ...
    deployment2 = Deployment(
        model_name="openai-2",
        mishikallm_params=MishikaLLM_Params(model="gpt-4"),
        model_info=ModelInfo(),
    )
...
async def test_route_with_no_matching_pattern():
        ...
        model_list=[
            {
                ...
                "mishikallm_params": {"model": "bedrock/meta.llama3*"},
            }
        ]
    )
    ...
    ## FAILS
    with pytest.raises(mishikallm.BadRequestError) as e:
    ...
    with pytest.raises(mishikallm.BadRequestError):
        ...
        model_list=[
            {
                ...
                "mishikallm_params": {"model": "anthropic/*", "api_key": "test"},
            }
        ]
    )
...
def test_pattern_matching_router_with_default_wildcard():
    ...
    router = Router(
        model_list=[
            {
                "model_name": "*",
                "mishikallm_params": {"model": "*"},
                "model_info": {"access_groups": ["default"]},
            },
            {
                "model_name": "anthropic-claude",
                "mishikallm_params": {"model": "anthropic/claude-3-5-sonnet"},
            },
        ]
    )
...
def test_pattern_matching_router_with_default_wildcard_and_model_wildcard():
    ...
    router = Router(
        model_list=[
            ...
            {
                "model_name": "llmengine/*",
                "mishikallm_params": {"model": "openai/*"},
            },
        ]
    )
...
def test_sorted_patterns():
    ...
    """
    from mishikallm.router_utils.pattern_match_deployments import PatternUtils
