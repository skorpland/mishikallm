...
from mishikallm.types.utils import StreamingChoices, ChatCompletionAudioResponse
...
from mishikallm import completion, stream_chunk_builder
...
def test_stream_chunk_builder_mishikallm_function_call():
    try:
        mishikallm.set_verbose = False
...
# test_stream_chunk_builder_mishikallm_function_call()


def test_stream_chunk_builder_mishikallm_tool_call():
    try:
...
# test_stream_chunk_builder_mishikallm_tool_call()


def test_stream_chunk_builder_mishikallm_tool_call_regular_message():
    try:
...
# test_stream_chunk_builder_mishikallm_tool_call_regular_message()


def test_stream_chunk_builder_mishikallm_usage_chunks():
    """
    Checks if stream_chunk_builder is able to correctly rebuild with given metadata from streaming chunks
    """
    from mishikallm.types.utils import Usage
    ...
    usage: mishikallm.Usage = response.usage
...
def test_stream_chunk_builder_mishikallm_mixed_calls():
...
def test_stream_chunk_builder_mishikallm_empty_chunks():
    with pytest.raises(mishikallm.APIError):
...
def test_stream_chunk_builder_multiple_tool_calls():
    ...
    for chunk in init_chunks:
        chunks.append(mishikallm.ModelResponse(**chunk, stream=True))
    ...
    expected_response = mishikallm.ModelResponse(**completed_response)
...
def test_stream_chunk_builder_openai_prompt_caching():
    ...
    chunks: List[mishikallm.ModelResponse] = []
    usage_obj = None
    for chunk in chat_completion:
        chunks.append(mishikallm.ModelResponse(**chunk.model_dump(), stream=True))

    print(f"chunks: {chunks}")

    usage_obj: mishikallm.Usage = chunks[-1].usage  # type: ignore
...
def test_stream_chunk_builder_openai_audio_output_usage():
    ...
    usage_obj: Optional[mishikallm.Usage] = None
...
def test_stream_chunk_builder_empty_initial_chunk():
    from mishikallm.mishikallm_core_utils.streaming_chunk_builder_utils import (
        ChunkProcessor,
    )
...
def load_env():
    ...
    tools = [
        {
            "type": "function",
            "function": mishikallm.utils.function_to_dict(get_current_weather),
        }
    ]
    OPENAI_GPT4oMINI = {
        "messages": messages,
...
def execute_completion(opts: dict):
    ...
    response_gen = mishikallm.completion(**opts)
    ...
    print("\n\n")
    assembly = mishikallm.stream_chunk_builder(partial_streaming_chunks)
