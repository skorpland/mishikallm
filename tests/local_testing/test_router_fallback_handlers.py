...
async def test_run_async_fallback(original_function):
    ...
    original_exception = mishikallm.exceptions.InternalServerError(
    ...
    if original_function == router._acompletion:
        assert isinstance(result, mishikallm.ModelResponse)
    elif original_function == router._atext_completion:
        assert isinstance(result, mishikallm.TextCompletionResponse)
    elif original_function == router._aembedding:
        assert isinstance(result, mishikallm.EmbeddingResponse)
...
async def test_failed_fallbacks_raise_most_recent_exception(original_function):
    ...
    with pytest.raises(mishikallm.exceptions.RateLimitError):
