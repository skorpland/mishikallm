...
@pytest.mark.asyncio
async def test_mishikallm_gateway_from_sdk():
    ...
    with patch.object(
        ...
        try:
            completion(
                model="mishikallm_proxy/my-vllm-model",
                messages=messages,
...
@pytest.mark.asyncio
async def test_mishikallm_gateway_from_sdk_structured_output():
    ...
    with patch.object(
        ...
        try:
            mishikallm.completion(
                model="mishikallm_proxy/openai/gpt-4o",
                messages=[
                ...
                base_url="https://mishikallm.ml-serving-internal.scale.com",
                client=openai_client,
            )
        except Exception as e:
            print(e)
...
@pytest.mark.asyncio
async def test_mishikallm_gateway_from_sdk_embedding(is_async):
    mishikallm.set_verbose = True
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_mishikallm_gateway_from_sdk_image_generation(is_async):
    mishikallm._turn_on_debug()
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_mishikallm_gateway_from_sdk_transcription(is_async):
    mishikallm.set_verbose = True
    mishikallm._turn_on_debug()
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_mishikallm_gateway_from_sdk_speech(is_async):
    mishikallm.set_verbose = True
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_mishikallm_gateway_from_sdk_rerank(is_async):
    mishikallm.set_verbose = True
    mishikallm._turn_on_debug()
...
def test_mishikallm_gateway_from_sdk_with_response_cost_in_additional_headers():
    mishikallm.set_verbose = True
    mishikallm._turn_on_debug()
    ...
    mock_response = MagicMock()
    mock_response.headers = {"x-mishikallm-response-cost": "120"}
    mock_response.parse.return_value = mishikallm.ModelResponse(
        **{
    ...
    with patch.object(
        ...
        assert (
            response._hidden_params["additional_headers"][
                "llm_provider-x-mishikallm-response-cost"
            ]
            == "120"
        )
...
def test_mishikallm_gateway_from_sdk_with_thinking_param():
    try: 
        response = mishikallm.completion(
            model="mishikallm_proxy/anthropic.claude-3-7-sonnet-20250219-v1:0",
            messages=[{"role": "user", "content": "Hello world"}],
            api_base="http://0.0.0.0:4000",
            api_key="sk-PIp1h0RekR",
            # client=openai_client,
