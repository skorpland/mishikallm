...
async def test_openai_prediction_param_with_caching():
    ...
    mishikallm.set_verbose = True
    mishikallm.cache = mishikallm.Cache(type=MishikaLLMCacheType.LOCAL)
    code = """
    /// <summary>
    /// Represents a user with a first name, last name, and username.
    /// </summary>
    ...
    completion_response_1 = mishikallm.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    # cache hit
    completion_response_2 = mishikallm.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    completion_response_3 = mishikallm.completion(
        model="gpt-4o-mini",
        messages=[
...
@patch("mishikallm.main.openai_chat_completions._get_openai_client")
...
def test_openai_chat_completion_streaming_handler_reasoning_content():
    from mishikallm.llms.openai.chat.gpt_transformation import (
...
def validate_web_search_annotations(annotations: ChatCompletionAnnotation):
    """validates mishikallm response contains web search annotations"""
...
def test_openai_web_search():
    ...
    print("mishikallm response: ", response.model_dump_json(indent=4))
...
def test_openai_web_search_streaming():
    ...
    for chunk in response:
        print("mishikallm response chunk: ", chunk)
        if (
...
class TestOpenAIGPT4OAudioTranscription(BaseLLMAudioTranscriptionTest):
    ...
    def get_custom_llm_provider(self) -> mishikallm.LlmProviders:
        return mishikallm.LlmProviders.OPENAI
