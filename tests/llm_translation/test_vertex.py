...
import mishikallm.mishikallm_core_utils.prompt_templates
import mishikallm.mishikallm_core_utils.prompt_templates.factory
...
import pytest
import mishikallm
from mishikallm import get_optional_params
from mishikallm.llms.custom_httpx.http_handler import HTTPHandler
from mishikallm.llms.vertex_ai.gemini.transformation import _process_gemini_image
from mishikallm.types.llms.vertex_ai import PartType, BlobType
import httpx
...
def test_logprobs_unit_test():
    from mishikallm import VertexGeminiConfig
...
def test_process_gemini_image():
    ...
    from mishikallm.types.llms.vertex_ai import PartType, FileDataType, BlobType
...
def test_vertex_embedding_url(model, expected_url):
    ...
    """
    from mishikallm.llms.vertex_ai.common_utils import _get_vertex_url
...
def mock_convert_url_to_base64():
    with patch(
        "mishikallm.mishikallm_core_utils.prompt_templates.factory.convert_url_to_base64",
    ) as mock:
...
def test_aaavertex_embeddings_distances(
    ...
    with patch.object(vertex_client, "post", return_value=mock_response), patch.object(
        mishikallm.main.vertex_multimodal_embedding,
    ...
    with patch.object(
        ...
        text_response = mishikallm.embedding(
            model="vertex_ai/multimodalembedding@001",
            input=[input_string],
            client=vertex_client,
        )
