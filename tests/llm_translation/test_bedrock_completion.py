...
from mishikallm.llms.bedrock.chat import BedrockLLM
from mishikallm.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
from mishikallm.mishikallm_core_utils.prompt_templates.factory import _bedrock_tools_pt
...
def bedrock_session_token_creds():
    ...
    if aws_session_token is not None:
        ...
        # aws_role_name = os.environ["AWS_TEMP_ROLE_NAME"]
        # TODO: This is using ai.moda's IAM role, we should use MishikaLLM's IAM role eventually
        aws_role_name = (
            "arn:aws:iam::335785316107:role/mishikallm-github-unit-tests-circleci"
        )
...
def process_stream_response(res, messages):
    ...
    if isinstance(res, mishikallm.utils.CustomStreamWrapper):
        chunks = []
        for part in res:
            chunks.append(part)
            text = part.choices[0].delta.content or ""
            print(text, end="")
        res = mishikallm.stream_chunk_builder(chunks, messages=messages)
    else:
        ...
        tools=[
            {
                "type": "function",
                "function": {
                    "name": "-DoSomethingVeryCool-forMishikallm_Testin999229291-0293993",
    ...
    if _choice_1.message.tool_calls is not None:
        ...
        for tool_call in _choice_1.message.tool_calls:
            ...
            if _tool_Call_name is not None and "DoSomethingVeryCool" in _tool_Call_name:
                assert (
                    _tool_Call_name
                    == "-DoSomethingVeryCool-forMishikallm_Testin999229291-0293993"
                )
...
def test_not_found_error():
    with pytest.raises(mishikallm.NotFoundError):
        completion(
...
def test_bedrock_converse_translation_tool_message():
    from mishikallm.types.utils import ChatCompletionMessageToolCall, Function
...
def test_base_aws_llm_get_credentials():
    ...
    from mishikallm.llms.bedrock.base_aws_llm import BaseAWSLLM
...
def test_bedrock_completion_test_4(modify_params):
    mishikallm.set_verbose = True
    mishikallm.modify_params = modify_params
    ...
    if modify_params:
        ...
        with pytest.raises(Exception) as e:
            mishikallm.completion(**data)
        assert "mishikallm.modify_params" in str(e.value)


def test_bedrock_context_window_error():
    with pytest.raises(mishikallm.ContextWindowExceededError) as e:
...
def test_bedrock_prompt_caching_message(messages, expected_cache_control):
    ...
    transformed_messages = mishikallm.AmazonConverseConfig()._transform_request(
...
def test_bedrock_supports_tool_call(model, expected_supports_tool_call):
    supported_openai_params = (
        mishikallm.AmazonConverseConfig().get_supported_openai_params(model=model)
    )
...
class TestBedrockConverseChatCrossRegion(BaseLLMChatTest):
    ...
    def test_multilingual_requests(self):
        ...
        Todo: if mishikallm.modify_params is True ensure it's a valid utf-8 sequence
        """
        pass
    ...
    def test_completion_cost(self):
        ...
        mishikallm.model_cost.pop(bedrock_model, None)
...
class TestBedrockRerank(BaseLLMRerankTest):
    def get_custom_llm_provider(self) -> mishikallm.LlmProviders:
        return mishikallm.LlmProviders.BEDROCK
...
def test_bedrock_no_default_message():
    ...
    mishikallm.modify_params = False
    formatted_messages = _bedrock_converse_messages_pt(
        messages=messages,
        model="anthropic.claude-3-sonnet-20240229-v1:0",
        llm_provider="bedrock",
...
def test_bedrock_nova_topk(top_k_param):
    ...
    original_transform = mishikallm.AmazonConverseConfig()._transform_request
    captured_data = None
    ...
    with patch(
        "mishikallm.AmazonConverseConfig._transform_request", side_effect=mock_transform
    ):
        mishikallm.completion(**data)
...
def test_bedrock_cross_region_inference(monkeypatch):
    ...
    monkeypatch.setenv("MISHIKALLM_LOCAL_MODEL_COST_MAP", "True")
    mishikallm.model_cost = mishikallm.get_model_cost_map(url="")
    mishikallm.add_known_models()
...
class TestBedrockEmbedding(BaseLLMEmbeddingTest):
    ...
    def test_bedrock_image_embedding_transformation(self):
        from mishikallm.llms.bedrock.embed.amazon_titan_multimodal_transformation import (
...
def test_bedrock_error_handling_streaming():
    from mishikallm.llms.bedrock.chat.invoke_handler import (
...
def test_handle_top_k_value_helper(model, expected_output):
    assert (
        mishikallm.AmazonConverseConfig()._handle_top_k_value(model, {"topK": 3})
        == expected_output
    )
    assert (
        mishikallm.AmazonConverseConfig()._handle_top_k_value(model, {"top_k": 3})
        == expected_output
    )
...
def test_bedrock_invoke_provider():
    assert (
        mishikallm.AmazonInvokeConfig().get_bedrock_invoke_provider(
...
async def test_bedrock_thinking_in_assistant_message(sync_mode):
    mishikallm._turn_on_debug()
    from mishikallm.llms.custom_httpx.http_handler import HTTPHandler, AsyncHTTPHandler
    ...
    with patch.object(client, "post") as mock_post:
        try:
            if sync_mode:
                response = mishikallm.completion(**params, client=client)
            else:
                response = await mishikallm.acompletion(**params, client=client)
        except Exception as e:
            print(e)
