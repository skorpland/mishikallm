...
import mishikallm
import pytest
from mishikallm.types.utils import ModelResponseStream, ModelResponse
...
def mock_provider_mapping():
    with patch("mishikallm.llms.huggingface.chat.transformation._fetch_inference_provider_mapping") as mock:
        mock.return_value = PROVIDER_MAPPING_RESPONSE
        yield mock
        
@pytest.fixture(autouse=True)
def clear_lru_cache():
    from mishikallm.llms.huggingface.common_utils import _fetch_inference_provider_mapping
...
class TestHuggingFace(BaseLLMChatTest):
    ...
    def test_get_complete_url(self, model, provider, expected_url):
        ...
        from mishikallm.llms.huggingface.chat.transformation import HuggingFaceChatConfig
