...
from mishikallm.llms.anthropic.chat import ModelResponseIterator
...
from mishikallm.types.llms.anthropic import AnthropicResponse
from mishikallm.types.utils import GenericStreamingChunk, ChatCompletionToolCallChunk
from mishikallm.types.llms.openai import ChatCompletionToolCallFunctionChunk
from mishikallm.llms.anthropic.common_utils import process_anthropic_headers
from mishikallm.llms.anthropic.chat.handler import AnthropicChatCompletion
...
def test_anthropic_beta_header(
    ...
    headers = mishikallm.AnthropicConfig().get_anthropic_headers(
...
def test_anthropic_tool_with_image():
    from mishikallm.mishikallm_core_utils.prompt_templates.factory import prompt_factory
    import json
...
def test_anthropic_map_openai_params_tools_and_json_schema():
    ...
    mapped_params = mishikallm.AnthropicConfig().map_openai_params(
...
def test_anthropic_json_mode_and_tool_call_response(
    ...
    result = mishikallm.AnthropicConfig()._transform_response_for_json_mode(
        json_mode=json_mode,
...
def test_map_stop_sequences(stop_input, expected_output, drop_params):
    ...
    mishikallm.drop_params = drop_params
    config = AnthropicConfig()
...
def test_anthropic_thinking_in_assistant_message(model):
    ...
    response = mishikallm.completion(**params)
