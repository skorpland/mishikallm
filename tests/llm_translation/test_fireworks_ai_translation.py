...
from mishikallm.llms.fireworks_ai.chat.transformation import FireworksAIConfig
...
class TestFireworksAIAudioTranscription(BaseLLMAudioTranscriptionTest):
    ...
    def get_custom_llm_provider(self) -> mishikallm.LlmProviders:
        return mishikallm.LlmProviders.FIREWORKS_AI
...
def test_transform_inline(content, model, expected_url):
    ...
    result = mishikallm.FireworksAIConfig()._add_transform_inline_image_block(
...
def test_global_disable_flag(model, is_disabled, expected_url):
    ...
    assert result["image_url"] == expected_url
    mishikallm.disable_add_transform_inline_image_block = False  # Reset for other tests
...
def test_global_disable_flag_with_transform_messages_helper(monkeypatch):
    ...
    monkeypatch.setattr(mishikallm, "disable_add_transform_inline_image_block", True)
