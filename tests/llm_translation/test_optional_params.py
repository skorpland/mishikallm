...
import mishikallm
from mishikallm.mishikallm_core_utils.prompt_templates.factory import map_system_message_pt
from mishikallm.types.completion import (
...
### Scenarios: w/ optional params + mishikallm.drop_params = True


def test_supports_system_message():
    """
    Check if mishikallm.completion(...,supports_system_message=False)
    """
    messages = [
...
def test_vertex_ai_llama_3_optional_params():
    mishikallm.vertex_llama3_models = ["meta/llama3-405b-instruct-maas"]
    mishikallm.drop_params = True
    optional_params = get_optional_params(
...
def test_vertex_ai_mistral_optional_params():
    mishikallm.vertex_mistral_models = ["mistral-large@2407"]
    mishikallm.drop_params = True
    optional_params = get_optional_params(
...
def test_azure_gpt_optional_params_gpt_vision():
    ...
    optional_params = mishikallm.utils.get_optional_params(
        model="",
        user="John",
        custom_llm_provider="azure",
        max_tokens=10,
...
def test_get_optional_params_image_gen():
    response = mishikallm.utils.get_optional_params_image_gen(
...
def test_o1_model_temperature_params(provider, temperature, expected_error):
    if expected_error:
        with pytest.raises(mishikallm.UnsupportedParamsError):
            get_optional_params(
                model="o1-preview",
                custom_llm_provider=provider,
...
def test_forward_user_param():
    from mishikallm.utils import get_supported_openai_params, get_optional_params
...
def test_gemini_frequency_penalty():
    from mishikallm.utils import get_supported_openai_params
...
def test_mishikallm_proxy_claude_3_5_sonnet():
    tools = [
        {
            "type": "function",
            "function": {
...
def test_is_vertex_anthropic_model():
    assert (
        mishikallm.VertexAIAnthropicConfig().is_supported_model(
            model="claude-3-5-sonnet", custom_llm_provider="mishikallm_proxy"
        )
        is False
    )
...
def test_azure_prediction_param():
    optional_params = get_optional_params(
        ...
        prediction={
            "type": "content",
            "content": "MishikaLLM is a very useful way to connect to a variety of LLMs.",
        },
    )
...
def test_mishikallm_proxy_thinking_param():
    optional_params = get_optional_params(
        model="gpt-4o",
        custom_llm_provider="mishikallm_proxy",
        thinking={"type": "enabled", "budget_tokens": 1024},
...
def test_azure_response_format_param():
    optional_params = mishikallm.get_optional_params(
