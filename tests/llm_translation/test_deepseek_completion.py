...
async def test_deepseek_provider_async_completion(stream):
    ...
    with patch(
        "mishikallm.llms.custom_httpx.llm_http_handler.AsyncHTTPHandler.post"
    ) as mock_post:
        mock_response_data = mishikallm.ModelResponse(
            choices=[
                mishikallm.Choices(
                    message=mishikallm.Message(content="Hello!"),
                    index=0,
                    finish_reason="stop",
                )
            ]
