...
import pytest
from mishikallm.llms.azure.common_utils import process_azure_headers
...
def test_azure_extra_headers(input, call_type, header_value):
    from mishikallm import embedding, image_generation
...
def test_process_azure_endpoint_url(api_base, model, expected_endpoint):
    from mishikallm.llms.azure.azure import AzureChatCompletion
...
class TestAzureEmbedding(BaseLLMEmbeddingTest):
    ...
    def get_custom_llm_provider(self) -> mishikallm.LlmProviders:
        return mishikallm.LlmProviders.AZURE
...
def test_get_azure_ad_token_from_username_password(
    ...
    from mishikallm.llms.azure.common_utils import (
...
@patch(
    "mishikallm.main.azure_chat_completions.make_sync_azure_openai_chat_completion_request"
)
...
@pytest.mark.parametrize("max_retries", [0, 4])
@pytest.mark.parametrize("stream", [True, False])
@patch("mishikallm.main.azure_chat_completions.make_azure_openai_chat_completion_request")
@pytest.mark.asyncio
...
@patch("mishikallm.llms.azure.common_utils.select_azure_base_url_or_endpoint")
@pytest.mark.asyncio
...
async def test_azure_embedding_max_retries_0(
    ...
    from mishikallm import aembedding, embedding
