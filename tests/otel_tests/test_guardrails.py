...
async def test_llm_guard_triggered_safe_request():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-mishikallm-applied-guardrails" in headers

        assert (
            headers["x-mishikallm-applied-guardrails"]
...
async def test_no_llm_guard_triggered():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-mishikallm-applied-guardrails" not in headers
...
async def test_guardrails_with_api_key_controls():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-mishikallm-applied-guardrails" in headers
        assert headers["x-mishikallm-applied-guardrails"] == "bedrock-pre-guard"
    ...
    async with aiohttp.ClientSession() as session:
        try:
            response, headers = await chat_completion(
                ...
                "sk-1234",
                model="fake-openai-endpoint",
                messages=[{"role": "user", "content": f"Hello do you like mishikallm?"}],
                guardrails=["custom-during-guard"],
            )
            pytest.fail("Should have thrown an exception")
        except Exception as e:
            print(e)
            assert "Guardrail failed words - `mishikallm` detected" in str(e)
