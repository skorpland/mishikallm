...
# In a real-world scenario, these would be instances of MishikaLLM_VerificationToken, MishikaLLM_UserTable, etc.
...
async def test_reset_budget_keys_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_mishikallm_keys()
...
async def test_reset_budget_users_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_mishikallm_users()
        await asyncio.sleep(0.1)
...
async def test_reset_budget_teams_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_mishikallm_teams()
        await asyncio.sleep(0.1)
...
async def test_service_logger_keys_success():
    ...
    with patch.object(
        ...
        with patch(
            "mishikallm.proxy.common_utils.reset_budget_job.verbose_proxy_logger.exception"
        ) as mock_verbose_exc:
            await job.reset_budget_for_mishikallm_keys()
