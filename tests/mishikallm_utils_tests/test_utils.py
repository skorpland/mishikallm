...
import mishikallm
from mishikallm.llms.custom_httpx.http_handler import AsyncHTTPHandler, headers
from mishikallm.mishikallm_core_utils.duration_parser import duration_in_seconds
from mishikallm.mishikallm_core_utils.duration_parser import (
...
def reset_mock_cache():
    from mishikallm.utils import _model_cache
...
def test_basic_trimming_no_max_tokens_specified():
    ...
    ) <= mishikallm.model_cost["gpt-4"]["max_tokens"]
...
def test_trimming_with_system_message_within_max_tokens():
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "This is a medium normal message, let's say mishikallm is awesome.",
        },
    ]
    trimmed_messages = trim_messages(
...
def test_trimming_with_tool_calls():
    from mishikallm.types.utils import ChatCompletionMessageToolCall, Function, Message
...
def test_trimming_with_model_cost_max_input_tokens(model):
    ...
    assert (
        ...
        < mishikallm.model_cost[model]["max_input_tokens"]
    )
...
def test_aget_valid_models():
    ...
    expected_models = (
        mishikallm.open_ai_chat_completion_models + mishikallm.open_ai_text_completion_models
    )
    ...
    # GEMINI
    expected_models = mishikallm.gemini_models
    old_environ = os.environ
...
def test_get_valid_models_with_custom_llm_provider(custom_llm_provider):
    ...
    from mishikallm.types.utils import LlmProviders
...
def test_supports_function_calling(model, expected_bool):
    try:
        assert mishikallm.supports_function_calling(model=model) == expected_bool
...
def test_supports_web_search(model, expected_bool):
    try:
        assert mishikallm.supports_web_search(model=model) == expected_bool
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_supports_reasoning(model, expected_bool):
    os.environ["MISHIKALLM_LOCAL_MODEL_COST_MAP"] = "True"
    mishikallm.model_cost = mishikallm.get_model_cost_map(url="")
    try:
        assert mishikallm.supports_reasoning(model=model) == expected_bool
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_get_chat_completion_prompt():
    ...
    assert mishikallm_logging_obj.messages == [
...
def test_redact_msgs_from_logs():
    ...
    _redacted_response_obj = redact_message_input_output_from_logging(
        result=response_obj,
        model_call_details=mishikallm_logging_obj.model_call_details,
    )
...
def test_redact_msgs_from_logs_with_dynamic_params():
    ...
    In all tests mishikallm.turn_off_message_logging is True
    ...
    3. standard_callback_dynamic_params.turn_off_message_logging not set, mishikallm.turn_off_message_logging is True: Redaction should occur.
    """
    from mishikallm.mishikallm_core_utils.mishikallm_logging import Logging
    from mishikallm.mishikallm_core_utils.redact_messages import (
        redact_message_input_output_from_logging,
    ...
    mishikallm_logging_obj.model_call_details["standard_callback_dynamic_params"] = (
    ...
    assert _redacted_response_obj.choices[0].message.content == "redacted-by-mishikallm"

    # Test Case 3: standard_callback_dynamic_params does not override mishikallm.turn_off_message_logging
    # since mishikallm.turn_off_message_logging is True redaction should occur
    standard_callback_dynamic_params = StandardCallbackDynamicParams()
    mishikallm_logging_obj.model_call_details["standard_callback_dynamic_params"] = (
        standard_callback_dynamic_params
    )
...
def test_logging_trace_id(langfuse_trace_id, langfuse_existing_trace_id):
    ...
    mishikallm_call_id = "my-unique-call-id"
    mishikallm_logging_obj = Logging(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "hi"}],
        stream=False,
    ...
    time.sleep(3)
    assert mishikallm_logging_obj._get_trace_id(service_name="langfuse") is not None

    ## if existing_trace_id exists
    if langfuse_existing_trace_id is not None:
        assert (
            mishikallm_logging_obj._get_trace_id(service_name="langfuse")
...
def test_convert_model_response_object():
        ...
        "model_response_object": mishikallm.ModelResponse(
            ...
            choices=[
                mishikallm.Choices(
                    finish_reason="stop",
                    index=0,
                    message=mishikallm.Message(content="default", role="assistant"),
                )
            ],
            created=1719376241,
    ...
    try:
        mishikallm.convert_to_model_response_object(**args)
...
def test_parse_content_for_reasoning(content, expected_reasoning, expected_content):
    assert mishikallm.utils._parse_content_for_reasoning(content) == (
        expected_reasoning,
        expected_content,
    )
...
def test_supports_response_schema(model, expected_bool):
    ...
    from mishikallm.utils import supports_response_schema
...
def test_supports_function_calling_v2(model, expected_bool):
    ...
    """
    from mishikallm.utils import supports_function_calling
...
def test_supports_vision(model, expected_bool):
    ...
    """
    from mishikallm.utils import supports_vision
...
def test_usage_object_null_tokens():
    ...
    Fixes https://github.com/skorpland/mishikallm/issues/5096
    """
    usage_obj = mishikallm.Usage(prompt_tokens=2, completion_tokens=None, total_tokens=2)
...
def test_is_base64_encoded():
    ...
    from mishikallm.utils import is_base64_encoded
...
def test_async_http_handler_force_ipv4(mock_async_client):
    """
    Test AsyncHTTPHandler when mishikallm.force_ipv4 is True

    This is prod test - we need to ensure that httpx always uses ipv4 when mishikallm.force_ipv4 is True
    """
    import httpx
    from mishikallm.llms.custom_httpx.http_handler import AsyncHTTPHandler

    # Set force_ipv4 to True
    mishikallm.force_ipv4 = True
...
def test_supports_audio_input(model, expected_bool):
    ...
    from mishikallm.utils import supports_audio_input, supports_audio_output
...
def test_validate_chat_completion_user_messages(messages, expected_bool):
    from mishikallm.utils import validate_chat_completion_user_messages
...
def test_validate_chat_completion_tool_choice(tool_choice, expected_bool):
    from mishikallm.utils import validate_chat_completion_tool_choice
...
def test_models_by_provider():
    ...
    from mishikallm import models_by_provider

    providers = set()
    for k, v in mishikallm.model_cost.items():
        if "_" in v["mishikallm_provider"] and "-" in v["mishikallm_provider"]:
            continue
        elif k == "sample_spec":
            continue
        elif (
            v["mishikallm_provider"] == "sagemaker"
            or v["mishikallm_provider"] == "bedrock_converse"
        ):
            continue
        else:
            providers.add(v["mishikallm_provider"])
...
@pytest.mark.parametrize(
    "mishikallm_params, disable_end_user_cost_tracking, expected_end_user_id",
    [
        ({}, False, None),
...
def test_get_end_user_id_for_cost_tracking(
    ...
    mishikallm.disable_end_user_cost_tracking = disable_end_user_cost_tracking
    assert (
        get_end_user_id_for_cost_tracking(mishikallm_params=mishikallm_params)
        == expected_end_user_id
    )


@pytest.mark.parametrize(
    "mishikallm_params, disable_end_user_cost_tracking_prometheus_only, expected_end_user_id",
    [
        ({}, False, None),
        ({"user_api_key_end_user_id": "123"}, False, "123"),
        ({"user_api_key_end_user_id": "123"}, True, None),
...
def test_get_end_user_id_for_cost_tracking_prometheus_only(
    ...
    assert (
        get_end_user_id_for_cost_tracking(
            mishikallm_params=mishikallm_params, service_type="prometheus"
        )
        == expected_end_user_id
    )
...
def test_is_prompt_caching_enabled_return_default_image_dimensions():
    ...
    """
    with patch("mishikallm.utils.token_counter") as mock_token_counter:
        mishikallm.utils.is_prompt_caching_valid_prompt(
            messages=[
                {
                    "role": "user",
                    "content": [
...
def test_token_counter_with_image_url_with_detail_high():
    ...
    """
    from mishikallm.constants import DEFAULT_IMAGE_TOKEN_COUNT
    from mishikallm._logging import verbose_logger
    import logging

    verbose_logger.setLevel(logging.DEBUG)

    _tokens = mishikallm.utils.token_counter(
        messages=[
            {
                "role": "user",
                "content": [
...
def test_fireworks_ai_document_inlining():
    ...
    - supports_vision
    """
    from mishikallm.utils import supports_pdf_input, supports_vision
...
def test_logprobs_type():
    from mishikallm.types.utils import Logprobs
...
def test_get_valid_models_openai_proxy(monkeypatch):
    from mishikallm.utils import get_valid_models
    import mishikallm

    mishikallm._turn_on_debug()

    monkeypatch.setenv("MISHIKALLM_PROXY_API_KEY", "sk-1234")
    monkeypatch.setenv("MISHIKALLM_PROXY_API_BASE", "https://mishikallm-api.up.railway.app/")
    ...
    with patch.object(
        mishikallm.module_level_client, "get", return_value=mock_response
    ) as mock_post:
        valid_models = get_valid_models(check_provider_endpoint=True)
        assert "mishikallm_proxy/gpt-4o" in valid_models
...
def test_get_valid_models_fireworks_ai(monkeypatch):
    ...
    monkeypatch.setattr(mishikallm, "provider_list", ["fireworks_ai"])
...
def test_get_potential_model_names():
    from mishikallm.utils import _get_potential_model_names
...
def test_get_num_retries(num_retries):
    from mishikallm.utils import _get_wrapper_num_retries
...
def test_add_custom_logger_callback_to_specific_event(monkeypatch):
    ...
    monkeypatch.setattr(mishikallm, "success_callback", [])
    monkeypatch.setattr(mishikallm, "failure_callback", [])

    _add_custom_logger_callback_to_specific_event("langfuse", "success")

    assert len(mishikallm.success_callback) == 1
    assert len(mishikallm.failure_callback) == 0
...
def test_add_custom_logger_callback_to_specific_event_e2e(monkeypatch):
    ...
    monkeypatch.setattr(mishikallm, "success_callback", [])
    monkeypatch.setattr(mishikallm, "failure_callback", [])
    monkeypatch.setattr(mishikallm, "callbacks", [])

    mishikallm.success_callback = ["humanloop"]

    curr_len_success_callback = len(mishikallm.success_callback)
    curr_len_failure_callback = len(mishikallm.failure_callback)
    ...
    assert len(mishikallm.success_callback) == curr_len_success_callback
    assert len(mishikallm.failure_callback) == curr_len_failure_callback
...
def test_custom_logger_exists_in_callbacks_individual_functions(monkeypatch):
    ...
    for list_name in [
        ...
        monkeypatch.setattr(mishikallm, list_name, [])
    ...
    mishikallm.success_callback.append(mock_logger)
    ...
    mishikallm._async_success_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == True
    assert _custom_logger_class_exists_in_failure_callbacks(mock_logger) == False
    ...
    mishikallm.failure_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == False
    ...
    mishikallm._async_failure_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == False
    assert _custom_logger_class_exists_in_failure_callbacks(mock_logger) == True
...
async def test_add_custom_logger_callback_to_specific_event_with_duplicates(
    ...
    """
    from mishikallm.integrations.langfuse.langfuse_prompt_management import (
        LangfusePromptManagement,
    )

    # Reset all callback lists
    monkeypatch.setattr(mishikallm, "callbacks", [])
    monkeypatch.setattr(mishikallm, "_async_success_callback", [])
    monkeypatch.setattr(mishikallm, "_async_failure_callback", [])
    monkeypatch.setattr(mishikallm, "success_callback", [])
    monkeypatch.setattr(mishikallm, "failure_callback", [])

    # Add logger to both success_callback and _async_success_callback
    langfuse_logger = LangfusePromptManagement()
    mishikallm.success_callback.append(langfuse_logger)
    mishikallm._async_success_callback.append(langfuse_logger)

    # Get initial lengths
    initial_success_callback_len = len(mishikallm.success_callback)
    initial_async_success_callback_len = len(mishikallm._async_success_callback)
    ...
    assert len(mishikallm.success_callback) == initial_success_callback_len
    assert len(mishikallm._async_success_callback) == initial_async_success_callback_len
...
async def test_add_custom_logger_callback_to_specific_event_with_duplicates_callbacks(
    ...
    # Add logger to both success_callback and _async_success_callback
    langfuse_logger = LangfusePromptManagement()
    mishikallm.callbacks.append(langfuse_logger)
    ...
    # Assert no new callbacks were added
    initial_callbacks_len = len(mishikallm.callbacks)
    initial_async_success_callback_len = len(mishikallm._async_success_callback)
    initial_success_callback_len = len(mishikallm.success_callback)
    print(
        f"Num callbacks before: mishikallm.callbacks: {len(mishikallm.callbacks)}, mishikallm._async_success_callback: {len(mishikallm._async_success_callback)}, mishikallm.success_callback: {len(mishikallm.success_callback)}"
    )
    ...
    assert len(mishikallm.callbacks) == initial_callbacks_len
    assert len(mishikallm._async_success_callback) == initial_async_success_callback_len
    assert len(mishikallm.success_callback) == initial_success_callback_len

    print(
        f"Num callbacks after 10 mock calls: mishikallm.callbacks: {len(mishikallm.callbacks)}, mishikallm._async_success_callback: {len(mishikallm._async_success_callback)}, mishikallm.success_callback: {len(mishikallm.success_callback)}"
    )


def test_add_custom_logger_callback_to_specific_event_e2e_failure(monkeypatch):
    from mishikallm.integrations.openmeter import OpenMeterLogger
    ...
    mishikallm.failure_callback = ["openmeter"]
    ...
    assert any(
        isinstance(callback, OpenMeterLogger) for callback in mishikallm.failure_callback
    )


@pytest.mark.asyncio
async def test_wrapper_kwargs_passthrough():
    from mishikallm.utils import client
    from mishikallm.mishikallm_core_utils.mishikallm_logging import (
        Logging as MishikaLLMLoggingObject,
    )
    ...
    # get mishikallm logging object
    mishikallm_logging_obj: MishikaLLMLoggingObject = mock_original.call_args.kwargs.get(
        "mishikallm_logging_obj"
    )
    assert mishikallm_logging_obj is not None

    print(
        f"mishikallm_logging_obj.model_call_details: {mishikallm_logging_obj.model_call_details}"
    )

    # get base model
    assert (
        mishikallm_logging_obj.model_call_details["mishikallm_params"]["base_model"]
        == "gpt-4o-mini"
    )


def test_dict_to_response_format_helper():
    from mishikallm.llms.base_llm.base_utils import _dict_to_response_format_helper
...
from mishikallm.utils import get_applied_guardrails
...
def test_get_applied_guardrails(test_case):
    ...
    # Setup
    mishikallm.callbacks = test_case["callbacks"]
...
def test_should_use_cohere_v1_client(endpoint, params, expected_bool):
    assert mishikallm.utils.should_use_cohere_v1_client(endpoint, params) == expected_bool


def test_add_openai_metadata():
    from mishikallm.utils import add_openai_metadata

    metadata = {
        "user_api_key_end_user_id": "123",
        "hidden_params": {"api_key": "123"},
        "mishikallm_parent_otel_span": MagicMock(),
        "none-val": None,
        "int-val": 1,
        "dict-val": {"a": 1, "b": 2},
    }
...
def test_message_object():
    from mishikallm.types.utils import Message
...
def test_delta_object():
    from mishikallm.types.utils import Delta
...
def test_claude_3_7_sonnet_supports_pdf_input(model, expected_bool):
    from mishikallm.utils import supports_pdf_input
...
def test_get_valid_models_from_provider_cache_invalidation(monkeypatch):
    ...
    _model_cache.set_cached_model_info("openai", mishikallm_params=None, available_models=["gpt-4o-mini"])
...
def test_get_valid_models_from_dynamic_api_key():
    ...
    creds = CredentialMishikaLLMParams(api_key="123")

    valid_models = get_valid_models(custom_llm_provider="anthropic", mishikallm_params=creds, check_provider_endpoint=True)
    assert len(valid_models) == 0

    creds = CredentialMishikaLLMParams(api_key=os.getenv("ANTHROPIC_API_KEY"))
    valid_models = get_valid_models(custom_llm_provider="anthropic", mishikallm_params=creds, check_provider_endpoint=True)
    assert len(valid_models) > 0
