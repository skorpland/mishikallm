...
import mishikallm
from mishikallm.utils import get_llm_provider, ProviderConfigManager, _check_provider_match
...
def test_supports_tool_choice_simple_tests():
    ...
    """
    assert mishikallm.utils.supports_tool_choice(model="gpt-4o") == True
    assert (
    ...
    assert (
        mishikallm.utils.supports_tool_choice(model="us.amazon.nova-micro-v1:0") is False
    )
    assert (
        mishikallm.utils.supports_tool_choice(model="bedrock/us.amazon.nova-micro-v1:0")
        is False
    )
    assert (
        mishikallm.utils.supports_tool_choice(
    ...
    assert mishikallm.utils.supports_tool_choice(model="perplexity/sonar") is False
...
def test_check_provider_match():
    ...
    model_info = {"mishikallm_provider": "bedrock"}
    assert mishikallm.utils._check_provider_match(model_info, "bedrock") is True
    assert mishikallm.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test bedrock_converse provider
    model_info = {"mishikallm_provider": "bedrock_converse"}
    assert mishikallm.utils._check_provider_match(model_info, "bedrock") is True
    assert mishikallm.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test non-matching provider
    model_info = {"mishikallm_provider": "bedrock"}
    assert mishikallm.utils._check_provider_match(model_info, "openai") is False
...
async def test_supports_tool_choice():
    """
    Test that mishikallm.utils.supports_tool_choice() returns the correct value
    ...
    mishikallm.model_cost = model_prices
    ...
    for model_name, model_info in model_prices.items():
        ...
        if (
            ...
            or model_info["mishikallm_provider"] in OLD_PROVIDERS
        ...
        supports_tool_choice_result = mishikallm.utils.supports_tool_choice(
