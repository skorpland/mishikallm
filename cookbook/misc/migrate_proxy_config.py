...
def migrate_models(config_file, proxy_base_url):
    ...
    for model in model_list:
        ...
        mishikallm_model_name = mishikallm_params.get("model", "") or ""
        if "vertex_ai/" in mishikallm_model_name:
            print("\033[91m\nSkipping Vertex AI model\033[0m", model)
            continue

        for param, value in mishikallm_params.items():
            if isinstance(value, str) and value.startswith("os.environ/"):
                ...
                mishikallm_params[param] = new_value
        if "api_key" not in mishikallm_params:
            new_value = input(f"Enter api key for {model_name}: ")
            mishikallm_params["api_key"] = new_value
        ...
        data = {"model_name": model_name, "mishikallm_params": mishikallm_params}
