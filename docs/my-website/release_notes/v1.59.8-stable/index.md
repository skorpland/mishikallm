...
4. Bedrock - docs on IAM role based access for bedrock - [Start Here](https://docs.21t.cc/docs/providers/bedrock#sts-role-based-auth)
...
12. OpenAI - `metadata` param preview support 
    1. SDK - enable via `mishikallm.enable_preview_features = True` 
    2. PROXY - enable via `mishikallm_settings::enable_preview_features: true` 
...
11. Public Teams - allow admins to expose teams for new users to ‘join’ on UI - [Start Here](https://docs.21t.cc/docs/proxy/public_teams)
12. Guardrails
...
2. Support s3 logging with team alias prefixes - [Start Here](https://docs.21t.cc/docs/proxy/logging#team-alias-prefix-in-object-key)
3. Prometheus [Start Here](../../docs/proxy/prometheus)
    1. fix mishikallm_llm_api_time_to_first_token_metric not populating for bedrock models
    ...
    4. emit `mishikallm_overhead_latency_metric` 
    5. Emit `mishikallm_team_budget_reset_at_metric` and `mishikallm_api_key_budget_remaining_hours_metric` 
...
7. New GCS Pub/Sub logging support [Start Here](https://docs.21t.cc/docs/proxy/logging#google-cloud-storage---pubsub-topic)
...
2. Allow specifying a health check model for wildcard routes - https://docs.21t.cc/docs/proxy/health#wildcard-routes
...
8. Response Headers - return 
    1. `x-mishikallm-timeout` 
    2. `x-mishikallm-attempted-retries`
    3. `x-mishikallm-overhead-duration-ms` 
    4. `x-mishikallm-response-duration-ms` 
...
3. Request Headers - support reading `x-mishikallm-timeout` param from request headers. Enables model timeout control when using Vercel’s AI SDK + MishikaLLM Proxy. [Start Here](../../docs/proxy/request_headers#mishikallm-headers)
4. JWT / OIDC Auth - new `role` based permissions for model authentication. [See Here](https://docs.21t.cc/docs/proxy/jwt_auth_arch)
