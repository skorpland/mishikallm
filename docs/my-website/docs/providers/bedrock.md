...
| Provider Route on MishikaLLM | `bedrock/`, [`bedrock/converse/`](#set-converse--invoke-route), [`bedrock/invoke/`](#set-invoke-route), [`bedrock/converse_like/`](#calling-via-internal-proxy), [`bedrock/llama/`](#deepseek-not-r1), [`bedrock/deepseek_r1/`](#deepseek-r1) |
...
MishikaLLM requires `boto3` to be installed on your system for Bedrock requests
```shell
...
MishikaLLM uses boto3 to handle authentication. All these options are supported - https://boto3.amazonaws.com/v1/documentation/api/latest/guide/credentials.html#credentials.
...
Here's how to call Bedrock with the MishikaLLM Proxy Server
...
If you pass a non-openai param to mishikallm, we'll assume it's provider-specific and send it as a kwarg in the request body. [See more](../completion/input.md#provider-specific-params)
...
MishikaLLM supports tool calling via Bedrock's Converse and Invoke API's.
...
Example of using [Bedrock Guardrails with MishikaLLM](https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails-use-converse-api.html)
...
MishikaLLM supports Bedrock [cross-region inferencing](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html) across all [supported bedrock models](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference-support.html).
...
Supported from MishikaLLM Version `v1.53.5`

:::

MishikaLLM defaults to the `invoke` route. MishikaLLM uses the `converse` route for Bedrock models that support it.
...
just set `mishikallm.modify_params=True` and MishikaLLM will automatically handle this with a default user_continue_message.
...
MishikaLLM supports Document Understanding for Bedrock models - [AWS Bedrock Docs](https://docs.aws.amazon.com/nova/latest/userguide/modalities-document.html).

:::info

MishikaLLM supports ALL Bedrock document types - 
...
### API keys
This can be set as env variables or passed as **params to mishikallm.embedding()**
```python
import os
...
### Advanced - [Pass model/provider-specific Params](https://docs.21t.cc/docs/completion/provider_specific_params#proxy-usage)
...
Pass AWS credentials as parameters to mishikallm.completion
```python
import os
from mishikallm import completion
...
| MishikaLLM Parameter | Boto3 Parameter | Description | Boto3 Documentation |
|------------------|-----------------|-------------|-------------------|
...
Pass an external BedrockRuntime.Client object as a parameter to mishikallm.completion. Useful when using an AWS credentials profile, SSO session, assumed role session, or if environment variables are not available for auth.
