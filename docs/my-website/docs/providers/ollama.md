...
# Ollama 
MishikaLLM supports all models from [Ollama](https://github.com/ollama/ollama)

<a target="_blank" href="https://colab.research.google.com/github/skorpland/mishikallm/blob/main/cookbook/mishikaLLM_Ollama.ipynb">
...
To use ollama JSON Mode pass `format="json"` to `mishikallm.completion()`
...
To use ollama tool calling, pass `tools=[{..}]` to `mishikallm.completion()` 
...
## [OPTIONAL] REGISTER MODEL - not all ollama models support function calling, mishikallm defaults to json mode tool calls if native tool calling not supported.
...
MishikaLLM supports calling Ollama's `/api/generate` endpoint on `/v1/completions` requests. 
...
```python
import mishikallm 
mishikallm._turn_on_debug() # turn on debug to see the request
from mishikallm import completion
...
Call `ollama/llava` in the same input/output format as OpenAI [`gpt-4-vision`](https://docs.21t.cc/docs/providers/openai#openai-vision-models)
...
## MishikaLLM/Ollama Docker Image 

For Ollama MishikaLLM Provides a Docker Image for an OpenAI API compatible server for local LLMs - llama2, mistral, codellama
...
### Quick Start:
Docker Hub: 
For ARM Processors: https://hub.docker.com/repository/docker/mishikallm/ollama/general
For Intel/AMD Processors: to be added
```shell
docker pull mishikallm/ollama
```

```shell
docker run --name ollama mishikallm/ollama
```
...
print(f'MishikaLLM: response from proxy with streaming')
...
for chunk in response:
    print(f'MishikaLLM: streaming response from proxy {chunk}')
```
...
[Follow these instructions](https://github.com/skorpland/mishikallm/issues/1517#issuecomment-1922022209/)
