...
# AWS Sagemaker
MishikaLLM supports All Sagemaker Huggingface Jumpstart Models

:::tip

**We support ALL Sagemaker models, just set `model=sagemaker/<any-model-on-sagemaker>` as a prefix when sending mishikallm requests**
...
### Usage - Streaming
Sagemaker currently does not support streaming - MishikaLLM fakes streaming by returning chunks of the response string
...
Here's how to call Sagemaker with the MishikaLLM Proxy Server
...
By default when `temperature=0` is sent in requests to MishikaLLM, MishikaLLM rounds up to `temperature=0.1` since Sagemaker fails most requests when `temperature=0`
...
Here's an example of using a sagemaker model with MishikaLLM 
...
MishikaLLM supports all Sagemaker Jumpstart Huggingface Embedding models. Here's how to call it: 
...
response = mishikallm.embedding(model="sagemaker/<your-deployment-name>", input=["good morning from mishikallm", "this is another item"])
