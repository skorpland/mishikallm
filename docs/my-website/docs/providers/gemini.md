...
| Provider Route on MishikaLLM | `gemini/` |
...
### Response schema 
MishikaLLM supports sending `response_schema` as a param for Gemini-1.5-Pro on Google AI Studio. 
...
To pass additional GenerationConfig params - e.g. `topK`, just pass it in the request body of the call, and MishikaLLM will pass it straight through as a key-value pair in the request body. 
...
MishikaLLM follows the OpenAI format and accepts sending inline data as an encoded base64 string. 
...
mishikallm.set_verbose = True # 👈 See Raw call 
...
**We support ALL Gemini models, just set `model=gemini/<any-model-on-gemini>` as a prefix when sending mishikallm requests**
