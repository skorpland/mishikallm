...
MishikaLLM supports all models on VLLM.
...
| Provider Route on MishikaLLM | `hosted_vllm/` (for OpenAI compatible server), `vllm/` (for vLLM sdk usage) |
...
vLLM Provides an OpenAI compatible endpoints - here's how to call it with MishikaLLM 

In order to use mishikallm to call a hosted vllm server add the following to your completion call
...
### Using - `mishikallm.completion`

```
pip install mishikallm vllm
```
```python
import mishikallm 

response = mishikallm.completion(
            model="vllm/facebook/opt-125m", # add a vllm prefix so mishikallm knows the custom_llm_provider==vllm
            messages=messages,
            temperature=0.2,
            max_tokens=80)
...
mishikallm.register_prompt_template(
	...
	roles={
        ...
        } # tell MishikaLLM how you want to map the openai messages to this model
)
