...
| Provider Route on MishikaLLM | `vertex_ai/` |
...
messages = [
    {
        "role": "system",
        "content": "Your name is Mishikallm Bot, you are a helpful assistant",
    },
...
From v`1.40.1+` MishikaLLM supports sending `response_schema` as a param for Gemini-1.5-Pro on Vertex AI. For other models (e.g. `gemini-1.5-flash` or `claude-3-5-sonnet`), MishikaLLM adds the schema to the message list with a user-controlled prompt.
...
```python
from mishikallm import completion, JSONSchemaValidationError
try: 
	completion(
    model="vertex_ai/gemini-1.5-pro", 
    messages=messages, 
...
MishikaLLM will validate the response against the schema, and raise a `JSONSchemaValidationError` if the response does not match the schema. 
...
client = OpenAI(
    ...
    base_url="http://0.0.0.0:4000/v1/" # point to mishikallm proxy
)
...
#### **Moving from Vertex AI SDK to MishikaLLM (GROUNDING)**
...
```python
import mishikallm
mishikallm.vertex_project = "hardy-device-38811" # Your Project ID
mishikallm.vertex_location = "us-central1"  # proj location

response = mishikallm.completion(model="gemini-pro", messages=[{"role": "user", "content": "write code for saying hi from MishikaLLM"}])
```

## Usage with MishikaLLM Proxy Server

Here's how to use Vertex AI with the MishikaLLM Proxy Server
...
1. Modify the config.yaml 
  ...
  <TabItem value="mishikallm_param" label="One location all vertex models">
...
as dynamic params for a `mishikallm.completion` call. 
...
response = completion(
    model="vertex_ai/gemini-pro", 
    messages=[{"role": "user", "content": "write code for saying hi from MishikaLLM"}]
    safety_settings=[
        {
...
model_list:
  - model_name: gemini-experimental
    mishikallm_params:
      ...
      vertex_project: mishikallm-epic
      vertex_location: us-central1
      safety_settings:
      - category: HARM_CATEGORY_HARASSMENT
        threshold: BLOCK_NONE
...
mishikallm.set_verbose = True 👈 See RAW REQUEST/RESPONSE 

mishikallm.vertex_ai_safety_settings = [
        {
            "category": "HARM_CATEGORY_HARASSMENT",
            "threshold": "BLOCK_NONE",
        },
...
# set directly on module 
mishikallm.vertex_location = "us-central1 # Your Location
```
## Anthropic 
...
You can call fine-tuned Vertex AI Gemini models through MishikaLLM
...
```yaml showLineNumbers title="Add to mishikallm config"
...
--header 'Content-Type: application/json' \
--header 'Authorization: <MISHIKALLM_KEY>' \
...
Call `gemini-pro-vision` in the same input/output format as OpenAI [`gpt-4-vision`](https://docs.21t.cc/docs/providers/openai#openai-vision-models)

MishikaLLM Supports the following image types passed in `url`
...
MishikaLLM supports Function Calling for Vertex AI gemini models. 
...
Pass any file supported by Vertex AI, through MishikaLLM. 

MishikaLLM Supports the following file types passed in url. 
...
| [param](../embedding/supported_embedding.md#input-params-for-mishikallmembedding) | type | [vertex equivalent](https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/text-embeddings-api) |
|-------|-------------|--------------------|
...
response = mishikallm.embedding(
    ...
    input=["good morning from mishikallm", "gm"]
...
<TabItem value="proxy" label="MishikaLLM PROXY (Unified Endpoint)">
...
<TabItem value="proxy-vtx" label="MishikaLLM PROXY (Vertex SDK)">
...
MISHIKALLM_PROXY_BASE = "http://0.0.0.0:4000/vertex-ai"
...
credentials = CredentialsWrapper(token=MISHIKALLM_PROXY_API_KEY)

vertexai.init(
    project="adroit-crow-413218",
    location="us-central1",
    api_endpoint=MISHIKALLM_PROXY_BASE,
...
| Model Name                   | FUsage                                            |
|------------------------------|--------------------------------------------------------------|
| `imagen-3.0-generate-001`      | `mishikallm.image_generation('vertex_ai/imagen-3.0-generate-001', prompt)` |
| `imagen-3.0-fast-generate-001` | `mishikallm.image_generation('vertex_ai/imagen-3.0-fast-generate-001', prompt)` |
| `imagegeneration@006`          | `mishikallm.image_generation('vertex_ai/imagegeneration@006', prompt)`  |
| `imagegeneration@005`          | `mishikallm.image_generation('vertex_ai/imagegeneration@005', prompt)`  |
| `imagegeneration@002`          | `mishikallm.image_generation('vertex_ai/imagegeneration@002', prompt)`  |
...
MishikaLLM supports calling [Vertex AI Text to Speech API](https://console.cloud.google.com/vertex-ai/generative/speech/text-to-speech) in the OpenAI text to speech API format
...
export GCS_BUCKET_NAME = "mishikallm-testing-bucket" # the bucket you want to store batch prediction files in
...
MishikaLLM expects the file to follow the **[OpenAI batches files format](https://platform.openai.com/docs/guides/batch)**
...
For `vertex_ai` mishikallm will upload the file to the provided `GCS_BUCKET_NAME`

```python
import os
oai_client = OpenAI(
    api_key="sk-1234",               # mishikallm proxy API key
    base_url="http://localhost:4000" # mishikallm proxy base url
)
...
file_obj = oai_client.files.create(
    ...
    purpose="batch",
    extra_body={"custom_llm_provider": "vertex_ai"}, # tell mishikallm to use vertex_ai for this file upload
)
```
...
```json
{
    "id": "gs://mishikallm-testing-bucket/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/d3f198cd-c0d1-436d-9b1e-28e3f282997a",
    "bytes": 416,
    "created_at": 1733392026,
    "filename": "mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/d3f198cd-c0d1-436d-9b1e-28e3f282997a",
    "object": "file",
    "purpose": "batch",
    "status": "uploaded",
    "status_details": null
...
create_batch_response = oai_client.batches.create(
    ...
    input_file_id=batch_input_file_id, # example input_file_id = "gs://mishikallm-testing-bucket/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/c2b1b785-252b-448c-b180-033c4c63b3ce"
    extra_body={"custom_llm_provider": "vertex_ai"}, # tell mishikallm to use `vertex_ai` for this batch request
)
```
...
```json
{
    ...
    "created_at": 1733392026,
    "endpoint": "",
    "input_file_id": "gs://mishikallm-testing-bucket/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/d3f198cd-c0d1-436d-9b1e-28e3f282997a",
    "object": "batch",
    "status": "validating",
    "cancelled_at": null,
    "cancelling_at": null,
    ...
    "in_progress_at": null,
    "metadata": null,
    "output_file_id": "gs://mishikallm-testing-bucket/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001",
    "request_counts": null
}
```
...
```json
{
    ...
    "endpoint": "",
    "input_file_id": "gs://example-bucket-1-mishikallm/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001/7b2e47f5-3dd4-436d-920f-f9155bbdc952",
    "object": "batch",
    "status": "completed",
    "cancelled_at": null,
    "cancelling_at": null,
    ...
    "in_progress_at": null,
    "metadata": null,
    "output_file_id": "gs://example-bucket-1-mishikallm/mishikallm-vertex-files/publishers/google/models/gemini-1.5-flash-001",
    "request_counts": null
}
```
...
ft_job = await client.fine_tuning.jobs.create(
    ...
    extra_body={"custom_llm_provider": "vertex_ai"}, # tell mishikallm proxy which provider to use
)
```
</TabItem>
...
Trying to deploy MishikaLLM on Google Cloud Run? Tutorial [here](https://docs.21t.cc/docs/proxy/deploy#deploy-on-google-cloud-run)
...
Once that's done, when you deploy the new container in the Google Cloud Run service, MishikaLLM will have automatic access to all Vertex AI endpoints.
