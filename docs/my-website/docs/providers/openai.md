...
# OpenAI
MishikaLLM supports OpenAI Chat + Embedding calls.
...
MishikaLLM supports OpenAI Audio Transcription endpoint.
...
Set `mishikallm.return_response_headers = True` to get raw response headers from OpenAI

You can expect to always get the `_response_headers` field from `mishikallm.completion()`, `mishikallm.embedding()` functions

<Tabs>
<TabItem value="mishikallm.completion" label="mishikallm.completion">
...
<TabItem value="mishikallm.completion - streaming" label="mishikallm.completion + stream">
...
<TabItem value="mishikallm.embedding" label="mishikallm.embedding">
...
- For `mishikallm.completion` set `mishikallm.client_session=httpx.Client(verify=False)`
- For `mishikallm.acompletion` set `mishikallm.aclient_session=AsyncClient.Client(verify=False)`
```python
import mishikallm, httpx

# for completion
mishikallm.client_session = httpx.Client(verify=False)
response = mishikallm.completion(
    model="gpt-3.5-turbo",
    messages=messages,
)
...
# for acompletion
mishikallm.aclient_session = httpx.AsyncClient(verify=False)
response = mishikallm.acompletion(
    model="gpt-3.5-turbo",
    messages=messages,
)
```
...
### Using OpenAI Proxy with MishikaLLM
```python
import os 
import mishikallm
from mishikallm import completion
...
mishikallm.api_base = "https://your_host/v1"
...
```bash
MishikaLLM:DEBUG: utils.py:255 - Request to mishikallm:
MishikaLLM:DEBUG: utils.py:255 - mishikallm.acompletion(... organization='my-special-org',)
```