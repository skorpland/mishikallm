...
MishikaLLM supports all models on Replicate
...
This is the call mishikallm will make to replicate, from the above example: 
...
Add the `replicate/deployments/` prefix to your model, so mishikallm will call the `deployments` endpoint. This will call `ishaan-jaff/ishaan-mistral` deployment on replicate
...
Replicate responses can take 3-5 mins due to replicate cold boots, if you're trying to debug try making the request with `mishikallm.set_verbose=True`. [More info on replicate cold boots](https://replicate.com/docs/how-does-replicate-work#cold-boots)

:::

## Replicate Models
mishikaLLM supports all replicate LLMs

For replicate models ensure to add a `replicate/` prefix to the `model` arg. mishikaLLM detects it using this arg. 

Below are examples on how to call replicate LLMs using mishikaLLM 
...
Send params [not supported by `mishikallm.completion()`](https://docs.21t.cc/docs/completion/input) but supported by Replicate by passing them to `mishikallm.completion`
