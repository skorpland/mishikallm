...
| Provider Route on MishikaLLM | `azure/`, [`azure/o_series/`](#azure-o-series-models) |
...
## API Keys, Params
api_key, api_base, api_version etc can be passed directly to `mishikallm.completion` - see here or set as `mishikallm.api_key` params see here
```python
import os
...
Here's how to call Azure OpenAI models with the MishikaLLM Proxy Server
...
model_list:
  - model_name: gpt-3.5-turbo
    mishikallm_params:
      ...
      api_key: os.environ/AZURE_API_KEY # The `os.environ/` prefix tells mishikallm to read this from the env.
```
...
**We support ALL Azure models, just set `model=azure/<your deployment name>` as a prefix when sending mishikallm requests**
...
mishikallm.completion(model="azure/my-o3-deployment", messages=[{"role": "user", "content": "Hello, world!"}]) # 👈 Note: 'o3' in the deployment name
```
</TabItem>
...
mishikallm.completion(model="azure/o_series/my-random-deployment-name", messages=[{"role": "user", "content": "Hello, world!"}]) # 👈 Note: 'o_series/' in the deployment name
```
</TabItem>
<TabItem value="proxy" label="PROXY">
...
This is a walkthrough on how to use Azure Active Directory Tokens - Microsoft Entra ID to make `mishikallm.completion()` calls 
...
Step 4 - Make mishikallm.completion call with Azure AD token
...
Here is an example of setting up `tenant_id`, `client_id`, `client_secret` in your mishikallm proxy `config.yaml`
```yaml
model_list:
  - model_name: gpt-3.5-turbo
    mishikallm_params:
...
Example video of using `tenant_id`, `client_id`, `client_secret` with MishikaLLM Proxy Server
...
Here is an example of setting up `client_id`, `azure_username`, `azure_password` in your mishikallm proxy `config.yaml`
```yaml
model_list:
  - model_name: gpt-3.5-turbo
    mishikallm_params:
...
| `custom_llm_provider` on MishikaLLM | `azure/` |
...
| Cost Tracking, Logging Support | ✅ MishikaLLM will log, track cost for Batch API Requests |
...
```python
list_batches_response = mishikallm.list_batches(
    custom_llm_provider="azure", 
    limit=2
)
...
response = mishikallm.responses(
    ...
    api_base="https://mishikallm8397336933.openai.azure.com/",
...
<TabItem value="proxy" label="OpenAI SDK with MishikaLLM Proxy">
...
See a detailed walthrough of parallel function calling with mishikallm [here](https://docs.21t.cc/docs/completion/function_call)
