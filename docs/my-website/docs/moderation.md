...
For `/moderations` endpoint, there is **no need to specify `model` in the request or on the mishikallm config.yaml**

Start mishikallm proxy server 
...
## Input Params
MishikaLLM accepts and translates the [OpenAI Moderation params](https://platform.openai.com/docs/api-reference/moderations) across all supported providers.
...
[**MishikaLLM follows OpenAI's output format**](https://platform.openai.com/docs/api-reference/moderations/object)
