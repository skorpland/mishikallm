...
We create a custom callback, to log to slack webhooks, see [custom callbacks on mishikallm](https://docs.21t.cc/docs/observability/custom_callback)
```python
def send_slack_alert(
    ...
    # Remove api_key from kwargs under mishikallm_params
    if kwargs.get('mishikallm_params'):
        kwargs['mishikallm_params'].pop('api_key', None)
        if kwargs['mishikallm_params'].get('metadata'):
            kwargs['mishikallm_params']['metadata'].pop('deployment', None)
    ...
    # Define the text payload, send data available in mishikallm custom_callbacks
    text_payload = f"""MishikaLLM Logging: kwargs: {str(kwargs)}\n\n, response: {str(completion_response)}\n\n, start time{str(start_time)} end time: {str(end_time)}
    """
    payload = {
        "text": text_payload
    }
...
### Pass callback to MishikaLLM
```python
mishikallm.success_callback = [send_slack_alert]
```

```python
import mishikallm
mishikallm.success_callback = [send_slack_alert] # log success
mishikallm.failure_callback = [send_slack_alert] # log exceptions
