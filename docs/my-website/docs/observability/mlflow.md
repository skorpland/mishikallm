...
MLflow’s integration with MishikaLLM supports advanced observability compatible with OpenTelemetry.
...
To enable MLflow auto tracing for MishikaLLM:
...
# Alternative, you can set the callback manually in MishikaLLM
# mishikallm.callbacks = ["mlflow"]
```
...
MLflow integration with MishikaLLM support tracking tool calls in addition to the messages.

```python
import mlflow

# Enable MLflow auto-tracing for MishikaLLM
mlflow.mishikallm.autolog()
...
MLflow MishikaLLM integration allow you to run qualitative assessment against LLM to evaluate or/and monitor your GenAI application.

Visit [Evaluate LLMs Tutorial](../tutorials/eval_suites.md) for the complete guidance on how to run evaluation suite with MishikaLLM and MLflow.
...
## Combine MishikaLLM Trace with Your Application Trace

MishikaLLM is often part of larger LLM applications, such as agentic models. MLflow Tracing allows you to instrument custom Python code, which can then be combined with MishikaLLM traces.
...
This approach generates a unified trace, combining your custom Python code with MishikaLLM calls.
