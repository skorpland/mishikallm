...
# [OLD PROXY 👉 [NEW proxy here](./simple_proxy)] Local MishikaLLM Proxy Server
...
config_list=[
    {
        "model": "my-fake-model",
        "api_base": "http://0.0.0.0:8000",  #mishikallm compatible endpoint
        "api_type": "open_ai",
...
$ mishikallm --model ollama/codellama --temperature 0.3 --max_tokens 2048
```
...
#INFO: mishikallm proxy running on http://0.0.0.0:8000
```
...
config_list=[
    {
        "model": "my-fake-model",
        "api_base": "http://localhost:8000",  #mishikallm compatible endpoint
        "api_type": "open_ai",
        "api_key": "NULL", # just a placeholder
    }
]
...
# configure the LLM
my_llm_config = OpenAIGPTConfig(
    #format: "local/[URL where MishikaLLM proxy is listening]
...
If you need to: 
* save API keys 
* set mishikallm params (e.g. drop unmapped params, set fallback models, etc.)
...
#### Save API Keys 
```shell 
$ mishikallm --api_key OPENAI_API_KEY=sk-...
```
MishikaLLM will save this to a locally stored config file, and persist this across sessions. 

MishikaLLM Proxy supports all mishikallm supported api keys. To add keys for a specific provider, check this list:

<Tabs>
<TabItem value="huggingface" label="Huggingface">

```shell
$ mishikallm --add_key HUGGINGFACE_API_KEY=my-api-key #[OPTIONAL]
```
...
```shell
$ mishikallm --add_key ANTHROPIC_API_KEY=my-api-key
```
...
```shell
$ mishikallm --add_key PERPLEXITYAI_API_KEY=my-api-key
```
...
```shell
$ mishikallm --add_key TOGETHERAI_API_KEY=my-api-key
```
...
```shell
$ mishikallm --add_key REPLICATE_API_KEY=my-api-key
```
...
```shell
$ mishikallm --add_key AWS_ACCESS_KEY_ID=my-key-id
$ mishikallm --add_key AWS_SECRET_ACCESS_KEY=my-secret-access-key
```
...
```shell
$ mishikallm --add_key PALM_API_KEY=my-palm-key
```
...
```shell
$ mishikallm --add_key AZURE_API_KEY=my-api-key
$ mishikallm --add_key AZURE_API_BASE=my-api-base
...
```shell
$ mishikallm --add_key AI21_API_KEY=my-api-key
```
...
```shell
$ mishikallm --add_key COHERE_API_KEY=my-api-key
```
...
**For that session**: 
```shell
mishikallm --model ollama/llama2 \
