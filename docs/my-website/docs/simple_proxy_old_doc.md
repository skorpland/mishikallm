...
### Using MishikaLLM Proxy - Curl Request, OpenAI Package
...
### Supported LLMs
All MishikaLLM supported LLMs are supported on the Proxy. Seel all [supported llms](https://docs.21t.cc/docs/providers)
<Tabs>
...
```shell
$ mishikallm --model huggingface/<your model name> --api_base http://0.0.0.0:8001
```
...
```shell
$ mishikallm --model sagemaker/jumpstart-dft-meta-textgeneration-llama-2-7b
```
...
Set `base_url` to the MishikaLLM Proxy server
...
MishikaLLM Proxy is running on port `4000`, set `4000` as the proxy below
```yaml
...
config_list=[
    {
        "model": "my-fake-model",
        "api_base": "http://localhost:4000",  #mishikallm compatible endpoint
        "api_type": "open_ai",
...
```shell
mishikallm --model ollama/codellama --temperature 0.3 --max_tokens 2048 --drop_params
```
...
| `mishikallm_settings`   | mishikallm Module settings, example `mishikallm.drop_params=True`, `mishikallm.set_verbose=True`, `mishikallm.api_base`, `mishikallm.cache` |
...
Use this config to load balance between multiple instances of the same model. The proxy will handle routing requests (using MishikaLLM's Router). **Set `rpm` in the config if you want maximize throughput**
...
Here's how you can use multiple llms with one proxy `config.yaml`. 
Here is how [MishikaLLM calls OpenAI Compatible Embedding models](https://docs.21t.cc/docs/embedding/supported_embedding#openai-compatible-embedding-models)
...
model_list:
  - model_name: custom_embedding_model
    mishikallm_params:
      model: openai/custom_embedding  # the `openai/` prefix tells mishikallm it's openai compatible
...
mishikallm_settings:
  set_verbose: True
  cache:          # init cache
    type: redis   # tell mishikallm to use redis caching
```

#### Step 2: Add Redis Credentials to .env
MishikaLLM requires the following REDIS credentials in your env to enable caching
...
```shell
mishikallm --model gpt-3.5-turbo --debug
```
...
```shell
mishikallm --model gpt-3.5-turbo --detailed_debug
```
...
You can also run `mishikallm -health` it makes a `get` request to `http://0.0.0.0:4000/health` for you
```
mishikallm --health
```
#### Response
...
### Step 2 Configure MishikaLLM proxy to log on OpenTelemetry
...
general_settings: 
  otel: True      # set OpenTelemetry=True, on mishikallm Proxy

```

#### Set OTEL collector endpoint
MishikaLLM will read the `OTEL_ENDPOINT` environment variable to send data to your OTEL collector 
...
curl --location 'http://0.0.0.0:4000/chat/completions' \
    ...
    "messages": [
        {
        "role": "user",
        "content": "request from MishikaLLM testing"
        }
    ]
    }'
```
...
SpanEvent #1
     -> Name: MishikaLLM: Request Headers
...
We will use the `--config` to set `mishikallm.success_callback = ["langfuse"]` this will log all successful LLM calls to langfuse
