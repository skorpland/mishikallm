...
| ✨ Cost Tracking | ✅ | MishikaLLM Enterprise only |
...
```python
list_batches_response = mishikallm.list_batches(custom_llm_provider="openai", limit=2)
...
MishikaLLM tracks batch processing costs by logging two key events:
...
- MishikaLLM polls the batch status until completion
...
## [Swagger API Reference](https://mishikallm-api.up.railway.app/#/batch)
