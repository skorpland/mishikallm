...
| End-user Tracking | ✅ | disable prometheus tracking via `mishikallm.disable_end_user_cost_tracking_prometheus_only`|
| Streaming | ✅ | |

Just replace `https://api.anthropic.com` with `MISHIKALLM_PROXY_BASE_URL/anthropic`
...
curl http://0.0.0.0:4000/anthropic/v1/messages \
     --header "x-api-key: $MISHIKALLM_API_KEY" \
...
|------------------------------------------------------|-----------------------------------|
| `https://api.anthropic.com`          | `http://0.0.0.0:4000/anthropic` (MISHIKALLM_PROXY_BASE_URL="http://0.0.0.0:4000")      |
| `bearer $ANTHROPIC_API_KEY`                                 | `bearer anything` (use `bearer MISHIKALLM_VIRTUAL_KEY` if Virtual Keys are setup on proxy)                    |
...
### Send `mishikallm_metadata` (tags, end-user cost tracking)
