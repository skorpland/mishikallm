...
Pass-through endpoints for Langfuse - call langfuse endpoints with MishikaLLM Virtual Key.

Just replace `https://us.cloud.langfuse.com` with `MISHIKALLM_PROXY_BASE_URL/langfuse` 🚀
...
langfuse = Langfuse(
    host="http://localhost:4000/langfuse", # your mishikallm proxy endpoint
    public_key="anything",        # no key required since this is a pass through
    secret_key="MISHIKALLM_VIRTUAL_KEY",        # no key required since this is a pass through
)
