...
Just replace `https://generativelanguage.googleapis.com` with `MISHIKALLM_PROXY_BASE_URL/gemini`
...
const genAI = new GoogleGenerativeAI("sk-1234"); // mishikallm proxy API key
...
|------------------------------------------------------|-----------------------------------|
| `https://generativelanguage.googleapis.com`          | `http://0.0.0.0:4000/gemini` (MISHIKALLM_PROXY_BASE_URL="http://0.0.0.0:4000")      |
| `key=$GOOGLE_API_KEY`                                 | `key=anything` (use `key=MISHIKALLM_VIRTUAL_KEY` if Virtual Keys are setup on proxy)                    |
...
Use this if you want `tags` to be tracked in the MishikaLLM DB and on logging callbacks.
