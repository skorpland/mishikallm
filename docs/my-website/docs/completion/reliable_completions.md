...
MishikaLLM helps prevent failed requests in 2 ways: 
- Retries
- Fallbacks: Context Window + General

## Helper utils 
MishikaLLM supports the following functions for reliability:
* `mishikallm.longer_context_model_fallback_dict`: Dictionary which has a mapping for those models which have larger equivalents  
    ...
    while response == None and time.time() - start_time < 45:
        for model in fallbacks:
            ...
            try:
                ...
                response = mishikallm.completion(**kwargs, model=model)
