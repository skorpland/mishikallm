...
## Common Params 
MishikaLLM accepts and translates the [OpenAI Chat Completion params](https://platform.openai.com/docs/api-reference/chat/create) across all providers. 
...
```python
from mishikallm import get_supported_openai_params
...
Use `mishikallm.get_supported_openai_params()` for an updated list of params for each model + provider 
...
By default, MishikaLLM raises an exception if the openai param being passed in isn't supported. 

To drop the param instead, set `mishikallm.drop_params = True` or `completion(..drop_params=True)`.

This **ONLY DROPS UNSUPPORTED OPENAI PARAMS**. 

MishikaLLM assumes any non-openai param is provider specific and passes it in as a kwarg in the request body
...
[**See All Message Values**](https://github.com/skorpland/mishikallm/blob/8600ec77042dacad324d3879a2bd918fc6a719fa/mishikallm/types/llms/openai.py#L392)
