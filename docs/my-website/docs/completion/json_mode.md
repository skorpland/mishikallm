...
Call `mishikallm.get_supported_openai_params` to check if a model/provider supports `response_format`. 
...
```python
from mishikallm import supports_response_schema
...
Not all vertex models support passing the json_schema to them (e.g. `gemini-1.5-flash`). To solve this, MishikaLLM supports client-side validation of the json schema. 
...
mishikallm.enable_json_schema_validation = True
mishikallm.set_verbose = True # see the raw request made by mishikallm
