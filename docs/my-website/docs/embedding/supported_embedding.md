...
response = embedding(model='text-embedding-ada-002', input=["good morning from mishikallm"])
```
## Proxy Usage 
...
## Input Params for `mishikallm.embedding()`
...
### Output from `mishikallm.embedding()`
...
**Note add `openai/` prefix to model so mishikallm knows to route to OpenAI**
...
MishikaLLM supports all Feature-Extraction + Sentence Similarity Embedding models: https://huggingface.co/models?pipeline_tag=feature-extraction
...
MishikaLLM infers input type (feature-extraction or sentence-similarity) by making a GET request to the api base. 
...
response = embedding(
    model='huggingface/microsoft/codebert-base', 
    input=["good morning from mishikallm", "you are a good bot"],
