...
Create a playground to **evaluate multiple LLM Providers in less than 10 minutes**. If you want to see this in prod, check out our [website](https://21t.cc/).

**What will it look like?**
<Image
  img={require('../../img/mishikallm_streamlit_playground.png')}
...
**Notes**:
* 🚨 Don't forget to replace the placeholder key values with your keys!
* `completion_with_retries`: LLM API calls can fail in production. This function wraps the normal mishikallm completion() call with [tenacity](https://tenacity.readthedocs.io/en/latest/) to retry the call in case it fails. 

MishikaLLM specific snippet:

```python 
import os
from mishikallm import completion_with_retries 
...
```zsh
cd mishikallm_playground_fe_template && streamlit run app.py
```
...
This is what you should see: 
<Image img={require('../../img/mishikallm_streamlit_playground.png')} alt="streamlit_playground" />
...
Next Steps: 
* [Check out the full list of LLM Providers you can now add](https://docs.21t.cc/docs/providers)