...
```shell
$ mishikallm --model huggingface/codellama/CodeLlama-34b-Instruct-hf --api_base https://my-endpoint.com
...
So instead of using the MishikaLLM default, let's use our own prompt template to use these in our messages. 

## Step 2: Create Custom Prompt Template

Our mishikallm server accepts prompt templates as part of a config file. You can save api keys, fallback models, prompt templates etc. in this config. [See a complete config file](../proxy_server.md)

For now, let's just create a simple config file with our prompt template, and tell our server about it. 

Create a file called `mishikallm_config.toml`:

```shell
$ touch mishikallm_config.toml
```
We want to add:
...
Let's save our custom template to our mishikallm server by running:
```shell
$ mishikallm --config -f ./mishikallm_config.toml 
```
MishikaLLM will save a copy of this file in it's package, so it can persist these settings across restarts.
