...
- `Internal Users` - users that can create keys, make LLM API calls, view usage on MishikaLLM 
- `Roles` define the permissions of an `Internal User`
- `Virtual Keys` - Keys are used for authentication to the MishikaLLM API. Keys are tied to a `Internal User` and `Team` 
...
[**API Reference for /organization/new**](https://mishikallm-api.up.railway.app/#/organization%20management/new_organization_organization_new_post)
