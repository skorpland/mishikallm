...
from mishikallm.proxy.guardrails.guardrail_helpers import should_proceed_based_on_metadata
...
class myCustomGuardrail(CustomGuardrail):
    ...
    async def async_pre_call_hook(
        ...
        # In this guardrail, if a user inputs `mishikallm` we will mask it and then send it to the LLM
        _messages = data.get("messages")
        if _messages:
            for message in _messages:
                _content = message.get("content")
                if isinstance(_content, str):
                    if "mishikallm" in _content.lower():
                        _content = _content.replace("mishikallm", "********")
    ...
    async def async_moderation_hook(
        ...
        # In this guardrail, if a user inputs `mishikallm` we will mask it.
        _messages = data.get("messages")
        if _messages:
            for message in _messages:
                _content = message.get("content")
                if isinstance(_content, str):
                    if "mishikallm" in _content.lower():
                        raise ValueError("Guardrail failed words - `mishikallm` detected")
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, mishikallm.ModelResponse):
            for choice in response.choices:
                if isinstance(choice, mishikallm.Choices):
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        See Aim guardrail implementation for an example - https://github.com/skorpland/mishikallm/blob/d0e022cfacb8e9ebc5409bb652059b6fd97b45c0/mishikallm/proxy/guardrails/guardrail_hooks/aim.py#L168
...
### 2. Pass your custom guardrail class in MishikaLLM `config.yaml`
...
Mount your `custom_guardrail.py` on the MishikaLLM Docker container

This mounts your `custom_guardrail.py` file from your local directory to the `/app` directory in the Docker container, making it accessible to the MishikaLLM Gateway.
...
Expect this to mask the word `mishikallm` before sending the request to the LLM API. [This runs the `async_pre_call_hook`](#1-write-a-customguardrail-class)
...
-d '{
    ...
    "messages": [
        {
            "role": "user",
            "content": "say the word - `mishikallm`"
        }
    ],
...
Expect this to fail since since `mishikallm` is in the message content. [This runs the `async_moderation_hook`](#1-write-a-customguardrail-class)
...
{
  "error": {
    "message": "Guardrail failed words - `mishikallm` detected",
    "type": "None",
    "param": "None",
    "code": "500"
  }
...
✨ This is an Enterprise only feature [Contact us to get a free trial](https://calendly.com/d/4mp-gd3-k5k/mishikallm-1-1-onboarding-chat)
...
`get_guardrail_dynamic_request_body_params` is a method of the `mishikallm.integrations.custom_guardrail.CustomGuardrail` class that fetches the dynamic guardrail params passed in the request body.
...
MishikaLLM Proxy allows you to pass `guardrails` in the request body, following the [`guardrails` spec](quick_start#spec-guardrails-parameter).
