...
Setup Prompt Injection Detection, PII Masking on MishikaLLM Proxy (AI Gateway)

## 1. Define guardrails on your MishikaLLM config.yaml
...
Your response headers will include `x-mishikallm-applied-guardrails` with the guardrail applied 

```
x-mishikallm-applied-guardrails: aporia-pre-guard
```
...
✨ This is an Enterprise only feature [Get a free trial](https://www.21t.cc/#trial)
...
1. Connect MishikaLLM to a [supported logging provider](../logging)
...
guardrails:
  - guardrail_name: string     # Required: Name of the guardrail
    mishikallm_params:            # Required: Configuration parameters
...
The `guardrails` parameter can be passed to any MishikaLLM Proxy endpoint (`/chat/completions`, `/completions`, `/embeddings`).
