...
```shell
x-mishikallm-model-api-base: https://exampleopenaiendpoint-production.up.railway.app/
```
...
Expect to see the following response header when this works
```shell
x-mishikallm-model-api-base: https://api.openai.com
```
...
You can also call via request header `x-mishikallm-tags`

```shell
curl -L -X POST 'http://0.0.0.0:4000/v1/chat/completions' \
-H 'Content-Type: application/json' \
-H 'Authorization: Bearer sk-1234' \
-H 'x-mishikallm-tags: free,my-custom-tag' \
-d '{
  "model": "gpt-4",
  "messages": [
    {
...
Expect to see the following response header when this works
```shell
x-mishikallm-model-id: default-model
```

## ✨ Team based tag routing (Enterprise)

MishikaLLM Proxy supports team-based tag routing, allowing you to associate specific tags with teams and route requests accordingly. Example **Team A can access gpt-4 deployment A, Team B can access gpt-4 deployment B** (LLM Access Control For Teams)

:::info

This is an enterprise feature, [Contact us here to get a free trial](https://calendly.com/d/4mp-gd3-k5k/mishikallm-1-1-onboarding-chat)
...
4. **Verify routing:**
   ...
   Check the `x-mishikallm-model-id` header in the response to confirm that the request was routed to the correct model based on the team's tags. You can use the `-i` flag with curl to include the response headers:
   ...
   ```
   x-mishikallm-model-id: team-a-model
   ```

   Similarly, when using Team B's key, you should see:
   ```
   x-mishikallm-model-id: team-b-model
   ```

By following these steps and using these curl commands, you can implement and test team-based tag routing in your MishikaLLM Proxy setup, ensuring that different teams are routed to the appropriate models or deployments based on their assigned tags.
