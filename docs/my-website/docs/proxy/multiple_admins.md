...
Requires Enterprise License, Get in touch with us [here](https://calendly.com/d/4mp-gd3-k5k/mishikallm-1-1-onboarding-chat)

:::

## 1. Switch on audit Logs 
Add `store_audit_logs` to your mishikallm config.yaml and then start the proxy
```shell
mishikallm_settings:
  store_audit_logs: true
```

## 2. Set `MishikaLLM-Changed-By` in request headers

Set the 'user_id' in request headers, when calling a management endpoint. [View Full List](https://mishikallm-api.up.railway.app/#/team%20management).

- Update Team budget with master key. 
- Attribute change to 'krrish@berri.ai'. 

**👉 Key change:** Passing `-H 'MishikaLLM-Changed-By: krrish@berri.ai'`

```shell
curl -X POST 'http://0.0.0.0:4000/team/update' \
    -H 'Authorization: Bearer sk-1234' \
    -H 'MishikaLLM-Changed-By: krrish@berri.ai' \
...
### `changed_by`
- **Type:** `String`
- **Description:** The `user_id` that performed the audited action. If `MishikaLLM-Changed-By` Header is passed then `changed_by=<value passed for MishikaLLM-Changed-By header>`
...
### `table_name`
- **Type:** `String`
- **Description:** This field stores the name of the table that was affected by the audited action. It can be one of the following values: `MishikaLLM_TeamTable`, `MishikaLLM_UserTable`, `MishikaLLM_VerificationToken`
