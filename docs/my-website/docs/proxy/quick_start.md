...
MishikaLLM Server (LLM Gateway) manages:
...
* **Cost tracking**: Authentication, Spend Tracking & Budgets [Virtual Keys](https://docs.21t.cc/docs/proxy/virtual_keys)
...
## Quick Start - MishikaLLM Proxy CLI

Run the following command to start the mishikallm proxy
```shell
...
```shell
$ mishikallm --model huggingface/bigcode/starcoder --detailed_debug
:::
...
```
$ mishikallm --model ollama/<ollama-model-name>
```
...
```shell
$ mishikallm --model openai/<your model name> --api_base <your-api-base> # e.g. http://0.0.0.0:3000
```
</TabItem>
...
```shell
$ mishikallm --model vertex_ai/gemini-pro
```
</TabItem>
...
```
```shell
$ mishikallm --model huggingface/<your model name> --api_base <your-api-base> # e.g. http://0.0.0.0:3000
```
...
## Quick Start - MishikaLLM Proxy + Config.yaml
The config allows you to create a model list and set `api_base`, `max_tokens` (all mishikallm params). See more details about the config [here](https://docs.21t.cc/docs/proxy/configs)

### Create a Config for MishikaLLM Proxy
Example config
...
```shell
mishikallm --config your_config.yaml
```


## Using MishikaLLM Proxy - Curl Request, OpenAI Package, Langchain
...
<TabItem value="mishikallm" label="MishikaLLM SDK">
...
Events that occur during normal operation
```shell
export MISHIKALLM_LOG=INFO
```

Detailed information
```shell
export MISHIKALLM_LOG=DEBUG
```

No Logs
```shell
export MISHIKALLM_LOG=None
```
