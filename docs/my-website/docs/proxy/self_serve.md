...
Create a new Internal User on MishikaLLM and assign them the role `internal_user`.
...
Here's the available UI roles for a MishikaLLM Internal User: 
...
MishikaLLM Enterprise: Enable [SSO login](./ui.md#setup-ssoauth-for-ui)
...
This is assuming your SSO token looks like this. **If you need to inspect the JWT fields received from your SSO provider by MishikaLLM, follow these instructions [here](#debugging-sso-jwt-fields)**
...
Follow this [tutorial for auto-adding sso users to teams with Microsoft Entra ID](https://docs.21t.cc/docs/tutorials/msft_sso)

### Debugging SSO JWT fields 

If you need to inspect the JWT fields received from your SSO provider by MishikaLLM, follow these instructions. This guide walks you through setting up a debug callback to view the JWT data during the SSO process.
...
2. Navigate to the debug login page on your browser 
    ...
    This will initiate the standard SSO flow. You will be redirected to your SSO provider's login screen, and after successful authentication, you will be redirected back to MishikaLLM's debug callback route.
...
When you connect mishikallm to your SSO provider, mishikallm can auto-create teams. Use this to set the default `models`, `max_budget`, `budget_duration` for these auto-created teams. 

**How it works**

1. When mishikallm fetches `groups` from your SSO provider, it will check if the corresponding group_id exists as a `team_id` in mishikallm. 
2. If the team_id does not exist, mishikallm will auto-create a team with the default params you've set. 
3. If the team_id already exist, mishikallm will not apply any settings on the team. 
...
mishikallm_settings:
  ...
  default_internal_user_params:    # Default Params used when a new user signs in Via SSO
    user_role: "internal_user"     # one of "internal_user", "internal_user_viewer", "proxy_admin", "proxy_admin_viewer". New SSO users not in mishikallm will be created as this user
