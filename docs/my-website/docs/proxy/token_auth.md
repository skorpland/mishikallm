...
✨ JWT-based Auth  is on MishikaLLM Enterprise
...
Create a client scope called `mishikallm_proxy_admin` in your OpenID provider (e.g. Keycloak).

Grant your user, `mishikallm_proxy_admin` scope when generating a JWT. 
...
--data-urlencode 'scope=mishikallm_proxy_admin' # 👈 grant this scope
```
</TabItem>
...
Use this if you want MishikaLLM to validate your JWT against multiple OIDC providers (e.g. Google Cloud, GitHub Auth)
...
Change the string in JWT 'scopes', that mishikallm evaluates to see if a user has admin access.
...
general_settings:
  ...
  enable_jwt_auth: True
  mishikallm_jwtauth:
    admin_jwt_scope: "mishikallm-proxy-admin"
```

### Tracking End-Users / Internal Users / Team / Org

Set the field in the jwt token, which corresponds to a mishikallm user / team / org.
...
Now mishikallm will automatically update the spend for the user/team/org in the db for each call. 
...
2. Create the teams on MishikaLLM 
...
- Validate if user id is in the DB (MishikaLLM_UserTable)
- Validate if any of the groups are in the DB (MishikaLLM_TeamTable)
...
Validate a JWT Token using custom logic, if you need an extra way to verify if tokens are valid for MishikaLLM Proxy.
...
general_settings:
  ...
  mishikallm_jwtauth:
    ...
    team_id_jwt_field: "mishikallm-team" # 👈 Set field in the JWT token that stores the team ID
...
--header 'Content-Type: application/json' \
--data '{
    "team_id": "mishikallm-test-client-id-new" # 👈 set team id
}'
```
...
environment_variables:
  JWT_AUDIENCE: "api://MishikaLLM_Proxy" # ensures audience is validated
```
...
- `roles_jwt_field`: The field in the JWT token that contains the roles. This field is a list of roles that the user has. To index into a nested field, use dot notation - eg. `resource_access.mishikallm-test-client-id.roles`.

- `role_mappings`: A list of role mappings. Map the received role in the JWT token to an internal role on MishikaLLM.
...
```bash
{
  "aud": "api://MishikaLLM_Proxy",
  "oid": "eec236bd-0135-4b28-9354-8fc4032d543e",
  "roles": ["mishikallm.api.consumer"] 
}
```
...
- `internal_role`: The internal role on MishikaLLM that will be used to control access. 
...
```bash
{
  "scope": ["mishikallm.api.consumer", "mishikallm.api.gpt_3_5_turbo"] # can be a list or a space-separated string
}
```
