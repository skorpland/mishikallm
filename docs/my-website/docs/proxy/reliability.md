...
2. mishikallm proxy will loop through all the model_groups specified in `fallbacks=["gpt-3.5-turbo"]`
...
If all models in a group are in cooldown (e.g. rate limited), MishikaLLM will fallback to the model with the specific model ID.
...
Validate it works, by checking the response header `x-mishikallm-model-id`

```bash
x-mishikallm-model-id: my-specific-model-id
```
...
**Note:** MishikaLLM can automatically infer region_name for Vertex AI, Bedrock, and IBM WatsonxAI based on your mishikallm params. For Azure, set `mishikallm.enable_preview = True`.
...
print(f"response.headers.get('x-mishikallm-model-api-base')")
```
