...
To get a license, get in touch with us [here](https://calendly.com/d/4mp-gd3-k5k/mishikallm-1-1-onboarding-chat)
...
- **Security**
    ...
    - ✅ [Use MishikaLLM keys/authentication on Pass Through Endpoints](pass_through#✨-enterprise---use-mishikallm-keysauthentication-on-pass-through-endpoints)
    - ✅ [Set Max Request Size / File Size on Requests](#set-max-request--response-size-on-mishikallm-proxy)
...
Start the mishikallm proxy with this config
...
```json
{
 ...
 "action": "created",
 "table_name": "MishikaLLM_TeamTable",
...
This means `["user"]` and `["metadata]["generation_name"]` are required in all LLM Requests to MishikaLLM
...
general_settings:
  master_key: sk-1234
  public_routes: ["MishikaLLMRoutes.public_routes", "/spend/calculate"]     # routes that can be accessed without any auth
  admin_only_routes: ["/key/generate"]  # Optional - routes that can only be accessed by Proxy Admin
  allowed_routes: ["/chat/completions", "/spend/calculate", "MishikaLLMRoutes.public_routes"] # Optional - routes that can be accessed by anyone after Authentication
```
...
- Virtual Keys & a database should be set up, see [virtual keys](https://docs.21t.cc/docs/proxy/virtual_keys)
...
**Step 2** Run mishikallm proxy with `--detailed_debug` to see the server logs
...
Expect to see the following warning on your mishikallm server logs

```shell
MishikaLLM Proxy:WARNING: secret_detection.py:88 - Detected and redacted secrets in message: ['Secret Keyword']
```


You can also see the raw request sent from mishikallm to the API Provider
```json
...
Requires a MishikaLLM Enterprise key to use. Get a free 2-week license [here](https://forms.gle/sTDVprBs18M4V8Le8)

:::

Set MishikaLLM Key in your environment
...
**Step 2.** Add `MISHIKALLM_SECRET_AWS_KMS_` to encrypted keys in env 

```env
MISHIKALLM_SECRET_AWS_KMS_DATABASE_URL="AQICAH.."
```

MishikaLLM will find this and use the decrypted `DATABASE_URL="postgres://.."` value in runtime.
...
## Set Max Request / Response Size on MishikaLLM Proxy
