...
👉 [Setup MishikaLLM with a Database](https://docs.21t.cc/docs/proxy/virtual_keys#setup)
...
Expect to see `x-mishikallm-response-cost` in the response headers with calculated cost
...
The following spend gets tracked in Table `MishikaLLM_SpendLogs`

```json
{
  "api_key": "fe6b0cab4ff5a5a8df823196cc8a450*****",                            # Hash of API Key used
  "user": "default_user",                                                       # Internal User (MishikaLLM_UserTable) that owns `api_key=sk-1234`. 
  "team_id": "e8d1460f-846c-45d7-9b43-55f3cc52ac32",                            # Team (MishikaLLM_TeamTable) that owns `api_key=sk-1234`
  ...
  "model_group": "llama3",                                                      # "model" passed to MishikaLLM
  "api_base": "https://api.groq.com/openai/v1/",                                # "api_base" of model used by MishikaLLM
...
Navigate to the Usage Tab on the MishikaLLM UI (found on https://your-proxy-endpoint/ui) and verify you see spend tracked under `Usage`
...
Schedule a [meeting with us to get your Enterprise License](https://calendly.com/d/4mp-gd3-k5k/mishikallm-1-1-onboarding-chat)
...
- Reset the Spend for all API Keys, Teams. The `spend` for ALL Teams and Keys in `MishikaLLM_TeamTable` and `MishikaLLM_VerificationToken` will be set to `spend=0`

- MishikaLLM will maintain all the logs in `MishikaLLMSpendLogs` for Auditing Purposes

##### Request 
Only the `MISHIKALLM_MASTER_KEY` you set can access this route
```shell
curl -X POST \
...
See our [Swagger API](https://mishikallm-api.up.railway.app/#/Budget%20%26%20Spend%20Tracking/get_user_daily_activity_user_daily_activity_get) for more details on the `/user/daily/activity` endpoint
...
Customer [this is `user` passed to `/chat/completions` request](#how-to-track-spend-with-mishikallm)
- [MishikaLLM API key](virtual_keys.md)
...
Internal User (Key Owner): This is the value of `user_id` passed when calling [`/key/generate`](https://mishikallm-api.up.railway.app/#/key%20management/generate_key_fn_key_generate_post)
