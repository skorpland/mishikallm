...
mishikallm_settings:
  ...
  # Networking settings
  request_timeout: 10 # (int) llm requesttimeout in seconds. Raise Timeout error if call takes longer than 10s. Sets mishikallm.request_timeout 
  force_ipv4: boolean # If true, mishikallm will force ipv4 for all LLM requests. Some users have seen httpx ConnectionError when using ipv6 + Anthropic API
  
  set_verbose: boolean # sets mishikallm.set_verbose=True to view verbose debug logs. DO NOT LEAVE THIS ON IN PRODUCTION
  ...
  cache_params:        # set cache params for redis
    ...
    namespace: "mishikallm.caching.caching" # namespace for redis cache
...
general_settings:
  ...
  enable_jwt_auth: boolean  # allow proxy admin to auth in via jwt tokens with 'mishikallm_proxy_admin' in claims
...
| langfuse_default_tags | array of strings | Default tags for Langfuse Logging. Use this if you want to control which MishikaLLM-specific fields are logged as tags by the MishikaLLM proxy. By default MishikaLLM Proxy logs no MishikaLLM-specific fields as tags. [Further docs](./logging#mishikallm-specific-tags-on-langfuse---cache_hit-cache_key) |
| set_verbose | boolean | If true, sets mishikallm.set_verbose=True to view verbose debug logs. DO NOT LEAVE THIS ON IN PRODUCTION |
| json_logs | boolean | If true, logs will be in json format. If you need to store the logs as JSON, just set the `mishikallm.json_logs = True`. We currently just log the raw POST request from mishikallm as a JSON [Further docs](./debugging) |
...
| disable_end_user_cost_tracking | boolean | If true, turns off end user cost tracking on prometheus metrics + mishikallm spend logs table on proxy. |
...
| enable_jwt_auth | boolean | allow proxy admin to auth in via jwt tokens with 'mishikallm_proxy_admin' in claims. [Doc on JWT Tokens](token_auth) |
...
| allow_requests_on_db_unavailable | boolean | If true, allows requests to succeed even if DB is unreachable. **Only use this if running MishikaLLM in your VPC** This will allow requests to work even when MishikaLLM cannot connect to the DB to verify a Virtual Key [Doc on graceful db unavailability](prod#5-if-running-mishikallm-on-vpc-gracefully-handle-db-unavailability) |
...
| mishikallm_key_header_name | str | If set, allows passing MishikaLLM keys as a custom header. [Doc on custom headers](./virtual_keys.md#custom-headers) |
...
| mishikallm_jwtauth | Dict[str, Any] | Settings for JWT authentication. [Docs](./token_auth.md) |
| mishikallm_license | str | The license key for the proxy. [Docs](../enterprise.md#how-does-deployment-with-enterprise-license-work) |
...
Most values can also be set via `mishikallm_settings`. If you see overlapping values, settings on `router_settings` will override those on `mishikallm_settings`.
:::
...
| redis_host | string | The host address for the Redis server. **Only set this if you have multiple instances of MishikaLLM Proxy and want current tpm/rpm tracking to be shared across them** |
| redis_password | string | The password for the Redis server. **Only set this if you have multiple instances of MishikaLLM Proxy and want current tpm/rpm tracking to be shared across them** |
| redis_port | string | The port number for the Redis server. **Only set this if you have multiple instances of MishikaLLM Proxy and want current tpm/rpm tracking to be shared across them**|
...
| default_mishikallm_params | Optional[dict] | The default mishikallm parameters to add to all requests (e.g. `temperature`, `max_tokens`). |
...
| DD_SERVICE | Service identifier for Datadog logs. Defaults to "mishikallm-server"
...
| GCS_PUBSUB_TOPIC_ID | PubSub Topic ID to send MishikaLLM SpendLogs to.
| GCS_PUBSUB_PROJECT_ID | PubSub Project ID to send MishikaLLM SpendLogs to.
...
| HOSTNAME | Hostname for the server, this will be [emitted to `datadog` logs](https://docs.21t.cc/docs/proxy/logging#datadog)
...
| MISHIKALLM_DONT_SHOW_FEEDBACK_BOX | Flag to hide feedback box in MishikaLLM UI
| MISHIKALLM_DROP_PARAMS | Parameters to drop in MishikaLLM requests
| MISHIKALLM_MODIFY_PARAMS | Parameters to modify in MishikaLLM requests
| MISHIKALLM_EMAIL | Email associated with MishikaLLM account
| MISHIKALLM_GLOBAL_MAX_PARALLEL_REQUEST_RETRIES | Maximum retries for parallel requests in MishikaLLM
| MISHIKALLM_GLOBAL_MAX_PARALLEL_REQUEST_RETRY_TIMEOUT | Timeout for retries of parallel requests in MishikaLLM
| MISHIKALLM_MIGRATION_DIR | Custom migrations directory for prisma migrations, used for baselining db in read-only file systems.
| MISHIKALLM_HOSTED_UI | URL of the hosted UI for MishikaLLM
| MISHIKALLM_LICENSE | License key for MishikaLLM usage
| MISHIKALLM_LOCAL_MODEL_COST_MAP | Local configuration for model cost mapping in MishikaLLM
| MISHIKALLM_LOG | Enable detailed logging for MishikaLLM
| MISHIKALLM_MODE | Operating mode for MishikaLLM (e.g., production, development)
| MISHIKALLM_SALT_KEY | Salt key for encryption in MishikaLLM
| MISHIKALLM_SECRET_AWS_KMS_MISHIKALLM_LICENSE | AWS KMS encrypted license for MishikaLLM
| MISHIKALLM_TOKEN | Access token for MishikaLLM integration
| MISHIKALLM_PRINT_STANDARD_LOGGING_PAYLOAD | If true, prints the standard logging payload to the console - useful for debugging
...
| MICROSOFT_SERVICE_PRINCIPAL_ID | Service Principal ID for Microsoft Enterprise Application. (This is an advanced feature if you want mishikallm to auto-assign members to Mishikallm Teams based on their Microsoft Entra ID Groups)
...
| POD_NAME | Pod name for the server, this will be [emitted to `datadog` logs](https://docs.21t.cc/docs/proxy/logging#datadog) as `POD_NAME` 
...
| MISHIKALLM_MASTER_KEY | Master key for proxy authentication
