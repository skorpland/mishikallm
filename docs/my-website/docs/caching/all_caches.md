...
[**See Code**](https://github.com/skorpland/mishikallm/blob/main/mishikallm/caching/caching.py)

:::info

- For Proxy Server? Doc here: [Caching Proxy Server](https://docs.21t.cc/docs/proxy/caching)
...
mishikallm.cache = Cache(type="redis", host=<host>, port=<port>, password=<password>)
...
# pass s3-bucket name
mishikallm.cache = Cache(type="s3", s3_bucket_name="cache-bucket-mishikallm", s3_region_name="us-west-2")
...
mishikallm.cache = Cache(
    ...
    ttl=120,
    redis_semantic_cache_embedding_model="text-embedding-ada-002", # this model is passed to mishikallm.embedding(), any mishikallm.embedding() model is supported here
)
response1 = completion(
    model="gpt-3.5-turbo",
    messages=[
...
mishikallm.cache = Cache(
    ...
    qdrant_semantic_cache_embedding_model="text-embedding-ada-002", # this model is passed to mishikallm.embedding(), any mishikallm.embedding() model is supported here
)
...
from mishikallm import completion
from mishikallm.caching.caching import Cache
mishikallm.cache = Cache(type="disk")
...
## Switch Cache On / Off Per MishikaLLM Call 

MishikaLLM supports 4 cache-controls:
...
[Let us know if you need more](https://github.com/skorpland/mishikallm/issues/1218)
<Tabs>
...
Use the context manager for easily enabling, disabling & updating the mishikallm cache 
...
Set your function as mishikallm.cache.get_cache_key
```python
from mishikallm.caching.caching import Cache
...
mishikallm.cache = cache # set mishikallm.cache to your cache 
