...
export interface Organization {
  ...
  updated_at: string;
  updated_by: string;
  mishikallm_budget_table: any;  // Simplified to any since we don't need the detailed structure
  teams: any[] | null;
  users: any[] | null;
  members: any[] | null;
}
...
let globalMishikallmHeaderName: string  = "Authorization";

// Function to set the global header name
export function setGlobalMishikallmHeaderName(headerName: string = "Authorization") {
  console.log(`setGlobalMishikallmHeaderName: ${headerName}`);
  globalMishikallmHeaderName = headerName;
}
...
export const modelCostMap = async (
  ...
  try {
    const url = proxyBaseUrl ? `${proxyBaseUrl}/get/mishikallm_model_cost_map` : `/get/mishikallm_model_cost_map`;
    const response = await fetch(
      url, {
        method: "GET",
        headers: {
          [globalMishikallmHeaderName]: `Bearer ${accessToken}`,
          "Content-Type": "application/json",
        },
      }
    );
    const jsonData = await response.json();
    console.log(`received mishikallm model cost data: ${jsonData}`);
    return jsonData;
  } catch (error) {
...
export const teamDailyActivityCall = async (accessToken: String, startTime: Date, endTime: Date, page: number = 1, teamIds: string[] | null = null) => {
  ...
  try {
    ...
    queryParams.append('exclude_team_ids', 'mishikallm-dashboard');
    const queryString = queryParams.toString();
    if (queryString) {
      url += `?${queryString}`;
    }
...
export const modelInfoV1Call = async (accessToken: String, modelId: String) => {
  ...
  try {
    ...
    url += `?mishikallm_model_id=${modelId}`;
...
export const modelAvailableCall = async (
   ...
   */
  console.log("in /models calls, globalMishikallmHeaderName", globalMishikallmHeaderName)
  try {
...
export const testConnectionRequest = async (
  accessToken: string,
  mishikallm_params: Record<string, any>,
  mode: string,
) => {
  try {
    console.log("Sending model connection test request:", JSON.stringify(mishikallm_params));
