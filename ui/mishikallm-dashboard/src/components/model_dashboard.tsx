...
import MishikaLLMModelNameField from "./add_model/mishikallm_model_name";
...
>([]);
  ...
  for (let i = 0; i < modelData.data.length; i++) {
    ...
    let mishikallm_model_name = curr_model?.mishikallm_params?.model;
    let custom_llm_provider = curr_model?.mishikallm_params?.custom_llm_provider;
    ...
    let cleanedMishikallmParams = {};

    const getProviderFromModel = (model: string) => {
      /**
       * Use model map
       * - check if model in model map
       * - return it's mishikallm_provider, if so
       */
      console.log(`GET PROVIDER CALLED! - ${modelMap}`);
      if (modelMap !== null && modelMap !== undefined) {
        if (typeof modelMap == "object" && model in modelMap) {
          return modelMap[model]["mishikallm_provider"];
        }
      }
      return "openai";
    };

    // Check if mishikallm_model_name is null or undefined
    if (mishikallm_model_name) {
      // Split mishikallm_model_name based on "/"
      let splitModel = mishikallm_model_name.split("/");
      ...
      if (!provider) {
        provider =
        splitModel.length === 1
          ? getProviderFromModel(mishikallm_model_name)
          : firstElement;
        
      }
      
    } else {
      // mishikallm_model_name is null or undefined, default provider to openai
      provider = "-";
    }
    ...
    if (curr_model?.mishikallm_params) {
      cleanedMishikallmParams = Object.fromEntries(
        Object.entries(curr_model?.mishikallm_params).filter(
    ...
    modelData.data[i].mishikallm_model_name = mishikallm_model_name;
    ...
    modelData.data[i].api_base = curr_model?.mishikallm_params?.api_base;
    modelData.data[i].cleanedMishikallmParams = cleanedMishikallmParams;
