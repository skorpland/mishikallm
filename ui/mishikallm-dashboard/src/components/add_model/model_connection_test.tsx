...
const ModelConnectionTest: React.FC<ModelConnectionTestProps> = ({ 
  ...
  const testModelConnection = async () => {
    ...
    try {
      ...
      const { mishikallmParamsObj, modelInfoObj, modelName: returnedModelName } = result[0];

      const response = await testConnectionRequest(accessToken, mishikallmParamsObj, modelInfoObj?.mode);
      if (response.status === "success") {
        ...
        setRawRequest(mishikallmParamsObj);
  ...
  const getCleanErrorMessage = (errorMsg: string) => {
    ...
    const cleanedError = mainError.replace(/^mishikallm\.(.*?)Error: /, '');
  ...
  return (
    <div style={{ padding: '24px', borderRadius: '8px', backgroundColor: '#fff' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Button 
          type="link" 
          href="https://docs.21t.cc/docs/providers" 
          target="_blank"
