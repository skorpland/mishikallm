...
const ConditionalPublicModelName: React.FC = () => {
      ...
      const updatedMappings = currentMappings.map((mapping: any) => {
        if (mapping.public_name === 'custom' || mapping.mishikallm_model === 'custom') {
          return {
            ...
            mishikallm_model: customModelName
          };
        }
        return mapping;
      });
    ...
    if (selectedModels.length > 0 && !selectedModels.includes('all-wildcard')) {
      ...
      const shouldUpdateMappings = currentMappings.length !== selectedModels.length || 
        !selectedModels.every(model => 
          currentMappings.some((mapping: { public_name: string; mishikallm_model: string }) => 
  ...
  const columns = [
    ...
    {
      title: 'MishikaLLM Model',
      dataIndex: 'mishikallm_model',
      key: 'mishikallm_model',
    }
  ];
  ...
  return (
    <>
      <Form.Item
        ...
        name="model_mappings"
        tooltip="Map public model names to MishikaLLM model names for load balancing"
        labelCol={{ span: 10 }}
        wrapperCol={{ span: 16 }}
        labelAlign="left"
        required={true}
