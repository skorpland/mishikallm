...
interface MishikaLLMModelNameFieldProps {
  selectedProvider: Providers;
  providerModels: string[];
  getPlaceholder: (provider: Providers) => string;
}

const MishikaLLMModelNameField: React.FC<MishikaLLMModelNameFieldProps> = ({
  selectedProvider,
  providerModels, 
  getPlaceholder,
}) => {
  ...
  const handleCustomModelNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const updatedMappings = currentMappings.map((mapping: any) => {
      if (mapping.public_name === 'custom' || mapping.mishikallm_model === 'custom') {
        return {
          public_name: customName,
          mishikallm_model: customName
        };
      }
      return mapping;
    });
  ...
  return (
    <>
      <Form.Item
        label="MishikaLLM Model Name(s)"
        tooltip="Actual model name used for making mishikallm.completion() / mishikallm.embedding() call."
        className="mb-0"
      >
        <Form.Item
          name="model"
      ...
      <Row>
        ...
        <Col span={10}>
          <Text className="mb-3 mt-1">
            Actual model name used for making mishikallm.completion() call. We loadbalance models with the same public name
          </Text>
        </Col>
      </Row>
    </>
...
export default MishikaLLMModelNameField;