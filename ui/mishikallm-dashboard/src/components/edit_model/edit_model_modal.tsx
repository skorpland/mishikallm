...
export const handleEditModelSubmit = async (formValues: Record<string, any>, accessToken: string | null, setEditModalVisible: (visible: boolean) => void, setSelectedModel: (model: any) => void) => {
  ...
  let newMishikaLLMParams: Record<string, any> = {};
  ...
  for (const [key, value] of Object.entries(formValues)) {
    if (key !== "model_id") {
      ...
      newMishikaLLMParams[key] = value === "" ? null : value;
    } else {
  ...
  let payload: {
    mishikallm_params: Record<string, any> | undefined;
    model_info: { id: any } | undefined;
  } = {
    mishikallm_params: Object.keys(newMishikaLLMParams).length > 0 ? newMishikaLLMParams : undefined,
...
const EditModelModal: React.FC<EditModelModalProps> = ({
  ...
  let mishikallm_params_to_edit: Record<string, any> = {};
  let model_name = "";
  let model_id = "";
  if (model) {
    mishikallm_params_to_edit = {
      ...model.mishikallm_params,
      input_cost_per_token: model.mishikallm_params?.input_cost_per_token ? (model.mishikallm_params.input_cost_per_token * 1_000_000) : undefined,
      output_cost_per_token: model.mishikallm_params?.output_cost_per_token ? (model.mishikallm_params.output_cost_per_token * 1_000_000) : undefined,
    };
    ...
    if (model_info) {
      ...
      mishikallm_params_to_edit.model_id = model_id;
    }
  }
  ...
  return (
    <Modal
      title={"Edit '" + model_name + "' MishikaLLM Params"}
      visible={visible}
      width={800}
      footer={null}
      onOk={handleOk}
    ...
    >
      <Form
        form={form}
        onFinish={onSubmit}
        initialValues={mishikallm_params_to_edit}
        labelCol={{ span: 8 }}
        wrapperCol={{ span: 16 }}
        labelAlign="left"
      >
