...
from mishikallm.integrations.additional_logging_utils import AdditionalLoggingUtils
from mishikallm.integrations.custom_logger import CustomLogger


class LoggingCallbackManager:
    """
    A centralized class that allows easy add / remove callbacks for mishikallm.
    ...
    def add_mishikallm_input_callback(self, callback: Union[CustomLogger, str]):
        """
        Add a input callback to mishikallm.input_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=mishikallm.input_callback
        )

    def add_mishikallm_service_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a service callback to mishikallm.service_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=mishikallm.service_callback
        )

    def add_mishikallm_callback(self, callback: Union[CustomLogger, str, Callable]):
        """
        Add a callback to mishikallm.callbacks

        Ensures no duplicates are added.
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=mishikallm.callbacks  # type: ignore
        )

    def add_mishikallm_success_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a success callback to `mishikallm.success_callback`
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=mishikallm.success_callback
        )

    def add_mishikallm_failure_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a failure callback to `mishikallm.failure_callback`
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=mishikallm.failure_callback
        )

    def add_mishikallm_async_success_callback(
        self, callback: Union[CustomLogger, Callable, str]
    ):
        """
        Add a success callback to mishikallm._async_success_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=mishikallm._async_success_callback
        )

    def add_mishikallm_async_failure_callback(
        self, callback: Union[CustomLogger, Callable, str]
    ):
        """
        Add a failure callback to mishikallm._async_failure_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=mishikallm._async_failure_callback
        )
    ...
    def _get_all_callbacks(self) -> List[Union[CustomLogger, Callable, str]]:
        """
        Get all callbacks from mishikallm.callbacks, mishikallm.success_callback, mishikallm.failure_callback, mishikallm._async_success_callback, mishikallm._async_failure_callback
        """
        return (
            mishikallm.callbacks
            + mishikallm.success_callback
