    ...
    try:
        ...
        if not _response_headers:
            _response_headers = getattr(
                original_exception, "mishikallm_response_headers", None
            )
    except Exception:
        return None
...
def extract_and_raise_mishikallm_exception(
    ...
    """
    Covers scenario where mishikallm sdk calling proxy.
    ...
    if match:
        ...
        exception_name = exception_name.strip().replace("mishikallm.", "")
        raised_exception_obj = getattr(mishikallm, exception_name, None)
...
def exception_type(  # type: ignore  # noqa: PLR0915
    ...
    if any(
        ...
        for exc_type in mishikallm.MISHIKALLM_EXCEPTION_TYPES
    ):
    ...
    mishikallm_response_headers = _get_response_headers(
    ...
    try:
        ...
        if model:
            ...
            try:
                ...
                messages = mishikallm.get_first_chars_messages(kwargs=completion_kwargs)
                ...
                if (
                    messages
                    and len(messages) > 0
                    and mishikallm.redact_messages_in_exceptions is False
                ):
                    extra_information += f"\nMessages: `{messages}`"
                ...
                # on mishikallm proxy add key name + team to exceptions
            ...
            ):  # handle special case where calling mishikallm proxy + exception str contains error message
            ...
            if (
                ...
                if hasattr(original_exception, "status_code"):
                    ...
                    if original_exception.status_code == 401:
                        ...
                        exception_mapping_worked = True
                        raise mishikallm.InternalServerError(
                            message=f"AnthropicException - {error_str}. Handle with `mishikallm.InternalServerError`.",
                            llm_provider="anthropic",
                            model=model,
                        )
                    elif original_exception.status_code == 503:
                        exception_mapping_worked = True
                        raise mishikallm.ServiceUnavailableError(
                            message=f"AnthropicException - {error_str}. Handle with `mishikallm.ServiceUnavailableError`.",
                            llm_provider="anthropic",
                            model=model,
                        )
            ...
            elif custom_llm_provider in mishikallm._openai_like_providers:
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Enable 'mishikallm.modify_params=True' (for PROXY do: `mishikallm_settings::modify_params: True`) to insert a dummy assistant message and fix this error.",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Pass in default user message via `completion(..,user_continue_message=)` or enable `mishikallm.modify_params=True`.\nFor Proxy: do via `mishikallm_settings::modify_params: True` or user_continue_message under `mishikallm_params`",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                ...
                if "Unable to locate credentials" in error_str:
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"mishikallm.BadRequestError: SagemakerException - {error_str}",
                        model=model,
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"mishikallm.BadRequestError: VertexAIException - {error_str}",
                        model=model,
                ...
                if "400 Request payload size exceeds" in error_str:
                    ...
                    exception_mapping_worked = True
                    raise mishikallm.InternalServerError(
                        message=f"mishikallm.InternalServerError: VertexAIException - {error_str}",
                        model=model,
                        llm_provider="vertex_ai",
                        response=httpx.Response(
                            status_code=500,
                    ...
                    exception_mapping_worked = True
                    raise RateLimitError(
                        message=f"mishikallm.RateLimitError: VertexAIException - {error_str}",
                        model=model,
                        llm_provider="vertex_ai",
                        mishikallm_debug_info=extra_information,
                        response=httpx.Response(
                ...
                if "Internal server error" in error_str:
                    ...
                    exception_mapping_worked = True
                    raise ContentPolicyViolationError(
                        message=f"mishikallm.ContentPolicyViolationError: AzureException - {message}",
                        llm_provider="azure",
                        model=model,
                        mishikallm_debug_info=extra_information,
                        response=getattr(original_exception, "response", None),
        ...
        if exception_mapping_worked:
            setattr(e, "mishikallm_response_headers", mishikallm_response_headers)
            raise e
        else:
            for error_type in mishikallm.MISHIKALLM_EXCEPTION_TYPES:
            ...
            setattr(raised_exc, "mishikallm_response_headers", mishikallm_response_headers)
            raise raised_exc
    ...
    try:
        ...
        if logger_fn and callable(logger_fn):
            try:
                ...
                verbose_logger.debug(
                    f"MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
                )
    except Exception:
        verbose_logger.debug(
            f"MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
...
def _add_key_name_and_team_to_alert(request_info: str, metadata: dict) -> str:
    """
    Internal helper function for mishikallm proxy
