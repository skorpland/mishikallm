...
from mishikallm.mishikallm_core_utils.default_encoding import encoding as default_encoding
...
from mishikallm.types.utils import Message, SelectTokenizerResponse
...
def get_modified_max_tokens(
    ...
    try:
        ...
        ## MODEL INFO
        _model_info = mishikallm.get_model_info(model=model)

        max_output_tokens = mishikallm.get_max_tokens(
            model=base_model
        ...
        input_tokens = mishikallm.token_counter(model=base_model, messages=messages)
        ...
        verbose_logger.debug(
            f"mishikallm.mishikallm_core_utils.token_counter.py::get_modified_max_tokens() - user_max_tokens: {user_max_tokens}"
        )

        return user_max_tokens
    except Exception as e:
        verbose_logger.error(
            "mishikallm.mishikallm_core_utils.token_counter.py::get_modified_max_tokens() - Error while checking max token limit: {}\nmodel={}, base_model={}".format(
...
class _MessageCountParams:
    ...
    def __init__(
        ...
        from mishikallm.utils import print_verbose
        ...
        elif actual_model in mishikallm.open_ai_chat_completion_models:
            self.tokens_per_message = 3
            self.tokens_per_name = 1
        elif actual_model in mishikallm.azure_llms:
            self.tokens_per_message = 3
            self.tokens_per_name = 1
        else:
            print_verbose(
...
def token_counter(
    ...
    """
    from mishikallm.utils import convert_list_message_to_dict
...
def _get_count_function(
    ...
    from mishikallm.utils import _select_tokenizer, print_verbose
...
def _fix_model_name(model: str) -> str:
    ...
    if model in mishikallm.azure_llms:
        # azure llms use gpt-35-turbo instead of gpt-3.5-turbo 🙃
        return model.replace("-35", "-3.5")
    elif model in mishikallm.open_ai_chat_completion_models:
