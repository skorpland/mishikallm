...
from mishikallm._logging import _is_debugging_on, verbose_logger
from mishikallm.batches.batch_utils import _handle_completed_batch
from mishikallm.caching.caching import DualCache, InMemoryCache
from mishikallm.caching.caching_handler import LLMCachingHandler
from mishikallm.constants import (
    DEFAULT_MOCK_RESPONSE_COMPLETION_TOKEN_COUNT,
    DEFAULT_MOCK_RESPONSE_PROMPT_TOKEN_COUNT,
)
from mishikallm.cost_calculator import (
    RealtimeAPITokenUsageProcessor,
    _select_model_name_for_cost_calc,
)
from mishikallm.integrations.agentops import AgentOps
from mishikallm.integrations.anthropic_cache_control_hook import AnthropicCacheControlHook
from mishikallm.integrations.arize.arize import ArizeLogger
...
from mishikallm.integrations.mlflow import MlflowLogger
from mishikallm.integrations.pagerduty.pagerduty import PagerDutyAlerting
from mishikallm.integrations.vector_stores.bedrock_vector_store import BedrockVectorStore
from mishikallm.mishikallm_core_utils.get_mishikallm_params import get_mishikallm_params
from mishikallm.mishikallm_core_utils.llm_cost_calc.tool_call_cost_tracking import (
    StandardBuiltInToolCostTracking,
)
from mishikallm.mishikallm_core_utils.model_param_helper import ModelParamHelper
from mishikallm.mishikallm_core_utils.redact_messages import (
    redact_message_input_output_from_custom_logger,
    redact_message_input_output_from_logging,
)
from mishikallm.responses.utils import ResponseAPILoggingUtils
...
from mishikallm.types.router import CustomPricingMishikaLLMParams
from mishikallm.types.utils import (
    ...
    ImageResponse,
    MishikaLLMBatch,
    MishikaLLMLoggingBaseClass,
    MishikaLLMRealtimeStreamLoggingObject,
    ModelResponse,
    ModelResponseStream,
    RawRequestTypedDict,
...
from mishikallm.utils import _get_base_model_from_metadata, executor, print_verbose
...
try:
    ...
    verbose_logger.debug(
        f"[Non-Blocking] Unable to import GenericAPILogger - MishikaLLM Enterprise Feature - {str(e)}"
    )
...
class ServiceTraceIDCache:
    ...
    def get_cache(self, mishikallm_call_id: str, service_name: str) -> Optional[str]:
        key_name = "{}:{}".format(service_name, mishikallm_call_id)
        response = self.cache.get_cache(key=key_name)
        return response

    def set_cache(self, mishikallm_call_id: str, service_name: str, trace_id: str) -> None:
        key_name = "{}:{}".format(service_name, mishikallm_call_id)
...
class Logging(MishikaLLMLoggingBaseClass):
    ...
    def __init__(
        ...
        call_type,
        start_time,
        mishikallm_call_id: str,
        function_id: str,
        mishikallm_trace_id: Optional[str] = None,
        ...
        self.mishikallm_call_id = mishikallm_call_id
        self.mishikallm_trace_id: str = mishikallm_trace_id or str(uuid.uuid4())
        ...
        # INITIAL MISHIKALLM_PARAMS
        mishikallm_params = {}
        if kwargs is not None:
            mishikallm_params = get_mishikallm_params(**kwargs)
            mishikallm_params = scrub_sensitive_keys_in_metadata(mishikallm_params)

        self.mishikallm_params = mishikallm_params

        self.model_call_details: Dict[str, Any] = {
            "mishikallm_trace_id": mishikallm_trace_id,
    ...
    def process_dynamic_callbacks(self):
        ...
        If a callback is in mishikallm._known_custom_logger_compatible_callbacks, it needs to be intialized and added to the respective dynamic_* callback list.
        """
    ...
    def _process_dynamic_callback_list(
        ...
        - If a callback is in mishikallm._known_custom_logger_compatible_callbacks,
    ...
    def update_environment_variables(
        ...
        **additional_params,
        ...
        if model is not None:
            self.model = model
        self.user = user
        self.mishikallm_params = {
            **self.mishikallm_params,
            **scrub_sensitive_keys_in_metadata(mishikallm_params),
        }
        self.logger_fn = mishikallm_params.get("logger_fn", None)
        ...
        self.model_call_details.update(
            {
                ...
                "mishikallm_params": self.mishikallm_params,
                "start_time": self.start_time,
                "stream": self.stream,
                "user": user,
                "call_type": str(self.call_type),
                "mishikallm_call_id": self.mishikallm_call_id,
        ...
        custom_pricing_keys = CustomPricingMishikaLLMParams.model_fields.keys()
        for key in custom_pricing_keys:
            if mishikallm_params.get(key) is not None:
    ...
    def _should_run_prompt_management_hooks_without_prompt_id(
        ...
        #############################################################################
        if mishikallm.vector_store_registry is not None:
            if mishikallm.vector_store_registry.get_vector_store_to_run(
    ...
    def get_custom_logger_for_prompt_management(
        ...
        for callback_name in mishikallm._known_custom_logger_compatible_callbacks:
        ...
        #########################################################
        if mishikallm.vector_store_registry is not None:
            if vector_store_to_run := mishikallm.vector_store_registry.get_vector_store_to_run(
                non_default_params=non_default_params
            ):
                vector_store_custom_logger = (
                    mishikallm.ProviderConfigManager.get_provider_vector_store_config(
                        provider=cast(
                            mishikallm.LlmProviders,
    ...
    def _pre_call(self, input, api_key, model=None, additional_args={}):
        ...
        self.model_call_details["mishikallm_params"]["api_base"] = (
            self._get_masked_api_base(additional_args.get("api_base", ""))
        )

    def pre_call(self, input, api_key, model=None, additional_args={}):  # noqa: PLR0915
        # Log the exact input to the LLM API
        mishikallm.error_logs["PRE_CALL"] = locals()
        try:
            self._pre_call(
                input=input,
                api_key=api_key,
            ...
            if (
                ...
                _mishikallm_params = self.model_call_details.get("mishikallm_params", {})
                _metadata = _mishikallm_params.get("metadata", {}) or {}
                try:
                    # [Non-blocking Extra Debug Information in metadata]
                    if turn_off_message_logging is True:
                        _metadata["raw_request"] = (
                            "redacted by mishikallm. \
            ...
            if self.logger_fn and callable(self.logger_fn):
                try:
                    ...
                    verbose_logger.exception(
                        "MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while logging {}".format(
                            str(e)
                        )
                    )
            ...
            callbacks = mishikallm.input_callback + (self.dynamic_input_callbacks or [])
                ...
                try:
                    if callback == "supabase" and supabaseClient is not None:
                        ...
                        supabaseClient.input_log_event(
                            ...
                            mishikallm_call_id=self.mishikallm_params["mishikallm_call_id"],
                    ...
                    verbose_logger.exception(
                        "mishikallm.Logging.pre_call(): Exception occured - {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
                        f"MishikaLLM.Logging: is sentry capture exception initialized {capture_exception}"
                    )
    ...
    def _print_llm_call_debugging_log(
        ...
        Prints the RAW curl command sent from MishikaLLM
        """
        if _is_debugging_on():
            if json_logs:
    ...
    def _get_masked_headers(
        ...
        Masks the headers of the request sent from MishikaLLM
        """
    ...
    def post_call(
        ...
        mishikallm.error_logs["POST_CALL"] = locals()
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.exception(
                        "MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while post-call logging with integrations {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
    ...
    def _response_cost_calculator(
        ...
        mishikallm_model_name: Optional[str] = None,
        ...
        custom_pricing = use_custom_pricing_for_model(
            mishikallm_params=(
                self.mishikallm_params if hasattr(self, "mishikallm_params") else None
            )
        )
        ...
        try:
            response_cost_calculator_kwargs = {
                ...
                "model": mishikallm_model_name or self.model,
                "cache_hit": cache_hit,
        ...
        try:
            response_cost = mishikallm.response_cost_calculator(
                **response_cost_calculator_kwargs
            )
    ...
    def should_run_callback(
        self, callback: mishikallm.CALLBACK_TYPES, mishikallm_params: dict, event_hook: str
    ) -> bool:
        if mishikallm.global_disable_no_log_param:
            return True

        if mishikallm_params.get("no-log", False) is True:
    ...
    def _success_handler_helper_fn(
        ...
        try:
            ...
            if self.call_type == CallTypes.arealtime.value and isinstance(result, list):
                ...
                #     mishikallm_model_name=self.model,
                # )
            ...
            if (
                ...
                if (
                    ...
                    or isinstance(logging_result, MishikaLLMBatch)
                    or isinstance(logging_result, ResponsesAPIResponse)
                    or isinstance(logging_result, OpenAIFileObject)
                    or isinstance(logging_result, MishikaLLMRealtimeStreamLoggingObject)
                    ...
                    if hidden_params:
                        ...
                        if self.model_call_details.get("mishikallm_params") is not None:
                            self.model_call_details["mishikallm_params"].setdefault(
                                "metadata", {}
                            )
                            if (
                                self.model_call_details["mishikallm_params"]["metadata"]
                                is None
                            ):
                                self.model_call_details["mishikallm_params"][
                                    "metadata"
                                ] = {}

                            self.model_call_details["mishikallm_params"]["metadata"][  # type: ignore
                                "hidden_params"
                            ] = getattr(
            ...
            if (
                ...
                mishikallm._current_cost += mishikallm.completion_cost(
                    model=self.model,
                    prompt="",
            ...
            raise Exception(f"[Non-Blocking] MishikaLLM.Success_Call Error: {str(e)}")

    def success_handler(  # noqa: PLR0915
        self, result=None, start_time=None, end_time=None, cache_hit=None, **kwargs
    ):
        verbose_logger.debug(
            f"Logging Details MishikaLLM-Success Call: Cache_hit={cache_hit}"
        )
        ...
        try:
            ...
            if complete_streaming_response is not None:
                verbose_logger.debug(
                    "Logging Details MishikaLLM-Success Call streaming complete"
                )
            ...
            callbacks = self.get_combined_callback_list(
                ...
                global_callbacks=mishikallm.success_callback,
            )
            ...
            for callback in callbacks:
                try:
                    mishikallm_params = self.model_call_details.get("mishikallm_params", {})
                    ...
                    if callback == "supabase" and supabaseClient is not None:
                        ...
                        supabaseClient.log_event(
                            ...
                            start_time=start_time,
                            end_time=end_time,
                            mishikallm_call_id=mishikallm_params.get(
                                "mishikallm_call_id", str(uuid.uuid4())
                            ),
                            print_verbose=print_verbose,
                        )
                    ...
                    if callback == "lunary" and lunaryLogger is not None:
                        ...
                        lunaryLogger.log_event(
                            ...
                            start_time=start_time,
                            end_time=end_time,
                            run_id=self.mishikallm_call_id,
                            print_verbose=print_verbose,
                        )
                    ...
                    if (
                        callback == "openmeter"
                        and self.model_call_details.get("mishikallm_params", {}).get(
                            "acompletion", False
                        )
                        is not True
                        and self.model_call_details.get("mishikallm_params", {}).get(
                    ...
                    print_verbose(
                        f"MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while success logging with integrations {traceback.format_exc()}"
                    )
                    print_verbose(
                        f"MishikaLLM.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while success logging {}".format(
                    str(e)
                ),
            )
    ...
    async def async_success_handler(  # noqa: PLR0915
        ...
        """
        print_verbose(
            "Logging Details MishikaLLM-Async Success Call, cache_hit={}".format(cache_hit)
        )

        ## CALCULATE COST FOR BATCH JOBS
        if self.call_type == CallTypes.aretrieve_batch.value and isinstance(
            result, MishikaLLMBatch
        ):
        ...
        if complete_streaming_response is not None:
            ...
            except mishikallm.NotFoundError:
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=mishikallm._async_success_callback,
        )
        ...
        for callback in callbacks:
            if isinstance(callback, CustomGuardrail):
                ...
                result = redact_message_input_output_from_custom_logger(
                    result=result, mishikallm_logging_obj=self, custom_logger=callback
                )
                self.model_call_details, result = await callback.async_logging_hook(
                    kwargs=self.model_call_details,
                    result=result,
        ...
        for callback in callbacks:
            ...
            try:
                ...
                verbose_logger.error(
                    f"MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while success logging {traceback.format_exc()}"
                )
                pass
    ...
    def _failure_handler_helper_fn(
        ...
        if hasattr(exception, "headers") and isinstance(exception.headers, dict):
            self.model_call_details.setdefault("mishikallm_params", {})
            metadata = (
                self.model_call_details["mishikallm_params"].get("metadata", {}) or {}
            )
    ...
    async def special_failure_handlers(self, exception: Exception):
        ...
        """
        from mishikallm.types.router import RouterErrors

        mishikallm_params: dict = self.model_call_details.get("mishikallm_params") or {}
        metadata = mishikallm_params.get("metadata") or {}
        ...
        for callback in mishikallm._async_failure_callback:
            if isinstance(callback, CustomLogger):  # custom logger class
    ...
    def failure_handler(  # noqa: PLR0915
        ...
        verbose_logger.debug(
            f"Logging Details MishikaLLM-Failure Call: {mishikallm.failure_callback}"
        )
        try:
            ...
            callbacks = self.get_combined_callback_list(
                ...
                global_callbacks=mishikallm.failure_callback,
            )
                ...
                try:
                    ...
                    if callback == "sentry":
                        ...
                        supabaseClient.log_event(
                            ...
                            start_time=start_time,
                            end_time=end_time,
                            mishikallm_call_id=self.model_call_details["mishikallm_call_id"],
                            print_verbose=print_verbose,
                        )
                    if (
                        callable(callback) and customLogger is not None
                    ...
                    print_verbose(
                        f"MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while failure logging with integrations {str(e)}"
                    )
                    print_verbose(
                        f"MishikaLLM.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while failure logging {}".format(
                    str(e)
                )
            )
    ...
    async def async_failure_handler(
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=mishikallm._async_failure_callback,
        )
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception(
                    "MishikaLLM.LoggingError: [Non-Blocking] Exception occurred while failure \
                        logging {}\nCallback={}".format(
                        str(e), callback
                    )
                )
    ...
    def _get_trace_id(self, service_name: Literal["langfuse"]) -> Optional[str]:
        ...
        if service_name == "langfuse":
            trace_id = in_memory_trace_id_cache.get_cache(
                mishikallm_call_id=self.mishikallm_call_id, service_name=service_name
            )
    ...
    def _should_run_sync_callbacks_for_async_calls(self) -> bool:
        ...
        _filtered_success_callbacks = self._remove_internal_mishikallm_callbacks(
    ...
    def _remove_internal_mishikallm_callbacks(self, callbacks: List) -> List:
        """
        Creates a filtered list of callbacks, excluding internal MishikaLLM callbacks.
        ...
        """
        filtered = [
            cb for cb in callbacks if not self._is_internal_mishikallm_proxy_callback(cb)
        ]
    ...
    def _is_internal_mishikallm_proxy_callback(self, cb) -> bool:
    ...
    def _remove_internal_custom_logger_callbacks(self, callbacks: List) -> List:
        ...
        for _c in callbacks:
            if isinstance(_c, CustomLogger):
                ...
                isinstance(_c, str)
                and _c in mishikallm._known_custom_logger_compatible_callbacks
            ):
                continue
    ...
    def _handle_anthropic_messages_response_logging(self, result: Any) -> ModelResponse:
        ...
        result = mishikallm.AnthropicConfig().transform_response(
...
def _init_custom_logger_compatible_class(  # noqa: PLR0915
    ...
    llm_router: Optional[
        Any
    ],  # expect mishikallm.Router, but typing errors due to circular import
    ...
    try:
        ...
        if logging_integration == "agentops":  # Add AgentOps initialization
            ...
            from mishikallm.integrations.opentelemetry import (
                OpenTelemetry,
                OpenTelemetryConfig,
            )
            ...
            from mishikallm.proxy.hooks.dynamic_rate_limiter import (
            ...
            if llm_router is not None and isinstance(llm_router, mishikallm.Router):
...
def _get_custom_logger_settings_from_proxy_server(callback_name: str) -> Dict:
    ...
    """
    from mishikallm.proxy.proxy_server import callback_settings
...
def use_custom_pricing_for_model(mishikallm_params: Optional[dict]) -> bool:
    """
    Check if the model uses custom pricing

    Returns True if any of `SPECIAL_MODEL_INFO_PARAMS` are present in `mishikallm_params` or `model_info`
    """
    if mishikallm_params is None:
        return False

    metadata: dict = mishikallm_params.get("metadata", {}) or {}
    model_info: dict = metadata.get("model_info", {}) or {}

    custom_pricing_keys = CustomPricingMishikaLLMParams.model_fields.keys()
    for key in custom_pricing_keys:
        if mishikallm_params.get(key, None) is not None:
            return True
...
class StandardLoggingPayloadSetup:
    ...
    def get_standard_logging_metadata(
        ...
        if mishikallm_params is not None:
            prompt_id = cast(Optional[str], mishikallm_params.get("prompt_id", None))
            prompt_variables = cast(
                Optional[dict], mishikallm_params.get("prompt_variables", None)
            )
        ...
        if isinstance(metadata, dict):
            ...
            _potential_requester_metadata = metadata.get(
                "metadata", None
            )  # check if user passed metadata in the sdk request - e.g. metadata for langsmith logging - https://docs.21t.cc/docs/observability/langsmith_integration#set-langsmith-fields
            if (
    ...
    def get_model_cost_information(
        ...
        if model_cost_name is None:
            ...
            try:
                _model_cost_information = mishikallm.get_model_info(
    ...
    def get_hidden_params(
        ...
        clean_hidden_params = StandardLoggingHiddenParams(
            ...
            mishikallm_overhead_time_ms=None,
            batch_models=None,
    ...
    def _get_standard_logging_payload_trace_id(
        ...
        """
        Returns the `mishikallm_trace_id` for this request

        This helps link sessions when multiple requests are made in a single session
        """
        dynamic_mishikallm_session_id = mishikallm_params.get("mishikallm_session_id")
        dynamic_mishikallm_trace_id = mishikallm_params.get("mishikallm_trace_id")

        # Note: we recommend using `mishikallm_session_id` for session tracking
        # `mishikallm_trace_id` is an internal mishikallm param
        if dynamic_mishikallm_session_id:
            return str(dynamic_mishikallm_session_id)
        elif dynamic_mishikallm_trace_id:
            return str(dynamic_mishikallm_trace_id)
        else:
            return logging_obj.mishikallm_trace_id
...
def get_standard_logging_object_payload(
    ...
    try:
        ...
        metadata: dict = (
            mishikallm_params.get("mishikallm_metadata")
            or mishikallm_params.get("metadata", None)
            or {}
        )
        ...
        id = response_obj.get("id", kwargs.get("mishikallm_call_id"))
        ...
        custom_pricing = use_custom_pricing_for_model(mishikallm_params=mishikallm_params)
        ...
        payload: StandardLoggingPayload = StandardLoggingPayload(
            ...
            api_base=StandardLoggingPayloadSetup.strip_trailing_slash(
                mishikallm_params.get("api_base", "")
            )
            or "",
...
def emit_standard_logging_payload(payload: StandardLoggingPayload):
    if os.getenv("MISHIKALLM_PRINT_STANDARD_LOGGING_PAYLOAD"):
...
def scrub_sensitive_keys_in_metadata(mishikallm_params: Optional[dict]):
    if mishikallm_params is None:
        mishikallm_params = {}
        ...
        for k, v in metadata["user_api_key_metadata"].items():
            if k == "logging":  # prevent logging user logging keys
                cleaned_user_api_key_metadata[k] = (
                    "scrubbed_by_mishikallm_for_sensitive_keys"
                )
            else:
                cleaned_user_api_key_metadata[k] = v

        metadata["user_api_key_metadata"] = cleaned_user_api_key_metadata
        mishikallm_params["metadata"] = metadata
