...
import mishikallm
from mishikallm.constants import REPLICATE_MODEL_NAME_WITH_ID_LENGTH
from mishikallm.secret_managers.main import get_secret, get_secret_str
...
def _is_non_openai_azure_model(model: str) -> bool:
    try:
        ...
        if (
            model_name in mishikallm.cohere_chat_models
            or f"mistral/{model_name}" in mishikallm.mistral_chat_models
        ):
            return True
    except Exception:
        return False
...
def handle_cohere_chat_model_custom_llm_provider(
    ...
    if custom_llm_provider:
        if custom_llm_provider == "cohere" and model in mishikallm.cohere_chat_models:
    ...
    if "/" in model:
        ...
        if (
            ...
            and _model in mishikallm.cohere_chat_models
        ):
...
def handle_anthropic_text_model_custom_llm_provider(
    ...
    if custom_llm_provider:
        if (
            ...
            and mishikallm.AnthropicTextConfig._is_anthropic_text_model(model)
        ):
    ...
    if "/" in model:
        ...
        if (
            ...
            and mishikallm.AnthropicTextConfig._is_anthropic_text_model(_model)
        ):
...
def get_llm_provider(  # noqa: PLR0915
    ...
    For router -> Can also give the whole mishikallm param dict -> this function will extract the relevant details
    ...
    """
    try:
        ## IF MISHIKALLM PARAMS GIVEN ##
        if mishikallm_params is not None:
            assert (
                custom_llm_provider is None and api_base is None and api_key is None
            ), "Either pass in mishikallm_params or the custom_llm_provider/api_base/api_key. Otherwise, these values will be overriden."
        ...
        elif model.split("/", 1)[0] in mishikallm.provider_list:
        ...
        if api_base:
            for endpoint in mishikallm.openai_compatible_endpoints:
        ...
        if (
            ...
            custom_llm_provider = "openai"
        elif model in mishikallm.open_ai_text_completion_models:
            custom_llm_provider = "text-completion-openai"
        ## anthropic
        elif model in mishikallm.anthropic_models:
            if mishikallm.AnthropicTextConfig._is_anthropic_text_model(model):
                custom_llm_provider = "anthropic_text"
            else:
                custom_llm_provider = "anthropic"
        ## cohere
        elif model in mishikallm.cohere_models or model in mishikallm.cohere_embedding_models:
            custom_llm_provider = "cohere"
        ## cohere chat models
        elif model in mishikallm.cohere_chat_models:
            custom_llm_provider = "cohere_chat"
        ## replicate
        elif model in mishikallm.replicate_models or (
        ...
        ## maritalk
        elif model in mishikallm.maritalk_models:
        ...
        ## vertex - text + chat + language (gemini) models
            ...
            or model in mishikallm.vertex_embedding_models
            or model in mishikallm.vertex_vision_models
            or model in mishikallm.vertex_ai_image_models
        ):
            custom_llm_provider = "vertex_ai"
        ## ai21
        elif model in 21t.cc21_chat_models or model in 21t.cc21_models:
            custom_llm_provider = "ai21_chat"
            api_base = (
                api_base
        ...
        ## petals
        elif model in mishikallm.petals_models:
            custom_llm_provider = "petals"
        ## bedrock
        elif (
            model in mishikallm.bedrock_models
            or model in mishikallm.bedrock_embedding_models
            or model in mishikallm.bedrock_converse_models
        ):
            custom_llm_provider = "bedrock"
        elif model in mishikallm.watsonx_models:
            custom_llm_provider = "watsonx"
        # openai embeddings
        elif model in mishikallm.open_ai_embedding_models:
            custom_llm_provider = "openai"
        elif model in mishikallm.empower_models:
            custom_llm_provider = "empower"
        elif model == "*":
            custom_llm_provider = "openai"
        if not custom_llm_provider:
            if mishikallm.suppress_debug_info is False:
                print()  # noqa
                print(  # noqa
                    "\033[1;31mProvider List: https://docs.21t.cc/docs/providers\033[0m"  # noqa
                )  # noqa
                print()  # noqa
            error_str = f"LLM Provider NOT provided. Pass in the LLM provider you are trying to call. You passed model={model}\n Pass model as E.g. For 'Huggingface' inference endpoints pass in `completion(model='huggingface/starcoder',..)` Learn more: https://docs.21t.cc/docs/providers"
            # maps to openai.NotFoundError, this is raised when openai does not recognize the llm
            raise mishikallm.exceptions.BadRequestError(  # type: ignore
                message=error_str,
                model=model,
        ...
        if isinstance(e, mishikallm.exceptions.BadRequestError):
            raise e
        else:
            error_str = (
...
def _get_openai_compatible_provider_info(  # noqa: PLR0915
    ...
    if custom_llm_provider == "perplexity":
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.PerplexityChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.DeepInfraConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.GroqChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        custom_llm_provider == "ai21" and model in 21t.cc21_chat_models
    ):
        api_base = (
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.HostedVLLMChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.LlamafileChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.LMStudioChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.FireworksAIConfig()._get_openai_compatible_provider_info(
        ...
        (
            ...
            dynamic_api_key,
            custom_llm_provider,
        ) = mishikallm.AzureAIStudioConfig()._get_openai_compatible_provider_info(
    ...
    elif custom_llm_provider == "mishikallm_proxy":
        api_base = api_base or get_secret_str("MISHIKALLM_PROXY_API_BASE")
        dynamic_api_key = api_key or get_secret_str("MISHIKALLM_PROXY_API_KEY")

    elif custom_llm_provider == "mistral":
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.MistralConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            ...
            api_base,
            dynamic_api_key,
        ) = mishikallm.JinaAIEmbeddingConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = mishikallm.XAIChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
