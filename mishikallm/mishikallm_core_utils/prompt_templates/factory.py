...
import mishikallm.types.llms
from mishikallm import verbose_logger
from mishikallm.llms.custom_httpx.http_handler import HTTPHandler, get_async_httpx_client
from mishikallm.types.llms.anthropic import *
from mishikallm.types.llms.bedrock import MessageBlock as BedrockMessageBlock
from mishikallm.types.llms.custom_http import httpxSpecialProvider
from mishikallm.types.llms.ollama import OllamaVisionModelObject
...
from mishikallm.types.llms.vertex_ai import FunctionCall as VertexFunctionCall
from mishikallm.types.llms.vertex_ai import FunctionResponse as VertexFunctionResponse
from mishikallm.types.llms.vertex_ai import PartType as VertexPartType
from mishikallm.types.utils import GenericImageParsingChunk
...
}  # similar to autogen. Only used if `mishikallm.modify_params=True`.
...
def hf_chat_template(  # noqa: PLR0915
    ...
    if chat_template is None:
        ...
        if model in mishikallm.known_tokenizer_config:
            tokenizer_config = mishikallm.known_tokenizer_config[model]
        else:
            tokenizer_config = _get_tokenizer_config(model)
            mishikallm.known_tokenizer_config.update({model: tokenizer_config})
...
def anthropic_messages_pt_xml(messages: list):
    ...
    1. Anthropic supports roles like "user" and "assistant", (here mishikallm translates system-> assistant)
    2. The first message always needs to be of role "user"
    3. Each message must alternate between "user" and "assistant" (this is not addressed as now by mishikallm)
    ...
    if not new_messages or new_messages[0]["role"] != "user":
        if mishikallm.modify_params:
            new_messages.insert(
                0, {"role": "user", "content": [{"type": "text", "text": "."}]}
            )
        else:
            raise Exception(
                "Invalid first message. Should always start with 'role'='user' for Anthropic. System prompt is sent separately for Anthropic. set 'mishikallm.modify_params = True' or 'mishikallm_settings:modify_params = True' on proxy, to insert a placeholder user message - '.' as the first message, "
            )
...
from mishikallm.types.llms.cohere import (
    CallObject,
    ChatHistory,
    ChatHistoryChatBot,
    ChatHistorySystem,
    ...
    try:
        ...
        for message in messages:
            if isinstance(message["content"], str):
                ...
                # see https://docs.21t.cc/docs/providers/openai#openai-vision-models
...
from mishikallm.types.llms.bedrock import (
    BedrockConverseReasoningContentBlock,
    BedrockConverseReasoningTextBlock,
)
from mishikallm.types.llms.bedrock import ContentBlock as BedrockContentBlock
from mishikallm.types.llms.bedrock import DocumentBlock as BedrockDocumentBlock
from mishikallm.types.llms.bedrock import ImageBlock as BedrockImageBlock
from mishikallm.types.llms.bedrock import SourceBlock as BedrockSourceBlock
from mishikallm.types.llms.bedrock import ToolBlock as BedrockToolBlock
from mishikallm.types.llms.bedrock import (
    ToolInputSchemaBlock as BedrockToolInputSchemaBlock,
)
from mishikallm.types.llms.bedrock import ToolResultBlock as BedrockToolResultBlock
from mishikallm.types.llms.bedrock import (
    ToolResultContentBlock as BedrockToolResultContentBlock,
)
from mishikallm.types.llms.bedrock import ToolSpecBlock as BedrockToolSpecBlock
from mishikallm.types.llms.bedrock import ToolUseBlock as BedrockToolUseBlock
...
class BedrockImageProcessor:
    ...
    def _validate_format(mime_type: str, image_format: str) -> str:
        ...
        supported_image_formats = (
            mishikallm.AmazonConverseConfig().get_supported_image_types()
        )
        supported_doc_formats = (
            mishikallm.AmazonConverseConfig().get_supported_document_types()
        )
...
def get_user_message_block_or_continue_message(
    ...
    if content_block is None or (
        user_continue_message is None and mishikallm.modify_params is False
    ):
...
def get_assistant_message_block_or_continue_message(
    ...
    # Handle Base case
    if content_block is None or (
        assistant_continue_message is None and mishikallm.modify_params is False
    ):
        return skip_empty_text_blocks(message=message)
                    ...
                    for element in message_block["content"]:
                        if isinstance(element, dict):
                            if element["type"] == "text":
                                ...
                                _parts.append(_part)
                            _cache_point_block = (
                                mishikallm.AmazonConverseConfig()._get_cache_point_block(
                                    message_block=cast(
            ...
            if user_content:
                if len(contents) > 0 and contents[-1]["role"] == "user":
                    if (
                        ...
                        or mishikallm.modify_params is True
                    ):
...
def make_valid_bedrock_tool_name(input_tool_name: str) -> str:
    ...
    if input_tool_name != valid_string:
        ...
        mishikallm.bedrock_tool_name_mappings.set_cache(
...
def _bedrock_tools_pt(tools: List) -> List[BedrockToolBlock]:
    ...
    """
    from mishikallm.mishikallm_core_utils.prompt_templates.common_utils import unpack_defs
...
def response_schema_prompt(model: str, response_schema: dict) -> str:
    ...
    if f"{model}/response_schema_prompt" in mishikallm.custom_prompt_dict:
        custom_prompt_details = mishikallm.custom_prompt_dict[
            f"{model}/response_schema_prompt"
        ]  # allow user to define custom response schema prompt by model
    elif "response_schema_prompt" in mishikallm.custom_prompt_dict:
        custom_prompt_details = mishikallm.custom_prompt_dict["response_schema_prompt"]
...
def prompt_factory(
    ...
    if custom_llm_provider == "ollama":
        ...
        if (
            ...
            or mishikallm.supports_vision(model=model)
            or mishikallm.supports_vision(model=custom_llm_provider + "/" + model)
        ):
            return _gemini_vision_convert_messages(messages=messages)
        else:
            return gemini_text_image_pt(messages=messages)
    elif custom_llm_provider == "mistral":
        return mishikallm.MistralConfig()._transform_messages(
