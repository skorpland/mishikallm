...
if TYPE_CHECKING:
    from mishikallm.mishikallm_core_utils.mishikallm_logging import (
        Logging as _MishikaLLMLoggingObject,
    )

    MishikaLLMLoggingObject = _MishikaLLMLoggingObject
else:
    MishikaLLMLoggingObject = Any


def redact_message_input_output_from_custom_logger(
    mishikallm_logging_obj: MishikaLLMLoggingObject, result, custom_logger: CustomLogger
):
    if (
        hasattr(custom_logger, "message_logging")
        and custom_logger.message_logging is not True
    ):
        return perform_redaction(mishikallm_logging_obj.model_call_details, result)
    return result
...
def perform_redaction(model_call_details: dict, result):
    ...
    model_call_details["messages"] = [
        {"role": "user", "content": "redacted-by-mishikallm"}
    ]
    ...
    if (
        ...
        for choice in _streaming_response.choices:
            if isinstance(choice, mishikallm.Choices):
                choice.message.content = "redacted-by-mishikallm"
            elif isinstance(choice, mishikallm.utils.StreamingChoices):
                choice.delta.content = "redacted-by-mishikallm"

    # Redact result
    if result is not None and isinstance(result, mishikallm.ModelResponse):
        ...
        return {"text": "redacted-by-mishikallm"}
...
def should_redact_message_logging(model_call_details: dict) -> bool:
    ...
    """
    _request_headers = (
        model_call_details.get("mishikallm_params", {}).get("metadata", {}) or {}
    )

    request_headers = _request_headers.get("headers", {})

    possible_request_headers = [
        "mishikallm-enable-message-redaction",  # old header. maintain backwards compatibility
        "x-mishikallm-enable-message-redaction",  # new header
    ]
    ...
    if (
        mishikallm.turn_off_message_logging is not True
    ...
    if request_headers and bool(
        request_headers.get("mishikallm-disable-message-redaction", False)
    ):
        return False
...
def redact_message_input_output_from_logging(
    ...
    only redacts when mishikallm.turn_off_message_logging == True
    """
...
def redact_user_api_key_info(metadata: dict) -> dict:
    ...
    SDK
    ```python
    mishikallm.redact_user_api_key_info = True
    ```
    ...
    ```
    """
    if mishikallm.redact_user_api_key_info is not True:
        return metadata
