...
from ...mishikallm_core_utils.get_llm_provider_logic import get_llm_provider
from ...types.router import MishikaLLM_Params


def get_api_base(
    model: str, optional_params: Union[dict, MishikaLLM_Params]
) -> Optional[str]:
    """
    ...
    Example:
    ```
    from mishikallm import get_api_base
    ...
    try:
        if isinstance(optional_params, MishikaLLM_Params):
            _optional_params = optional_params
        elif "model" in optional_params:
            _optional_params = MishikaLLM_Params(**optional_params)
        else:  # prevent needing to copy and pop the dict
            _optional_params = MishikaLLM_Params(
    ...
    if (
        ...
        from mishikallm.llms.vertex_ai.vertex_ai_partner_models.main import (
