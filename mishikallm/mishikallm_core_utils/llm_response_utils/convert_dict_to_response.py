...
from mishikallm.types.llms.databricks import DatabricksTool
...
from mishikallm.types.utils import Logprobs as TextCompletionLogprobs
from mishikallm.types.utils import (
    Message,
    ModelResponse,
    RerankResponse,
...
def convert_tool_call_to_json_mode(
    ...
    if _should_convert_tool_call_to_json_mode(
        ...
        if json_mode_content_str is not None:
            message = mishikallm.Message(content=json_mode_content_str)
            finish_reason = "stop"
...
class MishikaLLMResponseObjectHandler:
    @staticmethod
    ...
    def convert_chat_to_text_completion(
        ...
        """
        transformed_logprobs = MishikaLLMResponseObjectHandler._convert_provider_response_logprobs_to_text_completion_logprobs(
            response=response,
...
def convert_to_model_response_object(  # noqa: PLR0915
    ...
    try:
        if response_type == "completion" and (
            ...
            if "usage" in response_object and response_object["usage"] is not None:
                usage_object = mishikallm.Usage(**response_object["usage"])
            ...
            special_keys = list(mishikallm.ModelResponse.model_fields.keys())
            ...
            return MishikaLLMResponseObjectHandler.convert_to_image_response(
