...
from mishikallm.mishikallm_core_utils.core_helpers import process_response_headers
from mishikallm.mishikallm_core_utils.llm_response_utils.get_api_base import get_api_base
from mishikallm.mishikallm_core_utils.logging_utils import MishikaLLMLoggingObject
...
class ResponseMetadata:
    """
    Handles setting and managing `_hidden_params`, `response_time_ms`, and `mishikallm_overhead_time_ms` for MishikaLLM responses
    """
    ...
    def set_hidden_params(
        self, logging_obj: MishikaLLMLoggingObject, model: Optional[str], kwargs: dict
    ) -> None:
        ...
        new_params = {
            "mishikallm_call_id": getattr(logging_obj, "mishikallm_call_id", None),
            "api_base": get_api_base(model=model or "", optional_params=kwargs),
            "model_id": model_id,
            "response_cost": logging_obj._response_cost_calculator(
                result=self.result, mishikallm_model_name=model, router_model_id=model_id
            ),
    ...
    def set_timing_metrics(
        ...
        if llm_api_duration_ms is not None:
            ...
            self._update_hidden_params(
                {
                    "mishikallm_overhead_time_ms": overhead_ms,
