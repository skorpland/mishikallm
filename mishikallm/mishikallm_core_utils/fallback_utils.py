...
async def async_completion_with_fallbacks(**kwargs):
    ...
    base_kwargs = {**kwargs, **nested_kwargs, "mishikallm_call_id": mishikallm_call_id}

    # fields to remove
    base_kwargs.pop("model", None)  # Remove model as it will be set per fallback
    mishikallm_logging_obj = base_kwargs.pop("mishikallm_logging_obj", None)
    ...
    raise Exception(
        f"{most_recent_exception_str}. All fallback attempts failed. Enable verbose logging with `mishikallm.set_verbose=True` for details."
    )
