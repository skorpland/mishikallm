...
from mishikallm.mishikallm_core_utils.redact_messages import MishikaLLMLoggingObject
from mishikallm.mishikallm_core_utils.thread_pool_executor import executor
from mishikallm.types.llms.openai import ChatCompletionChunk
...
class CustomStreamWrapper:
    def __init__(
        ...
        self.logging_obj: MishikaLLMLoggingObject = logging_obj
        self.completion_stream = completion_stream
        self.sent_first_chunk = False
        self.sent_last_chunk = False

        mishikallm_params: GenericMishikaLLMParams = GenericMishikaLLMParams(
            **self.logging_obj.model_call_details.get("mishikallm_params", {})
        )
        self.merge_reasoning_content_in_choices: bool = (
            mishikallm_params.merge_reasoning_content_in_choices or False
        )
        ...
        _model_info: Dict = mishikallm_params.model_info or {}
        ...
        self._hidden_params = {
            ...
            "api_base": _api_base,
        }  # returned as x-mishikallm-model-id response header in proxy
    ...
    def check_is_function_call(self, logging_obj) -> bool:
        if hasattr(logging_obj, "optional_params") and isinstance(
            ...
            if (
                "mishikallm_param_is_function_call" in logging_obj.optional_params
                and logging_obj.optional_params["mishikallm_param_is_function_call"]
                is True
            ):
                return True
    ...
    def safety_checker(self) -> None:
        ...
        """
        if len(self.chunks) >= mishikallm.REPEATED_STREAMING_CHUNK_LIMIT:
            # Get the last n chunks
            last_chunks = self.chunks[-mishikallm.REPEATED_STREAMING_CHUNK_LIMIT :]
    ...
    def handle_baseten_chunk(self, chunk):
        try:
            ...
            verbose_logger.exception(
                "mishikallm.CustomStreamWrapper.handle_baseten_chunk(): Exception occured - {}".format(
                    str(e)
                )
            )
            return ""
    ...
    def chunk_creator(self, chunk: Any):  # type: ignore  # noqa: PLR0915
        ...
        try:
            ...
            if (
                ...
                and self.custom_llm_provider in mishikallm._custom_providers
            ):
                ...
                if anthropic_response_obj["usage"] is not None:
                    model_response.usage = mishikallm.Usage(
                        ...
                        try:
                            ...
                            if "Part has no text." in str(e):
                                ...
                                try:
                                    ...
                                    raise e
                                _delta_obj = mishikallm.utils.Delta(
                                    content=None,
                                    tool_calls=[
                                        {
                ...
                response_obj = cast(
                    Dict[str, Any],
                    mishikallm.CodestralTextCompletionConfig()._chunk_parser(chunk),
                )
                completion_obj["content"] = response_obj["text"]
                print_verbose(f"completion obj content: {completion_obj['content']}")
                if response_obj["is_finished"]:
                ...
                if response_obj["usage"] is not None:
                    if isinstance(response_obj["usage"], dict):
                        ...
                        setattr(
                            model_response,
                            "usage",
                            mishikallm.Usage(**response_obj["usage"].model_dump()),
                        )
                ...
                if original_chunk.choices and len(original_chunk.choices) > 0:
                    ...
                    if delta is not None and (
                        ...
                        try:
                            ...
                            verbose_logger.exception(
                                "mishikallm.CustomStreamWrapper.chunk_creator(): Exception occured - {}".format(
                                    str(e)
                                )
                            )
                            model_response.choices[0].delta = Delta()
    ...
    def set_logging_event_loop(self, loop):
        """
        import mishikallm, asyncio

        loop = asyncio.get_event_loop() # 👈 gets the current event loop

        response = mishikallm.completion(.., stream=True)
    ...
    def run_success_logging_and_cache_storage(self, processed_chunk, cache_hit: bool):
        ...
        """
        if mishikallm.disable_streaming_logging is True:
            """
            [NOT RECOMMENDED]
            Set this via `mishikallm.disable_streaming_logging = True`.
    ...
    def fetch_sync_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = self.make_call(client=mishikallm.module_level_client)
    ...
    async def fetch_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = await self.make_call(
                client=mishikallm.module_level_aclient
            )
    ...
    async def __anext__(self):  # noqa: PLR0915
        ...
        try:
            ...
            traceback_exception = traceback.format_exc()
            ## ADD DEBUG INFORMATION - E.G. MISHIKALLM REQUEST TIMEOUT
            traceback_exception += "\nMishikaLLM Default Request Timeout - {}".format(
                mishikallm.request_timeout
            )
