...
if TYPE_CHECKING:
    from mishikallm import ModelResponse as _ModelResponse
    from mishikallm.mishikallm_core_utils.mishikallm_logging import (
        Logging as MishikaLLMLoggingObject,
    )

    MishikaLLMModelResponse = _ModelResponse
else:
    MishikaLLMModelResponse = Any
...
def convert_mishikallm_response_object_to_str(
    response_obj: Union[Any, MishikaLLMModelResponse]
) -> Optional[str]:
    """
    Get the string of the response object from MishikaLLM

    """
    if isinstance(response_obj, mishikallm.ModelResponse):
        response_str = ""
...
def _set_duration_in_model_call_details(
    logging_obj: Any,  # we're not guaranteed this will be `MishikaLLMLoggingObject`
    start_time: datetime,
    end_time: datetime,
):
