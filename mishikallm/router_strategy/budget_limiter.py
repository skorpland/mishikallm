...
from mishikallm.caching.redis_cache import RedisPipelineIncrementOperation
from mishikallm.integrations.custom_logger import CustomLogger, Span
from mishikallm.mishikallm_core_utils.duration_parser import duration_in_seconds
from mishikallm.router_strategy.tag_based_routing import _get_tags_from_request_kwargs
from mishikallm.router_utils.cooldown_callbacks import (
    _get_prometheus_logger_from_callbacks,
)
from mishikallm.types.llms.openai import AllMessageValues
from mishikallm.types.router import DeploymentTypedDict, MishikaLLM_Params, RouterErrors
from mishikallm.types.utils import BudgetConfig
from mishikallm.types.utils import BudgetConfig as GenericBudgetInfo
from mishikallm.types.utils import GenericBudgetConfigType, StandardLoggingPayload
...
class RouterBudgetLimiting(CustomLogger):
    def __init__(
        ...
        # Add self to mishikallm callbacks if it's a list
        if isinstance(mishikallm.callbacks, list):
            mishikallm.logging_callback_manager.add_mishikallm_callback(self)  # type: ignore
    ...
    def _filter_out_deployments_above_budget(
        ...
        for deployment in healthy_deployments:
            ...
            if self.deployment_budget_config and is_within_budget:
                ...
                _mishikallm_params = deployment.get("mishikallm_params") or {}
                ...
                if model_id in deployment_configs:
                    ...
                    if config.max_budget and current_spend >= config.max_budget:
                        debug_msg = f"Exceeded budget for deployment model_name: {_model_name}, mishikallm_params.model: {_mishikallm_model_name}, model_id: {model_id}: {current_spend} >= {config.budget_duration}"
    ...
    async def _increment_spend_in_current_window(
        ...
        - Queues the increment operation to Redis Pipeline (using batched pipeline to optimize performance. Using Redis for multi instance environment of MishikaLLM)
        """
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        custom_llm_provider: str = kwargs.get("mishikallm_params", {}).get(
    ...
    def _get_llm_provider_for_deployment(self, deployment: Dict) -> Optional[str]:
        try:
            _mishikallm_params: MishikaLLM_Params = MishikaLLM_Params(
                **deployment.get("mishikallm_params", {"model": ""})
            )
            _, custom_llm_provider, _, _ = mishikallm.get_llm_provider(
                model=_mishikallm_params.model,
    ...
    def should_init_router_budget_limiter(
        ...
        if mishikallm.tag_budget_config is not None:
            return True
        ...
        for _model in model_list:
            _mishikallm_params = _model.get("mishikallm_params", {})
            if (
                _mishikallm_params.get("max_budget")
                or _mishikallm_params.get("budget_duration") is not None
            ):
                return True
        return False
    ...
    def _init_deployment_budgets(
        ...
        for _model in model_list:
            ...
            _max_budget = _mishikallm_params.get("max_budget")
            _budget_duration = _mishikallm_params.get("budget_duration")
    ...
    def _init_tag_budgets(self):
        if mishikallm.tag_budget_config is None:
            return
        ...
        for _tag, _tag_budget_config in mishikallm.tag_budget_config.items():
