...
import mishikallm
from mishikallm import ModelResponse, token_counter, verbose_logger
...
class LowestCostLoggingHandler(CustomLogger):
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if kwargs["mishikallm_params"].get("metadata") is None:
                ...
                model_group = kwargs["mishikallm_params"]["metadata"].get(
                    "model_group", None
                )

                id = kwargs["mishikallm_params"].get("model_info", {}).get("id", None)
                ...
                if isinstance(response_obj, ModelResponse):
                    ...
                    if _usage is not None and isinstance(_usage, mishikallm.Usage):
            ...
            verbose_logger.exception(
                "mishikallm.router_strategy.lowest_cost.py::log_success_event(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            verbose_logger.exception(
                "mishikallm.proxy.hooks.prompt_injection_detection.py::async_pre_call_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_get_available_deployments(  # noqa: PLR0915
        ...
        for item, item_map in all_deployments.items():
            ...
            _deployment_tpm = (
                ...
                or _deployment.get("mishikallm_params", {}).get("tpm", None)
            ...
            _deployment_rpm = (
                ...
                or _deployment.get("mishikallm_params", {}).get("rpm", None)
                or _deployment.get("model_info", {}).get("rpm", None)
                or float("inf")
            )
            item_mishikallm_model_name = _deployment.get("mishikallm_params", {}).get("model")
            item_mishikallm_model_cost_map = mishikallm.model_cost.get(
                item_mishikallm_model_name, {}
            )

            # check if user provided input_cost_per_token and output_cost_per_token in mishikallm_params
            item_input_cost = None
            item_output_cost = None
            if _deployment.get("mishikallm_params", {}).get("input_cost_per_token", None):
                item_input_cost = _deployment.get("mishikallm_params", {}).get(
                    "input_cost_per_token"
                )

            if _deployment.get("mishikallm_params", {}).get("output_cost_per_token", None):
                item_output_cost = _deployment.get("mishikallm_params", {}).get(
                    "output_cost_per_token"
                )

            if item_input_cost is None:
                item_input_cost = item_mishikallm_model_cost_map.get(
                    "input_cost_per_token", 5.0
                )

            if item_output_cost is None:
                item_output_cost = item_mishikallm_model_cost_map.get(
                    "output_cost_per_token", 5.0
                )

            # if mishikallm["model"] is not in model_cost map -> use item_cost = $10
            ...
            _deployment_api_base = _deployment.get("mishikallm_params", {}).get(
                "api_base", ""
            )
