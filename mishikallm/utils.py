...
# What is this?
## Generic utils.py file. Problem-specific utils (e.g. 'cost calculation), should all be in `mishikallm_core_utils/`.
import sys
import textwrap
import threading
import time
...
import mishikallm
import mishikallm._service_logger  # for storing API inputs, outputs, and metadata
import mishikallm.mishikallm_core_utils
import mishikallm.mishikallm_core_utils.audio_utils.utils
import mishikallm.mishikallm_core_utils.json_validation_rule
import mishikallm.llms
import mishikallm.llms.gemini
from mishikallm.caching._internal_lru_cache import lru_cache_wrapper
from mishikallm.caching.caching import DualCache
from mishikallm.caching.caching_handler import CachingHandlerResponse, LLMCachingHandler
from mishikallm.constants import (
...
from mishikallm.integrations.vector_stores.base_vector_store import BaseVectorStore
from mishikallm.mishikallm_core_utils.core_helpers import (
    map_finish_reason,
    process_response_headers,
)
from mishikallm.mishikallm_core_utils.credential_accessor import CredentialAccessor
from mishikallm.mishikallm_core_utils.default_encoding import encoding
from mishikallm.mishikallm_core_utils.exception_mapping_utils import (
    _get_response_headers,
    exception_type,
    get_error_message,
)
from mishikallm.mishikallm_core_utils.get_mishikallm_params import (
    _get_base_model_from_mishikallm_call_metadata,
    get_mishikallm_params,
)
from mishikallm.mishikallm_core_utils.get_llm_provider_logic import (
    _is_non_openai_azure_model,
    get_llm_provider,
)
from mishikallm.mishikallm_core_utils.get_supported_openai_params import (
    get_supported_openai_params,
)
from mishikallm.mishikallm_core_utils.llm_request_utils import _ensure_extra_body_is_safe
from mishikallm.mishikallm_core_utils.llm_response_utils.convert_dict_to_response import (
    MishikaLLMResponseObjectHandler,
...
from mishikallm.mishikallm_core_utils.llm_response_utils.get_api_base import get_api_base
from mishikallm.mishikallm_core_utils.llm_response_utils.get_formatted_prompt import (
    get_formatted_prompt,
)
from mishikallm.mishikallm_core_utils.llm_response_utils.get_headers import (
    get_response_headers,
)
from mishikallm.mishikallm_core_utils.llm_response_utils.response_metadata import (
    ResponseMetadata,
)
from mishikallm.mishikallm_core_utils.redact_messages import (
    MishikaLLMLoggingObject,
    redact_message_input_output_from_logging,
)
from mishikallm.mishikallm_core_utils.rules import Rules
from mishikallm.mishikallm_core_utils.streaming_handler import CustomStreamWrapper
from mishikallm.mishikallm_core_utils.token_counter import get_modified_max_tokens
from mishikallm.llms.bedrock.common_utils import BedrockModelInfo
from mishikallm.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
from mishikallm.router_utils.get_retry_from_policy import (
    get_num_retries_from_retry_policy,
    reset_retry_policy,
)
from mishikallm.secret_managers.main import get_secret
from mishikallm.types.llms.anthropic import (
...
from mishikallm.types.utils import FileTypes  # type: ignore
from mishikallm.types.utils import (
    ...
    Usage,
    all_mishikallm_params,
)

try:
    # Python 3.9+
    with resources.files("mishikallm.mishikallm_core_utils.tokenizers").joinpath(
        "anthropic_tokenizer.json"
    ).open("r", encoding="utf-8") as f:
        json_data = json.load(f)
except (ImportError, AttributeError, TypeError):
    with resources.open_text(
        "mishikallm.mishikallm_core_utils.tokenizers", "anthropic_tokenizer.json"
    ) as f:
        json_data = json.load(f)
...
from mishikallm.mishikallm_core_utils.thread_pool_executor import executor
from mishikallm.mishikallm_core_utils.token_counter import token_counter as token_counter_new
from mishikallm.llms.base_llm.anthropic_messages.transformation import (
    BaseAnthropicMessagesConfig,
)
from mishikallm.llms.base_llm.audio_transcription.transformation import (
    BaseAudioTranscriptionConfig,
)
from mishikallm.llms.base_llm.base_utils import (
    BaseLLMModelInfo,
    type_to_response_format_param,
)
from mishikallm.llms.base_llm.chat.transformation import BaseConfig
from mishikallm.llms.base_llm.completion.transformation import BaseTextCompletionConfig
...
from mishikallm.llms.base_llm.image_variations.transformation import (
...
from .types.router import MishikaLLM_Params
...
# All mishikaLLM Model responses will be in this format, Follows the OpenAI Format
# https://docs.21t.cc/docs/completion/output
# {
#   'choices': [
#      {
#         'finish_reason': 'stop',
...
def print_verbose(
    ...
    try:
        ...
        if mishikallm.set_verbose is True and logger_only is False:
...
def custom_llm_setup():
    ...
    """
    for custom_llm in mishikallm.custom_provider_map:
        if custom_llm["provider"] not in mishikallm.provider_list:
            mishikallm.provider_list.append(custom_llm["provider"])

        if custom_llm["provider"] not in mishikallm._custom_providers:
            mishikallm._custom_providers.append(custom_llm["provider"])
...
def _add_custom_logger_callback_to_specific_event(
    ...
    """
    from mishikallm import _custom_logger_compatible_callbacks_literal
    from mishikallm.mishikallm_core_utils.mishikallm_logging import (
        _init_custom_logger_compatible_class,
    )

    if callback not in mishikallm._known_custom_logger_compatible_callbacks:
        verbose_logger.debug(
            f"Callback {callback} is not a valid custom logger compatible callback. Known list - {mishikallm._known_custom_logger_compatible_callbacks}"
        )
        return
    ...
    if callback_class:
        if (
            ...
            mishikallm.logging_callback_manager.add_mishikallm_success_callback(
                callback_class
            )
            mishikallm.logging_callback_manager.add_mishikallm_async_success_callback(
                callback_class
            )
            if callback in mishikallm.success_callback:
                mishikallm.success_callback.remove(
                    callback
                )  # remove the string from the callback list
            if callback in mishikallm._async_success_callback:
                mishikallm._async_success_callback.remove(
                    callback
                )  # remove the string from the callback list
        elif (
            ...
            mishikallm.logging_callback_manager.add_mishikallm_failure_callback(
                callback_class
            )
            mishikallm.logging_callback_manager.add_mishikallm_async_failure_callback(
                callback_class
            )
            if callback in mishikallm.failure_callback:
                mishikallm.failure_callback.remove(
                    callback
                )  # remove the string from the callback list
            if callback in mishikallm._async_failure_callback:
                mishikallm._async_failure_callback.remove(
                    callback
                )  # remove the string from the callback list
...
def _custom_logger_class_exists_in_success_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in mishikallm.success_callback or mishikallm._async_success_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in mishikallm.success_callback or mishikallm._async_success_callback

    Prevents double adding a custom logger callback to the mishikallm callbacks
    """
    return any(
        isinstance(cb, type(callback_class))
        for cb in mishikallm.success_callback + mishikallm._async_success_callback
    )
...
def _custom_logger_class_exists_in_failure_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in mishikallm.failure_callback or mishikallm._async_failure_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in mishikallm.failure_callback or mishikallm._async_failure_callback

    Prevents double adding a custom logger callback to the mishikallm callbacks
    """
    return any(
        isinstance(cb, type(callback_class))
        for cb in mishikallm.failure_callback + mishikallm._async_failure_callback
    )
...
def get_applied_guardrails(kwargs: Dict[str, Any]) -> List[str]:
    ...
    applied_guardrails = []
    for callback in mishikallm.callbacks:
...
def load_credentials_from_list(kwargs: dict):
    ...
    """
    credential_name = kwargs.get("mishikallm_credential_name")
    if credential_name and mishikallm.credential_list:
...
def get_dynamic_callbacks(
    ...
    returned_callbacks = mishikallm.callbacks.copy()
...
def function_setup(  # noqa: PLR0915
    ...
    ### NOTICES ###
    from mishikallm import Logging as MishikaLLMLogging
    from mishikallm.mishikallm_core_utils.mishikallm_logging import set_callbacks

    if mishikallm.set_verbose is True:
        verbose_logger.warning(
            "`mishikallm.set_verbose` is deprecated. Please set `os.environ['MISHIKALLM_LOG'] = 'DEBUG'` for debug logs."
        )
    try:
        ...
        if len(all_callbacks) > 0:
            for callback in all_callbacks:
                ...
                if isinstance(callback, str):
                    callback = mishikallm.mishikallm_core_utils.mishikallm_logging._init_custom_logger_compatible_class(  # type: ignore
                        callback, internal_usage_cache=None, llm_router=None  # type: ignore
                    )
                    if callback is None or any(
                        isinstance(cb, type(callback))
                        for cb in mishikallm._async_success_callback
                    ):  # don't double add a callback
                        continue
                if callback not in mishikallm.input_callback:
                    mishikallm.input_callback.append(callback)  # type: ignore
                if callback not in mishikallm.success_callback:
                    mishikallm.logging_callback_manager.add_mishikallm_success_callback(callback)  # type: ignore
                if callback not in mishikallm.failure_callback:
                    mishikallm.logging_callback_manager.add_mishikallm_failure_callback(callback)  # type: ignore
                if callback not in mishikallm._async_success_callback:
                    mishikallm.logging_callback_manager.add_mishikallm_async_success_callback(callback)  # type: ignore
                if callback not in mishikallm._async_failure_callback:
                    mishikallm.logging_callback_manager.add_mishikallm_async_failure_callback(callback)  # type: ignore
            print_verbose(
                f"Initialized mishikallm callbacks, Async Success Callbacks: {mishikallm._async_success_callback}"
            )

        if (
            len(mishikallm.input_callback) > 0
            or len(mishikallm.success_callback) > 0
            or len(mishikallm.failure_callback) > 0
        ) and len(
            callback_list  # type: ignore
        ) == 0:  # type: ignore
            callback_list = list(
                set(
                    mishikallm.input_callback  # type: ignore
        ...
        ## ASYNC CALLBACKS
        if len(mishikallm.input_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(mishikallm.input_callback):  # type: ignore
                if inspect.iscoroutinefunction(callback):
                    mishikallm._async_input_callback.append(callback)
                    removed_async_items.append(index)

            # Pop the async items from input_callback in reverse order to avoid index issues
            for index in reversed(removed_async_items):
                mishikallm.input_callback.pop(index)
        if len(mishikallm.success_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(mishikallm.success_callback):  # type: ignore
                if inspect.iscoroutinefunction(callback):
                    mishikallm.logging_callback_manager.add_mishikallm_async_success_callback(
                        callback
                    )
                    ...
                    callback in mishikallm._known_custom_logger_compatible_callbacks
            ...
            for index in reversed(removed_async_items):
                mishikallm.success_callback.pop(index)

        if len(mishikallm.failure_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(mishikallm.failure_callback):  # type: ignore
                if inspect.iscoroutinefunction(callback):
                    mishikallm.logging_callback_manager.add_mishikallm_async_failure_callback(
                        callback
                    )
            ...
            for index in reversed(removed_async_items):
                mishikallm.failure_callback.pop(index)
        ### DYNAMIC CALLBACKS ###
        ...
        if add_breadcrumb:
            ...
            if mishikallm.turn_off_message_logging:
                # make a copy of the _model_Call_details and log it
                details_to_log.pop("messages", None)
                details_to_log.pop("input", None)
                details_to_log.pop("prompt", None)
            add_breadcrumb(
                category="mishikallm.llm_call",
        ...
        if (
            ...
            file_checksum = (
                mishikallm.mishikallm_core_utils.audio_utils.utils.get_audio_file_name(
                    file_obj=_file_obj
                )
            )
        ...
        logging_obj = MishikaLLMLogging(
            ...
            messages=messages,
            stream=stream,
            mishikallm_call_id=kwargs["mishikallm_call_id"],
            mishikallm_trace_id=kwargs.get("mishikallm_trace_id"),
        ...
        mishikallm_params: Dict[str, Any] = {"api_base": ""}
        if "metadata" in kwargs:
            mishikallm_params["metadata"] = kwargs["metadata"]
        ...
        verbose_logger.exception(
            "mishikallm.utils.py::function_setup() - [Non-Blocking] Error in function_setup"
        )
        raise e


async def _client_async_logging_helper(
    logging_obj: MishikaLLMLoggingObject,
    result,
    start_time,
    end_time,
...
def _get_wrapper_num_retries(
    ...
    if num_retries is None:
        num_retries = mishikallm.num_retries
...
def client(original_function):  # noqa: PLR0915
                    ...
                    if is_coroutine is True:
                        ...
                        if (
                            ...
                            if model_response is not None:
                                ...
                                if mishikallm.enable_json_schema_validation is True:
                                    try:
                                        if (
                                            ...
                                            if json_response_format is not None:
                                                mishikallm.mishikallm_core_utils.json_validation_rule.validate_schema(
    ...
    def wrapper(*args, **kwargs):  # noqa: PLR0915
        ...
        if _is_async_request(kwargs):
            ...
            if mishikallm.num_retries_per_request is not None:
                # check if previous_models passed in as ['mishikallm_params']['metadata]['previous_models']
                previous_models = kwargs.get("metadata", {}).get(
                    "previous_models", None
                )
                if previous_models is not None:
                    if mishikallm.num_retries_per_request <= len(previous_models):
            ...
            if "stream" in kwargs and kwargs["stream"] is True:
                if (
                    ...
                    for idx, chunk in enumerate(result):
                        chunks.append(chunk)
                    return mishikallm.stream_chunk_builder(
        ...
        # Prints Exactly what was passed to mishikallm function - don't execute any logic here - it should just print
        print_args_passed_to_mishikallm(original_function, args, kwargs)
        start_time = datetime.datetime.now()
        result = None
        logging_obj: Optional[MishikaLLMLoggingObject] = kwargs.get(
            "mishikallm_logging_obj", None
        )

        # only set mishikallm_call_id if its not in kwargs
        if "mishikallm_call_id" not in kwargs:
            kwargs["mishikallm_call_id"] = str(uuid.uuid4())
        ...
        try:
            ...
            kwargs["mishikallm_logging_obj"] = logging_obj
            ...
            for k, v in kwargs.items():
                if v is not None and isinstance(v, str) and v.startswith("os.environ/"):
                    kwargs[k] = mishikallm.get_secret(v)
            # [OPTIONAL] CHECK BUDGET
            if mishikallm.max_budget:
                if mishikallm._current_cost > mishikallm.max_budget:
                    raise BudgetExceededError(
                        current_cost=mishikallm._current_cost,
                        max_budget=mishikallm.max_budget,
                    )
            ...
            print_verbose(
                f"SYNC kwargs[caching]: {kwargs.get('caching', False)}; mishikallm.cache: {mishikallm.cache}; kwargs.get('cache')['no-cache']: {kwargs.get('cache', {}).get('no-cache', False)}"
            )
            ...
            if call_type == CallTypes.completion.value:
                num_retries = (
                    kwargs.get("num_retries", None) or mishikallm.num_retries or None
                )
                if kwargs.get("retry_policy", None):
                ...
                _is_mishikallm_router_call = "model_group" in kwargs.get(
                    "metadata", {}
                )  # check if call from mishikallm.router/proxy
                if (
                    num_retries and not _is_mishikallm_router_call
                ):  # only enter this if call is not from mishikallm router/proxy. router has it's own logic for retrying
                    if (
                        ...
                        return mishikallm.completion_with_retries(*args, **kwargs)
                elif (
                    isinstance(e, mishikallm.exceptions.ContextWindowExceededError)
                    and context_window_fallback_dict
                    and model in context_window_fallback_dict
                    and not _is_mishikallm_router_call
                ):
                    if len(args) > 0:
    ...
    async def wrapper_async(*args, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            # [OPTIONAL] CHECK CACHE
            print_verbose(
                f"ASYNC kwargs[caching]: {kwargs.get('caching', False)}; mishikallm.cache: {mishikallm.cache}; kwargs.get('cache'): {kwargs.get('cache', None)}"
            )
            ...
            if call_type == CallTypes.acompletion.value:
                ...
                if (
                    ...
                    try:
                        ...
                        return await mishikallm.acompletion_with_retries(*args, **kwargs)
                    except Exception:
                        pass
                elif (
                    isinstance(e, mishikallm.exceptions.ContextWindowExceededError)
...
def update_response_metadata(
    ...
    Updates response metadata, adds the following:
        ...
        - response._hidden_params["mishikallm_overhead_time_ms"]
...
def _select_tokenizer_helper(model: str) -> SelectTokenizerResponse:
    if mishikallm.disable_hf_tokenizer_download is True:
...
def _return_huggingface_tokenizer(model: str) -> Optional[SelectTokenizerResponse]:
    if model in mishikallm.cohere_models and "command-r" in model:
        # cohere
    ...
    # anthropic
    elif model in mishikallm.anthropic_models and "claude-3" not in model:
...
def token_counter(
    ...
    """
    The same as `mishikallm.mishikallm_core_utils.token_counter`.
...
def supports_response_schema(
    ...
    PROVIDERS_GLOBALLY_SUPPORT_RESPONSE_SCHEMA = [
        mishikallm.LlmProviders.PREDIBASE,
        mishikallm.LlmProviders.FIREWORKS_AI,
    ]
...
def register_model(model_cost: Union[str, dict]):  # noqa: PLR0915
    ...
    if isinstance(model_cost, dict):
        ...
        loaded_model_cost = mishikallm.get_model_cost_map(url=model_cost)
    ...
    for key, value in loaded_model_cost.items():
        ...
        mishikallm.model_cost.setdefault(model_cost_key, {}).update(updated_dictionary)
        verbose_logger.debug(
            f"added/updated model={model_cost_key} in mishikallm.model_cost: {model_cost_key}"
        )
        # add new model names to provider lists
        if value.get("mishikallm_provider") == "openai":
            if key not in mishikallm.open_ai_chat_completion_models:
                mishikallm.open_ai_chat_completion_models.append(key)
        elif value.get("mishikallm_provider") == "text-completion-openai":
            if key not in mishikallm.open_ai_text_completion_models:
                mishikallm.open_ai_text_completion_models.append(key)
        elif value.get("mishikallm_provider") == "cohere":
            if key not in mishikallm.cohere_models:
                mishikallm.cohere_models.append(key)
        elif value.get("mishikallm_provider") == "anthropic":
            if key not in mishikallm.anthropic_models:
                mishikallm.anthropic_models.append(key)
        elif value.get("mishikallm_provider") == "openrouter":
            split_string = key.split("/", 1)
            if key not in mishikallm.openrouter_models:
                mishikallm.openrouter_models.append(split_string[1])
        elif value.get("mishikallm_provider") == "vertex_ai-text-models":
            if key not in mishikallm.vertex_text_models:
                mishikallm.vertex_text_models.append(key)
        elif value.get("mishikallm_provider") == "vertex_ai-code-text-models":
            if key not in mishikallm.vertex_code_text_models:
                mishikallm.vertex_code_text_models.append(key)
        elif value.get("mishikallm_provider") == "vertex_ai-chat-models":
            if key not in mishikallm.vertex_chat_models:
                mishikallm.vertex_chat_models.append(key)
        elif value.get("mishikallm_provider") == "vertex_ai-code-chat-models":
        ...
        elif value.get("mishikallm_provider") == "aleph_alpha":
            if key not in mishikallm.aleph_alpha_models:
                mishikallm.aleph_alpha_models.append(key)
        elif value.get("mishikallm_provider") == "bedrock":
            if key not in mishikallm.bedrock_models:
                mishikallm.bedrock_models.append(key)
    return model_cost
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    drop_params is True or mishikallm.drop_params is True
                ) and k not in supported_params:  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                elif k not in supported_params:
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting user/encoding format is not supported by {custom_llm_provider}. To drop it from the call, set `mishikallm.drop_params = True`.",
                    )
            return non_default_params
    ...
    if custom_llm_provider == "openai" or custom_llm_provider == "azure":
        ...
        supported_params = mishikallm.GroqSTTConfig().get_supported_openai_params_stt()
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.GroqSTTConfig().map_openai_params_stt(
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    mishikallm.drop_params is True and k not in supported_params
                ):  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                elif k not in supported_params:
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting `{k}` is not supported by {custom_llm_provider}. To drop it from the call, set `mishikallm.drop_params = True`.",
                    )
            return non_default_params
    ...
    if (
        ...
        or custom_llm_provider in mishikallm.openai_compatible_providers
    ):
        optional_params = non_default_params
    elif custom_llm_provider == "bedrock":
        # use stability3 config class if model is a stability3 model
        config_class = (
            mishikallm.AmazonStability3Config
            if mishikallm.AmazonStability3Config._is_stability_3_model(model=model)
            else (
                mishikallm.AmazonNovaCanvasConfig
                if mishikallm.AmazonNovaCanvasConfig._is_nova_model(model=model)
                else mishikallm.AmazonStabilityConfig
            )
        )
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: Optional[list]):
        ...
        if unsupported_params:
            if mishikallm.drop_params is True or (
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"{custom_llm_provider} does not support parameters: {unsupported_params}, for model={model}. To drop these, set `mishikallm.drop_params=True` or for proxy:\n\n`mishikallm_settings:\n drop_params: true`\n",
                )
    ...
    if custom_llm_provider == "openai":
        ...
        if (
            ...
            raise UnsupportedParamsError(
                status_code=500,
                message="Setting dimensions is not supported for OpenAI `text-embedding-3` and later models. To drop it from the call, set `mishikallm.drop_params = True`.",
            )
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.TritonEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.DatabricksEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.nvidiaNimEmbeddingConfig.map_openai_params(
        ...
        (
            optional_params,
            kwargs,
        ) = mishikallm.VertexAITextEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}, kwargs=kwargs
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        supported_params = (
            mishikallm.LmStudioEmbeddingConfig().get_supported_openai_params()
        )
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.LmStudioEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        if "amazon.titan-embed-text-v1" in model:
            object: Any = mishikallm.AmazonTitanG1Config()
        elif "amazon.titan-embed-image-v1" in model:
            object = mishikallm.AmazonTitanMultimodalEmbeddingG1Config()
        elif "amazon.titan-embed-text-v2:0" in model:
            object = mishikallm.AmazonTitanV2Config()
        elif "cohere.embed-multilingual-v3" in model:
            object = mishikallm.BedrockCohereEmbeddingConfig()
        else:  # unmapped model
            supported_params = []
            _check_valid_arg(supported_params=supported_params)
            final_params = {**kwargs}
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.MistralEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.JinaAIEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.VoyageEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.InfinityEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = mishikallm.FireworksAIEmbeddingConfig().map_openai_params(
        ...
        and custom_llm_provider not in mishikallm.openai_compatible_providers
    ):
        if len(non_default_params.keys()) > 0:
            if (
                mishikallm.drop_params is True or drop_params is True
            ):  # drop the unsupported non-default values
                keys = list(non_default_params.keys())
                for k in keys:
                    non_default_params.pop(k, None)
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"Setting {non_default_params} is not supported by {custom_llm_provider}. To drop it from the call, set `mishikallm.drop_params = True`.",
                )
...
def get_optional_params(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    common_auth_dict = mishikallm.common_cloud_provider_auth_params
    if custom_llm_provider in common_auth_dict["providers"]:
        ...
        if custom_llm_provider == "azure":
            optional_params = mishikallm.AzureOpenAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "bedrock":
            optional_params = (
                mishikallm.AmazonBedrockGlobalConfig().map_special_auth_params(
                    non_default_params=passed_params, optional_params=optional_params
                )
            )
        elif (
            ...
            optional_params = mishikallm.VertexAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "watsonx":
            optional_params = mishikallm.IBMWatsonXAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
    ...
    def _check_valid_arg(supported_params: List[str]):
        ...
        Args:
            supported_params: List[str] - supported params from the mishikallm config
        """
        verbose_logger.info(
            f"\nMishikaLLM completion() model= {model}; provider = {custom_llm_provider}"
        )
        verbose_logger.debug(
            f"\nMishikaLLM: Params passed to completion() {passed_params}"
        )
        verbose_logger.debug(
            f"\nMishikaLLM: Non-Default params passed to completion() {non_default_params}"
        )
        unsupported_params = {}
        for k in non_default_params.keys():
            if k not in supported_params:
        ...
        if unsupported_params:
            if mishikallm.drop_params is True or (
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"{custom_llm_provider} does not support parameters: {list(unsupported_params.keys())}, for model={model}. To drop these, set `mishikallm.drop_params=True` or for proxy:\n\n`mishikallm_settings:\n drop_params: true`\n. \n If you want to use these params dynamically send allowed_openai_params={list(unsupported_params.keys())} in your request.",
                )
    ...
    if custom_llm_provider == "anthropic":
        ...
        optional_params = mishikallm.AnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = mishikallm.AnthropicTextConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = mishikallm.CohereConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        # handle cohere params
        optional_params = mishikallm.CohereChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.TritonConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        optional_params = mishikallm.MaritalkConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.ReplicateConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.PredibaseConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.HuggingFaceChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.TogetherAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        model in mishikallm.vertex_chat_models
        or model in mishikallm.vertex_code_chat_models
        or model in mishikallm.vertex_text_models
        or model in mishikallm.vertex_code_text_models
        or model in mishikallm.vertex_language_models
        or model in mishikallm.vertex_vision_models
    ):
        optional_params = mishikallm.VertexGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.GoogleAIStudioGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
    ...
    elif mishikallm.VertexAIAnthropicConfig.is_supported_model(
        model=model, custom_llm_provider=custom_llm_provider
    ):
        optional_params = mishikallm.VertexAIAnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        if model in mishikallm.vertex_mistral_models:
            if "codestral" in model:
                optional_params = (
                    mishikallm.CodestralTextCompletionConfig().map_openai_params(
                        model=model,
                        non_default_params=non_default_params,
                        optional_params=optional_params,
                        drop_params=(
                ...
                optional_params = mishikallm.MistralConfig().map_openai_params(
                    model=model,
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    drop_params=(
        ...
        elif model in mishikallm.vertex_ai_ai21_models:
            optional_params = mishikallm.VertexAIAi21Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
            ...
            optional_params = mishikallm.VertexAILlama3Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
        ...
        optional_params = mishikallm.SagemakerConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        if bedrock_route == "converse" or bedrock_route == "converse_like":
            optional_params = mishikallm.AmazonConverseConfig().map_openai_params(
                model=model,
                non_default_params=non_default_params,
                optional_params=optional_params,
                drop_params=(
            ...
            if bedrock_base_model.startswith("anthropic.claude-3"):
                optional_params = (
                    mishikallm.AmazonAnthropicClaude3Config().map_openai_params(
                        non_default_params=non_default_params,
                        optional_params=optional_params,
                        model=model,
                        drop_params=(
                ...
                optional_params = mishikallm.AmazonAnthropicConfig().map_openai_params(
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    model=model,
                    drop_params=(
        ...
        optional_params = mishikallm.CloudflareChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = mishikallm.OllamaConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.OllamaChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = mishikallm.NLPCloudConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.PetalsConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.DeepInfraConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.CodestralTextCompletionConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.DatabricksConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.NvidiaNimConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = mishikallm.CerebrasConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.XAIChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
        )
    elif custom_llm_provider == "ai21_chat" or custom_llm_provider == "ai21":
        optional_params = mishikallm.AI21ChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.FireworksAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.VolcEngineConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.HostedVLLMChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.VLLMConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.GroqChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.OpenAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.OpenrouterConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = mishikallm.IBMWatsonXChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        for param in passed_params.keys():
            if mishikallm.IBMWatsonXAIConfig().is_watsonx_text_param(param):
                raise ValueError(
                    f"MishikaLLM now defaults to Watsonx's `/text/chat` endpoint. Please use the `watsonx_text` provider instead, to call the `/text/generation` endpoint. Param: {param}"
                )
    elif custom_llm_provider == "watsonx_text":
        optional_params = mishikallm.IBMWatsonXAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        if mishikallm.AzureOpenAIO1Config().is_o_series_model(model=model):
            optional_params = mishikallm.AzureOpenAIO1Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
            ...
            verbose_logger.debug(
                "Azure optional params - api_version: api_version={}, mishikallm.api_version={}, os.environ['AZURE_API_VERSION']={}".format(
                    api_version, mishikallm.api_version, get_secret("AZURE_API_VERSION")
                )
            )
            api_version = (
                api_version
                or mishikallm.api_version
                or get_secret("AZURE_API_VERSION")
                or mishikallm.AZURE_DEFAULT_API_VERSION
            )
            optional_params = mishikallm.AzureOpenAIConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
        ...
        optional_params = mishikallm.OpenAILikeChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
    ...
    if (
        ...
        + mishikallm.openai_compatible_providers
    ):
...
def _get_order_filtered_deployments(healthy_deployments: List[Dict]) -> List:
    min_order = min(
        (
            deployment["mishikallm_params"]["order"]
            for deployment in healthy_deployments
            if "order" in deployment["mishikallm_params"]
        ),
        default=None,
    )
    ...
    if min_order is not None:
        filtered_deployments = [
            deployment
            for deployment in healthy_deployments
            if deployment["mishikallm_params"].get("order") == min_order
        ]
...
def _get_model_region(
    custom_llm_provider: str, mishikallm_params: MishikaLLM_Params
) -> Optional[str]:
    """
    ...
    if custom_llm_provider == "vertex_ai":
        ...
        vertex_ai_location = (
            mishikallm_params.vertex_location
            or mishikallm.vertex_location
        ...
        aws_region_name = mishikallm_params.aws_region_name
        if aws_region_name is not None:
            return aws_region_name
    elif custom_llm_provider == "watsonx":
        watsonx_region_name = mishikallm_params.watsonx_region_name
        if watsonx_region_name is not None:
            return watsonx_region_name
    return mishikallm_params.region_name


def _infer_model_region(mishikallm_params: MishikaLLM_Params) -> Optional[AllowedModelRegion]:
    """
    ...
    """
    model, custom_llm_provider, _, _ = mishikallm.get_llm_provider(
        model=mishikallm_params.model, mishikallm_params=mishikallm_params
    )

    model_region = _get_model_region(
        custom_llm_provider=custom_llm_provider, mishikallm_params=mishikallm_params
    )

    if model_region is None:
        verbose_logger.debug(
            "Cannot infer model region for model: {}".format(mishikallm_params.model)
        )
        return None

    if custom_llm_provider == "azure":
        eu_regions = mishikallm.AzureOpenAIConfig().get_eu_regions()
        us_regions = mishikallm.AzureOpenAIConfig().get_us_regions()
    elif custom_llm_provider == "vertex_ai":
        eu_regions = mishikallm.VertexAIConfig().get_eu_regions()
        us_regions = mishikallm.VertexAIConfig().get_us_regions()
    elif custom_llm_provider == "bedrock":
        eu_regions = mishikallm.AmazonBedrockGlobalConfig().get_eu_regions()
        us_regions = mishikallm.AmazonBedrockGlobalConfig().get_us_regions()
    elif custom_llm_provider == "watsonx":
        eu_regions = mishikallm.IBMWatsonXAIConfig().get_eu_regions()
        us_regions = mishikallm.IBMWatsonXAIConfig().get_us_regions()
    else:
        eu_regions = []
        us_regions = []
...
def _is_region_eu(mishikallm_params: MishikaLLM_Params) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if mishikallm_params.region_name == "eu":
        return True

    ## Else - try and infer from model region
    model_region = _infer_model_region(mishikallm_params=mishikallm_params)
    if model_region is not None and model_region == "eu":
        return True
    return False


def _is_region_us(mishikallm_params: MishikaLLM_Params) -> bool:
    """
    Return true/false if a deployment is in the US
    """
    if mishikallm_params.region_name == "us":
        return True
...
def is_region_allowed(
    mishikallm_params: MishikaLLM_Params, allowed_model_region: str
) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if mishikallm_params.region_name == allowed_model_region:
        return True
    return False


def get_model_region(
    mishikallm_params: MishikaLLM_Params, mode: Optional[str]
) -> Optional[str]:
    """
    Pass the mishikallm params for an azure model, and get back the region
    """
    if (
        "azure" in mishikallm_params.model
        and isinstance(mishikallm_params.api_key, str)
        and isinstance(mishikallm_params.api_base, str)
    ):
        _model = mishikallm_params.model.replace("azure/", "")
        response: dict = mishikallm.AzureChatCompletion().get_headers(
            model=_model,
            api_key=mishikallm_params.api_key,
            api_base=mishikallm_params.api_base,
            api_version=mishikallm_params.api_version or mishikallm.AZURE_DEFAULT_API_VERSION,
            timeout=10,
            mode=mode or "chat",
        )
...
def get_api_key(llm_provider: str, dynamic_api_key: Optional[str]):
    api_key = dynamic_api_key or mishikallm.api_key
    # openai
    if llm_provider == "openai" or llm_provider == "text-completion-openai":
        api_key = api_key or mishikallm.openai_key or get_secret("OPENAI_API_KEY")
    # anthropic
    ...
    # aleph_alpha
        ...
        api_key = (
            api_key or mishikallm.aleph_alpha_key or get_secret("ALEPH_ALPHA_API_KEY")
        )
    # baseten
    elif llm_provider == "baseten":
        api_key = api_key or mishikallm.baseten_key or get_secret("BASETEN_API_KEY")
    # cohere
    elif llm_provider == "cohere" or llm_provider == "cohere_chat":
        api_key = api_key or mishikallm.cohere_key or get_secret("COHERE_API_KEY")
    # huggingface
    elif llm_provider == "huggingface":
        api_key = (
            api_key or mishikallm.huggingface_key or get_secret("HUGGINGFACE_API_KEY")
        )
    # nlp_cloud
    elif llm_provider == "nlp_cloud":
        api_key = api_key or mishikallm.nlp_cloud_key or get_secret("NLP_CLOUD_API_KEY")
    # replicate
    elif llm_provider == "replicate":
        api_key = api_key or mishikallm.replicate_key or get_secret("REPLICATE_API_KEY")
    # together_ai
    elif llm_provider == "together_ai":
        api_key = (
            api_key
            or mishikallm.togetherai_api_key
...
def get_max_tokens(model: str) -> Optional[int]:
    ...
    def _get_max_position_embeddings(model_name):
        ...
        try:
            ...
            response = mishikallm.module_level_client.get(config_url)
    ...
    try:
        if model in mishikallm.model_cost:
            if "max_output_tokens" in mishikallm.model_cost[model]:
                return mishikallm.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in mishikallm.model_cost[model]:
                return mishikallm.model_cost[model]["max_tokens"]
        model, custom_llm_provider, _, _ = get_llm_provider(model=model)
        if custom_llm_provider == "huggingface":
            max_tokens = _get_max_position_embeddings(model_name=model)
            return max_tokens
        if model in mishikallm.model_cost:  # check if extracted model is in model_list
            if "max_output_tokens" in mishikallm.model_cost[model]:
                return mishikallm.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in mishikallm.model_cost[model]:
                return mishikallm.model_cost[model]["max_tokens"]
...
def _get_model_info_from_model_cost(key: str) -> dict:
    return mishikallm.model_cost[key]
...
def _check_provider_match(model_info: dict, custom_llm_provider: Optional[str]) -> bool:
    ...
    if custom_llm_provider and (
        "mishikallm_provider" in model_info
        and model_info["mishikallm_provider"] != custom_llm_provider
    ):
...
def _get_model_info_helper(  # noqa: PLR0915
    ...
    """
    try:
        azure_llms = {**mishikallm.azure_llms, **mishikallm.azure_embedding_models}
        if model in azure_llms:
            model = azure_llms[model]
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai_beta":
            custom_llm_provider = "vertex_ai"
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai":
            if "meta/" + model in mishikallm.vertex_llama3_models:
                model = "meta/" + model
            elif model + "@latest" in mishikallm.vertex_mistral_models:
                model = model + "@latest"
            elif model + "@latest" in mishikallm.vertex_ai_ai21_models:
                model = model + "@latest"
        ##########################
        ...
        verbose_logger.debug(
            f"checking potential_model_names in mishikallm.model_cost: {potential_model_names}"
        )
        ...
        if custom_llm_provider == "huggingface":
            ...
            return ModelInfoBase(
                ...
                mishikallm_provider="huggingface",
                mode="chat",
            ...
            return mishikallm.OllamaConfig().get_model_info(model)
        else:
            """
            Check if: (in order of specificity)
            1. 'custom_llm_provider/model' in mishikallm.model_cost. Checks "groq/llama3-8b-8192" if model="llama3-8b-8192" and custom_llm_provider="groq"
            2. 'model' in mishikallm.model_cost. Checks "gemini-1.5-pro-002" in  mishikallm.model_cost if model="gemini-1.5-pro-002" and custom_llm_provider=None
            3. 'combined_stripped_model_name' in mishikallm.model_cost. Checks if 'gemini/gemini-1.5-flash' in model map, if 'gemini/gemini-1.5-flash-001' given.
            4. 'stripped_model_name' in mishikallm.model_cost. Checks if 'ft:gpt-3.5-turbo' in model map, if 'ft:gpt-3.5-turbo:my-org:custom_suffix:id' given.
            5. 'split_model' in mishikallm.model_cost. Checks "llama3-8b-8192" in mishikallm.model_cost if model="groq/llama3-8b-8192"
            """

            _model_info: Optional[Dict[str, Any]] = None
            key: Optional[str] = None

            if combined_model_name in mishikallm.model_cost:
                ...
                if not _check_provider_match(
                    ...
                    _model_info = None
            if _model_info is None and model in mishikallm.model_cost:
                key = model
                _model_info = _get_model_info_from_model_cost(key=cast(str, key))
                if not _check_provider_match(
                    model_info=_model_info, custom_llm_provider=custom_llm_provider
            ...
            if (
                _model_info is None
                and combined_stripped_model_name in mishikallm.model_cost
            ):
                ...
                if not _check_provider_match(
                    ...
                    _model_info = None
            if _model_info is None and stripped_model_name in mishikallm.model_cost:
                ...
                if not _check_provider_match(
                    ...
                    _model_info = None
            if _model_info is None and split_model in mishikallm.model_cost:
                key = split_model
                _model_info = _get_model_info_from_model_cost(key=cast(str, key))
                if not _check_provider_match(
                    model_info=_model_info, custom_llm_provider=custom_llm_provider
            ...
            return ModelInfoBase(
                ...
                mishikallm_provider=_model_info.get(
                    "mishikallm_provider", custom_llm_provider
                ),
...
def get_model_info(model: str, custom_llm_provider: Optional[str] = None) -> ModelInfo:
    ...
    Parameters:
    - model (str): The name of the model.
    - custom_llm_provider (str | null): the provider used for the model. If provided, used to check if the mishikallm model info is for that provider.

    Returns:
        dict: A dictionary containing the following information:
            key: Required[str] # the key in mishikallm.model_cost which is returned
            ...
            mishikallm_provider: Required[str]
            mode: Required[
                Literal[
    ...
    """
    supported_openai_params = mishikallm.get_supported_openai_params(
        model=model, custom_llm_provider=custom_llm_provider
    )
...
def function_to_dict(input_function):  # noqa: C901
    ...
    -------
    dictionnary
        A dictionnary to add to the list passed to `functions` parameter of `mishikallm.completion`
    """
...
def get_provider_fields(custom_llm_provider: str) -> List[ProviderField]:
    ...
    if custom_llm_provider == "databricks":
        return mishikallm.DatabricksConfig().get_required_params()

    elif custom_llm_provider == "ollama":
        return mishikallm.OllamaConfig().get_required_params()

    elif custom_llm_provider == "azure_ai":
        return mishikallm.AzureAIStudioConfig().get_required_params()
...
def validate_environment(  # noqa: PLR0915
    ...
    if custom_llm_provider:
        ...
        if (
            model in mishikallm.open_ai_chat_completion_models
            or model in mishikallm.open_ai_text_completion_models
            or model in mishikallm.open_ai_embedding_models
            or model in mishikallm.openai_image_generation_models
        ):
            if "OPENAI_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("OPENAI_API_KEY")
        ## anthropic
        elif model in mishikallm.anthropic_models:
            if "ANTHROPIC_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("ANTHROPIC_API_KEY")
        ## cohere
        elif model in mishikallm.cohere_models:
            if "COHERE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("COHERE_API_KEY")
        ## replicate
        elif model in mishikallm.replicate_models:
            if "REPLICATE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("REPLICATE_API_KEY")
        ## openrouter
        elif model in mishikallm.openrouter_models:
            if "OPENROUTER_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("OPENROUTER_API_KEY")
        ## vertex - text + chat models
            ...
            model in mishikallm.vertex_chat_models
            or model in mishikallm.vertex_text_models
            or model in mishikallm.models_by_provider["vertex_ai"]
        ):
            if "VERTEXAI_PROJECT" in os.environ and "VERTEXAI_LOCATION" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.extend(["VERTEXAI_PROJECT", "VERTEXAI_LOCATION"])
        ## huggingface
        elif model in mishikallm.huggingface_models:
            if "HUGGINGFACE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("HUGGINGFACE_API_KEY")
        ## ai21
            ...
            if "AI21_API_KEY" in os.environ:
                ...
                missing_keys.append("AI21_API_KEY")
        ## together_ai
        elif model in mishikallm.together_ai_models:
            if "TOGETHERAI_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("TOGETHERAI_API_KEY")
        ## aleph_alpha
        elif model in mishikallm.aleph_alpha_models:
            if "ALEPH_ALPHA_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("ALEPH_ALPHA_API_KEY")
        ## baseten
        elif model in mishikallm.baseten_models:
            if "BASETEN_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("BASETEN_API_KEY")
        ## nlp_cloud
        elif model in mishikallm.nlp_cloud_models:
            if "NLP_CLOUD_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("NLP_CLOUD_API_KEY")
...
def acreate(*args, **kwargs):  ## Thin client to handle the acreate langchain call
    return mishikallm.acompletion(*args, **kwargs)
...
def valid_model(model):
    try:
        ...
        if (
            ...
            mishikallm.completion(model=model, messages=messages)
    except Exception:
...
def check_valid_key(model: str, api_key: str):
    """
    Checks if a given API key is valid for a specific model by making a mishikallm.completion call with max_tokens=10
...
def register_prompt_template(
    ...
    if tokenizer_config:
        for m in potential_models:
            mishikallm.known_tokenizer_config[m] = {
                "tokenizer": tokenizer_config,
                "status": "success",
            }
    else:
        for m in potential_models:
            mishikallm.custom_prompt_dict[m] = {
                "roles": roles,
                "initial_prompt_value": initial_prompt_value,
                "final_prompt_value": final_prompt_value,
            }

    return mishikallm.custom_prompt_dict
...
def mock_completion_streaming_obj(
    ...
    if isinstance(mock_response, mishikallm.MockException):
        raise mock_response
    for i in range(0, len(mock_response), 3):
        ...
        if n is None:
            ...
            _all_choices = []
            for j in range(n):
                _streaming_choice = mishikallm.utils.StreamingChoices(
                    index=j,
...
def trim_messages(
    ...
    Args:
        ...
        model: The MishikaLLM model being used (determines the token limit).
    ...
    try:
        if max_tokens is None:
            ...
            if model in mishikallm.model_cost:
                max_tokens_for_model = mishikallm.model_cost[model].get(
                    "max_input_tokens", mishikallm.model_cost[model]["max_tokens"]
                )
                max_tokens = int(max_tokens_for_model * trim_ratio)
            else:
                # if user did not specify max (input) tokens
                # or passed an llm mishikallm does not know
...
from mishikallm.caching.in_memory_cache import InMemoryCache
...
class AvailableModelsCache(InMemoryCache):
    ...
    def _get_cache_key(
        ...
        mishikallm_params: Optional[MishikaLLM_Params],
    ) -> str:
        valid_str = ""

        if mishikallm_params is not None:
            valid_str = mishikallm_params.model_dump_json()
        if custom_llm_provider is not None:
    ...
    def get_cached_model_info(
        self,
        custom_llm_provider: Optional[str] = None,
        mishikallm_params: Optional[MishikaLLM_Params] = None,
    ) -> Optional[List[str]]:
        """Get cached model info"""
        # Check if environment has changed
        if mishikallm_params is None and self._check_env_changed():
            self.cache_dict.clear()
            return None

        cache_key = self._get_cache_key(custom_llm_provider, mishikallm_params)
...
def _infer_valid_provider_from_env_vars(
    ...
    for provider in mishikallm.provider_list:
        if custom_llm_provider and provider != custom_llm_provider:
            continue

        # edge case mishikallm has together_ai as a provider, it should be togetherai
        env_provider_1 = provider.replace("_", "")
        env_provider_2 = provider

        # mishikallm standardizes expected provider keys to
...
def _get_valid_models_from_provider_api(
    ...
    try:
        cached_result = _model_cache.get_cached_model_info(
            custom_llm_provider, mishikallm_params
        )

        if cached_result is not None:
            return cached_result
        models = provider_config.get_models(
            api_key=mishikallm_params.api_key if mishikallm_params is not None else None,
            api_base=mishikallm_params.api_base if mishikallm_params is not None else None,
        )

        _model_cache.set_cached_model_info(custom_llm_provider, mishikallm_params, models)
        return models
    except Exception as e:
...
def get_valid_models(
    ...
    try:
        check_provider_endpoint = (
            check_provider_endpoint or mishikallm.check_provider_endpoint
        )
        # get keys set in .env
        ...
        for provider in valid_providers:
            ...
            if provider == "azure":
                ...
                models_for_provider = copy.deepcopy(
                    mishikallm.models_by_provider.get(provider, [])
                )
...
def print_args_passed_to_mishikallm(original_function, args, kwargs):
    ...
    try:
        ...
        print_verbose(
            "\033[92mRequest to mishikallm:\033[0m",
        )
        if args and kwargs:
            print_verbose(
                f"\033[92mmishikallm.{original_function.__name__}({args_str}, {kwargs_str})\033[0m"
            )
        elif args:
            print_verbose(
                f"\033[92mmishikallm.{original_function.__name__}({args_str})\033[0m"
            )
        elif kwargs:
            print_verbose(
                f"\033[92mmishikallm.{original_function.__name__}({kwargs_str})\033[0m"
            )
        else:
            print_verbose(f"\033[92mmishikallm.{original_function.__name__}()\033[0m")
        print_verbose("\n")  # new line after
    except Exception:
...
def _get_base_model_from_metadata(model_call_details=None):
    if model_call_details is None:
        return None
    mishikallm_params = model_call_details.get("mishikallm_params", {})
    if mishikallm_params is not None:
        _base_model = mishikallm_params.get("base_model", None)
        if _base_model is not None:
            return _base_model
        metadata = mishikallm_params.get("metadata", {})

        return _get_base_model_from_mishikallm_call_metadata(metadata=metadata)
    return None
...
class ProviderConfigManager:
    ...
    def get_provider_chat_config(  # noqa: PLR0915
        ...
        if (
            ...
            and mishikallm.openaiOSeriesConfig.is_model_o_series_model(model=model)
        ):
            return mishikallm.openaiOSeriesConfig
        elif mishikallm.LlmProviders.DEEPSEEK == provider:
            return mishikallm.DeepSeekChatConfig()
        elif mishikallm.LlmProviders.GROQ == provider:
            return mishikallm.GroqChatConfig()
        elif mishikallm.LlmProviders.DATABRICKS == provider:
            return mishikallm.DatabricksConfig()
        elif mishikallm.LlmProviders.XAI == provider:
            return mishikallm.XAIChatConfig()
        elif mishikallm.LlmProviders.LLAMA == provider:
            return mishikallm.LlamaAPIConfig()
        elif mishikallm.LlmProviders.TEXT_COMPLETION_OPENAI == provider:
            return mishikallm.OpenAITextCompletionConfig()
        elif mishikallm.LlmProviders.COHERE_CHAT == provider:
            return mishikallm.CohereChatConfig()
        elif mishikallm.LlmProviders.COHERE == provider:
            return mishikallm.CohereConfig()
        elif mishikallm.LlmProviders.SNOWFLAKE == provider:
            return mishikallm.SnowflakeConfig()
        elif mishikallm.LlmProviders.CLARIFAI == provider:
            return mishikallm.ClarifaiConfig()
        elif mishikallm.LlmProviders.ANTHROPIC == provider:
            return mishikallm.AnthropicConfig()
        elif mishikallm.LlmProviders.ANTHROPIC_TEXT == provider:
            return mishikallm.AnthropicTextConfig()
        elif mishikallm.LlmProviders.VERTEX_AI_BETA == provider:
            return mishikallm.VertexGeminiConfig()
        elif mishikallm.LlmProviders.VERTEX_AI == provider:
            if "gemini" in model:
                return mishikallm.VertexGeminiConfig()
            elif "claude" in model:
                return mishikallm.VertexAIAnthropicConfig()
            elif model in mishikallm.vertex_mistral_models:
                if "codestral" in model:
                    return mishikallm.CodestralTextCompletionConfig()
                else:
                    return mishikallm.MistralConfig()
            elif model in mishikallm.vertex_ai_ai21_models:
                return mishikallm.VertexAIAi21Config()
            else:  # use generic openai-like param mapping
                return mishikallm.VertexAILlama3Config()
        elif mishikallm.LlmProviders.CLOUDFLARE == provider:
            return mishikallm.CloudflareChatConfig()
        elif mishikallm.LlmProviders.SAGEMAKER_CHAT == provider:
            return mishikallm.SagemakerChatConfig()
        elif mishikallm.LlmProviders.SAGEMAKER == provider:
            return mishikallm.SagemakerConfig()
        elif mishikallm.LlmProviders.FIREWORKS_AI == provider:
            return mishikallm.FireworksAIConfig()
        elif mishikallm.LlmProviders.FRIENDLIAI == provider:
            return mishikallm.FriendliaiChatConfig()
        elif mishikallm.LlmProviders.WATSONX == provider:
            return mishikallm.IBMWatsonXChatConfig()
        elif mishikallm.LlmProviders.WATSONX_TEXT == provider:
            return mishikallm.IBMWatsonXAIConfig()
        elif mishikallm.LlmProviders.EMPOWER == provider:
            return mishikallm.EmpowerChatConfig()
        elif mishikallm.LlmProviders.GITHUB == provider:
            return mishikallm.GithubChatConfig()
        elif (
            mishikallm.LlmProviders.CUSTOM == provider
            or mishikallm.LlmProviders.CUSTOM_OPENAI == provider
            or mishikallm.LlmProviders.OPENAI_LIKE == provider
        ):
            return mishikallm.OpenAILikeChatConfig()
        elif mishikallm.LlmProviders.AIOHTTP_OPENAI == provider:
            return mishikallm.AiohttpOpenAIChatConfig()
        elif mishikallm.LlmProviders.HOSTED_VLLM == provider:
            return mishikallm.HostedVLLMChatConfig()
        elif mishikallm.LlmProviders.LLAMAFILE == provider:
            return mishikallm.LlamafileChatConfig()
        elif mishikallm.LlmProviders.LM_STUDIO == provider:
            return mishikallm.LMStudioChatConfig()
        elif mishikallm.LlmProviders.GALADRIEL == provider:
            return mishikallm.GaladrielChatConfig()
        elif mishikallm.LlmProviders.REPLICATE == provider:
            return mishikallm.ReplicateConfig()
        elif mishikallm.LlmProviders.HUGGINGFACE == provider:
            return mishikallm.HuggingFaceChatConfig()
        elif mishikallm.LlmProviders.TOGETHER_AI == provider:
            return mishikallm.TogetherAIConfig()
        elif mishikallm.LlmProviders.OPENROUTER == provider:
            return mishikallm.OpenrouterConfig()
        elif mishikallm.LlmProviders.GEMINI == provider:
            return mishikallm.GoogleAIStudioGeminiConfig()
        elif (
            mishikallm.LlmProviders.AI21 == provider
            or mishikallm.LlmProviders.AI21_CHAT == provider
        ):
            return mishikallm.AI21ChatConfig()
        elif mishikallm.LlmProviders.AZURE == provider:
            if mishikallm.AzureOpenAIO1Config().is_o_series_model(model=model):
                return mishikallm.AzureOpenAIO1Config()
            return mishikallm.AzureOpenAIConfig()
        elif mishikallm.LlmProviders.AZURE_AI == provider:
            return mishikallm.AzureAIStudioConfig()
        elif mishikallm.LlmProviders.AZURE_TEXT == provider:
            return mishikallm.AzureOpenAITextConfig()
        elif mishikallm.LlmProviders.HOSTED_VLLM == provider:
            return mishikallm.HostedVLLMChatConfig()
        elif mishikallm.LlmProviders.NLP_CLOUD == provider:
            return mishikallm.NLPCloudConfig()
        elif mishikallm.LlmProviders.OOBABOOGA == provider:
            return mishikallm.OobaboogaConfig()
        elif mishikallm.LlmProviders.OLLAMA_CHAT == provider:
            return mishikallm.OllamaChatConfig()
        elif mishikallm.LlmProviders.DEEPINFRA == provider:
            return mishikallm.DeepInfraConfig()
        elif mishikallm.LlmProviders.PERPLEXITY == provider:
            return mishikallm.PerplexityChatConfig()
        elif (
            mishikallm.LlmProviders.MISTRAL == provider
            or mishikallm.LlmProviders.CODESTRAL == provider
        ):
            return mishikallm.MistralConfig()
        elif mishikallm.LlmProviders.NVIDIA_NIM == provider:
            return mishikallm.NvidiaNimConfig()
        elif mishikallm.LlmProviders.CEREBRAS == provider:
            return mishikallm.CerebrasConfig()
        elif mishikallm.LlmProviders.VOLCENGINE == provider:
            return mishikallm.VolcEngineConfig()
        elif mishikallm.LlmProviders.TEXT_COMPLETION_CODESTRAL == provider:
            return mishikallm.CodestralTextCompletionConfig()
        elif mishikallm.LlmProviders.SAMBANOVA == provider:
            return mishikallm.SambanovaConfig()
        elif mishikallm.LlmProviders.MARITALK == provider:
            return mishikallm.MaritalkConfig()
        elif mishikallm.LlmProviders.CLOUDFLARE == provider:
            return mishikallm.CloudflareChatConfig()
        elif mishikallm.LlmProviders.ANTHROPIC_TEXT == provider:
            return mishikallm.AnthropicTextConfig()
        elif mishikallm.LlmProviders.VLLM == provider:
            return mishikallm.VLLMConfig()
        elif mishikallm.LlmProviders.OLLAMA == provider:
            return mishikallm.OllamaConfig()
        elif mishikallm.LlmProviders.PREDIBASE == provider:
            return mishikallm.PredibaseConfig()
        elif mishikallm.LlmProviders.TRITON == provider:
            return mishikallm.TritonConfig()
        elif mishikallm.LlmProviders.PETALS == provider:
            return mishikallm.PetalsConfig()
        elif mishikallm.LlmProviders.BEDROCK == provider:
            bedrock_route = BedrockModelInfo.get_bedrock_route(model)
            bedrock_invoke_provider = mishikallm.BedrockLLM.get_bedrock_invoke_provider(
                model=model
            )
            base_model = BedrockModelInfo.get_base_model(model)

            if bedrock_route == "converse" or bedrock_route == "converse_like":
                return mishikallm.AmazonConverseConfig()
            elif bedrock_invoke_provider == "amazon":  # amazon titan llms
                return mishikallm.AmazonTitanConfig()
            elif bedrock_invoke_provider == "anthropic":
                if base_model.startswith("anthropic.claude-3"):
                    return mishikallm.AmazonAnthropicClaude3Config()
                else:
                    return mishikallm.AmazonAnthropicConfig()
            elif (
                bedrock_invoke_provider == "meta" or bedrock_invoke_provider == "llama"
            ):  # amazon / meta llms
                return mishikallm.AmazonLlamaConfig()
            elif bedrock_invoke_provider == "ai21":  # ai21 llms
                return mishikallm.AmazonAI21Config()
            elif bedrock_invoke_provider == "cohere":  # cohere models on bedrock
                return mishikallm.AmazonCohereConfig()
            elif bedrock_invoke_provider == "mistral":  # mistral models on bedrock
                return mishikallm.AmazonMistralConfig()
            elif bedrock_invoke_provider == "deepseek_r1":  # deepseek models on bedrock
                return mishikallm.AmazonDeepSeekR1Config()
            elif bedrock_invoke_provider == "nova":
                return mishikallm.AmazonInvokeNovaConfig()
            else:
                return mishikallm.AmazonInvokeConfig()
        elif mishikallm.LlmProviders.MISHIKALLM_PROXY == provider:
            return mishikallm.MishikaLLMProxyChatConfig()
        elif mishikallm.LlmProviders.OPENAI == provider:
            return mishikallm.OpenAIGPTConfig()
        return None
    ...
    def get_provider_embedding_config(
        ...
        if mishikallm.LlmProviders.VOYAGE == provider:
            return mishikallm.VoyageEmbeddingConfig()
        elif mishikallm.LlmProviders.TRITON == provider:
            return mishikallm.TritonEmbeddingConfig()
        elif mishikallm.LlmProviders.WATSONX == provider:
            return mishikallm.IBMWatsonXEmbeddingConfig()
        elif mishikallm.LlmProviders.INFINITY == provider:
            return mishikallm.InfinityEmbeddingConfig()
    ...
    def get_provider_rerank_config(
        ...
        if mishikallm.LlmProviders.COHERE == provider:
            if should_use_cohere_v1_client(api_base, present_version_params):
                return mishikallm.CohereRerankConfig()
            else:
                return mishikallm.CohereRerankV2Config()
        elif mishikallm.LlmProviders.AZURE_AI == provider:
            return mishikallm.AzureAIRerankConfig()
        elif mishikallm.LlmProviders.INFINITY == provider:
            return mishikallm.InfinityRerankConfig()
        elif mishikallm.LlmProviders.JINA_AI == provider:
            return mishikallm.JinaAIRerankConfig()
        return mishikallm.CohereRerankConfig()
    ...
    def get_provider_anthropic_messages_config(
        ...
        if mishikallm.LlmProviders.ANTHROPIC == provider:
            return mishikallm.AnthropicMessagesConfig()
        return None
    ...
    def get_provider_audio_transcription_config(
        ...
        if mishikallm.LlmProviders.FIREWORKS_AI == provider:
            return mishikallm.FireworksAIAudioTranscriptionConfig()
        elif mishikallm.LlmProviders.DEEPGRAM == provider:
            return mishikallm.DeepgramAudioTranscriptionConfig()
        elif mishikallm.LlmProviders.OPENAI == provider:
            if "gpt-4o" in model:
                return mishikallm.OpenAIGPTAudioTranscriptionConfig()
            else:
                return mishikallm.OpenAIWhisperAudioTranscriptionConfig()
        return None
    ...
    def get_provider_responses_api_config(
        ...
        if mishikallm.LlmProviders.OPENAI == provider:
            return mishikallm.OpenAIResponsesAPIConfig()
        elif mishikallm.LlmProviders.AZURE == provider:
            return mishikallm.AzureOpenAIResponsesAPIConfig()
        return None
    ...
    def get_provider_text_completion_config(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            return mishikallm.FireworksAITextCompletionConfig()
        elif LlmProviders.TOGETHER_AI == provider:
            return mishikallm.TogetherAITextCompletionConfig()
        return mishikallm.OpenAITextCompletionConfig()
    ...
    def get_provider_model_info(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            ...
            return mishikallm.GeminiModelInfo()
        elif LlmProviders.MISHIKALLM_PROXY == provider:
            return mishikallm.MishikaLLMProxyChatConfig()
        elif LlmProviders.TOPAZ == provider:
            return mishikallm.TopazModelInfo()
        elif LlmProviders.ANTHROPIC == provider:
            return mishikallm.AnthropicModelInfo()
        elif LlmProviders.XAI == provider:
            return mishikallm.XAIModelInfo()
        elif LlmProviders.VLLM == provider:
            from mishikallm.llms.vllm.common_utils import (
    ...
    def get_provider_image_variation_config(
        ...
        if LlmProviders.OPENAI == provider:
            return mishikallm.OpenAIImageVariationConfig()
        elif LlmProviders.TOPAZ == provider:
            return mishikallm.TopazImageVariationConfig()
        return None
    ...
    def get_provider_files_config(
        ...
        if LlmProviders.GEMINI == provider:
            from mishikallm.llms.gemini.files.transformation import (
                GoogleAIStudioFilesHandler,  # experimental approach, to reduce bloat on __init__.py
            )

            return GoogleAIStudioFilesHandler()
        elif LlmProviders.VERTEX_AI == provider:
            from mishikallm.llms.vertex_ai.files.transformation import VertexAIFilesConfig
    ...
    def get_provider_vector_store_config(
        ...
        from mishikallm.integrations.vector_stores.bedrock_vector_store import (
            BedrockVectorStore,
        )
...
def get_end_user_id_for_cost_tracking(
    ...
    service_type: Literal["mishikallm_logging", "prometheus"] = "mishikallm_logging",
) -> Optional[str]:
    """
    Used for enforcing `disable_end_user_cost_tracking` param.

    service_type: "mishikallm_logging" or "prometheus" - used to allow prometheus only disable cost tracking.
    """
    _metadata = cast(dict, mishikallm_params.get("metadata", {}) or {})

    end_user_id = cast(
        Optional[str],
        mishikallm_params.get("user_api_key_end_user_id")
        or _metadata.get("user_api_key_end_user_id"),
    )
    if mishikallm.disable_end_user_cost_tracking:
        return None
    if (
        service_type == "prometheus"
        and mishikallm.disable_end_user_cost_tracking_prometheus_only
    ):
        return None
    return end_user_id
...
def get_non_default_completion_params(kwargs: dict) -> dict:
    openai_params = mishikallm.OPENAI_CHAT_COMPLETION_PARAMS
    default_params = openai_params + all_mishikallm_params
    non_default_params = {
...
def return_raw_request(endpoint: CallTypes, kwargs: dict) -> RawRequestTypedDict:
    ...
    This is currently in BETA, and tested for `/chat/completions` -> `mishikallm.completion` calls.
    """
    from datetime import datetime

    from mishikallm.mishikallm_core_utils.mishikallm_logging import Logging

    mishikallm_logging_obj = Logging(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "hi"}],
        stream=False,
        call_type="acompletion",
        mishikallm_call_id="1234",
        start_time=datetime.now(),
        function_id="1234",
        log_raw_request_response=True,
    )

    llm_api_endpoint = getattr(mishikallm, endpoint.value)
    ...
    raw_request_typed_dict = mishikallm_logging_obj.model_call_details.get(
