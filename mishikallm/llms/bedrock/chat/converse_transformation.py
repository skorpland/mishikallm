...
from mishikallm.utils import add_dummy_tool, has_tool_call_blocks
...
class AmazonConverseConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> List[str]:
        ...
        if mishikallm.utils.supports_tool_choice(
        ...
        if tool_choice == "none":
            if mishikallm.drop_params is True or drop_params is True:
                ...
                raise mishikallm.utils.UnsupportedParamsError(
                    message="Bedrock doesn't support tool_choice={}. To drop it from the call, set `mishikallm.drop_params = True.".format(
                        tool_choice
                    ),
                    status_code=400,
                )
            ...
            raise mishikallm.utils.UnsupportedParamsError(
                message="Bedrock doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `mishikallm.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
    ...
    def map_openai_params(
        ...
        for param, value in non_default_params.items():
            if param == "response_format" and isinstance(value, dict):
                ...
                if (
                    mishikallm.utils.supports_tool_choice(
                        model=model, custom_llm_provider=self.custom_llm_provider
                    )
        ...
        if (
            ...
            if mishikallm.modify_params:
                ...
                raise mishikallm.UnsupportedParamsError(
                    message="Bedrock doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `mishikallm.modify_params = True` // `mishikallm_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
    ...
    async def _async_transform_request(
        ...
        bedrock_messages = (
            await BedrockConverseMessagesProcessor._bedrock_converse_messages_pt_async(
                ...
                user_continue_message=mishikallm_params.pop("user_continue_message", None),
            )
        )
