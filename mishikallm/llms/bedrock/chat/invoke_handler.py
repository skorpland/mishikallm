"""
TODO: DELETE FILE. Bedrock LLM is no longer used. Goto `mishikallm/llms/bedrock/chat/invoke_transformations/base_invoke_transformation.py`
"""
...
from mishikallm.caching.caching import InMemoryCache
from mishikallm.mishikallm_core_utils.core_helpers import map_finish_reason
from mishikallm.mishikallm_core_utils.mishikallm_logging import Logging
from mishikallm.mishikallm_core_utils.logging_utils import track_llm_api_timing
...
from mishikallm.types.utils import ChatCompletionMessageToolCall, Choices, Delta
from mishikallm.types.utils import GenericStreamingChunk as GChunk
...
from mishikallm.utils import CustomStreamWrapper, get_secret
...
async def make_call(
    ...
    bedrock_invoke_provider: Optional[mishikallm.BEDROCK_INVOKE_PROVIDERS_LITERAL] = None,
):
    try:
        if client is None:
            client = get_async_httpx_client(
                llm_provider=mishikallm.LlmProviders.BEDROCK
        ...
        if fake_stream:
            model_response: (
                ModelResponse
            ) = mishikallm.AmazonConverseConfig()._transform_response(
                model=model,
                response=response,
...
class BedrockLLM(BaseAWSLLM):
    ...
    def process_response(  # noqa: PLR0915
            ...
            if provider == "cohere":
                ...
                if model.startswith("anthropic.claude-3"):
                    ...
                    if (
                        ...
                        # streaming_model_response.choices = [mishikallm.utils.StreamingChoices()]
                        streaming_choice = mishikallm.utils.StreamingChoices()
                        ...
                        if isinstance(model_response.choices[0], mishikallm.Choices):
                            if getattr(
                            ...
                            return mishikallm.CustomStreamWrapper(
        ...
        if stream and provider == "ai21":
            ...
            streaming_choice = mishikallm.utils.StreamingChoices()
            streaming_choice.index = model_response.choices[0].index
            delta_obj = mishikallm.utils.Delta(
        ...
        prompt_tokens = int(
            bedrock_input_tokens or mishikallm.token_counter(messages=messages)
        )

        completion_tokens = int(
            bedrock_output_tokens
            or mishikallm.token_counter(
    ...
    def completion(  # noqa: PLR0915
        ...
        if aws_region_name is None:
            # check env #
            mishikallm_aws_region_name = get_secret("AWS_REGION_NAME", None)

            if mishikallm_aws_region_name is not None and isinstance(
                mishikallm_aws_region_name, str
            ):
                aws_region_name = mishikallm_aws_region_name
        ...
        if provider == "cohere":
            if model.startswith("cohere.command-r"):
                ## LOAD CONFIG
                config = mishikallm.AmazonCohereChatConfig().get_config()
                ...
                ## LOAD CONFIG
                config = mishikallm.AmazonCohereConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            if model.startswith("anthropic.claude-3"):
                ...
                ## LOAD CONFIG
                config = mishikallm.AmazonAnthropicClaude3Config.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
                ...
                ## LOAD CONFIG
                config = mishikallm.AmazonAnthropicConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = mishikallm.AmazonAI21Config.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = mishikallm.AmazonMistralConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
            ...
            ## LOAD CONFIG
            config = mishikallm.AmazonTitanConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > amazon_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = mishikallm.AmazonLlamaConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
        ...
        if client is None:
            ...
            if timeout is not None:
                if isinstance(timeout, float) or isinstance(timeout, int):
                    timeout = httpx.Timeout(timeout)
                _params["timeout"] = timeout
            client = get_async_httpx_client(params=_params, llm_provider=mishikallm.LlmProviders.BEDROCK)  # type: ignore
        else:
    ...
    def _get_provider_from_model_path(
        model_path: str,
    ) -> Optional[mishikallm.BEDROCK_INVOKE_PROVIDERS_LITERAL]:
        """
        ...
        if len(parts) >= 1:
            provider = parts[0]
            if provider in get_args(mishikallm.BEDROCK_INVOKE_PROVIDERS_LITERAL):
                return cast(mishikallm.BEDROCK_INVOKE_PROVIDERS_LITERAL, provider)
        return None

    def get_bedrock_model_id(
        self,
        optional_params: dict,
        provider: Optional[mishikallm.BEDROCK_INVOKE_PROVIDERS_LITERAL],
        model: str,
    ) -> str:
            ...
            if "start" in chunk_data:
                ...
                if start_obj is not None:
                    if "toolUse" in start_obj and start_obj["toolUse"] is not None:
                        ## check tool name was formatted by mishikallm
...
class AmazonDeepSeekR1StreamDecoder(AWSEventStreamDecoder):
    def __init__(
        ...
        super().__init__(model=model)
        from mishikallm.llms.bedrock.chat.invoke_transformations.amazon_deepseek_transformation import (
...
class MockResponseIterator:  # for returning ai21 streaming responses
    ...
    def _handle_json_mode_chunk(
        ...
        if self.json_mode is True and tool_calls is not None:
            message = mishikallm.AnthropicConfig()._convert_tool_response_to_message(
