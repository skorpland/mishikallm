...
def init_bedrock_client(
    ...
    ssl_verify = os.getenv("SSL_VERIFY", mishikallm.ssl_verify)
    ...
    elif mishikallm_aws_region_name:
        region_name = mishikallm_aws_region_name
    ...
    if (
        ...
        # aws_access_key_id is not None, assume user is trying to auth using mishikallm.completion
...
def get_bedrock_tool_name(response_tool_name: str) -> str:
    """
    If mishikallm formatted the input tool name, we need to convert it back to the original name.
    ...
    if response_tool_name in mishikallm.bedrock_tool_name_mappings.cache_dict:
        response_tool_name = mishikallm.bedrock_tool_name_mappings.cache_dict[
            response_tool_name
        ]
...
class BedrockModelInfo(BaseLLMModelInfo):
    ...
    @staticmethod
    def get_non_mishikallm_routing_model_name(model: str) -> str:
    ...
    def get_base_model(model: str) -> str:
        ...
        model = BedrockModelInfo.get_non_mishikallm_routing_model_name(model=model)
    ...
    def get_bedrock_route(model: str) -> Literal["converse", "invoke", "converse_like"]:
        ...
        alt_model = BedrockModelInfo.get_non_mishikallm_routing_model_name(model=model)
        if "invoke/" in model:
            return "invoke"
            ...
            base_model in mishikallm.bedrock_converse_models
            or alt_model in mishikallm.bedrock_converse_models
        ):
            return "converse"
        return "invoke"
