...
class BaseOpenAILLM:
    ...
    def get_cached_openai_client(
        ...
        _cached_client = mishikallm.in_memory_llm_clients_cache.get_cache(_cache_key)
    ...
    def get_openai_client_cache_key(
        ...
        MISHIKALLM_CLIENT_SPECIFIC_PARAMS = [
            "timeout",
            "max_retries",
            "organization",
            "api_base",
    ...
    def _get_async_http_client() -> Optional[httpx.AsyncClient]:
        if mishikallm.aclient_session is not None:
            return mishikallm.aclient_session

        return httpx.AsyncClient(
            limits=httpx.Limits(max_connections=1000, max_keepalive_connections=100),
            verify=mishikallm.ssl_verify,
        )

    @staticmethod
    def _get_sync_http_client() -> Optional[httpx.Client]:
        if mishikallm.client_session is not None:
            return mishikallm.client_session
        return httpx.Client(
            limits=httpx.Limits(max_connections=1000, max_keepalive_connections=100),
            verify=mishikallm.ssl_verify,
        )
