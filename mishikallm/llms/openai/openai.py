...
from mishikallm.constants import DEFAULT_MAX_RETRIES
...
from mishikallm.llms.base_llm.base_model_iterator import BaseModelResponseIterator
...
class OpenAIConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list:
        ...
        elif mishikallm.openAIGPTAudioConfig.is_model_gpt_audio_model(model=model):
            return mishikallm.openAIGPTAudioConfig.get_supported_openai_params(model=model)
        else:
            return mishikallm.openAIGPTConfig.get_supported_openai_params(model=model)
    ...
    def map_openai_params(
        ...
        if openaiOSeriesConfig.is_model_o_series_model(model=model):
            ...
            return mishikallm.openAIGPTAudioConfig.map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=drop_params,
        ...
        return mishikallm.openAIGPTConfig.map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=drop_params,
...
class OpenAIChatCompletion(BaseLLM, BaseOpenAILLM):
    ...
    async def make_openai_chat_completion_request(
        ...
        """
        Helper to:
        - call chat.completions.create.with_raw_response when mishikallm.return_response_headers is True
            ...
            for _ in range(
                ...
                try:
                    ...
                    if (
                        ...
                        mishikallm.remove_index_from_tool_calls(messages=messages)
                    else:
                        raise e
    ...
    def get_stream_options(
        ...
        if stream_options is not None:
            ...
            # by default mishikallm will include usage for openai endpoints
    ...
    async def make_openai_embedding_request(
        ...
        """
        Helper to:
        - call embeddings.create.with_raw_response when mishikallm.return_response_headers is True
    ...
    def embedding(  # type: ignore
        ...
        try:
            ...
            max_retries = max_retries or mishikallm.DEFAULT_MAX_RETRIES
            if not isinstance(max_retries, int):
...
class OpenAIAssistantsAPI(BaseLLM):
    ...
    def create_thread(
        ...
        Here's an example:
        ```
        from mishikallm.llms.openai.openai import OpenAIAssistantsAPI, MessageData
