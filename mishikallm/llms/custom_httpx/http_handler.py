...
import mishikallm
from mishikallm.constants import _DEFAULT_TTL_FOR_HTTPX_CLIENTS
from mishikallm.mishikallm_core_utils.logging_utils import track_llm_api_timing
from mishikallm.types.llms.custom_http import *
...
class AsyncHTTPHandler:
    ...
    def create_client(
        ...
        # /path/to/client.pem
        cert = os.getenv("SSL_CERTIFICATE", mishikallm.ssl_certificate)
    ...
    async def post(
        ...
        timeout: Optional[Union[float, httpx.Timeout]] = None,
        stream: bool = False,
        logging_obj: Optional[MishikaLLMLoggingObject] = None,
    ):
        ...
        try:
            ...
            raise mishikallm.Timeout(
                ...
                llm_provider="mishikallm-httpx-handler",
                headers=headers,
            )
    ...
    def _create_async_transport(self) -> Optional[AsyncHTTPTransport]:
        """
        Create an async transport with IPv4 only if mishikallm.force_ipv4 is True.
        Otherwise, return None.

        Some users have seen httpx ConnectionError when using ipv6 - forcing ipv4 resolves the issue for them
        """
        if mishikallm.force_ipv4:
...
class HTTPHandler:
    ...
    def _create_sync_transport(self) -> Optional[HTTPTransport]:
        """
        Create an HTTP transport with IPv4 only if mishikallm.force_ipv4 is True.
        Otherwise, return None.
...
def get_async_httpx_client(
    ...
    _cached_client = mishikallm.in_memory_llm_clients_cache.get_cache(_cache_key_name)
    if _cached_client:
