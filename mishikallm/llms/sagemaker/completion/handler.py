...
from mishikallm.mishikallm_core_utils.asyncify import asyncify
...
class SagemakerLLM(BaseAWSLLM):
    ...
    def completion(  # noqa: PLR0915
        ...
        ## Load Config
        config = mishikallm.SagemakerConfig.get_config()
        ...
        try:
            ...
            if "Inference Component Name header is required" in error_message:
                error_message += "\n pass in via `mishikallm.completion(..., model_id={InferenceComponentName})`"
        ...
        try:
            if client is None:
                client = get_async_httpx_client(
                    llm_provider=mishikallm.LlmProviders.SAGEMAKER
    ...
    async def async_streaming(
        ...
        if model_id is not None:  # Fixes https://github.com/skorpland/mishikallm/issues/8889
            prepared_request.headers.update(
                {"X-Amzn-SageMaker-Inference-Component": model_id}
            )
