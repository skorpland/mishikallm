...
import mishikallm
from mishikallm.constants import AZURE_OPERATION_POLLING_TIMEOUT, DEFAULT_MAX_RETRIES
                ...
                if isinstance(value, list):
                    for item in value:
                        if "file_id" in item:
                            ...
                            if mishikallm.drop_params is True:
                                pass
                            else:
                                raise mishikallm.utils.UnsupportedParamsError(
                                    message="Azure doesn't support {}. To drop it from the call, set `mishikallm.drop_params = True.".format(
                                        value
                                    ),
                                    status_code=400,
                                )
...
class AzureChatCompletion(BaseAzureLLM, BaseLLM):
    ...
    def completion(  # noqa: PLR0915
        ...
        try:
            ...
            if "gateway.ai.cloudflare.com" in api_base:
                ...
                data = mishikallm.AzureOpenAIConfig().transform_request(
                    model=model,
                    messages=messages,
    ...
    async def acompletion(
        ...
        mishikallm_params: Optional[dict] = {},
    ):
        response = None
        try:
    ...
    def embedding(
        ...
        try:
            ...
            if max_retries is None:
                max_retries = mishikallm.DEFAULT_MAX_RETRIES
            ## LOGGING
            logging_obj.pre_call(
                input=input,
                api_key=api_key,
    ...
    def make_sync_azure_httpx_request(
        ...
        if client is None:
            ...
            sync_handler = HTTPHandler(**_params, client=mishikallm.client_session)  # type: ignore
        else:
    ...
    def get_headers(
        ...
        client_session = mishikallm.client_session or httpx.Client()
        if "gateway.ai.cloudflare.com" in api_base:
