...
from mishikallm.types.llms.azure import (
            ...
            if param == "tool_choice":
                ...
                if (
                    ...
                    if mishikallm.drop_params is True or (
                        ...
                        raise UnsupportedParamsError(
                            status_code=400,
                            message=f"Azure does not support '{value}' as a {param} param, for api_version={api_version}. To drop 'tool_choice=required' for calls with this Azure API version, set `mishikallm.drop_params=True` or for proxy:\n\n`mishikallm_settings:\n drop_params: true`\nAzure API Reference: https://learn.microsoft.com/en-us/azure/ai-services/openai/reference#chat-completions",
                        )
                else:
