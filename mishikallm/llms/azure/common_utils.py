...
from mishikallm.llms.openai.common_utils import BaseOpenAILLM
from mishikallm.secret_managers.get_azure_ad_token_provider import (
...
def get_azure_ad_token_from_oidc(
    ...
    client = mishikallm.module_level_client
...
class BaseAzureLLM(BaseOpenAILLM):
    ...
    def initialize_azure_sdk_client(
        ...
        azure_ad_token = mishikallm_params.get("azure_ad_token")
        tenant_id = mishikallm_params.get("tenant_id", os.getenv("AZURE_TENANT_ID"))
        client_id = mishikallm_params.get("client_id", os.getenv("AZURE_CLIENT_ID"))
        client_secret = mishikallm_params.get(
            "client_secret", os.getenv("AZURE_CLIENT_SECRET")
        )
        azure_username = mishikallm_params.get(
            "azure_username", os.getenv("AZURE_USERNAME")
        )
        azure_password = mishikallm_params.get(
            "azure_password", os.getenv("AZURE_PASSWORD")
        )
        max_retries = mishikallm_params.get("max_retries")
        timeout = mishikallm_params.get("timeout")
        ...
        if azure_ad_token is not None and azure_ad_token.startswith("oidc/"):
            ...
            and mishikallm.enable_azure_ad_token_refresh is True
        ):
            verbose_logger.debug(
        ...
        if api_version is None:
            api_version = os.getenv(
                "AZURE_API_VERSION", mishikallm.AZURE_DEFAULT_API_VERSION
            )
