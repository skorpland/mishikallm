...
from mishikallm.types.utils import CompletionTokensDetailsWrapper
from mishikallm.types.utils import Message as MishikallmMessage
from mishikallm.types.utils import PromptTokensDetailsWrapper
...
if TYPE_CHECKING:
    ...
    LoggingClass = MishikaLLMLoggingObj
else:
    LoggingClass = Any
...
class AnthropicConfig(AnthropicModelInfo, BaseConfig):
    ...
    def _map_stop_sequences(
        ...
        if isinstance(stop, str):
            if (
                stop.isspace() and mishikallm.drop_params is True
            ...
            for v in stop:
                if (
                    v.isspace() and mishikallm.drop_params is True
                ):  # anthropic doesn't allow whitespace characters as stop-sequences
                    continue
                new_v.append(v)
            if len(new_v) > 0:
    ...
    def transform_request(
        ...
        if (
            ...
            if mishikallm.modify_params:
                ...
                raise mishikallm.UnsupportedParamsError(
                    message="Anthropic doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `mishikallm.modify_params = True` // `mishikallm_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
        ...
        ## Load Config
        config = mishikallm.AnthropicConfig.get_config()
        ...
        _mishikallm_metadata = mishikallm_params.get("metadata", None)
        if (
            _mishikallm_metadata
            and isinstance(_mishikallm_metadata, dict)
            and "user_id" in _mishikallm_metadata
        ):
            optional_params["metadata"] = {"user_id": _mishikallm_metadata["user_id"]}
    ...
    ) -> Optional[MishikallmMessage]:
        _message: Optional[MishikallmMessage] = None
        ...
        try:
            if json_mode_content_str is not None:
                ...
                if (
                    ...
                    _message = mishikallm.Message(content=json.dumps(values))
                    return _message
                else:
            ...
            return mishikallm.Message(content=json_mode_content_str)
        return None
