...
import mishikallm
from mishikallm.caching.caching import Cache, MishikaLLMCacheType
...
from mishikallm.llms.openai.openai import AllMessageValues
...
local_cache_obj = Cache(
    type=MishikaLLMCacheType.LOCAL
    ...
    async def async_check_and_create_cache(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            client = get_async_httpx_client(
                params={"timeout": timeout}, llm_provider=mishikallm.LlmProviders.VERTEX_AI
            )
        else:
            client = client
