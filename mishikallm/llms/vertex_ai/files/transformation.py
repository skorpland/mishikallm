...
from mishikallm.llms.base_llm.files.transformation import (
    BaseFilesConfig,
    MishikaLLMLoggingObj,
)
from mishikallm.llms.vertex_ai.common_utils import (
    _convert_vertex_datetime_to_openai_datetime,
)
from mishikallm.llms.vertex_ai.gemini.transformation import _transform_request_body
...
from mishikallm.types.llms.vertex_ai import GcsBucketResponse
from mishikallm.types.utils import ExtractedFileData, LlmProviders
...
class VertexAIFilesConfig(VertexBase, BaseFilesConfig):
    ...
    def validate_environment(
        ...
        if not api_key:
            api_key, _ = self.get_access_token(
                credentials=mishikallm_params.get("vertex_credentials"),
                project_id=mishikallm_params.get("vertex_project"),
            )
            if not api_key:
    ...
    def _get_gcs_object_name_from_batch_jsonl(
        ...
        named as: mishikallm-vertex-{model}-{uuid}
        """
        _model = openai_jsonl_content[0].get("body", {}).get("model", "")
        if "publishers/google/models" not in _model:
            _model = f"publishers/google/models/{_model}"
        object_name = f"mishikallm-vertex-files/{_model}/{uuid.uuid4()}"
        return object_name
    ...
    def get_complete_file_url(
        ...
        """
        bucket_name = mishikallm_params.get("bucket_name") or os.getenv("GCS_BUCKET_NAME")
        if not bucket_name:
