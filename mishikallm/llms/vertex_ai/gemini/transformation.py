...
from mishikallm.types.files import (
...
def _transform_request_body(
    ...
    # Check for any 'mishikallm_param_*' set during optional param mapping

    remove_keys = []
    for k, v in optional_params.items():
        if k.startswith("mishikallm_param_"):
            mishikallm_params.update({k: v})
    ...
    try:
        if custom_llm_provider == "gemini":
            content = mishikallm.GoogleAIStudioGeminiConfig()._transform_messages(
                messages=messages
            )
        else:
            content = mishikallm.VertexGeminiConfig()._transform_messages(
                messages=messages
            )
...
async def async_transform_request_body(
    ...
    extra_headers: Optional[dict],
    optional_params: dict,
    logging_obj: mishikallm.mishikallm_core_utils.mishikallm_logging.Logging,  # type: ignore
    custom_llm_provider: Literal["vertex_ai", "vertex_ai_beta", "gemini"],
    mishikallm_params: dict,
) -> RequestBody:
    from ..context_caching.vertex_ai_context_caching import ContextCachingEndpoints
