...
from mishikallm.types.llms.anthropic import AnthropicThinkingParam
...
from mishikallm.utils import CustomStreamWrapper, ModelResponse, supports_reasoning
...
class VertexGeminiConfig(VertexAIBaseConfig, BaseConfig):
    ...
    def map_tool_choice_values(
        ...
        if tool_choice == "none":
            ...
            raise mishikallm.utils.UnsupportedParamsError(
                message="VertexAI doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `mishikallm.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
        ...
        for tool in value:
            ...
            if tool.get("googleSearch", None) is not None:
                ...
                verbose_logger.warning(
                    "Invalid tool={}. Use `mishikallm.set_verbose` or `mishikallm --detailed_debug` to see raw request."
                )
    ...
    def map_openai_params(
        ...
        for param, value in non_default_params.items():
            if param == "temperature":
                ...
                optional_params["mishikallm_param_is_function_call"] = (
        ...
        if mishikallm.vertex_ai_safety_settings is not None:
            optional_params["safety_settings"] = mishikallm.vertex_ai_safety_settings
        return optional_params
    ...
    def _process_candidates(self, _candidates, model_response, mishikallm_params):
        ...
        for idx, candidate in enumerate(_candidates):
            ...
            if "parts" in candidate["content"]:
                ...
                functions, tools = self._transform_parts(
                    ...
                    is_function_call=mishikallm_params.get(
        ...
        try:
            ...
            if _candidates:
                (
                    ...
                    _candidates, model_response, mishikallm_params
                )
...
async def make_call(
    ...
    if client is None:
        client = get_async_httpx_client(
            llm_provider=mishikallm.LlmProviders.VERTEX_AI,
        )
...
class VertexLLM(VertexBase):
    ...
    async def async_completion(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            client = get_async_httpx_client(
                params=_async_client_params, llm_provider=mishikallm.LlmProviders.VERTEX_AI
            )
        else:
...
class ModelResponseIterator:
    ...
    def handle_accumulated_json_chunk(self, chunk: str) -> GenericStreamingChunk:
        chunk = mishikallm.CustomStreamWrapper._strip_sse_data_from_chunk(chunk) or ""
