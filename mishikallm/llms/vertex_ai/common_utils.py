...
import mishikallm
from mishikallm import supports_response_schema, supports_system_messages, verbose_logger
from mishikallm.constants import DEFAULT_MAX_RECURSE_DEPTH
...
from mishikallm.types.llms.vertex_ai import PartType, Schema
...
def get_supports_system_message(
    ...
    try:
        ...
        if mishikallm.VertexGeminiConfig._is_model_gemini_spec_model(model):
...
def _get_vertex_url(
    ...
    model = mishikallm.VertexGeminiConfig.get_model_for_vertex_ai_url(model=model)
    if mode == "chat":
