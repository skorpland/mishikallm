...
from ..mishikallm_core_utils.core_helpers import map_finish_reason
...
class MishikaLLMPydanticObjectBase(BaseModel):
    """
...
class MishikaLLMCommonStrings(Enum):
    redacted_by_mishikallm = "redacted by mishikallm. 'mishikallm.turn_off_message_logging=True'"
    llm_provider_not_provided = "Unmapped LLM provider for this endpoint. You passed model={model}, custom_llm_provider={custom_llm_provider}. Check supported provider and route: https://docs.21t.cc/docs/providers"
...
class ModelInfo(ModelInfoBase, total=False):
    """
    Model info for a given model, this is information found in mishikallm.model_prices_and_context_window.json
    """
...
class StreamingChoices(OpenAIObject):
    def __init__(
        ...
        **params,
        ...
        # https://github.com/skorpland/mishikallm/issues/8455
...
class LoggedMishikaLLMParams(TypedDict, total=False):
...
class StandardLoggingUserAPIKeyMetadata(TypedDict):
    user_api_key_hash: Optional[str]  # hash of the mishikallm virtual key used
...
class StandardLoggingMCPToolCall(TypedDict, total=False):
    ...
    (this is to render the logo on the logs page on mishikallm ui)
    """
...
class StandardLoggingHiddenParams(TypedDict):
    ...
    mishikallm_overhead_time_ms: Optional[float]
    additional_headers: Optional[StandardLoggingAdditionalHeaders]
    batch_models: Optional[List[str]]
    mishikallm_model_name: Optional[str]  # the model name sent to the provider by mishikallm
    usage_object: Optional[dict]
...
all_mishikallm_params = [
    ...
    "mishikallm_credential_name",
    "allowed_openai_params",
    "mishikallm_session_id",
...
class LlmProviders(str, Enum):
    ...
    GITHUB = "github"
    CUSTOM = "custom"
    MISHIKALLM_PROXY = "mishikallm_proxy"
...
class MishikaLLMLoggingBaseClass:
    """
...
class MISHIKALLM_IMAGE_VARIATION_PROVIDERS(Enum):
    """
...
class MishikaLLMRealtimeStreamLoggingObject(MishikaLLMPydanticObjectBase):
...
class SpecialEnums(Enum):
    LITELM_MANAGED_FILE_ID_PREFIX = "mishikallm_proxy"
    MISHIKALLM_MANAGED_FILE_COMPLETE_STR = "mishikallm_proxy:{};unified_id,{}"

    MISHIKALLM_MANAGED_RESPONSE_COMPLETE_STR = (
        "mishikallm:custom_llm_provider:{};model_id:{};response_id:{}"
    )
