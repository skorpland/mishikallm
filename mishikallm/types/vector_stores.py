...
class MishikaLLM_VectorStoreConfig(TypedDict, total=False):
    """Parameters for initializing a vector store on Mishikallm proxy config.yaml"""

    vector_store_name: str
    mishikallm_params: Optional[Dict[str, Any]]


class MishikaLLM_ManagedVectorStore(TypedDict, total=False):
    """MishikaLLM managed vector store object - this is is the object stored in the database"""
    ...
    # credential fields
    mishikallm_credential_name: Optional[str]


class MishikaLLM_ManagedVectorStoreListResponse(TypedDict, total=False):
    """Response format for listing vector stores"""

    object: Literal["list"]  # Always "list"
    data: List[MishikaLLM_ManagedVectorStore]
