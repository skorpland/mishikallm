...
from mishikallm.utils import ModelResponse
...
class BudgetManager:
    def __init__(
        ...
        self.api_base = api_base or "https://api.21t.cc"
    ...
    def projected_cost(self, model: str, messages: list, user: str):
        ...
        prompt_tokens = mishikallm.token_counter(model=model, text=text)
        prompt_cost, _ = mishikallm.cost_per_token(
    ...
    def update_cost(
        ...
        if model and input_text and output_text:
            ...
            cost = mishikallm.completion_cost(completion_response=completion_obj)
            model = completion_obj[
                "model"
            ]  # if this throws an error try, model = completion_obj['model']
        else:
            raise ValueError(
                "Either a chat completion object or the text response needs to be passed in. Learn more - https://docs.21t.cc/docs/budget_manager"
            )
