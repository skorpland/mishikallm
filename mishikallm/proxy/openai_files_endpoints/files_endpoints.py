...
import mishikallm
from mishikallm import CreateFileRequest, get_secret_str
...
from mishikallm.proxy.hooks.managed_files import _PROXY_MishikaLLMManagedFiles
...
async def route_create_file(
    ...
    if (
        mishikallm.enable_loadbalancing_on_batch_endpoints is True
        and is_router_model
        ...
        managed_files_obj = cast(
            Optional[_PROXY_MishikaLLMManagedFiles],
        ...
        response = await mishikallm.acreate_file(**_create_file_request, custom_llm_provider=custom_llm_provider)  # type: ignore
...
async def create_file(
    ...
    try:
        ...
        if mishikallm.enable_loadbalancing_on_batch_endpoints is True:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.create_file(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def get_file_content(
    ...
    try:
        ...
        ## check if file_id is a mishikallm managed file
        is_base64_unified_file_id = (
            _PROXY_MishikaLLMManagedFiles._is_base64_encoded_unified_file_id(file_id)
        )
        if is_base64_unified_file_id:
            ...
            response = await mishikallm.afile_content(
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.retrieve_file_content(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def get_file(
    ...
    try:
        ...
        if is_base64_unified_file_id:
            ...
            response = await mishikallm.afile_retrieve(
                custom_llm_provider=custom_llm_provider, file_id=file_id, **data  # type: ignore
            )
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.retrieve_file(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def delete_file(
    ...
    try:
        ...
        if is_base64_unified_file_id:
            ...
            response = await mishikallm.afile_delete(
                custom_llm_provider=custom_llm_provider, file_id=file_id, **data  # type: ignore
            )
...
async def list_files(
    ...
    try:
        ...
        response = await mishikallm.afile_list(
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.list_files(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
