...
from mishikallm.constants import MAX_TEAM_LIST_LIMIT
...
from mishikallm._service_logger import ServiceLogging, ServiceTypes
...
from mishikallm.integrations.SlackAlerting.utils import _add_langfuse_trace_id_to_alert
from mishikallm.mishikallm_core_utils.mishikallm_logging import Logging
from mishikallm.llms.custom_httpx.httpx_handler import HTTPHandler
from mishikallm.proxy._types import (
    AlertType,
    CallInfo,
...
from mishikallm.proxy.db.create_views import (
    create_missing_views,
    should_create_missing_views,
)
from mishikallm.proxy.db.db_spend_update_writer import DBSpendUpdateWriter
from mishikallm.proxy.db.log_db_metrics import log_db_metrics
from mishikallm.proxy.db.prisma_client import PrismaWrapper
from mishikallm.proxy.hooks import PROXY_HOOKS, get_proxy_hook
from mishikallm.proxy.hooks.cache_control_check import _PROXY_CacheControlCheck
from mishikallm.proxy.hooks.max_budget_limiter import _PROXY_MaxBudgetLimiter
from mishikallm.proxy.hooks.parallel_request_limiter import (
    _PROXY_MaxParallelRequestsHandler,
)
from mishikallm.proxy.mishikallm_pre_call_utils import MishikaLLMProxyRequestSetup
from mishikallm.secret_managers.main import str_to_bool
from mishikallm.types.integrations.slack_alerting import DEFAULT_ALERT_TYPES
from mishikallm.types.utils import CallTypes, LLMResponseTypes, LoggedMishikaLLMParams
...
def print_verbose(print_statement):
    """
    Prints the given `print_statement` to the console if `mishikallm.set_verbose` is True.
    ...
    if mishikallm.set_verbose:
        print(f"MishikaLLM Proxy: {print_statement}")  # noqa
...
def safe_deep_copy(data):
    ...
    The MishikaLLM Request has some object that can-not be pickled / deep copied

    Use this function to safely deep copy the MishikaLLM Request
    """
    if mishikallm.safe_memory_mode is True:
        return data

    mishikallm_parent_otel_span: Optional[Any] = None
    # Step 1: Remove the mishikallm_parent_otel_span
    mishikallm_parent_otel_span = None
    if isinstance(data, dict):
        # remove mishikallm_parent_otel_span since this is not picklable
        if "metadata" in data and "mishikallm_parent_otel_span" in data["metadata"]:
            mishikallm_parent_otel_span = data["metadata"].pop("mishikallm_parent_otel_span")
    new_data = copy.deepcopy(data)

    # Step 2: re-add the mishikallm_parent_otel_span after doing a deep copy
    if isinstance(data, dict) and mishikallm_parent_otel_span is not None:
        if "metadata" in data:
            data["metadata"]["mishikallm_parent_otel_span"] = mishikallm_parent_otel_span
    return new_data
...
class InternalUsageCache:
    ...
    async def async_get_cache(
        self,
        key,
        mishikallm_parent_otel_span: Union[Span, None],
        ...
        **kwargs,
        ...
        return await self.dual_cache.async_get_cache(
            ...
            parent_otel_span=mishikallm_parent_otel_span,
            **kwargs,
        )
...
class ProxyLogging:
    ...
    def __init__(
        ...
        ## INITIALIZE  MISHIKALLM CALLBACKS ##
    ...
    def startup_event(
        ...
        self._init_mishikallm_callbacks(
            llm_router=llm_router
        )  # INITIALIZE MISHIKALLM CALLBACKS ON SERVER STARTUP <- do this to catch any logging errors on startup, not when calls are being made
        ...
        if updated_slack_alerting is True:
            ...
            if self.alerting is not None and "slack" in self.alerting:
                ...
                if "daily_reports" in self.alert_types:
                    mishikallm.logging_callback_manager.add_mishikallm_callback(self.slack_alerting_instance)  # type: ignore
    ...
    def _add_proxy_hooks(self, llm_router: Optional[Router] = None):
        """
        Add proxy hooks to mishikallm.callbacks
        """
        ...
        for hook in PROXY_HOOKS:
            ...
            mishikallm.logging_callback_manager.add_mishikallm_callback(proxy_hook_obj)
    ...
    def _init_mishikallm_callbacks(self, llm_router: Optional[Router] = None):
        self._add_proxy_hooks(llm_router)
        mishikallm.logging_callback_manager.add_mishikallm_callback(self.service_logging_obj)  # type: ignore
        for callback in mishikallm.callbacks:
            ...
            if callback not in mishikallm.service_callback:
                mishikallm.service_callback.append(callback)  # type: ignore
        ...
        if (
            ...
            callback_list = list(
                set(
                    mishikallm.input_callback
                    + mishikallm.success_callback
                    + mishikallm.failure_callback
                )
            )
            mishikallm.mishikallm_core_utils.mishikallm_logging.set_callbacks(
                callback_list=callback_list
            )

    async def update_request_status(
        self, mishikallm_call_id: str, status: Literal["success", "fail"]
    ):
        ...
        await self.internal_usage_cache.async_set_cache(
            key="request_status:{}".format(mishikallm_call_id),
            value=status,
            local_only=True,
    ...
    async def pre_call_hook(
        ...
        try:
            for callback in mishikallm.callbacks:
                _callback = None
                if isinstance(callback, str):
                    _callback = mishikallm.mishikallm_core_utils.mishikallm_logging.get_custom_logger_compatible_class(
                        callback
                    )
                else:
    ...
    async def alerting_handler(
        ...
        for client in self.alerting:
            if client == "slack":
                ...
                if mishikallm.utils.sentry_sdk_instance is not None:
                    mishikallm.utils.sentry_sdk_instance.capture_message(formatted_message)
                else:
    ...
    async def failure_handler(
        ...
        if mishikallm.utils.capture_exception:
            mishikallm.utils.capture_exception(error=original_exception)
    ...
    async def post_call_failure_hook(
        ...
        await self.update_request_status(
            mishikallm_call_id=request_data.get("mishikallm_call_id", ""), status="fail"
        )
    ...
    async def _handle_logging_proxy_only_error(
        ...
        """
        Handle logging for proxy only errors by calling `mishikallm_logging_obj.async_failure_handler`

        Is triggered when self._is_proxy_only_error() returns True
        """
        mishikallm_logging_obj: Optional[Logging] = request_data.get(
            "mishikallm_logging_obj", None
        )
        if mishikallm_logging_obj is None:
            import uuid

            request_data["mishikallm_call_id"] = str(uuid.uuid4())
            user_api_key_logged_metadata = (
                MishikaLLMProxyRequestSetup.get_sanitized_user_information_from_key(
                    user_api_key_dict=user_api_key_dict
                )
            )

            mishikallm_logging_obj, data = mishikallm.utils.function_setup(
                original_function=route or "IGNORE_THIS",
                rules_obj=mishikallm.utils.Rules(),
        ...
        if mishikallm_logging_obj is not None:
            ...
            mishikallm_param_keys = LoggedMishikaLLMParams.__annotations__.keys()
            for k, v in request_data.items():
                if k in mishikallm_param_keys:
                    _mishikallm_params[k] = v
            ...
            if "messages" in request_data and isinstance(
                ...
                mishikallm_logging_obj.model_call_details["messages"] = input
                mishikallm_logging_obj.call_type = CallTypes.acompletion.value
            elif "prompt" in request_data and isinstance(request_data["prompt"], str):
                input = request_data["prompt"]
                mishikallm_logging_obj.model_call_details["prompt"] = input
                mishikallm_logging_obj.call_type = CallTypes.atext_completion.value
            elif "input" in request_data and isinstance(request_data["input"], list):
                input = request_data["input"]
                mishikallm_logging_obj.model_call_details["input"] = input
                mishikallm_logging_obj.call_type = CallTypes.aembedding.value
            mishikallm_logging_obj.pre_call(
                input=input,
                api_key="",
            )

            # log the custom exception
            await mishikallm_logging_obj.async_failure_handler(
                exception=original_exception,
                traceback_exception=traceback.format_exc(),
            )

            threading.Thread(
                target=mishikallm_logging_obj.failure_handler,
                args=(
                    original_exception,
    ...
    async def async_post_call_streaming_hook(
        ...
        if isinstance(response, (ModelResponse, ModelResponseStream)):
            response_str = mishikallm.get_response_string(response_obj=response)
...
class PrismaClient:
    ...
    async def check_view_exists(self):
        """
        Checks if the MishikaLLM_VerificationTokenView and MonthlyGlobalSpend exists in the user's db.

        MishikaLLM_VerificationTokenView: This view is used for getting the token + team data in user_api_key_auth
        ...
        try:
            ...
            required_view = "MishikaLLM_VerificationTokenView"
            ...
            if ret[0]["view_count"] == expected_total_views:
                ...
                if ret[0]["view_names"] and required_view not in ret[0]["view_names"]:
                    ...
                    await self.db.execute_raw(
                        """
                            CREATE VIEW "MishikaLLM_VerificationTokenView" AS
                            SELECT
                            v.*,
                            ...
                            FROM "MishikaLLM_VerificationToken" v
                            LEFT JOIN "MishikaLLM_TeamTable" t ON v.team_id = t.team_id;
                        """
                    )

                    verbose_proxy_logger.info(
                        "MishikaLLM_VerificationTokenView Created in DB!"
                    )
                else:
    ...
    async def get_generic_data(
        ...
        try:
            if table_name == "users":
                response = await self.db.mishikallm_usertable.find_first(
                    where={key: value}  # type: ignore
                )
            elif table_name == "keys":
                response = await self.db.mishikallm_verificationtoken.find_first(  # type: ignore
                    where={key: value}  # type: ignore
                )
            elif table_name == "config":
                response = await self.db.mishikallm_config.find_first(  # type: ignore
                    where={key: value}  # type: ignore
                )
            ...
            error_msg = f"MishikaLLM Prisma Client Exception get_generic_data: {str(e)}"
        ...
        try:
            ...
            if (token is not None and table_name is None) or (
                ...
                if query_type == "find_unique" and hashed_token is not None:
                    ...
                    response = await self.db.mishikallm_verificationtoken.find_unique(
                        where={"token": hashed_token},  # type: ignore
                        include={"mishikallm_budget_table": True},
                    )
                    if response is not None:
                    ...
                    response = await self.db.mishikallm_verificationtoken.find_many(
                ...
                if query_type == "find_unique":
                    ...
                    response = await self.db.mishikallm_usertable.find_unique(  # type: ignore
                        where=key_val,  # type: ignore
                        include={"organization_memberships": True},
                    )
                elif query_type == "find_all" and key_val is not None:
                    response = await self.db.mishikallm_usertable.find_many(
                        where=key_val  # type: ignore
                    )  # type: ignore
                    ...
                    if expires is not None:
                        response = await self.db.mishikallm_usertable.find_many(  # type: ignore
                            order={"spend": "desc"},
                            where={  # type:ignore
                                "OR": [
                        ...
                        FROM
                            "MishikaLLM_UserTable" u
                        LEFT JOIN "MishikaLLM_VerificationToken" v ON u.user_id = v.user_id
                        GROUP BY
                            u.user_id
                ...
                if key_val is not None:
                    if query_type == "find_unique":
                        response = await self.db.mishikallm_spendlogs.find_unique(  # type: ignore
                            where={  # type: ignore
                                key_val["key"]: key_val["value"],  # type: ignore
                            }
                        )
                    elif query_type == "find_all":
                        response = await self.db.mishikallm_spendlogs.find_many(  # type: ignore
                            where={
                                key_val["key"]: key_val["value"],  # type: ignore
                            }
                        )
                ...
                if query_type == "find_unique":
                    response = await self.db.mishikallm_teamtable.find_unique(
                        where={"team_id": team_id},  # type: ignore
                        include={"mishikallm_model_table": True},  # type: ignore
                    )
                elif query_type == "find_all" and reset_at is not None:
                    response = await self.db.mishikallm_teamtable.find_many(
                        where={  # type:ignore
                            "budget_reset_at": {"lt": reset_at},
                        }
                    )
                ...
                if query_type == "find_unique":
                    response = await self.db.mishikallm_usernotifications.find_unique(  # type: ignore
                        where={"user_id": user_id}  # type: ignore
                    )
                elif query_type == "find_all":
                    response = await self.db.mishikallm_usernotifications.find_many()  # type: ignore
                return response
                ...
                if query_type == "find_unique":
                    ...
                    sql_query = f"""
                        SELECT 
                            ...
                            b.max_budget AS mishikallm_budget_table_max_budget,
                            b.tpm_limit AS mishikallm_budget_table_tpm_limit,
                            b.rpm_limit AS mishikallm_budget_table_rpm_limit,
                            b.model_max_budget as mishikallm_budget_table_model_max_budget,
                            b.soft_budget as mishikallm_budget_table_soft_budget
                        FROM "MishikaLLM_VerificationToken" AS v
                        LEFT JOIN "MishikaLLM_TeamTable" AS t ON v.team_id = t.team_id
                        LEFT JOIN "MishikaLLM_TeamMembership" AS tm ON v.team_id = tm.team_id AND tm.user_id = v.user_id
                        LEFT JOIN "MishikaLLM_ModelTable" m ON t.model_id = m.id
                        LEFT JOIN "MishikaLLM_BudgetTable" AS b ON v.budget_id = b.budget_id
                        WHERE v.token = '{token}'
                    """
                    ...
                    if response is not None:
                        ...
                        response = MishikaLLM_VerificationTokenView(
            ...
            prisma_query_info = f"MishikaLLM Prisma Client Exception: Error with `get_data`. Args passed in: {args_passed_in}"
    ...
    async def insert_data(  # noqa: PLR0915
        ...
        try:
            ...
            if table_name == "key":
                ...
                print_verbose(
                    "PrismaClient: Before upsert into mishikallm_verificationtoken"
                )
                new_verification_token = await self.db.mishikallm_verificationtoken.upsert(  # type: ignore
                    where={
                        "token": hashed_token,
                    },
                    data={
                ...
                db_data = self.jsonify_object(data=data)
                try:
                    new_user_row = await self.db.mishikallm_usertable.upsert(
                    ...
                    if (
                        "Foreign key constraint failed on the field: `MishikaLLM_UserTable_organization_id_fkey (index)`"
                        in str(e)
                    ):
                        raise HTTPException(
                            status_code=400,
                            detail={
                                "error": f"Foreign Key Constraint failed. Organization ID={db_data['organization_id']} does not exist in MishikaLLM_OrganizationTable. Create via `/organization/new`."
                            },
                        )
                    raise e
                ...
                new_team_row = await self.db.mishikallm_teamtable.upsert(
                ...
                for k, v in data.items():
                    ...
                    updated_table_row = self.db.mishikallm_config.upsert(
                ...
                db_data = self.jsonify_object(data=data)
                new_spend_row = await self.db.mishikallm_spendlogs.upsert(
                ...
                new_user_notification_row = (
                    await self.db.mishikallm_usernotifications.upsert(  # type: ignore
                        where={"request_id": data["request_id"]},
                        data={
                            "create": {**db_data},  # type: ignore
                            "update": {},  # don't do anything if it already exists
            ...
            error_msg = f"MishikaLLM Prisma Client Exception in insert_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def update_data(  # noqa: PLR0915
        ...
        try:
            ...
            if token is not None:
                ...
                token = _hash_token_if_needed(token=token)
                db_data["token"] = token
                response = await self.db.mishikallm_verificationtoken.update(
                ...
                update_user_row = await self.db.mishikallm_usertable.upsert(
                    where={"user_id": user_id},  # type: ignore
                    data={
                        "create": {**db_data},  # type: ignore
                        "update": {
                ...
                update_team_row = await self.db.mishikallm_teamtable.upsert(
                    where={"team_id": team_id},  # type: ignore
                    data={
                        "create": {**db_data},  # type: ignore
                        "update": {
                ...
                for idx, t in enumerate(data_list):
                    ...
                    batcher.mishikallm_verificationtoken.update(
                ...
                for idx, user in enumerate(data_list):
                    ...
                    batcher.mishikallm_usertable.upsert(
                ...
                for idx, team in enumerate(data_list):
                    ...
                    batcher.mishikallm_teamtable.upsert(
            ...
            error_msg = f"MishikaLLM Prisma Client Exception - update_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def delete_data(
        ...
        try:
            if tokens is not None and isinstance(tokens, List):
                ...
                deleted_tokens = await self.db.mishikallm_verificationtoken.delete_many(
                ...
                await self.db.mishikallm_teamtable.delete_many(
                    where={"team_id": {"in": team_id_list}}
                )
                ...
                # admin only endpoint -> `/team/delete`
                await self.db.mishikallm_verificationtoken.delete_many(
                    where={"team_id": {"in": team_id_list}}
                )
        except Exception as e:
            import traceback

            error_msg = f"MishikaLLM Prisma Client Exception - delete_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def connect(self):
        ...
        try:
            ...
            error_msg = f"MishikaLLM Prisma Client Exception connect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def disconnect(self):
        ...
        try:
            ...
            error_msg = f"MishikaLLM Prisma Client Exception disconnect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
                ...
                async with prisma_client.db.tx(
                    ...
                    async with transaction.batch_() as batcher:
                        for (
                            ...
                            if mishikallm.max_end_user_budget is not None:
                                pass
                            batcher.mishikallm_endusertable.upsert(
                ...
                try:
                    ...
                    if (
                        ...
                        for j in range(0, len(logs_to_process), BATCH_SIZE):
                            ...
                            await prisma_client.db.mishikallm_spendlogs.create_many(
...
def _raise_failed_update_spend_exception(
    ...
    error_msg = (
        f"[Non-Blocking]MishikaLLM Prisma Client Exception - update spend logs: {str(e)}"
    )
    error_traceback = error_msg + "\n" + traceback.format_exc()
    end_time = time.time()
    _duration = end_time - start_time
...
def _premium_user_check():
    ...
    """
    from mishikallm.proxy.proxy_server import premium_user

    if not premium_user:
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"This feature is only available for MishikaLLM Enterprise users. {CommonProxyErrors.not_premium_user.value}"
            },
        )
