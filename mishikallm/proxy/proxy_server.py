...
if TYPE_CHECKING:
    ...
    from mishikallm.integrations.opentelemetry import OpenTelemetry
...
sys.path.insert(
    0, os.path.abspath("../..")
)  # Adds the parent directory to the system path - for mishikallm local dev
...
try:
    ...
    raise ImportError(f"Missing dependency {e}. Run `pip install 'mishikallm[proxy]'`")
...
def generate_feedback_box():
    ...
    print("\033[1;37m" + "#" + "-" * box_width + "#\033[0m")  # noqa
    print()  # noqa
    print(" Thank you for using MishikaLLM! - Krrish & Ishaan")  # noqa
    print()  # noqa
    print()  # noqa
    print()  # noqa
    print(  # noqa
...
from mishikallm._logging import verbose_proxy_logger, verbose_router_logger
from mishikallm.caching.caching import DualCache, RedisCache
from mishikallm.constants import (
    DAYS_IN_A_MONTH,
    DEFAULT_HEALTH_CHECK_INTERVAL,
    DEFAULT_MODEL_CREATED_AT_TIME,
    MISHIKALLM_PROXY_ADMIN_NAME,
    PROMETHEUS_FALLBACK_STATS_SEND_TIME_HOURS,
    PROXY_BATCH_WRITE_AT,
    PROXY_BUDGET_RESCHEDULER_MAX_TIME,
    PROXY_BUDGET_RESCHEDULER_MIN_TIME,
)
from mishikallm.exceptions import RejectedRequestError
from mishikallm.integrations.SlackAlerting.slack_alerting import SlackAlerting
from mishikallm.mishikallm_core_utils.core_helpers import (
    _get_parent_otel_span_from_kwargs,
    get_mishikallm_metadata_from_kwargs,
)
...
from mishikallm.proxy._experimental.mcp_server.server import router as mcp_router
from mishikallm.proxy._experimental.mcp_server.tool_registry import (
    global_mcp_tool_registry,
)
from mishikallm.proxy._types import *
from mishikallm.proxy.analytics_endpoints.analytics_endpoints import (
    router as analytics_router,
)
from mishikallm.proxy.anthropic_endpoints.endpoints import router as anthropic_router
from mishikallm.proxy.auth.auth_checks import (
    ExperimentalUIJWTToken,
    get_team_object,
    log_db_metrics,
)
from mishikallm.proxy.auth.auth_utils import check_response_size_is_safe
from mishikallm.proxy.auth.handle_jwt import JWTHandler
from mishikallm.proxy.auth.mishikallm_license import LicenseCheck
from mishikallm.proxy.auth.model_checks import (
    get_complete_model_list,
    get_key_models,
    get_team_models,
)
from mishikallm.proxy.auth.user_api_key_auth import (
    user_api_key_auth,
    user_api_key_auth_websocket,
)
from mishikallm.proxy.batches_endpoints.endpoints import router as batches_router

## Import All Misc routes here ##
from mishikallm.proxy.caching_routes import router as caching_router
...
from mishikallm.proxy.common_utils.debug_utils import init_verbose_loggers
from mishikallm.proxy.common_utils.debug_utils import router as debugging_endpoints_router
from mishikallm.proxy.common_utils.encrypt_decrypt_utils import (
    decrypt_value_helper,
    encrypt_value_helper,
)
from mishikallm.proxy.common_utils.html_forms.ui_login import html_form
from mishikallm.proxy.common_utils.http_parsing_utils import (
    _read_request_body,
    check_file_size_under_limit,
    get_form_data,
)
from mishikallm.proxy.common_utils.load_config_utils import (
    get_config_file_contents_from_gcs,
    get_file_contents_from_s3,
)
from mishikallm.proxy.common_utils.openai_endpoint_utils import (
    remove_sensitive_info_from_deployment,
)
from mishikallm.proxy.common_utils.proxy_state import ProxyState
from mishikallm.proxy.common_utils.reset_budget_job import ResetBudgetJob
from mishikallm.proxy.common_utils.swagger_utils import ERROR_RESPONSES
from mishikallm.proxy.credential_endpoints.endpoints import router as credential_router
from mishikallm.proxy.db.exception_handler import PrismaDBExceptionHandler
from mishikallm.proxy.fine_tuning_endpoints.endpoints import router as fine_tuning_router
from mishikallm.proxy.fine_tuning_endpoints.endpoints import set_fine_tuning_config
from mishikallm.proxy.guardrails.guardrail_endpoints import router as guardrails_router
from mishikallm.proxy.guardrails.init_guardrails import (
    init_guardrails_v2,
    initialize_guardrails,
)
from mishikallm.proxy.health_check import perform_health_check
from mishikallm.proxy.health_endpoints._health_endpoints import router as health_router
from mishikallm.proxy.hooks.model_max_budget_limiter import (
    _PROXY_VirtualKeyModelMaxBudgetLimiter,
)
from mishikallm.proxy.hooks.prompt_injection_detection import (
    _OPTIONAL_PromptInjectionDetection,
)
from mishikallm.proxy.hooks.proxy_track_cost_callback import _ProxyDBLogger
from mishikallm.proxy.mishikallm_pre_call_utils import add_mishikallm_data_to_request
from mishikallm.proxy.management_endpoints.budget_management_endpoints import (
    router as budget_management_router,
)
from mishikallm.proxy.management_endpoints.common_utils import _user_has_admin_view
from mishikallm.proxy.management_endpoints.customer_endpoints import (
    router as customer_router,
)
from mishikallm.proxy.management_endpoints.internal_user_endpoints import (
    router as internal_user_router,
)
from mishikallm.proxy.management_endpoints.internal_user_endpoints import user_update
...
from mishikallm.proxy.management_endpoints.model_management_endpoints import (
    ...
    _deduplicate_mishikallm_router_models,
)
from mishikallm.proxy.management_endpoints.model_management_endpoints import (
    router as model_management_router,
)
from mishikallm.proxy.management_endpoints.organization_endpoints import (
    router as organization_router,
)
from mishikallm.proxy.management_endpoints.scim.scim_v2 import scim_router
from mishikallm.proxy.management_endpoints.tag_management_endpoints import (
    router as tag_management_router,
)
from mishikallm.proxy.management_endpoints.team_callback_endpoints import (
    router as team_callback_router,
)
from mishikallm.proxy.management_endpoints.team_endpoints import router as team_router
from mishikallm.proxy.management_endpoints.team_endpoints import (
    update_team,
    validate_membership,
)
from mishikallm.proxy.management_endpoints.ui_sso import (
    get_disabled_non_admin_personal_key_creation,
)
from mishikallm.proxy.management_endpoints.ui_sso import router as ui_sso_router
from mishikallm.proxy.management_helpers.audit_logs import create_audit_log_for_update
from mishikallm.proxy.middleware.prometheus_auth_middleware import PrometheusAuthMiddleware
from mishikallm.proxy.openai_files_endpoints.files_endpoints import (
    router as openai_files_router,
)
from mishikallm.proxy.openai_files_endpoints.files_endpoints import set_files_config
from mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
    passthrough_endpoint_router,
)
from mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
    router as llm_passthrough_router,
)
from mishikallm.proxy.pass_through_endpoints.pass_through_endpoints import (
    initialize_pass_through_endpoints,
)
from mishikallm.proxy.pass_through_endpoints.pass_through_endpoints import (
    router as pass_through_router,
)
from mishikallm.proxy.rerank_endpoints.endpoints import router as rerank_router
from mishikallm.proxy.response_api_endpoints.endpoints import router as response_router
from mishikallm.proxy.route_llm_request import route_request
from mishikallm.proxy.spend_tracking.spend_management_endpoints import (
    router as spend_management_router,
)
from mishikallm.proxy.spend_tracking.spend_tracking_utils import get_logging_payload
from mishikallm.proxy.types_utils.utils import get_instance_fn
from mishikallm.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
    router as ui_crud_endpoints_router,
)
from mishikallm.proxy.utils import (
    PrismaClient,
    ProxyLogging,
    ProxyUpdateSpend,
    _cache_user_row,
...
from mishikallm.proxy.vertex_ai_endpoints.langfuse_endpoints import (
...
from mishikallm.scheduler import FlowItem, Scheduler
from mishikallm.secret_managers.aws_secret_manager import load_aws_kms
from mishikallm.secret_managers.google_kms import load_google_kms
from mishikallm.secret_managers.main import (
    get_secret,
    get_secret_bool,
    get_secret_str,
    str_to_bool,
)
from mishikallm.types.integrations.slack_alerting import SlackAlertingArgs
...
from mishikallm.types.router import DeploymentTypedDict
from mishikallm.types.router import ModelInfo as RouterModelInfo
from mishikallm.types.router import RouterGeneralSettings, updateDeployment
from mishikallm.types.scheduler import DefaultPriorities
from mishikallm.types.utils import CredentialItem, CustomHuggingfaceTokenizer
from mishikallm.types.utils import ModelInfo as ModelMapInfo
from mishikallm.types.utils import RawRequestTypedDict, StandardLoggingPayload
from mishikallm.utils import _add_custom_logger_callback_to_specific_event
...
try:
    # when using mishikallm cli
    import mishikallm.proxy.enterprise as enterprise
    from enterprise.proxy.enterprise_routes import router as enterprise_router
except Exception:
    # when using mishikallm docker image
    try:
...
global_max_parallel_request_retries_env: Optional[str] = os.getenv(
    "MISHIKALLM_GLOBAL_MAX_PARALLEL_REQUEST_RETRIES"
)
...
global_max_parallel_request_retry_timeout_env: Optional[str] = os.getenv(
    "MISHIKALLM_GLOBAL_MAX_PARALLEL_REQUEST_RETRY_TIMEOUT"
)
...
ui_message = (
    f"👉 [```MishikaLLM Admin Panel on /ui```]({ui_link}). Create, Edit Keys with SSO"
)
ui_message += "\n\n💸 [```MishikaLLM Model Cost Map```](https://models.21t.cc/)."

custom_swagger_message = "[**Customize Swagger Docs**](https://docs.21t.cc/docs/proxy/enterprise#swagger-docs---custom-routes--branding)"

### CUSTOM BRANDING [ENTERPRISE FEATURE] ###
_title = os.getenv("DOCS_TITLE", "MishikaLLM API") if premium_user else "MishikaLLM API"
_description = (
    os.getenv(
        "DOCS_DESCRIPTION",
...
async def proxy_shutdown_event():
    ...
    verbose_proxy_logger.info("Shutting down MishikaLLM Proxy Server")
    if prisma_client:
        verbose_proxy_logger.debug("Disconnecting from Prisma")
        await prisma_client.disconnect()

    if mishikallm.cache is not None:
        await mishikallm.cache.disconnect()
    ...
    if "langfuse" in mishikallm.success_callback:
        try:
            # flush langfuse logs on shutdow
            from mishikallm.utils import langFuseLogger
...
async def proxy_startup_event(app: FastAPI):
    global prisma_client, master_key, use_background_health_checks, llm_router, llm_model_list, general_settings, proxy_budget_rescheduler_min_time, proxy_budget_rescheduler_max_time, mishikallm_proxy_admin_name, db_writer_client, store_model_in_db, premium_user, _license_check
    import json

    init_verbose_loggers()
    ## CHECK PREMIUM USER
    verbose_proxy_logger.debug(
        "mishikallm.proxy.proxy_server.py::startup() - CHECKING PREMIUM USER - {}".format(
            premium_user
        )
    )
    ...
    master_key = get_secret_str("MISHIKALLM_MASTER_KEY")
    ### LOAD CONFIG ###
    ...
    if env_config_yaml is not None:
        ...
        elif os.environ.get("MISHIKALLM_CONFIG_BUCKET_NAME") is not None and isinstance(
            worker_config, str
        ):
            (
                llm_router,
    ...
    if prisma_client is not None and mishikallm.max_budget > 0:
        ProxyStartupEvent._add_proxy_budget_to_db(
            mishikallm_proxy_budget_name=mishikallm_proxy_admin_name
        )
...
def custom_openapi():
    ...
    openai_routes = MishikaLLMRoutes.openai_routes.value
...
mishikallm.logging_callback_manager.add_mishikallm_callback(model_max_budget_limiter)
...
queue: List = []
mishikallm_proxy_budget_name = "mishikallm-proxy-budget"
mishikallm_proxy_admin_name = MISHIKALLM_PROXY_ADMIN_NAME
ui_access_mode: Literal["admin", "all"] = "all"
proxy_budget_rescheduler_min_time = PROXY_BUDGET_RESCHEDULER_MIN_TIME
proxy_budget_rescheduler_max_time = PROXY_BUDGET_RESCHEDULER_MAX_TIME
proxy_batch_write_at = PROXY_BATCH_WRITE_AT
mishikallm_master_key_hash = None
...
async def check_request_disconnection(request: Request, llm_api_call_task):
    ...
    - cancel the mishikallm.router task
...
def load_from_azure_key_vault(use_azure_key_vault: bool = False):
    ...
    try:
        ...
        mishikallm._key_management_system = KeyManagementSystem.AZURE_KEY_VAULT
...
def cost_tracking():
    ...
    if prisma_client is not None:
        mishikallm.logging_callback_manager.add_mishikallm_callback(_ProxyDBLogger())
...
async def update_cache(  # noqa: PLR0915
    ...
    async def _update_key_cache(token: str, response_cost: float):
        ...
        existing_spend_obj: MishikaLLM_VerificationTokenView = await user_api_key_cache.async_get_cache(key=hashed_token)  # type: ignore
        verbose_proxy_logger.debug(
        ...
        if (
            ...
            and existing_spend_obj.mishikallm_budget_table is not None
            and (
                _is_projected_spend_over_limit(
                    current_spend=new_spend,
                    soft_budget_limit=existing_spend_obj.mishikallm_budget_table[
                        "soft_budget"
                    ],
                )
                is True
            ...
            projected_spend, projected_exceeded_date = _get_projected_spend_over_limit(
                current_spend=new_spend,
                soft_budget_limit=existing_spend_obj.mishikallm_budget_table.get(
                    "soft_budget", None
                ),
            )  # type: ignore
            soft_limit = existing_spend_obj.mishikallm_budget_table.get(
            ...
            call_info = CallInfo(
                ...
                event_group=Mishikallm_EntityType.KEY,
            )
            # alert user
            asyncio.create_task(
    ...
    async def _update_user_cache():
        ...
        try:
            ...
            global_proxy_spend = await user_api_key_cache.async_get_cache(
                key="{}:spend".format(mishikallm_proxy_admin_name)
            )
            if global_proxy_spend is None:
                ...
                values_to_update_in_cache.append(
                    ("{}:spend".format(mishikallm_proxy_admin_name), increment)
                )
        except Exception as e:
            verbose_proxy_logger.debug(
    ...
    async def _update_end_user_cache():
        ...
        try:
            ...
            existing_spend_obj = await user_api_key_cache.async_get_cache(key=_id)
            if existing_spend_obj is None:
                # if user does not exist in MishikaLLM_UserTable, create a new user
    ...
    async def _update_team_cache():
        ...
        try:
            # Fetch the existing cost for the given user
            existing_spend_obj: Optional[MishikaLLM_TeamTable] = (
    ...
    asyncio.create_task(
        user_api_key_cache.async_set_cache_pipeline(
            ...
            ttl=60,
            mishikallm_parent_otel_span=parent_otel_span,
        )
    )
...
def run_ollama_serve():
    try:
        ...
        verbose_proxy_logger.debug(
            f"""
            MishikaLLM Warning: proxy started with `ollama` model\n`ollama serve` failed with Exception{e}. \nEnsure you run `ollama serve`
        """
        )
...
class ProxyConfig:
    ...
    async def save_config(self, new_config: dict):
        ...
        - if api_key is passed, save that to the local environment or connected secret manage (maybe expose `mishikallm.save_secret()`)
        """
    ...
    def load_team_config(self, team_id: str):
        ...
        ## MISHIKALLM MODULE SETTINGS (e.g. mishikallm.drop_params=True,..)
        mishikallm_settings = config.get("mishikallm_settings", {})
        all_teams_config = mishikallm_settings.get("default_team_settings", None)
    ...
    def _init_cache(
        ...
        from mishikallm import Cache

        if "default_in_memory_ttl" in cache_params:
            mishikallm.default_in_memory_ttl = cache_params["default_in_memory_ttl"]

        if "default_redis_ttl" in cache_params:
            mishikallm.default_redis_ttl = cache_params["default_in_redis_ttl"]

        mishikallm.cache = Cache(**cache_params)

        if mishikallm.cache is not None and isinstance(mishikallm.cache.cache, RedisCache):
            ## INIT PROXY REDIS USAGE CLIENT ##
            redis_usage_cache = mishikallm.cache.cache
    ...
    async def get_config(self, config_file_path: Optional[str] = None) -> dict:
        ...
        if os.environ.get("MISHIKALLM_CONFIG_BUCKET_NAME") is not None:
            bucket_name = os.environ.get("MISHIKALLM_CONFIG_BUCKET_NAME")
            object_key = os.environ.get("MISHIKALLM_CONFIG_BUCKET_OBJECT_KEY")
            bucket_type = os.environ.get("MISHIKALLM_CONFIG_BUCKET_TYPE")
            verbose_proxy_logger.debug(
    ...
    async def load_config(  # noqa: PLR0915
        ...
        if environment_variables:
            ...
            # check if mishikallm_license in general_settings
            if "MISHIKALLM_LICENSE" in environment_variables:
                _license_check.license_str = os.getenv("MISHIKALLM_LICENSE", None)
                premium_user = _license_check.is_premium()
        ...
        ## MISHIKALLM MODULE SETTINGS (e.g. mishikallm.drop_params=True,..)
        mishikallm_settings = config.get("mishikallm_settings", None)
        if mishikallm_settings is None:
            mishikallm_settings = {}
        if mishikallm_settings:
            # ANSI escape code for blue text
            blue_color_code = "\033[94m"
            reset_color_code = "\033[0m"
            for key, value in mishikallm_settings.items():
                if key == "cache" and value is True:
                    print(f"{blue_color_code}\nSetting Cache on Proxy")  # noqa
                    from mishikallm.caching.caching import Cache

                    cache_params = {}
                    if "cache_params" in mishikallm_settings:
                        cache_params_in_config = mishikallm_settings["cache_params"]
                    ...
                    if mishikallm.cache is not None:
                        verbose_proxy_logger.debug(
                            f"{blue_color_code}Set Cache on MishikaLLM Proxy{reset_color_code}"
                        )
                    ...
                    mishikallm.guardrail_name_config_map = guardrail_name_config_map
                elif key == "callbacks":
                    ...
                    mishikallm.post_call_rules = [
                        get_instance_fn(value=value, config_file_path=config_file_path)
                    ]
                    verbose_proxy_logger.debug(
                        f"mishikallm.post_call_rules: {mishikallm.post_call_rules}"
                    )
                elif key == "max_internal_user_budget":
                    mishikallm.max_internal_user_budget = float(value)  # type: ignore
                elif key == "default_max_internal_user_budget":
                    mishikallm.default_max_internal_user_budget = float(value)
                    if mishikallm.max_internal_user_budget is None:
                        mishikallm.max_internal_user_budget = (
                            mishikallm.default_max_internal_user_budget
                        )
                elif key == "custom_provider_map":
                    from mishikallm.utils import custom_llm_setup
                    ...
                    for callback in value:
                        ...
                        # these are mishikallm callbacks - "langfuse", "sentry", "wandb"
                        else:
                            mishikallm.logging_callback_manager.add_mishikallm_success_callback(
                                callback
                            )
                            if "prometheus" in callback:
                                from mishikallm.integrations.prometheus import (
                                    PrometheusLogger,
                                )

                                PrometheusLogger._mount_metrics_endpoint(premium_user)
                    print(  # noqa
                        f"{blue_color_code} Initialized Success Callbacks - {mishikallm.success_callback} {reset_color_code}"
                    )  # noqa
                    ...
                    print(  # noqa
                        f"{blue_color_code} Initialized Failure Callbacks - {mishikallm.failure_callback} {reset_color_code}"
                    )  # noqa
                elif key == "cache_params":
                    # this is set in the cache branch
                    # see usage here: https://docs.21t.cc/docs/proxy/caching
                    pass
                    ...
                    verbose_proxy_logger.debug(
                        f"{blue_color_code} setting mishikallm.{key}={value}{reset_color_code}"
                    )
                    setattr(mishikallm, key, value)
                    ...
                    if value is not None and isinstance(value, dict):
                        ...
                        mishikallm.upperbound_key_generate_params = (
                            MishikaLLM_UpperboundKeyGenerateParams(**value)
                        )
                    else:
                        raise Exception(
        ...
        ## GENERAL SERVER SETTINGS (e.g. master key,..) # do this after initializing mishikallm, to ensure sentry logging works for proxylogging
        ...
        if general_settings:
            ...
            if key_management_settings is not None:
                mishikallm._key_management_settings = KeyManagementSettings(
            ...
            if database_url and database_url.startswith("os.environ/"):
                verbose_proxy_logger.debug("GOING INTO MISHIKALLM.GET_SECRET!")
                database_url = get_secret(database_url)
                verbose_proxy_logger.debug("RETRIEVED DB URL: %s", database_url)
            ### MASTER KEY ###
            master_key = general_settings.get(
                "master_key", get_secret("MISHIKALLM_MASTER_KEY", None)
            )
            ...
            if master_key is not None and isinstance(master_key, str):
                mishikallm_master_key_hash = hash_token(master_key)
            ...
            if allowed_ips is not None and premium_user is False:
                raise ValueError(
                    "allowed_ips is an Enterprise Feature. Please add a valid MISHIKALLM_LICENSE to your envionment."
                )
            ## BUDGET RESCHEDULER ##
            ...
            # check if mishikallm_license in general_settings
            if "mishikallm_license" in general_settings:
                _license_check.license_str = general_settings["mishikallm_license"]
                premium_user = _license_check.is_premium()

        router_params: dict = {
            "cache_responses": mishikallm.cache
        ...
        if model_list:
            ...
            print(  # noqa
                "\033[32mMishikaLLM: Proxy initialized with Config, Set models:\033[0m"
            )  # noqa
            for model in model_list:
                ### LOAD FROM os.environ/ ###
                for k, v in model["mishikallm_params"].items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        model["mishikallm_params"][k] = get_secret(v)
                print(f"\033[32m    {model.get('model_name', '')}\033[0m")  # noqa
                mishikallm_model_name = model["mishikallm_params"]["model"]
                mishikallm_model_api_base = model["mishikallm_params"].get("api_base", None)
                if "ollama" in mishikallm_model_name and mishikallm_model_api_base is None:
                    run_ollama_serve()
        ...
        if assistant_settings:
            for k, v in assistant_settings["mishikallm_params"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    _v = v.replace("os.environ/", "")
                    v = os.getenv(_v)
                    assistant_settings["mishikallm_params"][k] = v
        ...
        if router_settings and isinstance(router_settings, dict):
            arg_spec = inspect.getfullargspec(mishikallm.Router)
        ...
        mishikallm.credential_list = credential_list_dict
    ...
    def _init_non_llm_configs(self, config: dict):
        ...
        if mcp_servers_config:
            from mishikallm.proxy._experimental.mcp_server.mcp_server_manager import (
        ...
        if vector_store_registry_config:
            from mishikallm.vector_stores.vector_store_registry import VectorStoreRegistry

            if mishikallm.vector_store_registry is None:
                mishikallm.vector_store_registry = VectorStoreRegistry()

            # Load vector stores from config
            mishikallm.vector_store_registry.load_vector_stores_from_config(
    ...
    def _load_alerting_settings(self, general_settings: dict):
        ...
        for _alert in _alerting_callbacks:
            if _alert == "slack":
                ...
                if _alert in mishikallm._known_custom_logger_compatible_callbacks:
                    ...
                    if _logger is not None:
                        mishikallm.logging_callback_manager.add_mishikallm_callback(_logger)
        pass
    ...
    def initialize_secret_manager(self, key_management_system: Optional[str]):
        ...
        if key_management_system is not None:
            if key_management_system == KeyManagementSystem.AZURE_KEY_VAULT.value:
                ...
                from mishikallm.secret_managers.aws_secret_manager_v2 import (
                    AWSSecretsManagerV2,
                )
                ...
                from mishikallm.secret_managers.google_secret_manager import (
                    GoogleSecretManager,
                )

                GoogleSecretManager()
            elif key_management_system == KeyManagementSystem.HASHICORP_VAULT.value:
                from mishikallm.secret_managers.hashicorp_secret_manager import (
    ...
    def get_model_info_with_id(self, model, db_model=False) -> RouterModelInfo:
        ...
        if premium_user is True:
            # seeing "created_at", "updated_at", "created_by", "updated_by" is a MishikaLLM Enterprise Feature
            ...
            for model in model_list:
                ...
                if model_id is None:
                    ...
                    model_id = llm_router._generate_model_id(
                        ...
                        mishikallm_params=model["mishikallm_params"],
                    )
    ...
    def _add_deployment(self, db_models: list) -> int:
        ...
        ## ADD MODEL LOGIC
        for m in db_models:
            _mishikallm_params = m.mishikallm_params
            if isinstance(_mishikallm_params, dict):
                # decrypt values
                for k, v in _mishikallm_params.items():
                    if isinstance(v, str):
                        # decrypt value
                        ...
                        if len(_value) > 0:
                            _mishikallm_params[k] = _value
                _mishikallm_params = MishikaLLM_Params(**_mishikallm_params)

            else:
                verbose_proxy_logger.error(
                    f"Invalid model added to proxy db. Invalid mishikallm params. mishikallm_params={_mishikallm_params}"
                )
            ...
            added = llm_router.upsert_deployment(
                deployment=Deployment(
                    ...
                    mishikallm_params=_mishikallm_params,
        ...
        for m in new_models:
            ...
            if isinstance(_mishikallm_params, dict):
                ...
                for k, v in _mishikallm_params.items():
                    ...
                    _mishikallm_params[k] = decrypted_value
                _mishikallm_params = MishikaLLM_Params(**_mishikallm_params)
            else:
                verbose_proxy_logger.error(
                    f"Invalid model added to proxy db. Invalid mishikallm params. mishikallm_params={_mishikallm_params}"
    ...
    def _add_callbacks_from_db_config(self, config_data: dict) -> None:
        """
        Adds callbacks from DB config to mishikallm
        """
        mishikallm_settings = config_data.get("mishikallm_settings", {}) or {}
        success_callbacks = mishikallm_settings.get("success_callback", None)
        failure_callbacks = mishikallm_settings.get("failure_callback", None)

        if success_callbacks is not None and isinstance(success_callbacks, list):
            for success_callback in success_callbacks:
                if (
                    success_callback
                    in mishikallm._known_custom_logger_compatible_callbacks
                ):
                    _add_custom_logger_callback_to_specific_event(
                        success_callback, "success"
                    )
                elif success_callback not in mishikallm.success_callback:
                    mishikallm.logging_callback_manager.add_mishikallm_success_callback(
                        success_callback
                    )

        # Add failure callbacks from DB to mishikallm
        if failure_callbacks is not None and isinstance(failure_callbacks, list):
            for failure_callback in failure_callbacks:
                ...
                elif failure_callback not in mishikallm.failure_callback:
                    mishikallm.logging_callback_manager.add_mishikallm_failure_callback(
                        failure_callback
                    )
    ...
    def _add_environment_variables_from_db_config(self, config_data: dict) -> None:
        """
        Adds environment variables from DB config to mishikallm
        """
    ...
    async def _add_router_settings_from_db_config(
        ...
        """
        Adds router settings from DB config to mishikallm proxy
        """
        if llm_router is not None and prisma_client is not None:
            db_router_settings = await prisma_client.db.mishikallm_config.find_first(
    ...
    def _add_general_settings_from_db_config(
        ...
        """
        Adds general settings from DB config to mishikallm proxy
    ...
    async def _get_models_from_db(self, prisma_client: PrismaClient) -> list:
        try:
            new_models = await prisma_client.db.mishikallm_proxymodeltable.find_many()
        except Exception as e:
            verbose_proxy_logger.exception(
                "mishikallm.proxy_server.py::add_deployment() - Error getting new models from DB - {}".format(
                    str(e)
                )
            )
            new_models = []
    ...
    async def add_deployment(
        ...
        try:
            ...
            db_general_settings = await prisma_client.db.mishikallm_config.find_first(
            ...
            verbose_proxy_logger.exception(
                "mishikallm.proxy.proxy_server.py::ProxyConfig:add_deployment - {}".format(
                    str(e)
                )
            )
        ...
        try:
            ...
            if mishikallm.vector_store_registry is None:
                ...
                for vector_store in vector_stores:
                    mishikallm.vector_store_registry.add_vector_store_to_registry(
                        vector_store=vector_store
                    )
        except Exception as e:
            verbose_proxy_logger.exception(
                "mishikallm.proxy.proxy_server.py::ProxyConfig:_init_vector_stores_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def delete_credentials(self, db_credentials: List[CredentialItem]):
        ...
        Compare to the mishikallm.credential_list
        Delete any from mishikallm.credential_list that are not in the all-up list
        """
        ## CONFIG credentials ##
        config = await self.get_config(config_file_path=user_config_file_path)
        ...
        ## DELETE ##
        idx_to_delete = []
        for idx, credential in enumerate(mishikallm.credential_list):
            if credential.credential_name not in [
                cred.credential_name for cred in combined_list
            ]:
                idx_to_delete.append(idx)
        for idx in sorted(idx_to_delete, reverse=True):
            mishikallm.credential_list.pop(idx)

    async def get_credentials(self, prisma_client: PrismaClient):
        try:
            credentials = await prisma_client.db.mishikallm_credentialstable.find_many()
            ...
            verbose_proxy_logger.exception(
                "mishikallm.proxy_server.py::get_credentials() - Error getting credentials from DB - {}".format(
                    str(e)
                )
            )
            return []
...
async def initialize(  # noqa: PLR0915
    ...
    if os.getenv("MISHIKALLM_DONT_SHOW_FEEDBACK_BOX", "").lower() != "true":
    ...
    if debug is True:  # this needs to be first, so users can see Router init debugg
        ...
        from mishikallm._logging import (
            verbose_logger,
    ...
    if detailed_debug is True:
        ...
        # users can control proxy debugging using env variable = 'MISHIKALLM_LOG'
        mishikallm_log_setting = os.environ.get("MISHIKALLM_LOG", "")
        if mishikallm_log_setting is not None:
            if mishikallm_log_setting.upper() == "INFO":
                import logging
            ...
            elif mishikallm_log_setting.upper() == "DEBUG":
                import logging
    ...
    if api_version:
        os.environ["AZURE_API_VERSION"] = (
            api_version  # set this for azure - mishikallm can read this from the env
        )
    ...
    if drop_params is True:  # mishikallm-specific param
        mishikallm.drop_params = True
        dynamic_config["general"]["drop_params"] = True
    if add_function_to_prompt is True:  # mishikallm-specific param
        mishikallm.add_function_to_prompt = True
        dynamic_config["general"]["add_function_to_prompt"] = True
    if max_budget:  # mishikallm-specific param
        mishikallm.max_budget = max_budget
...
async def async_assistants_data_generator(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.async_assistants_data_generator(): Exception occured - {}".format(
                str(e)
            )
        )
        ...
        verbose_proxy_logger.debug(
            f"\033[1;31mAn error occurred: {e}\n\n Debug this by setting `--debug`, e.g. `mishikallm --model gpt-3.5-turbo --debug`"
        )
...
async def async_data_generator(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.async_data_generator(): Exception occured - {}".format(
                str(e)
            )
        )
        await proxy_logging_obj.post_call_failure_hook(
...
def get_mishikallm_model_info(model: dict = {}):
    model_info = model.get("model_info", {})
    model_to_lookup = model.get("mishikallm_params", {}).get("model", None)
    try:
        if "azure" in model_to_lookup:
            model_to_lookup = model_info.get("base_model", None)
        mishikallm_model_info = mishikallm.get_model_info(model_to_lookup)
        return mishikallm_model_info
    except Exception:
        # this should not block returning on /model/info
        # if mishikallm does not have info on the model it should return {}
        return {}
...
class ProxyStartupEvent:
    ...
    def _initialize_jwt_auth(
        ...
        if general_settings.get("mishikallm_jwtauth", None) is not None:
            for k, v in general_settings["mishikallm_jwtauth"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    general_settings["mishikallm_jwtauth"][k] = get_secret(v)
            mishikallm_jwtauth = MishikaLLM_JWTAuth(**general_settings["mishikallm_jwtauth"])
        else:
            mishikallm_jwtauth = MishikaLLM_JWTAuth()
    ...
    @classmethod
    def _add_proxy_budget_to_db(cls, mishikallm_proxy_budget_name: str):
        """Adds a global proxy budget to db"""
        if mishikallm.budget_duration is None:
            raise Exception(
        ...
        asyncio.create_task(
            generate_key_helper_fn(  # type: ignore
                request_type="user",
                user_id=mishikallm_proxy_budget_name,
                duration=None,
                models=[],
                aliases={},
                config={},
                ...
                budget_duration=mishikallm.budget_duration,
    ...
    async def initialize_scheduled_background_jobs(
        ...
        if mishikallm.prometheus_initialize_budget_metrics is True:
    ...
    def _init_dd_tracer(cls):
        ...
        """
        from mishikallm.mishikallm_core_utils.dd_tracing import _should_use_dd_tracer
...
async def chat_completion(  # noqa: PLR0915
    ...
    try:
        ...
        _chat_response = mishikallm.ModelResponse()
        _chat_response.choices[0].message.content = e.message  # type: ignore

        if data.get("stream", None) is not None and data["stream"] is True:
            _iterator = mishikallm.utils.ModelResponseIterator(
                model_response=_chat_response, convert_to_delta=True
            )
            _streaming_response = mishikallm.CustomStreamWrapper(
                completion_stream=_iterator,
                model=data.get("model", ""),
                custom_llm_provider="cached_response",
                logging_obj=data.get("mishikallm_logging_obj", None),
            )
        ...
        _usage = mishikallm.Usage(prompt_tokens=0, completion_tokens=0, total_tokens=0)
...
async def completion(  # noqa: PLR0915
    ...
    try:
        ...
        if data["model"] in mishikallm.model_alias_map:
            data["model"] = mishikallm.model_alias_map[data["model"]]
        ...
        mishikallm_call_id = hidden_params.get("mishikallm_call_id", None) or ""
        ...
        if _data.get("stream", None) is not None and _data["stream"] is True:
            ...
            _streaming_response = mishikallm.TextCompletionStreamWrapper(
                completion_stream=_iterator,
            ...
            _response = mishikallm.TextCompletionResponse()
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.completion(): Exception occured - {}".format(
                str(e)
            )
        )
        error_msg = f"{str(e)}"
...
"""
    ...
    try:
            ...
            if llm_model_list is not None and data["model"] in router_model_names:
                for m in llm_model_list:
                    if m["model_name"] == data["model"] and (
                        m["mishikallm_params"]["model"] in mishikallm.open_ai_embedding_models
                        or m["mishikallm_params"]["model"].startswith("azure/")
                    ):
                        pass
                    else:
                        ...
                        for i in data["input"]:
                            input_list.append(
                                mishikallm.decode(model="gpt-3.5-turbo", tokens=i)
                            )
        ...
        mishikallm_debug_info = getattr(e, "mishikallm_debug_info", "")
        verbose_proxy_logger.debug(
            "\033[1;31mAn error occurred: %s %s\n\n Debug this by setting `--debug`, e.g. `mishikallm --model gpt-3.5-turbo --debug`",
            e,
            mishikallm_debug_info,
        )
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.embeddings(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def image_generation(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.image_generation(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def audio_speech(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.audio_speech(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def audio_transcriptions(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.audio_transcription(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
from mishikallm import _arealtime
...
async def get_assistants(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as MishikaLLM adds more providers for acreate_batch
        if llm_router is None:
            raise HTTPException(
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.get_assistants(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def create_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.create_assistant(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def delete_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.delete_assistant(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def create_threads(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.create_threads(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def get_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.get_thread(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def add_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.add_messages(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def get_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.get_messages(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def run_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.run_thread(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def moderations(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.moderations(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def token_counter(request: TokenCountRequest):
    ...
    if deployment is not None:
        mishikallm_model_name = deployment.get("mishikallm_params", {}).get("model")
        # remove the custom_llm_provider_prefix in the mishikallm_model_name
        if "/" in mishikallm_model_name:
            mishikallm_model_name = mishikallm_model_name.split("/", 1)[1]

    model_to_use = (
        mishikallm_model_name or request.model
    )  # use mishikallm model name, if it's not avalable then fallback to request.model
    ...
    _tokenizer_used = mishikallm.utils._select_tokenizer(
...
async def supported_openai_params(model: str):
    """
    Returns supported openai params for a given mishikallm model name
    ...
    """
    try:
        model, custom_llm_provider, _, _ = mishikallm.get_llm_provider(model=model)
        return {
...
async def transform_request(request: TransformRequestBody):
    from mishikallm.utils import return_raw_request
...
async def _check_if_model_is_user_added(
    ...
    for model in models:
        ...
        if id is None:
            continue
        db_model = await prisma_client.db.mishikallm_proxymodeltable.find_unique(
            where={"model_id": id}
        )
...
def _check_if_model_is_team_model(
    models: List[DeploymentTypedDict], user_row: MishikaLLM_UserTable
) -> List[Dict]:
    """
...
async def non_admin_all_models(
    ...
    if user_api_key_dict.user_id:
        try:
            user_row = await prisma_client.db.mishikallm_usertable.find_unique(
    ...
    unique_models = _deduplicate_mishikallm_router_models(models=all_models)
...
async def model_info_v2(
    ...
    if llm_router is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": f"No model list passed, models router={llm_router}. You can add a model through the config.yaml or on the MishikaLLM Admin UI."
            },
        )
    ...
    # fill in model info based on config.yaml and mishikallm model_prices_and_context_window.json
    for _model in all_models:
        ...
        # read mishikallm model_prices_and_context_window.json to get the following:
        # input_cost_per_token, output_cost_per_token, max_tokens
        mishikallm_model_info = get_mishikallm_model_info(model=_model)

        # 2nd pass on the model, try seeing if we can find model in mishikallm model_cost map
        if mishikallm_model_info == {}:
            # use mishikallm_param model_name to get model_info
            mishikallm_params = _model.get("mishikallm_params", {})
            mishikallm_model = mishikallm_params.get("model", None)
            try:
                mishikallm_model_info = mishikallm.get_model_info(model=mishikallm_model)
            except Exception:
                mishikallm_model_info = {}
        # 3rd pass on the model, try seeing if we can find model but without the "/" in model cost map
        if mishikallm_model_info == {}:
            # use mishikallm_param model_name to get model_info
            mishikallm_params = _model.get("mishikallm_params", {})
            mishikallm_model = mishikallm_params.get("model", None)
            split_model = mishikallm_model.split("/")
            if len(split_model) > 0:
                mishikallm_model = split_model[-1]
            try:
                mishikallm_model_info = mishikallm.get_model_info(
                    model=mishikallm_model, custom_llm_provider=split_model[0]
                )
            except Exception:
                mishikallm_model_info = {}
        for k, v in mishikallm_model_info.items():
            if k not in model_info:
                model_info[k] = v
        _model["model_info"] = model_info
        # don't return the api key / vertex credentials
        # don't return the llm credentials
        _model["mishikallm_params"].pop("api_key", None)
        _model["mishikallm_params"].pop("vertex_credentials", None)
        _model["mishikallm_params"].pop("aws_access_key_id", None)
        _model["mishikallm_params"].pop("aws_secret_access_key", None)
...
async def model_metrics_exceptions(
    ...
    sql_query = """
        WITH cte AS (
            SELECT 
                CASE WHEN api_base = '' THEN mishikallm_model_name ELSE CONCAT(mishikallm_model_name, '-', api_base) END AS combined_model_api_base,
                exception_type,
                COUNT(*) AS num_rate_limit_exceptions
            FROM "MishikaLLM_ErrorLogs"
            WHERE 
...
def _get_proxy_model_info(model: dict) -> dict:
    ...
    # read mishikallm model_prices_and_context_window.json to get the following:
    # input_cost_per_token, output_cost_per_token, max_tokens
    mishikallm_model_info = get_mishikallm_model_info(model=model)
...
async def model_info_v1(  # noqa: PLR0915
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    mishikallm_model_id: Optional[str] = None,
):
    """
    Provides more info about each model in /models, including config.yaml descriptions (except api key and api base)

    Parameters:
        mishikallm_model_id: Optional[str] = None (this is the value of `x-mishikallm-model-id` returned in response headers)

        - When mishikallm_model_id is passed, it will return the info for that specific model
        - When mishikallm_model_id is not passed, it will return the info for all models
    ...
    if user_model is not None:
        # user is trying to get specific model from mishikallm router
        try:
            model_info: Dict = cast(Dict, mishikallm.get_model_info(model=user_model))
        except Exception:
            model_info = {}
    ...
    if llm_model_list is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Model List not loaded in. Make sure you passed models in your config.yaml or on the MishikaLLM Admin UI. - https://docs.21t.cc/docs/proxy/configs"
            },
        )
    ...
    if llm_router is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Router is not loaded in. Make sure you passed models in your config.yaml or on the MishikaLLM Admin UI. - https://docs.21t.cc/docs/proxy/configs"
            },
        )

    if mishikallm_model_id is not None:
        # user is trying to get specific model from mishikallm router
        deployment_info = llm_router.get_deployment(model_id=mishikallm_model_id)
        if deployment_info is None:
            raise HTTPException(
                status_code=400,
                detail={
                    "error": f"Model id = {mishikallm_model_id} not found on mishikallm proxy"
                },
            )
...
async def model_group_info(
    ...
    """
    Get information about all the deployments on mishikallm proxy, including config.yaml descriptions (except api key and api base)
    ...
    Learn how to use and set wildcard models [here](https://docs.21t.cc/docs/wildcard_routing)
    ...
    if (
        ...
        user_object = await prisma_client.db.mishikallm_usertable.find_first(
            where={"user_id": user_api_key_dict.user_id}
        )
        user_object_typed = MishikaLLM_UserTable(**user_object.model_dump())
        user_models = []
...
async def model_settings():
    ...
    for provider in mishikallm.provider_list:
        returned_list.append(
            ProviderInfo(
                name=provider,
                fields=mishikallm.get_provider_fields(custom_llm_provider=provider),
            )
        )
...
async def alerting_settings(
    ...
    if user_api_key_dict.user_role != MishikallmUserRoles.PROXY_ADMIN:
        raise HTTPException(
            status_code=400,
            detail={
...
async def async_queue_request(
    ...
    try:
        ...
        fastapi_response.headers.update({"x-mishikallm-priority": str(data["priority"])})
        return response
    except Exception as e:
        await proxy_logging_obj.post_call_failure_hook(
            user_api_key_dict=user_api_key_dict, original_exception=e, request_data=data
...
async def fallback_login(request: Request):
    ...
    PROXY_BASE_URL should be the your deployed proxy endpoint, e.g. PROXY_BASE_URL="https://mishikallm-production-7002.up.railway.app/"
    Example:
    """
...
async def login(request: Request):  # noqa: PLR0915
    ...
    if master_key is None:
        raise ProxyException(
            message="Master Key not set for Proxy. Please set Master Key to use Admin UI. Set `MISHIKALLM_MASTER_KEY` in .env or set general_settings:master_key in config.yaml.  https://docs.21t.cc/docs/proxy/virtual_keys. If set, use `--detailed_debug` to debug issue.",
            type=ProxyErrorTypes.auth_error,
            param="master_key",
            code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    ...
    if ui_password is None:
        raise ProxyException(
            message="set Proxy master key to use UI. https://docs.21t.cc/docs/proxy/virtual_keys. If set, use `--detailed_debug` to debug issue.",
            type=ProxyErrorTypes.auth_error,
            param="UI_PASSWORD",
            code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    ...
    _user_row: Optional[MishikaLLM_UserTable] = None
    user_role: Optional[
        Literal[
            MishikallmUserRoles.PROXY_ADMIN,
            MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY,
            MishikallmUserRoles.INTERNAL_USER,
            MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY,
        ]
    ] = None
    if prisma_client is not None:
        _user_row = cast(
            Optional[MishikaLLM_UserTable],
            await prisma_client.db.mishikallm_usertable.find_first(
    ...
    if secrets.compare_digest(username, ui_username) and secrets.compare_digest(
        ...
        user_role = MishikallmUserRoles.PROXY_ADMIN
        user_id = mishikallm_proxy_admin_name

        # we want the key created to have PROXY_ADMIN_PERMISSIONS
        key_user_id = mishikallm_proxy_admin_name
        if (
            os.getenv("PROXY_ADMIN_ID", None) is not None
            and os.environ["PROXY_ADMIN_ID"] == user_id
        ) or user_id == mishikallm_proxy_admin_name:
            # checks if user is admin
            key_user_id = os.getenv("PROXY_ADMIN_ID", mishikallm_proxy_admin_name)
        ...
        if os.getenv("DATABASE_URL") is not None:
            response = await generate_key_helper_fn(
                ...
                **{
                    "user_role": MishikallmUserRoles.PROXY_ADMIN,
                    "duration": "24hr",
                    "key_max_budget": mishikallm.max_ui_session_budget,
                    "models": [],
                    "aliases": {},
                    "config": {},
                    "spend": 0,
                    "user_id": key_user_id,
                    "team_id": "mishikallm-dashboard",
                },  # type: ignore
            )
        else:
            raise ProxyException(
        ...
        mishikallm_dashboard_ui = os.getenv("PROXY_BASE_URL", "")
        if mishikallm_dashboard_ui.endswith("/"):
            mishikallm_dashboard_ui += "ui/"
        else:
            mishikallm_dashboard_ui += "/ui/"
        import jwt

        if get_secret_bool("EXPERIMENTAL_UI_LOGIN"):
            user_info: Optional[MishikaLLM_UserTable] = None
            if _user_row is not None:
                user_info = _user_row
            elif (
                user_id is not None
            ):  # if user_id is not None, we are using the UI_USERNAME and UI_PASSWORD
                user_info = MishikaLLM_UserTable(
                    user_id=user_id,
                    user_role=user_role,
                    models=[],
                    max_budget=mishikallm.max_ui_session_budget,
                )
        ...
        jwt_token = jwt.encode(  # type: ignore
            {
                ...
                "auth_header_name": general_settings.get(
                    "mishikallm_key_header_name", "Authorization"
                ),
        ...
        mishikallm_dashboard_ui += "?login=success"
        redirect_response = RedirectResponse(url=mishikallm_dashboard_ui, status_code=303)
        ...
        user_role = getattr(
            _user_row, "user_role", MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY
        )
        ...
        if secrets.compare_digest(password, _password) or secrets.compare_digest(
            ...
            redirect_response = RedirectResponse(
                url=mishikallm_dashboard_ui, status_code=303
            )
            redirect_response.set_cookie(key="token", value=jwt_token)
            return redirect_response
        else:
...
async def onboarding(invite_link: str):
    ...
    invite_obj = await prisma_client.db.mishikallm_invitationlink.find_unique(
    ...
    utc_now_date = mishikallm.utils.get_utc_datetime().date()
    ...
    #### INVALIDATE LINK
    current_time = mishikallm.utils.get_utc_datetime()

    _ = await prisma_client.db.mishikallm_invitationlink.update(
        where={"id": invite_link},
        data={
    ...
    ### GET USER OBJECT ###
    user_obj = await prisma_client.db.mishikallm_usertable.find_unique(
    ...
    mishikallm_dashboard_ui = os.getenv("PROXY_BASE_URL", "")
    if mishikallm_dashboard_ui.endswith("/"):
        mishikallm_dashboard_ui += "ui/onboarding"
    else:
        mishikallm_dashboard_ui += "/ui/onboarding"
    import jwt
    ...
    mishikallm_dashboard_ui += "?token={}&user_email={}".format(jwt_token, user_email)
    return {
        "login_url": mishikallm_dashboard_ui,
        "token": jwt_token,
        "user_email": user_email,
    }
...
async def claim_onboarding_link(data: InvitationClaim):
    ...
    user_obj = await prisma_client.db.mishikallm_usertable.update(
...
async def new_invitation(
    ...
    curl -X POST 'http://localhost:4000/invitation/new' \
        -H 'Content-Type: application/json' \
        -d '{
            "user_id": "1234" // 👈 id of user in 'MishikaLLM_UserTable'
        }'
    ```
    """
    global prisma_client
    ...
    try:
        response = await prisma_client.db.mishikallm_invitationlink.create(
            data={
                "user_id": data.user_id,
                "created_at": current_time,
                "expires_at": expires_at,
                "created_by": user_api_key_dict.user_id or mishikallm_proxy_admin_name,
                "updated_at": current_time,
                "updated_by": user_api_key_dict.user_id or mishikallm_proxy_admin_name,
            }  # type: ignore
        )
        return response
    except Exception as e:
        if "Foreign key constraint failed on the field" in str(e):
            raise HTTPException(
                status_code=400,
                detail={
                    "error": "User id does not exist in 'MishikaLLM_UserTable'. Fix this by creating user via `/user/new`."
                },
            )
...
async def invitation_info(
    ...
    response = await prisma_client.db.mishikallm_invitationlink.find_unique(
...
async def invitation_update(
    ...
    curl -X POST 'http://localhost:4000/invitation/update' \
        -H 'Content-Type: application/json' \
        -d '{
            "invitation_id": "1234" // 👈 id of invitation in 'MishikaLLM_InvitationTable'
    ...
    current_time = mishikallm.utils.get_utc_datetime()
    response = await prisma_client.db.mishikallm_invitationlink.update(
...
async def invitation_delete(
    ...
    response = await prisma_client.db.mishikallm_invitationlink.delete(
        where={"id": data.invitation_id}
    )
...
async def update_config(config_info: ConfigYAML):  # noqa: PLR0915
    ...
    Currently supports modifying General Settings + MishikaLLM settings
    """
    ...
    try:
        ...
        # update the mishikallm settings
        if config_info.mishikallm_settings is not None:
            config.setdefault("mishikallm_settings", {})
            updated_mishikallm_settings = config_info.mishikallm_settings
            config["mishikallm_settings"] = {
                **updated_mishikallm_settings,
                **config["mishikallm_settings"],
            }

            # if mishikallm.success_callback in updated_mishikallm_settings and config["mishikallm_settings"]
            if (
                "success_callback" in updated_mishikallm_settings
                and "success_callback" in config["mishikallm_settings"]
            ):
                # check both success callback are lists
                if isinstance(
                    config["mishikallm_settings"]["success_callback"], list
                ) and isinstance(updated_mishikallm_settings["success_callback"], list):
                    combined_success_callback = (
                        config["mishikallm_settings"]["success_callback"]
                        + updated_mishikallm_settings["success_callback"]
                    )
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.update_config(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def update_config_general_settings(
    ...
    """
    Update a specific field in mishikallm general settings
    """
    global prisma_client
    ## VALIDATION ##
    """
    ...
    response = await prisma_client.db.mishikallm_config.upsert(
        where={"param_name": "general_settings"},
        data={
...
async def delete_config_general_settings(
    ...
    """
    Delete the db value of this field in mishikallm general settings. Resets it to it's initial default value on mishikallm.
    """
    global prisma_client
    ## VALIDATION ##
    """
...
async def get_config():  # noqa: PLR0915
    ...
    try:
        ...
        config_data = await proxy_config.get_config()
        _mishikallm_settings = config_data.get("mishikallm_settings", {})
        _general_settings = config_data.get("general_settings", {})
        environment_variables = config_data.get("environment_variables", {})

        # check if "langfuse" in mishikallm_settings
        _success_callbacks = _mishikallm_settings.get("success_callback", [])
        _data_to_return = []
        """
        [
            {
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.get_config(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def config_yaml_endpoint(config_info: ConfigYAML):
    ...
    - **model_list**: *Optional[List[ModelParams]]* - A list of supported models on the server, along with model-specific configurations. ModelParams includes "model_name" (name of the model), "mishikallm_params" (mishikallm-specific parameters for the model), and "model_info" (additional info about the model such as id, mode, cost per token, etc).

    - **mishikallm_settings**: *Optional[dict]*: Settings for the mishikallm module. You can specify multiple properties like "drop_params", "set_verbose", "api_base", "cache".
...
@router.get(
    "/get/mishikallm_model_cost_map",
    include_in_schema=False,
    dependencies=[Depends(user_api_key_auth)],
)
async def get_mishikallm_model_cost_map():
    try:
        _model_cost_map = mishikallm.model_cost
