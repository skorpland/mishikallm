...
from mishikallm.proxy._types import ProviderBudgetResponse, ProviderBudgetResponseObject
from mishikallm.proxy.auth.user_api_key_auth import user_api_key_auth
from mishikallm.proxy.spend_tracking.spend_tracking_utils import (
    get_spend_by_team_and_customer,
)
from mishikallm.proxy.utils import handle_exception_on_proxy

if TYPE_CHECKING:
    from mishikallm.proxy.proxy_server import PrismaClient
else:
    PrismaClient = Any
...
async def view_spend_tags(
    ...
    """
    MishikaLLM Enterprise - View Spend Per Request Tag
...
async def get_global_activity(
    ...
    try:
        ...
        if (
            user_api_key_dict.user_role == MishikallmUserRoles.INTERNAL_USER
            or user_api_key_dict.user_role == MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY
        ):
...
async def get_global_spend_provider(
    ...
    from mishikallm.proxy.proxy_server import llm_router, prisma_client
        ...
        for row in db_response:
            ...
            if llm_router is not None:
                ...
                if _deployment is not None:
                    try:
                        _, _provider, _, _ = mishikallm.get_llm_provider(
                            model=_deployment.mishikallm_params.model,
                            custom_llm_provider=_deployment.mishikallm_params.custom_llm_provider,
                            api_base=_deployment.mishikallm_params.api_base,
                            mishikallm_params=_deployment.mishikallm_params,
                        )
...
async def get_global_spend_report(
    ...
    from mishikallm.proxy.proxy_server import premium_user, prisma_client
...
async def global_view_spend_tags(
    ...
    """
    MishikaLLM Enterprise - View Spend Per Request Tag. Used by MishikaLLM UI
...
async def _get_spend_report_for_time_range(
    ...
    try:
        ...
        FROM
            "MishikaLLM_SpendLogs" s
        LEFT JOIN
            "MishikaLLM_TeamTable" t ON s.team_id = t.team_id
        WHERE
...
async def calculate_spend(request: SpendCalculateRequest):
    ...
    Note: If you see a spend of $0.0 you need to set custom_pricing for your model: https://docs.21t.cc/docs/proxy/custom_pricing
    ...
    try:
        ...
        from mishikallm.cost_calculator import CostPerToken
        ...
        if request.model is not None:
            ...
            1. user passes model, and model is defined on mishikallm config.yaml or in DB. use info on config or in DB in this case
            2. user passes model, and model is not defined on mishikallm config.yaml or in DB. Pass model as is to mishikallm.completion_cost
            3. user passes completion_response
            
            """
            if _model_in_llm_router is not None:
                _mishikallm_params = _model_in_llm_router.get("mishikallm_params")
                _mishikallm_model_name = _mishikallm_params.get("model")
                input_cost_per_token = _mishikallm_params.get("input_cost_per_token")
                output_cost_per_token = _mishikallm_params.get("output_cost_per_token")
                if (
            ...
            _completion_response = mishikallm.ModelResponse(**request.completion_response)
...
async def ui_view_spend_logs(  # noqa: PLR0915
    ...
    Returns:
        {
            "data": List[MishikaLLM_SpendLogs],  # Paginated spend logs
    ...
    try:
        ...
        total_records = await prisma_client.db.mishikallm_spendlogs.count(
            where=where_conditions,
        )

        # Get paginated data
        data = await prisma_client.db.mishikallm_spendlogs.find_many(
            where=where_conditions,
            order={
                "startTime": "desc",
            },
...
async def ui_view_request_response_for_request_id(
    ...
    """
    custom_loggers = (
        mishikallm.logging_callback_manager.get_active_additional_logging_utils_from_custom_logger()
    )
...
-H "Authorization: Bearer sk-1234"
    ...
    try:
        ...
        if (
            ...
            # SQL query
            response = await prisma_client.db.mishikallm_spendlogs.group_by(
...
async def global_spend_reset():
    ...
    Globally reset spend for All API Keys and Teams, maintain MishikaLLM_SpendLogs

    1. MishikaLLM_SpendLogs will maintain the logs on spend, no data gets deleted from there
    2. MishikaLLM_VerificationTokens spend will be set = 0
    3. MishikaLLM_TeamTable spend will be set = 0
    ...
    await prisma_client.db.mishikallm_verificationtoken.update_many(
        data={"spend": 0.0}, where={}
    )
    await prisma_client.db.mishikallm_teamtable.update_many(data={"spend": 0.0}, where={})
...
async def global_spend_logs(
    ...
    from mishikallm.integrations.prometheus_helpers.prometheus_api import (
...
async def global_spend():
    ...
    try:
        ...
        return {"spend": total_spend, "max_budget": mishikallm.max_budget}
    except Exception as e:
        error_trace = traceback.format_exc()
        error_str = str(e) + "\n" + error_trace
        if isinstance(e, HTTPException):
...
async def global_spend_key_internal_user(
    ...
    sql_query = """
        ...
        FROM 
            top_api_keys t
        LEFT JOIN 
            "MishikaLLM_VerificationToken" v ON t.api_key = v.token
        ORDER BY 
            t.total_spend DESC;
...
async def global_view_all_end_users():
    ...
    sql_query = """
    SELECT DISTINCT end_user FROM "MishikaLLM_SpendLogs"
    """
...
async def provider_budgets() -> ProviderBudgetResponse:
    """
    Provider Budget Routing - Get Budget, Spend Details https://docs.21t.cc/docs/proxy/provider_budget_routing
    ...
    try:
        ...
        if provider_budget_config is None:
            raise ValueError(
                "No provider budget config found. Please set a provider budget config in the router settings. https://docs.21t.cc/docs/proxy/provider_budget_routing"
            )
...
async def ui_view_session_spend_logs(
    ...
    try:
        ...
        # Query the database
        result = await prisma_client.db.mishikallm_spendlogs.find_many(
