...
from mishikallm.constants import REDACTED_BY_LITELM_STRING
from mishikallm.mishikallm_core_utils.core_helpers import get_mishikallm_metadata_from_kwargs
from mishikallm.proxy._types import SpendLogsMetadata, SpendLogsPayload
from mishikallm.proxy.utils import PrismaClient, hash_token
...
def get_spend_logs_id(
    ...
    if call_type == "aretrieve_batch" or call_type == "acreate_file":
        ...
        id = cast(Optional[str], response_obj.get("id")) or cast(
            Optional[str], kwargs.get("mishikallm_call_id")
        )
    return id
...
def get_logging_payload(  # noqa: PLR0915
    ...
    from mishikallm.proxy.proxy_server import general_settings, master_key
    ...
    metadata = get_mishikallm_metadata_from_kwargs(kwargs)
    completion_start_time = kwargs.get("completion_start_time", end_time)
    call_type = kwargs.get("call_type")
    cache_hit = kwargs.get("cache_hit", False)
    usage = cast(dict, response_obj).get("usage", None) or {}
    if isinstance(usage, mishikallm.Usage):
        usage = dict(usage)
    ...
    end_user_id = get_end_user_id_for_cost_tracking(mishikallm_params)
    ...
    if api_key is not None and isinstance(api_key, str):
        ...
        if (
            ...
            api_key = "mishikallm_proxy_master_key"  # use a known alias, if the user disabled storing master key in db
    ...
    if mishikallm.cache is not None:
        cache_key = mishikallm.cache.get_cache_key(**kwargs)
    else:
        cache_key = "Cache OFF"
    ...
    try:
        payload: SpendLogsPayload = SpendLogsPayload(
            ...
            proxy_server_request=_get_proxy_server_request_for_spend_logs_payload(
                metadata=metadata, mishikallm_params=mishikallm_params
            ),
...
def _get_session_id_for_spend_log(
    ...
    # Users can dynamically set the trace_id for each request by passing `mishikallm_trace_id` in kwargs
    if kwargs.get("mishikallm_trace_id") is not None:
        return str(kwargs.get("mishikallm_trace_id"))
...
def _get_proxy_server_request_for_spend_logs_payload(
    ...
    if _should_store_prompts_and_responses_in_spend_logs():
        _proxy_server_request = cast(
            Optional[dict], mishikallm_params.get("proxy_server_request", {})
        )
