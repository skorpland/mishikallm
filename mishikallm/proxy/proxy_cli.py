...
config_filename = "mishikallm.secrets"
...
class MishikaLLMDatabaseConnectionPool(Enum):
...
class ProxyInitializationHelpers:
    @staticmethod
    def _echo_mishikallm_version():
        pkg_version = importlib.metadata.version("mishikallm")  # type: ignore
        click.echo(f"\nMishikaLLM: Current Version = {pkg_version}\n")

    @staticmethod
    def _run_health_check(host, port):
        print("\nMishikaLLM: Health Testing models in config")  # noqa
    ...
    def _run_test_chat_completion(
        ...
        click.echo(
            f"\nMishikaLLM: Making a test ChatCompletions request to your proxy. Model={request_model}"
        )
        import openai
        ...
        click.echo(f"\nMishikaLLM: response from proxy {response}")

        print(  # noqa
            f"\n MishikaLLM: Making a test ChatCompletions + streaming r equest to proxy. Model={request_model}"
        )
        ...
        for chunk in stream_response:
            click.echo(f"MishikaLLM: streaming response from proxy {chunk}")
    ...
    def _get_default_unvicorn_init_args(
        ...
        uvicorn_args = {
            "app": "mishikallm.proxy.proxy_server:app",
            "host": host,
            "port": port,
        }
    ...
    def _init_hypercorn_server(
        ...
        """
        Initialize mishikallm with `hypercorn`
        """
        import asyncio
        ...
        print(  # noqa
            f"\033[1;32mMishikaLLM Proxy: Starting server on {host}:{port} using Hypercorn\033[0m\n"  # noqa
        )  # noqa
        config = Config()
        ...
        if ssl_certfile_path is not None and ssl_keyfile_path is not None:
            print(  # noqa
                f"\033[1;32mMishikaLLM Proxy: Using SSL with certfile: {ssl_certfile_path} and keyfile: {ssl_keyfile_path}\033[0m\n"  # noqa
            )
    ...
    def _run_gunicorn_server(
        ...
        class StandaloneApplication(gunicorn.app.base.BaseApplication):
            def __init__(self, app, options=None):
                ...
                print()  # noqa
                print(  # noqa
                    '\033[1;34mMishikaLLM: Test your local proxy with: "mishikallm --test" This runs an openai.ChatCompletion request to your proxy [In a new terminal tab]\033[0m\n'
                )
                print(  # noqa
                    f"\033[1;34mMishikaLLM: Curl Command Test for your local proxy\n {curl_command} \033[0m\n"
                )
                print(  # noqa
                    "\033[1;34mDocs: https://docs.21t.cc/docs/simple_proxy\033[0m\n"
                )  # noqa
                print(  # noqa
            ...
            def load_config(self):
                # note: This Loads the gunicorn config - has nothing to do with MishikaLLM Proxy config
        ...
        print(  # noqa
            f"\033[1;32mMishikaLLM Proxy: Starting server on {host}:{port} with {num_workers} workers\033[0m\n"  # noqa
        )
        gunicorn_options = {
            "bind": f"{host}:{port}",
...
@click.option(
    "--model", "-m", default=None, help="The model name to pass to mishikallm expects"
)
@click.option(
    "--alias",
    default=None,
    help='The alias for the model - use this to give a mishikallm model name (e.g. "huggingface/codellama/CodeLlama-7b-Instruct-hf") a more user-friendly name ("codellama")',
)
@click.option(
    "--add_key", default=None, help="The model name to pass to mishikallm expects"
)
...
@click.option(
    ...
    "-c",
    default=None,
    help="Path to the proxy configuration file (e.g. config.yaml). Usage `mishikallm --config config.yaml`",
)
@click.option(
    "--max_budget",
    default=None,
...
def run_server(  # noqa: PLR0915
    ...
    if local:
        ...
        try:
            ...
            if "mishikallm[proxy]" in str(e):
                # user is missing a proxy dependency, ask them to pip install mishikallm[proxy]
                raise e
            else:
                # this is just a local/relative import error, user git cloned mishikallm
                from proxy_server import (
                    KeyManagementSettings,
                    ProxyConfig,
                    app,
    ...
    if version is True:
        ProxyInitializationHelpers._echo_mishikallm_version()
        return
    ...
    if test is True:
        ...
        try:
            ...
            raise ImportError(
                "uvicorn, gunicorn needs to be imported. Run - `pip install 'mishikallm[proxy]'`"
            )
        ...
        if iam_token_db_auth:
            from mishikallm.proxy.auth.rds_iam_token import generate_iam_auth_token
        ...
        from mishikallm.secret_managers.aws_secret_manager import decrypt_env_var
        ...
        if config is not None:
            ...
            try:
                ...
                raise ImportError(
                    "yaml needs to be imported. Run - `pip install 'mishikallm[proxy]'`"
                )
            ...
            ### MISHIKALLM SETTINGS ###
            mishikallm_settings = _config.get("mishikallm_settings", None)
            if (
                mishikallm_settings is not None
                and "json_logs" in mishikallm_settings
                and mishikallm_settings["json_logs"] is True
            ):
                import mishikallm

                mishikallm.json_logs = True

                mishikallm._turn_on_json()
            ### GENERAL SETTINGS ###
            ...
            db_connection_pool_limit = general_settings.get(
                ...
                MishikaLLMDatabaseConnectionPool.database_connection_pool_limit.value,
            )
            db_connection_timeout = general_settings.get(
                "database_connection_pool_timeout",
                MishikaLLMDatabaseConnectionPool.database_connection_pool_timeout.value,
            )
        ...
        if (
            ...
            if is_prisma_runnable:
                from mishikallm.proxy.db.check_migration import check_prisma_schema_diff
                from mishikallm.proxy.db.prisma_client import (
                    PrismaManager,
        ...
        from mishikallm.proxy.proxy_server import app  # noqa
