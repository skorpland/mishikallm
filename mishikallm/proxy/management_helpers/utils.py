...
from mishikallm.proxy._types import (  # key request types; user request types; team request types; customer request types
...
def get_new_internal_user_defaults(
    ...
    user_info = mishikallm.default_internal_user_params or {}

    returned_dict: SSOUserDefinedValues = {
        "models": user_info.get("models", None),
        "max_budget": user_info.get("max_budget", mishikallm.max_internal_user_budget),
        "budget_duration": user_info.get(
            "budget_duration", mishikallm.internal_user_budget_duration
        ),
...
async def add_new_member(
...
) -> Tuple[MishikaLLM_UserTable, Optional[MishikaLLM_TeamMembership]]:
    """
    ...
    """
    returned_user: Optional[MishikaLLM_UserTable] = None
    returned_team_membership: Optional[MishikaLLM_TeamMembership] = None
    ## ADD TEAM ID, to USER TABLE IF NEW ##
    if new_member.user_id is not None:
        new_user_defaults = get_new_internal_user_defaults(user_id=new_member.user_id)
        _returned_user = await prisma_client.db.mishikallm_usertable.upsert(
        ...
        if _returned_user is not None:
            returned_user = MishikaLLM_UserTable(**_returned_user.model_dump())
        ...
        if existing_user_row is None or (
            ...
            _returned_user = await prisma_client.db.mishikallm_usertable.update(
    ...
    if (
        ...
        response = await prisma_client.db.mishikallm_budgettable.create(
            data={
        ...
        returned_team_membership = MishikaLLM_TeamMembership(
...
async def send_management_endpoint_alert(
    ...
    """
    from mishikallm.proxy.proxy_server import premium_user, proxy_logging_obj
