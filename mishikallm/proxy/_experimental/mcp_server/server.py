...
from mishikallm.constants import MCP_TOOL_NAME_PREFIX
...
from mishikallm.types.mcp_server.mcp_server_manager import MCPInfo
from mishikallm.types.utils import StandardLoggingMCPToolCall
from mishikallm.utils import client

# Check if MCP is available
# "mcp" requires python 3.10 or higher, but several mishikallm users use python 3.8
...
if MCP_AVAILABLE:
    ...
    # required dependency for `mishikallm` pip package
    ######################################################
    ...
    server: Server = Server("mishikallm-mcp-server")
    ...
    async def call_mcp_tool(
        ...
        if mishikallm_logging_obj:
            mishikallm_logging_obj.model_call_details["mcp_tool_call_metadata"] = (
                standard_logging_mcp_tool_call
            )
            mishikallm_logging_obj.model_call_details["model"] = (
                f"{MCP_TOOL_NAME_PREFIX}: {standard_logging_mcp_tool_call.get('name') or ''}"
            )
            mishikallm_logging_obj.model_call_details["custom_llm_provider"] = (
    ...
    async def call_tool_rest_api(
        ...
        """
        from mishikallm.proxy.proxy_server import add_mishikallm_data_to_request, proxy_config
    ...
    options = InitializationOptions(
        server_name="mishikallm-mcp-server",
