...
from mishikallm.proxy.proxy_server import MishikaLLM_TeamTable, UserAPIKeyAuth
...
def can_modify_guardrails(team_obj: Optional[MishikaLLM_TeamTable]) -> bool:
    if team_obj is None:
        return True
        ...
        if "guardrails" in data["metadata"]:
            ...
            if isinstance(request_guardrails, dict):
                ...
                for _guardrail_name, should_run in request_guardrails.items():
                    ...
                    guardrail_item: GuardrailItem = mishikallm.guardrail_name_config_map[
                        _guardrail_name
                    ]
