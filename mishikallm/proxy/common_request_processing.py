...
from mishikallm.proxy._types import ProxyException, UserAPIKeyAuth
...
class ProxyBaseLLMRequestProcessing:
    ...
    def get_custom_headers(
        ...
        **kwargs,
        ...
        headers = {
            "x-mishikallm-call-id": call_id,
            "x-mishikallm-model-id": model_id,
            "x-mishikallm-cache-key": cache_key,
            "x-mishikallm-model-api-base": (
                api_base.split("?")[0] if api_base else None
            ),  # don't include query params, risk of leaking sensitive info
            "x-mishikallm-version": version,
            "x-mishikallm-model-region": model_region,
            "x-mishikallm-response-cost": str(response_cost),
            "x-mishikallm-key-tpm-limit": str(user_api_key_dict.tpm_limit),
            "x-mishikallm-key-rpm-limit": str(user_api_key_dict.rpm_limit),
            "x-mishikallm-key-max-budget": str(user_api_key_dict.max_budget),
            "x-mishikallm-key-spend": str(user_api_key_dict.spend),
            "x-mishikallm-response-duration-ms": str(
                hidden_params.get("_response_ms", None)
            ),
            "x-mishikallm-overhead-duration-ms": str(
                hidden_params.get("mishikallm_overhead_time_ms", None)
            ),
            "x-mishikallm-fastest_response_batch_completion": (
                str(fastest_response_batch_completion)
                if fastest_response_batch_completion is not None
                else None
            ),
            "x-mishikallm-timeout": str(timeout) if timeout is not None else None,
    ...
    async def common_processing_pre_call_logic(
    ...
    ) -> Tuple[dict, MishikaLLMLoggingObj]:
        self.data = await add_mishikallm_data_to_request(
            data=self.data,
            request=request,
        ...
        if (
            ...
            and self.data["model"] in mishikallm.model_alias_map
        ):
            self.data["model"] = mishikallm.model_alias_map[self.data["model"]]

        self.data["mishikallm_call_id"] = request.headers.get(
            "x-mishikallm-call-id", str(uuid.uuid4())
        )
        ...
        logging_obj, self.data = mishikallm.utils.function_setup(
        ...
        self.data["mishikallm_logging_obj"] = logging_obj
    ...
    async def base_process_llm_request(
        ...
        asyncio.create_task(
            proxy_logging_obj.update_request_status(
                mishikallm_call_id=self.data.get("mishikallm_call_id", ""), status="success"
            )
        )
        if (
            ...
            custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
                user_api_key_dict=user_api_key_dict,
                call_id=logging_obj.mishikallm_call_id,
                model_id=model_id,
                cache_key=cache_key,
                api_base=api_base,
                version=version,
    ...
    async def _handle_llm_api_exception(
        ...
        verbose_proxy_logger.exception(
            f"mishikallm.proxy.proxy_server._handle_llm_api_exception(): Exception occured - {str(e)}"
        )
        ...
        _mishikallm_logging_obj: Optional[MishikaLLMLoggingObj] = self.data.get(
            "mishikallm_logging_obj", None
        )
        custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=user_api_key_dict,
            call_id=(
                _mishikallm_logging_obj.mishikallm_call_id if _mishikallm_logging_obj else None
            ),
            version=version,
            response_cost=0,
            model_region=getattr(user_api_key_dict, "allowed_model_region", ""),
