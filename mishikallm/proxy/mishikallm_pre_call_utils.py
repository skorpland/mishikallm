...
from mishikallm.proxy._types import (
    AddTeamCallback,
    CommonProxyErrors,
    MishikallmDataForBackendLLMCall,
    SpecialHeaders,
...
from mishikallm.types.llms.anthropic import ANTHROPIC_API_HEADERS
...
if TYPE_CHECKING:
    from mishikallm.proxy.proxy_server import ProxyConfig as _ProxyConfig
...
def _get_metadata_variable_name(request: Request) -> str:
    ...
    For all /thread or /assistant endpoints we need to call this "mishikallm_metadata"
    ...
    if any(
        [
            mishikallm_metadata_route in request.url.path
            for mishikallm_metadata_route in MISHIKALLM_METADATA_ROUTES
        ]
    ):
        return "mishikallm_metadata"
    else:
...
def convert_key_logging_metadata_to_callback(
    ...
    ):  # assume 'success_and_failure' = mishikallm.callbacks
        if team_callback_settings_obj.success_callback is None:
            team_callback_settings_obj.success_callback = []
        if team_callback_settings_obj.failure_callback is None:
            team_callback_settings_obj.failure_callback = []
    ...
    for var, value in data.callback_vars.items():
        ...
        team_callback_settings_obj.callback_vars[var] = str(
            mishikallm.utils.get_secret(value, default_value=value) or value
        )
...
def clean_headers(
    headers: Headers, mishikallm_key_header_name: Optional[str] = None
) -> dict:
    """
    Removes mishikallm api key from headers
    """
    special_headers = [v.value.lower() for v in SpecialHeaders._member_map_.values()]
    special_headers = special_headers
    if mishikallm_key_header_name is not None:
        special_headers.append(mishikallm_key_header_name.lower())
    clean_headers = {}
...
class MishikaLLMProxyRequestSetup:
    @staticmethod
    def _get_timeout_from_request(headers: dict) -> Optional[float]:
        ...
        const openaiProvider = createOpenAI({
            baseURL: mishikaLLM.baseURL,
            apiKey: mishikaLLM.apiKey,
        ...
        ```
        """
        timeout_header = headers.get("x-mishikallm-timeout", None)
    ...
    def add_headers_to_llm_call(
        ...
        returned_headers = MishikaLLMProxyRequestSetup._get_forwardable_headers(headers)

        if mishikallm.add_user_information_to_llm_headers is True:
            mishikallm_logging_metadata_headers = (
                MishikaLLMProxyRequestSetup.get_sanitized_user_information_from_key(
                    user_api_key_dict=user_api_key_dict
                )
            )
            for k, v in mishikallm_logging_metadata_headers.items():
                if v is not None:
                    returned_headers["x-mishikallm-{}".format(k)] = v

        return returned_headers

    @staticmethod
    def add_mishikallm_data_for_backend_llm_call(
        *,
        headers: dict,
        ...
        - Adds org id
        """
        data = MishikallmDataForBackendLLMCall()
        if (
            general_settings
            and general_settings.get("forward_client_headers_to_llm_api") is True
        ):
            _headers = MishikaLLMProxyRequestSetup.add_headers_to_llm_call(
                headers, user_api_key_dict
            )
            if _headers != {}:
                data["headers"] = _headers
        _organization = MishikaLLMProxyRequestSetup.get_openai_org_id_from_headers(
    ...
    def add_key_level_controls(
        ...
        if "tags" in key_metadata and key_metadata["tags"] is not None:
            data[_metadata_variable_name][
                "tags"
            ] = MishikaLLMProxyRequestSetup._merge_tags(
    ...
    def add_request_tag_to_metadata(
        ...
        if "x-mishikallm-tags" in headers:
            if isinstance(headers["x-mishikallm-tags"], str):
                _tags = headers["x-mishikallm-tags"].split(",")
                tags = [tag.strip() for tag in _tags]
            elif isinstance(headers["x-mishikallm-tags"], list):
                tags = headers["x-mishikallm-tags"]
...
async def add_mishikallm_data_to_request(  # noqa: PLR0915
    data: dict,
    request: Request,
    user_api_key_dict: UserAPIKeyAuth,
    proxy_config: ProxyConfig,
    ...
    """
    Adds MishikaLLM-specific data to the request.
    ...
    from mishikallm.proxy.proxy_server import llm_router, premium_user
    ...
    _headers = clean_headers(
        ...
        mishikallm_key_header_name=(
            general_settings.get("mishikallm_key_header_name")
    ...
    data.update(
        MishikaLLMProxyRequestSetup.add_mishikallm_data_for_backend_llm_call(
            headers=_headers,
            user_api_key_dict=user_api_key_dict,
    ...
    data[_metadata_variable_name]["mishikallm_api_version"] = version
    ...
    if "tags" in team_metadata and team_metadata["tags"] is not None:
        data[_metadata_variable_name]["tags"] = MishikaLLMProxyRequestSetup._merge_tags(
            request_tags=data[_metadata_variable_name].get("tags"),
    ...
    # Add the OTEL Parent Trace before sending it MishikaLLM
    data[_metadata_variable_name][
        "mishikallm_parent_otel_span"
    ...
    tags = MishikaLLMProxyRequestSetup.add_request_tag_to_metadata(
    ...
    verbose_proxy_logger.debug(
        "[PROXY] returned data from mishikallm_pre_call_utils: %s", data
    )
    ...
    asyncio.create_task(
        service_logger_obj.async_service_success_hook(
            ...
            call_type="add_mishikallm_data_to_request",
...
def _add_otel_traceparent_to_data(data: dict, request: Request):
    from mishikallm.proxy.proxy_server import open_telemetry_logger

    if data is None:
        return
    if open_telemetry_logger is None:
        # if user is not use OTEL don't send extra_headers
        # relevant issue: https://github.com/skorpland/mishikallm/issues/4448
        return

    if mishikallm.forward_traceparent_to_llm_provider is True:
        if request.headers:
            if "traceparent" in request.headers:
                # we want to forward this to the LLM Provider
                # Relevant issue: https://github.com/skorpland/mishikallm/issues/4419
