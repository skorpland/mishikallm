...
from mishikallm.proxy.auth.auth_checks import can_user_call_model
...
from mishikallm.proxy.utils import PrismaClient
...
async def new_organization(
    ...
    user_object_correct_type: Optional[MishikaLLM_UserTable] = None
    ...
    if data.budget_id is None:
        ...
        """
        budget_params = MishikaLLM_BudgetTable.model_fields.keys()

        # Only include Budget Params when creating an entry in mishikallm_budgettable
        _json_data = data.json(exclude_none=True)
        _budget_data = {k: v for k, v in _json_data.items() if k in budget_params}
        budget_row = MishikaLLM_BudgetTable(**_budget_data)
    ...
    organization_row = MishikaLLM_OrganizationTable(
    ...
    response = await prisma_client.db.mishikallm_organizationtable.create(
        data={
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=MishikaLLM_OrganizationTableWithMembers,
)
async def update_organization(
    data: MishikaLLM_OrganizationTableUpdate,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
):
    """
    ...
    response = await prisma_client.db.mishikallm_organizationtable.update(
        where={"organization_id": data.organization_id},
        data=updated_organization_row,
        include={"members": True, "teams": True, "mishikallm_budget_table": True},
    )
...
@router.delete(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[MishikaLLM_OrganizationTableWithMembers],
)
async def delete_organization(
    ...
    for organization_id in data.organization_ids:
        ...
        await prisma_client.db.mishikallm_teamtable.delete_many(
        ...
        deleted_org = await prisma_client.db.mishikallm_organizationtable.delete(
            where={"organization_id": organization_id},
            include={"members": True, "teams": True, "mishikallm_budget_table": True},
        )
...
async def list_organization(
    ...
    if user_api_key_dict.user_role == MishikallmUserRoles.PROXY_ADMIN:
        response = await prisma_client.db.mishikallm_organizationtable.find_many(
            include={"mishikallm_budget_table": True, "members": True, "teams": True}
        )
    # if internal user - get orgs they are a member of
    else:
        org_memberships = (
            await prisma_client.db.mishikallm_organizationmembership.find_many(
                where={"user_id": user_api_key_dict.user_id}
            )
        )
        org_objects = await prisma_client.db.mishikallm_organizationtable.find_many(
            where={
                "organization_id": {
...
async def info_organization(organization_id: str):
    ...
    response: Optional[
        MishikaLLM_OrganizationTableWithMembers
    ] = await prisma_client.db.mishikallm_organizationtable.find_unique(
        where={"organization_id": organization_id},
        include={"mishikallm_budget_table": True, "members": True, "teams": True},
    )
    ...
    response_pydantic_obj = MishikaLLM_OrganizationTableWithMembers(
        **response.model_dump()
    )
...
async def organization_member_add(
    ...
    - member: Union[List[Member], Member] (required)
        - role: Literal[MishikallmUserRoles] (required)
    ...
    2. Creates a new Internal User if the user_id or user_email is not found in MishikaLLM_UserTable
    3. Add Internal User to the `MishikaLLM_OrganizationMembership` table
    """
    try:
        from mishikallm.proxy.proxy_server import prisma_client
...
async def find_member_if_email(
    ...
    """
    Find a member if the user_email is in MishikaLLM_UserTable
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Unique user not found for user_email={user_email}. Potential duplicate OR non-existent user_email in MishikaLLM_UserTable. Use 'user_id' instead."
            },
        )
    existing_user_email_row_pydantic = MishikaLLM_UserTable(
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=MishikaLLM_OrganizationMembershipTable,
)
@management_endpoint_wrapper
    ...
    try:
        ...
        try:
            existing_organization_membership = (
                await prisma_client.db.mishikallm_organizationmembership.find_unique(
                    where={
        ...
        if data.role is not None:
            await prisma_client.db.mishikallm_organizationmembership.update(
                where={
                    "user_id_organization_id": {
                        "user_id": data.user_id,
                        "organization_id": data.organization_id,
        ...
        final_organization_membership_pydantic = MishikaLLM_OrganizationMembershipTable(
...
async def organization_member_delete(
    ...
    try:
        ...
        member_to_delete = await prisma_client.db.mishikallm_organizationmembership.delete(
            where={
                "user_id_organization_id": {
                    "user_id": data.user_id,
                    "organization_id": data.organization_id,
...
) -> Tuple[MishikaLLM_UserTable, MishikaLLM_OrganizationMembershipTable]:
    """
    Add a member to an organization

    - Checks if member.user_id or member.user_email is in MishikaLLM_UserTable
    - If not found, create a new user in MishikaLLM_UserTable
    - Add user to organization in MishikaLLM_OrganizationMembership
    """

    try:
        user_object: Optional[MishikaLLM_UserTable] = None
        existing_user_id_row = None
        existing_user_email_row = None
        ## Check if user exists in MishikaLLM_UserTable - user exists - either the user_id or user_email is in MishikaLLM_UserTable
        if member.user_id is not None:
            existing_user_id_row = await prisma_client.db.mishikallm_usertable.find_unique(
        ...
        if existing_user_id_row is None and member.user_email is not None:
            try:
                ...
                raise ValueError(
                    f"Potential NON-Existent or Duplicate user email in DB: Error finding a unique instance of user_email={member.user_email} in MishikaLLM_UserTable.: {e}"
                )
        ...
        if existing_user_id_row is None and existing_user_email_row is None:
            ...
            if _returned_user is not None:
                user_object = MishikaLLM_UserTable(**_returned_user.model_dump())
            ...
            user_object = MishikaLLM_UserTable(**existing_user_email_row.model_dump())
        elif existing_user_id_row is not None:
            user_object = MishikaLLM_UserTable(**existing_user_id_row.model_dump())
        else:
            raise HTTPException(
                status_code=404,
                detail={
        ...
        if user_object is None:
            raise ValueError(
                f"User does not exist in MishikaLLM_UserTable. user_id={member.user_id} and user_email={member.user_email}"
            )

        # Add user to organization
        _organization_membership = (
            await prisma_client.db.mishikallm_organizationmembership.create(
                data={
        ...
        organization_membership = MishikaLLM_OrganizationMembershipTable(
