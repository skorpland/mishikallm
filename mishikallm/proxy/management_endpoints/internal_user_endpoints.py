...
These are members of a Team on MishikaLLM
...
from mishikallm.proxy.management_endpoints.common_daily_activity import get_daily_activity
...
from mishikallm.types.proxy.management_endpoints.common_daily_activity import (
    ...
    MishikaLLM_DailyUserSpend,
    MetricWithMetadata,
    SpendAnalyticsPaginatedResponse,
    SpendMetrics,
)
from mishikallm.types.proxy.management_endpoints.internal_user_endpoints import (
    UserListResponse,
)
...
async def create_internal_user_audit_log(
    ...
    mishikallm_changed_by: Optional[str],
    user_api_key_dict: UserAPIKeyAuth,
    mishikallm_proxy_admin_name: Optional[str],
    ...
    - user_row: MishikaLLM_UserTable - The user row to create the audit log for.
    - mishikallm_changed_by: Optional[str] - The user id of the user who is changing the user.
    - user_api_key_dict: UserAPIKeyAuth - The user api key dictionary.
    - mishikallm_proxy_admin_name: Optional[str] - The name of the proxy admin.
    """
    if not mishikallm.store_audit_logs:
        return
    ...
    await create_audit_log_for_update(
        request_data=MishikaLLM_AuditLogs(
            ...
            table_name=MishikallmTableNames.USER_TABLE_NAME,
            object_id=user_id,
            action=action,
...
def _update_internal_new_user_params(data_json: dict, data: NewUserRequest) -> dict:
    ...
    if data.user_role and data.user_role.is_internal_user_role:
        ...
        if mishikallm.default_internal_user_params:
            for key, value in mishikallm.default_internal_user_params.items():
    ...
    if "max_budget" in data_json and data_json["max_budget"] is None:
        if is_internal_user and mishikallm.max_internal_user_budget is not None:
            data_json["max_budget"] = mishikallm.max_internal_user_budget

    if "budget_duration" in data_json and data_json["budget_duration"] is None:
        if is_internal_user and mishikallm.internal_user_budget_duration is not None:
            data_json["budget_duration"] = mishikallm.internal_user_budget_duration
...
async def _check_duplicate_user_email(
    ...
    if user_email:
        ...
        existing_user = await prisma_client.db.mishikallm_usertable.find_first(
...
async def new_user(
    ...
    Internal Users can access MishikaLLM Admin UI to make keys, request access to models.
    ...
    - user_role: Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role here: `https://github.com/skorpland/mishikallm/mishikallm/proxy/_types.py#L20`
    ...
    - aliases: Optional[dict] - Model aliases for the user - [Docs](https://mishikallm.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    - model_max_budget: Optional[dict] - Model-specific max budget for user. [Docs](https://docs.21t.cc/docs/proxy/users#add-model-specific-budgets-to-keys)
    - model_rpm_limit: Optional[float] - Model-specific rpm limit for user. [Docs](https://docs.21t.cc/docs/proxy/users#add-model-specific-limits-to-keys)
    - model_tpm_limit: Optional[float] - Model-specific tpm limit for user. [Docs](https://docs.21t.cc/docs/proxy/users#add-model-specific-limits-to-keys)
    ...
    try:
            ...
            try:
                ...
                if e.status_code == 400 and (
                    ...
                    verbose_proxy_logger.debug(
                        "mishikallm.proxy.management_endpoints.internal_user_endpoints.new_user(): User already exists in team - {}".format(
                            str(e)
                        )
                    )
                else:
                    verbose_proxy_logger.debug(
                        "mishikallm.proxy.management_endpoints.internal_user_endpoints.new_user(): Exception occured - {}".format(
                            str(e)
                        )
                    )
        ...
        if data.send_invite_email is True:
            ...
            if "email" not in general_settings.get("alerting", []):
                raise ValueError(
                    "Email alerting not setup on config.yaml. Please set `alerting=['email']. \nDocs: https://docs.21t.cc/docs/proxy/email`"
                )

            event = WebhookEvent(
                event="internal_user_created",
                event_group=Mishikallm_EntityType.USER,
                event_message="Welcome to MishikaLLM Proxy",
        ...
        try:
            ...
            user_row: BaseModel = await prisma_client.db.mishikallm_usertable.find_first(
                where={"user_id": response["user_id"]}
            )

            user_row_mishikallm_typed = MishikaLLM_UserTable(
                **user_row.model_dump(exclude_none=True)
            )
            asyncio.create_task(
                create_internal_user_audit_log(
                    user_id=user_row_mishikallm_typed.user_id,
                    action="created",
                    mishikallm_changed_by=user_api_key_dict.user_id,
                    user_api_key_dict=user_api_key_dict,
                    mishikallm_proxy_admin_name=mishikallm_proxy_admin_name,
                    before_value=None,
                    after_value=user_row_mishikallm_typed.model_dump_json(
                        exclude_none=True
                    ),
                )
            )
...
async def ui_get_available_role(
    ...
    _data_to_return = {}
    for role in MishikallmUserRoles:
...
def get_team_from_list(
    team_list: Optional[Union[List[MishikaLLM_TeamTable], List[TeamListResponseObject]]],
    team_id: str,
) -> Optional[Union[MishikaLLM_TeamTable, MishikaLLM_TeamMembership]]:
...
async def user_info(
    ...
    try:
        ...
        if (
            user_id is None
            and user_api_key_dict.user_role == MishikallmUserRoles.PROXY_ADMIN
        ):
        ...
        team_list = []
        team_id_list = []
        from mishikallm.proxy.management_endpoints.team_endpoints import list_team
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.user_info(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def _get_user_info_for_proxy_admin():
    ...
    - get all teams in MishikaLLM_TeamTable
    - get all keys in MishikaLLM_VerificationToken table
    ...
    sql_query = """
        SELECT 
            (SELECT json_agg(t.*) FROM "MishikaLLM_TeamTable" t) as teams,
            (SELECT json_agg(k.*) FROM "MishikaLLM_VerificationToken" k WHERE k.team_id != 'mishikallm-dashboard' OR k.team_id IS NULL) as keys
    """
    if prisma_client is None:
        raise Exception(
            "Database not connected. Connect a database to your proxy - https://docs.21t.cc/docs/simple_proxy#managing-auth---virtual-keys"
    ...
    # cast all keys to MishikaLLM_VerificationToken
    keys_in_db = []
    for key in _keys_in_db:
        if key.get("models") is None:
            key["models"] = []
        keys_in_db.append(MishikaLLM_VerificationToken(**key))

    # cast all teams to MishikaLLM_TeamTable
    _teams_in_db: List = results[0]["teams"] or []
    _teams_in_db = [MishikaLLM_TeamTable(**team) for team in _teams_in_db]
...
def _process_keys_for_user_info(
    keys: Optional[List[MishikaLLM_VerificationToken]],
    all_teams: Optional[Union[List[MishikaLLM_TeamTable], List[TeamListResponseObject]]],
):
    from mishikallm.proxy.proxy_server import general_settings, mishikallm_master_key_hash
    ...
    if keys is None:
        ...
        for key in keys:
            if (
                key.token == mishikallm_master_key_hash
            ...
            if (
                ...
                and _key["team_id"] != "mishikallm-dashboard"
            ):
...
def _update_internal_user_params(data_json: dict, data: UpdateUserRequest) -> dict:
    ...
    for k, v in data_json.items():
        if (
            ...
            and k not in MishikaLLM_ManagementEndpoint_MetadataFields
        ):  # models default to [], spend defaults to 0, we should not reset these values
            non_default_values[k] = v

    is_internal_user = False
    if data.user_role == MishikallmUserRoles.INTERNAL_USER:
        is_internal_user = True
    ...
    if "max_budget" not in non_default_values:
        if (
            is_internal_user and mishikallm.max_internal_user_budget is not None
        ):  # applies internal user limits, if user role updated
            non_default_values["max_budget"] = mishikallm.max_internal_user_budget
...
async def user_update(
    ...
    --data '{
        "user_id": "test-mishikallm-user-4",
    ...
    """
    from mishikallm.proxy.proxy_server import mishikallm_proxy_admin_name, prisma_client
    ...
    try:
        ...
        if data.user_id is not None:
            existing_user_row = await prisma_client.db.mishikallm_usertable.find_first(
                where={"user_id": data.user_id}
            )
            if existing_user_row is not None:
                existing_user_row = MishikaLLM_UserTable(
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.user_update(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def get_user_key_counts(
    ...
    for user_id in user_ids:
        count = await prisma_client.db.mishikallm_verificationtoken.count(
            where={
                "user_id": user_id,
                "OR": [
                    {"team_id": None},
...
async def get_users(
    ...
    users = await prisma_client.db.mishikallm_usertable.find_many(
    ...
    total_count = await prisma_client.db.mishikallm_usertable.count(where=where_conditions)
    ...
    # Prepare response
    user_list: List[MishikaLLM_UserTableWithKeyCount] = []
    if users is not None:
        for user in users:
            user_list.append(
                MishikaLLM_UserTableWithKeyCount(
...
async def delete_user(
    ...
    await prisma_client.db.mishikallm_verificationtoken.delete_many(
        where={"user_id": {"in": data.user_ids}}
    )

    ## DELETE ASSOCIATED INVITATION LINKS
    await prisma_client.db.mishikallm_invitationlink.delete_many(
        where={"user_id": {"in": data.user_ids}}
    )

    ## DELETE ASSOCIATED ORGANIZATION MEMBERSHIPS
    await prisma_client.db.mishikallm_organizationmembership.delete_many(
        where={"user_id": {"in": data.user_ids}}
    )

    ## DELETE USERS
    deleted_users = await prisma_client.db.mishikallm_usertable.delete_many(
        where={"user_id": {"in": data.user_ids}}
    )
...
async def add_internal_user_to_organization(
    user_id: str,
    organization_id: str,
    user_role: MishikallmUserRoles,
):
    """
    Helper function to add an internal user to an organization

    Adds the user to MishikaLLM_OrganizationMembership table
    ...
    try:
        ...
        new_membership = await prisma_client.db.mishikallm_organizationmembership.create(
            data={
                "user_id": user_id,
...
@router.get(
    ...
    include_in_schema=False,
    responses={
        200: {"model": List[MishikaLLM_UserTableFiltered]},
    },
)
async def ui_view_users(
    ...
    try:
        ...
        users: Optional[List[BaseModel]] = (
            await prisma_client.db.mishikallm_usertable.find_many(
                where=where_conditions,
                skip=skip,
                take=page_size,
        ...
        return [MishikaLLM_UserTableFiltered(**user.model_dump()) for user in users]
...
def update_metrics(
    group_metrics: SpendMetrics, record: MishikaLLM_DailyUserSpend
) -> SpendMetrics:
...
def update_breakdown_metrics(
    ...
    record: MishikaLLM_DailyUserSpend,
...
async def get_user_daily_activity(
    ...
    try:
        ...
        return await get_daily_activity(
            ...
            table_name="mishikallm_dailyuserspend",
