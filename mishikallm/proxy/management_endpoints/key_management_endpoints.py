...
from mishikallm.caching import DualCache
from mishikallm.constants import LENGTH_OF_MISHIKALLM_GENERATED_KEY, UI_SESSION_TOKEN_TEAM_ID
from mishikallm.mishikallm_core_utils.duration_parser import duration_in_seconds
...
from mishikallm.proxy.hooks.key_management_event_hooks import KeyManagementEventHooks
...
from mishikallm.proxy.management_helpers.utils import management_endpoint_wrapper
from mishikallm.proxy.spend_tracking.spend_tracking_utils import _is_master_key
...
from mishikallm.types.router import Deployment
...
def _is_team_key(data: Union[GenerateKeyRequest, MishikaLLM_VerificationToken]):
    return data.team_id is not None


def _get_user_in_team(
    team_table: MishikaLLM_TeamTableCachedObj, user_id: Optional[str]
) -> Optional[Member]:
    if user_id is None:
        return None
...
def _team_key_operation_team_member_check(
    ...
    team_table: MishikaLLM_TeamTableCachedObj,
...
def _team_key_generation_check(
    ...
    if user_api_key_dict.user_role == MishikallmUserRoles.PROXY_ADMIN.value:
        return True
    if (
        mishikallm.key_generation_settings is not None
        and "team_key_generation" in mishikallm.key_generation_settings
    ):
        _team_key_generation = mishikallm.key_generation_settings["team_key_generation"]
    else:
...
def _personal_key_membership_check(
    ...
    if user_api_key_dict.user_role not in personal_key_generation["allowed_user_roles"]:
        raise HTTPException(
            status_code=400,
            detail=f"Personal key creation has been restricted by admin. Allowed roles={mishikallm.key_generation_settings['personal_key_generation']['allowed_user_roles']}. Your role={user_api_key_dict.user_role}",  # type: ignore
        )
...
def _personal_key_generation_check(
    ...
    if (
        mishikallm.key_generation_settings is None
        or mishikallm.key_generation_settings.get("personal_key_generation") is None
    ):
        return True

    _personal_key_generation = mishikallm.key_generation_settings["personal_key_generation"]  # type: ignore
...
def key_generation_check(
    team_table: Optional[MishikaLLM_TeamTableCachedObj],
    user_api_key_dict: UserAPIKeyAuth,
    data: GenerateKeyRequest,
    route: KeyManagementRoutes,
) -> bool:
    ...
    if is_team_key:
        if team_table is None and mishikallm.key_generation_settings is not None:
            raise HTTPException(
                status_code=400,
...
async def generate_key_fn(  # noqa: PLR0915
    ...
    Docs: https://docs.21t.cc/docs/proxy/virtual_keys
    ...
    - aliases: Optional[dict] - Any alias mappings, on top of anything in the config.yaml model list. - https://docs.21t.cc/docs/proxy/virtual_keys#managing-auth---upgradedowngrade-models
    - config: Optional[dict] - any key-specific configs, overrides config in config.yaml
    - spend: Optional[int] - Amount spent by key. Default is 0. Will be updated by proxy whenever key is used. https://docs.21t.cc/docs/proxy/virtual_keys#managing-auth---tracking-spend
    ...
    - allowed_cache_controls: Optional[list] - List of allowed cache control values. Example - ["no-cache", "no-store"]. See all values - https://docs.21t.cc/docs/proxy/caching#turn-on--off-caching-per-request
    ...
    - enforced_params: Optional[List[str]] - List of enforced params for the key (Enterprise only). [Docs](https://docs.21t.cc/docs/proxy/enterprise#enforce-required-params-for-llm-requests)
    ...
    try:
        ...
        team_table: Optional[MishikaLLM_TeamTableCachedObj] = None
        ...
        if mishikallm.default_key_generate_params is not None:
            for elem in data:
                key, value = elem
                if value is None and key in [
                    ...
                    setattr(
                        data, key, mishikallm.default_key_generate_params.get(key, None)
                    )
                elif key == "models" and value == []:
                    setattr(data, key, mishikallm.default_key_generate_params.get(key, []))
                elif key == "metadata" and value == {}:
                    setattr(data, key, mishikallm.default_key_generate_params.get(key, {}))

        # check if user set default key/generate params on config.yaml
        if mishikallm.upperbound_key_generate_params is not None:
            for elem in data:
                key, value = elem
                upperbound_value = getattr(
                    mishikallm.upperbound_key_generate_params, key, None
                )
        ...
        # TODO: @ishaan-jaff: Migrate all budget tracking to use MishikaLLM_BudgetTable
        _budget_id = data.budget_id
        if prisma_client is not None and data.soft_budget is not None:
            # create the Budget Row for the MishikaLLM Verification Token
            budget_row = MishikaLLM_BudgetTable(
            ...
            _budget = await prisma_client.db.mishikallm_budgettable.create(
                data={
        ...
        asyncio.create_task(
            KeyManagementEventHooks.async_key_generated_hook(
                ...
                response=response,
                user_api_key_dict=user_api_key_dict,
                mishikallm_changed_by=mishikallm_changed_by,
            )
        )
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.generate_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
def prepare_metadata_fields(
    ...
    """
    Check MishikaLLM_ManagementEndpoint_MetadataFields (proxy/_types.py) for fields that are allowed to be updated
    """
    ...
    try:
        for k, v in data_json.items():
            if k in MishikaLLM_ManagementEndpoint_MetadataFields:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.prepare_metadata_fields(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def update_key_fn(
    ...
    - guardrails: Optional[List[str]] - List of active guardrails for the key
    - blocked: Optional[bool] - Whether the key is blocked
    - aliases: Optional[dict] - Model aliases for the key - [Docs](https://mishikallm.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.update_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def delete_key_fn(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.delete_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
-H "Authorization: Bearer sk-02Wr4IAlN3NvPXvL5JVvDA"
    ...
    try:
        ...
        key_info = await prisma_client.db.mishikallm_verificationtoken.find_unique(
        ...
        if llm_router._is_model_access_group_for_wildcard_route(
            ...
            if not premium_user:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail={
                        "error": "Setting a model access group on a wildcard model is only available for MishikaLLM Enterprise users.{}".format(
...
async def generate_key_helper_fn(  # noqa: PLR0915
    ...
    budget_id: Optional[float] = None,  # budget id <-> MishikaLLM_BudgetTable
    ...
    if prisma_client is None:
        raise Exception(
            "Connect Proxy to database to generate keys - https://docs.21t.cc/docs/proxy/virtual_keys "
        )
    ...
    if token is None:
        if key is not None:
            ...
            token = f"sk-{secrets.token_urlsafe(LENGTH_OF_MISHIKALLM_GENERATED_KEY)}"
    ...
    try:
        ...
        if isinstance(saved_token["permissions"], str):
            if (
                ...
                raise ValueError(
                    "get_spend_routes permission is only available for MishikaLLM Enterprise users"
                )
        ...
        if prisma_client is not None:
            ...
            if user_id == mishikallm_proxy_budget_name or (
                table_name is not None and table_name == "user"
            ):
                # do not create a key for mishikallm_proxy_budget_name or if table name is set to just 'user'
                # we only need to ensure this exists in the user table
                # the MishikaLLM_VerificationToken table will increase in size if we don't do this check
                return user_data
            ...
            key_data["mishikallm_budget_table"] = getattr(
                create_key_response, "mishikallm_budget_table", None
            )
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.generate_key_helper_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def _team_key_deletion_check(
    user_api_key_dict: UserAPIKeyAuth,
    key_info: MishikaLLM_VerificationToken,
    ...
    if is_team_key and key_info.team_id is not None:
        ...
        if (
            ...
            _team_key_generation = mishikallm.key_generation_settings[
                "team_key_generation"
            ]
        else:
            _team_key_generation = TeamUIKeyGenerationConfig(
...
async def delete_verification_tokens(
    ...
    user_api_key_cache: DualCache,
    user_api_key_dict: UserAPIKeyAuth,
) -> Tuple[Optional[Dict], List[MishikaLLM_VerificationToken]]:
    """
    ...
    Returns:
        Tuple[Optional[Dict], List[MishikaLLM_VerificationToken]]:
            ...
            List[MishikaLLM_VerificationToken]:
    ...
    try:
        if prisma_client:
            ...
            ] = await prisma_client.db.mishikallm_verificationtoken.find_many(
            ...
            # else
                ...
                for key in _keys_being_deleted:
                    ...
                    async def _delete_key(key: MishikaLLM_VerificationToken):
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.delete_verification_tokens(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def delete_key_aliases(
    ...
    _keys_being_deleted = await prisma_client.db.mishikallm_verificationtoken.find_many(
...
async def _rotate_master_key(
    ...
    2. Decrypt the values
        - ModelTable
            - [{"model_name": "str", "mishikallm_params": {}}]
        - ConfigTable
    3. Encrypt the values with the new master key
    4. Update the values in the DB
    """
    from mishikallm.proxy.proxy_server import proxy_config

    try:
        models: Optional[
            List
        ] = await prisma_client.db.mishikallm_proxymodeltable.find_many()
    except Exception:
        models = None
    ...
    if models:
        ...
        await prisma_client.db.mishikallm_proxymodeltable.delete_many()
        verbose_proxy_logger.info("Creating %s models", len(new_models))
        await prisma_client.db.mishikallm_proxymodeltable.create_many(
            data=new_models,
        )
    # 3. process config table
    try:
        config = await prisma_client.db.mishikallm_config.find_many()
    except Exception:
        config = None
    ...
    if config:
        ...
        if environment_variables_dict:
            ...
            if encrypted_env_vars:
                await prisma_client.db.mishikallm_config.update(
...
async def regenerate_key_fn(
    ...
    try:
        ...
        _key_in_db = await prisma_client.db.mishikallm_verificationtoken.find_unique(
        ...
        new_token = f"sk-{secrets.token_urlsafe(LENGTH_OF_MISHIKALLM_GENERATED_KEY)}"
        ...
        updated_token = await prisma_client.db.mishikallm_verificationtoken.update(
            where={"token": hashed_api_key},
...
async def validate_key_list_check(
    ...
    complete_user_info_db_obj: Optional[
        BaseModel
    ] = await prisma_client.db.mishikallm_usertable.find_unique(
    ...
    complete_user_info = MishikaLLM_UserTable(**complete_user_info_db_obj.model_dump())
...
async def get_admin_team_ids(
    complete_user_info: Optional[MishikaLLM_UserTable],
    user_api_key_dict: UserAPIKeyAuth,
    prisma_client: PrismaClient,
) -> List[str]:
    """
    ...
    teams: Optional[
        List[BaseModel]
    ] = await prisma_client.db.mishikallm_teamtable.find_many(
        where={"team_id": {"in": complete_user_info.teams}}
    )
    if teams is None:
        return []

    teams_pydantic_obj = [MishikaLLM_TeamTable(**team.model_dump()) for team in teams]
...
async def list_keys(
    ...
    try:
        ...
        if user_id is None and user_api_key_dict.user_role not in [
            MishikallmUserRoles.PROXY_ADMIN.value,
            MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY.value,
        ]:
            user_id = user_api_key_dict.user_id
...
async def _list_key_helper(
    ...
    total_count = await prisma_client.db.mishikallm_verificationtoken.count(
        where=where  # type: ignore
    )
...
async def block_key(
    ...
    if mishikallm.store_audit_logs is True:
        ...
        record = await prisma_client.db.mishikallm_verificationtoken.find_unique(
        ...
        asyncio.create_task(
            create_audit_log_for_update(
                request_data=MishikaLLM_AuditLogs(
                    ...
                    table_name=MishikallmTableNames.KEY_TABLE_NAME,
    ...
    record = await prisma_client.db.mishikallm_verificationtoken.update(
...
async def test_key_logging(
    ...
    from mishikallm.proxy.proxy_server import general_settings, proxy_config
    ...
    try:
        data = {
            "model": "openai/mishikallm-key-health-test",
            "messages": [
                {
                    "role": "user",
                    "content": "Hello, this is a test from mishikallm /key/health. No LLM API call was made for this",
                }
            ],
...
async def _enforce_unique_key_alias(
    ...
    if key_alias is not None and prisma_client is not None:
        ...
        existing_key = await prisma_client.db.mishikallm_verificationtoken.find_first(
            where=where_clause
        )
...
def validate_model_max_budget(model_max_budget: Optional[Dict]) -> None:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid model_max_budget: {str(e)}. Example of valid model_max_budget: https://docs.21t.cc/docs/proxy/users"
        )
