...
from mishikallm.constants import MISHIKALLM_PROXY_ADMIN_NAME
from mishikallm.proxy._types import (
    CommonProxyErrors,
    MishikaLLM_ProxyModelTable,
...
from mishikallm.proxy.common_utils.encrypt_decrypt_utils import encrypt_value_helper
from mishikallm.proxy.management_endpoints.common_utils import _is_user_team_admin
from mishikallm.proxy.management_endpoints.team_endpoints import (
    team_model_add,
    update_team,
)
from mishikallm.proxy.management_helpers.audit_logs import create_object_audit_log
from mishikallm.proxy.utils import PrismaClient
from mishikallm.types.router import (
    Deployment,
    DeploymentTypedDict,
    MishikaLLMParamsTypedDict,
    updateDeployment,
)
from mishikallm.utils import get_utc_datetime
...
async def get_db_model(
    ...
    db_model = cast(
        Optional[BaseModel],
        await prisma_client.db.mishikallm_proxymodeltable.find_unique(
...
def update_db_model(
    ...
    merged_deployment_dict = DeploymentTypedDict(
        ...
        mishikallm_params=MishikaLLMParamsTypedDict(
            **db_model.mishikallm_params.model_dump(exclude_none=True)  # type: ignore
        ),
    )
    # update model name
    ...
    # update mishikallm params
    if updated_patch.mishikallm_params:
        # Encrypt any sensitive values
        encrypted_params = {
            k: encrypt_value_helper(v)
            for k, v in updated_patch.mishikallm_params.model_dump(
                exclude_none=True
            ).items()
        }

        merged_deployment_dict["mishikallm_params"].update(encrypted_params)  # type: ignore
    ...
    if "mishikallm_params" in merged_deployment_dict:
        prisma_compatible_model_dict["mishikallm_params"] = json.dumps(
            merged_deployment_dict["mishikallm_params"]
        )
...
async def patch_model(
    ...
    try:
        ...
        update_data["updated_by"] = (
            user_api_key_dict.user_id or mishikallm_proxy_admin_name
        )
        update_data["updated_at"] = cast(str, get_utc_datetime())

        # Perform partial update
        updated_model = await prisma_client.db.mishikallm_proxymodeltable.update(
            where={"model_id": model_id},
            data=update_data,
        )
...
async def _add_model_to_db(
...
) -> Optional[MishikaLLM_ProxyModelTable]:
    # encrypt mishikallm params #
    _mishikallm_params_dict = model_params.mishikallm_params.dict(exclude_none=True)
    _orignal_mishikallm_model_name = model_params.mishikallm_params.model
    for k, v in _mishikallm_params_dict.items():
        encrypted_value = encrypt_value_helper(
            value=v, new_encryption_key=new_encryption_key
        )
        model_params.mishikallm_params[k] = encrypted_value
    _data: dict = {
        "model_id": model_params.model_info.id,
        "model_name": model_params.model_name,
        "mishikallm_params": model_params.mishikallm_params.model_dump_json(exclude_none=True),  # type: ignore
    ...
    if should_create_model_in_db:
        model_response = await prisma_client.db.mishikallm_proxymodeltable.create(
            data=_data  # type: ignore
        )
    else:
        model_response = MishikaLLM_ProxyModelTable(**_data)
...
class ModelManagementAuthChecks:
    ...
    def can_user_make_team_model_call(
        team_id: str,
        user_api_key_dict: UserAPIKeyAuth,
        team_obj: Optional[MishikaLLM_TeamTable] = None,
    ...
    async def can_user_make_model_call(
        ...
        if (
            ...
            team_obj_row = await prisma_client.db.mishikallm_teamtable.find_unique(
                where={"team_id": model_params.model_info.team_id}
            )
            ...
            team_obj = MishikaLLM_TeamTable(**team_obj_row.model_dump())
        ...
        elif user_api_key_dict.user_role != MishikallmUserRoles.PROXY_ADMIN:
            raise HTTPException(
                status_code=403,
                detail={
    ...
    try:
        ...
        if prisma_client is None:
            raise HTTPException(
                status_code=500,
                detail={
                    "error": "No DB Connected. Here's how to do it - https://docs.21t.cc/docs/proxy/virtual_keys"
                },
            )

        model_in_db = await prisma_client.db.mishikallm_proxymodeltable.find_unique(
        ...
        if store_model_in_db is True:
            ...
            """
            # encrypt mishikallm params #
            result = await prisma_client.db.mishikallm_proxymodeltable.delete(
                where={"model_id": model_info.id}
            )
            ...
            asyncio.create_task(
                create_object_audit_log(
                    ...
                    action="deleted",
                    user_api_key_dict=user_api_key_dict,
                    table_name=MishikallmTableNames.PROXY_MODEL_TABLE_NAME,
...
async def add_new_model(
    ...
    try:
        ...
        model_response: Optional[MishikaLLM_ProxyModelTable] = None
        # update DB
        if store_model_in_db is True:
            """
            - store model_list in db
            ...
            try:
                _original_mishikallm_model_name = model_params.model_name
                ...
                if "slack" in _alerting:
                    ...
                    await proxy_logging_obj.slack_alerting_instance.model_added_alert(
                        model_name=model_params.model_name,
                        mishikallm_model_name=_original_mishikallm_model_name,
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.add_new_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def update_model(
    ...
    try:
        ...
        _existing_mishikallm_params = (
            await prisma_client.db.mishikallm_proxymodeltable.find_unique(
                where={"model_id": _model_id}
            )
        )

        if _existing_mishikallm_params is None:
            if (
        ...
        deployment = Deployment(**_existing_mishikallm_params.model_dump())
        ...
        # update DB
        if store_model_in_db is True:
            _existing_mishikallm_params_dict = dict(
                _existing_mishikallm_params.mishikallm_params
            )

            if model_params.mishikallm_params is None:
                raise Exception("mishikallm_params not provided")

            _new_mishikallm_params_dict = model_params.mishikallm_params.dict(
                exclude_none=True
            )

            ### ENCRYPT PARAMS ###
            for k, v in _new_mishikallm_params_dict.items():
                encrypted_value = encrypt_value_helper(value=v)
                model_params.mishikallm_params[k] = encrypted_value

            ### MERGE WITH EXISTING DATA ###
            merged_dictionary = {}
            _mp = model_params.mishikallm_params.dict()

            for key, value in _mp.items():
                if value is not None:
                    merged_dictionary[key] = value
                elif (
                    key in _existing_mishikallm_params_dict
                    and _existing_mishikallm_params_dict[key] is not None
                ):
                    merged_dictionary[key] = _existing_mishikallm_params_dict[key]
                else:
                    pass

            _data: dict = {
                "mishikallm_params": json.dumps(merged_dictionary),  # type: ignore
                "updated_by": user_api_key_dict.user_id or MISHIKALLM_PROXY_ADMIN_NAME,
            }
            model_response = await prisma_client.db.mishikallm_proxymodeltable.update(
                where={"model_id": _model_id},
                data=_data,  # type: ignore
            )
            ...
            asyncio.create_task(
                create_object_audit_log(
                    ...
                    table_name=MishikallmTableNames.PROXY_MODEL_TABLE_NAME,
                    before_value=(
                        _existing_mishikallm_params.model_dump_json(exclude_none=True)
                        if isinstance(_existing_mishikallm_params, BaseModel)
                        else None
                    ),
                    after_value=(
                        model_response.model_dump_json(exclude_none=True)
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.update_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
def _deduplicate_mishikallm_router_models(models: List[Dict]) -> List[Dict]:
    """
