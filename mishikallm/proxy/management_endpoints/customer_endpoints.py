    ...
    try:
        ...
        if prisma_client is not None:
            for id in data.user_ids:
                record = await prisma_client.db.mishikallm_endusertable.upsert(
...
async def unblock_user(data: BlockUsers):
    ...
    if (
        not any(isinstance(x, _ENTERPRISE_BlockedUserList) for x in mishikallm.callbacks)
        or mishikallm.blocked_user_list is None
    ):
        raise HTTPException(
            status_code=400,
            detail={
    ...
    if isinstance(mishikallm.blocked_user_list, list):
        for id in data.user_ids:
            mishikallm.blocked_user_list.remove(id)
    else:
        raise HTTPException(
            status_code=500,
            detail={
    ...
    return {"blocked_users": mishikallm.blocked_user_list}
...
async def new_end_user(
    ...
    try:
        ...
        if _new_budget is not None:
            try:
                budget_record = await prisma_client.db.mishikallm_budgettable.create(
                    data={
                        **_new_budget.model_dump(exclude_unset=True),
                        "created_by": user_api_key_dict.user_id or mishikallm_proxy_admin_name,  # type: ignore
        ...
        ## WRITE TO DB ##
        end_user_record = await prisma_client.db.mishikallm_endusertable.create(
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.management_endpoints.customer_endpoints.new_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=MishikaLLM_EndUserTable,
)
@router.get(
    "/end_user/info",
    tags=["Customer Management"],
...
async def end_user_info(
    ...
    Example curl:
    ```
    curl -X GET 'http://localhost:4000/customer/info?end_user_id=test-mishikallm-user-4' \
        -H 'Authorization: Bearer sk-1234'
    ```
    """
    from mishikallm.proxy.proxy_server import prisma_client
    ...
    user_info = await prisma_client.db.mishikallm_endusertable.find_first(
        where={"user_id": end_user_id}, include={"mishikallm_budget_table": True}
    )
...
async def update_end_user(
    ...
    try:
        ...
        if data.user_id is not None and len(data.user_id) > 0:
            ...
            response = await prisma_client.db.mishikallm_endusertable.update(
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.update_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def delete_end_user(
    ...
    try:
        ...
        if (
            ...
            response = await prisma_client.db.mishikallm_endusertable.delete_many(
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.delete_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[MishikaLLM_EndUserTable],
)
@router.get(
    "/end_user/list",
    tags=["Customer Management"],
...
async def list_end_user(
    ...
    response = await prisma_client.db.mishikallm_endusertable.find_many(
        include={"mishikallm_budget_table": True}
    )

    returned_response: List[MishikaLLM_EndUserTable] = []
    for item in response:
        returned_response.append(MishikaLLM_EndUserTable(**item.model_dump()))
