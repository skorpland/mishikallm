...
/sso/callback - returns JWT Redirect Response that redirects to MishikaLLM UI
...
from mishikallm.constants import MAX_SPENDLOG_ROWS_TO_QUERY
...
from mishikallm.proxy.auth.auth_checks import ExperimentalUIJWTToken, get_user_object
from mishikallm.proxy.auth.auth_utils import _has_user_setup_sso
...
from mishikallm.proxy.common_utils.admin_ui_utils import (
    admin_ui_disabled,
    show_missing_vars_in_env,
)
from mishikallm.proxy.common_utils.html_forms.jwt_display_template import (
    jwt_display_template,
)
from mishikallm.proxy.common_utils.html_forms.ui_login import html_form
from mishikallm.proxy.management_endpoints.internal_user_endpoints import new_user
from mishikallm.proxy.management_endpoints.sso_helper_utils import (
    check_is_admin_only_access,
    has_admin_ui_access,
)
from mishikallm.proxy.management_endpoints.team_endpoints import new_team, team_member_add
from mishikallm.proxy.management_endpoints.types import CustomOpenID
from mishikallm.proxy.utils import PrismaClient, ProxyLogging
from mishikallm.secret_managers.main import get_secret_bool, str_to_bool
from mishikallm.types.proxy.management_endpoints.ui_sso import *
    ...
    if (
        ...
        if premium_user is not True:
            raise ProxyException(
                message="You must be a MishikaLLM Enterprise user to use SSO. If you have a license please set `MISHIKALLM_LICENSE` in your env. If you want to obtain a license meet with us here: https://calendly.com/d/4mp-gd3-k5k/mishikallm-1-1-onboarding-chat You are seeing this error message because You set one of `MICROSOFT_CLIENT_ID`, `GOOGLE_CLIENT_ID`, or `GENERIC_CLIENT_ID` in your env. Please unset this",
...
async def create_team_member_add_task(team_id, user_info):
    ...
    try:
        ...
        return await team_member_add(
            ...
            user_api_key_dict=UserAPIKeyAuth(user_role=MishikallmUserRoles.PROXY_ADMIN),
...
async def add_missing_team_member(
    user_info: Union[NewUserResponse, MishikaLLM_UserTable], sso_teams: List[str]
):
    """
...
def get_disabled_non_admin_personal_key_creation():
    key_generation_settings = mishikallm.key_generation_settings
...
) -> Optional[Union[MishikaLLM_UserTable, NewUserResponse]]:
    try:
        user_info: Optional[Union[MishikaLLM_UserTable, NewUserResponse]] = (
        ...
        verbose_proxy_logger.debug(
            f"user_info: {user_info}; mishikallm.default_internal_user_params: {mishikallm.default_internal_user_params}"
        )

        # Upsert SSO User to MishikaLLM DB
...
def apply_user_info_values_to_sso_user_defined_values(
    ...
    if user_info is None or user_info.user_role is None:
        user_defined_values["user_role"] = MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY
    else:
...
async def auth_callback(request: Request):  # noqa: PLR0915
    ...
    user_info = None
    user_id_models: List = []
    max_internal_user_budget = mishikallm.max_internal_user_budget
    internal_user_budget_duration = mishikallm.internal_user_budget_duration
    ...
    if user_defined_values is None:
        raise Exception(
            "Unable to map user identity to known values. 'user_defined_values' is None. File an issue - https://github.com/skorpland/mishikallm/issues"
        )
    ...
    user_role = (
        ...
        or MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY.value
    )
    if (
    ...
    if get_secret_bool("EXPERIMENTAL_UI_LOGIN"):
        _user_info: Optional[MishikaLLM_UserTable] = None
        if (
            user_defined_values is not None
            and user_defined_values["user_id"] is not None
        ):
            _user_info = MishikaLLM_UserTable(
    ...
    verbose_proxy_logger.info(f"Redirecting to {mishikallm_dashboard_ui}")
...
async def insert_sso_user(
    ...
    """
    Helper function to create a New User in MishikaLLM DB after a successful SSO login

    Args:
        result_openid (OpenID): User information in OpenID format if the login was successful.
        user_defined_values (Optional[SSOUserDefinedValues], optional): MishikaLLM SSOValues / fields that were read
    ...
    if mishikallm.default_internal_user_params:
        user_defined_values.update(mishikallm.default_internal_user_params)  # type: ignore

    # Set budget for internal users
    if user_defined_values.get("user_role") == MishikallmUserRoles.INTERNAL_USER.value:
        if user_defined_values.get("max_budget") is None:
            user_defined_values["max_budget"] = mishikallm.max_internal_user_budget
        if user_defined_values.get("budget_duration") is None:
            user_defined_values["budget_duration"] = (
                mishikallm.internal_user_budget_duration
            )
...
async def get_ui_settings(request: Request):
    from mishikallm.proxy.proxy_server import general_settings, proxy_state
...
class SSOAuthenticationHandler:
    ...
    async def get_sso_login_redirect(
        ...
        raise ValueError(
            "Unknown SSO provider. Please setup SSO with client IDs https://docs.21t.cc/docs/proxy/admin_ui_sso"
        )
    ...
    async def upsert_sso_user(
        ...
        user_info: Optional[Union[NewUserResponse, MishikaLLM_UserTable]],
        user_email: Optional[str],
        user_defined_values: Optional[SSOUserDefinedValues],
        prisma_client: PrismaClient,
    ):
        """
        Connects the SSO Users to the User Table in MishikaLLM DB

        - If user on MishikaLLM DB, update the user_email with the SSO user_email
        - If user not on MishikaLLM DB, insert the user into MishikaLLM DB
        """
        try:
            if user_info is not None:
                user_id = user_info.user_id
                await prisma_client.db.mishikallm_usertable.update_many(
                    where={"user_id": user_id}, data={"user_email": user_email}
                )
            else:
                verbose_proxy_logger.info(
                    "user not in DB, inserting user into MishikaLLM DB"
                )
                # user not in DB, insert User into MishikaLLM DB
            ...
            verbose_proxy_logger.error(f"Error upserting SSO user into MishikaLLM DB: {e}")
            return user_info
    ...
    async def add_user_to_teams_from_sso_response(
        ...
        The `team_ids` field is populated by mishikallm after processing the SSO response
        """
        if user_info is None:
            verbose_proxy_logger.debug(
                "User not found in MishikaLLM DB, skipping team member addition"
            )
            return
    ...
    @staticmethod
    async def create_mishikallm_team_from_sso_group(
        mishikallm_team_id: str,
        mishikallm_team_name: Optional[str] = None,
    ):
        """
        Creates a Mishikallm Team from a SSO Group ID

        Your SSO provider might have groups that should be created on MishikaLLM

        Use this helper to create a Mishikallm Team from a SSO Group ID

        Args:
            mishikallm_team_id (str): The ID of the Mishikallm Team
            mishikallm_team_name (Optional[str]): The name of the Mishikallm Team
        """
        ...
        try:
            team_obj = await prisma_client.db.mishikallm_teamtable.find_first(
                where={"team_id": mishikallm_team_id}
            )
            ...
            if team_obj:
                verbose_proxy_logger.debug(
                    f"Team already exists: {mishikallm_team_id} - {mishikallm_team_name}"
                )
                return

            team_request: NewTeamRequest = NewTeamRequest(
                team_id=mishikallm_team_id,
                team_alias=mishikallm_team_name,
            )
            if mishikallm.default_team_params:
                team_request = SSOAuthenticationHandler._cast_and_deepcopy_mishikallm_default_team_params(
                    default_team_params=mishikallm.default_team_params,
                    mishikallm_team_id=mishikallm_team_id,
                    mishikallm_team_name=mishikallm_team_name,
            ...
            await new_team(
                ...
                user_api_key_dict=UserAPIKeyAuth(
                    token="",
                    key_alias=f"mishikallm.{MicrosoftSSOHandler.__name__}",
                ),
            )
        except Exception as e:
            verbose_proxy_logger.exception(f"Error creating Mishikallm Team: {e}")

    @staticmethod
    def _cast_and_deepcopy_mishikallm_default_team_params(
        ...
        """
        Casts and deepcopies the mishikallm.default_team_params to a NewTeamRequest object

        - Ensures we create a new DefaultTeamSSOParams object
        - Handle the case where mishikallm.default_team_params is a dict or a DefaultTeamSSOParams object
        - Adds the mishikallm_team_id and mishikallm_team_name to the DefaultTeamSSOParams object
        """
        if isinstance(default_team_params, dict):
            _team_request = deepcopy(default_team_params)
            _team_request["team_id"] = mishikallm_team_id
            _team_request["team_alias"] = mishikallm_team_name
            team_request = NewTeamRequest(**_team_request)
        elif isinstance(mishikallm.default_team_params, DefaultTeamSSOParams):
            _default_team_params = deepcopy(mishikallm.default_team_params)
...
class MicrosoftSSOHandler:
    ...
    # used for debugging to show the user groups mishikallm found from Graph API
        ...
        try:
            ...
            if service_principal_id:
                ...
                if len(service_principal_group_ids) > 0:
                    await MicrosoftSSOHandler.create_mishikallm_teams_from_service_principal_team_ids(
    ...
    @staticmethod
    async def create_mishikallm_teams_from_service_principal_team_ids(
        service_principal_teams: List[MicrosoftServicePrincipalTeam],
    ):
        """
        Creates Mishikallm Teams from the Service Principal Group IDs

        When a user sets a `SERVICE_PRINCIPAL_ID` in the env, mishikallm will fetch groups under that service principal and create Mishikallm Teams from them
        """
        verbose_proxy_logger.debug(
            f"Creating Mishikallm Teams from Service Principal Teams: {service_principal_teams}"
        )
        for service_principal_team in service_principal_teams:
            mishikallm_team_id: Optional[str] = service_principal_team.get("principalId")
            mishikallm_team_name: Optional[str] = service_principal_team.get(
                "principalDisplayName"
            )
            if not mishikallm_team_id:
                verbose_proxy_logger.debug(
                    f"Skipping team creation for {mishikallm_team_name} because it has no principalId"
                )
                continue

            await SSOAuthenticationHandler.create_mishikallm_team_from_sso_group(
                mishikallm_team_id=mishikallm_team_id,
                mishikallm_team_name=mishikallm_team_name,
            )
...
async def debug_sso_callback(request: Request):
    ...
    from mishikallm.proxy.proxy_server import jwt_handler
