...
from mishikallm.proxy._types import (
    ...
    CommonProxyErrors,
    DeleteTeamRequest,
    MishikaLLM_AuditLogs,
    MishikaLLM_ManagementEndpoint_MetadataFields_Premium,
    MishikaLLM_ModelTable,
    MishikaLLM_OrganizationTable,
    MishikaLLM_TeamMembership,
    MishikaLLM_TeamTable,
    MishikaLLM_TeamTableCachedObj,
    MishikaLLM_UserTable,
    MishikallmTableNames,
    MishikallmUserRoles,
    Member,
    NewTeamRequest,
    ProxyErrorTypes,
...
from mishikallm.proxy.management_endpoints.common_utils import (
    _is_user_team_admin,
...
from mishikallm.proxy.management_helpers.team_member_permission_checks import (
    TeamMemberPermissionChecks,
)
from mishikallm.proxy.management_helpers.utils import (
    add_new_member,
...
from mishikallm.types.proxy.management_endpoints.common_daily_activity import (
    SpendAnalyticsPaginatedResponse,
)
from mishikallm.types.proxy.management_endpoints.team_endpoints import (
...
def _is_available_team(team_id: str, user_api_key_dict: UserAPIKeyAuth) -> bool:
    if mishikallm.default_internal_user_params is None:
        return False
    if "available_teams" in mishikallm.default_internal_user_params:
        return team_id in mishikallm.default_internal_user_params["available_teams"]
    return False


async def get_all_team_memberships(
    prisma_client: PrismaClient, team_id: List[str], user_id: Optional[str] = None
) -> List[MishikaLLM_TeamMembership]:
    """Get all team memberships for a given user"""
    ## GET ALL MEMBERSHIPS ##
    if not isinstance(user_id, str):
        user_id = str(user_id)

    team_memberships = await prisma_client.db.mishikallm_teammembership.find_many(
        where=(
    ...
    returned_tm: List[MishikaLLM_TeamMembership] = []
    for tm in team_memberships:
        returned_tm.append(MishikaLLM_TeamMembership(**tm.model_dump()))
...
@router.post(
    ...
    response_model=MishikaLLM_TeamTable,
)
@management_endpoint_wrapper
async def new_team(  # noqa: PLR0915
    ...
    mishikallm_changed_by: Optional[str] = Header(
        None,
        description="The mishikallm-changed-by header enables tracking of actions performed by authorized users on behalf of other users, providing an audit trail for accountability",
    ),
):
    """
    Allow users to create a new team. Apply user permissions to their team.

    👉 [Detailed Doc on setting team budgets](https://docs.21t.cc/docs/proxy/team_budgets)
    ...
    - budget_duration: Optional[str] - The duration of the budget for the team. Doc [here](https://docs.21t.cc/docs/proxy/team_budgets)
    ...
    - tags: Optional[List[str]] - Tags for [tracking spend](https://mishikallm.vercel.app/docs/proxy/enterprise#tracking-spend-for-custom-tags) and/or doing [tag-based routing](https://mishikallm.vercel.app/docs/proxy/tag_routing).
    - organization_id: Optional[str] - The organization id of the team. Default is None. Create via `/organization/new`.
    - model_aliases: Optional[dict] - Model aliases for the team. [Docs](https://docs.21t.cc/docs/proxy/team_based_routing#create-team-with-model-alias)
    - guardrails: Optional[List[str]] - Guardrails for the team. [Docs](https://docs.21t.cc/docs/proxy/guardrails)
    Returns:
    ...
    try:
        ...
        if (
            ...
            or user_api_key_dict.user_role != MishikallmUserRoles.PROXY_ADMIN
        ...
        if data.model_aliases is not None and isinstance(data.model_aliases, dict):
            mishikallm_modeltable = MishikaLLM_ModelTable(
            ...
            model_dict = await prisma_client.db.mishikallm_modeltable.create(
                {**mishikallm_modeltable.json(exclude_none=True)}  # type: ignore
            )  # type: ignore

            _model_id = model_dict.id

        ## ADD TO TEAM TABLE
        complete_team_data = MishikaLLM_TeamTable(
            **data.json(),
            model_id=_model_id,
        )

        # Set Management Endpoint Metadata Fields
        for field in MishikaLLM_ManagementEndpoint_MetadataFields_Premium:
        ...
        team_row: MishikaLLM_TeamTable = await prisma_client.db.mishikallm_teamtable.create(
        ...
        # Enterprise Feature - Audit Logging. Enable with mishikallm.store_audit_logs = True
        if mishikallm.store_audit_logs is True:
            ...
            asyncio.create_task(
                create_audit_log_for_update(
                    request_data=MishikaLLM_AuditLogs(
                        ...
                        changed_by=mishikallm_changed_by
                        or user_api_key_dict.user_id
                        or mishikallm_proxy_admin_name,
...
async def _update_model_table(
    ...
    mishikallm_proxy_admin_name: str,
) -> Optional[str]:
    """
    ...
    if data.model_aliases is not None and isinstance(data.model_aliases, dict):
        ...
        if model_id is None:
            model_dict = await prisma_client.db.mishikallm_modeltable.create(
                data={**mishikallm_modeltable.json(exclude_none=True)}  # type: ignore
            )
        else:
            model_dict = await prisma_client.db.mishikallm_modeltable.upsert(
                where={"id": model_id},
                data={
                    "update": {**mishikallm_modeltable.json(exclude_none=True)},  # type: ignore
                    "create": {**mishikallm_modeltable.json(exclude_none=True)},  # type: ignore
                },
            )  # type: ignore
...
def validate_team_org_change(
    team: MishikaLLM_TeamTable, organization: MishikaLLM_OrganizationTable, llm_router: Router
) -> bool:
    """
    ...
    if (
        team.max_budget
        and organization.mishikallm_budget_table
        and organization.mishikallm_budget_table.max_budget
        and team.max_budget > organization.mishikallm_budget_table.max_budget
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has max_budget {team.max_budget} that is greater than the organization's max_budget {organization.mishikallm_budget_table.max_budget}."
            },
        )
    ...
    if (
        team.tpm_limit
        and organization.mishikallm_budget_table
        and organization.mishikallm_budget_table.tpm_limit
        and team.tpm_limit > organization.mishikallm_budget_table.tpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has tpm_limit {team.tpm_limit} that is greater than the organization's tpm_limit {organization.mishikallm_budget_table.tpm_limit}."
            },
        )
    if (
        team.rpm_limit
        and organization.mishikallm_budget_table
        and organization.mishikallm_budget_table.rpm_limit
        and team.rpm_limit > organization.mishikallm_budget_table.rpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has rpm_limit {team.rpm_limit} that is greater than the organization's rpm_limit {organization.mishikallm_budget_table.rpm_limit}."
            },
        )
    return True
...
async def update_team(
    ...
    ```
    """
    from mishikallm.proxy.auth.auth_checks import _cache_team_object
    from mishikallm.proxy.proxy_server import (
        create_audit_log_for_update,
        mishikallm_proxy_admin_name,
        llm_router,
    ...
    existing_team_row = await prisma_client.db.mishikallm_teamtable.find_unique(
    ...
    if (
        ...
        organization_row = await prisma_client.db.mishikallm_organizationtable.find_unique(
            where={"organization_id": data.organization_id},
            include={"mishikallm_budget_table": True, "users": True},
        )
        ...
        validate_team_org_change(
            team=MishikaLLM_TeamTable(**existing_team_row.model_dump()),
            organization=MishikaLLM_OrganizationTable(**organization_row.model_dump()),
            llm_router=llm_router,
        )
    ...
    _team_metadata_fields = MishikaLLM_ManagementEndpoint_MetadataFields_Premium
    ...
    team_row: Optional[
        MishikaLLM_TeamTable
    ] = await prisma_client.db.mishikallm_teamtable.update(
        where={"team_id": data.team_id},
        data=updated_kv,
        include={"mishikallm_model_table": True},  # type: ignore
    )
    ...
    await _cache_team_object(
        ...
        team_table=MishikaLLM_TeamTableCachedObj(**team_row.model_dump()),
...
def team_member_add_duplication_check(
    data: TeamMemberAddRequest,
    existing_team_row: MishikaLLM_TeamTable,
):
...
async def team_member_add(
    ...
    complete_team_data = MishikaLLM_TeamTable(**existing_team_row.model_dump())
    ...
    if (
        ...
        and user_api_key_dict.user_role != MishikallmUserRoles.PROXY_ADMIN.value
    ...
    updated_team = await prisma_client.db.mishikallm_teamtable.update(
        where={"team_id": data.team_id},
...
async def team_member_delete(
    ...
    _existing_team_row = await prisma_client.db.mishikallm_teamtable.find_unique(
        where={"team_id": data.team_id}
    )
    ...
    existing_team_row = MishikaLLM_TeamTable(**_existing_team_row.model_dump())

    ## CHECK IF USER IS PROXY ADMIN OR TEAM ADMIN

    if (
        user_api_key_dict.user_role != MishikallmUserRoles.PROXY_ADMIN.value
        and not _is_user_team_admin(
    ...
    _ = await prisma_client.db.mishikallm_teamtable.update(
        where={
            "team_id": data.team_id,
        },
    ...
    existing_user_rows = await prisma_client.db.mishikallm_usertable.find_many(
    ...
    if existing_user_rows is not None and (
        ...
        for existing_user in existing_user_rows:
            ...
            if data.team_id in existing_user.teams:
                ...
                await prisma_client.db.mishikallm_usertable.update(
                    where={
...
async def team_member_update(
    ...
    if data.max_budget_in_team is not None:
        if identified_budget_id is None:
            new_budget = await prisma_client.db.mishikallm_budgettable.create(
                data={
            ...
            await prisma_client.db.mishikallm_teammembership.create(
                data={
                    "team_id": data.team_id,
            ...
            await prisma_client.db.mishikallm_budgettable.update(
...
async def delete_team(
    ...
    team_rows: List[MishikaLLM_TeamTable] = []
    for team_id in data.team_ids:
        try:
            team_row_base: Optional[
                BaseModel
            ] = await prisma_client.db.mishikallm_teamtable.find_unique(
        ...
        team_row_pydantic = MishikaLLM_TeamTable(**team_row_base.model_dump())
    ...
    if mishikallm.store_audit_logs is True:
        ...
        for team_id in data.team_ids:
            team_row: Optional[MishikaLLM_TeamTable] = await prisma_client.get_data(  # type: ignore
...
def validate_membership(
    user_api_key_dict: UserAPIKeyAuth, team_table: MishikaLLM_TeamTable
):
    if (
        user_api_key_dict.user_role == MishikallmUserRoles.PROXY_ADMIN.value
        or user_api_key_dict.user_role == MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY.value
    ):
        return
...
def _unfurl_all_proxy_models(
    team_info: MishikaLLM_TeamTable, llm_router: Router
) -> MishikaLLM_TeamTable:
    if (
    ...
    try:
        if prisma_client is None:
            raise HTTPException(
                ...
                detail={
                    "error": "Database not connected. Connect a database to your proxy - https://docs.21t.cc/docs/simple_proxy#managing-auth---virtual-keys"
                },
            )
        if team_id is None:
            raise HTTPException(
        ...
        validate_membership(
            user_api_key_dict=user_api_key_dict,
            team_table=MishikaLLM_TeamTable(**team_info.model_dump()),
        )
        ...
        if isinstance(team_info, dict):
            _team_info = MishikaLLM_TeamTable(**team_info)
        elif isinstance(team_info, BaseModel):
            _team_info = MishikaLLM_TeamTable(**team_info.model_dump())
        else:
            _team_info = MishikaLLM_TeamTable()
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.management_endpoints.team_endpoints.py::team_info - Exception occurred - {}\n{}".format(
...
async def block_team(
    ...
    record = await prisma_client.db.mishikallm_teamtable.update(
...
async def list_available_teams(
    http_request: Request,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    response_model=List[MishikaLLM_TeamTable],
):
    from mishikallm.proxy.proxy_server import prisma_client
    ...
    available_teams = cast(
        Optional[List[str]],
        (
            mishikallm.default_internal_user_params.get("available_teams")
            if mishikallm.default_internal_user_params is not None
            else None
        ),
    )
    if available_teams is None:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "No available teams for user to join. See how to set available teams here: https://docs.21t.cc/docs/proxy/self_serve#all-settings-for-self-serve--sso-flow"
            },
        )

    # filter out teams that the user is already a member of
    user_info = await prisma_client.db.mishikallm_usertable.find_unique(
    ...
    user_info_correct_type = MishikaLLM_UserTable(**user_info.model_dump())

    available_teams = [
        team for team in available_teams if team not in user_info_correct_type.teams
    ]

    available_teams_db = await prisma_client.db.mishikallm_teamtable.find_many(
        where={"team_id": {"in": available_teams}}
    )

    available_teams_correct_type = [
        MishikaLLM_TeamTable(**team.model_dump()) for team in available_teams_db
    ]
...
async def list_team_v2(
    ...
    if user_id is None and user_api_key_dict.user_role != MishikallmUserRoles.PROXY_ADMIN:
    ...
    if user_id:
        try:
            user_object = await prisma_client.db.mishikallm_usertable.find_unique(
        ...
        user_object_correct_type = MishikaLLM_UserTable(**user_object.model_dump())
    ...
    teams = await prisma_client.db.mishikallm_teamtable.find_many(
    ...
    total_count = await prisma_client.db.mishikallm_teamtable.count(where=where_conditions)
...
async def list_team(
    ...
    response = await prisma_client.db.mishikallm_teamtable.find_many(
        include={
            "mishikallm_model_table": True,
        }
    )
    ...
    for team in filtered_response:
        _team_memberships: List[MishikaLLM_TeamMembership] = []
        for tm in returned_tm:
            if tm.team_id == team.team_id:
                _team_memberships.append(tm)

        # add all keys that belong to the team
        keys = await prisma_client.db.mishikallm_verificationtoken.find_many(
...
async def get_paginated_teams(
    ...
    page_size: int = 10,
    page: int = 1,
) -> Tuple[List[MishikaLLM_TeamTable], int]:
    """
    ...
    Returns:
        Tuple[List[MishikaLLM_TeamTable], int] - (list of teams, total count)
    """
    try:
        # Calculate skip for pagination
        skip = (page - 1) * page_size
        # Get total count
        total_count = await prisma_client.db.mishikallm_teamtable.count()
...
@router.get(
    ...
    responses={
        200: {"model": List[MishikaLLM_TeamTable]},
    },
)
async def ui_view_teams(
    ...
    Returns:
        List[MishikaLLM_SpendLogs]: Paginated list of matching user records
    """
    from mishikallm.proxy.proxy_server import prisma_client
...
async def team_model_add(
    ...
    # Get existing team
    team_row = await prisma_client.db.mishikallm_teamtable.find_unique(
        where={"team_id": data.team_id}
    )
    ...
    team_obj = MishikaLLM_TeamTable(**team_row.model_dump())
...
async def get_team_daily_activity(
    ...
    team_aliases = await prisma_client.db.mishikallm_teamtable.find_many(
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="mishikallm_dailyteamspend",
