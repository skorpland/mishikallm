"""
SCIM v2 Endpoints for MishikaLLM Proxy using Internal User/Team Management
...
from mishikallm.proxy.management_endpoints.scim.scim_transformations import (
    ScimTransformations,
)
from mishikallm.proxy.management_endpoints.team_endpoints import new_team
from mishikallm.types.proxy.management_endpoints.scim_v2 import *
...
async def get_users(
    ...
    try:
        ...
        users: List[MishikaLLM_UserTable] = (
        ...
        total_count = await prisma_client.db.mishikallm_usertable.count(
            where=where_conditions
        )
        ...
        for user in users:
            scim_user = await ScimTransformations.transform_mishikallm_user_to_scim_user(
                user=user
            )
...
async def get_user(
    ...
    try:
        user = await prisma_client.db.mishikallm_usertable.find_unique(
        ...
        # Convert to SCIM format
        scim_user = await ScimTransformations.transform_mishikallm_user_to_scim_user(user)
        return scim_user
...
async def create_user(
    ...
    try:
        ...
        existing_user = None
        if user.userName:
            existing_user = await prisma_client.db.mishikallm_usertable.find_unique(
        ...
        created_user = await new_user(
            data=NewUserRequest(
                ...
                metadata={
                    "scim_metadata": MishikaLLM_UserScimMetadata(
    ...
    try:
        ...
        if existing_user.teams:
            for team_id in existing_user.teams:
                team = await prisma_client.db.mishikallm_teamtable.find_unique(
        ...
        # Delete user
        await prisma_client.db.mishikallm_usertable.delete(where={"user_id": user_id})
    ...
    try:
        ...
        for team in teams:
            ...
            for member_id in team.members or []:
                member = await prisma_client.db.mishikallm_usertable.find_unique(
...
async def get_group(
    ...
    try:
        ...
        scim_group = await ScimTransformations.transform_mishikallm_team_to_scim_group(
            team
        )
        return scim_group
...
async def create_group(
    ...
    try:
        ...
        existing_team = await prisma_client.db.mishikallm_teamtable.find_unique(
            where={"team_id": team_id}
        )
...
async def delete_group(
    ...
    try:
        ...
        # Delete team
        await prisma_client.db.mishikallm_teamtable.delete(where={"team_id": group_id})
