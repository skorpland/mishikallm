...
class ScimTransformations:
    ...
    @staticmethod
    async def transform_mishikallm_user_to_scim_user(
        user: Union[MishikaLLM_UserTable, NewUserResponse],
    ) -> SCIMUser:
    ...
    @staticmethod
    def _get_scim_user_name(user: Union[MishikaLLM_UserTable, NewUserResponse]) -> str:
        """
    ...
    @staticmethod
    def _get_scim_family_name(user: Union[MishikaLLM_UserTable, NewUserResponse]) -> str:
        """
        SCIM requires a family name with length > 0
        """
        metadata = user.metadata or {}
        if "scim_metadata" in metadata:
            scim_metadata: MishikaLLM_UserScimMetadata = MishikaLLM_UserScimMetadata(
    ...
    @staticmethod
    def _get_scim_given_name(user: Union[MishikaLLM_UserTable, NewUserResponse]) -> str:
        """
    ...
    @staticmethod
    async def transform_mishikallm_team_to_scim_group(
        team: Union[MishikaLLM_TeamTable, dict],
    ) -> SCIMGroup:
        from mishikallm.proxy.proxy_server import prisma_client
        ...
        if isinstance(team, dict):
            team = MishikaLLM_TeamTable(**team)
