...
from mishikallm.constants import DEFAULT_PROMPT_INJECTION_SIMILARITY_THRESHOLD
...
from mishikallm.proxy._types import MishikaLLMPromptInjectionParams, UserAPIKeyAuth
from mishikallm.router import Router
from mishikallm.utils import get_formatted_prompt
...
class _OPTIONAL_PromptInjectionDetection(CustomLogger):
    ...
    def __init__(
        self,
        prompt_injection_params: Optional[MishikaLLMPromptInjectionParams] = None,
    ):
    ...
    async def async_moderation_hook(  # type: ignore
        ...
        if (
            ...
            if isinstance(response, mishikallm.ModelResponse) and isinstance(
                response.choices[0], mishikallm.Choices
            ):
