...
from mishikallm.mishikallm_core_utils.mishikallm_logging import StandardLoggingPayloadSetup
from mishikallm.proxy._types import UserAPIKeyAuth
from mishikallm.proxy.auth.auth_checks import log_db_metrics
...
class _ProxyDBLogger(CustomLogger):
    ...
    async def async_post_call_failure_hook(
        ...
        if "mishikallm_params" not in request_data:
            request_data["mishikallm_params"] = {}
        request_data["mishikallm_params"]["proxy_server_request"] = (
            request_data.get("proxy_server_request") or {}
        )
        request_data["mishikallm_params"]["metadata"] = existing_metadata
    ...
    async def _PROXY_track_cost_callback(
        ...
        completion_response: Optional[
            Union[mishikallm.ModelResponse, Any]
        ...
        try:
            ...
            metadata = get_mishikallm_metadata_from_kwargs(kwargs=kwargs)
            ...
            if response_cost is not None:
                ...
                if kwargs["stream"] is not True or (
                    ...
                    raise Exception(
                        f"Cost tracking failed for model={model}.\nDebug info - {cost_tracking_failure_debug_info}\nAdd custom pricing - https://docs.21t.cc/docs/proxy/custom_pricing"
                    )
        except Exception as e:
            error_msg = f"Error in tracking cost callback - {str(e)}\n Traceback:{traceback.format_exc()}"
            model = kwargs.get("model", "")
            metadata = get_mishikallm_metadata_from_kwargs(kwargs=kwargs)
            mishikallm_metadata = kwargs.get("mishikallm_params", {}).get(
                "mishikallm_metadata", {}
            )
            old_metadata = kwargs.get("mishikallm_params", {}).get("metadata", {})
