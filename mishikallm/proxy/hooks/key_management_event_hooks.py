...
MISHIKALLM_PREFIX_STORED_VIRTUAL_KEYS = "mishikallm/"
...
class KeyManagementEventHooks:
    ...
    async def async_key_generated_hook(
        ...
        mishikallm_changed_by: Optional[str] = None,
    ):
        """
        ...
        """
        from mishikallm.proxy.management_helpers.audit_logs import (
            create_audit_log_for_update,
        )
        from mishikallm.proxy.proxy_server import mishikallm_proxy_admin_name
    ...
    async def async_key_deleted_hook(
        data: KeyRequest,
        keys_being_deleted: List[MishikaLLM_VerificationToken],
        response: dict,
        user_api_key_dict: UserAPIKeyAuth,
        mishikallm_changed_by: Optional[str] = None,
    ):
        ...
        if mishikallm.store_audit_logs is True and data.keys is not None:
    ...
    async def _store_virtual_key_in_secret_manager(secret_name: str, secret_token: str):
        ...
        """
        if mishikallm._key_management_settings is not None:
            if mishikallm._key_management_settings.store_virtual_keys is True:
                from mishikallm.secret_managers.base_secret_manager import (
                    BaseSecretManager,
                )

                # store the key in the secret manager
                if isinstance(mishikallm.secret_manager_client, BaseSecretManager):
                    await mishikallm.secret_manager_client.async_write_secret(
    ...
    async def _rotate_virtual_key_in_secret_manager(
        ...
        if mishikallm._key_management_settings is not None:
            if mishikallm._key_management_settings.store_virtual_keys is True:
                ...
                # store the key in the secret manager
                if isinstance(mishikallm.secret_manager_client, BaseSecretManager):
                    await mishikallm.secret_manager_client.async_rotate_secret(
    ...
    def _get_secret_name(secret_name: str) -> str:
        if mishikallm._key_management_settings.prefix_for_stored_virtual_keys.endswith(
            "/"
        ):
            return f"{mishikallm._key_management_settings.prefix_for_stored_virtual_keys}{secret_name}"
        else:
            return f"{mishikallm._key_management_settings.prefix_for_stored_virtual_keys}/{secret_name}"
                ...
                if isinstance(mishikallm.secret_manager_client, BaseSecretManager):
                    for key in keys_being_deleted:
                        if key.key_alias is not None:
                            await mishikallm.secret_manager_client.async_delete_secret(
                                secret_name=KeyManagementEventHooks._get_secret_name(
                                    key.key_alias
                                )
                            )
