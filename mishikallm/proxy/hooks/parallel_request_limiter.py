...
import mishikallm
from mishikallm import DualCache, ModelResponse
...
from mishikallm.proxy._types import CommonProxyErrors, CurrentItemRateLimit, UserAPIKeyAuth
...
class _PROXY_MaxParallelRequestsHandler(CustomLogger):
    ...
    async def check_key_in_limits(
        ...
        await self.internal_usage_cache.async_batch_set_cache(
            ...
            ttl=60,
            mishikallm_parent_otel_span=user_api_key_dict.parent_otel_span,
            local_only=True,
        )
        return new_val
    ...
    async def async_pre_call_hook(  # noqa: PLR0915
        ...
        if (
            ...
            _remaining_limits_data = {
                f"mishikallm-key-remaining-tokens-{_model}": _remaining_tokens,
                f"mishikallm-key-remaining-requests-{_model}": _remaining_requests,
            }
    ...
    async def async_log_success_event(  # noqa: PLR0915
        ...
        mishikallm_parent_otel_span: Union[Span, None] = _get_parent_otel_span_from_kwargs(
            kwargs=kwargs
        )
        try:
            self.print_verbose("INSIDE parallel request limiter ASYNC SUCCESS LOGGING")

            global_max_parallel_requests = kwargs["mishikallm_params"]["metadata"].get(
                "global_max_parallel_requests", None
            )
            user_api_key = kwargs["mishikallm_params"]["metadata"]["user_api_key"]
            user_api_key_user_id = kwargs["mishikallm_params"]["metadata"].get(
                "user_api_key_user_id", None
            )
            user_api_key_team_id = kwargs["mishikallm_params"]["metadata"].get(
                "user_api_key_team_id", None
            )
            user_api_key_model_max_budget = kwargs["mishikallm_params"]["metadata"].get(
                "user_api_key_model_max_budget", None
            )
            user_api_key_end_user_id = kwargs.get("user")

            user_api_key_metadata = (
                kwargs["mishikallm_params"]["metadata"].get("user_api_key_metadata", {})
                or {}
            )
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            mishikallm_parent_otel_span: Union[
                Span, None
            ] = _get_parent_otel_span_from_kwargs(kwargs=kwargs)
            _metadata = kwargs["mishikallm_params"].get("metadata", {}) or {}
    ...
    async def get_internal_user_object(
        ...
        It uses the `get_user_object` function from `mishikallm.proxy.auth.auth_checks`
