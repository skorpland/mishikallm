...
import mishikallm
from mishikallm import ModelResponse, Router
    ...
    async def async_set_cache_sadd(self, model: str, value: List):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "mishikallm.proxy.hooks.dynamic_rate_limiter.py::async_set_cache_sadd(): Exception occured - {}".format(
                    str(e)
                )
            )
            raise e
...
class _PROXY_DynamicRateLimitHandler(CustomLogger):
    ...
    async def check_available_usage(
        ...
        try:
            weight: float = 1
            if (
                mishikallm.priority_reservation is None
                or priority not in mishikallm.priority_reservation
            ):
                verbose_proxy_logger.error(
                    "Priority Reservation not set. priority={}, but mishikallm.priority_reservation is {}.".format(
                        priority, mishikallm.priority_reservation
                    )
                )
            elif priority is not None and mishikallm.priority_reservation is not None:
                if os.getenv("MISHIKALLM_LICENSE", None) is None:
                    verbose_proxy_logger.error(
                        "PREMIUM FEATURE: Reserving tpm/rpm by priority is a premium feature. Please add a 'MISHIKALLM_LICENSE' to your .env to enable this.\nGet a license: https://docs.21t.cc/docs/proxy/enterprise."
                    )
                else:
                    weight = mishikallm.priority_reservation[priority]
            ...
            verbose_proxy_logger.exception(
                "mishikallm.proxy.hooks.dynamic_rate_limiter.py::check_available_usage: Exception occurred - {}".format(
                    str(e)
                )
            )
    ...
    ]:  # raise exception if invalid, return a str for the user to receive - if rejected, or return a modified dictionary for passing into mishikallm
        """
        - For a model group
        ...
        try:
            if isinstance(response, ModelResponse):
                ...
                response._hidden_params[
                    ...
                    "x-mishikallm-model_group": model_info["model_name"],
                    "x-ratelimit-remaining-mishikallm-project-tokens": available_tpm,
                    "x-ratelimit-remaining-mishikallm-project-requests": available_rpm,
            ...
            verbose_proxy_logger.exception(
                "mishikallm.proxy.hooks.dynamic_rate_limiter.py::async_post_call_success_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
            return response
