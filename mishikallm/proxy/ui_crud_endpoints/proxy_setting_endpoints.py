...
async def _get_settings_with_schema(
    ...
    Args:
        settings_key: The key in mishikallm_settings to get
        settings_class: The Pydantic class to use for schema
        config: The config dictionary
    """
    from pydantic import TypeAdapter

    mishikallm_settings = config.get("mishikallm_settings", {}) or {}
    settings_data = mishikallm_settings.get(settings_key, {}) or {}
...
async def get_sso_settings():
    """
    Get all SSO settings from the mishikallm_settings configuration.
...
async def _update_mishikallm_setting(
    ...
    """
    Common utility function to update `mishikallm_settings` in both memory and config.

    Args:
        settings: The settings object to update
        settings_key: The key in mishikallm_settings to update
    ...
    if "mishikallm_settings" not in config:
        config["mishikallm_settings"] = {}

    config["mishikallm_settings"][settings_key] = settings.model_dump(exclude_none=True)
...
async def update_internal_user_settings(settings: DefaultInternalUserParams):
    ...
    """
    return await _update_mishikallm_setting(
        settings=settings,
        settings_key="default_internal_user_params",
        in_memory_var=mishikallm.default_internal_user_params,
...
async def update_default_team_settings(settings: DefaultTeamSSOParams):
    ...
    return await _update_mishikallm_setting(
        settings=settings,
        settings_key="default_team_params",
        in_memory_var=mishikallm.default_team_params,
