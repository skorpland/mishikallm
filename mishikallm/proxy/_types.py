...
from mishikallm.types.llms.openai import AllMessageValues, OpenAIFileObject
from mishikallm.types.router import RouterErrors, UpdateRouterConfig
from mishikallm.types.utils import (
    CallTypes,
    EmbeddingResponse,
    GenericBudgetConfigType,
    ImageResponse,
    MishikaLLMPydanticObjectBase,
    ModelResponse,
    ProviderField,
...
class MishikaLLMTeamRoles(enum.Enum):
    # team admin
    TEAM_ADMIN = "admin"
    # team member
    TEAM_MEMBER = "user"
...
class MishikallmUserRoles(str, enum.Enum):
    ...
    def description(self):
        ...
        """
        descriptions = {
            "proxy_admin": "admin over mishikallm proxy, has all permissions",
...
class MishikallmTableNames(str, enum.Enum):
    """
    Enum for Table Names used by MishikaLLM
    """

    TEAM_TABLE_NAME = "MishikaLLM_TeamTable"
    USER_TABLE_NAME = "MishikaLLM_UserTable"
    KEY_TABLE_NAME = "MishikaLLM_VerificationToken"
    PROXY_MODEL_TABLE_NAME = "MishikaLLM_ProxyModelTable"
    MANAGED_FILE_TABLE_NAME = "MishikaLLM_ManagedFileTable"


class Mishikallm_EntityType(enum.Enum):
    """
    Enum for types of entities on mishikallm
...
class MishikaLLM_UpperboundKeyGenerateParams(MishikaLLMPydanticObjectBase):
    """
...
class MishikaLLMRoutes(enum.Enum):
    openai_route_names = [
        "chat_completion",
        "completion",
        "embeddings",
...
class MishikaLLMPromptInjectionParams(MishikaLLMPydanticObjectBase):
...
class ProxyChatCompletionRequest(MishikaLLMPydanticObjectBase):
    model: str
    ...
    model_config = ConfigDict(
        extra="allow"
    )  # allow params not defined here, these fall in mishikallm.completion(**kwargs)


class ModelInfoDelete(MishikaLLMPydanticObjectBase):
    id: str


class ModelInfo(MishikaLLMPydanticObjectBase):
    id: Optional[str]
...
class ProviderInfo(MishikaLLMPydanticObjectBase):
    name: str
    fields: List[ProviderField]


class BlockUsers(MishikaLLMPydanticObjectBase):
    user_ids: List[str]  # required


class ModelParams(MishikaLLMPydanticObjectBase):
    model_name: str
...
class GenerateRequestBase(MishikaLLMPydanticObjectBase):
    """
...
class GenerateKeyResponse(KeyRequestBase):
    ...
    mishikallm_budget_table: Optional[Any] = None
...
class KeyRequest(MishikaLLMPydanticObjectBase):
...
class MishikaLLM_ModelTable(MishikaLLMPydanticObjectBase):
...
class MishikaLLM_ProxyModelTable(MishikaLLMPydanticObjectBase):
    model_id: str
    model_name: str
    mishikallm_params: dict
    model_info: dict
    ...
    def check_potential_json_str(cls, values):
        if isinstance(values.get("mishikallm_params"), str):
            try:
                values["mishikallm_params"] = json.loads(values["mishikallm_params"])
            except json.JSONDecodeError:
                pass
...
class DeleteUserRequest(MishikaLLMPydanticObjectBase):
    user_ids: List[str]  # required
...
class BudgetNewRequest(MishikaLLMPydanticObjectBase):
...
class BudgetRequest(MishikaLLMPydanticObjectBase):
    budgets: List[str]


class BudgetDeleteRequest(MishikaLLMPydanticObjectBase):
    id: str


class CustomerBase(MishikaLLMPydanticObjectBase):
    user_id: str
    ...
    budget_id: Optional[str] = None
    mishikallm_budget_table: Optional[BudgetNewRequest] = None
    blocked: bool = False
...
class UpdateCustomerRequest(MishikaLLMPydanticObjectBase):
    """
...
class DeleteCustomerRequest(MishikaLLMPydanticObjectBase):
    """
...
class MemberBase(MishikaLLMPydanticObjectBase):
    user_id: Optional[str] = None
    user_email: Optional[str] = None
...
class OrgMember(MemberBase):
    role: Literal[
        MishikallmUserRoles.ORG_ADMIN,
        MishikallmUserRoles.INTERNAL_USER,
        MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY,
    ]


class TeamBase(MishikaLLMPydanticObjectBase):
...
class GlobalEndUsersSpend(MishikaLLMPydanticObjectBase):
    api_key: Optional[str] = None
    startTime: Optional[datetime] = None
    endTime: Optional[datetime] = None


class UpdateTeamRequest(MishikaLLMPydanticObjectBase):
    """
...
class ResetTeamBudgetRequest(MishikaLLMPydanticObjectBase):
    """
...
class DeleteTeamRequest(MishikaLLMPydanticObjectBase):
    team_ids: List[str]  # required


class BlockTeamRequest(MishikaLLMPydanticObjectBase):
    team_id: str  # required


class BlockKeyRequest(MishikaLLMPydanticObjectBase):
    key: str  # required


class AddTeamCallback(MishikaLLMPydanticObjectBase):
    callback_name: str
...
class TeamCallbackMetadata(MishikaLLMPydanticObjectBase):
...
class MishikaLLM_TeamTable(TeamBase):
    ...
    mishikallm_model_table: Optional[MishikaLLM_ModelTable] = None
    updated_at: Optional[datetime] = None
    created_at: Optional[datetime] = None
...
class MishikaLLM_TeamTableCachedObj(MishikaLLM_TeamTable):
    last_refreshed_at: Optional[float] = None


class TeamRequest(MishikaLLMPydanticObjectBase):
    teams: List[str]


class MishikaLLM_BudgetTable(MishikaLLMPydanticObjectBase):
    """Represents user-controllable params for a MishikaLLM_BudgetTable record"""
...
class MishikaLLM_TeamMemberTable(MishikaLLM_BudgetTable):
    """
...
class NewOrganizationRequest(MishikaLLM_BudgetTable):
    organization_id: Optional[str] = None
...
class OrganizationRequest(MishikaLLMPydanticObjectBase):
    organizations: List[str]


class DeleteOrganizationRequest(MishikaLLMPydanticObjectBase):
...
class KeyManagementSettings(MishikaLLMPydanticObjectBase):
    hosted_keys: Optional[List] = None
    store_virtual_keys: Optional[bool] = False
    """
    If True, virtual keys created by mishikallm will be stored in the secret manager
    """
    prefix_for_stored_virtual_keys: str = "mishikallm/"
    """
...
class TeamDefaultSettings(MishikaLLMPydanticObjectBase):
    team_id: str
...
class DynamoDBArgs(MishikaLLMPydanticObjectBase):
    billing_mode: Literal["PROVISIONED_THROUGHPUT", "PAY_PER_REQUEST"]
    read_capacity_units: Optional[int] = None
    write_capacity_units: Optional[int] = None
    ssl_verify: Optional[bool] = None
    region_name: str
    user_table_name: str = "MishikaLLM_UserTable"
    key_table_name: str = "MishikaLLM_VerificationToken"
    config_table_name: str = "MishikaLLM_Config"
    spend_table_name: str = "MishikaLLM_SpendLogs"
...
class PassThroughGenericEndpoint(MishikaLLMPydanticObjectBase):
    path: str = Field(description="The route to be added to the MishikaLLM Proxy Server.")
    target: str = Field(
...
class PassThroughEndpointResponse(MishikaLLMPydanticObjectBase):
    endpoints: List[PassThroughGenericEndpoint]


class ConfigFieldUpdate(MishikaLLMPydanticObjectBase):
    field_name: str
    field_value: Any
    config_type: Literal["general_settings"]


class ConfigFieldDelete(MishikaLLMPydanticObjectBase):
    config_type: Literal["general_settings"]
    field_name: str
...
class ConfigList(MishikaLLMPydanticObjectBase):
    field_name: str
    field_type: str
    field_description: str
    field_value: Any
...
class ConfigGeneralSettings(MishikaLLMPydanticObjectBase):
    """
    ...
    custom_auth: Optional[str] = Field(
        None,
        description="override user_api_key_auth with your own auth script - https://docs.21t.cc/docs/proxy/virtual_keys#custom-auth",
    )
    max_parallel_requests: Optional[int] = Field(
        None,
    ...
    pass_through_endpoints: Optional[List[PassThroughGenericEndpoint]] = Field(
        default=None,
        description="Set-up pass-through endpoints for provider-specific endpoints. Docs - https://docs.21t.cc/docs/proxy/pass_through",
    )


class ConfigYAML(MishikaLLMPydanticObjectBase):
    """
    ...
    mishikallm_settings: Optional[dict] = Field(
        None,
        description="mishikallm Module settings. See __init__.py for all, example mishikallm.drop_params=True, mishikallm.set_verbose=True, mishikallm.api_base, mishikallm.cache",
    )
    general_settings: Optional[ConfigGeneralSettings] = None
    router_settings: Optional[UpdateRouterConfig] = Field(
        None,
        description="mishikallm router object settings. See router.py __init__ for all, example router.num_retries=5, router.timeout=5, router.max_retries=5, router.retry_after=5",
    )

    model_config = ConfigDict(protected_namespaces=())


class MishikaLLM_VerificationToken(MishikaLLMPydanticObjectBase):
    token: Optional[str] = None
    key_name: Optional[str] = None
    key_alias: Optional[str] = None
    spend: float = 0.0
    ...
    blocked: Optional[bool] = None
    mishikallm_budget_table: Optional[dict] = None
...
class MishikaLLM_VerificationTokenView(MishikaLLM_VerificationToken):
    """
    Combined view of mishikallm verification token + mishikallm team table (select values)
    """
    ...
    def __init__(self, **kwargs):
        # Handle mishikallm_budget_table_* keys
        for key, value in list(kwargs.items()):
            if key.startswith("mishikallm_budget_table_") and value is not None:
                # Extract the corresponding attribute name
                attr_name = key.replace("mishikallm_budget_table_", "")
...
class UserAPIKeyAuth(
    MishikaLLM_VerificationTokenView
    ...
    api_key: Optional[str] = None
    user_role: Optional[MishikallmUserRoles] = None
    allowed_model_region: Optional[AllowedModelRegion] = None
...
class UserInfoResponse(MishikaLLMPydanticObjectBase):
...
class MishikaLLM_Config(MishikaLLMPydanticObjectBase):
    param_name: str
    param_value: Dict


class MishikaLLM_OrganizationMembershipTable(MishikaLLMPydanticObjectBase):
    """
    ...
    mishikallm_budget_table: Optional[MishikaLLM_BudgetTable] = None

    model_config = ConfigDict(protected_namespaces=())


class MishikaLLM_OrganizationTableUpdate(MishikaLLMPydanticObjectBase):
    """Represents user-controllable params for a MishikaLLM_OrganizationTable record"""
...
class MishikaLLM_UserTable(MishikaLLMPydanticObjectBase):
    user_id: str
    max_budget: Optional[float] = None
    spend: float = 0.0
    ...
    rpm_limit: Optional[int] = None
    user_role: Optional[str] = None
    organization_memberships: Optional[List[MishikaLLM_OrganizationMembershipTable]] = None
    teams: List[str] = []
    sso_user_id: Optional[str] = None
    budget_duration: Optional[str] = None
    budget_reset_at: Optional[datetime] = None
...
class MishikaLLM_OrganizationTable(MishikaLLMPydanticObjectBase):
    """Represents user-controllable params for a MishikaLLM_OrganizationTable record"""
    ...
    created_by: str
    updated_by: str
    users: Optional[List[MishikaLLM_UserTable]] = None
    mishikallm_budget_table: Optional[MishikaLLM_BudgetTable] = None


class MishikaLLM_OrganizationTableWithMembers(MishikaLLM_OrganizationTable):
    """Returned by the /organization/info endpoint and /organization/list endpoint"""

    members: List[MishikaLLM_OrganizationMembershipTable] = []
    teams: List[MishikaLLM_TeamTable] = []
    mishikallm_budget_table: Optional[MishikaLLM_BudgetTable] = None
    created_at: datetime
    updated_at: datetime


class NewOrganizationResponse(MishikaLLM_OrganizationTable):
    organization_id: str  # type: ignore
    created_at: datetime
    updated_at: datetime


class MishikaLLM_UserTableFiltered(BaseModel):  # done to avoid exposing sensitive data
    user_id: str
    user_email: Optional[str] = None


class MishikaLLM_UserTableWithKeyCount(MishikaLLM_UserTable):
    key_count: int = 0


class MishikaLLM_EndUserTable(MishikaLLMPydanticObjectBase):
    user_id: str
    blocked: bool
    alias: Optional[str] = None
    spend: float = 0.0
...
class MishikaLLM_SpendLogs(MishikaLLMPydanticObjectBase):
    request_id: str
    api_key: str
    model: Optional[str] = ""
...
class MishikaLLM_ErrorLogs(MishikaLLMPydanticObjectBase):
    request_id: Optional[str] = str(uuid.uuid4())
    api_base: Optional[str] = ""
    model_group: Optional[str] = ""
    mishikallm_model_name: Optional[str] = ""
...
class MishikaLLM_AuditLogs(MishikaLLMPydanticObjectBase):
    id: str
    updated_at: datetime
    changed_by: Optional[Any] = None
    changed_by_api_key: Optional[str] = None
    action: AUDIT_ACTIONS
    table_name: MishikallmTableNames
    object_id: str
...
class MishikaLLM_SpendLogs_ResponseObject(MishikaLLMPydanticObjectBase):
    response: Optional[List[Union[MishikaLLM_SpendLogs, Any]]] = None


class TokenCountRequest(MishikaLLMPydanticObjectBase):
    model: str
    prompt: Optional[str] = None
    messages: Optional[List[dict]] = None


class TokenCountResponse(MishikaLLMPydanticObjectBase):
    total_tokens: int
    request_model: str
    model_used: str
    tokenizer_type: str
...
class CallInfo(MishikaLLMPydanticObjectBase):
    ...
    event_group: Mishikallm_EntityType
...
class InvitationNew(MishikaLLMPydanticObjectBase):
    user_id: str


class InvitationUpdate(MishikaLLMPydanticObjectBase):
    invitation_id: str
    is_accepted: bool


class InvitationDelete(MishikaLLMPydanticObjectBase):
    invitation_id: str


class InvitationModel(MishikaLLMPydanticObjectBase):
    id: str
    user_id: str
    is_accepted: bool
...
class InvitationClaim(MishikaLLMPydanticObjectBase):
    invitation_link: str
    user_id: str
    password: str


class ConfigFieldInfo(MishikaLLMPydanticObjectBase):
    field_name: str
    field_value: Any


class CallbackOnUI(MishikaLLMPydanticObjectBase):
    mishikallm_callback_name: str
    mishikallm_callback_params: Optional[list]
    ui_callback_name: str


class AllCallbacks(MishikaLLMPydanticObjectBase):
    langfuse: CallbackOnUI = CallbackOnUI(
        mishikallm_callback_name="langfuse",
        ui_callback_name="Langfuse",
        mishikallm_callback_params=[
            "LANGFUSE_PUBLIC_KEY",
            "LANGFUSE_SECRET_KEY",
        ],
    )
    ...
    openmeter: CallbackOnUI = CallbackOnUI(
        mishikallm_callback_name="openmeter",
    ...
    custom_callback_api: CallbackOnUI = CallbackOnUI(
        mishikallm_callback_name="custom_callback_api",
        mishikallm_callback_params=["GENERIC_LOGGER_ENDPOINT"],
        ui_callback_name="Custom Callback API",
    )

    datadog: CallbackOnUI = CallbackOnUI(
        mishikallm_callback_name="datadog",
        mishikallm_callback_params=["DD_API_KEY", "DD_SITE"],
        ui_callback_name="Datadog",
    )

    braintrust: CallbackOnUI = CallbackOnUI(
        mishikallm_callback_name="braintrust",
        mishikallm_callback_params=["BRAINTRUST_API_KEY"],
        ui_callback_name="Braintrust",
    )

    langsmith: CallbackOnUI = CallbackOnUI(
        mishikallm_callback_name="langsmith",
        mishikallm_callback_params=[
            "LANGSMITH_API_KEY",
            "LANGSMITH_PROJECT",
...
class SpanAttributes(str, enum.Enum):
    ...
    # I chose to not add a new dependency to mishikallm for this
...
class ManagementEndpointLoggingPayload(MishikaLLMPydanticObjectBase):
    route: str
    request_data: dict
...
class ProxyException(Exception):
    ...
    def __init__(
        ...
        # Mishikallm router.py returns "No healthy deployment available" when there are no deployments available
        # Should map to 429 errors https://github.com/skorpland/mishikallm/issues/2487
        if (
...
class CommonProxyErrors(str, enum.Enum):
    db_not_connected_error = (
        "DB not connected. See https://docs.21t.cc/docs/proxy/virtual_keys"
    )
    no_llm_router = "No models configured on proxy"
    not_allowed_access = "Admin-only endpoint. Not allowed to access this."
    not_premium_user = "You must be a MishikaLLM Enterprise user to use this feature. If you have a license please set `MISHIKALLM_LICENSE` in your env. Get a 7 day trial key here: https://www.21t.cc/#trial. \nPricing: https://www.21t.cc/#pricing"
    max_parallel_request_limit_reached = (
        "Crossed TPM / RPM / Max Parallel Request Limit"
    )


class SpendCalculateRequest(MishikaLLMPydanticObjectBase):
...
class VirtualKeyEvent(MishikaLLMPydanticObjectBase):
...
class CreatePassThroughEndpoint(MishikaLLMPydanticObjectBase):
    path: str
    target: str
    headers: dict


class MishikaLLM_TeamMembership(MishikaLLMPydanticObjectBase):
    user_id: str
    team_id: str
    budget_id: str
    mishikallm_budget_table: Optional[MishikaLLM_BudgetTable]


#### Organization / Team Member Requests ####


class MemberAddRequest(MishikaLLMPydanticObjectBase):
...
class OrgMemberAddRequest(MishikaLLMPydanticObjectBase):
...
class TeamAddMemberResponse(MishikaLLM_TeamTable):
    updated_users: List[MishikaLLM_UserTable]
    updated_team_memberships: List[MishikaLLM_TeamMembership]


class OrganizationAddMemberResponse(MishikaLLMPydanticObjectBase):
    organization_id: str
    updated_users: List[MishikaLLM_UserTable]
    updated_organization_memberships: List[MishikaLLM_OrganizationMembershipTable]


class MemberDeleteRequest(MishikaLLMPydanticObjectBase):
    user_id: Optional[str] = None
    user_email: Optional[str] = None
...
class MemberUpdateResponse(MishikaLLMPydanticObjectBase):
    user_id: str
    user_email: Optional[str] = None
...
class OrganizationMemberUpdateRequest(OrganizationMemberDeleteRequest):
    ...
    role: Optional[MishikallmUserRoles] = None

    @field_validator("role")
    def validate_role(
        cls, value: Optional[MishikallmUserRoles]
    ) -> Optional[MishikallmUserRoles]:
...
class TeamInfoResponseObject(TypedDict):
    team_id: str
    team_info: MishikaLLM_TeamTable
    keys: List
    team_memberships: List[MishikaLLM_TeamMembership]


class TeamListResponseObject(MishikaLLM_TeamTable):
    team_memberships: List[MishikaLLM_TeamMembership]
...
class SpecialHeaders(enum.Enum):
    """Used by user_api_key_auth.py to get mishikallm key"""
    ...
    custom_mishikallm_api_key = "x-mishikallm-api-key"


class MishikallmDataForBackendLLMCall(TypedDict, total=False):
    headers: dict
    organization: str
...
class UserManagementEndpointParamDocStringEnums(str, enum.Enum):
    ...
    user_role_doc_str = """Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role here: `https://github.com/skorpland/mishikallm/mishikallm/proxy/_types.py#L20`"""
    ...
    aliases_doc_str = """Optional[dict] - Model aliases for the user - [Docs](https://mishikallm.vercel.app/docs/proxy/virtual_keys#model-aliases)"""
    config_doc_str = """Optional[dict] - [DEPRECATED PARAM] User-specific config."""
    allowed_cache_controls_doc_str = """Optional[list] - List of allowed cache control values. Example - ["no-cache", "no-store"]. See all values - https://docs.21t.cc/docs/proxy/caching#turn-on--off-caching-per-request-"""
    blocked_doc_str = (
    ...
    model_max_budget_doc_str = """Optional[dict] - Model-specific max budget for user. [Docs](https://docs.21t.cc/docs/proxy/users#add-model-specific-budgets-to-keys)"""
    model_rpm_limit_doc_str = """Optional[float] - Model-specific rpm limit for user. [Docs](https://docs.21t.cc/docs/proxy/users#add-model-specific-limits-to-keys)"""
    model_tpm_limit_doc_str = """Optional[float] - Model-specific tpm limit for user. [Docs](https://docs.21t.cc/docs/proxy/users#add-model-specific-limits-to-keys)"""
...
MishikaLLM_ManagementEndpoint_MetadataFields = [
    "model_rpm_limit",
    "model_tpm_limit",
    "guardrails",
    "tags",
...
class ProviderBudgetResponseObject(MishikaLLMPydanticObjectBase):
    """
...
class ProviderBudgetResponse(MishikaLLMPydanticObjectBase):
    """
...
UI_TEAM_ID = "mishikallm-dashboard"


class JWTAuthBuilderResult(TypedDict):
    is_proxy_admin: bool
    team_object: Optional[MishikaLLM_TeamTable]
    user_object: Optional[MishikaLLM_UserTable]
    end_user_object: Optional[MishikaLLM_EndUserTable]
    org_object: Optional[MishikaLLM_OrganizationTable]
    token: str
    team_id: Optional[str]
    user_id: Optional[str]
...
RBAC_ROLES = Literal[
    MishikallmUserRoles.PROXY_ADMIN,
    MishikallmUserRoles.TEAM,
    MishikallmUserRoles.INTERNAL_USER,
]


class OIDCPermissions(MishikaLLMPydanticObjectBase):
    models: Optional[List[str]] = None
...
class MishikaLLM_JWTAuth(MishikaLLMPydanticObjectBase):
    """
    A class to define the roles and permissions for a MishikaLLM Proxy w/ JWT Auth.
    ...
    - user_id_jwt_field: The field in the JWT token that stores the user id (maps to `MishikaLLMUserTable`). Use this for internal employees.
    - user_email_jwt_field: The field in the JWT token that stores the user email (maps to `MishikaLLMUserTable`). Use this for internal employees.
    - user_allowed_email_subdomain: If specified, only emails from specified subdomain will be allowed to access proxy.
    - end_user_id_jwt_field: The field in the JWT token that stores the end-user ID (maps to `MishikaLLMEndUserTable`). Turn this off by setting to `None`. Enables end-user cost tracking. Use this for external customers.
    - public_key_ttl: Default - 600s. TTL for caching public JWT keys.
    - public_allowed_routes: list of allowed routes for authenticated but unknown mishikallm role jwt tokens.
    ...
    admin_jwt_scope: str = "mishikallm_proxy_admin"
...
class DefaultInternalUserParams(MishikaLLMPydanticObjectBase):
    """
    ...
    user_role: Optional[
        ...
        default=MishikallmUserRoles.INTERNAL_USER,
...
class SpendUpdateQueueItem(TypedDict, total=False):
    entity_type: Mishikallm_EntityType
    entity_id: str
    response_cost: Optional[float]


class MishikaLLM_ManagedFileTable(MishikaLLMPydanticObjectBase):
    unified_file_id: str
