...
from mishikallm.caching import DualCache, RedisCache
from mishikallm.constants import DB_SPEND_UPDATE_JOB_NAME
...
from mishikallm.proxy.db.db_transaction_queue.daily_spend_update_queue import (
    DailySpendUpdateQueue,
)
from mishikallm.proxy.db.db_transaction_queue.pod_lock_manager import PodLockManager
from mishikallm.proxy.db.db_transaction_queue.redis_update_buffer import RedisUpdateBuffer
from mishikallm.proxy.db.db_transaction_queue.spend_update_queue import SpendUpdateQueue
...
class DBSpendUpdateWriter:
    ...
    async def update_database(
        # MishikaLLM management object fields
        self,
        token: Optional[str],
        ...
        completion_response: Optional[Union[mishikallm.ModelResponse, Any, Exception]],
        ...
        from mishikallm.proxy.utils import ProxyUpdateSpend, hash_token
        ...
        try:
            ...
            asyncio.create_task(
                self._update_user_db(
                    ...
                    mishikallm_proxy_budget_name=mishikallm_proxy_budget_name,
        ...
        try:
            ...
            await self.spend_update_queue.add_update(
                update=SpendUpdateQueueItem(
                    entity_type=Mishikallm_EntityType.KEY,
    ...
    async def _update_user_db(
        ...
        mishikallm_proxy_budget_name: Optional[str],
        end_user_id: Optional[str] = None,
    ):
        """
        - Update that user's row
        - Update mishikallm-proxy-budget row (global proxy spend)
        """
        ## if an end-user is passed in, do an upsert - we can't guarantee they already exist in db
        existing_user_obj = await user_api_key_cache.async_get_cache(key=user_id)
        if existing_user_obj is not None and isinstance(existing_user_obj, dict):
            existing_user_obj = MishikaLLM_UserTable(**existing_user_obj)
        try:
            if prisma_client is not None:  # update
                ...
                if (
                    ...
                    user_ids.append(mishikallm_proxy_budget_name)
                ...
                if end_user_id is not None:
                    await self.spend_update_queue.add_update(
                        update=SpendUpdateQueueItem(
                            entity_type=Mishikallm_EntityType.END_USER,
    ...
    async def _update_team_db(
        ...
        try:
            ...
            await self.spend_update_queue.add_update(
                update=SpendUpdateQueueItem(
                    entity_type=Mishikallm_EntityType.TEAM,
                    entity_id=team_id,
                    response_cost=response_cost,
                )
            )
            ...
            try:
                ...
                if user_id is not None:
                    ...
                    await self.spend_update_queue.add_update(
                        update=SpendUpdateQueueItem(
                            entity_type=Mishikallm_EntityType.TEAM_MEMBER,
    ...
    async def _update_org_db(
        ...
        try:
            ...
            await self.spend_update_queue.add_update(
                update=SpendUpdateQueueItem(
                    entity_type=Mishikallm_EntityType.ORGANIZATION,
                    entity_id=org_id,
                    response_cost=response_cost,
                )
            )
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            for (
                                ...
                                batcher.mishikallm_usertable.update_many(
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            for (
                                ...
                                batcher.mishikallm_verificationtoken.update_many(  # 'update_many' prevents error from being raised if no row exists
                                    where={"token": token},
                                    data={"spend": {"increment": response_cost}},
                                )
                    break
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            for (
                                ...
                                batcher.mishikallm_teamtable.update_many(  # 'update_many' prevents error from being raised if no row exists
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            for (
                                ...
                                batcher.mishikallm_teammembership.update_many(  # 'update_many' prevents error from being raised if no row exists
                    ...
                    async with prisma_client.db.tx(
                        ...
                        async with transaction.batch_() as batcher:
                            for (
                                ...
                                batcher.mishikallm_organizationtable.update_many(  # 'update_many' prevents error from being raised if no row exists
    ...
    async def _update_daily_spend(
        ...
        """
        from mishikallm.proxy.utils import _raise_failed_update_spend_exception
    ...
    async def update_daily_user_spend(
        ...
        """
        Batch job to update MishikaLLM_DailyUserSpend table using in-memory daily_spend_transactions
        """
    ...
    async def update_daily_team_spend(
        ...
        """
        Batch job to update MishikaLLM_DailyTeamSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_tag_spend(
        ...
        """
        Batch job to update MishikaLLM_DailyTagSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
