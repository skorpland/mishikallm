...
from mishikallm import Router, provider_list
from mishikallm._logging import verbose_proxy_logger
from mishikallm.proxy._types import *
from mishikallm.types.router import CONFIGURABLE_CLIENTSIDE_AUTH_PARAMS
...
async def pre_db_read_auth_checks(
    ...
    """
    from mishikallm.proxy.proxy_server import general_settings, llm_router, premium_user
...
def route_in_additonal_public_routes(current_route: str):
    ...
    In order to use this the mishikallm config.yaml should have the following in general_settings:

    ```yaml
    general_settings:
        master_key: sk-1234
        public_routes: ["MishikaLLMRoutes.public_routes", "/spend/calculate"]
    ```
    """
...
def should_run_auth_on_pass_through_provider_route(route: str) -> bool:
    """
    Use this to decide if the rest of the MishikaLLM Virtual Key auth checks should run on /vertex-ai/{endpoint} routes
    Use this to decide if the rest of the MishikaLLM Virtual Key auth checks should run on provider pass through routes
    ex /vertex-ai/{endpoint} routes
    Run virtual key auth if the following is try:
    - User is premium_user
    - User has enabled mishikallm_setting.use_client_credentials_pass_through_routes
    """
    from mishikallm.proxy.proxy_server import general_settings, premium_user
    ...
    # only enabled for MishikaLLM Enterprise
    return True
...
def get_end_user_id_from_request_body(request_body: dict) -> Optional[str]:
    ...
    # anthropic - check 'mishikallm_metadata'
    end_user_id = request_body.get("mishikallm_metadata", {}).get("user", None)
    if end_user_id:
