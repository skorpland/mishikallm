...
def organization_role_based_access_check(
    ...
    2. IF route is a MishikaLLMRoutes.org_admin_only_routes, then check if user is an Org Admin for that organization
    ...
    if route == "/organization/new":
        if user_object.user_role != MishikallmUserRoles.PROXY_ADMIN.value:
    ...
    if route in MishikaLLMRoutes.org_admin_only_routes.value:
        (
            _user_organizations,
        ...
        user_role: Optional[MishikallmUserRoles] = _user_organization_role_mapping.get(
        ...
        if user_role != MishikallmUserRoles.ORG_ADMIN.value:
            raise ProxyException(
        ...
        if (
            ...
            if _user_role_in_passed_org != MishikallmUserRoles.ORG_ADMIN.value:
                raise ProxyException(
...
def get_user_organization_info(
...
) -> Tuple[List[str], Dict[str, Optional[MishikallmUserRoles]]]:
    """
    Helper function to extract user organization information.

    Args:
        user_object (MishikaLLM_UserTable): The user object containing organization memberships.

    Returns:
        Tuple[List[str], Dict[str, Optional[MishikallmUserRoles]]]: A tuple containing:
    ...
    _user_organization_role_mapping: Dict[str, Optional[MishikallmUserRoles]] = {}
...
def _user_is_org_admin(
    ...
    for _membership in user_object.organization_memberships:
        if _membership.organization_id == request_data.get("organization_id", None):
            if _membership.user_role == MishikallmUserRoles.ORG_ADMIN.value:
                return True
