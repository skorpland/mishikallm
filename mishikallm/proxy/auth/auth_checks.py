...
from mishikallm.caching.dual_cache import LimitedSizeOrderedDict
from mishikallm.constants import DEFAULT_IN_MEMORY_TTL
from mishikallm.mishikallm_core_utils.get_llm_provider_logic import get_llm_provider
from mishikallm.proxy._types import (
    RBAC_ROLES,
    CallInfo,
    MishikaLLM_EndUserTable,
    Mishikallm_EntityType,
    MishikaLLM_JWTAuth,
    MishikaLLM_OrganizationMembershipTable,
...
from mishikallm.proxy.auth.route_checks import RouteChecks
from mishikallm.proxy.route_llm_request import route_request
from mishikallm.proxy.utils import PrismaClient, ProxyLogging, log_db_metrics
...
all_routes = MishikaLLMRoutes.openai_routes.value + MishikaLLMRoutes.management_routes.value
...
async def common_checks(
    ...
    7. [OPTIONAL] If 'mishikallm.max_budget' is set (>0), is proxy under budget
    ...
    if (
        ...
        if user_budget < user_object.spend:
            raise mishikallm.BudgetExceededError(
    ...
    if end_user_object is not None and end_user_object.mishikallm_budget_table is not None:
        end_user_budget = end_user_object.mishikallm_budget_table.max_budget
    ...
    if (
        ...
        if global_proxy_spend > mishikallm.max_budget:
            raise mishikallm.BudgetExceededError(
                current_cost=global_proxy_spend, max_budget=mishikallm.max_budget
            )
    ...
    if _request_metadata.get("guardrails"):
        ...
        from mishikallm.proxy.guardrails.guardrail_helpers import can_modify_guardrails
    ...
    token_type: Literal["ui", "api"] = (
        "ui" if token_team is not None and token_team == "mishikallm-dashboard" else "api"
    )
...
def _is_ui_route(
    route: str,
    user_obj: Optional[MishikaLLM_UserTable] = None,
) -> bool:
    """
    - Check if the route is a UI used route
    """
    # this token is only used for managing the ui
    allowed_routes = MishikaLLMRoutes.ui_routes.value
...
def _get_user_role(
    user_obj: Optional[MishikaLLM_UserTable],
    ...
    try:
        role = MishikallmUserRoles(_user_role)
    except ValueError:
        return MishikallmUserRoles.INTERNAL_USER
...
def _is_user_proxy_admin(user_obj: Optional[MishikaLLM_UserTable]):
    if user_obj is None:
        return False

    if (
        user_obj.user_role is not None
        and user_obj.user_role == MishikallmUserRoles.PROXY_ADMIN.value
    ):
        return True
...
def _allowed_routes_check(user_route: str, allowed_routes: list) -> bool:
    ...
    - allowed_routes: List[str|MishikaLLMRoutes] - the list of allowed routes for the user.
    """

    for allowed_route in allowed_routes:
        if (
            allowed_route in MishikaLLMRoutes.__members__
            and user_route in MishikaLLMRoutes[allowed_route].value
        ):
            return True
...
def allowed_routes_check(
    ...
    user_route: str,
    mishikallm_proxy_roles: MishikaLLM_JWTAuth,
) -> bool:
    """
    Check if user -> not admin - allowed to access these routes
    """

    if user_role == MishikallmUserRoles.PROXY_ADMIN:
        is_allowed = _allowed_routes_check(
            user_route=user_route,
            allowed_routes=mishikallm_proxy_roles.admin_allowed_routes,
        )
        return is_allowed

    elif user_role == MishikallmUserRoles.TEAM:
        if mishikallm_proxy_roles.team_allowed_routes is None:
            """
            ...
            return is_allowed
        elif mishikallm_proxy_roles.team_allowed_routes is not None:
            is_allowed = _allowed_routes_check(
                user_route=user_route,
                allowed_routes=mishikallm_proxy_roles.team_allowed_routes,
            )
            return is_allowed
    return False
...
def allowed_route_check_inside_route(
    ...
    if (
        ...
        and user_api_key_dict.user_role != MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY
    ):
        ret_val = False
...
def get_actual_routes(allowed_routes: list) -> list:
    ...
    for route_name in allowed_routes:
        try:
            route_value = MishikaLLMRoutes[route_name].value
...
async def get_end_user_object(
...
) -> Optional[MishikaLLM_EndUserTable]:
    """
    ...
    def check_in_budget(end_user_obj: MishikaLLM_EndUserTable):
        if end_user_obj.mishikallm_budget_table is None:
            return
        end_user_budget = end_user_obj.mishikallm_budget_table.max_budget
    ...
    if cached_user_obj is not None:
        if isinstance(cached_user_obj, dict):
            return_obj = MishikaLLM_EndUserTable(**cached_user_obj)
            check_in_budget(end_user_obj=return_obj)
            return return_obj
        elif isinstance(cached_user_obj, MishikaLLM_EndUserTable):
            return_obj = cached_user_obj
            check_in_budget(end_user_obj=return_obj)
            return return_obj
    # else, check db
    try:
        response = await prisma_client.db.mishikallm_endusertable.find_unique(
        ...
        _response = MishikaLLM_EndUserTable(**response.dict())

        check_in_budget(end_user_obj=_response)

        return _response
    except Exception as e:  # if end-user not in db
        if isinstance(e, mishikallm.BudgetExceededError):
            raise e
        return None
...
async def _get_fuzzy_user_object(
    ...
    if sso_user_id is not None:
        response = await prisma_client.db.mishikallm_usertable.find_unique(
            where={"sso_user_id": sso_user_id},
            include={"organization_memberships": True},
        )

    if response is None and user_email is not None:
        response = await prisma_client.db.mishikallm_usertable.find_first(
        ...
        if response is not None and sso_user_id is not None:  # update sso_user_id
            asyncio.create_task(  # background task to update user with sso id
                prisma_client.db.mishikallm_usertable.update(
...
async def get_user_object(
    ...
    - if valid, return MishikaLLM_UserTable object with defined limits
    ...
    if not check_db_only:
        ...
        if cached_user_obj is not None:
            if isinstance(cached_user_obj, dict):
                return MishikaLLM_UserTable(**cached_user_obj)
            elif isinstance(cached_user_obj, MishikaLLM_UserTable):
    ...
    try:
        ...
        if response is None:
            if user_id_upsert:
                response = await prisma_client.db.mishikallm_usertable.create(
        ...
        if (
            ...
            # dump each organization membership to type MishikaLLM_OrganizationMembershipTable
            _dumped_memberships = [
                MishikaLLM_OrganizationMembershipTable(**membership.model_dump())
                for membership in response.organization_memberships
                if membership is not None
            ]
            response.organization_memberships = _dumped_memberships

        _response = MishikaLLM_UserTable(**dict(response))
...
async def _get_team_db_check(
    ...
    response = await prisma_client.db.mishikallm_teamtable.find_unique(
        where={"team_id": team_id}
    )

    if response is None and team_id_upsert:
        response = await prisma_client.db.mishikallm_teamtable.create(
...
async def _get_team_object_from_db(team_id: str, prisma_client: PrismaClient):
    return await prisma_client.db.mishikallm_teamtable.find_unique(
        where={"team_id": team_id}
    )
...
async def _get_team_object_from_user_api_key_cache(
    ...
    _response = MishikaLLM_TeamTableCachedObj(**response.dict())
...
async def _get_team_object_from_cache(
...
) -> Optional[MishikaLLM_TeamTableCachedObj]:
    cached_team_obj: Optional[MishikaLLM_TeamTableCachedObj] = None
    ...
    if cached_team_obj is not None:
        if isinstance(cached_team_obj, dict):
            return MishikaLLM_TeamTableCachedObj(**cached_team_obj)
        elif isinstance(cached_team_obj, MishikaLLM_TeamTableCachedObj):
...
async def get_team_object(
    ...
    - if valid, return MishikaLLM_TeamTable object with defined limits
    - if not, then raise an error
    ...
    if prisma_client is None:
        raise Exception(
            "No DB Connected. See - https://docs.21t.cc/docs/proxy/virtual_keys"
        )
...
class ExperimentalUIJWTToken:
    @staticmethod
    def get_experimental_ui_login_jwt_auth_token(user_info: MishikaLLM_UserTable) -> str:
        ...
        valid_token = UserAPIKeyAuth(
            ...
            user_role=MishikallmUserRoles(user_info.user_role),
        )
    ...
    def get_key_object_from_ui_hash_key(
        ...
        from mishikallm.proxy.auth.user_api_key_auth import UserAPIKeyAuth
        from mishikallm.proxy.common_utils.encrypt_decrypt_utils import (
...
async def get_org_object(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional[MishikaLLM_OrganizationTable]:
    """
    - Check if org id in proxy Org Table
    - if valid, return MishikaLLM_OrganizationTable object
    - if not, then raise an error
    """
    if prisma_client is None:
        raise Exception(
    ...
    if cached_org_obj is not None:
        if isinstance(cached_org_obj, dict):
            return MishikaLLM_OrganizationTable(**cached_org_obj)
        elif isinstance(cached_org_obj, MishikaLLM_OrganizationTable):
            return cached_org_obj
    # else, check db
    try:
        response = await prisma_client.db.mishikallm_organizationtable.find_unique(
...
def _can_object_call_model(
    ...
    """
    if model in mishikallm.model_alias_map:
        model = mishikallm.model_alias_map[model]
...
async def can_key_call_model(
    ...
    valid_token: UserAPIKeyAuth,
    llm_router: Optional[mishikallm.Router],
) -> Literal[True]:
    """
    Checks if token can call a given model
    ...
    if (
        ...
        if valid_token:
            call_info = CallInfo(
                ...
                team_id=valid_token.team_id,
                team_alias=valid_token.team_alias,
                event_group=Mishikallm_EntityType.TEAM,
            )
            asyncio.create_task(
                proxy_logging_obj.budget_alerts(
                    type="team_budget",
