"""
This file handles authentication for the MishikaLLM Proxy.

it checks if the user passed a valid API Key to the MishikaLLM Proxy
...
from mishikallm.proxy.auth.auth_exception_handler import UserAPIKeyAuthExceptionHandler
from mishikallm.proxy.auth.auth_utils import (
...
from mishikallm.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
from mishikallm.proxy.auth.oauth2_check import check_oauth2_token
from mishikallm.proxy.auth.oauth2_proxy_hook import handle_oauth2_proxy_request
from mishikallm.proxy.common_utils.http_parsing_utils import _read_request_body
from mishikallm.proxy.utils import PrismaClient, ProxyLogging
from mishikallm.secret_managers.main import get_secret_bool
from mishikallm.types.services import ServiceTypes

user_api_key_service_logger_obj = ServiceLogging()  # used for tracking latency on OTEL

custom_mishikallm_key_header = APIKeyHeader(
    name=SpecialHeaders.custom_mishikallm_api_key.value,
    auto_error=False,
...
async def get_global_proxy_spend(
    ...
    if mishikallm.max_budget > 0:  # user set proxy max budget
        # check cache
        ...
        if global_proxy_spend is not None:
            user_info = CallInfo(
                ...
                token=token,
                event_group=Mishikallm_EntityType.PROXY,
            )
            asyncio.create_task(
...
def get_rbac_role(jwt_handler: JWTHandler, scopes: List[str]) -> str:
    ...
    if is_admin:
        return MishikallmUserRoles.PROXY_ADMIN
    else:
        return MishikallmUserRoles.TEAM
...
async def _user_api_key_auth_builder(  # noqa: PLR0915
    ...
    request_data: dict,
    custom_mishikallm_key_header: Optional[str] = None,
) -> UserAPIKeyAuth:
    ...
    try:
        ...
        if isinstance(custom_mishikallm_key_header, str):
            api_key = custom_mishikallm_key_header
            ...
            for endpoint in pass_through_endpoints:
                if endpoint.get("path", "") == route:
                    ...
                    if headers is not None:
                        header_key: str = headers.get("mishikallm_user_api_key", "")
                        if request.headers.get(key=header_key) is not None:
                            api_key = request.headers.get(key=header_key)

        # if user wants to pass MishikaLLM_Master_Key as a custom header, example pass mishikallm keys as X-MishikaLLM-Key: Bearer sk-1234
        custom_mishikallm_key_header_name = general_settings.get("mishikallm_key_header_name")
        if custom_mishikallm_key_header_name is not None:
            api_key = get_api_key_from_custom_header(
                request=request,
                custom_mishikallm_key_header_name=custom_mishikallm_key_header_name,
            )

        if open_telemetry_logger is not None:
            parent_otel_span = (
                open_telemetry_logger.create_mishikallm_proxy_request_started_span(
        ...
        ### MISHIKALLM-DEFINED AUTH FUNCTION ###
        #### IF JWT ####
        """
        MishikaLLM supports using JWTs.
        ...
        if (
            route in MishikaLLMRoutes.public_routes.value  # type: ignore
            or route_in_additonal_public_routes(current_route=route)
        ):
            # check if public endpoint
            return UserAPIKeyAuth(user_role=MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY)
            ...
            if should_run_auth_on_pass_through_provider_route(route=route) is False:
                return UserAPIKeyAuth(
                    user_role=MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY
                )
        ...
        for mapped_route in MishikaLLMRoutes.mapped_pass_through_routes.value:  # type: ignore
            if route.startswith(mapped_route):
                is_mapped_pass_through_route = True
        if is_mapped_pass_through_route:
            if request.headers.get("mishikallm_user_api_key") is not None:
                api_key = request.headers.get("mishikallm_user_api_key") or ""
            ...
            for endpoint in pass_through_endpoints:
                if isinstance(endpoint, dict) and endpoint.get("path", "") == route:
                    ...
                    if (
                        ...
                        - check the langfuse public key if it contains the mishikallm api key
                        """
                        import base64
                        ...
                        if headers is not None:
                            header_key = headers.get("mishikallm_user_api_key", "")
                            if (
        ...
        if end_user_id:
            try:
                ...
                if _end_user_object is not None:
                    ...
                    if _end_user_object.mishikallm_budget_table is not None:
                        budget_info = _end_user_object.mishikallm_budget_table
        ...
        if (
            ...
            and valid_token.user_role == MishikallmUserRoles.PROXY_ADMIN
        ):
        ...
        if (
            ...
            try:
                team_obj: MishikaLLM_TeamTableCachedObj = await get_team_object(
        ...
        if route in MishikaLLMRoutes.master_key_only_routes.value:  # type: ignore
            raise Exception(
        ...
        if isinstance(
            ...
            assert api_key.startswith(
                "sk-"
            ), "MishikaLLM Virtual Key expected. Received={}, expected to start with 'sk-'.".format(
                api_key
            )  # prevent token hashes from being used
        else:
            verbose_logger.warning(
                "mishikallm.proxy.proxy_server.user_api_key_auth(): Warning - Key={} is not a string.".format(
                    api_key
                )
            )
        ...
        if valid_token is None:
            raise Exception(
                "Invalid proxy server token passed. Received API Key (hashed)={}. Unable to find token in cache or `MishikaLLM_VerificationTokenTable`".format(
                    api_key
                )
            )
        ...
        if valid_token is not None:
            ...
            if valid_token.user_id is not None:
                try:
                    ...
                    verbose_logger.debug(
                        "mishikallm.proxy.auth.user_api_key_auth.py::user_api_key_auth() - Unable to get user from db/cache. Setting user_obj to None. Exception received - {}".format(
                            str(e)
                        )
                    )
                    user_obj = None
                ...
                if prisma_client is not None:
                    ...
                    if team_member_info is None:
                        ...
                        if _user_id is not None and _team_id is not None:
                            team_member_info = await prisma_client.db.mishikallm_teammembership.find_first(
                                where={
                                    "user_id": _user_id,
                                    "team_id": _team_id,
                                },  # type: ignore
                    ...
                    if (
                        ...
                        and team_member_info.mishikallm_budget_table is not None
                    ):
                        team_member_budget = (
                            team_member_info.mishikallm_budget_table.max_budget
                        )
            ...
            if route in MishikaLLMRoutes.llm_api_routes.value:
            ...
            if valid_token.team_id is not None:
                _team_obj: Optional[MishikaLLM_TeamTable] = MishikaLLM_TeamTable(
                    team_id=valid_token.team_id,
            ...
            global_proxy_spend = None
            if (
                mishikallm.max_budget > 0 and prisma_client is not None
        ...
        # check if token is from mishikallm-ui, mishikallm ui makes keys to allow users to login with sso. These keys can only be used for MishikaLLM UI functions
...
async def user_api_key_auth(
    ...
    custom_mishikallm_key_header: Optional[str] = fastapi.Security(
        custom_mishikallm_key_header
    ),
) -> UserAPIKeyAuth:
    """
    ...
    user_api_key_auth_obj = await _user_api_key_auth_builder(
        ...
        request_data=request_data,
        custom_mishikallm_key_header=custom_mishikallm_key_header,
    )
...
async def _return_user_api_key_auth_obj(
    ...
    retrieved_user_role = (
        user_role or _get_user_role(user_obj=user_obj) or MishikallmUserRoles.INTERNAL_USER
    )
...
def get_api_key_from_custom_header(
    request: Request, custom_mishikallm_key_header_name: str
) -> str:
    """
    ...
    Args:
        ...
        custom_mishikallm_key_header_name (str): Custom header name

    Returns:
        Optional[str]: API key
    """
    api_key: str = ""
    # use this as the virtual key passed to mishikallm proxy
    custom_mishikallm_key_header_name = custom_mishikallm_key_header_name.lower()
    _headers = {k.lower(): v for k, v in request.headers.items()}
    verbose_proxy_logger.debug(
        "searching for custom_mishikallm_key_header_name= %s, in headers=%s",
        custom_mishikallm_key_header_name,
        _headers,
    )
    custom_api_key = _headers.get(custom_mishikallm_key_header_name)
    if custom_api_key:
        api_key = _get_bearer_token(api_key=custom_api_key)
        verbose_proxy_logger.debug(
            "Found custom API key using header: {}, setting api_key={}".format(
                custom_mishikallm_key_header_name, api_key
            )
        )
    else:
        verbose_proxy_logger.exception(
            f"No MishikaLLM Virtual Key pass. Please set header={custom_mishikallm_key_header_name}: Bearer <api_key>"
        )
    return api_key
