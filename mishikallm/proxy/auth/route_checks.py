...
class RouteChecks:
    ...
    def non_proxy_admin_allowed_routes_check(
        ...
        _user_role: Optional[MishikallmUserRoles],
        route: str,
        request: Request,
        ...
        if RouteChecks.is_llm_api_route(route=route):
            ...
            route in MishikaLLMRoutes.info_routes.value
            ...
            route in MishikaLLMRoutes.global_spend_tracking_routes.value
            and getattr(valid_token, "permissions", None) is not None
            and "get_spend_routes" in getattr(valid_token, "permissions", [])
        ):
            pass
        elif _user_role == MishikallmUserRoles.PROXY_ADMIN_VIEW_ONLY.value:
            if RouteChecks.is_llm_api_route(route=route):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
            ...
            if RouteChecks.check_route_access(
                route=route, allowed_routes=MishikaLLMRoutes.management_routes.value
            ):
            ...
            _user_role == MishikallmUserRoles.INTERNAL_USER.value
            and RouteChecks.check_route_access(
                route=route, allowed_routes=MishikaLLMRoutes.internal_user_routes.value
            )
        ):
            pass
            ...
            route=route, allowed_routes=MishikaLLMRoutes.org_admin_allowed_routes.value
        ):
            pass
        elif (
            _user_role == MishikallmUserRoles.INTERNAL_USER_VIEW_ONLY.value
            and RouteChecks.check_route_access(
                route=route,
                allowed_routes=MishikaLLMRoutes.internal_user_view_only_routes.value,
            )
        ):
            pass
        elif RouteChecks.check_route_access(
            route=route, allowed_routes=MishikaLLMRoutes.self_managed_routes.value
    ...
    def custom_admin_only_route_check(route: str):
        from mishikallm.proxy.proxy_server import general_settings, premium_user
    ...
    def is_llm_api_route(route: str) -> bool:
        ...
        if route in MishikaLLMRoutes.openai_routes.value:
            return True

        if route in MishikaLLMRoutes.anthropic_routes.value:
            return True

        # fuzzy match routes like "/v1/threads/thread_49EIN5QF32s4mH20M7GFKdlZ"
        # Check for routes with placeholders
        for openai_route in MishikaLLMRoutes.openai_routes.value:
        ...
        for _llm_passthrough_route in MishikaLLMRoutes.mapped_pass_through_routes.value:
