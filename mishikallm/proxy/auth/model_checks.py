...
from mishikallm.proxy._types import SpecialModelNames, UserAPIKeyAuth
from mishikallm.router import Router
from mishikallm.types.router import MishikaLLM_Params
...
def get_provider_models(
    provider: str, mishikallm_params: Optional[MishikaLLM_Params] = None
) -> Optional[List[str]]:
    """
    Returns the list of known models by provider
    """
    if provider == "*":
        return get_valid_models(mishikallm_params=mishikallm_params)

    if provider in mishikallm.models_by_provider:
        provider_models = get_valid_models(
            custom_llm_provider=provider, mishikallm_params=mishikallm_params
        )
        # provider_models = copy.deepcopy(mishikallm.models_by_provider[provider])
...
def get_known_models_from_wildcard(
    wildcard_model: str, mishikallm_params: Optional[MishikaLLM_Params] = None
) -> List[str]:
    try:
    ...
    wildcard_models = get_provider_models(
        provider=provider, mishikallm_params=mishikallm_params
    )
...
def _get_wildcard_models(
    ...
    for model in unique_models:
        if _check_wildcard_routing(model=model):
            ...
            ## get mishikallm params from model
                    ...
                    for router_model in model_list:
                        wildcard_models = get_known_models_from_wildcard(
                            ...
                            mishikallm_params=MishikaLLM_Params(
                                **router_model["mishikallm_params"]  # type: ignore
                            ),
                        )
