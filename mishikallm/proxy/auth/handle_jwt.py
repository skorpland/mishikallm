...
JWT token must have 'mishikallm_proxy_admin' in scope. 
"""
...
from mishikallm.mishikallm_core_utils.dot_notation_indexing import get_nested_value
...
from mishikallm.proxy.auth.auth_checks import can_team_access_model
...
class JWTHandler:
    ...
    - if role="mishikallm_proxy_user" -> allow making calls + info. Can not edit budgets
    """
    ...
    def update_environment(
        ...
        self.mishikallm_jwtauth = mishikallm_jwtauth
        self.leeway = leeway
    ...
    def _rbac_role_from_role_mapping(self, token: dict) -> Optional[RBAC_ROLES]:
        ...
        """
        if self.mishikallm_jwtauth.role_mappings is None:
            return None
        ...
        for role_mapping in self.mishikallm_jwtauth.role_mappings:
    ...
    def is_admin(self, scopes: list) -> bool:
        if self.mishikallm_jwtauth.admin_jwt_scope in scopes:
            return True
        return False

    def get_team_ids_from_jwt(self, token: dict) -> List[str]:
        if (
            self.mishikallm_jwtauth.team_ids_jwt_field is not None
            and token.get(self.mishikallm_jwtauth.team_ids_jwt_field) is not None
        ):
            return token[self.mishikallm_jwtauth.team_ids_jwt_field]
        return []
    ...
    def get_end_user_id(
        ...
        try:
            if self.mishikallm_jwtauth.end_user_id_jwt_field is not None:
                user_id = token[self.mishikallm_jwtauth.end_user_id_jwt_field]
            else:
                user_id = None
        except KeyError:
    ...
    def is_required_team_id(self) -> bool:
        ...
        - False: if not
        """
        if self.mishikallm_jwtauth.team_id_jwt_field is None:
            return False
        return True
    ...
    def is_enforced_email_domain(self) -> bool:
        ...
        if self.mishikallm_jwtauth.user_allowed_email_domain is not None and isinstance(
            self.mishikallm_jwtauth.user_allowed_email_domain, str
        ):
            return True
        return False
    ...
    def get_team_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.mishikallm_jwtauth.team_id_jwt_field is not None:
                team_id = token[self.mishikallm_jwtauth.team_id_jwt_field]
            elif self.mishikallm_jwtauth.team_id_default is not None:
                team_id = self.mishikallm_jwtauth.team_id_default
            else:
                team_id = None
        except KeyError:
    ...
    def is_upsert_user_id(self, valid_user_email: Optional[bool] = None) -> bool:
        ...
        if valid_user_email is False:
            return False
        return self.mishikallm_jwtauth.user_id_upsert

    def get_user_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.mishikallm_jwtauth.user_id_jwt_field is not None:
                user_id = token[self.mishikallm_jwtauth.user_id_jwt_field]
            else:
                user_id = default_value
        except KeyError:
            user_id = default_value
    ...
    def get_user_roles(
        ...
        """
        try:
            if self.mishikallm_jwtauth.user_roles_jwt_field is not None:
                user_roles = get_nested_value(
                    data=token,
                    key_path=self.mishikallm_jwtauth.user_roles_jwt_field,
    ...
    def get_jwt_role(
        ...
        """
        try:
            if self.mishikallm_jwtauth.roles_jwt_field is not None:
                user_roles = get_nested_value(
                    data=token,
                    key_path=self.mishikallm_jwtauth.roles_jwt_field,
                    default=default_value,
                )
            else:
                user_roles = default_value
    ...
    def is_allowed_user_role(self, user_roles: Optional[List[str]]) -> bool:
        ...
        if (
            ...
            and self.mishikallm_jwtauth.user_allowed_roles is not None
            and any(
                role in self.mishikallm_jwtauth.user_allowed_roles for role in user_roles
            )
        ):
            return True
        return False
    ...
    def get_user_email(
        ...
        try:
            if self.mishikallm_jwtauth.user_email_jwt_field is not None:
                user_email = token[self.mishikallm_jwtauth.user_email_jwt_field]
            else:
                user_email = None
        except KeyError:
    ...
    def get_object_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.mishikallm_jwtauth.object_id_jwt_field is not None:
                object_id = token[self.mishikallm_jwtauth.object_id_jwt_field]
            else:
    ...
    def get_org_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.mishikallm_jwtauth.org_id_jwt_field is not None:
                org_id = token[self.mishikallm_jwtauth.org_id_jwt_field]
            else:
                org_id = None
        except KeyError:
    ...
    async def get_public_key(self, kid: Optional[str]) -> dict:
        ...
        for key_url in keys_url_list:
            cache_key = f"mishikallm_jwt_auth_keys_{key_url}"
            ...
            if cached_keys is None:
                ...
                await self.user_api_key_cache.async_set_cache(
                    key=cache_key,
                    value=keys,
                    ttl=self.mishikallm_jwtauth.public_key_ttl,  # cache for 10 mins
                )
            else:
                keys = cached_keys
    ...
    def is_allowed_domain(self, user_email: str) -> bool:
        if self.mishikallm_jwtauth.user_allowed_email_domain is None:
            return True

        email_domain = user_email.split("@")[-1]  # Extract domain from email
        if email_domain == self.mishikallm_jwtauth.user_allowed_email_domain:
            return True
        else:
            return False
...
class JWTAuthManager:
    ...
    async def check_rbac_role(
        ...
        if jwt_handler.mishikallm_jwtauth.enforce_rbac is True:
    ...
    async def check_admin_access(
        ...
        is_allowed = allowed_routes_check(
            ...
            user_route=route,
            mishikallm_proxy_roles=jwt_handler.mishikallm_jwtauth,
        )
        if not is_allowed:
            allowed_routes: List[Any] = jwt_handler.mishikallm_jwtauth.admin_allowed_routes
    ...
    async def find_and_validate_specific_team_id(
    ...
    ) -> Tuple[Optional[str], Optional[MishikaLLM_TeamTable]]:
        ...
        if not individual_team_id and jwt_handler.is_required_team_id() is True:
            raise Exception(
                f"No team id found in token. Checked team_id field '{jwt_handler.mishikallm_jwtauth.team_id_jwt_field}'"
            )

        ## VALIDATE TEAM OBJECT ###
        team_object: Optional[MishikaLLM_TeamTable] = None
        if individual_team_id:
            team_object = await get_team_object(
                ...
                team_id_upsert=jwt_handler.mishikallm_jwtauth.team_id_upsert,
            )
    ...
    async def find_team_with_model_access(
        ...
        if not team_ids:
            if jwt_handler.mishikallm_jwtauth.enforce_team_based_model_access:
                raise HTTPException(
                    status_code=403,
            ...
            try:
                ...
                if team_object and team_object.models is not None:
                    ...
                    if isinstance(team_models, list) and (
                        ...
                        is_allowed = allowed_routes_check(
                            user_role=MishikallmUserRoles.TEAM,
                            user_route=route,
                            mishikallm_proxy_roles=jwt_handler.mishikallm_jwtauth,
                        )
                        if is_allowed:
    ...
    async def get_objects(
        ...
        org_object: Optional[MishikaLLM_OrganizationTable] = None
        if org_id:
            org_object = (
        ...
        end_user_object: Optional[MishikaLLM_EndUserTable] = None
        if end_user_id:
            end_user_object = (
    ...
    async def auth_builder(
        ...
        if jwt_handler.mishikallm_jwtauth.custom_validate:
            if not jwt_handler.mishikallm_jwtauth.custom_validate(jwt_valid_token):
                raise HTTPException(
                    status_code=403,
        ...
        if (
            jwt_handler.mishikallm_jwtauth.enforce_scope_based_access
            and jwt_handler.mishikallm_jwtauth.scope_mappings
        ):
            JWTAuthManager.check_scope_based_access(
                scope_mappings=jwt_handler.mishikallm_jwtauth.scope_mappings,
                scopes=scopes,
        ...
        if rbac_role and object_id:
            if rbac_role == MishikallmUserRoles.TEAM:
                team_id = object_id
            elif rbac_role == MishikallmUserRoles.INTERNAL_USER:
                user_id = object_id
