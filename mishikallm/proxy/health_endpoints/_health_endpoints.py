...
from mishikallm.constants import HEALTH_CHECK_TIMEOUT_SECONDS
...
from mishikallm.proxy.health_check import (
    _clean_endpoint_data,
    _update_mishikallm_params_for_health_check,
...
async def health_services_endpoint(  # noqa: PLR0915
    ...
    try:
        ...
        if (
            ...
            or (service in mishikallm.success_callback and service != "langfuse")
        ):
            _ = await mishikallm.acompletion(
                model="openai/mishikallm-mock-response-model",
                messages=[{"role": "user", "content": "Hey, how's it going?"}],
                user="mishikallm:/health/services",
            ...
            from mishikallm.integrations.datadog.datadog import DataDogLogger
            ...
            from mishikallm.integrations.langfuse.langfuse import LangFuseLogger
        ...
        if service == "slack" or service == "slack_budget_alerts":
            if "slack" in general_settings.get("alerting", []):
                # test_message = f"""\n🚨 `ProjectedLimitExceededError` 💸\n\n`Key Alias:` mishikallm-ui-test-alert \n`Expected Day of Error`: 28th March \n`Current Spend`: $100.00 \n`Projected Spend at end of month`: $1000.00 \n`Soft Limit`: $700"""
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.health_services_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def health_endpoint(
    ...
    See more 👉 https://docs.21t.cc/docs/proxy/health
    ...
    try:
        if llm_model_list is None:
            # if no router set, check if user set a model using mishikallm --model ollama/llama2
        ...
        verbose_proxy_logger.error(
            "mishikallm.proxy.proxy_server.py::health_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def active_callbacks():
    """
    Returns a list of mishikallm level settings
    ...
    {
        "alerting": _alerting,
        "mishikallm.callbacks": mishikallm_callbacks,
        "mishikallm.input_callback": mishikallm_input_callbacks,
        "mishikallm.failure_callback": mishikallm_failure_callbacks,
        "mishikallm.success_callback": mishikallm_success_callbacks,
        "mishikallm._async_success_callback": mishikallm_async_success_callbacks,
        "mishikallm._async_failure_callback": mishikallm_async_failure_callbacks,
        "mishikallm._async_input_callback": mishikallm_async_input_callbacks,
        "all_mishikallm_callbacks": all_mishikallm_callbacks,
        "num_callbacks": len(all_mishikallm_callbacks),
        "num_alerting": _num_alerting,
        "mishikallm.request_timeout": mishikallm.request_timeout,
    }
    ```
    """

    from mishikallm.proxy.proxy_server import general_settings, proxy_logging_obj

    _alerting = str(general_settings.get("alerting"))
    # get success callbacks

    mishikallm_callbacks = [str(x) for x in mishikallm.callbacks]
    mishikallm_input_callbacks = [str(x) for x in mishikallm.input_callback]
    mishikallm_failure_callbacks = [str(x) for x in mishikallm.failure_callback]
    mishikallm_success_callbacks = [str(x) for x in mishikallm.success_callback]
    mishikallm_async_success_callbacks = [str(x) for x in mishikallm._async_success_callback]
    mishikallm_async_failure_callbacks = [str(x) for x in mishikallm._async_failure_callback]
    mishikallm_async_input_callbacks = [str(x) for x in mishikallm._async_input_callback]

    all_mishikallm_callbacks = (
        mishikallm_callbacks
        + mishikallm_input_callbacks
        + mishikallm_failure_callbacks
        + mishikallm_success_callbacks
        + mishikallm_async_success_callbacks
        + mishikallm_async_failure_callbacks
        + mishikallm_async_input_callbacks
    )
...
async def health_readiness():
    ...
    """
    from mishikallm.proxy.proxy_server import prisma_client, version
    ...
    try:
        ...
        try:
            ...
            success_callback_names = [
                callback_name(x) for x in mishikallm.success_callback
            ]
        except AttributeError:
            # don't let this block the /health/readiness response, if we can't convert to str -> return mishikallm.success_callback
            success_callback_names = mishikallm.success_callback

        # check Cache
        cache_type = None
        if mishikallm.cache is not None:
            from mishikallm.caching.caching import RedisSemanticCache

            cache_type = mishikallm.cache.type

            if isinstance(mishikallm.cache.cache, RedisSemanticCache):
                # ping the cache
                # TODO: @ishaan-jaff - we should probably not ping the cache on every /health/readiness check
                try:
                    index_info = await mishikallm.cache.cache._index_info()
                except Exception as e:
...
@router.get(
    "/health/liveliness",  # Historical MishikaLLM name; doesn't match k8s terminology but kept for backwards compatibility
    tags=["health"],
    dependencies=[Depends(user_api_key_auth)],
)
@router.get(
...
async def test_model_connection(
    ...
    mishikallm_params: Dict = fastapi.Body(
        None,
        description="Parameters for mishikallm.completion, mishikallm.embedding for the health check",
    ),
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
):
    """
    ...
    curl -X POST 'http://localhost:4000/health/test_connection' \\
      ...
      -d '{
        "mishikallm_params": {
            ...
            "mishikallm_credential_name": null,
            "api_key": "6xxxxxxx",
            "api_base": "https://mishikallm8397336933.openai.azure.com/openai/deployments/gpt-4o/chat/completions?api-version=2024-10-21",
        },
        "mode": "chat"
      }'
    ```
    ...
    try:
        ...
        mishikallm_params = _update_mishikallm_params_for_health_check(
            model_info={},
            mishikallm_params=mishikallm_params,
        )
        mode = mode or mishikallm_params.pop("mode", None)
        result = await run_with_timeout(
            mishikallm.ahealth_check(
                model_params=mishikallm_params,
                mode=mode,
                prompt="test from mishikallm",
                input=["test from mishikallm"],
            ),
        ...
        cleaned_result = _clean_endpoint_data(
            {**mishikallm_params, **result}, details=True
        )
        ...
        verbose_proxy_logger.error(
            f"mishikallm.proxy.health_endpoints.test_model_connection(): Exception occurred - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
