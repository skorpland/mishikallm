...
Use mishikallm with Anthropic SDK, Vertex AI SDK, Cohere SDK, etc.
"""
...
from mishikallm.constants import BEDROCK_AGENT_RUNTIME_PASS_THROUGH_ROUTES
...
async def gemini_proxy_route(
    ...
    """
    [Docs](https://docs.21t.cc/docs/pass_through/google_ai_studio)
    """
    ## CHECK FOR MISHIKALLM API KEY IN THE QUERY PARAMS - ?..key=MISHIKALLM_API_KEY
...
async def cohere_proxy_route(
    ...
    """
    [Docs](https://docs.21t.cc/docs/pass_through/cohere)
    """
...
async def vllm_proxy_route(
    ...
    """
    [Docs](https://docs.21t.cc/docs/pass_through/vllm)
    """
...
async def mistral_proxy_route(
    ...
    """
    [Docs](https://docs.21t.cc/docs/anthropic_completion)
    """
...
async def bedrock_proxy_route(
    ...
    """
    [Docs](https://docs.21t.cc/docs/pass_through/bedrock)
    """
    ...
    aws_region_name = mishikallm.utils.get_secret(secret_name="AWS_REGION_NAME")
    ...
    # Add or update query parameters
    from mishikallm.llms.bedrock.chat import BedrockConverseLLM
...
async def assemblyai_proxy_route(
    ...
    from mishikallm.proxy.pass_through_endpoints.llm_provider_handlers.assembly_passthrough_logging_handler import (
...
async def azure_proxy_route(
    ...
    azure_api_key = passthrough_endpoint_router.get_credentials(
        custom_llm_provider=mishikallm.LlmProviders.AZURE.value,
        region_name=None,
    )
    ...
    return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
        ...
        custom_llm_provider=mishikallm.LlmProviders.AZURE,
    )
...
async def _base_vertex_proxy_route(
    ...
    """
    from mishikallm.llms.vertex_ai.common_utils import (
    ...
    headers: dict = {}
    api_key_to_use = get_mishikallm_virtual_key(request=request)
    user_api_key_dict = await user_api_key_auth(
        request=request,
...
async def vertex_proxy_route(
    ...
    """
    Call MishikaLLM proxy via Vertex AI SDK.

    [Docs](https://docs.21t.cc/docs/pass_through/vertex_ai)
    """
...
async def openai_proxy_route(
    ...
    openai_api_key = passthrough_endpoint_router.get_credentials(
        custom_llm_provider=mishikallm.LlmProviders.OPENAI.value,
        region_name=None,
    )
    ...
    return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
        ...
        custom_llm_provider=mishikallm.LlmProviders.OPENAI,
    )
...
class BaseOpenAIPassThroughHandler:
    ...
    async def _base_openai_pass_through_handler(
        ...
        base_target_url: str,
        api_key: str,
        custom_llm_provider: mishikallm.LlmProviders,
    ):
        encoded_endpoint = httpx.URL(endpoint).path
        # Ensure endpoint starts with '/' for proper URL construction
        if not encoded_endpoint.startswith("/"):
    ...
    @staticmethod
    def _join_url_paths(
        base_url: httpx.URL, path: str, custom_llm_provider: mishikallm.LlmProviders
    ) -> str:
        """
