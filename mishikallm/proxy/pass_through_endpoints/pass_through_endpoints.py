...
from mishikallm.llms.custom_httpx.http_handler import get_async_httpx_client
...
from mishikallm.types.utils import StandardLoggingUserAPIKeyMetadata
...
async def set_env_variables_in_header(custom_headers: Optional[dict]) -> Optional[dict]:
    ...
    for key, value in custom_headers.items():
        # langfuse Api requires base64 encoded headers - it's simpleer to just ask mishikallm users to set their langfuse public and secret keys
...
async def chat_completion_pass_through_endpoint(  # noqa: PLR0915
    ...
    try:
        ...
        if "api_key" in data:
            llm_response = asyncio.create_task(mishikallm.aadapter_completion(**data))
        elif (
            ...
            llm_response = asyncio.create_task(llm_router.aadapter_completion(**data))
        elif user_model is not None:  # `mishikallm --model <your-model-name>`
            llm_response = asyncio.create_task(mishikallm.aadapter_completion(**data))
        else:
            raise HTTPException(
...
class HttpPassThroughEndpointHelpers:
    ...
    def get_response_headers(
        ...
        mishikallm_call_id: Optional[str] = None,
        ...
        if mishikallm_call_id:
            return_headers["x-mishikallm-call-id"] = mishikallm_call_id
        if custom_headers:
...
async def pass_through_request(  # noqa: PLR0915
    ...
    try:
        ...
        logging_obj.update_environment_variables(
            ...
            user="unknown",
            optional_params={},
            mishikallm_params=kwargs["mishikallm_params"],
            call_type="pass_through_endpoint",
        )
        logging_obj.model_call_details["mishikallm_call_id"] = mishikallm_call_id
        ...
        if stream:
            ...
            return StreamingResponse(
                PassThroughStreamingHandler.chunk_processor(
                    response=response,
                    request_body=_parsed_body,
                    mishikallm_logging_obj=logging_obj,
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.pass_through_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
...
def _init_kwargs_for_pass_through_endpoint(
    ...
    _mishikallm_metadata: Optional[dict] = _parsed_body.pop("mishikallm_metadata", None)
    _metadata = dict(
    ...
    if _mishikallm_metadata:
        _metadata.update(_mishikallm_metadata)
...
def create_pass_through_route(
    ...
    try:
        ...
        mishikallm.adapters = [{"id": adapter_id, "adapter": adapter}]
...
async def initialize_pass_through_endpoints(pass_through_endpoints: list):
    ...
    from mishikallm.proxy._types import CommonProxyErrors, MishikaLLMRoutes
    from mishikallm.proxy.proxy_server import app, premium_user
    ...
    for endpoint in pass_through_endpoints:
        ...
        if _auth is not None and str(_auth).lower() == "true":
            ...
            MishikaLLMRoutes.openai_routes.value.append(_path)
...
async def get_pass_through_endpoints(
    ...
    """
    from mishikallm.proxy.proxy_server import get_config_general_settings
