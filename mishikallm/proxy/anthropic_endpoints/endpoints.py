...
async def anthropic_response(  # noqa: PLR0915
    ...
    """
    Use `{PROXY_BASE_URL}/anthropic/v1/messages` instead - [Docs](https://docs.21t.cc/docs/anthropic_completion).
    ...
    try:
        ...
        if (
            ...
            llm_response = asyncio.create_task(mishikallm.anthropic_messages(**data))
        else:
            raise HTTPException(
        ...
        verbose_proxy_logger.exception(
            "mishikallm.proxy.proxy_server.anthropic_response(): Exception occured - {}".format(
                str(e)
            )
        )
        error_msg = f"{str(e)}"
