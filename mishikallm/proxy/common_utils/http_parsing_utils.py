...
def check_file_size_under_limit(
    ...
    if llm_router is not None and request_data["model"] in router_model_names:
        try:
            ...
            if (
                deployment
                and deployment.mishikallm_params is not None
                and deployment.mishikallm_params.max_file_size_mb is not None
            ):
                max_file_size_mb = deployment.mishikallm_params.max_file_size_mb
        except Exception as e:
