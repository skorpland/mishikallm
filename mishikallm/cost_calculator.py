...
from mishikallm.mishikallm_core_utils.llm_cost_calc.utils import (
    _generic_cost_per_character,
    generic_cost_per_token,
    select_cost_metric_for_model,
)
from mishikallm.llms.anthropic.cost_calculation import (
    cost_per_token as anthropic_cost_per_token,
)
from mishikallm.llms.azure.cost_calculation import (
    cost_per_token as azure_openai_cost_per_token,
)
from mishikallm.llms.bedrock.image.cost_calculator import (
    cost_calculator as bedrock_image_cost_calculator,
)
from mishikallm.llms.databricks.cost_calculator import (
    cost_per_token as databricks_cost_per_token,
)
from mishikallm.llms.deepseek.cost_calculator import (
    cost_per_token as deepseek_cost_per_token,
)
from mishikallm.llms.fireworks_ai.cost_calculator import (
    cost_per_token as fireworks_ai_cost_per_token,
)
from mishikallm.llms.gemini.cost_calculator import cost_per_token as gemini_cost_per_token
from mishikallm.llms.openai.cost_calculation import (
    cost_per_second as openai_cost_per_second,
)
from mishikallm.llms.openai.cost_calculation import cost_per_token as openai_cost_per_token
from mishikallm.llms.together_ai.cost_calculator import get_model_params_and_category
from mishikallm.llms.vertex_ai.cost_calculator import (
    cost_per_character as google_cost_per_character,
)
from mishikallm.llms.vertex_ai.cost_calculator import (
    cost_per_token as google_cost_per_token,
)
from mishikallm.llms.vertex_ai.cost_calculator import cost_router as google_cost_router
from mishikallm.llms.vertex_ai.image_generation.cost_calculator import (
...
from mishikallm.types.rerank import RerankBilledUnits, RerankResponse
...
def cost_per_token(  # noqa: PLR0915
    ...
    model_cost_ref = mishikallm.model_cost
    ...
    if custom_llm_provider is not None:
        ...
        _, custom_llm_provider, _, _ = mishikallm.get_llm_provider(model=model)
    ...
    if len(model_parts) > 1:
        ...
        model_without_prefix = model
    """
    Code block that formats model to lookup in mishikallm.model_cost
    ...
    if call_type == "speech" or call_type == "aspeech":
        speech_model_info = mishikallm.get_model_info(
...
def get_replicate_completion_pricing(completion_response: dict, total_time=0.0):
    ...
    # for all mishikallm currently supported LLMs, almost all requests go to a100_80gb
...
def _get_provider_for_cost_calc(
    ...
    try:
        ...
        verbose_logger.debug(
            f"mishikallm.cost_calculator.py::_get_provider_for_cost_calc() - Error inferring custom_llm_provider - {str(e)}"
        )
        return None
...
def _select_model_name_for_cost_calc(
    ...
    if custom_pricing is True:
        if router_model_id is not None and router_model_id in mishikallm.model_cost:
...
def _is_known_usage_objects(usage_obj):
    ...
    return isinstance(usage_obj, mishikallm.Usage) or isinstance(
...
def completion_cost(  # noqa: PLR0915
    ...
    """
    Calculate the cost of a given completion call fot GPT-3.5-turbo, llama2, any mishikallm supported llm.

    Parameters:
        completion_response (mishikallm.ModelResponses): [Required] The response received from a MishikaLLM completion request.
    ...
    Exceptions:
        Raises exception if model not in the mishikallm model cost map. Register model, via custom pricing or PR - https://github.com/skorpland/mishikallm/blob/main/model_prices_and_context_window.json
            ...
            try:
                ...
                if completion_response is not None and (
                    ...
                    if isinstance(usage_obj, BaseModel) and not _is_known_usage_objects(
                        ...
                        setattr(
                            completion_response,
                            "usage",
                            mishikallm.Usage(**usage_obj.model_dump()),
                        )
                    if usage_obj is None:
                        _usage = {}
                    elif isinstance(usage_obj, BaseModel):
                ...
                if custom_llm_provider is None:
                    try:
                        ...
                        verbose_logger.debug(
                            "mishikallm.cost_calculator.py::completion_cost() - Error inferring custom_llm_provider - {}".format(
                                str(e)
                            )
                        )
                if (
                    ...
                    prompt_characters = mishikallm.utils._count_characters(text=prompt)
                elif (
                    ...
                    completion_response, MishikaLLMRealtimeStreamLoggingObject
                ):
                    if (
                ...
                # see https://replicate.com/pricing
                    ...
                    model in mishikallm.replicate_models or "replicate" in model
                ) and model not in mishikallm.model_cost:
                ...
                if (
                    ...
                    if len(messages) > 0:
                        prompt_string = mishikallm.utils.get_formatted_prompt(
                            data={"messages": messages}, call_type="completion"
                        )

                        prompt_characters = mishikallm.utils._count_characters(
                            text=prompt_string
                        )
                    if completion_response is not None and isinstance(
                        completion_response, ModelResponse
                    ):
                        completion_string = mishikallm.utils.get_response_string(
                            response_obj=completion_response
                        )
                        completion_characters = mishikallm.utils._count_characters(
                ...
                verbose_logger.debug(
                    "mishikallm.cost_calculator.py::completion_cost() - Error calculating cost for model={} - {}".format(
                        model, str(e)
                    )
                )
...
def get_response_cost_from_hidden_params(
    ...
    if (
        additional_headers
        and "llm_provider-x-mishikallm-response-cost" in additional_headers
    ):
        response_cost = additional_headers["llm_provider-x-mishikallm-response-cost"]
        if response_cost is None:
            return None
        return float(additional_headers["llm_provider-x-mishikallm-response-cost"])
    return None
...
def rerank_cost(
    ...
    """
    _, custom_llm_provider, _, _ = mishikallm.get_llm_provider(
        model=model, custom_llm_provider=custom_llm_provider
    )
    ...
    try:
        ...
        try:
            model_info: Optional[ModelInfo] = mishikallm.get_model_info(
                model=model, custom_llm_provider=custom_llm_provider
            )
        except Exception:
            model_info = None
...
def default_image_cost_calculator(
    ...
    for model in models_to_check:
        if model in mishikallm.model_cost:
            cost_info = mishikallm.model_cost[model]
            break
...
class RealtimeAPITokenUsageProcessor:
    ...
    def create_logging_realtime_object(
        ...
        return MishikaLLMRealtimeStreamLoggingObject(
            usage=usage,
            results=results,
        )
...
def handle_realtime_stream_cost_calculation(
    ...
    potential_model_names.append(mishikallm_model_name)
