...
def get_secret(  # noqa: PLR0915
    ...
    key_management_system = mishikallm._key_management_system
    key_management_settings = mishikallm._key_management_settings
    secret = None
    ...
    try:
        if (
            ...
            and mishikallm.secret_manager_client is not None
        ):
            try:
                client = mishikallm.secret_manager_client
                key_manager = "local"
                ...
                if (
                    ...
                    response = client.decrypt(
                        request={
                            "name": mishikallm._google_kms_resource_name,
...
def _should_read_secret_from_secret_manager() -> bool:
    ...
    """
    if mishikallm.secret_manager_client is not None:
        if mishikallm._key_management_settings is not None:
            if (
                mishikallm._key_management_settings.access_mode == "read_only"
                or mishikallm._key_management_settings.access_mode == "read_and_write"
            ):
                return True
    return False
