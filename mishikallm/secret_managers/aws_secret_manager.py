...
Relevant issue: https://github.com/skorpland/mishikallm/issues/1883
...
def load_aws_kms(use_aws_kms: Optional[bool]):
    ...
    try:
        ...
        mishikallm.secret_manager_client = kms_client
        mishikallm._key_management_system = KeyManagementSystem.AWS_KMS
...
class AWSKeyManagementService_V2:
    ...
    def validate_environment(
        ...
        is_mishikallm_license_in_env: bool = False

        if os.getenv("MISHIKALLM_LICENSE", None) is not None:
            is_mishikallm_license_in_env = True
        elif os.getenv("MISHIKALLM_SECRET_AWS_KMS_MISHIKALLM_LICENSE", None) is not None:
            is_mishikallm_license_in_env = True
        if is_mishikallm_license_in_env is False:
            raise ValueError(
                "AWSKeyManagementService V2 is an Enterprise Feature. Please add a valid MISHIKALLM_LICENSE to your envionment."
            )
