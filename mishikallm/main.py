...
import mishikallm
from mishikallm import (  # type: ignore
    Logging,
    client,
    exception_type,
...
from mishikallm.exceptions import MishikaLLMUnknownProvider
from mishikallm.integrations.custom_logger import CustomLogger
from mishikallm.mishikallm_core_utils.audio_utils.utils import get_audio_file_for_health_check
from mishikallm.mishikallm_core_utils.health_check_utils import (
...
from mishikallm.mishikallm_core_utils.llm_request_utils import (
    pick_cheapest_chat_models_from_llm_provider,
)
from mishikallm.mishikallm_core_utils.mock_functions import (
    mock_embedding,
    mock_image_generation,
)
from mishikallm.mishikallm_core_utils.prompt_templates.common_utils import (
...
from mishikallm.realtime_api.main import _realtime_health_check
from mishikallm.secret_managers.main import get_secret_str
from mishikallm.types.router import GenericMishikaLLMParams
from mishikallm.types.utils import RawRequestTypedDict
...
from .mishikallm_core_utils.fallback_utils import (
    async_completion_with_fallbacks,
    completion_with_fallbacks,
)
from .mishikallm_core_utils.prompt_templates.common_utils import (
    get_completion_messages,
    update_messages_with_model_file_ids,
)
from .mishikallm_core_utils.prompt_templates.factory import (
    custom_prompt,
...
from .mishikallm_core_utils.streaming_chunk_builder_utils import ChunkProcessor
...
from .types.utils import (
    MISHIKALLM_IMAGE_VARIATION_PROVIDERS,
...
class MishikaLLM:
    def __init__(
        ...
        *,
        ...
        max_retries: Optional[int] = mishikallm.num_retries,
...
async def acompletion(
    ...
    # Optional mishikaLLM function params
    thinking: Optional[AnthropicThinkingParam] = None,
    **kwargs,
) -> Union[ModelResponse, CustomStreamWrapper]:
    """
    Asynchronously executes a mishikallm.completion() call for any of mishikallm supported llms (example gpt-4, gpt-3.5-turbo, claude-2, command-nightly)

    Parameters:
        model (str): The name of the language model to use for text completion. see all supported LLMs: https://docs.21t.cc/docs/providers/
    ...
    #########################################################
    #########################################################
    mishikallm_logging_obj = kwargs.get("mishikallm_logging_obj", None)
    if isinstance(mishikallm_logging_obj, MishikaLLMLoggingObj) and (
        mishikallm_logging_obj.should_run_prompt_management_hooks(
        ...
        (
            ...
            messages,
            _,
        ) = await mishikallm_logging_obj.async_get_chat_completion_prompt(
            model=model,
            messages=messages,
            non_default_params=kwargs,
            prompt_id=kwargs.get("prompt_id", None),
    ...
    fallbacks = fallbacks or mishikallm.model_fallbacks
    if fallbacks is not None:
        ...
        if response is None:
            raise Exception(
                "No response from fallbacks. Got none. Turn on `mishikallm.set_verbose=True` to see more details."
            )
        return response
    ...
    try:
        ...
        if (
            ...
            response = mishikallm.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
                response_object=response,
                model_response_object=mishikallm.ModelResponse(),
            )
...
def _handle_mock_potential_exceptions(
    ...
    if isinstance(mock_response, Exception):
        if isinstance(mock_response, openai.APIError):
            raise mock_response
        raise mishikallm.MockException(
    ...
    elif isinstance(mock_response, str) and mock_response == "mishikallm.RateLimitError":
        ...
        and mock_response == "mishikallm.ContextWindowExceededError"
    ):
        ...
        isinstance(mock_response, str)
        and mock_response == "mishikallm.InternalServerError"
    ):
...
def mock_completion(
    ...
    **kwargs,
    ...
    Returns:
        mishikallm.ModelResponse: A ModelResponse simulating a completion response with the specified model, messages, and mock response.
    ...
    try:
        ...
        if isinstance(mock_response, str) and mock_response.startswith(
            ...
            mock_response = mishikallm.MockException(
        ...
        if n is None:
            ...
            _all_choices = []
            for i in range(n):
                _choice = mishikallm.utils.Choices(
                    index=i,
                    message=mishikallm.utils.Message(
        ...
        try:
            _, custom_llm_provider, _, _ = mishikallm.utils.get_llm_provider(model=model)
...
def completion(  # type: ignore # noqa: PLR0915
    ...
    **kwargs,
    ...
    """
    Perform a completion() using any of mishikallm supported llms (example gpt-4, gpt-3.5-turbo, claude-2, command-nightly)
    Parameters:
        model (str): The name of the language model to use for text completion. see all supported LLMs: https://docs.21t.cc/docs/providers/
        messages (List): A list of message objects representing the conversation context (default is an empty list).
    ...
    mishikallm_params = {}  # used to prevent unbound var errors
    ## PROMPT MANAGEMENT HOOKS ##
    if isinstance(mishikallm_logging_obj, MishikaLLMLoggingObj) and (
        mishikallm_logging_obj.should_run_prompt_management_hooks(
        ...
        (
            ...
            messages,
            optional_params,
        ) = mishikallm_logging_obj.get_chat_completion_prompt(
            model=model,
            messages=messages,
    ...
    try:
        ...
        logging = mishikallm_logging_obj
        fallbacks = fallbacks or mishikallm.model_fallbacks
        if fallbacks is not None:
            return completion_with_fallbacks(**args)
        if model_list is not None:
            deployments = [
                m["mishikallm_params"] for m in model_list if m["model_name"] == model
            ]
            return mishikallm.batch_completion_models(deployments=deployments, **args)
        if mishikallm.model_alias_map and model in mishikallm.model_alias_map:
            model = mishikallm.model_alias_map[
                model
            ]  # update the model to the actual value if an alias has been passed in
        model_response = ModelResponse()
        setattr(model_response, "usage", mishikallm.Usage())
        if (
        ...
        if mishikallm.add_function_to_prompt and optional_params.get(
        ...
        # For logging - save the values of the mishikallm-specific params passed in
        mishikallm_params = get_mishikallm_params(
            ...
            model_alias_map=mishikallm.model_alias_map,
            ...
            mishikallm_session_id=kwargs.get("mishikallm_session_id"),
            hf_model_name=hf_model_name,
            custom_prompt_dict=custom_prompt_dict,
            mishikallm_metadata=kwargs.get("mishikallm_metadata"),
        ...
        if custom_llm_provider == "azure":
            ...
            api_base = api_base or mishikallm.api_base or get_secret("AZURE_API_BASE")
            ...
            azure_ad_token_provider = mishikallm_params.get(
                "azure_ad_token_provider", None
            )

            headers = headers or mishikallm.headers
            ...
            if mishikallm.AzureOpenAIO1Config().is_o_series_model(model=model):
                ## LOAD CONFIG - if set
                config = mishikallm.AzureOpenAIO1Config.get_config()
                ...
                ## LOAD CONFIG - if set
                config = mishikallm.AzureOpenAIConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in optional_params
                    ):  # completion(top_k=3) > azure_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            api_version = (
                api_version or mishikallm.api_version or get_secret("AZURE_API_VERSION")
            )
            ...
            api_key = (
                api_key
                or mishikallm.api_key  # for deepinfra/perplexity/anyscale/friendliai we check in get_llm_provider and pass in the api key from there
            ...
            in mishikallm.openai_text_completion_compatible_providers
            ...
            ## LOAD CONFIG - if set
            config = mishikallm.OpenAITextCompletionConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_text_config(top_k=3) <- allows for dynamic variables to be passed in
                    optional_params[k] = v
            if mishikallm.organization:
                openai.organization = mishikallm.organization
            ...
            if (
                ...
                _response = mishikallm.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
            ...
            ## LOAD CONFIG - if set
            config = mishikallm.GroqChatConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
            ...
            if (
                mishikallm.enable_preview_features and metadata is not None
            ):  # [PREVIEW] allow metadata to be passed to OPENAI
                optional_params["metadata"] = add_openai_metadata(metadata)

            ## LOAD CONFIG - if set
            config = mishikallm.OpenAIConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            or model in mishikallm.replicate_models
        ):
            # Setting the relevant API KEY for replicate, replicate defaults to using os.environ.get("REPLICATE_API_TOKEN")
            replicate_key = (
                api_key
                or mishikallm.replicate_key
                or mishikallm.api_key
            ...
            custom_prompt_dict = custom_prompt_dict or mishikallm.custom_prompt_dict
            ...
            or model in mishikallm.clarifai_models
        ):
            clarifai_key = None
            clarifai_key = (
                api_key
                or mishikallm.clarifai_key
                or mishikallm.api_key
            ...
            api_base = mishikallm.ClarifaiConfig()._convert_model_to_url(model, api_base)
            response = base_llm_http_handler.completion(
                model=model,
                stream=stream,
            ...
            response = anthropic_chat_completions.completion(
                ...
                api_base=api_base,
                acompletion=acompletion,
                custom_prompt_dict=mishikallm.custom_prompt_dict,
                model_response=model_response,
                print_verbose=print_verbose,
                optional_params=optional_params,
                mishikallm_params=mishikallm_params,
            ...
            model_response = aleph_alpha.completion(
                ...
                logger_fn=logger_fn,
                encoding=encoding,
                default_max_tokens_to_sample=mishikallm.max_tokens,
            ...
            huggingface_key = (
                api_key
                or mishikallm.huggingface_key
                or os.environ.get("HF_TOKEN")
                or os.environ.get("HUGGINGFACE_API_KEY")
                or mishikallm.api_key
            )
            hf_headers = headers or mishikallm.headers
            response = base_llm_http_handler.completion(
                model=model,
                messages=messages,
                headers=hf_headers,
            ...
            api_key = (
                api_key
                or mishikallm.api_key  # for databricks we check in get_llm_provider and pass in the api key from there
                or mishikallm.databricks_key
            ...
            api_key = (
                api_key
                or mishikallm.api_key
                or mishikallm.openrouter_key
                or get_secret("OPENROUTER_API_KEY")
                or get_secret("OR_API_KEY")
            )

            openrouter_site_url = get_secret("OR_SITE_URL") or "https://21t.cc"
            openrouter_app_name = get_secret("OR_APP_NAME") or "mishikaLLM"
            ...
            _headers = headers or mishikallm.headers
            if _headers:
            ...
            ## Load Config
            config = mishikallm.OpenrouterConfig.get_config()
            for k, v in config.items():
                if k == "extra_body":
            ...
            or (model in mishikallm.together_ai_models)
        ):
            """
            ...
            api_base = api_base or mishikallm.api_base or get_secret("GEMINI_API_BASE")
            ...
            response = vertex_chat_completion.completion(  # type: ignore
                ...
                mishikallm_params=mishikallm_params,  # type: ignore
                logger_fn=logger_fn,
                encoding=encoding,
            ...
            api_base = api_base or mishikallm.api_base or get_secret("VERTEXAI_API_BASE")
            ...
            if (
                ...
                mishikallm_params.get("base_model") is not None
                and "gemini" in mishikallm_params["base_model"]
            ):
            ...
            tenant_id = (
                ...
                or mishikallm.predibase_tenant_id
            ...
            api_key = api_key or mishikallm.api_key or get_secret("CODESTRAL_API_KEY")

            text_completion_model_response = mishikallm.TextCompletionResponse(
                stream=stream
            )
            ...
            if "aws_bedrock_client" in optional_params:
                verbose_logger.warning(
                    "'aws_bedrock_client' is a deprecated param. Please move to another auth method - https://docs.21t.cc/docs/providers/bedrock#boto3---authentication."
                )
            ...
            api_base = mishikallm.api_base or api_base
            response = base_llm_http_handler.completion(
                model=model,
                stream=stream,
                messages=messages,
            ...
            api_key = (
                api_key
                or mishikallm.cloudflare_api_key
                or mishikallm.api_key
            ...
            or mishikallm.api_base == "https://app.baseten.co"
        ):
            custom_llm_provider = "baseten"
            baseten_key = (
                api_key
                or mishikallm.baseten_key
            ...
            if inspect.isgenerator(model_response) or (
                ...
                return response
            response = model_response
        elif custom_llm_provider == "petals" or model in mishikallm.petals_models:
            api_base = api_base or mishikallm.api_base
            ...
            if stream is True:  ## [BETA]
                ...
                return response
            response = model_response
        elif custom_llm_provider == "snowflake" or model in mishikallm.snowflake_models:
            try:
                client = (
            ...
            url = mishikallm.api_base or api_base or ""
            if url is None or url == "":
                raise ValueError(
                    "api_base not set. Set api_base or mishikallm.api_base for custom endpoints"
                )

            """
            assume input to custom LLM api bases follow this format:
            resp = mishikallm.module_level_client.post(
                api_base,
                json={
            ...
            custom_llm_provider in mishikallm._custom_providers
        ):  # Assume custom LLM provider
            # Get the Custom Handler
            custom_handler: Optional[CustomLLM] = None
            for item in mishikallm.custom_provider_map:
                if item["provider"] == custom_llm_provider:
                    custom_handler = item["custom_handler"]

            if custom_handler is None:
                raise MishikaLLMUnknownProvider(
...
def completion_with_retries(*args, **kwargs):
    """
    Executes a mishikallm.completion() with 3 retries
    """
    try:
        import tenacity
    except Exception as e:
...
def embedding(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    - mishikallm_call_id: The call ID for mishikallm logging.
    - mishikallm_logging_obj: The mishikallm logging object.
    ...
    max_retries = kwargs.get("max_retries", None)
    mishikallm_logging_obj: MishikaLLMLoggingObj = kwargs.get("mishikallm_logging_obj")  # type: ignore
    ...
    default_params = openai_params + mishikallm_params
    non_default_params = {
    ...
    logging: Logging = mishikallm_logging_obj  # type: ignore
    logging.update_environment_variables(
        model=model,
        user=user,
        optional_params=optional_params,
    ...
    try:
        ...
        if azure is True or custom_llm_provider == "azure":
            ...
            model in mishikallm.open_ai_embedding_models
            or custom_llm_provider == "openai"
            or custom_llm_provider == "together_ai"
            or custom_llm_provider == "nvidia_nim"
            or custom_llm_provider == "mishikallm_proxy"
        ):
            api_base = (
                api_base
                or mishikallm.api_base
            ...
            api_base = api_base or mishikallm.api_base or get_secret("DATABRICKS_API_BASE")  # type: ignore
            ...
            api_base = (
                api_base or mishikallm.api_base or get_secret_str("OPENAI_LIKE_API_BASE")
            )
            ...
            gemini_api_key = (
                api_key or get_secret_str("GEMINI_API_KEY") or mishikallm.api_key
            )

            api_base = api_base or mishikallm.api_base or get_secret_str("GEMINI_API_BASE")
            ...
            api_key = api_key or mishikallm.api_key or get_secret_str("MISTRAL_API_KEY")
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
                api_base=api_base,
            ...
            api_key = (
                api_key or mishikallm.api_key or get_secret_str("FIREWORKS_AI_API_KEY")
            )
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
        ...
        ## LOGGING
        mishikallm_logging_obj.post_call(
            input=input,
            api_key=api_key,
            original_response=str(e),
        )
    ...
    *args, **kwargs
    ...
    try:
        ...
        if (
            ...
            text_completion_response = mishikallm.utils.MishikaLLMResponseObjectHandler.convert_chat_to_text_completion(
...
def text_completion(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    # default values for all optional params are none, mishikallm only passes them to the llm when they are set to non None values
    ...
    if _model is not None and (
        ...
        if _model not in mishikallm.open_ai_chat_completion_models:
    ...
    text_completion_response = (
        mishikallm.utils.MishikaLLMResponseObjectHandler.convert_chat_to_text_completion(
            response=response,
            text_completion_response=text_completion_response,
        )
    )
...
async def aadapter_completion(
    *, adapter_id: str, **kwargs
    ...
    try:
        ...
        for item in mishikallm.adapters:
            if item["id"] == adapter_id:
                translation_obj = item["adapter"]

        if translation_obj is None:
            raise ValueError(
                "No matching adapter given. Received 'adapter_id'={}, mishikallm.adapters={}".format(
                    adapter_id, mishikallm.adapters
                )
            )
...
def moderation(
    ...
    return mishikallm.utils.MishikaLLMResponseObjectHandler.convert_to_moderation_response(
...
async def amoderation(
    ...
    **kwargs,
    ...
    optional_params = GenericMishikaLLMParams(**kwargs)
    mishikallm_logging_obj: Optional[MishikaLLMLoggingObj] = kwargs.get(
    ...
    except mishikallm.BadRequestError:
        # `model` is optional field for moderation - get_llm_provider will throw BadRequestError if model is not set / not recognized
        pass

    # update mishikallm_logging_obj with environment variables
    custom_llm_provider = custom_llm_provider or mishikallm.LlmProviders.OPENAI.value
    if mishikallm_logging_obj is not None:
...
def image_generation(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    try:
        ...
        if extra_headers is not None:
            headers.update(extra_headers)
        model_response: ImageResponse = mishikallm.utils.ImageResponse()
        ...
        mishikallm_params = all_mishikallm_params
        default_params = openai_params + mishikallm_params
        non_default_params = {
            k: v for k, v in kwargs.items() if k not in default_params
        }  # model-specific params - pass them straight to the model/provider
        ...
        logging: Logging = mishikallm_logging_obj
        logging.update_environment_variables(
            model=model,
            user=user,
            optional_params=optional_params,
...
def image_variation(
    ...
    **kwargs,
    ...
    client = kwargs.get("client", None)
    # get logging object
    mishikallm_logging_obj = cast(MishikaLLMLoggingObj, kwargs.get("mishikallm_logging_obj"))

    # get the mishikallm params
    mishikallm_params = get_mishikallm_params(**kwargs)
    # get the custom llm provider
    model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
        model=model,
        custom_llm_provider=mishikallm_params.get("custom_llm_provider", None),
        api_base=mishikallm_params.get("api_base", None),
        api_key=mishikallm_params.get("api_key", None),
    )

    # route to the correct provider w/ the params
    try:
        llm_provider = LlmProviders(custom_llm_provider)
        image_variation_provider = MISHIKALLM_IMAGE_VARIATION_PROVIDERS(llm_provider)
    except ValueError:
        raise ValueError(
            f"Invalid image variation provider: {custom_llm_provider}. Supported providers are: {MISHIKALLM_IMAGE_VARIATION_PROVIDERS}"
        )
    ...
    if provider_config is None:
        raise ValueError(
            f"image variation provider has no known model info config - required for getting api keys, etc.: {custom_llm_provider}. Supported providers are: {MISHIKALLM_IMAGE_VARIATION_PROVIDERS}"
        )

    api_key = provider_config.get_api_key(mishikallm_params.get("api_key", None))
    api_base = provider_config.get_api_base(mishikallm_params.get("api_base", None))

    if image_variation_provider == MISHIKALLM_IMAGE_VARIATION_PROVIDERS.OPENAI:
        if api_key is None:
        ...
        response = openai_image_variations.image_variations(
            ...
            model=model,
            image=image,
            timeout=mishikallm_params.get("timeout", None),
            custom_llm_provider=custom_llm_provider,
            logging_obj=mishikallm_logging_obj,
            optional_params={},
            mishikallm_params=mishikallm_params,
        )
    elif image_variation_provider == MISHIKALLM_IMAGE_VARIATION_PROVIDERS.TOPAZ:
        if api_key is None:
...
def transcription(
    ...
    **kwargs,
    ...
    if mishikallm_logging_obj:
        mishikallm_logging_obj.model_call_details["client"] = str(client)

    if max_retries is None:
        max_retries = openai.DEFAULT_MAX_RETRIES

    model_response = mishikallm.utils.TranscriptionResponse()
    ...
    if custom_llm_provider == "azure":
        ...
        api_version = (
            api_version or mishikallm.api_version or get_secret_str("AZURE_API_VERSION")
        )
        ...
        # set API KEY
        api_key = api_key or mishikallm.api_key or mishikallm.openai_key or get_secret("OPENAI_API_KEY")  # type: ignore
...
def speech(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    user = kwargs.get("user", None)
    mishikallm_call_id: Optional[str] = kwargs.get("mishikallm_call_id", None)
    proxy_server_request = kwargs.get("proxy_server_request", None)
    extra_headers = kwargs.get("extra_headers", None)
    model_info = kwargs.get("model_info", None)
    model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
    ...
    if max_retries is None:
        max_retries = mishikallm.num_retries or openai.DEFAULT_MAX_RETRIES
    mishikallm_params_dict = get_mishikallm_params(**kwargs)
    logging_obj = kwargs.get("mishikallm_logging_obj", None)
    ...
    if (
        ...
        api_key = (
            api_key
            or mishikallm.api_key  # for deepinfra/perplexity/anyscale we check in get_llm_provider and pass in the api key from there
            or mishikallm.openai_key
            or get_secret("OPENAI_API_KEY")
        )  # type: ignore
        ...
        api_base = api_base or mishikallm.api_base or get_secret("AZURE_API_BASE")  # type: ignore

        api_version = api_version or mishikallm.api_version or get_secret("AZURE_API_VERSION")  # type: ignore
        ...
        generic_optional_params = GenericMishikaLLMParams(**kwargs)
    ...
    if response is None:
        raise Exception(
            "Unable to map the custom llm provider={} to a known provider={}.".format(
                custom_llm_provider, mishikallm.provider_list
            )
        )
    return response
...
async def ahealth_check_wildcard_models(
    ...
    model_params["mishikallm_logging_obj"] = mishikallm_logging_obj
...
async def ahealth_check(
    ...
    try:
        ...
        if model in mishikallm.model_cost and mode is None:
            mode = mishikallm.model_cost[model].get("mode")
        ...
        mode_handlers = {
            "chat": lambda: mishikallm.acompletion(
                **model_params,
            ),
            "completion": lambda: mishikallm.atext_completion(
                **_filter_model_params(model_params),
                prompt=prompt or "test",
            ),
            "embedding": lambda: mishikallm.aembedding(
                **_filter_model_params(model_params),
                input=input or ["test"],
            ),
            "audio_speech": lambda: mishikallm.aspeech(
                **_filter_model_params(model_params),
                input=prompt or "test",
                voice="alloy",
            ),
            "audio_transcription": lambda: mishikallm.atranscription(
                **_filter_model_params(model_params),
            ...
            "rerank": lambda: mishikallm.arerank(
                **_filter_model_params(model_params),
                query=prompt or "",
        ...
        if mode in mode_handlers:
            ...
            raise Exception(
                f"Mode {mode} not supported. See modes here: https://docs.21t.cc/docs/proxy/health"
            )
    except Exception as e:
        ...
        if mode is None:
            return {
                "error": f"error:{str(e)}. Missing `mode`. Set the `mode` for the model - https://docs.21t.cc/docs/proxy/health#embedding-models  \nstacktrace: {stack_trace}"
            }
...
####### HELPER FUNCTIONS ################
## Set verbose to true -> ```mishikallm.set_verbose = True```
...
def config_completion(**kwargs):
    if mishikallm.config_path is not None:
        config_args = read_config_args(mishikallm.config_path)
        # overwrite any args passed in with config args
        return completion(**kwargs, **config_args)
    else:
        raise ValueError(
            "No config path set, please set a config path using `mishikallm.config_path = 'path/to/config.json'`"
        )
...
def stream_chunk_builder(  # noqa: PLR0915
    ...
    try:
        ...
        if isinstance(
            chunks[0]["choices"][0], mishikallm.utils.TextChoices
        ):  # route to the text completion logic
        ...
        verbose_logger.exception(
            "mishikallm.main.py::stream_chunk_builder() - Exception occurred - {}".format(
                str(e)
            )
        )
        raise mishikallm.APIError(
