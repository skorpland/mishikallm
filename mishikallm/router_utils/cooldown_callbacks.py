...
async def router_cooldown_event_callback(
    ...
    """
    Callback triggered when a deployment is put into cooldown by mishikallm
    ...
    _deployment = mishikallm_router_instance.get_deployment(model_id=deployment_id)
    if _deployment is None:
        verbose_logger.warning(
            f"in router_cooldown_event_callback but _deployment is None for deployment_id={deployment_id}. Doing nothing"
        )
        return
    _mishikallm_params = _deployment["mishikallm_params"]
    temp_mishikallm_params = copy.deepcopy(_mishikallm_params)
    temp_mishikallm_params = dict(temp_mishikallm_params)
    _model_name = _deployment.get("model_name", None) or ""
    _api_base = (
        mishikallm.get_api_base(model=_model_name, optional_params=temp_mishikallm_params)
        or ""
    )
    model_info = _deployment["model_info"]
    model_id = model_info.id

    mishikallm_model_name = temp_mishikallm_params.get("model") or ""
    llm_provider = ""
    try:
        _, llm_provider, _, _ = mishikallm.get_llm_provider(
            model=mishikallm_model_name,
            custom_llm_provider=temp_mishikallm_params.get("custom_llm_provider"),
        )
    except Exception:
        pass
    ...
    if prometheusLogger is not None:
        prometheusLogger.set_deployment_complete_outage(
            mishikallm_model_name=_model_name,
            model_id=model_id,
            api_base=_api_base,
...
def _get_prometheus_logger_from_callbacks() -> Optional[PrometheusLogger]:
    ...
    for _callback in mishikallm._async_success_callback:
        if isinstance(_callback, PrometheusLogger):
            return _callback
    for global_callback in mishikallm.callbacks:
