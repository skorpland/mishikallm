...
from mishikallm.types.router import MishikaLLMParamsTypedDict
...
async def run_async_fallback(
    *args: Tuple[Any],
    mishikallm_router: MishikallmRouter,
    ...
    **kwargs,
    ...
    Args:
        mishikallm_router: The mishikallm router instance.
    ...
    for mg in fallback_model_group:
        ...
        try:
            # LOGGING
            kwargs = mishikallm_router.log_retry(kwargs=kwargs, e=original_exception)
            ...
            response = await mishikallm_router.async_function_with_fallbacks(
                *args, **kwargs
            )
...
async def log_success_fallback_event(
    ...
    for _callback in mishikallm.callbacks:
        if isinstance(_callback, CustomLogger) or (
            _callback in mishikallm._known_custom_logger_compatible_callbacks
        ):
            try:
                _callback_custom_logger: Optional[CustomLogger] = None
                if _callback in mishikallm._known_custom_logger_compatible_callbacks:
...
def _check_non_standard_fallback_format(fallbacks: Optional[List[Any]]) -> bool:
    ...
    - List[Dict[<MishikaLLMParamsTypedDict>, Any]]: e.g. [{"model": "claude-3-haiku", "messages": [{"role": "user", "content": "Hey, how's it going?"}]}]
    ...
    if all(isinstance(item, str) for item in fallbacks):
        ...
        for key in MishikaLLMParamsTypedDict.__annotations__.keys():
