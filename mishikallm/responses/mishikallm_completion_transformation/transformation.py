"""
Handles transforming from Responses API -> MishikaLLM completion  (Chat Completion API)
"""
...
class ChatCompletionSession(TypedDict, total=False):
    ...
    mishikallm_session_id: Optional[str]


########### End of Initialize Classes used for Responses API  ###########


class MishikaLLMCompletionResponsesConfig:
    @staticmethod
    def get_supported_openai_params(model: str) -> list:
        """
        MishikaLLM Adapter from OpenAI Responses API to Chat Completion API supports a subset of OpenAI Responses API params
        """
        return [
            "input",
            "model",
    ...
    def transform_responses_api_request_to_chat_completion_request(
        ...
        **kwargs,
        ...
        mishikallm_completion_request: dict = {
            "messages": MishikaLLMCompletionResponsesConfig.transform_responses_api_input_to_messages(
                input=input,
                responses_api_request=responses_api_request,
            ),
            "model": model,
            "tool_choice": responses_api_request.get("tool_choice"),
            "tools": MishikaLLMCompletionResponsesConfig.transform_responses_api_tools_to_chat_completion_tools(
        ...
        # Responses API `Completed` events require usage, we pass `stream_options` to mishikallm.completion to include usage
        if stream is True:
            stream_options = {
                "include_usage": True,
            }
            mishikallm_completion_request["stream_options"] = stream_options
            mishikallm_logging_obj: Optional[MishikaLLMLoggingObj] = kwargs.get(
                "mishikallm_logging_obj"
            )
            if mishikallm_logging_obj:
                mishikallm_logging_obj.stream_options = stream_options

        # only pass non-None values
        mishikallm_completion_request = {
            k: v for k, v in mishikallm_completion_request.items() if v is not None
        }

        return mishikallm_completion_request
    ...
    def transform_responses_api_input_to_messages(
        ...
        if responses_api_request.get("instructions"):
            messages.append(
                MishikaLLMCompletionResponsesConfig.transform_instructions_to_system_message(
                    responses_api_request.get("instructions")
                )
            )

        messages.extend(
            MishikaLLMCompletionResponsesConfig._transform_response_input_param_to_chat_completion_message(
                input=input,
            )
        )
    ...
    async def async_responses_api_session_handler(
        ...
        if (
            ...
            chat_completion_session = ChatCompletionSession(
                messages=[], mishikallm_session_id=None
            )
            ...
            _messages = mishikallm_completion_request.get("messages") or []
            session_messages = chat_completion_session.get("messages") or []
            mishikallm_completion_request["messages"] = session_messages + _messages
            mishikallm_completion_request["mishikallm_trace_id"] = (
                chat_completion_session.get("mishikallm_session_id")
            )
        return mishikallm_completion_request
    ...
    def _transform_response_input_param_to_chat_completion_message(
        ...
        if isinstance(input, str):
            ...
            for _input in input:
                chat_completion_messages = MishikaLLMCompletionResponsesConfig._transform_responses_api_input_item_to_chat_completion_message(
                    input_item=_input
                )
                if MishikaLLMCompletionResponsesConfig._is_input_item_tool_call_output(
                    input_item=_input
                ):
    ...
    def _transform_responses_api_input_item_to_chat_completion_message(
        ...
        - ItemReference
        """
        if MishikaLLMCompletionResponsesConfig._is_input_item_tool_call_output(input_item):
            # handle executed tool call results
            return MishikaLLMCompletionResponsesConfig._transform_responses_api_tool_call_output_to_chat_completion_message(
            ...
            return [
                GenericChatCompletionMessage(
                    ...
                    content=MishikaLLMCompletionResponsesConfig._transform_responses_api_content_to_chat_completion_content(
        ...
        if isinstance(content, str):
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    content_list.append(
                        {
                            "type": MishikaLLMCompletionResponsesConfig._get_chat_completion_request_content_type(
    ...
    def transform_chat_completion_response_to_responses_api_response(
        ...
        responses_api_response: ResponsesAPIResponse = ResponsesAPIResponse(
            ...
            output=MishikaLLMCompletionResponsesConfig._transform_chat_completion_choices_to_responses_output(
            ...
            usage=MishikaLLMCompletionResponsesConfig._transform_chat_completion_usage_to_responses_usage(
                chat_completion_response=chat_completion_response
            ),
    ...
    def _transform_chat_completion_choices_to_responses_output(
        ...
        for choice in choices:
            responses_output.append(
                GenericResponseOutputItem(
                    ...
                    content=[
                        MishikaLLMCompletionResponsesConfig._transform_chat_message_to_response_output_text(
                            choice.message
                        )
                    ],
                )
        ...
        tool_calls = MishikaLLMCompletionResponsesConfig.transform_chat_completion_tools_to_responses_tools(
            chat_completion_response=chat_completion_response
        )
    ...
    def _transform_responses_api_outputs_to_chat_completion_messages(
        ...
        for _output_item in output_items:
            ...
            if output_item.get("type") == "function_call":
                ...
                messages.append(
                    MishikaLLMCompletionResponsesConfig._transform_responses_output_tool_call_to_chat_completion_output_tool_call(
    ...
    def _transform_chat_message_to_response_output_text(
        ...
        return OutputText(
            ...
            annotations=MishikaLLMCompletionResponsesConfig._transform_chat_completion_annotations_to_response_output_annotations(
