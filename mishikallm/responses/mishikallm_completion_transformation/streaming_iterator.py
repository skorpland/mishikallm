...
from mishikallm.responses.streaming_iterator import ResponsesAPIStreamingIterator
...
from mishikallm.types.utils import Delta as ChatCompletionDelta
...
class MishikaLLMCompletionStreamingIterator(ResponsesAPIStreamingIterator):
    """
    ...
    def __init__(
        ...
        self.mishikallm_custom_stream_wrapper: mishikallm.CustomStreamWrapper = (
            mishikallm_custom_stream_wrapper
        )
    ...
    async def __anext__(
        ...
        try:
            while True:
                ...
                try:
                    chunk = await self.mishikallm_custom_stream_wrapper.__anext__()
    ...
    def __next__(
        ...
        try:
            while True:
                ...
                # Get the next chunk from the stream
                try:
                    chunk = self.mishikallm_custom_stream_wrapper.__next__()
                    self.collected_chat_completion_chunks.append(chunk)
                    response_api_chunk = (
                        self._transform_chat_completion_chunk_to_response_api_chunk(
                            chunk
    ...
    def _get_delta_string_from_streaming_choices(
        ...
        It's unclear how users expect mishikallm to translate multiple-choices-per-chunk to the responses API output.
        """
        choice = choices[0]
    ...
    def _emit_response_completed_event(self) -> Optional[ResponseCompletedEvent]:
        mishikallm_model_response: Optional[
            Union[ModelResponse, TextCompletionResponse]
        ] = stream_chunk_builder(chunks=self.collected_chat_completion_chunks)
        if mishikallm_model_response and isinstance(mishikallm_model_response, ModelResponse):

            return ResponseCompletedEvent(
                type=ResponsesAPIStreamEvents.RESPONSE_COMPLETED,
                response=MishikaLLMCompletionResponsesConfig.transform_chat_completion_response_to_responses_api_response(
                    request_input=self.request_input,
                    chat_completion_response=mishikallm_model_response,
