# used for /metrics endpoint on MishikaLLM Proxy
#### What this does ####
...
from mishikallm.proxy._types import MishikaLLM_TeamTable, UserAPIKeyAuth
from mishikallm.types.integrations.prometheus import *
from mishikallm.types.utils import StandardLoggingPayload
from mishikallm.utils import get_end_user_id_for_cost_tracking
...
class PrometheusLogger(CustomLogger):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            from mishikallm.proxy.proxy_server import CommonProxyErrors, premium_user

            if premium_user is not True:
                verbose_logger.warning(
                    f"🚨🚨🚨 Prometheus Metrics is on MishikaLLM Enterprise\n🚨 {CommonProxyErrors.not_premium_user.value}"
                )
                self.mishikallm_not_a_premium_user_metric = Counter(
                    name="mishikallm_not_a_premium_user_metric",
                    documentation=f"🚨🚨🚨 Prometheus Metrics is on MishikaLLM Enterprise. 🚨 {CommonProxyErrors.not_premium_user.value}",
                )
                return

            self.mishikallm_proxy_failed_requests_metric = Counter(
                name="mishikallm_proxy_failed_requests_metric",
                documentation="Total number of failed responses from proxy - the client did not get a success response from mishikallm proxy",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_proxy_failed_requests_metric"
                ),
            )

            self.mishikallm_proxy_total_requests_metric = Counter(
                name="mishikallm_proxy_total_requests_metric",
                documentation="Total number of requests made to the proxy server - track number of client side requests",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_proxy_total_requests_metric"
                ),
            )

            # request latency metrics
            self.mishikallm_request_total_latency_metric = Histogram(
                "mishikallm_request_total_latency_metric",
                "Total latency (seconds) for a request to MishikaLLM",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_request_total_latency_metric"
                ),
                buckets=LATENCY_BUCKETS,
            )

            self.mishikallm_llm_api_latency_metric = Histogram(
                "mishikallm_llm_api_latency_metric",
                "Total latency (seconds) for a models LLM API call",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_llm_api_latency_metric"
                ),
                buckets=LATENCY_BUCKETS,
            )

            self.mishikallm_llm_api_time_to_first_token_metric = Histogram(
                "mishikallm_llm_api_time_to_first_token_metric",
            ...
            # Counter for spend
            self.mishikallm_spend_metric = Counter(
                "mishikallm_spend_metric",
            ...
            self.mishikallm_tokens_metric = Counter(
                "mishikallm_total_tokens",
            ...
            self.mishikallm_input_tokens_metric = Counter(
                "mishikallm_input_tokens",
                "Total number of input tokens from LLM requests",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_input_tokens_metric"
                ),
            )

            self.mishikallm_output_tokens_metric = Counter(
                "mishikallm_output_tokens",
                "Total number of output tokens from LLM requests",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_output_tokens_metric"
                ),
            )

            # Remaining Budget for Team
            self.mishikallm_remaining_team_budget_metric = Gauge(
                "mishikallm_remaining_team_budget_metric",
                "Remaining budget for team",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_remaining_team_budget_metric"
                ),
            )

            # Max Budget for Team
            self.mishikallm_team_max_budget_metric = Gauge(
                "mishikallm_team_max_budget_metric",
                "Maximum budget set for team",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_team_max_budget_metric"
                ),
            )

            # Team Budget Reset At
            self.mishikallm_team_budget_remaining_hours_metric = Gauge(
                "mishikallm_team_budget_remaining_hours_metric",
                "Remaining days for team budget to be reset",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_team_budget_remaining_hours_metric"
                ),
            )

            # Remaining Budget for API Key
            self.mishikallm_remaining_api_key_budget_metric = Gauge(
                "mishikallm_remaining_api_key_budget_metric",
                "Remaining budget for api key",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_remaining_api_key_budget_metric"
                ),
            )

            # Max Budget for API Key
            self.mishikallm_api_key_max_budget_metric = Gauge(
                "mishikallm_api_key_max_budget_metric",
                "Maximum budget set for api key",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_api_key_max_budget_metric"
                ),
            )

            self.mishikallm_api_key_budget_remaining_hours_metric = Gauge(
                "mishikallm_api_key_budget_remaining_hours_metric",
                "Remaining hours for api key budget to be reset",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_api_key_budget_remaining_hours_metric"
                ),
            )

            ########################################
            # MishikaLLM Virtual API KEY metrics
            ########################################
            # Remaining MODEL RPM limit for API Key
            self.mishikallm_remaining_api_key_requests_for_model = Gauge(
                "mishikallm_remaining_api_key_requests_for_model",
                "Remaining Requests API Key can make for model (model based rpm limit on key)",
                labelnames=["hashed_api_key", "api_key_alias", "model"],
            )

            # Remaining MODEL TPM limit for API Key
            self.mishikallm_remaining_api_key_tokens_for_model = Gauge(
                "mishikallm_remaining_api_key_tokens_for_model",
            ...
            self.mishikallm_remaining_requests_metric = Gauge(
                "mishikallm_remaining_requests",
                "LLM Deployment Analytics - remaining requests for model, returned from LLM API Provider",
                labelnames=[
                    "model_group",
                    "api_provider",
                    "api_base",
                    "mishikallm_model_name",
                    "hashed_api_key",
                    "api_key_alias",
                ],
            )

            self.mishikallm_remaining_tokens_metric = Gauge(
                "mishikallm_remaining_tokens",
            ...
            self.mishikallm_overhead_latency_metric = Histogram(
                "mishikallm_overhead_latency_metric",
                "Latency overhead (milliseconds) added by MishikaLLM processing",
                labelnames=[
                    "model_group",
                    "api_provider",
                    "api_base",
            ...
            self.mishikallm_provider_remaining_budget_metric = Gauge(
                "mishikallm_provider_remaining_budget_metric",
            ...
            # Get all keys
            _logged_llm_labels = [
                UserAPIKeyLabelNames.v2_MISHIKALLM_MODEL_NAME.value,
            ...
            self.mishikallm_deployment_state = Gauge(
                "mishikallm_deployment_state",
                "LLM Deployment Analytics - The state of the deployment: 0 = healthy, 1 = partial outage, 2 = complete outage",
                labelnames=_logged_llm_labels,
            )

            self.mishikallm_deployment_cooled_down = Counter(
                "mishikallm_deployment_cooled_down",
                "LLM Deployment Analytics - Number of times a deployment has been cooled down by MishikaLLM load balancing logic. exception_status is the status of the exception that caused the deployment to be cooled down",
                labelnames=_logged_llm_labels + [EXCEPTION_STATUS],
            )

            self.mishikallm_deployment_success_responses = Counter(
                name="mishikallm_deployment_success_responses",
                documentation="LLM Deployment Analytics - Total number of successful LLM API calls via mishikallm",
                labelnames=[REQUESTED_MODEL] + _logged_llm_labels + team_and_key_labels,
            )
            self.mishikallm_deployment_failure_responses = Counter(
                name="mishikallm_deployment_failure_responses",
            ...
            self.mishikallm_deployment_failure_by_tag_responses = Counter(
                "mishikallm_deployment_failure_by_tag_responses",
            ...
            self.mishikallm_deployment_total_requests = Counter(
                name="mishikallm_deployment_total_requests",
                documentation="LLM Deployment Analytics - Total number of LLM API calls via mishikallm - success + failure",
                labelnames=[REQUESTED_MODEL] + _logged_llm_labels + team_and_key_labels,
            )
            ...
            self.mishikallm_deployment_latency_per_output_token = Histogram(
                name="mishikallm_deployment_latency_per_output_token",
                documentation="LLM Deployment Analytics - Latency per output token",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_deployment_latency_per_output_token"
                ),
            )

            self.mishikallm_deployment_successful_fallbacks = Counter(
                "mishikallm_deployment_successful_fallbacks",
            ...
            self.mishikallm_deployment_failed_fallbacks = Counter(
                "mishikallm_deployment_failed_fallbacks",
            ...
            self.mishikallm_llm_api_failed_requests_metric = Counter(
                name="mishikallm_llm_api_failed_requests_metric",
                documentation="deprecated - use mishikallm_proxy_failed_requests_metric",
                labelnames=[
                    "end_user",
                    "hashed_api_key",
                    "api_key_alias",
            ...
            self.mishikallm_requests_metric = Counter(
                name="mishikallm_requests_metric",
                documentation="deprecated - use mishikallm_proxy_total_requests_metric. Total number of LLM calls to mishikallm - track total per API Key, team, user",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="mishikallm_requests_metric"
                ),
            )
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _metadata = mishikallm_params.get("metadata", {})
        end_user_id = get_end_user_id_for_cost_tracking(
            mishikallm_params, service_type="prometheus"
        )
        ...
        if (
            ...
            from mishikallm.proxy.utils import hash_token
        ...
        if (
            ...
            self.mishikallm_proxy_total_requests_metric.labels(**_labels).inc()
    ...
    def _increment_token_metrics(
        ...
        # token metrics
        self.mishikallm_tokens_metric.labels(
            end_user_id,
            user_api_key,
            user_api_key_alias,
            model,
        ...
        self.mishikallm_input_tokens_metric.labels(**_labels).inc(
        ...
        self.mishikallm_output_tokens_metric.labels(**_labels).inc(
    ...
    async def _increment_remaining_budget_metrics(
        ...
        _team_spend = mishikallm_params.get("metadata", {}).get(
            "user_api_key_team_spend", None
        )
        _team_max_budget = mishikallm_params.get("metadata", {}).get(
            "user_api_key_team_max_budget", None
        )

        _api_key_spend = mishikallm_params.get("metadata", {}).get(
            "user_api_key_spend", None
        )
        _api_key_max_budget = mishikallm_params.get("metadata", {}).get(
    ...
    def _increment_top_level_request_and_spend_metrics(
        ...
        self.mishikallm_requests_metric.labels(**_labels).inc()

        self.mishikallm_spend_metric.labels(
            end_user_id,
            user_api_key,
            user_api_key_alias,
            model,
    ...
    def _set_virtual_key_rate_limit_metrics(
        ...
        from mishikallm.proxy.common_utils.callback_utils import (
            get_model_group_from_mishikallm_kwargs,
        )

        # Set remaining rpm/tpm for API Key + model
        # see parallel_request_limiter.py - variables are set there
        model_group = get_model_group_from_mishikallm_kwargs(kwargs)
        remaining_requests_variable_name = (
            f"mishikallm-key-remaining-requests-{model_group}"
        )
        ...
        self.mishikallm_remaining_api_key_requests_for_model.labels(
            user_api_key, user_api_key_alias, model_group
        ).set(remaining_requests)

        self.mishikallm_remaining_api_key_tokens_for_model.labels(
            user_api_key, user_api_key_alias, model_group
    ...
    def _set_latency_metrics(
        ...
        if (
            ...
            self.mishikallm_llm_api_time_to_first_token_metric.labels(
                model,
                user_api_key,
                user_api_key_alias,
                user_api_team,
        ...
        if api_call_total_time_seconds is not None:
            ...
            self.mishikallm_llm_api_latency_metric.labels(**_labels).observe(
        ...
        if total_time_seconds is not None:
            ...
            self.mishikallm_request_total_latency_metric.labels(**_labels).observe(
                total_time_seconds
            )
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            self.mishikallm_llm_api_failed_requests_metric.labels(
                end_user_id,
                user_api_key,
                user_api_key_alias,
                model,
    ...
    async def async_post_call_failure_hook(
        ...
        try:
            ...
            self.mishikallm_proxy_failed_requests_metric.labels(**_labels).inc()
    ...
    def set_llm_deployment_failure_metrics(self, request_kwargs: dict):
        ...
        try:
            ...
            _mishikallm_params = request_kwargs.get("mishikallm_params", {}) or {}
            mishikallm_model_name = request_kwargs.get("model", None)
            ...
            llm_provider = _mishikallm_params.get("custom_llm_provider", None)

            """
            log these labels
            ["mishikallm_model_name", "model_id", "api_base", "api_provider"]
            """
            self.set_deployment_partial_outage(
                mishikallm_model_name=mishikallm_model_name,
                model_id=model_id,
                api_base=api_base,
                api_provider=llm_provider,
            )
            self.mishikallm_deployment_failure_responses.labels(
                mishikallm_model_name=mishikallm_model_name,
                model_id=model_id,
                api_base=api_base,
                api_provider=llm_provider,
            ...
            if standard_logging_payload is not None and isinstance(
                ...
                _tags = standard_logging_payload["request_tags"]
                for tag in _tags:
                    self.mishikallm_deployment_failure_by_tag_responses.labels(
                        **{
                            UserAPIKeyLabelNames.REQUESTED_MODEL.value: model_group,
                            UserAPIKeyLabelNames.TAG.value: tag,
                            UserAPIKeyLabelNames.v2_MISHIKALLM_MODEL_NAME.value: mishikallm_model_name,
            ...
            self.mishikallm_deployment_total_requests.labels(
                mishikallm_model_name=mishikallm_model_name,
                model_id=model_id,
                api_base=api_base,
                api_provider=llm_provider,
    ...
    def set_llm_deployment_success_metrics(
        ...
        try:
            ...
            _mishikallm_params = request_kwargs.get("mishikallm_params", {}) or {}
            _metadata = _mishikallm_params.get("metadata", {})
            mishikallm_model_name = request_kwargs.get("model", None)
            llm_provider = _mishikallm_params.get("custom_llm_provider", None)
            ...
            if mishikallm_overhead_time_ms := standard_logging_payload[
                "hidden_params"
            ].get("mishikallm_overhead_time_ms"):
                self.mishikallm_overhead_latency_metric.labels(
                    model_group,
                    llm_provider,
                    api_base,
                    mishikallm_model_name,
                    ...
                    mishikallm_overhead_time_ms / 1000
                )  # set as seconds
            ...
            if remaining_requests:
                ...
                "mishikallm_model_name"
                """
                self.mishikallm_remaining_requests_metric.labels(
                    model_group,
                    llm_provider,
                    api_base,
                    mishikallm_model_name,
            ...
            if remaining_tokens:
                self.mishikallm_remaining_tokens_metric.labels(
                    model_group,
                    llm_provider,
                    api_base,
                    mishikallm_model_name,
            ...
            """
            log these labels
            ["mishikallm_model_name", "requested_model", model_id", "api_base", "api_provider"]
            """
            ...
            self.mishikallm_deployment_success_responses.labels(
                mishikallm_model_name=mishikallm_model_name,
                model_id=model_id,
                api_base=api_base,
                api_provider=llm_provider,
            ...
            if output_tokens is not None and output_tokens > 0:
                ...
                self.mishikallm_deployment_latency_per_output_token.labels(
                    **_labels
    ...
    async def log_success_fallback_event(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=PrometheusMetricLabels.get_labels(
                label_name="mishikallm_deployment_successful_fallbacks"
            ),
            enum_values=enum_values,
        )
        self.mishikallm_deployment_successful_fallbacks.labels(**_labels).inc()
    ...
    async def log_failure_fallback_event(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=PrometheusMetricLabels.get_labels(
                label_name="mishikallm_deployment_failed_fallbacks"
            ),
            enum_values=enum_values,
        )
        self.mishikallm_deployment_failed_fallbacks.labels(**_labels).inc()

    def set_mishikallm_deployment_state(
        self,
        state: int,
        mishikallm_model_name: str,
        model_id: Optional[str],
        api_base: Optional[str],
        api_provider: str,
    ):
        self.mishikallm_deployment_state.labels(
            mishikallm_model_name, model_id, api_base, api_provider
        ).set(state)
    ...
    def set_deployment_healthy(
        ...
        self.set_mishikallm_deployment_state(
            0, mishikallm_model_name, model_id, api_base, api_provider
        )
    ...
    def set_deployment_partial_outage(
        ...
        self.set_mishikallm_deployment_state(
            1, mishikallm_model_name, model_id, api_base, api_provider
        )
    ...
    def set_deployment_complete_outage(
        ...
        self.set_mishikallm_deployment_state(
            2, mishikallm_model_name, model_id, api_base, api_provider
        )
    ...
    def increment_deployment_cooled_down(
        ...
        """
        increment metric when mishikallm.Router / load balancing logic places a deployment in cool down
        """
        self.mishikallm_deployment_cooled_down.labels(
            mishikallm_model_name, model_id, api_base, api_provider, exception_status
        ).inc()
    ...
    def track_provider_remaining_budget(
        ...
        """
        self.mishikallm_provider_remaining_budget_metric.labels(provider).set(
    ...
    async def _initialize_team_budget_metrics(self):
        ...
        ) -> Tuple[List[MishikaLLM_TeamTable], Optional[int]]:
    ...
    async def _initialize_api_key_budget_metrics(self):
        ...
        from mishikallm.constants import UI_SESSION_TOKEN_TEAM_ID
    ...
    async def initialize_remaining_budget_metrics(self):
        ...
        """
        from mishikallm.constants import PROMETHEUS_EMIT_BUDGET_METRICS_JOB_NAME
        from mishikallm.proxy.proxy_server import proxy_logging_obj
    ...
    async def _set_team_list_budget_metrics(self, teams: List[MishikaLLM_TeamTable]):
    ...
    async def _set_team_budget_metrics_after_api_request(
        ...
        - Assemble a MishikaLLM_TeamTable object
    ...
    async def _assemble_team_object(
        ...
        - `budget_reset_at`
        """
        from mishikallm.proxy.auth.auth_checks import get_team_object
        from mishikallm.proxy.proxy_server import prisma_client, user_api_key_cache

        _total_team_spend = (spend or 0) + response_cost
        team_object = MishikaLLM_TeamTable(
            team_id=team_id,
    ...
    def _set_team_budget_metrics(
        ...
        self.mishikallm_remaining_team_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if team.max_budget is not None:
            ...
            self.mishikallm_team_max_budget_metric.labels(**_labels).set(team.max_budget)
        ...
        if team.budget_reset_at is not None:
            ...
            self.mishikallm_team_budget_remaining_hours_metric.labels(**_labels).set(
    ...
    def _set_key_budget_metrics(self, user_api_key_dict: UserAPIKeyAuth):
        ...
        self.mishikallm_remaining_api_key_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if user_api_key_dict.max_budget is not None:
            ...
            self.mishikallm_api_key_max_budget_metric.labels(**_labels).set(
                user_api_key_dict.max_budget
            )

        if user_api_key_dict.budget_reset_at is not None:
            self.mishikallm_api_key_budget_remaining_hours_metric.labels(**_labels).set(
                self._get_remaining_hours_for_budget_reset(
    ...
    async def _assemble_key_object(
        ...
        """
        from mishikallm.proxy.auth.auth_checks import get_key_object
        from mishikallm.proxy.proxy_server import prisma_client, user_api_key_cache
    ...
    def initialize_budget_metrics_cron_job(scheduler: AsyncIOScheduler):
        ...
        """
        from mishikallm.constants import PROMETHEUS_BUDGET_METRICS_REFRESH_INTERVAL_MINUTES
        from mishikallm.integrations.custom_logger import CustomLogger
        from mishikallm.integrations.prometheus import PrometheusLogger

        prometheus_loggers: List[
            CustomLogger
        ] = mishikallm.logging_callback_manager.get_custom_loggers_for_type(
    ...
    def _mount_metrics_endpoint(premium_user: bool):
        ...
        from mishikallm.proxy._types import CommonProxyErrors
...
def prometheus_label_factory(
    ...
    if UserAPIKeyLabelNames.END_USER.value in filtered_labels:
        filtered_labels["end_user"] = get_end_user_id_for_cost_tracking(
            mishikallm_params={"user_api_key_end_user_id": enum_values.end_user},
...
def get_custom_labels_from_metadata(metadata: dict) -> Dict[str, str]:
    ...
    """
    keys = mishikallm.custom_prometheus_metadata_labels
