...
async def get_fallback_metric_from_prometheus():
    ...
    relevant_metrics = [
        ...
        "mishikallm_deployment_failed_fallbacks_total",
    ]
...
async def get_daily_spend_from_prometheus(api_key: Optional[str]):
    ...
    if api_key is None:
        query = "sum(delta(mishikallm_spend_metric_total[1d]))"
    else:
        query = (
            f'sum(delta(mishikallm_spend_metric_total{{hashed_api_key="{api_key}"}}[1d]))'
        )
