...
class OpikLogger(CustomBatchLogger):
    ...
    def _create_opik_payload(  # noqa: PLR0915
        ...
        # Get metadata
        _mishikallm_params = kwargs.get("mishikallm_params", {}) or {}
        mishikallm_params_metadata = _mishikallm_params.get("metadata", {}) or {}

        # Extract opik metadata
        mishikallm_opik_metadata = mishikallm_params_metadata.get("opik", {})
        verbose_logger.debug(
            f"mishikallm_opik_metadata - {json.dumps(mishikallm_opik_metadata, default=str)}"
        )
        project_name = mishikallm_opik_metadata.get("project_name", self.opik_project_name)

        # Extract trace_id and parent_span_id
        current_span_data = mishikallm_opik_metadata.get("current_span_data", None)
        if isinstance(current_span_data, dict):
            ...
            parent_span_id = None
        # Create Opik tags
        opik_tags = mishikallm_opik_metadata.get("tags", [])
        ...
        metadata = mishikallm_opik_metadata
        if "current_span_data" in metadata:
            del metadata["current_span_data"]
        metadata["created_from"] = "mishikallm"
