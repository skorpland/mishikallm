...
class MlflowLogger(CustomLogger):
    ...
    def _handle_stream_event(self, kwargs, response_obj, start_time, end_time):
        ...
        mishikallm_call_id = kwargs.get("mishikallm_call_id")

        if mishikallm_call_id not in self._stream_id_to_span:
            with self._lock:
                # Check again after acquiring lock
                if mishikallm_call_id not in self._stream_id_to_span:
                    # Start a new span for the first chunk of the stream
                    span = self._start_span_or_trace(kwargs, start_time)
                    self._stream_id_to_span[mishikallm_call_id] = span

        # Add chunk as event to the span
        span = self._stream_id_to_span[mishikallm_call_id]
        ...
        if final_response := kwargs.get("complete_streaming_response"):
            ...
            with self._lock:
                self._stream_id_to_span.pop(mishikallm_call_id)
    ...
    def _extract_attributes(self, kwargs):
        ...
        With the latest version of mishikallm, the standard_logging_object contains
    ...
    def _start_span_or_trace(self, kwargs, start_time):
        ...
        span_name = f"mishikallm-{call_type}"
