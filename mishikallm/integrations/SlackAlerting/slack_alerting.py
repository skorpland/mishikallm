...
from mishikallm.constants import HOURS_IN_A_DAY
from mishikallm.integrations.custom_batch_logger import CustomBatchLogger
...
from mishikallm.types.integrations.slack_alerting import *
...
class SlackAlerting(CustomBatchLogger):
    ...
    def _response_taking_too_long_callback_helper(
        ...
        try:
            ...
            api_base = mishikallm.get_api_base(model=model, optional_params=mishikallm_params)
    ...
    async def response_taking_too_long_callback(
        ...
        if mishikallm.turn_off_message_logging or mishikallm.redact_messages_in_exceptions:
            messages = "Message not logged. mishikallm.redact_messages_in_exceptions=True"
        request_info = f"\nRequest Model: `{model}`\nAPI Base: `{api_base}`\nMessages: `{messages}`"
        ...
        if time_difference_float > self.alerting_threshold:
            ...
            if (
                kwargs is not None
                and "mishikallm_params" in kwargs
                and "metadata" in kwargs["mishikallm_params"]
            ):
                _metadata: dict = kwargs["mishikallm_params"]["metadata"]
    ...
    async def send_daily_reports(self, router) -> bool:  # noqa: PLR0915
        ...
        # format alert -> return the mishikallm model name + api base
        ...
        for i in range(len(top_5_failed)):
            ...
            if isinstance(_deployment, dict):
                deployment_name = _deployment["mishikallm_params"].get("model", "")
            else:
                return False

            api_base = mishikallm.get_api_base(
                model=deployment_name,
                optional_params=(
                    _deployment["mishikallm_params"] if _deployment is not None else {}
                ),
            )
            if api_base is None:
                api_base = ""
    ...
    async def response_taking_too_long(
        ...
        if request_data is not None:
            ...
            if (
                mishikallm.turn_off_message_logging
                or mishikallm.redact_messages_in_exceptions
            ):
                messages = (
                    "Message not logged. mishikallm.redact_messages_in_exceptions=True"
                )
            request_info = f"\nRequest Model: `{model}`\nMessages: `{messages}`"
        else:
            request_info = ""
        ...
        if type == "hanging_request":
            ...
            if request_data is not None:
                if request_data.get("deployment", None) is not None and isinstance(
                    ...
                    _api_base = mishikallm.get_api_base(
                        model=model,
    ...
    def _get_user_info_str(self, user_info: CallInfo) -> str:
        ...
        for k, v in _all_fields_as_dict.items():
            if isinstance(v, Mishikallm_EntityType):
                v = v.value
            msg += f"*{k}:* `{v}`\n"
    ...
    async def customer_spend_alert(
        ...
        if (
            ...
            event = WebhookEvent(
                ...
                event="spend_tracked",
                event_group=Mishikallm_EntityType.END_USER,
...
"""
    ...
    async def region_outage_alerts(
        ...
        ### GET PROVIDER ###
        provider = deployment.mishikallm_params.custom_llm_provider
        if provider is None:
            model, provider, _, _ = mishikallm.get_llm_provider(model=model)

        ### GET REGION ###
        region_name = deployment.mishikallm_params.region_name
        if region_name is None:
            region_name = mishikallm.utils._get_model_region(
                custom_llm_provider=provider, mishikallm_params=deployment.mishikallm_params
            )
        ...
        try:
            ...
            if provider is None:
                try:
                    ...
                    provider = ""
            api_base = mishikallm.get_api_base(
                model=model, optional_params=deployment.mishikallm_params
            )
    ...
    async def model_added_alert(
        self, model_name: str, mishikallm_model_name: str, passed_model_info: Any
    ):
        base_model_from_user = getattr(passed_model_info, "base_model", None)
        model_info = {}
        base_model = ""
        if base_model_from_user is not None:
            model_info = mishikallm.model_cost.get(base_model_from_user, {})
            base_model = f"Base Model: `{base_model_from_user}`\n"
        else:
            model_info = mishikallm.model_cost.get(mishikallm_model_name, {})
        model_info_str = ""
...
"""
    ...
    async def send_key_created_or_user_invited_email(
        ...
        try:
            from mishikallm.proxy.utils import send_email
            ...
            if email_logo_url is None:
                email_logo_url = MISHIKALLM_LOGO_URL
            if email_support_contact is None:
                email_support_contact = MISHIKALLM_SUPPORT_CONTACT
            ...
            email_html_content = "Alert from MishikaLLM Server"
            ...
            email_event = {
                "to": recipient_email,
                "subject": f"MishikaLLM: {event_name}",
    ...
    async def send_email_alert_using_smtp(
        ...
        if webhook_event.event == "budget_crossed":
            ...
            <img src="{email_logo_url}" alt="MishikaLLM Logo" width="150" height="50" />
        ...
        if webhook_event.event_group == "team":
            from mishikallm.integrations.email_alerting import send_team_budget_alert
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            if "daily_reports" in self.alert_types:
                ...
                model_info = mishikallm_params.get("model_info", {}) or {}
                model_id = model_info.get("id", "") or ""
                response_s: timedelta = end_time - start_time

                final_value = response_s

                if isinstance(response_obj, mishikallm.ModelResponse) and (
                ...
                await self.async_update_daily_reports(
                    DeploymentMetrics(
                        ...
                        updated_at=mishikallm.utils.get_utc_datetime(),
                    )
                )
        except Exception as e:
            verbose_proxy_logger.error(
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _mishikallm_params = kwargs.get("mishikallm_params", {})
        _model_info = _mishikallm_params.get("model_info", {}) or {}
    ...
    async def _request_is_completed(self, request_data: Optional[dict]) -> bool:
        ...
        if (
            request_data.get("mishikallm_status", "") != "success"
            and request_data.get("mishikallm_status", "") != "fail"
        ):
            ## CHECK IF CACHE IS UPDATED
            mishikallm_call_id = request_data.get("mishikallm_call_id", "")
            status: Optional[str] = await self.internal_usage_cache.async_get_cache(
                key="request_status:{}".format(mishikallm_call_id), local_only=True
            )
