...
from mishikallm.proxy._types import AlertType
from mishikallm.secret_managers.main import get_secret

if TYPE_CHECKING:
    from mishikallm.mishikallm_core_utils.mishikallm_logging import Logging as _Logging
...
async def _add_langfuse_trace_id_to_alert(
    ...
    if (
        ...
        and request_data.get("mishikallm_logging_obj", None) is not None
    ):
        trace_id: Optional[str] = None
        mishikallm_logging_obj: Logging = request_data["mishikallm_logging_obj"]

        for _ in range(3):
            trace_id = mishikallm_logging_obj._get_trace_id(service_name="langfuse")
            if trace_id is not None:
                break
            await asyncio.sleep(3)  # wait 3s before retrying for trace id

        _langfuse_object = mishikallm_logging_obj._get_callback_object(
