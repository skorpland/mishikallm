...
`async_log_success_event` - used by mishikallm proxy to send logs to datadog
`log_success_event` - sync version of logging to DataDog, only used on mishikallm Python SDK, if user opts in to using sync functions
...
from mishikallm.types.integrations.base_health_check import IntegrationHealthCheckStatus
from mishikallm.types.integrations.datadog import *
from mishikallm.types.services import ServiceLoggerPayload, ServiceTypes
...
class DataDogLogger(
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        """
        try:
            if mishikallm.datadog_use_v1 is True:
    ...
    def _create_v0_logging_payload(
        ...
        (Not Recommended) If you want this to get logged set `mishikallm.datadog_use_v1 = True`
        """
        import json
    ...
    def _get_datadog_tags(
        ...
        base_tags = {
            ...
            "service": os.getenv("DD_SERVICE", "mishikallm"),
    ...
    def _get_datadog_source():
        return os.getenv("DD_SOURCE", "mishikallm")

    @staticmethod
    def _get_datadog_service():
        return os.getenv("DD_SERVICE", "mishikallm-server")
