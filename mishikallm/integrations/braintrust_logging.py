...
class BraintrustLogger(CustomLogger):
    ...
    async def create_default_project_and_experiment(self):
        project = await global_braintrust_http_handler.post(
            f"{self.api_base}/project", headers=self.headers, json={"name": "mishikallm"}
        )
    ...
    def log_success_event(  # noqa: PLR0915
        ...
        try:
            ...
            if usage_obj and isinstance(usage_obj, mishikallm.Usage):
                mishikallm.utils.get_logging_id(start_time, response_obj)
                metrics = {
