...
from mishikallm.caching.caching import S3Cache
from mishikallm.mishikallm_core_utils.logging_utils import (
...
if TYPE_CHECKING:
    ...
    from mishikallm.utils import CustomStreamWrapper
else:
...
class LLMCachingHandler:
    ...
    async def _async_get_cache(
        ...
        cached_result: Optional[Any] = None
        if (
            (kwargs.get("caching", None) is None and mishikallm.cache is not None)
            or kwargs.get("caching", False) is True
        ) and (
            kwargs.get("cache", {}).get("no-cache", False) is not True
        ):  # allow users to control returning cached responses from the completion function
            if mishikallm.cache is not None and self._is_call_type_supported_by_cache(
                ...
                if cached_result is not None and not isinstance(cached_result, list):
                    ...
                    model, _, _, _ = mishikallm.get_llm_provider(
                        model=model,
                        custom_llm_provider=kwargs.get("custom_llm_provider", None),
                        api_base=kwargs.get("api_base", None),
                        api_key=kwargs.get("api_key", None),
                    )
                    self._update_mishikallm_logging_obj_environment(
                    ...
                    cache_key = mishikallm.cache._get_preset_cache_key_from_kwargs(
                        **kwargs
                    )
                    if (
                    ...
                    and not isinstance(
                        mishikallm.cache.cache, S3Cache
    ...
    def _sync_get_cache(
        ...
        if mishikallm.cache is not None and self._is_call_type_supported_by_cache(
            ...
            cached_result = mishikallm.cache.get_cache(**new_kwargs)
            ...
            for val in non_null_list:
                ...
                if cr is not None:
                    ...
                    if isinstance(original_kwargs_input[idx], str):
                        from mishikallm.utils import token_counter
    ...
    def _async_log_cache_hit_on_callbacks(
        ...
        Args:
            logging_obj (MishikaLLMLoggingObj): The logging object.
    ...
    async def _retrieve_from_cache(
        ...
        if call_type == CallTypes.aembedding.value and isinstance(
            ...
            for idx, i in enumerate(new_kwargs["input"]):
                preset_cache_key = mishikallm.cache.get_cache_key(
                    **{**new_kwargs, "input": i}
                )
                tasks.append(mishikallm.cache.async_get_cache(cache_key=preset_cache_key))
            ...
            if mishikallm.cache._supports_async() is True:
                cached_result = await mishikallm.cache.async_get_cache(**new_kwargs)
    ...
    def _convert_cached_result_to_model_response(
        ...
        Returns:
            Optional[Any]:
        """
        from mishikallm.utils import convert_to_model_response_object
    ...
    async def async_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            if (
                isinstance(result, mishikallm.ModelResponse)
                or isinstance(result, mishikallm.EmbeddingResponse)
                ...
                if (
                    ...
                    asyncio.create_task(
                        mishikallm.cache.async_add_cache_pipeline(result, **new_kwargs)
                    )
                elif isinstance(mishikallm.cache.cache, S3Cache):
                    threading.Thread(
                        target=mishikallm.cache.add_cache,
                        args=(result,),
                        kwargs=new_kwargs,
                    ).start()
                else:
                    asyncio.create_task(
                        mishikallm.cache.async_add_cache(
                            result.model_dump_json(), **new_kwargs
                        )
                    )
            else:
                asyncio.create_task(mishikallm.cache.async_add_cache(result, **new_kwargs))
    ...
    def sync_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            mishikallm.cache.add_cache(result, **new_kwargs)
    ...
    def _should_store_result_in_cache(
        ...
        """
        return (
            (mishikallm.cache is not None)
            and mishikallm.cache.supported_call_types is not None
            and (str(original_function.__name__) in mishikallm.cache.supported_call_types)
    ...
    def _is_call_type_supported_by_cache(
        ...
        Defined on `mishikallm.types.utils.CallTypes`
    ...
    async def _add_streaming_response_to_cache(self, processed_chunk: ModelResponse):
        ...
        - If 'streaming_chunk' has a 'finish_reason' then assemble a mishikallm.ModelResponse object
    ...
    def _update_mishikallm_logging_obj_environment(
        self,
        logging_obj: MishikaLLMLoggingObj,
        model: str,
        kwargs: Dict[str, Any],
        ...
        """
        Helper function to update the MishikaLLMLoggingObj environment variables.

        Args:
            logging_obj (MishikaLLMLoggingObj): The logging object to update.
        ...
        if mishikallm.cache is not None:
            mishikallm_params[
                "preset_cache_key"
            ] = mishikallm.cache._get_preset_cache_key_from_kwargs(**kwargs)
        else:
            mishikallm_params["preset_cache_key"] = None
