...
from mishikallm.constants import CACHED_STREAMING_CHUNK_DELAY
from mishikallm.mishikallm_core_utils.model_param_helper import ModelParamHelper
from mishikallm.types.caching import *
from mishikallm.types.utils import EmbeddingResponse, all_mishikallm_params
...
#### MishikaLLM.Completion / Embedding Cache ####
class Cache:
    def __init__(
        self,
        type: Optional[MishikaLLMCacheType] = MishikaLLMCacheType.LOCAL,
        mode: Optional[
            CacheMode
        ...
        **kwargs,
        ...
        Returns:
            None. Cache is set as a mishikallm param
        """
        if type == MishikaLLMCacheType.REDIS:
        ...
        elif type == MishikaLLMCacheType.REDIS_SEMANTIC:
        ...
        elif type == MishikaLLMCacheType.QDRANT_SEMANTIC:
        ...
        elif type == MishikaLLMCacheType.LOCAL:
            self.cache = InMemoryCache()
        elif type == MishikaLLMCacheType.S3:
            self.cache = S3Cache(
        ...
        elif type == MishikaLLMCacheType.DISK:
            self.cache = DiskCache(disk_cache_dir=disk_cache_dir)
        if "cache" not in mishikallm.input_callback:
            mishikallm.input_callback.append("cache")
        if "cache" not in mishikallm.success_callback:
            mishikallm.logging_callback_manager.add_mishikallm_success_callback("cache")
        if "cache" not in mishikallm._async_success_callback:
            mishikallm.logging_callback_manager.add_mishikallm_async_success_callback("cache")
        ...
        if self.type == MishikaLLMCacheType.LOCAL and default_in_memory_ttl is not None:
            self.ttl = default_in_memory_ttl

        if (
            self.type == MishikaLLMCacheType.REDIS
            or self.type == MishikaLLMCacheType.REDIS_SEMANTIC
    ...
    def get_cache_key(self, **kwargs) -> str:
        ...
        Args:
            **kwargs: kwargs to mishikallm.completion() or embedding()
        ...
        mishikallm_param_kwargs = all_mishikallm_params
        for param in kwargs:
            if param in combined_kwargs:
                ...
                param not in mishikallm_param_kwargs
            ):  # check if user passed in optional param - e.g. top_k
                if (
                    mishikallm.enable_caching_on_provider_specific_optional_params is True
    ...
    def _get_model_param_value(self, kwargs: dict) -> str:
        ...
        1. If caching groups are set, then return the caching group as the model https://docs.21t.cc/docs/routing#caching-across-model-groups
        2. Else if a model_group is set, then return the model_group as the model. This is used for all requests sent through the mishikallm.Router()
        3. Else use the `model` passed in kwargs
        """
        metadata: Dict = kwargs.get("metadata", {}) or {}
        mishikallm_params: Dict = kwargs.get("mishikallm_params", {}) or {}
        metadata_in_mishikallm_params: Dict = mishikallm_params.get("metadata", {}) or {}
        model_group: Optional[str] = metadata.get(
            "model_group"
        ) or metadata_in_mishikallm_params.get("model_group")
    ...
    def _get_file_param_value(self, kwargs: dict) -> str:
        ...
        return (
            ...
            or mishikallm_params.get("file_name")
        )

    def _get_preset_cache_key_from_kwargs(self, **kwargs) -> Optional[str]:
        """
        Get the preset cache key from kwargs["mishikallm_params"]
        ...
        """
        if kwargs:
            if "mishikallm_params" in kwargs:
                return kwargs["mishikallm_params"].get("preset_cache_key", None)
        return None
    ...
    def _set_preset_cache_key_in_kwargs(self, preset_cache_key: str, **kwargs) -> None:
        ...
        Placed in kwargs["mishikallm_params"]
        """
        if kwargs:
            if "mishikallm_params" in kwargs:
                kwargs["mishikallm_params"]["preset_cache_key"] = preset_cache_key
    ...
    def get_cache(self, **kwargs):
        ...
        Args:
            *args: args to mishikallm.completion() or embedding()
            **kwargs: kwargs to mishikallm.completion() or embedding()
    ...
    def add_cache(self, result, **kwargs):
        ...
        try:
            ...
            verbose_logger.exception(f"MishikaLLM Cache: Excepton add_cache: {str(e)}")
    ...
    def _supports_async(self) -> bool:
        ...
        """
        if self.type and self.type == MishikaLLMCacheType.S3:
            return False
        return True
...
def enable_cache(
    ...
    **kwargs,
    ...
    Raises:
        None
    """
    print_verbose("MishikaLLM: Enabling Cache")
    if "cache" not in mishikallm.input_callback:
        mishikallm.input_callback.append("cache")
    if "cache" not in mishikallm.success_callback:
        mishikallm.logging_callback_manager.add_mishikallm_success_callback("cache")
    ...
    print_verbose(f"MishikaLLM: Cache enabled, mishikallm.cache={mishikallm.cache}")
    print_verbose(f"MishikaLLM Cache: {vars(mishikallm.cache)}")
...
def update_cache(
    ...
    **kwargs,
    ...
    """
    Update the cache for MishikaLLM.
    ...
    """
    print_verbose("MishikaLLM: Updating Cache")
    mishikallm.cache = Cache(
        type=type,
        host=host,
        port=port,
    ...
    print_verbose(f"MishikaLLM: Cache Updated, mishikallm.cache={mishikallm.cache}")
    print_verbose(f"MishikaLLM Cache: {vars(mishikallm.cache)}")


def disable_cache():
    """
    Disable the cache used by MishikaLLM.

    This function disables the cache used by the MishikaLLM module. It removes the cache-related callbacks from the input_callback, success_callback, and _async_success_callback lists. It also sets the mishikallm.cache attribute to None.
    ...
    print_verbose("MishikaLLM: Disabling Cache")
    with suppress(ValueError):
        mishikallm.input_callback.remove("cache")
        mishikallm.success_callback.remove("cache")
        mishikallm._async_success_callback.remove("cache")

    mishikallm.cache = None
    print_verbose(f"MishikaLLM: Cache disabled, mishikallm.cache={mishikallm.cache}")
