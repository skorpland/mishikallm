...
import mishikallm.mishikallm_core_utils.exception_mapping_utils
from mishikallm import get_secret_str
from mishikallm._logging import verbose_router_logger
from mishikallm.caching.caching import (
    DualCache,
    InMemoryCache,
    RedisCache,
    RedisClusterCache,
)
from mishikallm.constants import DEFAULT_MAX_LRU_CACHE_SIZE
from mishikallm.integrations.custom_logger import CustomLogger
from mishikallm.mishikallm_core_utils.asyncify import run_async_function
from mishikallm.mishikallm_core_utils.core_helpers import _get_parent_otel_span_from_kwargs
from mishikallm.mishikallm_core_utils.credential_accessor import CredentialAccessor
from mishikallm.mishikallm_core_utils.dd_tracing import tracer
from mishikallm.mishikallm_core_utils.mishikallm_logging import Logging as MishikaLLMLogging
from mishikallm.router_strategy.budget_limiter import RouterBudgetLimiting
from mishikallm.router_strategy.least_busy import LeastBusyLoggingHandler
from mishikallm.router_strategy.lowest_cost import LowestCostLoggingHandler
from mishikallm.router_strategy.lowest_latency import LowestLatencyLoggingHandler
from mishikallm.router_strategy.lowest_tpm_rpm import LowestTPMLoggingHandler
from mishikallm.router_strategy.lowest_tpm_rpm_v2 import LowestTPMLoggingHandler_v2
from mishikallm.router_strategy.simple_shuffle import simple_shuffle
from mishikallm.router_strategy.tag_based_routing import get_deployments_for_tag
from mishikallm.router_utils.add_retry_fallback_headers import (
    add_fallback_headers_to_response,
    add_retry_headers_to_response,
)
from mishikallm.router_utils.batch_utils import _get_router_metadata_variable_name
from mishikallm.router_utils.client_initalization_utils import InitalizeCachedClient
from mishikallm.router_utils.clientside_credential_handler import (
    get_dynamic_mishikallm_params,
    is_clientside_credential,
)
from mishikallm.router_utils.cooldown_cache import CooldownCache
...
from mishikallm.router_utils.fallback_event_handlers import (
...
from mishikallm.router_utils.handle_error import (
    async_raise_no_deployment_exception,
    send_llm_exception_alert,
)
from mishikallm.router_utils.pre_call_checks.prompt_caching_deployment_check import (
    PromptCachingDeploymentCheck,
)
from mishikallm.router_utils.pre_call_checks.responses_api_deployment_check import (
    ResponsesApiDeploymentCheck,
)
from mishikallm.router_utils.router_callbacks.track_deployment_metrics import (
...
from mishikallm.types.router import (
    CONFIGURABLE_CLIENTSIDE_AUTH_PARAMS,
    VALID_MISHIKALLM_ENVIRONMENTS,
    AlertingConfig,
    AllowedFailsPolicy,
    AssistantsTypedDict,
    CredentialMishikaLLMParams,
    CustomPricingMishikaLLMParams,
...
from mishikallm.types.utils import GenericBudgetConfigType
from mishikallm.types.utils import ModelInfo
from mishikallm.types.utils import ModelInfo as ModelMapInfo
from mishikallm.types.utils import StandardLoggingPayload
from mishikallm.utils import (
    CustomStreamWrapper,
    EmbeddingResponse,
    ModelResponse,
    Rules,
...
class Router:
    ...
    def __init__(  # noqa: PLR0915
        ...
        default_mishikallm_params: Optional[
            dict
        ...
        Args:
            ...
            default_mishikallm_params (dict): Default parameters for Router.chat.completion.create. Defaults to {}.
        ...
        Returns:
            Router: An instance of the mishikallm.Router class.
        ...
        from mishikallm._service_logger import ServiceLogging

        self.set_verbose = set_verbose
        self.debug_level = debug_level
        self.enable_pre_call_checks = enable_pre_call_checks
        self.enable_tag_filtering = enable_tag_filtering
        mishikallm.suppress_debug_info = True  # prevents 'Give Feedback/Get help' message from being emitted on Router - Relevant Issue: https://github.com/skorpland/mishikallm/issues/5942
        ...
        self.deployment_names: List = (
            []
        )  # names of models under mishikallm_params. ex. azure/chatgpt-v-2
        ...
        if cache_responses:
            if mishikallm.cache is None:
                ...
                mishikallm.cache = mishikallm.Cache(type=cache_type, **cache_config)  # type: ignore
        ...
        if model_list is not None:
            ...
            for m in model_list:
                if "model" in m["mishikallm_params"]:
                    self.deployment_latency_map[m["mishikallm_params"]["model"]] = 0
        else:
        ...
        if allowed_fails is not None:
            ...
            self.allowed_fails = mishikallm.allowed_fails
        ...
        elif mishikallm.num_retries is not None:
            self.num_retries = mishikallm.num_retries
        else:
            self.num_retries = openai.DEFAULT_MAX_RETRIES

        if max_fallbacks is not None:
            self.max_fallbacks = max_fallbacks
        elif mishikallm.max_fallbacks is not None:
            self.max_fallbacks = mishikallm.max_fallbacks
        else:
            self.max_fallbacks = mishikallm.ROUTER_MAX_FALLBACKS

        self.timeout = timeout or mishikallm.request_timeout
        ...
        _fallbacks = fallbacks or mishikallm.fallbacks

        self.validate_fallbacks(fallback_param=_fallbacks)
        ### set fallbacks
        self.fallbacks = _fallbacks

        if default_fallbacks is not None or mishikallm.default_fallbacks is not None:
            _fallbacks = default_fallbacks or mishikallm.default_fallbacks
        ...
        self.context_window_fallbacks = (
            context_window_fallbacks or mishikallm.context_window_fallbacks
        )

        _content_policy_fallbacks = (
            content_policy_fallbacks or mishikallm.content_policy_fallbacks
        )
        ...
        default_mishikallm_params = default_mishikallm_params or {}
        self.chat = mishikallm.Chat(params=default_mishikallm_params, router_obj=self)

        # default mishikallm args
        self.default_mishikallm_params = default_mishikallm_params
        self.default_mishikallm_params.setdefault("timeout", timeout)
        self.default_mishikallm_params.setdefault("max_retries", 0)
        self.default_mishikallm_params.setdefault("metadata", {}).update(
        ...
        ## USAGE TRACKING ##
        if isinstance(mishikallm._async_success_callback, list):
    ...
    def discard(self):
        ...
        """
        mishikallm.logging_callback_manager.remove_callback_from_list_by_object(
            mishikallm._async_success_callback, self
        )
        mishikallm.logging_callback_manager.remove_callback_from_list_by_object(
            mishikallm.success_callback, self
        )
        mishikallm.logging_callback_manager.remove_callback_from_list_by_object(
            mishikallm._async_failure_callback, self
        )
        mishikallm.logging_callback_manager.remove_callback_from_list_by_object(
            mishikallm.failure_callback, self
        )
        mishikallm.logging_callback_manager.remove_callback_from_list_by_object(
            mishikallm.input_callback, self
        )
        mishikallm.logging_callback_manager.remove_callback_from_list_by_object(
            mishikallm.service_callback, self
        )
        mishikallm.logging_callback_manager.remove_callback_from_list_by_object(
            mishikallm.callbacks, self
        )
    ...
    def routing_strategy_init(
        ...
        if (
            ...
            ## add callback
            if isinstance(mishikallm.input_callback, list):
                mishikallm.input_callback.append(self.leastbusy_logger)  # type: ignore
            else:
                mishikallm.input_callback = [self.leastbusy_logger]  # type: ignore
            if isinstance(mishikallm.callbacks, list):
                mishikallm.logging_callback_manager.add_mishikallm_callback(self.leastbusy_logger)  # type: ignore
        elif (
            ...
            if isinstance(mishikallm.callbacks, list):
                mishikallm.logging_callback_manager.add_mishikallm_callback(self.lowesttpm_logger)  # type: ignore
        elif (
            ...
            if isinstance(mishikallm.callbacks, list):
                mishikallm.logging_callback_manager.add_mishikallm_callback(self.lowesttpm_logger_v2)  # type: ignore
        elif (
            ...
            if isinstance(mishikallm.callbacks, list):
                mishikallm.logging_callback_manager.add_mishikallm_callback(self.lowestlatency_logger)  # type: ignore
        elif (
            ...
            if isinstance(mishikallm.callbacks, list):
                mishikallm.logging_callback_manager.add_mishikallm_callback(self.lowestcost_logger)  # type: ignore
        else:
            pass

    def initialize_assistants_endpoint(self):
        ## INITIALIZE PASS THROUGH ASSISTANTS ENDPOINT ##
        self.acreate_assistants = self.factory_function(mishikallm.acreate_assistants)
        self.adelete_assistant = self.factory_function(mishikallm.adelete_assistant)
        self.aget_assistants = self.factory_function(mishikallm.aget_assistants)
        self.acreate_thread = self.factory_function(mishikallm.acreate_thread)
        self.aget_thread = self.factory_function(mishikallm.aget_thread)
        self.a_add_message = self.factory_function(mishikallm.a_add_message)
        self.aget_messages = self.factory_function(mishikallm.aget_messages)
        self.arun_thread = self.factory_function(mishikallm.arun_thread)

    def initialize_router_endpoints(self):
        self.amoderation = self.factory_function(
            mishikallm.amoderation, call_type="moderation"
        )
        self.aanthropic_messages = self.factory_function(
            mishikallm.anthropic_messages, call_type="anthropic_messages"
        )
        self.aresponses = self.factory_function(
            mishikallm.aresponses, call_type="aresponses"
        )
        self.afile_delete = self.factory_function(
            mishikallm.afile_delete, call_type="afile_delete"
        )
        self.afile_content = self.factory_function(
            mishikallm.afile_content, call_type="afile_content"
        )
        self.responses = self.factory_function(mishikallm.responses, call_type="responses")
        self.aget_responses = self.factory_function(
            mishikallm.aget_responses, call_type="aget_responses"
        )
        self.adelete_responses = self.factory_function(
            mishikallm.adelete_responses, call_type="adelete_responses"
        )
        ...
        if optional_pre_call_checks is not None:
            for pre_call_check in optional_pre_call_checks:
                ...
                if _callback is not None:
                    mishikallm.logging_callback_manager.add_mishikallm_callback(_callback)
    ...
    def print_deployment(self, deployment: dict):
        ...
        try:
            ...
            mishikallm_params: dict = _deployment_copy["mishikallm_params"]
            if "api_key" in mishikallm_params:
                mishikallm_params["api_key"] = mishikallm_params["api_key"][:2] + "*" * 10
    ...
    def _completion(
        ...
        try:
            ...
            data = deployment["mishikallm_params"].copy()
            ...
            verbose_router_logger.info(
                f"mishikallm.completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            ...
            if isinstance(response, ModelResponse):
                ...
                if _should_raise:
                    raise mishikallm.ContentPolicyViolationError(
            ...
            verbose_router_logger.info(
                f"mishikallm.completion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            raise e
    ...
    async def acompletion(
        ...
        **kwargs,
        ...
        try:
            ...
            asyncio.create_task(
                send_llm_exception_alert(
                    mishikallm_router_instance=self,
    ...
    async def _acompletion(
        ...
        try:
            ...
            _response = mishikallm.acompletion(
                **{
                    **data,
                    "messages": messages,
                    "caching": self.cache_responses,
            ...
            logging_obj: Optional[MishikaLLMLogging] = kwargs.get(
            ...
            verbose_router_logger.info(
                f"mishikallm.acompletion(model={model_name})\033[32m 200 OK\033[0m"
            )
            # debug how often this deployment picked
            self._track_deployment_metrics(
                deployment=deployment,
            ...
            verbose_router_logger.info(
                f"mishikallm.acompletion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    def _update_kwargs_before_fallbacks(self, model: str, kwargs: dict) -> None:
        ...
        kwargs.setdefault("mishikallm_trace_id", str(uuid.uuid4()))
        kwargs.setdefault("metadata", {}).update({"model_group": model})

    def _update_kwargs_with_default_mishikallm_params(
        self, kwargs: dict, metadata_variable_name: Optional[str] = "metadata"
    ) -> None:
        """
        Adds default mishikallm params to kwargs, if set.

        Handles inserting this as either "metadata" or "mishikallm_metadata" depending on the metadata_variable_name
        """
        # 1) copy your defaults and pull out metadata
        defaults = self.default_mishikallm_params.copy()
        ...
        # 3) merge in metadata, this handles inserting this as either "metadata" or "mishikallm_metadata"
    ...
    def _handle_clientside_credential(
        ...
        mishikallm_params = deployment["mishikallm_params"].copy()
        dynamic_mishikallm_params = get_dynamic_mishikallm_params(
            mishikallm_params=mishikallm_params, request_kwargs=kwargs
        )
        metadata = kwargs.get("metadata", {})
        model_group = cast(str, metadata.get("model_group"))
        _model_id = self._generate_model_id(
            model_group=model_group, mishikallm_params=dynamic_mishikallm_params
        )
        ...
        deployment_pydantic_obj = Deployment(
            ...
            mishikallm_params=MishikaLLM_Params(**dynamic_mishikallm_params),
    ...
    def _update_kwargs_with_deployment(
        ...
        """
        model_info = deployment.get("model_info", {}).copy()
        deployment_model_name = deployment["mishikallm_params"]["model"]
        deployment_api_base = deployment["mishikallm_params"].get("api_base")
        if is_clientside_credential(request_kwargs=kwargs):
            ...
            deployment_model_name = deployment_pydantic_obj.mishikallm_params.model
            deployment_api_base = deployment_pydantic_obj.mishikallm_params.api_base
        ...
        kwargs["timeout"] = self._get_timeout(
            kwargs=kwargs, data=deployment["mishikallm_params"]
        )

        self._update_kwargs_with_default_mishikallm_params(
    ...
    def _get_stream_timeout(
        ...
        return (
            ...
            or data.get(
                "stream_timeout", None
            )  # timeout set on mishikallm_params for this deployment
            or self.stream_timeout  # timeout set on router
            or self.default_mishikallm_params.get("stream_timeout", None)
        )
    ...
    def _get_non_stream_timeout(
        ...
        timeout = (
            ...
            or self.default_mishikallm_params.get("timeout", None)
        )
        return timeout
    ...
    async def abatch_completion(
        ...
        **kwargs,
        ...
        1. Batch Process 1 request to N models on mishikallm.Router. Pass messages as List[Dict[str, str]] to use this
        2. Batch Process N requests to M models on mishikallm.Router. Pass messages as List[List[Dict[str, str]]] to use this
        ...
        ```
            response = await router.abatch_completion(
                ...
                messages=[
                    {"role": "user", "content": "is mishikallm becoming a better product ?"}
                ],
                max_tokens=15,
            )
        ```
    ...
    async def abatch_completion_one_model_multiple_requests(
        ...
        """
        Async Batch Completion - Batch Process multiple Messages to one model_group on mishikallm.Router
        ...
        if make_request:
            try:
                ...
                _response._hidden_params["additional_headers"].update(
                    {"x-mishikallm-request-prioritization-used": True}
                )
                return _response
            except Exception as e:
    ...
    def _is_prompt_management_model(self, model: str) -> bool:
        ...
        mishikallm_model = model_list[0]["mishikallm_params"].get("model", None)

        if mishikallm_model is None:
            return False

        if "/" in mishikallm_model:
            split_mishikallm_model = mishikallm_model.split("/")[0]
            if split_mishikallm_model in mishikallm._known_custom_logger_compatible_callbacks:
                return True
        return False
    ...
    async def _prompt_management_factory(
        ...
        mishikallm_logging_object = kwargs.get("mishikallm_logging_obj", None)
        if mishikallm_logging_object is None:
            mishikallm_logging_object, kwargs = function_setup(
                **{
        ...
        mishikallm_logging_object = cast(MishikaLLMLogging, mishikallm_logging_object)
        ...
        mishikallm_model = prompt_management_deployment["mishikallm_params"].get(
            "model", None
        )
        ...
        prompt_variables = kwargs.get(
            "prompt_variables"
        ) or prompt_management_deployment["mishikallm_params"].get(
        ...
        (
            ...
            messages,
            optional_params,
        ) = mishikallm_logging_object.get_chat_completion_prompt(
            model=mishikallm_model,
            messages=messages,
        ...
        kwargs["model"] = model
        kwargs["messages"] = messages
        kwargs["mishikallm_logging_obj"] = mishikallm_logging_object
        ...
        if _model_list is None or len(_model_list) == 0:  # if direct call to model
            ...
            return await mishikallm.acompletion(**kwargs)
    ...
    def _image_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.image_generation(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"mishikallm.image_generation(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _atranscription(self, file: FileTypes, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = mishikallm.atranscription(
                **{
                    **data,
                    "file": file,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.atranscription(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"mishikallm.atranscription(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def aspeech(self, model: str, input: str, voice: str, **kwargs):
        ...
        try:
            ...
            data = deployment["mishikallm_params"].copy()
            data["model"]
            for k, v in self.default_mishikallm_params.items():
                if (
                    k not in kwargs
    ...
    async def _arerank(self, model: str, **kwargs):
        ...
        try:
            ...
            response = await mishikallm.arerank(
                **{
                    **data,
                    "caching": self.cache_responses,
                    "client": model_client,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.arerank(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"mishikallm.arerank(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _arealtime(self, model: str, **kwargs):
        ...
        try:
            ...
            return await mishikallm._arealtime(**{**data, "caching": self.cache_responses, **kwargs})  # type: ignore
        except Exception as e:
    ...
    def text_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            # call via mishikallm.completion()
            return mishikallm.text_completion(**{**data, "prompt": prompt, "caching": self.cache_responses, **kwargs})  # type: ignore
        except Exception as e:
            raise e
    ...
    async def _atext_completion(self, model: str, prompt: str, **kwargs):
        try:
            ...
            response = mishikallm.atext_completion(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.atext_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"mishikallm.atext_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    async def _aadapter_completion(self, adapter_id: str, model: str, **kwargs):
        try:
            ...
            response = mishikallm.aadapter_completion(
                **{
                    **data,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.aadapter_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"mishikallm.aadapter_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _ageneric_api_call_with_fallbacks(
        ...
        """
        Make a generic LLM API call through the router, this allows you to use retries/fallbacks with mishikallm router

        Args:
            model: The model to use
            handler_function: The handler function to call (e.g., mishikallm.anthropic_messages)
    ...
    def _generic_api_call_with_fallbacks(
        ...
        Args:
            model: The model to use
            original_function: The handler function to call (e.g., mishikallm.completion)
            **kwargs: Additional arguments to pass to the handler function
        Returns:
            The response from the handler function
        """
    ...
    def _embedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.embedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"mishikallm.embedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aembedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = mishikallm.aembedding(
                **{
                    **data,
                    "input": input,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.aembedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"mishikallm.aembedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _acreate_file(
        ...
        **kwargs,
        ...
        try:
            ...
            response = mishikallm.acreate_file(
                **{
                    **data,
                    "custom_llm_provider": custom_llm_provider,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"mishikallm.acreate_file(model={model_name})\033[32m 200 OK\033[0m"
            )

            return response  # type: ignore
        except Exception as e:
            verbose_router_logger.exception(
                f"mishikallm.acreate_file(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _acreate_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            kwargs.setdefault(metadata_variable_name, {}).update(
                {
                    "deployment": deployment["mishikallm_params"]["model"],
                    "model_info": deployment.get("model_info", {}),
                    "api_base": deployment.get("mishikallm_params", {}).get("api_base"),
                }
            )
            ...
            response = mishikallm.acreate_batch(
                **{
                    **data,
                    "custom_llm_provider": custom_llm_provider,
                    "caching": self.cache_responses,
            ...
            verbose_router_logger.exception(
                f"mishikallm._acreate_batch(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
            ...
            async def try_retrieve_batch(model_name):
                try:
                    ...
                    _, custom_llm_provider, _, _ = get_llm_provider(  # type: ignore
                        model=model_name["mishikallm_params"]["model"]
                    )
                    new_kwargs = copy.deepcopy(kwargs)
                    new_kwargs.pop("custom_llm_provider", None)
                    return await mishikallm.aretrieve_batch(
    ...
    async def alist_batches(
        ...
        **kwargs,
        ...
        async def try_retrieve_batch(model: DeploymentTypedDict):
            try:
                # Update kwargs with the current model name or any other model-specific adjustments
                return await mishikallm.alist_batches(
                    **{**model["mishikallm_params"], **kwargs}
                )
            except Exception:
                return None
    ...
    async def _pass_through_moderation_endpoint_factory(
        ...
        **kwargs,
        ...
        if kwargs.get("model") and self.get_model_list(model_name=kwargs["model"]):
            ...
            kwargs["model"] = deployment["mishikallm_params"]["model"]
    ...
    async def _init_responses_api_endpoints(
        ...
        **kwargs,
        ...
        """
        from mishikallm.responses.utils import ResponsesAPIRequestUtils
        ...
        **kwargs,
        ...
        if custom_llm_provider is None:
            if self.assistants_config is not None:
                ...
                kwargs.update(self.assistants_config["mishikallm_params"])
            else:
                raise Exception(
    ...
    async def async_function_with_fallbacks(self, *args, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            try:
                ...
                if isinstance(e, mishikallm.ContextWindowExceededError):
                    if context_window_fallbacks is not None:
                        ...
                        error_message = "model={}. context_window_fallbacks={}. fallbacks={}.\n\nSet 'context_window_fallback' - https://docs.21t.cc/docs/routing#fallbacks".format(
                ...
                elif isinstance(e, mishikallm.ContentPolicyViolationError):
                    if content_policy_fallbacks is not None:
                        ...
                        error_message = "model={}. content_policy_fallback={}. fallbacks={}.\n\nSet 'content_policy_fallback' - https://docs.21t.cc/docs/routing#fallbacks".format(
                ...
                verbose_router_logger.error(
                    "mishikallm.router.py::async_function_with_fallbacks() - Error occurred while trying to do fallbacks - {}\n{}\n\nDebug Information:\nCooldown Deployments={}".format(
                        str(new_exception),
    ...
    def _handle_mock_testing_fallbacks(
        ...
        """
        Helper function to raise a mishikallm Error for mock testing purposes.

        Raises:
            mishikallm.InternalServerError: when `mock_testing_fallbacks=True` passed in request params
            mishikallm.ContextWindowExceededError: when `mock_testing_context_fallbacks=True` passed in request params
            mishikallm.ContentPolicyViolationError: when `mock_testing_content_policy_fallbacks=True` passed in request params
        """
        ...
        if mock_testing_fallbacks is not None and mock_testing_fallbacks is True:
            ...
            raise mishikallm.ContextWindowExceededError(
                model=model_group,
                llm_provider="",
    ...
    async def async_function_with_retries(self, *args, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            if type(original_exception) in mishikallm.MISHIKALLM_EXCEPTION_TYPES:
    ...
    def _handle_mock_testing_rate_limit_error(
        ...
        """
        Helper function to raise a mock mishikallm.RateLimitError error for testing purposes.

        Raises:
            mishikallm.RateLimitError error when `mock_testing_rate_limit_error=True` passed in request params
        """
        ...
        if available_models is not None and len(available_models) == 1:
            num_retries = cast(
                Optional[int], available_models[0]["mishikallm_params"].get("num_retries")
            )
        ...
        if (
            ...
            verbose_router_logger.info(
                f"mishikallm.router.py::_mock_rate_limit_error() - Raising mock RateLimitError for model={model_group}"
            )
            raise mishikallm.RateLimitError(
                model=model_group,
                llm_provider="",
    ...
    def should_retry_this_error(
        ...
        if (
            isinstance(error, mishikallm.ContextWindowExceededError)
            and context_window_fallbacks is not None
        ):
            raise error

        if (
            isinstance(error, mishikallm.ContentPolicyViolationError)
            and content_policy_fallbacks is not None
        ):
            raise error

        if isinstance(error, mishikallm.NotFoundError):
            raise error
    ...
    def _time_to_sleep_before_retry(
        ...
        if hasattr(e, "mishikallm_response_headers"):
            response_headers = e.mishikallm_response_headers  # type: ignore

        if response_headers is not None:
            timeout = mishikallm._calculate_retry_after(
                remaining_retries=remaining_retries,
                max_retries=num_retries,
    ...
    async def deployment_callback_on_success(
        ...
        try:
            ...
            if kwargs["mishikallm_params"].get("metadata") is None:
                pass
            else:
                deployment_name = kwargs["mishikallm_params"]["metadata"].get(
                    "deployment", None
            ...
            verbose_router_logger.exception(
                "mishikallm.router.Router::deployment_callback_on_success(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    def sync_deployment_callback_on_success(
        ...
        if kwargs["mishikallm_params"].get("metadata") is None:
            ...
            model_group = kwargs["mishikallm_params"]["metadata"].get("model_group", None)
            model_info = kwargs["mishikallm_params"].get("model_info", {}) or {}
    ...
    def deployment_callback_on_failure(
        ...
        try:
            ...
            _model_info = kwargs.get("mishikallm_params", {}).get("model_info", {})

            exception_headers = mishikallm.mishikallm_core_utils.exception_mapping_utils._get_response_headers(
                original_exception=exception
            )

            _time_to_cooldown = kwargs.get("mishikallm_params", {}).get(
                "cooldown_time", self.cooldown_time
            )

            if exception_headers is not None:
                _time_to_cooldown = (
                    mishikallm.utils._get_retry_after_from_exception_header(
    ...
    async def async_deployment_callback_on_failure(
        ...
        deployment_name = kwargs["mishikallm_params"]["metadata"].get(
            "deployment", None
        )  # handles wildcard routes - by giving the original name sent to `mishikallm.completion`
        model_group = kwargs["mishikallm_params"]["metadata"].get("model_group", None)
        model_info = kwargs["mishikallm_params"].get("model_info", {}) or {}
        id = model_info.get("id", None)
        if model_group is None or id is None:
    ...
    def _get_healthy_deployments(self, model: str, parent_otel_span: Optional[Span]):
        ...
        unhealthy_deployments = _get_cooldown_deployments(
            mishikallm_router_instance=self, parent_otel_span=parent_otel_span
        )
    ...
    def routing_strategy_pre_call_checks(self, deployment: dict):
        ...
        """
        for _callback in mishikallm.callbacks:
    ...
    async def async_routing_strategy_pre_call_checks(
        ...
        for _callback in mishikallm.callbacks:
            if isinstance(_callback, CustomLogger):
                ...
                except mishikallm.RateLimitError as e:
                    ## LOG FAILURE EVENT
    ...
    def _generate_model_id(self, model_group: str, mishikallm_params: dict):
        """
        Helper function to consistently generate the same id for a deployment

        - create a string from all the mishikallm params
        - hash
        - use hash as id
        """
        concat_str = model_group
        for k, v in mishikallm_params.items():
            if isinstance(k, str):
                concat_str += k
    ...
    def _create_deployment(
        ...
        - None: If the deployment is not active for the current environment (if 'supported_environments' is set in mishikallm_params)
        """
        deployment = Deployment(
            **deployment_info,
            model_name=_model_name,
            mishikallm_params=MishikaLLM_Params(**_mishikallm_params),
            model_info=_model_info,
        )

        for field in CustomPricingMishikaLLMParams.model_fields.keys():
            if deployment.mishikallm_params.get(field) is not None:
                _model_info[field] = deployment.mishikallm_params[field]

        ## REGISTER MODEL INFO IN MISHIKALLM MODEL COST MAP
        ...
        _model_name = deployment.mishikallm_params.model
        if deployment.mishikallm_params.custom_llm_provider is not None:
            _model_name = (
                deployment.mishikallm_params.custom_llm_provider + "/" + _model_name
            )
    ...
    def deployment_is_active_for_environment(self, deployment: Deployment) -> bool:
        ...
        Requires `MISHIKALLM_ENVIRONMENT` to be set in .env. Valid values for environment:
            - development
            - staging
            - production

        Raises:
        - ValueError: If MISHIKALLM_ENVIRONMENT is not set in .env or not one of the valid values
        ...
        if (
            ...
            return True
        mishikallm_environment = get_secret_str(secret_name="MISHIKALLM_ENVIRONMENT")
        if mishikallm_environment is None:
            raise ValueError(
                "Set 'supported_environments' for model but not 'MISHIKALLM_ENVIRONMENT' set in .env"
            )

        if mishikallm_environment not in VALID_MISHIKALLM_ENVIRONMENTS:
            raise ValueError(
                f"MISHIKALLM_ENVIRONMENT must be one of {VALID_MISHIKALLM_ENVIRONMENTS}. but set as: {mishikallm_environment}"
            )

        for _env in deployment.model_info["supported_environments"]:
            if _env not in VALID_MISHIKALLM_ENVIRONMENTS:
                raise ValueError(
                    f"supported_environments must be one of {VALID_MISHIKALLM_ENVIRONMENTS}. but set as: {_env} for deployment: {deployment}"
                )

        if mishikallm_environment in deployment.model_info["supported_environments"]:
            return True
        return False
    ...
    def set_model_list(self, model_list: list):
        ...
        for model in original_model_list:
            ...
            _mishikallm_params = model.pop("mishikallm_params")
            ## check if mishikallm params in os.environ
            if isinstance(_mishikallm_params, dict):
                for k, v in _mishikallm_params.items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        _mishikallm_params[k] = get_secret(v)

            _model_info: dict = model.pop("model_info", {})

            # check if model info has id
            if "id" not in _model_info:
                _id = self._generate_model_id(_model_name, _mishikallm_params)
                _model_info["id"] = _id

            if _mishikallm_params.get("organization", None) is not None and isinstance(
                _mishikallm_params["organization"], list
            ):  # Addresses https://github.com/skorpland/mishikallm/issues/3949
                for org in _mishikallm_params["organization"]:
                    _mishikallm_params["organization"] = org
                    self._create_deployment(
                        deployment_info=model,
                        _model_name=_model_name,
                        _mishikallm_params=_mishikallm_params,
    ...
    def _add_deployment(self, deployment: Deployment) -> Deployment:
        ...
        self.deployment_names.append(deployment.mishikallm_params.model)
        ############ Users can either pass tpm/rpm as a mishikallm_param or a router param ###########
        # for get_available_deployment, we use the mishikallm_param["rpm"]
        # in this snippet we also set rpm to be a mishikallm_param
        if (
            deployment.mishikallm_params.rpm is None
            and getattr(deployment, "rpm", None) is not None
        ):
            deployment.mishikallm_params.rpm = getattr(deployment, "rpm")

        if (
            deployment.mishikallm_params.tpm is None
            and getattr(deployment, "tpm", None) is not None
        ):
            deployment.mishikallm_params.tpm = getattr(deployment, "tpm")
        ...
        (
            ...
            dynamic_api_key,
            api_base,
        ) = mishikallm.get_llm_provider(
            model=deployment.mishikallm_params.model,
            custom_llm_provider=deployment.mishikallm_params.get(
                "custom_llm_provider", None
            ),
        )
        ...
        #     if deployment.mishikallm_params.model == "*":
        #         # user wants to pass through all requests to mishikallm.acompletion for unknown deployments
        ...
        data_sources = deployment.mishikallm_params.get("dataSources", []) or []
        ...
        # done reading model["mishikallm_params"]
        if custom_llm_provider not in mishikallm.provider_list:
            raise Exception(f"Unsupported provider - {custom_llm_provider}")

        # # init OpenAI, Azure clients
        # InitalizeOpenAISDKClient.set_client(
        #     mishikallm_router_instance=self, model=deployment.to_json(exclude_none=True)
        # )
    ...
    def _initialize_deployment_for_pass_through(
        ...
        """
        Optional: Initialize deployment for pass-through endpoints if `deployment.mishikallm_params.use_in_pass_through` is True
        ...
        """
        if deployment.mishikallm_params.use_in_pass_through is True:
            from mishikallm.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
                passthrough_endpoint_router,
            )

            if deployment.mishikallm_params.mishikallm_credential_name is not None:
                credential_values = CredentialAccessor.get_credential_values(
                    deployment.mishikallm_params.mishikallm_credential_name
                )
            else:
                credential_values = {}
            ...
            if custom_llm_provider == "vertex_ai":
                vertex_project = (
                    ...
                    or deployment.mishikallm_params.vertex_project
                )
                vertex_location = (
                    credential_values.get("vertex_location")
                    or deployment.mishikallm_params.vertex_location
                )
                vertex_credentials = (
                    credential_values.get("vertex_credentials")
                    or deployment.mishikallm_params.vertex_credentials
                )
                ...
                if (
                    ...
                    raise ValueError(
                        "vertex_project, vertex_location, and vertex_credentials must be set in mishikallm_params for pass-through endpoints"
                    )
                ...
                api_base = (
                    ...
                    or deployment.mishikallm_params.api_base
                )
                api_key = (
                    credential_values.get("api_key")
                    or deployment.mishikallm_params.api_key
                )
    ...
    def upsert_deployment(self, deployment: Deployment) -> Optional[Deployment]:
        ...
        if _deployment_on_router is not None:
            ...
            if deployment.mishikallm_params == _deployment_on_router.mishikallm_params:
                # No need to update
                return None

            # if there is a new mishikallm param -> then update the deployment
    ...
    def get_deployment_credentials(self, model_id: str) -> Optional[dict]:
        ...
        if deployment is None:
            return None
        return CredentialMishikaLLMParams(
            **deployment.mishikallm_params.model_dump(exclude_none=True)
    ...
    def get_router_model_info(
        ...
        Augment mishikallm info with additional params set in `model_info`.
        ...
        if base_model is None:
            base_model = deployment.get("mishikallm_params", {}).get("base_model", None)

        model = base_model

        ## GET PROVIDER
        _model, custom_llm_provider, _, _ = mishikallm.get_llm_provider(
            model=deployment.get("mishikallm_params", {}).get("model", ""),
            mishikallm_params=MishikaLLM_Params(**deployment.get("mishikallm_params", {})),
        )

        ## SET MODEL TO 'model=' - if base_model is None + not azure
        if custom_llm_provider == "azure" and base_model is None:
            verbose_router_logger.error(
                "Could not identify azure model. Set azure 'base_model' for accurate max tokens, cost tracking, etc.- https://docs.21t.cc/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models"
            )
                ...
                for potential_model in potential_models:
                    try:
                        if potential_model.get("model_info", {}).get(
                            ...
                            model = potential_model.get("mishikallm_params", {}).get(
                                "model"
                            )
                            break
                    except Exception:
        ...
        ## GET MISHIKALLM MODEL INFO - raises exception, if model is not mapped
        if not model.startswith("{}/".format(custom_llm_provider)):
            model_info_name = "{}/{}".format(custom_llm_provider, model)
        else:
            model_info_name = model

        model_info = mishikallm.get_model_info(model=model_info_name)
    ...
    def get_model_info(self, id: str) -> Optional[dict]:
        ...
        Returns
        - dict: the model in list with 'model_name', 'mishikallm_params', Optional['model_info']
    ...
    def get_deployment_model_info(
        ...
        2. If not, check if mishikallm model name is in model info
        3. If not, return None
        """
        from mishikallm.utils import _update_dictionary

        model_info: Optional[ModelInfo] = None
        custom_model_info: Optional[dict] = None
        mishikallm_model_name_model_info: Optional[ModelInfo] = None

        try:
            custom_model_info = mishikallm.model_cost.get(model_id)
        except Exception:
            pass

        try:
            mishikallm_model_name_model_info = mishikallm.get_model_info(model=model_name)
        except Exception:
            pass

        if custom_model_info is not None and mishikallm_model_name_model_info is not None:
            model_info = cast(
                ModelInfo,
                _update_dictionary(
                    cast(dict, mishikallm_model_name_model_info).copy(),
                    custom_model_info,
                ),
            )
        elif mishikallm_model_name_model_info is not None:
            model_info = mishikallm_model_name_model_info
    ...
    def _set_model_group_info(  # noqa: PLR0915
        ...
        for model in model_list:
            ...
            mishikallm_params = MishikaLLM_Params(**model["mishikallm_params"])  # type: ignore
            # get configurable clientside auth params
            configurable_clientside_auth_params = (
                mishikallm_params.configurable_clientside_auth_params
            )
            # get model tpm
            ...
            if _deployment_tpm is None:
                _deployment_tpm = model.get("mishikallm_params", {}).get("tpm", None)  # type: ignore
            if _deployment_tpm is None:
            ...
            if _deployment_rpm is None:
                _deployment_rpm = model.get("mishikallm_params", {}).get("rpm", None)  # type: ignore
            if _deployment_rpm is None:
            ...
            try:
                ...
                if model_id is not None:
                    model_info = self.get_deployment_model_info(
                        model_id=model_id, model_name=mishikallm_params.model
                    )
                else:
                    model_info = None
            except Exception:
            ...
            # get llm provider
            mishikallm_model, llm_provider = "", ""
            try:
                mishikallm_model, llm_provider, _, _ = mishikallm.get_llm_provider(
                    model=mishikallm_params.model,
                    custom_llm_provider=mishikallm_params.custom_llm_provider,
                )
            except mishikallm.exceptions.BadRequestError as e:
                verbose_router_logger.error(
                    "mishikallm.router.py::get_model_group_info() - {}".format(str(e))
                )

            if model_info is None:
                supported_openai_params = mishikallm.get_supported_openai_params(
                    model=mishikallm_model, custom_llm_provider=llm_provider
                )
                ...
                model_info = ModelMapInfo(
                    ...
                    mishikallm_provider=llm_provider,
                    mode="chat",
    ...
    async def get_model_group_usage(
        ...
        for model in model_list:
            ...
            mishikallm_model: Optional[str] = model["mishikallm_params"].get(
                "model"
            )  # USE THE MODEL SENT TO mishikallm.completion() - consistent with how global_router cache is written.
            if id is None or mishikallm_model is None:
                continue
            tpm_keys.append(
        ...
        if client_type == "max_parallel_requests":
            ...
            if client is None:
                InitalizeCachedClient.set_max_parallel_requests_client(
                    mishikallm_router_instance=self, model=deployment
                )
                client = self.cache.get_cache(
                    key=cache_key, local_only=True, parent_otel_span=parent_otel_span
                )
    ...
    def _pre_call_checks(  # noqa: PLR0915
        ...
        - model context window < message length. For azure openai models, requires 'base_model' is set. - https://docs.21t.cc/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models
        ...
        try:
            input_tokens = mishikallm.token_counter(messages=messages)
        except Exception as e:
            verbose_router_logger.error(
                "mishikallm.router.py::_pre_call_checks: failed to count tokens. Returning initial list of deployments. Got - {}".format(
                    str(e)
                )
            )
        ...
        for idx, deployment in enumerate(_returned_deployments):
            ...
            try:
                base_model = deployment.get("model_info", {}).get("base_model", None)
                if base_model is None:
                    base_model = deployment.get("mishikallm_params", {}).get(
                        "base_model", None
                    )
                model_info = self.get_router_model_info(
                    deployment=deployment, received_model_name=model
                )
                model = base_model or deployment.get("mishikallm_params", {}).get(
                    "model", None
                )
            ...
            _mishikallm_params = deployment.get("mishikallm_params", {})
            ...
            if (
                ...
                if (
                    isinstance(_mishikallm_params, dict)
                    and _mishikallm_params.get("rpm", None) is not None
                ):
                    if (
                        isinstance(_mishikallm_params["rpm"], int)
                        and _mishikallm_params["rpm"] <= current_request
                    ):
                        invalid_model_indices.append(idx)
            ...
            if request_kwargs is not None and mishikallm.drop_params is False:
                # get supported params
                model, custom_llm_provider, _, _ = mishikallm.get_llm_provider(
                    model=model, mishikallm_params=MishikaLLM_Params(**_mishikallm_params)
                )
                ...
                if supported_openai_params is None:
                    ...
                    non_default_params = mishikallm.utils.get_non_default_params(
        ...
        if len(invalid_model_indices) == len(_returned_deployments):
            ...
            if _rate_limit_error is True:  # allow generic fallback logic to take place
                ...
                raise mishikallm.ContextWindowExceededError(
                    message="mishikallm._pre_call_checks: Context Window exceeded for given call. No models have context window large enough for this call.\n{}".format(
                        _potential_error_str
                    ),
                    model=model,
                    llm_provider="",
        ...
        if len(_returned_deployments) > 0:
            _returned_deployments = mishikallm.utils._get_order_filtered_deployments(
    ...
    def _get_deployment_by_mishikallm_model(self, model: str) -> List:
        """
        Get the deployment by mishikallm model.
        """
        return [m for m in self.model_list if m["mishikallm_params"]["model"] == model]
    ...
    def _common_checks_available_deployment(
        ...
        """
        # check if aliases set on mishikallm model alias map
        if specific_deployment is True:
            return model, self._get_deployment_by_mishikallm_model(model=model)
        elif model in self.get_model_ids():
            deployment = self.get_deployment(model_id=model)
            if deployment is not None:
                deployment_model = deployment.mishikallm_params.model
                return deployment_model, deployment.model_dump(exclude_none=True)
            raise ValueError(
                f"MishikaLLM Router: Trying to call specific deployment, but Model ID :{model} does not exist in \
        ...
        if model not in self.model_names:
            ...
            if self.default_deployment is not None:
                ...
                updated_deployment["mishikallm_params"]["model"] = model
        ...
        if len(healthy_deployments) == 0:
            ...
            healthy_deployments = self._get_deployment_by_mishikallm_model(model=model)
        ...
        if len(healthy_deployments) == 0:
            raise mishikallm.BadRequestError(
        ...
        if mishikallm.model_alias_map and model in mishikallm.model_alias_map:
            model = mishikallm.model_alias_map[
                model
    ...
    async def async_get_available_deployment(
        ...
        try:
            ...
            if request_kwargs is not None:
                logging_obj = request_kwargs.get("mishikallm_logging_obj", None)
    ...
    def _initialize_alerting(self):
        ...
        mishikallm.logging_callback_manager.add_mishikallm_callback(_slack_alerting_logger)  # type: ignore
        mishikallm.logging_callback_manager.add_mishikallm_success_callback(
            _slack_alerting_logger.response_taking_too_long_callback
        )
        verbose_router_logger.info(
            "\033[94m\nInitialized Alerting for mishikallm.Router\033[0m\n"
        )
    ...
    def set_custom_routing_strategy(
        ...
        """
        Sets get_available_deployment and async_get_available_deployment on an instanced of mishikallm.Router

        Use this to set your custom routing strategy

        Args:
            CustomRoutingStrategy: mishikallm.router.CustomRoutingStrategyBase
        """
