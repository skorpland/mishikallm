# mishikallm/proxy/vector_stores/vector_store_registry.py
import json
from datetime import datetime, timezone
from typing import TYPE_CHECKING, Any, Dict, List, Optional

from mishikallm._logging import verbose_logger
from mishikallm.types.vector_stores import (
    MishikaLLM_ManagedVectorStore,
    MishikaLLM_ManagedVectorStoreListResponse,
    MishikaLLM_VectorStoreConfig,
)
...
class VectorStoreRegistry:
    def __init__(self, vector_stores: List[MishikaLLM_ManagedVectorStore] = []):
        self.vector_stores: List[MishikaLLM_ManagedVectorStore] = vector_stores
        self.vector_store_ids_to_vector_store_map: Dict[
            str, MishikaLLM_ManagedVectorStore
        ] = {}
    ...
    def get_vector_store_to_run(
        self, non_default_params: Dict, tools: Optional[List[Dict]] = None
    ) -> Optional[MishikaLLM_ManagedVectorStore]:
        """
    ...
    def load_vector_stores_from_config(self, vector_stores_config: List[Dict]):
        """
        Loads vector stores from the mishikallm proxy config.yaml
        """
        for vector_store_config in vector_stores_config:
            # cast to VectorStoreConfig
            mishikallm_vector_store_config = MishikaLLM_VectorStoreConfig(
                **vector_store_config
            )
            vector_store_name = mishikallm_vector_store_config.get("vector_store_name")
            vector_store_mishikallm_params: Dict[str, Any] = (
                mishikallm_vector_store_config.get("mishikallm_params") or {}
            )

            vector_store_id = vector_store_mishikallm_params.get("vector_store_id")
            if vector_store_id is None:
                raise ValueError(
                    f"vector_store_id is required for initializing vector store, got vector_store_id={vector_store_id}"
                )
            custom_llm_provider = vector_store_mishikallm_params.get("custom_llm_provider")
            if custom_llm_provider is None:
                raise ValueError(
                    f"custom_llm_provider is required for initializing vector store, got custom_llm_provider={custom_llm_provider}"
                )

            mishikallm_managed_vector_store = MishikaLLM_ManagedVectorStore(
                vector_store_id=vector_store_id,
                custom_llm_provider=custom_llm_provider,
                vector_store_name=vector_store_name,
                vector_store_description=vector_store_mishikallm_params.get(
                    "vector_store_description"
                ),
                vector_store_metadata=vector_store_mishikallm_params.get(
                    "vector_store_metadata"
                ),
                created_at=datetime.now(timezone.utc),
                updated_at=datetime.now(timezone.utc),
            )
            self.vector_stores.append(mishikallm_managed_vector_store)
    ...
    def list_all_vector_stores(self) -> MishikaLLM_ManagedVectorStoreListResponse:
        """
        List all vector stores in the required format

        Returns:
            MishikaLLM_ManagedVectorStoreListResponse: A standardized response with vector store data
        """
        # Prepare the response
        response = MishikaLLM_ManagedVectorStoreListResponse(
            object="list",
    ...
    def add_vector_store_to_registry(self, vector_store: MishikaLLM_ManagedVectorStore):
        """
    ...
    ) -> List[MishikaLLM_ManagedVectorStore]:
        """
        Get vector stores from the database
        """
        vector_stores_from_db: List[MishikaLLM_ManagedVectorStore] = []
        if prisma_client is not None:
            _vector_stores_from_db = (
                await prisma_client.db.mishikallm_managedvectorstorestable.find_many(
                    order={"created_at": "desc"},
                )
            )
            for vector_store in _vector_stores_from_db:
                _dict_vector_store = dict(vector_store)
                _mishikallm_managed_vector_store = MishikaLLM_ManagedVectorStore(
                    **_dict_vector_store
                )
                vector_stores_from_db.append(_mishikallm_managed_vector_store)
